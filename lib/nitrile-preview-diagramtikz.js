...
class NitrilePreviewDiagramTikz extends NitrilePreviewDiagram {
  ...
  do_finalize(s,style) {
    ...
    if(0){
      ...
      if(style && style.width && style.aspectratio){
        let w = this.translator.string_to_latex_width(style.width,zoom);
        let h = this.translator.string_to_latex_height_with_aspect_ratio(style.width,style.aspectratio,zoom);
        text = `\\resizebox{${w}}{${h}}{${text}}`;
  ...
  to_shadeonlys(g) {
    let d = [];
    var shading = this.to_shade_str(g);
    if(shading=='axis'){
      var s1 = this.to_topcolor_str(g);
      var s2 = this.to_botcolor_str(g);
      var s3 = this.to_midcolor_str(g);
      var angle = this.to_angle_str(g);
      if (s1) {
        d.push(`top color=${s1}`);
      }
      if (s2) {
        d.push(`bottom color=${s2}`);
      }
      if (s3) {
        d.push(`middle color=${s3}`);
      }
      d.push(`shading angle=${angle}`);
      d.push(`shading=axis`)
    }else if(shading=='radial'){
      var s1 = this.to_innercolor_str(g);
      var s2 = this.to_outercolor_str(g);
      if (s1) {
        d.push(`inner color=${s1}`);
      }
      if (s2) {
        d.push(`outer color=${s2}`);
      }
      d.push(`shading=radial`)
    }else if(shading=='ball'){
      var s1 = this.to_ballcolor_str(g);
      if (s1) {
        d.push(`ball color=${s1}`);
      }
    }
  ...
  to_topcolor_str(g) {
    if (g.topcolor) {
      return (`${this.to_colors(g.topcolor)}`);
    }
    if (this.config.topcolor) {
      return (`${this.to_colors(this.config.topcolor)}`);
    }
    return '';
  }
  to_botcolor_str(g) {
    if (g.botcolor) {
      return (`${this.to_colors(g.botcolor)}`);
    }
    if (this.config.botcolor) {
      return (`${this.to_colors(this.config.botcolor)}`);
    }
    return '';
  }
  to_midcolor_str(g) {
    if (g.midcolor) {
      return (`${this.to_colors(g.midcolor)}`);
    }
    if (this.config.midcolor) {
      return (`${this.to_colors(this.config.midcolor)}`);
    }
    return '';
  }
  to_ballcolor_str(g) {
    if (g.ballcolor) {
      return (`${this.to_colors(g.ballcolor)}`);
    }
    if (this.config.ballcolor) {
      return (`${this.to_colors(this.config.ballcolor)}`);
    }
    return '';
  }
  to_innercolor_str(g) {
    if (g.innercolor) {
      return (`${this.to_colors(g.innercolor)}`);
    }
    if (this.config.innercolor) {
      return (`${this.to_colors(this.config.innercolor)}`);
    }
    return '';
  }
  to_outercolor_str(g) {
    if (g.outercolor) {
      return (`${this.to_colors(g.outercolor)}`);
    }
    if (this.config.outercolor) {
      return (`${this.to_colors(this.config.outercolor)}`);
    }
    return '';
  }
  to_ballcolor_str(g) {
