...
class NitrilePreviewDiagramTikz extends NitrilePreviewDiagram {
  ...
  coordsToDraw(coords) {
    ///***NOTE: returns a list of items, where each item is three members: {iscycled,d,attrflag}
  ...
  has_fills(g) {
    var s = this.to_fillcolor_string(g);
    return (s && s !== 'none') ? true : false;
  }
  has_strokes(g) {
    var s = this.to_linecolor_string(g);
    return (s && s !== 'none') ? true : false;
  }
  has_texts(g) {
    var s = this.to_fontcolor_string(g);
    return (s && s !== 'none') ? true : false;
  }
