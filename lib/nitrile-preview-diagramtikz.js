...
class NitrilePreviewDiagramTikz extends NitrilePreviewDiagram {
  ...
  do_finalize(s,style) {
    ...
    if(0){
      ...
      var text = d.join('\n');
      ///width and/or height
      let width = this.translator.string_to_latex_width(style.width,style.zoom);
      let height = this.translator.string_to_latex_height(style.height,style.zoom);
      if(width && height){
    ...
    /// to add a frame we can use \\fbox{...}
    /// If you want to make the framed box (\fbox) exactly around the tikzpicture set the length \fboxsep to 0pt
    if(style.frame==1){
