...
class NitrilePreviewDiagramTikz extends NitrilePreviewDiagram {
  ...
  p_rrect(x,y,w,h,g){
    ...
    var mypath = `(${this.fix(x*unit)}mm,${this.fix((y+Dy)*unit)}mm)\
                 ..controls(${this.fix(x*unit)}mm,${this.fix(y*unit)}mm)and(${this.fix(x*unit)}mm,${this.fix(y*unit)}mm)..(${this.fix((x+Dx)*unit)}mm,${this.fix(y*unit)}mm)--(${this.fix((x+w-Dx)*unit)}mm,${this.fix(y*unit)}mm)\
                 ..controls(${this.fix((x+w)*unit)}mm,${this.fix(y*unit)}mm)and(${this.fix((x+w)*unit)}mm,${this.fix(y*unit)}mm)..(${this.fix((x+w)*unit)}mm,${this.fix((y+Dy)*unit)}mm)--(${this.fix((x+w)*unit)}mm,${this.fix((y+h-Dy)*unit)}mm)\
                 ..controls(${this.fix((x+w)*unit)}mm,${this.fix((y+h)*unit)}mm)and(${this.fix((x+w)*unit)}mm,${this.fix((y+h)*unit)}mm)..(${this.fix((x+w-Dx)*unit)}mm,${this.fix((y+h)*unit)}mm)--(${this.fix((x+Dx)*unit)}mm,${this.fix((y+h)*unit)}mm)\
                 ..controls(${this.fix(x*unit)}mm,${this.fix((y+h)*unit)}mm)and(${this.fix(x*unit)}mm,${this.fix((y+h)*unit)}mm)..(${this.fix((x)*unit)}mm,${this.fix((y+h-Dy)*unit)}mm)--cycle`;
    if (this.has_fills(g)) {
      d.push(`\\fill[${this.g_to_tikz_fillonly_str(g)}] ${mypath};`);
    }
    if (this.has_strokes(g)) {
  ...
  p_line(x1,y1,x2,y2,g){
    ...
    var arrowhead_str = this.integer_to_tikz_arrowhead_str(arrowhead);
    if(this.has_strokes(g)){
  ...
  p_arc(x1,y1,x2,y2,Rx,Ry,bigarcflag,g){
    ...
    if (Number.isFinite(Cx) && Number.isFinite(Cy)) {        
      ...
      if(this.has_strokes(g)){
        if (sweepflag) {//clockwise
          d.push(`\\draw[${this.g_to_tikz_drawonly_str(g)}${arrowhead_str}] (${x0*unit}mm,${y0*unit}mm) [rotate around={${this.fix(Phi)}:(${this.fix(x0*unit)}mm,${this.fix(y0*unit)}mm)}] arc [start angle=${ang2},end angle=${ang1},x radius=${this.fix(Rx*unit)}mm,y radius=${this.fix(Ry*unit)}mm][reset cm];`);
        } else {//counter-clockwise
          d.push(`\\draw[${this.g_to_tikz_drawonly_str(g)}${arrowhead_str}] (${x0*unit}mm,${y0*unit}mm) [rotate around={${this.fix(Phi)}:(${this.fix(x0*unit)}mm,${this.fix(y0*unit)}mm)}] arc [start angle=${ang1},end angle=${ang2},x radius=${this.fix(Rx*unit)}mm,y radius=${this.fix(Ry*unit)}mm][reset cm];`);
        }
      }
    }
    return d.join('\n')
  ...
  p_dot_square(x,y,g){
    ...
    r *= 0.35;///pt to mm
    if(this.has_dots(g)){
      return `\\fill[${this.g_to_tikz_dotonly_str(g)}] (${x * unit - r}mm,${y * unit - r}mm) rectangle (${x*unit+r}mm,${y*unit+r}mm);`;
    }
  ...
  p_dot_circle(x,y,g){
    ...
    ///   because there is no path before it
    if(this.has_dots(g)){
      return `\\fill[${this.g_to_tikz_dotonly_str(g)}] (${x * unit}mm,${y * unit}mm) circle [radius=${this.g_to_dotsize_float(g)*0.5}pt];`;
    }
    //return (`fill fullcircle scaled(${this.g_to_dotsize_float(g)}) shifted(${x}*u,${y}*u) ${this.to_dotcolors(g)};`);
  }
  ...
  txt_to_label_normal_text(s,g) {
    ...
    var fontstyle = this.g_to_latex_fontstyle_command(g);
  ...
  integer_to_tikz_arrowhead_str(arrowhead){
    if(arrowhead==1){
      var arrowhead_str = ',->';
    }else if(arrowhead==2){
      var arrowhead_str = ',<-';
