...
class NitrilePreviewDiagramTikz extends NitrilePreviewDiagram {
  ...
  p_text(x,y,txt,ts,ta,g){
    ...
    */
    ...
    y += dy;
    let fs = this.to_fontsize_str(g);
    let tabular_align_letter = this.string_to_tabular_align_letter(ta);
    var tex_label = this.to_tex_text(txt,tabular_align_letter);
    return `\\draw (${this.fix(x*unit)}mm,${this.fix(y*unit)}mm) node[${d.join(',')}] {\\fontsize{${fs}pt}{${fs}pt}\\selectfont{}${tex_label}};`;
  ...
  to_tex_text(txt,tabular_align_letter) {
    txt = txt || '';
    ...
    let d = [];
    d.push(`\\begin{tabular}{@{}${tabular_align_letter}@{}}`)
    ss.forEach(s => {
      d.push(`${s} \\\\`);
    })
    d.push(`\\end{tabular}`)
