...
class NitrilePreviewDiagramTikz extends NitrilePreviewDiagram {
  ...
  coords_to_draw(coords) {
    ...
    for (var i in coords) {
      ...
      join = join || '';
      if (d.length==0) {
        d.push(`(${this.fix(x*u)}mm,${this.fix(y*u)}mm)`);
        x0 = x;
        y0 = y;
        hints = pt[12]||hints;
        continue;
        ...
        d.push(`${bezier}(${this.fix(x*u)}mm,${this.fix(y*u)}mm)`);
        x0 = x;
        y0 = y;
        hints = pt[12]||hints;
      }
        ...
        if (Number.isFinite(Cx) && Number.isFinite(Cy)) {        
          ...
          if (sweepflag) {//clockwise
            d.push(`[rotate around={${this.fix(Phi)}:(${this.fix(x0*u)}mm,${this.fix(y0*u)}mm)}] arc [start angle=${ang2},end angle=${ang1},x radius=${this.fix(Rx*u)}mm,y radius=${this.fix(Ry*u)}mm][reset cm]`);
          } else {//counter-clockwise
            d.push(`[rotate around={${this.fix(Phi)}:(${this.fix(x0*u)}mm,${this.fix(y0*u)}mm)}] arc [start angle=${ang1},end angle=${ang2},x radius=${this.fix(Rx*u)}mm,y radius=${this.fix(Ry*u)}mm][reset cm]`);
          }
        }
        x0 = x;
        y0 = y;
  ...
  p_fill(coords,_g){
    ...
    for(var item of items) {
      ...
      if(this.has_shades(g)){
        d.push(`\\shade[${this.to_shadeonlys(g,1)}] ${s};`);
      }else if(this.has_fills(g)){
        d.push(`\\fill[${this.to_fillonlys(g,1)}] ${s};`);
      }
    }
    return d.join('\n');
  }
  p_draw(coords,_g){
    ...
    for(var item of items) {
      ...
      if(iscycled){
        if(this.has_shades(g)){
          d.push(`\\shade[${this.to_shadeonlys(g)}] ${s};`);  
        }else if(this.has_fills(g)){
          d.push(`\\fill[${this.to_fillonlys(g,1)}] ${s};`);
        }
      }
      if (this.has_strokes(g)) {
        d.push(`\\draw[${this.to_draws(g)}] ${s};`);
      }
    }
    return d.join('\n');
  }
  ...
  p_hexgon(x,y,w,h,g){
    let u = this.viewport_unit;
    let gap = h*0.25*u;
    var mypath = `(${this.fix(x*u+gap)}mm,${this.fix(y*u)}mm)--(${this.fix((x+w)*u-gap)}mm,${this.fix(y*u)}mm)--(${this.fix((x+w)*u)}mm,${this.fix((y+0.5*h)*u)}mm)--(${this.fix((x+w)*u-gap)}mm,${this.fix((y+h)*u)}mm)--(${this.fix(x*u+gap)}mm,${this.fix((y+h)*u)}mm)--(${this.fix(x*u)}mm,${this.fix((y+0.5*h)*u)}mm)--cycle`;
    var o = [];
    if (this.has_fills(g)) {
      o.push(`\\fill[${this.to_fills(g)}] ${mypath};`);
    }
  ...
  p_triangle(x,y,w,h,g){
    let u = this.viewport_unit;
    var mypath = `(${this.fix(x*u)}mm,${this.fix(y*u)}mm)--(${this.fix((x+w)*u)}mm,${this.fix(y*u)}mm)--(${this.fix((x+0.5*w)*u)}mm,${this.fix((y+h)*u)}mm)--cycle`;
    var o = [];
    if (this.has_fills(g)) {
      o.push(`\\fill[${this.to_fills(g)}] ${mypath};`);
    }
  ...
  p_rhbar(x,y,g){
    ...
    var mypath = `(${x*u}mm,${y*u}mm)--(${(x+delta_x)*u}mm,${y*u}mm)`;
    if(this.has_strokes(g)){
      o.push(`\\draw[${this.to_draws(g)}] ${mypath};`);
    }
    return o.join('\n');
  ...
  p_label(x,y,txt,ta,g){
    ...
    if(fontcolor){
      d.push(`text=${this.string_to_tikz_color(fontcolor,g.user)}`)
    }
    let math = this.to_math_int(g);
    if(math){
      var tex_label = this.to_label_math_text(txt,g);
    }else{
      var tex_label = this.to_label_normal_text(txt,g);
    }
    if(this.has_texts(g)){
  ...
  p_text(x,y,txt,ta,g){
    ...
    */
    ...
    var tex_label = this.to_text_normal_text(txt,g);
    if(this.has_texts(g)){
  ...
  p_dot_square(x,y,g){
    ...
    let r2 = this.to_dotsize_float(g);
    let r = r2*0.5;
    r2 *= 0.35;///pt to mm
    r *= 0.35;///pt to mm
    if(this.has_dots(g)){
    ...
    //return (`fill fullcircle scaled(${this.to_dotsize_float(g)}) shifted(${x}*u,${y}*u) ${this.to_dotcolors(g)};`);
  }
  p_dot_circle(x,y,g){
    ///reject points that are not within the viewport
    if (x < 0 || x > this.viewport_width) {
    ...
    ///   because there is no path before it
    if(this.has_dots(g)){
      return `\\fill[${this.to_dotcolors(g)}] (${x * u}mm,${y * u}mm) circle [radius=${this.to_dotsize_float(g)*0.5}pt];`;
    }
    //return (`fill fullcircle scaled(${this.to_dotsize_float(g)}) shifted(${x}*u,${y}*u) ${this.to_dotcolors(g)};`);
  }
  ...
  to_draws(g) {
    ...
    if (linecolor) {
      o.push(`color=${this.string_to_tikz_color(linecolor,g.user)}`);
    }
    var linecap = this.to_linecap_string(g);
    if (linecap) {
      o.push(`line cap=${this.string_to_tikz_linecap(linecap)}`)
    } 
    var linejoin = this.to_linejoin_string(g);
    if (linejoin) {
      o.push(`line join=${this.string_to_tikz_linejoin(linejoin)}`)
    }
    return o.join(',');
  }
  to_fills(g,isinherit) {
    let d = [];
    var fillcolor = this.to_fillcolor_string(g);
    var opacity = this.to_opacity_float(g);
    if (fillcolor) {
      d.push(`fill=${this.string_to_tikz_color(fillcolor,g.user)}`);
    } else if (isinherit) {
      d.push('fill=black');
    }
    if(opacity < 1) {
  ...
  to_shadeonlys(g) {
    ...
    var ss = this.to_tikz_shadecolor_ss(g);
    if(shade=='linear'){
      var angle = this.to_angle_float(g);
      if(ss.length==0){
  ...
  to_fillonlys(g,isinherit) {
    ...
    var has_fill = 0;
    if (fillcolor) {
      let tikz_color = `${this.string_to_tikz_color(fillcolor,g.user)}`;
      if(tikz_color){
        d.push(`fill=${tikz_color}`)
        has_fill = 1;
      }
    } else if (isinherit) {
      d.push('fill=black');
  ...
  string_to_tikz_color(s,user) {
    ...
    if (s === 'none'){
      ...
      s = s.slice(1);
      s = this.webrgb_to_definecolor(s);
      return s;
    } 
    else {
      return 'black';
  ...
  to_label_math_text(s,g) {
    s = s || '';
    var fs = this.string_to_latex_fontsize(g.fontsize)
  ...
  to_label_normal_text(s,g) {
    s = s || '';
    var fs = this.string_to_latex_fontsize(g.fontsize)
    var fontfamily = this.string_to_latex_fontfamily(g.fontfamily);
    var fontstyle = this.string_to_latex_fontstyle(g.fontfamily);
  ...
  to_dotcolors(g) {
    ...
    if (dotcolor) {
      return (`color=${this.string_to_tikz_color(dotcolor,g.user)}`);
    }
    return '';
  }
