...
class NitrilePreviewDiagramTikz extends NitrilePreviewDiagram {
  ...
  p_label(x,y,txt,ts,ta,g){
    ...
    if(fontcolor){
      d.push(`text=${this.string_to_tikz_color(fontcolor)}`)
    }
  ...
  to_draws(g) {
    ...
    if (linecolor) {
      o.push(`color=${this.string_to_tikz_color(linecolor)}`);
    }
  ...
  to_fills(g,isinherit) {
    ...
    if (fillcolor) {
      d.push(`fill=${this.string_to_tikz_color(fillcolor)}`);
    } else if (isinherit) {
      d.push('fill=black');
    }
    if(opacity < 1) {
  ...
  to_fillonlys(g,isinherit) {
    ...
    var has_fill = 0;
    if (fillcolor) {
      let tikz_color = `${this.string_to_tikz_color(fillcolor)}`;
      if(tikz_color){
  ...
  string_to_tikz_color(s) {
    ...
    if (s === 'none'){
      ...
      s = s.slice(1);///getrid of the first #
  ...
  to_tikz_shadecolor_ss(g) {
    ...
    ss = ss.map(s => this.string_to_tikz_color(s));
    return ss;
  }
  ...
  to_dotcolors(g) {
    ...
    if (dotcolor) {
      return (`color=${this.string_to_tikz_color(dotcolor)}`);
    }
    return '';
  }
