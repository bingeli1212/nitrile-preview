...
class NitrilePreviewDiagramTikz extends NitrilePreviewDiagram {
  ...
  p_TRIANGLE(x,y,w,h,g){
    ...
    h = this.scale_dist(h);
    let unit = this.viewport_unit;
    var mypath = `(${this.fix(x*unit)}mm,${this.fix((y+h)*unit)}mm)--(${this.fix((x+0.5*w)*unit)}mm,${this.fix(y*unit)}mm)--(${this.fix((x+w)*unit)}mm,${this.fix((y+h)*unit)}mm)--cycle`;
    if (this.has_fills(g)) {
      d.push(`\\fill[${this.g_to_tikz_fillonly_str(g)}] ${mypath};`);
    }
    if (this.has_strokes(g)) {
  ...
  txt_to_label_math_text(s,g) {
    s = s || '';
    var s = this.translator.phrase_to_inlinemath(g,s);
    return s;
  }
  ...
  to_framed(style,ss){
    ...
    var y = vh;
    ss.forEach((s,i) => {
      s = this.translator.polish(s,style);
      s = s.replace(/\s/g,"~");
      y -= fs;
