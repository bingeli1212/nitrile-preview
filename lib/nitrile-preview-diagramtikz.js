...
class NitrilePreviewDiagramTikz extends NitrilePreviewDiagram {
  ...
  p_fillclipath(_coords,g){
    ...
    names.forEach(x => {
      if (this.my_paths.has(x)) {
        ...
        this.offset_coords(path, this.refx, this.refy, this.refsx,this.refsy,0);
  ...
  p_circle(x,y,radius,g){
    ...
    if (this.has_fills(g)) {
      d.push(`\\fill[${this.g_to_tikz_fillonly_str(g)}] (${this.fix(x*unit)}mm,${this.fix(y*unit)}mm) circle [radius=${this.fix(radius*unit)}mm];`);
    }
    if (this.has_strokes(g)) {
      d.push(`\\draw[${this.g_to_tikz_drawonly_str(g)}] (${this.fix(x*unit)}mm,${this.fix(y*unit)}mm) circle [radius=${this.fix(radius*unit)}mm];`);
    }
    return d.join('\n');
  }
  p_rect(x,y,w,h,g){
