...
class NitrilePreviewDiagramTikz extends NitrilePreviewDiagram {
  ...
  p_polygon(points,g){
    ...
    var s = d.join('--');
    if(this.has_shades(g)){
      text=(`\\shade[${this.g_to_tikz_shadeonly_str(g)}] ${s};`);
    }else if(this.has_fills(g)){
      text=(`\\fill[${this.g_to_tikz_fillonly_str(g)}] ${s};`);
    }
    if(this.has_strokes(g)){
      text=(`\\draw[${this.g_to_tikz_drawonly_str(g)}] ${s};`);
    }
  ...
  to_ink(style,ss){
    ...
    var fs = 12;
    var textcolor = '';
    d.push(`\\draw[color=white] (0,0) rectangle (${vw}pt,${vh}pt);`);
    var x = 0;
    var y = vh;
    ss.forEach((s,i) => {
      ...
      s = s.replace(/\s/g,"~");
      y -= fs;
      d.push(`\\draw (0pt,${y}pt) node[anchor=${anchor}] {\\ttfamily\\fontsize{${fs}pt}{${fs}pt}\\selectfont{}${textcolor}${s}};`);
    });
    var s = d.join('\n');
    let all = [];
    all.push(`\\begin{tikzpicture}`);
    ...
    if(stretch){
      ...
      img = `\\resizebox{${vw/vh*height}mm}{${height}mm}{${img}}`;
    }
    var img = `\\mbox{${img}}`;
    if(style.frame){
      img = `\\frame{${img}}`;
    ...
    if(style.float){
      ...
      img = `\\hfill\\break${img}\\hfill\\break{}`;
    }
    var subc = '';
