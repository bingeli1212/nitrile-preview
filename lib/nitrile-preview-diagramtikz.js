...
class NitrilePreviewDiagramTikz extends NitrilePreviewDiagram {
  ...
  do_one_frame(style,ss,env) {
    ...
    if(this.imgs.length){
      ...
      let src = this.translator.choose_image_file(srcs);
      let width = xm*unit;
      let height = ym*unit;
      if(fit=='contain'){
        all.push(`\\node (image) at (${width/2}mm,${height/2}mm) { \\includegraphics[width=${width}mm,height=${height}mm,keepaspectratio]{${src}} } ;`);
      }else if(fit=='cover'){
        //currently 'cover' is unsupported on LATEX because of lack of capability offered by includegrahics command
        all.push(`\\node (image) at (${width/2}mm,${height/2}mm) { \\includegraphics[width=${width}mm,height=${height}mm,keepaspectratio]{${src}} } ;`);
      }else{
        //default is 'fill'
        all.push(`\\node (image) at (${width/2}mm,${height/2}mm) { \\includegraphics[width=${width}mm,height=${height}mm]{${src}} } ;`);
      }
    }
  ...
  p_slopedmath(x1,y1,x2,y2,txt,ta,g){
    ...
    */
    ...
    var fontsize = this.g_to_fontsize_float();
    if(!fontsize){
      fontsize = this.fontsize;
    }
    var fontcolor = this.g_to_fontcolor_string(g);
