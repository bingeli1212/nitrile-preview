...
class NitrilePreviewDiagramTikz extends NitrilePreviewDiagram {
  ...
  do_finalize(s,style) {
    ...
    if(this.iscontext){
      ...
      if(style && style.width && style.aspectratio){
        let w = this.translator.string_to_latex_width(style.width);
        let h = this.translator.string_to_latex_height_with_aspect_ratio(style.width,style.aspectratio);
  ...
  p_shape(x,y,p,g){
    ...
    for (var item of items) {
      ...
      if (!d) continue;
      if (iscycled && this.has_fills(g)) {
        o.push(`\\fill[${this.to_fills(g)}] ${d} [reset cm];`);
      }
