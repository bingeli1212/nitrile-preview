...
class NitrilePreviewDiagramTikz extends NitrilePreviewDiagram {
  ...
  p_label(x,y,txt,ta,g){
    ...
    if(math){
      var tex_label = this.string_to_label_math_text(txt,g);
    }else{
      var tex_label = this.string_to_label_normal_text(txt,g);
    }
    if(this.has_texts(g)){
  ...
  to_draws(g) {
    ...
    if (linecolor) {
      o.push(`color=${this.string_to_tikz_color(linecolor,g.hints)}`);
    }
  ...
  to_fillonlys(g,isinherit) {
    ...
    var has_fill = 0;
    if (fillcolor) {
      let tikz_color = `${this.string_to_tikz_color(fillcolor,g.hints)}`;
      if(tikz_color){
  ...
  string_to_label_normal_text(s,g) {
    s = s || '';
    var fs = this.string_to_latex_fontsize(g.fontsize)
  ...
  to_dotcolors(g) {
    ...
    if (dotcolor) {
      return (`color=${this.string_to_tikz_color(dotcolor,g.hints)}`);
    }
    return '';
  }
