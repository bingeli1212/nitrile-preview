...
class NitrilePreviewDiagramTikz extends NitrilePreviewDiagram {
  ...
  p_circle(x,y,radius,g){
    ...
    var o = [];
    if (this.has_fills(g)) {
      o.push(`\\fill[${this.g_to_tikz_fillonly_str(g)}] (${x*u}mm,${y*u}mm) circle [radius=${this.fix(radius*u)}mm];`);
    }
    if (this.has_stroke(g)) {
      o.push(`\\draw[${this.g_to_tikz_drawonly_str(g)}] (${x*u}mm,${y*u}mm) circle [radius=${this.fix(radius*u)}mm];`);
    }
    return o.join('\n');
  }
  p_rect(x,y,w,h,g){
    ...
    var o = [];
    if (this.has_fills(g)) {
      o.push(`\\fill[${this.g_to_tikz_fillonly_str(g)}] ${mypath};`);
    }
    if (this.has_strokes(g)) {
  ...
  p_qbezier_line(x0,y0, x1,y1, x2,y2, g){
    ...
    if(this.has_strokes(g)){
      if(g.arrowhead==1){
        o.push(`\\draw[${this.g_to_tikz_drawonly_str(g)},->] ${path};`);
      }else if(g.arrowhead==2){
        o.push(`\\draw[${this.g_to_tikz_drawonly_str(g)},<-] ${path};`);
      }else if(g.arrowhead==3){
        o.push(`\\draw[${this.g_to_tikz_drawonly_str(g)},<->] ${path};`);
  ...
  p_ellipse(x,y,Rx,Ry,angle,g){
    ...
    var o = [];
    if (this.has_fills(g)) {
      o.push(`\\fill[${this.g_to_tikz_fillonly_str(g)}] (${this.fix(x * u)}mm,${this.fix(y * u)}mm) circle [x radius=${this.fix(Rx * u)}mm,y radius=${this.fix(Ry * u)}mm, rotate=${angle}];`);
    }
    if (this.has_strokes(g)) {
      o.push(`\\draw[${this.g_to_tikz_drawonly_str(g)}] (${this.fix(x * u)}mm,${this.fix(y * u)}mm) circle [x radius=${this.fix(Rx * u)}mm,y radius=${this.fix(Ry * u)}mm, rotate=${angle}];`);
    }
    return o.join('\n');
  }
  p_dot_square(x,y,g){
    ...
    r *= 0.35;///pt to mm
    if(this.has_dots(g)){
      return `\\fill[${this.g_to_tikz_dotonly_str(g)}] (${x * u - r}mm,${y * u - r}mm) rectangle (${x*u+r}mm,${y*u+r}mm);`;
    }
  ...
  p_dot_circle(x,y,g){
    ...
    ///   because there is no path before it
    if(this.has_dots(g)){
      return `\\fill[${this.g_to_tikz_dotonly_str(g)}] (${x * u}mm,${y * u}mm) circle [radius=${this.g_to_dotsize_float(g)*0.5}pt];`;
    }
    //return (`fill fullcircle scaled(${this.g_to_dotsize_float(g)}) shifted(${x}*u,${y}*u) ${this.to_dotcolors(g)};`);
  }
  p_arc(x1,y1,x2,y2,Rx,Ry,bigarcflag,g){
    ...
    if (Number.isFinite(Cx) && Number.isFinite(Cy)) {        
      ...
      if(this.has_strokes(g)){
        if (sweepflag) {//clockwise
          o.push(`\\draw[${this.g_to_tikz_drawonly_str(g)}] (${x0*u}mm,${y0*u}mm) [rotate around={${this.fix(Phi)}:(${this.fix(x0*u)}mm,${this.fix(y0*u)}mm)}] arc [start angle=${ang2},end angle=${ang1},x radius=${this.fix(Rx*u)}mm,y radius=${this.fix(Ry*u)}mm][reset cm];`);
        } else {//counter-clockwise
          o.push(`\\draw[${this.g_to_tikz_drawonly_str(g)}] (${x0*u}mm,${y0*u}mm) [rotate around={${this.fix(Phi)}:(${this.fix(x0*u)}mm,${this.fix(y0*u)}mm)}] arc [start angle=${ang1},end angle=${ang2},x radius=${this.fix(Rx*u)}mm,y radius=${this.fix(Ry*u)}mm][reset cm];`);
        }
      }
    }
    return o.join('\n')
