...
class NitrilePreviewDiagramTikz extends NitrilePreviewDiagram {
  ...
  do_finalize(s,style) {
    ...
    if(this.config.gridlines=='none'){
      ...
      all.push(`\\draw[help lines, step=${unit}mm,color=${gridcolor},ultra thin] (0,0) grid (${xm*unit}mm,${ym*unit}mm);`);
    }
    all.push(s);
    ...
    if(style.stretch){
      text = `\\resizebox{${style.stretch}\\linewidth}{!}{${text}}`;
    }else if(style.width && style.height){
      text = `\\resizebox{${style.width}mm}{${style.height}mm}{${text}}`;
    }else if(style.width){
      text = `\\resizebox{${style.width}mm}{!}{${text}}`;
    }else if(style.height){
      text = `\\resizebox{!}{${style.height}mm}{${text}}`;
    }
    ///set the outline
    if(style.frame==1){
  ...
  p_circle(x,y,r,g){
    ...
    if (this.has_shades(g)) {
      d.push(`\\fill[${this.g_to_tikz_shadeonly_str(g)}] (${this.fix(x*unit)}mm,${this.fix(y*unit)}mm) circle [radius=${this.fix(r*unit)}mm];`);
    }else if (this.has_fills(g)) {
      d.push(`\\fill[${this.g_to_tikz_fillonly_str(g)}] (${this.fix(x*unit)}mm,${this.fix(y*unit)}mm) circle [radius=${this.fix(r*unit)}mm];`);
    }
    if (this.has_strokes(g)) {
      d.push(`\\draw[${this.g_to_tikz_drawonly_str(g)}] (${this.fix(x*unit)}mm,${this.fix(y*unit)}mm) circle [radius=${this.fix(r*unit)}mm];`);
    }
    return d.join('\n');
  }
  ///
  ...
  p_arc(cx,cy,r,start_a,span_a,g){
    ...
    if (Number.isFinite(Cx) && Number.isFinite(Cy)) {        
      ...
      if(this.has_strokes(g)){
        //counter-clockwise
        d.push(`\\draw[${this.g_to_tikz_drawonly_str(g)}${arrowhead_str}] (${this.fix(x0*unit)}mm,${this.fix(y0*unit)}mm) [rotate around={${this.fix(Phi)}:(${this.fix(x0*unit)}mm,${this.fix(y0*unit)}mm)}] arc [start angle=${this.fix(ang1)},end angle=${this.fix(ang2)},x radius=${this.fix(Rx*unit)}mm,y radius=${this.fix(Ry*unit)}mm][reset cm];`);
      }
    }
    return d.join('\n')
  }
  p_pie(cx,cy,r,start_a,span_a,g){
    ...
    if (Number.isFinite(Cx) && Number.isFinite(Cy)) {        
      ...
      var arrowhead_str = this.int_to_tikz_arrowhead_str(arrowhead);
      if(this.has_shades(g)) {
        d.push(`\\fill[${this.g_to_tikz_shadeonly_str(g)}${arrowhead_str}] (${this.fix(cx*unit)}mm,${this.fix(cy*unit)}mm)--(${this.fix(x0*unit)}mm,${this.fix(y0*unit)}mm) [rotate around={${this.fix(Phi)}:(${this.fix(x0*unit)}mm,${this.fix(y0*unit)}mm)}] arc [start angle=${this.fix(ang1)},end angle=${this.fix(ang2)},x radius=${this.fix(Rx*unit)}mm,y radius=${this.fix(Ry*unit)}mm][reset cm];`);
      }else if(this.has_fills(g)){
  ...
  p_ellipse(x,y,Rx,Ry,angle,g){
    ...
    var o = [];
    if (this.has_shades(g)) {
      o.push(`\\fill[${this.g_to_tikz_shadeonly_str(g)}] (${this.fix(x * unit)}mm,${this.fix(y * unit)}mm) circle [x radius=${this.fix(Rx * unit)}mm,y radius=${this.fix(Ry * unit)}mm, rotate=${angle}];`);
    }else if (this.has_fills(g)) {
