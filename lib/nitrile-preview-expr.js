  ...
  extract_next_scalar(s,g,z=0) {
    ...
    if((v=this.re_scalar_dvar.exec(s))!==null){
      ...
      if(g && g.hasOwnProperty('$')){
        let pp = g['$'];//'pp' is an array, and each array element is an object such that the number is its p.text member
        if(pp && Array.isArray(pp) && id>=1 && id<=pp.length){
          num = pp[id-1].text;//hardcoded to be the 'text' member
