...
class NitrilePreviewExpr extends NitrilePreviewBase {
  ...
  constructor(diagram) {
    ...
    this.re_scalar_dvar      = /^(\$)([A-Za-z]?)(~+|\d*)\s*(.*)$/;
  ...
  extract_next_scalar(s,g,z=0) {
    ...
    if((v=this.re_scalar_dvar.exec(s))!==null){
      let CN = this.col_letter_to_col_num(v[2]);
      let RN = v[3];
      s = v[4];
      let num = Complex.NAN;
      ...
      if(g && g.hasOwnProperty('$')){
        let rows = g['$'];//'rows' is an array
        if(RN==''){
          RN=parseInt(g['_']);
        }else if(RN[0]=='~'){
          RN=parseInt(g['_'])-RN.length;
        }else{
          RN=parseInt(RN);
        }
        if(Array.isArray(rows) && typeof RN==='number' && RN>=1 && RN<=rows.length){
          let pp = rows[RN-1];
          if(Array.isArray(pp) && CN>=1 && CN<=pp.length){
            num = pp[CN-1].text;//hardcoded to be the 'text' member
            if(num instanceof Complex){
              //no-op
            }else if(typeof num === 'number'){
              num = Complex.create(p);
  ...
  exec_complex_func(func_name,func_args,g,z=0) {
    ...
    switch(func_name) {
      ...
      case 'log10':
        if(func_args.length==1){
          return func_args[0].log().div(Math.LN10);
        }else{
          return Complex.NAN;
        }
      case 'log2':
        if(func_args.length==1){
          return func_args[0].log().div(Math.LN2);
        }else{
          return Complex.NAN;
        }
      case 'exp':
  ...
  */
  ...
  read_arr(line,env) {
    ...
    while(line.length) {
      ...
      /// '1'  
      /// '1+2' 
      if((v=this.re_is_apo.exec(line))!==null){
        let v1 = v[1]; //'1+2'
        line = v[2];
      ...
      if((v=this.re_is_avr.exec(line))!==null){
        let name = v[1].slice(1);//remove the starting '@'
        line = v[2];
