...
class NitrilePreviewExpr extends NitrilePreviewBase {
  ...
  constructor(diagram) {
    ...
    // scalar function
    this.re_scalar_complex   = /^\$([A-Za-z][A-Za-z0-9]*)\.(re|im)\s*(.*)$/;
  ...
  read_arr(line,env) {
    ...
    while(line.length) {
      ...
      if((v=this.re_is_brackets.exec(line))!==null){ 
        var expr = v[1];
        line = v[2];
        if((v=this.re_is_symbolname.exec(expr))!==null){
          /// '[a]'
          let symbol = v[1];
        ...
        if((v=this.re_is_range_spread.exec(expr))!==null){
          /// '1:10;4'
          let [a1] = this.extract_next_expr(v[1],env);
          let [a2] = this.extract_next_expr(v[2],env);
          let [a3] = this.extract_next_expr(v[3],env);
          if(a1.isFinite()&&a2.isFinite()&&a3.isFinite()){
            let n = Math.abs(Math.round(a3.re));
            if(n==0){
              all.push(a1);
              all.push(a2);
            }else if(n > 0){
              all.push(a1);
              for(let i=0; i < n; ++i){
                let a = a2.sub(a1).mul(i+1).div(n+1).add(a1);
                all.push(a);
              }
              all.push(a2);
            }
