...
class NitrilePreviewExpr extends NitrilePreviewBase {
  ...
  constructor(diagram) {
    ...
    this.re_scalar_string   = /^#([A-Za-z][A-Za-z0-9]*)\s*(.*)$/;
    this.re_scalar_atsign   = /^@\s*(.*)$/;
  ...
  extract_next_scalar(s,g,z=0) {
    ...
    if (this.diagram && (v=this.re_scalar_string.exec(s))!==null){
      /// '#a', '#a0', '#b'
      let phrase = v[0];
      let symbol = v[1];
      s = v[2];
    ...
    /// '@'
    if((v=this.re_scalar_atsign.exec(s))!==null){
      let key = '@';
      s = v[1];
      let num = Complex.NAN;
