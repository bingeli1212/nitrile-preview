...
class NitrilePreviewExpr extends NitrilePreviewBase {
  ...
  constructor(diagram) {
    ...
    this.re_scalar_arraypt  = /^([A-Za-z][A-Za-z0-9]*_\d+)\s*(.*)$/;
  ...
  extract_next_scalar(s,g,z=0) {
    ...
    if (this.diagram && (v=this.re_scalar_arraypt.exec(s))!==null){
      ...
      let symbol = v[1];
      s = v[2];
      let arr = this.diagram.get_array_from_symbol(symbol);
      if(arr.length){
        var a = arr[0];///a complex number
      }else{
        var a = Complex.NAN;
      }
      return [a,s,phrase];
  ...
  exec_float_func(func_name,func_args,g,z=0){
    ...
    var a = this.exec_complex_func(func_name,func_args,g,z);
    return a.re;
  }
