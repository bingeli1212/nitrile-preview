...
class NitrilePreviewDiagramTikz extends NitrilePreviewDiagram {
  ...
  to_diagram(style,ss){
    ...
    var backgroundalpha = this.g_to_backgroundalpha_float(this.env);
    var all = [];
    all.push(`\\begin{tikzpicture}`);
    all.push(`\\clip (0,0) rectangle (${xm*unit}mm,${ym*unit}mm);`);
    if(background=='grid'){  
      all.push(`% <-- background:grid -->`);
      all.push(`\\draw[step=${unit*grid}mm,opacity=${backgroundalpha},thin] (0,0) grid (${xm*unit}mm,${ym*unit}mm);`);
    }else{
      ...
      all.push(`% <-- white background -->`);
      all.push(`\\draw[color={rgb,15:red,15;green,15;blue,15},thin] (0,0) rectangle (${xm*unit}mm,${ym*unit}mm);`);
    }
    var imgsrc = '';
    if(this.my_images.length){
      ...
      all.push(`\\node[] (image) at (${x+(w/2)}mm,${y+(h/2)}mm) { \\includegraphics[width=${w}mm,height=${h}mm]{${imgsrc}} };`);
      all.push(s);
    }else{
      all.push(s);
    }
    ...
    var img = all.join('\n');
    var tikzpicture = all.join('\n');
    var width = this.assert_int(style.width,0);
    var height = this.assert_int(style.height,0);
    var stretch = this.assert_float(style.stretch,0,0,1);
    if(stretch){
      img = `\\resizebox{${this.stretch}\\linewidth}{!}{${img}}`;
    ...
    if(style.frame){
      ...
      img = `\\makebox{${img}}`;///this step is necessary to remove any baseline that is embedded within each tikzpicture because otherwise this picture will try to baseline-align with neighboring text such that it rises above it
    }
    var output = { img, imgsrc, tikzpicture, style, width, height, stretch, inkwidth, inkheight };
    return(output);
  }
  ///
  ///
  ...
  p_header(opt,x,y,w,h,txt,g){
    ...
    if(is_math){
      ...
      var tex_label = this.translator.smooth(g,txt);
    }
    if(opt=='l'){
      var anchor = this.string_to_tikz_anchor('br');
      if(anchor){
        d.push(`anchor=${anchor}`);
      }
      return `\\draw[] (${this.fix(0*unit)}mm,${this.fix(h*unit)}mm) node[${d.join(',')}] {${tex_label}};`;
    }else if(opt=='r'){
      var anchor = this.string_to_tikz_anchor('bl');
      if(anchor){
        d.push(`anchor=${anchor}`);
      }
      return `\\draw[] (${this.fix(w*unit)}mm,${this.fix(h*unit)}mm) node[${d.join(',')}] {${tex_label}};`;
    }else{
      var anchor = this.string_to_tikz_anchor('b');
      if(anchor){
        d.push(`anchor=${anchor}`);
      }
      return `\\draw[] (${this.fix(w/2*unit)}mm,${this.fix(h*unit)}mm) node[${d.join(',')}] {${tex_label}};`;
    }
  }
  p_footer(opt,x,y,w,h,txt,g){
    ...
    if(opt=='l'){
      var anchor = this.string_to_tikz_anchor('ur');
      if(anchor){
        d.push(`anchor=${anchor}`);
      }
      return `\\draw[] (${this.fix(0*unit)}mm,${this.fix(0*unit)}mm) node[${d.join(',')}] {${tex_label}};`;
    }else if(opt=='r'){
      var anchor = this.string_to_tikz_anchor('ul');
      if(anchor){
        d.push(`anchor=${anchor}`);
      }
      return `\\draw[] (${this.fix(w*unit)}mm,${this.fix(0*unit)}mm) node[${d.join(',')}] {${tex_label}};`;
    }else{
      var anchor = this.string_to_tikz_anchor('u');
      if(anchor){
        d.push(`anchor=${anchor}`);
      }
      return `\\draw[] (${this.fix(w/2*unit)}mm,${this.fix(0*unit)}mm) node[${d.join(',')}] {${tex_label}};`;
    }
  }
  p_image(x,y,w,h,ta,imgsrc,g){
    ...
    switch(ta){
      case 'c' :                 break;
      case 'ul': x-=w/2; y+=h/2; break;
      case 'bl': x-=w/2; y-=h/2; break;
      case 'ur' : x+=w/2; y+=h/2; break;
      case 'br' : x+=w/2; y-=h/2; break;
      case 'u' :         y+=h/2; break;
      case 'b' :         y-=h/2; break;
      case 'l' : x-=w/2;         break;
      case 'r'  : x+=w/2;         break;
    ...
    var img = '';
    if(this.g_to_fit_string(g)=='contain' && this.g_to_fit_string(g)=='cover'){
      img = (`(image) at (${x+(w/2)}mm,${y+(h/2)}mm) { \\includegraphics[width=${w}mm,height=${h}mm,keepaspectratio]{${imgsrc}} } `);//position is at center of viewport
    }else{
      //default is 'fill'
      img = (`(image) at (${x+(w/2)}mm,${y+(h/2)}mm) { \\includegraphics[width=${w}mm,height=${h}mm]{${imgsrc}} } `);//position is at center of viewport
    }
    if(imgsrc){
      //var canvas = `transform canvas={xshift=${this.origin_x*this.viewport_unit}mm,yshift=${this.origin_y*this.viewport_unit}mm,xscale=${this.origin_sx},yscale=${this.origin_sy}}`;
      var canvas = '';//p_image is not subject to origin transformation
      return `\\node[${canvas}] ${img};`;///by default without anchor= it is centered 
    }
    return '';
  }
  ...
  p_grid(density,g){
    if(this.has_strokes(g)){
      var canvas = `transform canvas={xshift=${this.origin_x*this.viewport_unit}mm,yshift=${this.origin_y*this.viewport_unit}mm,xscale=${this.origin_sx},yscale=${this.origin_sy}}`;
      return `\\draw[${canvas},${this.g_to_tikz_drawonly_str(g)},step=${this.viewport_unit/density}mm] (0mm,0mm) grid (${this.viewport_width*this.viewport_unit}mm,${this.viewport_height*this.viewport_unit}mm);`;
    }
    return '';
  }
  p_cairo(x,y,txt,extent,g){
    ...
    var align = this.string_to_node_align_s('br');
    d.push(`align=${align}`);
    var fs = `\\fontsize{${fontsize}pt}{${fontsize}pt}\\selectfont`;
    d.push(`font=${fs}`);
    return `\\draw[${canvas}] (${this.fix(x*unit)}mm,${this.fix(y*unit)}mm) node[${d.join(',')}] {${tex_label}};`;
  ...
  p_plot(fn,cx,cy,sx,sy,xmin,xmax,ymin,ymax,x1,x2,xstep,g){
    ...
    for(var i=0; i <= this.MAX_INT; i++){
      ...
      if(y.isFinite()){
        ...
        if(x >= xmin && x <= xmax && y >= ymin && y <= ymax){
          ...
          x += cx;
          y += cy;
          d.push(`(${this.fix(x*this.viewport_unit)}mm,${this.fix(y*this.viewport_unit)}mm)`);
          continue;
        }
      }
      if(d.length){
    ...
    all = all.map(s => `\\draw[${canvas},${this.g_to_tikz_drawonly_str(g)}] ${s};`);
    return all.join('\n');   
    /*
    let unit = this.viewport_unit;
    ...
    for(var pt of coords){
      ...
      if(accept){
        if(d.length==0){
          ...
          d.push(`(${this.fix(x1*unit)}mm,${this.fix(y1*unit)}mm)`);
        }
        d.push(`(${this.fix(x2*unit)}mm,${this.fix(y2*unit)}mm)`);
      }else{
        d = [];
        all.push(d);
      }
  ...
  g_to_tikz_drawonly_str(g) {
    ...
    var alpha = this.g_to_linealpha_float(g);
    if(alpha < 1) {
  ...
  g_to_tikz_shadeonly_str(g) {
    ...
    var s = this.g_to_fillalpha_float(g);
    if(s < 1) {
      d.push(`opacity=${s}`);
    }
    if(1){
  ...
  string_to_tikz_anchor(ta){
    if( ta.localeCompare('l') == 0   ) return 'east';
    if( ta.localeCompare('r') == 0    ) return 'west';
    if( ta.localeCompare('u') == 0   ) return 'south';
    if( ta.localeCompare('b') == 0   ) return 'north';
    if( ta.localeCompare('ul') == 0  ) return 'south east';
    if( ta.localeCompare('ur') == 0   ) return 'south west';
    if( ta.localeCompare('bl') == 0  ) return 'north east';
    if( ta.localeCompare('br') == 0   ) return 'north west';
    if( ta.localeCompare('c') == 0   ) return '';
    return 'south west';
  }
  string_to_tabular_align_letter(ta){
    if( ta.localeCompare('l') == 0   ) return 'r';
    if( ta.localeCompare('r') == 0    ) return 'l';
    if( ta.localeCompare('u') == 0   ) return 'c';
    if( ta.localeCompare('b') == 0   ) return 'c';
    if( ta.localeCompare('ul') == 0  ) return 'r';
    if( ta.localeCompare('ur') == 0   ) return 'l';
    if( ta.localeCompare('bl') == 0  ) return 'r';
    if( ta.localeCompare('br') == 0   ) return 'l';
    if( ta.localeCompare('c') == 0   ) return 'c';
    return 'l';
  }
  string_to_node_align_s(ta){
    if( ta.localeCompare('l') == 0   ) return 'right';
    if( ta.localeCompare('r') == 0    ) return 'left';
    if( ta.localeCompare('u') == 0   ) return 'center';
    if( ta.localeCompare('b') == 0   ) return 'center';
    if( ta.localeCompare('ul') == 0  ) return 'right';
    if( ta.localeCompare('ur') == 0   ) return 'left';
    if( ta.localeCompare('bl') == 0  ) return 'right';
    if( ta.localeCompare('br') == 0   ) return 'left';
    if( ta.localeCompare('c') == 0   ) return 'center';
    return 'left';
  }
  ...
  txt_to_label_normal_text(s,g) {
    s = s || '';
    var s = this.translator.smooth(g,s);
    return s;
  }
  txt_to_singleline_text(s,g) {
    s = s || '';
    var fontfamily     = this.g_to_text_fontfamily_flag(g);
    var fontstyle      = this.g_to_text_fontstyle_flag(g);
    var fontweight     = this.g_to_text_fontweight_flag(g);
    var textdecoration = this.g_to_text_textdecoration_flag(g);
    let ss = [s];
    ss = ss.map(x => x.trim());
