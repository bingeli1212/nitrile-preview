...
class NitrilePreviewServer {
  ...
  constructor(intargets) {
    ...
    this.targets = o.filter((p) => p.target);
  }
  ...
  replace_cmd(cmd,target,deps){
    ...
    cmd = cmd.replace(/(\$\(.*?\))|(\$<)/g,(m,m1,m2) => {
      if(m1){
        m1 = m1.slice(2,-1);
        if(m1.startsWith("patsubst")){
          let [s1,s2,s3] = m1.slice(8).split(',').map(s => s.trim()).map(s => this.replace_cmd(s,target,deps));
          let ss = s3.split(/\s+/).filter(s => s.length).map(s => s.slice(0,-s1.length)+s2);
          return ss.join(' ');
        }
      }else if(m2){
        m2 = deps[0];
