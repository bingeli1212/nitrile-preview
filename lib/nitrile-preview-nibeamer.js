...
class NitrilePreviewBeamer extends NitrilePreviewLatex {
  constructor(parser) {
    ...
    //this.latex_details_ul_symbol = '\\ding{228}';
  }
  to_document() {
  ...
  write_one_frame(id,subid,frame,issub,topframe) {
    ...
    if(wrap_contents.length){
      ...
      norm_contents.forEach((x,i) => {
        ...
        all.push('');
        if(i>0){
          all.push('\\vspace{5pt}');//when 'columns' is enabled, there seems to be no vertical separation between paragraph anymore, this is to force a 5pt vertical gap between paragraphs
        }
        all.push(latex.trim());
      })         
      all.push('');
      ...
      wrap_contents.forEach((x,i) => {
        ...
        if(x.title){
          all.push(`\\caption{${this.uncode(x.style,x.title)}}`);
        }
        all.push('\\end{figure}')
      })           
      all.push('\\end{column}')
    ...
    if(frame.subframes && Array.isArray(frame.subframes)){
      ...
      frame.subframes.forEach((x,i,arr) => {
        all.push(`\\item[${i+1}.]\\underline{${this.uncode(x.style,x.title)}} `);
      });  
      all.push('\\end{list}')
    }
    all.push('');
    ...
    frame.solutions.forEach((o,i,arr) => {     
      ...
      o.contents.forEach(x => {
        if(x.id=='column'){
          right_contents.push(x);//wrap region
        }else{
          left_contents.push(x);//normal region
        }
      });
      if(right_contents.length){
        all.push('\\begin{columns}');
        all.push('\\begin{column}{0.5\\textwidth}')
        left_contents.forEach((x,i) => {
          //'x' is a block
          let latex = this.translate_block(x);
          all.push('');
          all.push(latex.trim());
