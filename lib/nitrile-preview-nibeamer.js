...
class NitrilePreviewBeamer extends NitrilePreviewLatex {
  constructor(parser) {
    ...
    if(this.program=='xelatex'||this.program=='lualatex'){
      this.icon_checkedbox  = this.smooth(this.style,String.fromCodePoint(0x2612));
      this.icon_hollowbox   = this.smooth(this.style,String.fromCodePoint(0x2610));
      this.icon_coffeecup   = this.smooth(this.style,String.fromCodePoint(0x2615));
      this.icon_writinghand = this.smooth(this.style,String.fromCodePoint(0x270D));
    }else{
...
`;
  ...
  write_one_frame(id,order,icon,frame,issub,topframe) {
    ...
    var xs1 = [];   
    var xs2 = [];
    if(frame.contents.length && frame.contents[0].style.isbundle && frame.contents[0].bundle.g.wrapfig && frame.contents[0].bundle.key=='dia'){
      let x = frame.contents.shift();
      xs1.push(x); 
      xs2 = frame.contents;
    }else{
      xs2 = frame.contents;
    ...
    if(xs1.length){
      let all = [];
      let itm = this.do_bundle(xs1[0].style,xs1[0].bundle);
      var text1 = itm.tikzpicture;
      var text1 = `\\resizebox{\\textwidth}{!}{${text1}}`;
      var text1 = `\\frame{${text1}}`;
    }
    if(1){
      let all = [];
      xs2.forEach((x,i) => {
