...
class NitrilePreviewDiagramMF extends NitrilePreviewDiagram {
  ...
  do_one_frame(style,ss,env) {
    ...
    var gridcolor = this.string_to_mp_color(gridcolor,0);
    ///CONFIG AND VIEWPORT
    this.config = {...style};
    ...
    if (background=='grid'){ 
      d.push(`for i=0 step ${grid} until ${ym}: draw (0,i*u) --- (${xm}*u,i*u) withcolor ${gridcolor}; endfor;`);
      d.push(`for i=0 step ${grid} until ${xm}: draw (i*u,0) --- (i*u,${ym}*u) withcolor ${gridcolor}; endfor;`);
    } else {
      d.push(`for i=0 step ${ym} until ${ym}: draw (0,i*u) --- (${xm}*u,i*u) withcolor white; endfor;`);
      d.push(`for i=0 step ${xm} until ${xm}: draw (i*u,0) --- (i*u,${ym}*u) withcolor white; endfor;`);
    }
    d.push(s);
    var s = d.join('\n');
    var d = [];
    ...
    d.push(`draw unitsquare sized(vw,vh) withcolor white ;`);
    d.push(s);
    ...
    if (stretch) {
      d.push(`currentpicture := currentpicture xsized (\\the\\textwidth*${stretch}) ;`)
  ...
  p_polyline(points,g){
    ...
    var s = d.join('--');
    if(this.has_strokes(g)){
      text=(`${this.to_leads(g)} draw (${s}) scaled(u) ${this.to_draws(g)};`);
    }
    return text;
    //var mypath = `(${this.fix(x1)},${this.fix(y1)})--(${this.fix(x2)},${this.fix(y2)})--(${this.fix(x3)},${this.fix(y3)})--cycle`;
  }
  p_polygon(points,g){
    var d = [];
    for(var i=1; i < points.length; i+=2){
    ...
    var s = d.join('--');
    if(this.has_shades(g)){
      text=(`${this.to_leads(g)} fill (${s}) scaled(u) ${this.to_shadeonlys(g)};`);
    }else if(this.has_fills(g)){
      text=(`${this.to_leads(g)} fill (${s}) scaled(u) ${this.to_fills(g)};`);
    }
    if(this.has_strokes(g)){
      text=(`${this.to_leads(g)} draw (${s}) scaled(u) ${this.to_draws(g)};`);
    }
  ...
  to_ink(style,ss){
    ...
    let height = this.g_to_height_float(style);
    if (stretch) {
      o.push(`textwidth := textwidth*${stretch};`);
      o.push(`ratio_w := textwidth/vw;`);  
      o.push(`currentpicture := currentpicture scaled ratio_w;`)
    }
    else if (width && height){
      o.push(`textwidth := ${width}mm;`)
      o.push(`ratio_w := textwidth/vw;`);  
      o.push(`textheight := ${height}mm;`);
      ...
      o.push(`textheight := ${height}mm;`)
      o.push(`ratio_h := textheight/vh;`);  
      o.push(`currentpicture := currentpicture scaled ratio_h;`)
    }
    if(style.frame){
