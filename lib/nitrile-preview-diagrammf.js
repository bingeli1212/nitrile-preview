...
class NitrilePreviewDiagramMF extends NitrilePreviewDiagram {
  ...
  do_finalize(s,style) {
    ...
    if (this.conf_to_stringig.gridlines=='none') {
      ...
      let gridcolor = this.g_to_gridcolor_string(this.conf_to_stringig);
  ...
  to_framed(style,ss){
    ...
    var vh = this.assert_int(style.frameheight,12);
    var gap = 2;
    if(vh < npara*12){;
    ...
    o.push(`numeric textheight; textheight := \\the\\textwidth;`);
    ...
    o.push(`clip currentpicture to unitsquare xscaled (vw) yscaled (vh) ;`);
    if(style.outline){
