...
class NitrilePreviewDiagramMF extends NitrilePreviewDiagram {
  ...
  p_circle(x,y,radius,g){
    ...
    var o = [];
    if (this.has_fills(g)){
      o.push(`fill fullcircle scaled(${this.fix(radius*2)}) shifted(${this.fix(x)},${this.fix(y)}) scaled(u) ${this.to_fills(g)};`);
    } 
    o.push(`${this.to_leads(g)} draw fullcircle scaled(${this.fix(radius*2)}) shifted(${this.fix(x)},${this.fix(y)}) scaled(u) ${this.to_draws(g)};`);
    return o.join('\n');
  }
  ...
  p_rect(x,y,w,h,g){
    ...
    w = this.scale_dist(w);
    h = this.scale_dist(h);
    var mypath = `(${this.fix(x)},${this.fix(y)})--(${this.fix(x+w)},${this.fix(y)})--(${this.fix(x+w)},${this.fix(y+h)})--(${this.fix(x)},${this.fix(y+h)})--cycle`;
    var o = [];
    if (this.has_fills(g)){
  ...
  p_hexgon(x,y,w,h,g){
    var p = this.to_p_hexgon(x,y,w,h);
    return this.p_draw(p,g);
  }
  p_triangle(x,y,w,h,g){
    var p = this.to_p_triangle(x,y,w,h);
    return this.p_draw(p,g);
  }
  p_TRIANGLE(x,y,w,h,g){
    var p = this.to_p_TRIANGLE(x,y,w,h);
