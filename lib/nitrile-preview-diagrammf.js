...
class NitrilePreviewDiagramMF extends NitrilePreviewDiagram {
  ...
  do_finalize(s,style) {
    ...
    d.push(`numeric vw; vw := ${xm}*u;`);
    d.push(`numeric vh; vh := ${ym}*u;`);
    d.push(s);
    d.push(`clip currentpicture to unitsquare xscaled (vw) yscaled (vh) ;`);
    if (style && style.stretch) {
      d.push(`currentpicture := currentpicture xsized (\\the\\textwidth*${style.stretch}) ;`)
    } else if (style && style.width && style.height){
      let width = this.g_to_width_float(style);
      let height = this.g_to_height_float(sytle);
      d.push(`currentpicture := currentpicture xsized (${width}mm) ysized (${height}mm) ;`)
    } else if (style && style.width) {
      let width = this.g_to_width_float(style);
      d.push(`currentpicture := currentpicture xsized (${width}mm) ;`)
    } else if (style && style.height) {
      let height = this.g_to_height_float(style);
      d.push(`currentpicture := currentpicture ysized (${height}mm) ;`)
    }
    if(style.frame){
  ...
  p_rrect(x,y,w,h,Dx,Dy,g){
    var p = this.to_q_RREC(x,y,w,h,Dx,Dy);
    return this.p_draw(p,g);
  }
  ///
  ///Bezier curve
  ...
  p_labelmath(x,y,txt,ta,g){
    ...
    var o = [];
    o.push(`pair a ; a := (${this.fix(x)}*u,${this.fix(y)}*u) ;`);
    o.push(`label.${ta}("${textext}",a) ;`);
    return o.join('\n');
  }
    ...
    */
    ...
    if(this.has_texts(g)){
      ...
      if(ta=='bot'){
        ...
        //'ctr'
        o.push(`draw one shifted (0,+bbheight(one)/2) rotated (${this.fix(ang)}) shifted (${this.fix(x)}*u,${this.fix(y)}*u) withcolor ${fontcolor};`);
      }
    }
    return o.join('\n')
  }
  ...
  p_circle(x,y,r,g){
    ...
    var o = [];
    if (this.has_shades(g)){
      o.push(`fill fullcircle scaled(${this.fix(r*2)}) shifted(${this.fix(x)},${this.fix(y)}) scaled(u) ${this.to_shadeonlys(g)};`);
    }else if (this.has_fills(g)){
  ...
  p_ellipse(x,y,Rx,Ry,angle,g){
    ...
    if (ang2 < ang1) { ang2 += 360; }
    if (this.has_shades(g)){
      o.push(`fill fullcircle xscaled(${this.fix(2*Rx)}*u) yscaled(${this.fix(2*Ry)}*u) rotated(${this.fix(Phi)}) shifted(${this.fix(Cx)}*u,${this.fix(Cy)}*u) ${this.to_shadeonlys(g)};`);
    }else if (this.has_fills(g)){
  ...
  p_pie(cx,cy,r,start_a,span_a,g){
    ...
    if (Number.isFinite(Cx) && Number.isFinite(Cy)) {
      ...
      if (sweepflag) {
        if (this.has_shades(g)){
          o.push(`fill ((${this.fix(Cx)},${this.fix(Cy)})--(${this.fix(x0)},${this.fix(y0)})--(subpath (${this.fix(ang2 / 45)},${this.fix(ang1 / 45)}) of fullcircle xscaled(${this.fix(2*Rx)}) yscaled(${this.fix(2*Ry)}) rotated(${Phi}) shifted(${this.fix(Cx)},${this.fix(Cy)}))--cycle) scaled(u) ${this.to_shadeonlys(g)};`);
        }else if (this.has_fills(g)){
        ...
        if (this.has_shades(g)){
          o.push(`fill ((${this.fix(Cx)},${this.fix(Cy)})--(${this.fix(x0)},${this.fix(y0)})--(subpath (${this.fix(ang1 / 45)},${this.fix(ang2 / 45)}) of fullcircle xscaled(${this.fix(2*Rx)}) yscaled(${this.fix(2*Ry)}) rotated(${Phi}) shifted(${this.fix(Cx)},${this.fix(Cy)}))--cycle) scaled(u) ${this.to_shadeonlys(g)};`);
        }else if (this.has_fills(g)){
  ...
  p_image(imagefile){
    var o = [];
    o.push(`picture p ; p := externalfigure "${imagefile}" ;`);
    o.push(`numeric sx ; sx := bbwidth(p)/vw ;`);
    o.push(`numeric sy ; sy := bbheight(p)/vh ;`);
    o.push(`if sx > sy : p := p xsized(vw) ; else : p := p ysized(vh) ; fi ;`);
    o.push(`draw p shifted((vw-bbwidth(p))/2,(vh-bbheight(p))/2) ;`);
    return o.join('\n');
  }
  ///
  /// g_to_XXX
