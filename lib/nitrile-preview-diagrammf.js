...
class NitrilePreviewDiagramMF extends NitrilePreviewDiagram {
  ...
  do_one_frame(style,ss,env) {
    ...
    if (this.config.gridlines=='none') {
      ...
      d.push(`for i=0 step ${grid} until ${ym}: draw (0,i*u) --- (${xm}*u,i*u) withcolor ${s}; endfor;`);
      d.push(`for i=0 step ${grid} until ${xm}: draw (i*u,0) --- (i*u,${ym}*u) withcolor ${s}; endfor;`);
    }
    d.push(s);
    var s = d.join('\n');
    var d = [];
  ...
  ///triangle drawing
  ///
  p_triangle(x1,y1,x2,y2,x3,y3,g){
    [x1,y1] = this.move_xy(x1,y1);
    [x2,y2] = this.move_xy(x2,y2);
