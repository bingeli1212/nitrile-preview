  ...
  p_slopedtext(x1,y1,x2,y2,txt,ta,g){
    ...
    */
    ...
    if(this.has_texts(g)){
      ...
      if(ta=='bot'){
        //'bot'
        o.push(`draw one shifted (0,-bbheight(one)) rotated (${this.fix(ang)}) shifted (${x}*u,${y}*u) withcolor ${fontcolor};`);
      }else{
        //'top'
        o.push(`draw one shifted (0,+bbheight(one)) rotated (${this.fix(ang)}) shifted (${x}*u,${y}*u) withcolor ${fontcolor};`);
      }
