...
class NitrilePreviewDiagramMF extends NitrilePreviewDiagram {
  ...
  do_finalize(s,style) {
    ...
    if (this.config.grid && this.config.grid=='boxed') {
      ...
      o.push(`for i=0 upto ${ym}: draw (0,i*u) --- (${xm}*u,i*u) withcolor .92white; endfor;`);
      o.push(`for i=0 upto ${xm}: draw (i*u,0) --- (i*u,${ym}*u) withcolor .92white; endfor;`);
    }
    o.push(s);
    var s = o.join('\n');
    let d = [];
  ...
  p_rrect(x,y,w,h,Dx,Dy,g){
    var p = this.to_p_rrect(x,y,w,h,Dx,Dy);
  ...
  p_text(x,y,txt,ta,g) {
    ...
    if(ta=='top'||ta=='ulft'||ta=='urt'){
      ...
      ss.forEach((s,j,arr) => {
        ...
        o.push(`draw pic[${j+1}] shifted (${dx}*bbwidth(pic[${j+1}]),${dy}*fh) shifted (${x}*u,${y}*u) withcolor ${fontcolor};`)
        dy -= 1;
      })
  ...
  to_tex_label(txt,math,fontsize){
    ...
    if (math==1) {
      // math text
      let style = {};
      var s = this.translator.do_phrase(style,'inlinemath',txt);
