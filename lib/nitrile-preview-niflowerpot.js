...
class NitrilePreviewFlowerpot extends NitrilePreviewHtml {
  constructor(parser) {
    ...
    this.add_css_map_entry(this.css_map,
      'FIGURE, COLUMN, LISTING, EQUATION, TABLE', [
        'margin-left: 0pt',
        'margin-right: 0pt',
      ]
    );
...
`;
  ...
  write_one_frame(id,order,frame,issub,parentid,toc,pages,topframe){
    ...
    if(frame.subframes && Array.isArray(frame.subframes)){
      ...
      frame.subframes.forEach((x,i,arr) => {
        if(i==0){
          all.push(`<ol class='PARA OL' style=''>`);
        }
        //let icon = `<span style='position:absolute;right:100%;top:50%;transform:translate(-1.12ex,-50%);'>${j+1}</span>`;
        let icon = this.icon_folder;
        all.push(`<li class='PACK LI' value='${i+1}' style='position:relative;font-style:oblique;'> ${icon} <a href='#frame-${id}x${i+1}'> ${this.uncode(x.style,x.title)} </a> </li>`);
        if(i+1==arr.length){
          all.push(`</ol>`);
        }
      });
  ...
  to_sidesvg_for_id(id,toc,max_n=30){
    ...
    toc.forEach((p,j) => {
      ...
      all.push(`<text id="svg${p.id}" x="${x}" y="${y-dy+8}" stroke="none" fill="${fontcolor}" font-size="5pt" >${p.order} ${icon} ${this.polish(p.title)} ${p.posticon} </text>`)
    })
    var text = all.join('\n');
