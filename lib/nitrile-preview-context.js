...
\\stoptext
  ...
  phrase_to_ref(label){
    ...
    if(g){
      ...
      if(sig=='HDGS'){
        return `Section~${secsign}\\in[${label}]`;
      }
      ...
      // }
      if(name=='equation'){
        return `Equation~\\in[${label}]`;
      }
      if(name=='table'){
        return `Table~\\in[${label}]`;
      }
      if(name=='figure'){
        return `Figure~\\in[${label}]`;
      }
      if(name=='listing'){
        return `Listing~\\in[${label}]`;
      }
    }
  ...
  phrase_to_img(cnt) {
    ...
    if(g.src){
      ...
      if (g.width && g.aspectratio) {
        let width = this.string_to_context_width(g.width);
        let height = this.string_to_context_height_with_aspect_ratio(g.width,g.aspectratio);
  ...
  string_to_context_height_with_aspect_ratio(width_s,aspect_ratio_s) {
    /// take an input string that is 100% and convert it to '\textwidth'.
    /// take an input string that is 50% and convert it to '0.5\textwidth'.
    /// take an input string that is 10cm and returns "10cm"
    if (!width_s) {
    ...
    var re = /^(.*)\%$/;
    if (re.test(width_s)) {
      var str0 = width_s.slice(0,width_s.length-1);
      var num = parseFloat(str0)/100;
      if (Number.isFinite(num)) {
        num *= ratio;
        var num = num.toFixed(3);
  ...
  rows_to_tabulate(rows,style) {
    ...
    if(border==1||border==2||border==3){
      ...
      var pcol = pcols.join('|');
      var pcol = `|${pcol}|`;
      d.push(`\\starttabulate[${pcol}][distance=0pt,unit=0.5em]`);
    }else{
  ...
  __rows_to_longtable__bTABLE__(rows, style) {
    var fr = style.fr || '';
    var glue = style.glue || '';
    var nrows = rows.length? rows[0].length : 0;
    var border = style.border||0;
