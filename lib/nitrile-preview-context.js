...
\\stoptext
  ...
  do_SAMP(block){
    var {id,row1,row2,sig,title,body,style} = block;
...
\stopcombination}
  ...
  float_to_table(title,ss,g){
    ...
    var text = this.untext(ss,g);
    var label = g.label||'';
    ///remove the 'hbox' command
    var re = /\\dontleavehmode\\hbox\{(.*)\}$/gs;
