...
\\stoptext
  ...
  phrase_to_img(g) {
    ...
    if(g.src){
      let src = g.src;
      this.imgs.push(src);
      let width = this.string_to_context_length(g.width);
      let height = this.string_to_context_length(g.height);
      let text;
      if (width && height) {
        ...
        text = `\\externalfigure[${src}][height=${height}]`;
      } else {
    ...
    if(g.href){
      ...
      if(href.startsWith('#')){
        ...
        if(this.notes.has(id)){
          ...
          let style = g;
          return this.fence_to_diagram(ss,style);
        }
      }
      return '';
    }
