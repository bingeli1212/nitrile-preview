...
class NitrilePreviewContext extends NitrilePreviewLatex {
  ...
  constructor(parser) {
    ...
    //this.unicode_symbol_map = this.tokenizer.build_unicode_symbol_map_for_cex();
  }
  to_conf_titlepage(){
...
\\stoptext
  ...
  do_PLST(block){
    ...
    for (var item of items) {
      var {bull,bullet,value,text,br,keys,more} = item;
      text = text || '';
      let extra_text = '';
      if (more && more.length) {
        more.forEach((p) => {
      ...
      switch (bull) {
        ...
        case 'LI': {
          if(bullet[0]=='+'){
            ...
            let startcmd = `\\startlatexdesc`;
            let stopcmd = `\\stoplatexdesc`;
            if(isbroad){
              startcmd = `\\startlatexdescbroad`;
              stopcmd = `\\stoplatexdescbroad`;
            }
            if (text) {
              o.push(`${startcmd}{${keys.join(', ')}} \\\\ ${text} ${extra_text} \n${stopcmd}`);
            } else {
              o.push(`${startcmd}{${keys.join(', ')}} {} ${extra_text} \n${stopcmd}`);
            }
          } else {
            text = this.unmask(text);
            if(keys){
            ...
            if(item.type == 'A'){
              o.push(`\\sym {${this.to_A_letter(item.value)}${item.ending}} ${text} ${extra_text}`)
            }else if(item.type == 'a'){
              o.push(`\\sym {${this.to_a_letter(item.value)}${item.ending}} ${text} ${extra_text}`)
            }else if(item.type == 'I'){
              o.push(`\\sym {${this.to_I_letter(item.value)}${item.ending}} ${text} ${extra_text}`)
            }else if(item.type == 'i'){
              o.push(`\\sym {${this.to_i_letter(item.value)}${item.ending}} ${text} ${extra_text}`)
            }else if(item.value) {
              o.push(`\\sym {${item.value}${item.ending}} ${text} ${extra_text}`);
            }else if(bullet[0]=='*'){
  ...
  do_FLOA(block) {
    ...
    switch (block.style.floatname) {
      ...
      case 'Listing': {
        ...
        text = d.join('\n');
        let o = [];
        let ss = block.body.map(s => this.polish_verb(s));
        text = this.ss_to_listing(ss,block.style);
        o.push(`\\placelisting`);
        o.push(`[here,split]`);
        o.push(`[${label}]`);
        o.push(`{${this.unmask(caption)}}`);
  ...
  phrase_to_ref(label){
    ...
    if(g){
      let { sig, title, name, idnum, chnum } = g;
      var secsign = String.fromCharCode(0xA7);
      if(sig=='HDGS'){
        return `\\in[${label}]`;
      }
      // if(floatname == 'Listing'){  
      //   if(chnum) {
      //     idnum = `${chnum}.${idnum}`;
      //   }
  ...
  to_formula_math_array(str,style) {
    var d = this.tokenizer.to_cmath_array(str,style);
    return d;
  }

  phrase_to_math(str,style) {
    var s = this.tokenizer.to_cmath(str,style);
    return `${s}`;
  }
  ...
  fence_to_math(ss,style){
    var text = this.make_line(ss,style,0);
    return text;
  }
  make_line(ss, style, isequation) {
    let label = style.label||'';
    if (label.length == 0) {
      label = '[+]';
    } else {
    ...
    if(isequation){
      lines.unshift(`\\setupformulas[align=center]`)
    }else{
      lines.unshift(`\\setupformulas[align=right]`)
    }
    return lines.join('\n');
  }
  phrase_to_img(cnt) {
    ...
    if(g.src){
      ...
      if (g.width && g.aspectratio) {
        let width = this.string_to_context_width(g.width,zoom);
        let height = this.string_to_context_height_with_aspect_ratio(g.width,g.aspectratio,zoom);
        text = `\\externalfigure[${src}][width=${width},height=${height}]`;
      } else if (g.width) {
        let width = this.string_to_context_width(g.width,zoom);
        text = `\\externalfigure[${src}][width=${width}]`;
      } else {
        text = `\\externalfigure[${src}]`;
      }
      if(g.frame==1){
        text = `\\framed{${text}}`
      }
  ...
  para_to_equation(ss,style){
    ...
    clusters.forEach((ss) => {
      let isequation = 1;
      let text = this.make_line(ss,style,isequation);
      d.push(text)
    })
    let text = d.join('\n');
    text = `\\placeformula\n${text}`
  ...
  para_to_longtable(ss, style) {
    ...
    let text = this.rows_to_longtable(rows, style);
    return text;
  }
  para_to_multi(ss,style){
    ...
    var gap = 0.02;
    var frs = this.string_to_frac(style.fr,n,gap);
    itms = itms.map((s,i) => {
      style.side = i;
        style.gap = 0.02;
      s = this.unmask(s,style);
      return s;
    });
  ...
  para_to_figure(ss,style){
    ...
    var text = this.imgrid_to_combination(itms,style);
    if (1) {
      let label = style.label || '';
      let caption = this.fetch_caption_from_notes();
      let o = [];
  ...
  para_to_listing(ss,style){
    ss = ss.map(s => this.polish_verb(s));
    let caption = this.fetch_caption_from_notes();
    let label = style.label||'';
    let text = this.ss_to_listing(ss,style);
    let o = [];
    o.push(`\\placelisting`);
    o.push(`[here,split]`);
    o.push(`[${label}]`);
  ...
  string_to_context_width(s,zoom) {
  ...
  rows_to_tabulate(rows,style) {
    ...
    if(border==1||border==2||border==3){
      var pcols = 'x'.repeat(ncols).split('').map(x => `k1l`);
      var pcol = pcols.join('|');
      var pcol = `|${pcol}|`;
      d.push(`\\starttabulate[${pcol}][distance=0pt,unit=0.5em,before={},after={}]`);
    }else{
      var pcols = 'x'.repeat(ncols).split('').map(x => `i1l`);
      var pcol = pcols.join('|');
      var pcol = `|${pcol}|`;
      d.push(`\\starttabulate[${pcol}][distance=0pt,unit=0.5em,before={},after={}]`);
    }
    ...
    for (var j = 0; j < nrows; ++j) {
      ...
      if(j>0 && border==1 && !top_border){
        d.push('\\HL')
      }
      top_border=0;
      if(border==1||border==2||border==3){
        d.push(`\\VL ${pp.join(' \\VL ')} \\VL \\MR`);
      }else{
        d.push(`\\NC ${pp.join(' \\NC ')} \\NC \\MR`);
      }
    }
    if(border==1||border==2||border==3||border==4){
      d.push('\\HL')
  ...
  smooth (unsafe) {
    ...
    for (let c of unsafe) {
      if (this.unicode_symbol_map.has(c)) {
        c = this.unicode_symbol_map.get(c).cex;
      }
      new_unsafe += c;
    }
    unsafe = new_unsafe;
  ...
  __rows_to_longtable(rows, style) {
    ...
    if(1){
      ...
      if(this.pp_is_hline(rows[0])){
        mflag = 1;
        rows = rows.slice(1);
      }else if(this.pp_is_dhline(rows[0])){
        mflag = 1;
        rows = rows.slice(1);
      }else if(border==1){
        mflag = 1;
  ...
  rows_to_longtable(rows, style) {
    ...
    if(border==1){
      vlines = this.string_to_array('*');
      hlines = this.string_to_array('t m b r');
    }else if(border==2){
      vlines = this.string_to_array('*');
      hlines = this.string_to_array('t b');
    }else if(border==3){
      vlines = this.string_to_array(`0 ${n}`);
      hlines = this.string_to_array('t b');
    }else if(border==4){
      vlines = this.string_to_array('');
      hlines = this.string_to_array('t b');
