...
\\stoptext
  ...
  do_FLOA(block) {
    ...
    switch (block.floatname) {
      ...
      case 'Listing': {
        ...
        text = d.join('\n');
        // let idnum = block.style.idnum;
        // let chnum = block.style.chnum;
        // if(chnum){
        //   idnum = `${chnum}.${idnum}`;
        // }
        let o = [];
  ...
  phrase_to_ref(label){
    if(this.ref_map.has(label)){
      let g = this.ref_map.get(label);
      let { sig, floatname, idnum, chnum } = g;
