...
\\stoptext
  ...
  fence_to_img(ss,id,g) {
    ...
    if (g.width) {
      let width = this.string_to_contex_width(g.width,1);
      let height = this.string_to_contex_width(g.width,rate);
  ...
  fence_to_tabular(ss,g){
    ...
    var pcol = pcols.join('|');
    o.push(`\\starttable[|${pcol}|]`);
    if(title){
      let title_text = this.uncode(title,g);
      o.push(`\\NC \\use{${n}}\\ReFormat[c]{${title_text}} \\MR`)
    }
  ...
  text_to_typeface (type,cnt,cnt2) {
    ...
    switch (type) {
      ...
      case 'frac': {
        cnt2 = cnt2||'\\questiondown';
        return `\\math{\\frac{${cnt}}{${cnt2}}}`
        break;
      }
      case 'sup': {
        return `\\high{${cnt}}`
