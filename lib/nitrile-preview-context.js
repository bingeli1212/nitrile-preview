...
\\stoptext
  ...
  do_PLST(block){
    ...
    for (var item of items) {
      ...
      if (more && more.length) {
        more.forEach((p) => {
          let { lines } = p;
          extra_text += `\n\n\\blank\\noindent ${this.untext(lines,style,switches)}`;
        });
      }
      switch (bull) {
        case 'OL': {
        ...
        case 'LI': {
          if(bullet[0]=='+'){
            let  startcmd = `\\startlatexdescbroad`;
            let  stopcmd = `\\stoplatexdescbroad`;
            o.push(`${startcmd}{${text}} \\\\ ${extra_text} \n${stopcmd}`);
          } else {
            if(item.type == 'A'){
  ...
  do_PRIM(block){
    ...
    title = this.uncode(title);
    let s0 = body[0] || '';
    text = this.untext(body,style,switches);
    if (hdgn===0) {
  ...
  fence_to_vbox(ss,style){
    ...
    if(style.width){
      var width = this.string_to_context_width(style.width);
    }else{
      var width = `\\textwidth`;
    }
    var o = [];
  ...
  fence_to_img(ss,id,g) {
    ...
    let rate = asp[1]/asp[0];
    if (g.width) {
      let width = this.string_to_context_width(g.width,1);
      let height = this.string_to_context_width(g.width,rate);
      text = `\\externalfigure[${fname}][width=${width},height=${height}]`;
    } else {
      let width = `\\textwidth`;
      let height = `${rate.toFixed(3)}\\textwidth`
      text = `\\externalfigure[${fname}][width=${width},height=${height}]`;
    } 
    if(g.frame==1){
      text = `\\framed{${text}}`
  ...
  fence_to_table(ss,style) {
    ...
    if(1){
      ...
      var pcol = this.to_tabulate_lcr_pcol(n,style);
      var hrows = this.string_to_array(style.hline);
      d.push(`\\starttabulate[${pcol}][distance=0pt,unit=0.1em,before={},after={}]`);
      if(hrows.indexOf('t')>=0){
        d.push('\\HL');
      }
      d.push(text);
      if(hrows.indexOf('b')>=0){
        d.push('\\HL');
      }
      d.push('\\stoptabulate')
      text = d.join('\n');
  ...
  fence_to_equation(ss,style){
    ...
    if(style.label){
      let o = [];
      o.push(`\\placeformula[${style.label}]`)
      o.push(text);
      text = o.join('\n')
    }
    return text;
  ...
  fence_to_table(ss,style){
    ...
    if(style.long){
      ...
      if(1){//TODO: need to adjust for when style.fr is set
        let d = [];
        var n = (rows.length)?rows[0].length:0;
        var pcol = this.to_tabulate_lcr_pcol(n,style);
        var hrows = this.string_to_array(style.hline);
  ...
  fence_to_figure(ss,style){
    var {caption,ss} = this.extract_body_caption(ss,style);
    let label = style.label || '';
    let itms = this.ss_to_figure_itms(ss,style);
    itms.forEach(p => {
      p.sub = this.uncode(p.sub);
    })
  ...
  rows_to_tabulate(rows,style) {
    ...
    for (var j = 0; j < nrows; ++j) {
      ...
      ///add HL
      if(j==0){
      }else if(top_border==2){
        d.push('\\HL');
        d.push('\\TB[0.2em]')
        d.push('\\HL');
        top_border=0;
      }else if(top_border==1){
        d.push('\\HL');
        top_border=0;
      }else if(hrows.indexOf('+')>=0){
        d.push('\\HL');
      }
      ///add row
      var mypp = [];
  ...
  rows_to_longtable(rows, style) {
    ...
    var header = rows[0];///always the same width as n
    var header = header.map(p => p.text);
    var rows = rows.slice(1);
    if(!header){
      header = [];
    }
