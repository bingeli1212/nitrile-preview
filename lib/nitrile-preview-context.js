...
\\setupbodyfont[linuxlibertineni]
\\starttext
${titlelines.join('\n')}
${toclines.join('\n')}
${texlines.join('\n')}
\\stoptext
  ...
  text_to_typeface (type,cnt,cnt2) {
    ...
    switch (type) {
      ...
      case 'frac': {
        //NOTE: \dontleavehmode\hbox{\starttable[s(0pt)|c|s(0pt)|]\NC{}a+b\NC\NR\HL\NC{}a\NC\NR\stoptable}
        cnt2 = cnt2||'\\questiondown';
        return `\\dontleavehmode\\hbox{\\starttable[s(0pt)|c|s(0pt)|]\\NC{}${cnt}\\NC\\NR\\HL\\NC{}${cnt2}\\NC\\NR\\stoptable}`;
        break;
      }
      case 'sup': {
        return `\\high{${cnt}}`
...
\stopcombination}
    */
    ...
    if(g.subfigure){
      let itms = this.ss_to_figure_itms(ss,g);
      itms.forEach((p,i,arr) => {
        var {id,ss,g,sub} = p;
    ...
    if(caption){
      var caption_text = this.uncode(caption,g);
      o.push(`{${caption_text}}`);
    }else{
      o.push(`{}`);
    }
  ...
  para_to_equation(caption,ss,g){
    var g = this.update_style_from_switches(g,'equation')
    let text = this.tokenizer.to_cmath(ss.join('\n'),g,1);
    let label = g.label||'';
    if(label=='none'){
      label = '';
    }
  ...
  para_to_longtable(caption,ss,g) {
  ...
  para_to_table(caption,ss,g){
    var g = this.update_style_from_switches(g,'table')
    var text = this.fence_to_tabular(ss,g);
    var o = [];
    o.push(`\\placetable`);
    o.push('[]')
    if(g.label){
