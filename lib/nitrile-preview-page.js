...
`;
  ...
  hdgs_to_part(title,label,partnum,chapternum,level,style){
    return(`<h1 row='${style.row}'>Part.${partnum} ${this.uncode(style,title)}</h1>`);
  }
  hdgs_to_chapter(title,label,partnum,chapternum,level,style){
    return(`<h1 row='${style.row}'>Chapter.${chapternum} &#160; ${this.uncode(style,title)}</h1>`);
  }
  hdgs_to_section(title,label,partnum,chapternum,level,style){
    if(chapternum){
      return(`<h2 row='${style.row}'${chapternum}.${level}. ${this.uncode(style,title)}</h2>`);  
    }
    return(`<h2 row='${style.row}'>${level}. ${this.uncode(style,title)}</h2>`);
  }
  hdgs_to_subsection(title,label,partnum,chapternum,level,style){
    if(chapternum){
      return(`<h3 row='${style.row}'>${chapternum}.${level}. ${this.uncode(style,title)}</h3>`);  
    }
    return(`<h3 row='${style.row}'>${level}. ${this.uncode(style,title)}</h3>`);
  }
  hdgs_to_subsubsection(title,label,partnum,chapternum,level,style){
    if(chapternum){
      return(`<h4 row='${style.row}'>${chapternum}.${level}. ${this.uncode(style,title)}</h4>`);  
    }
    return(`<h4 row='${style.row}'>${level}. ${this.uncode(style,title)}</h4>`);
  }
}
