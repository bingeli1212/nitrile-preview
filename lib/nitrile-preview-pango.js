...
`;
  ...
  fence_to_list(style,body){
    ...
    itms.forEach((p,i,arr) => {
      ...
      if(p.type=='UL'){
        dy += this.do_para_item(0,dy,bullet,p.text,this.frame_extent,this.fontsize_factor,style)
      }
      else if(p.type=='OL'){
        let lead = `${i+1}`;
        dy += this.do_para_item(0,dy,lead,p.text,this.frame_extent,this.fontsize_factor,style)
      }
      else if(p.type == '1'){
        ...
        dy += this.do_para_dl(0,dy,p.value,p.text,this.frame_extent,this.fontsize_factor,style)
      }
      else if(p.type == 'CB'){
        if(p.bull.trim().length || this.is_in_list(p.start,check_ss)){
          //filled CB
          dy += this.do_para_cb(0,dy,squarebox,p.text,this.frame_extent,this.fontsize_factor,style)
        }else{
          //empty CB
          dy += this.do_para_cb(0,dy,squareboxo,p.text,this.frame_extent,this.fontsize_factor,style)
        }
      }
      else if(p.type == 'RB'){
        if(p.bull.trim().length || this.is_in_list(p.start,check_ss)){
          //filled CB
          dy += this.do_para_cb(0,dy,circlebox,p.text,this.frame_extent,this.fontsize_factor,style)
        }else{
          //empty CB
          dy += this.do_para_cb(0,dy,circleboxo,p.text,this.frame_extent,this.fontsize_factor,style)
        }
      }
      else {
        dy += this.do_para_item(0,dy,bullet,p.text,this.frame_extent,this.fontsize_factor,style)
