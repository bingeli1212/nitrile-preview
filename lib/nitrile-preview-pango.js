...
`;
  ...
  write_frame(frame){
    ...
    this.do_frametitle(frame.style,icon,frame.title);
    return;
  ...
  to_titlepage(){
    ...
    var dx = 0;
    var dy = 60;
    all.push(this.do_para(this.frame_padding_left+dx,this.frame_padding_top+dy,this.uncode(style,title),this.frame_extent,this.frontmattertitle_factor,'center',style));
    dy += 80;
    all.push(this.do_para(this.frame_padding_left+dx,this.frame_padding_top+dy,this.uncode(style,subtitle),this.frame_extent,this.fontsize_factor,'center',style));
    dy += 23;
    all.push(this.do_para(this.frame_padding_left+dx,this.frame_padding_top+dy,this.uncode(style,institute),this.frame_extent,this.fontsize_factor,'center',style));
    dy += 23;
    all.push(this.do_para(this.frame_padding_left+dx,this.frame_padding_top+dy,this.uncode(style,author),this.frame_extent,this.fontsize_factor,'center',style));
    dy += 23;
    all.push(this.do_para(this.frame_padding_left+dx,this.frame_padding_top+dy,this.uncode(style,date),this.frame_extent,this.fontsize_factor,'center',style));
    this.frame_svgs.push(all);
  }
  ///
  ///
  ///
  do_para(x,y,txt,extent,factor,align,g){
    if(!txt) return '';
    var all = [];
    ...
    for(var i=0; i <= max_seq; ++i){
      ...
      var dy = 0;
      var text = this.do_line(x+dx,y+dy,pp,i,factor,g);
      all.push(text);
    }
    return all.join('\n');
  }
  ...
  do_frametitle(style,icon,title){
    var dx = 0;
    var dy = 0;
    var text = this.uncode(style,title);
    var factor = this.frametitle_factor;
    this.frame_svg.push(this.do_para(this.frame_padding_left+dx,this.frame_padding_top+dy,title,this.frame_extent,factor,'left',style));
    this.frame_svg.push(this.do_para(0,this.frame_padding_top+dy,icon,this.frame_padding_left-3,factor,'right',style));
  }
}
