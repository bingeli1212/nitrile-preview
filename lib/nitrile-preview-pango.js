...
`;
  ...
  write_frame(frame){
    ...
    ///frame title
    var line_height = this.do_frametitle(frame.style,icon,frame.title);
    this.frame_svg_y += line_height;
    this.frame_svg_y += this.frame_svg_blank;
    ///frame contents
    frame.contents.forEach((x,i) => {
      ///'x' is a block
      this.translate_block(x);
    })
  ...
  ///
  ///
  do_para_cb(px,py,lead,txt,extent,factor,style){
    ///the indent
    var dx = 30;
    ///draw the lead
    var line_height_lead = this.do_para(px,py,lead,dx,factor,'left',style)
    ///draw the text
    var segment_array = this.diagram.tokenizer.to_cairo_segment_array(txt,style);
    var pp = this.diagram.cairo_segment_array_to_pp(segment_array,extent-dx,0,0,factor);
  ...
  ///
  ///
  do_para_item(px,py,lead,txt,extent,factor,style){
    var segment_array = this.diagram.tokenizer.to_cairo_segment_array(txt,style);
    ...
    var line_height = 0;
    for(var i=0; i <= max_seq; ++i){
      line_height += this.do_line(px+dx,py+dy,pp,i,factor,style);
    }
    return line_height;
  }
  ///
  ...
  fence_to_list(style,body){
    ...
    itms.forEach((p,i,arr) => {
      ...
      if(p.type=='UL'){
        line_height += this.do_para_item(0,0,bullet,p.text,this.frame_extent,this.fontsize_factor,style)
      }
      else if(p.type=='OL'){
        let lead = `${i+1}`;
        line_height += this.do_para_item(0,0,lead,p.text,this.frame_extent,this.fontsize_factor,style)
      }
      else if(p.type == '1'){
        ...
        line_height += this.do_para_dl(0,0,p.value,p.text,this.frame_extent,this.fontsize_factor,style)
      }
      else if(p.type == 'CB'){
        if(p.bull.trim().length || this.is_in_list(p.start,check_ss)){
          //filled CB
          line_height += this.do_para_cb(0,0,squarebox,p.text,this.frame_extent,this.fontsize_factor,style)
        }else{
          //empty CB
          line_height += this.do_para_cb(0,0,squareboxo,p.text,this.frame_extent,this.fontsize_factor,style)
        }
      }
      else if(p.type == 'RB'){
        if(p.bull.trim().length || this.is_in_list(p.start,check_ss)){
          //filled CB
          line_height += this.do_para_cb(0,0,circlebox,p.text,this.frame_extent,this.fontsize_factor,style)
        }else{
          //empty CB
          line_height += this.do_para_cb(0,0,circleboxo,p.text,this.frame_extent,this.fontsize_factor,style)
        }
      }
      else {
        line_height += this.do_para_item(0,0,bullet,p.text,this.frame_extent,this.fontsize_factor,style)
