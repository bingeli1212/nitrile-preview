...
`;
  ...
  fence_to_verse(g,ss){
    ...
    if(typeof g.extent === 'number'){
      extent = g.extent;
    }
    var all = [];
    itms.forEach((p,i,arr) => {
      if(p.type=='UL'){
        var [height,cnt] = this.do_list_item(0,dy,bullet,p.text,extent,this.fontsize_factor,g)
        dy += height;
        all.push(cnt);
      }
      else if(p.type=='OL'){
        let lead = `${i+1}`;
        var [height,cnt] = this.do_list_item(0,dy,lead,p.text,extent,this.fontsize_factor,g)
        dy += height;
        all.push(cnt);
      }
      else if(p.type == '1'){
        ...
        var [height,cnt] = this.do_para_list_item_dl(0,dy,p.value,p.text,extent,this.fontsize_factor,g)
        dy += height;
        all.push(cnt);
      }
      else if(p.type == 'CB'){
        if(p.bull.trim().length || this.is_in_list(p.start,check_ss)){
          //filled CB
          var [height,cnt] = this.do_list_item(0,dy,squarebox,p.text,extent,this.fontsize_factor,g)
          dy += height;
          all.push(cnt);
        }else{
          //empty CB
          var [height,cnt] = this.do_list_item(0,dy,squareboxo,p.text,extent,this.fontsize_factor,g)
          dy += height;
          all.push(cnt);
        }
      }
        ...
        if(p.bull.trim().length || this.is_in_list(p.start,check_ss)){
          //filled CB
          var [height,cnt] = this.do_list_item(0,dy,circlebox,p.text,extent,this.fontsize_factor,g)
          dy += height;
          all.push(cnt);
        }else{
          //empty CB
          var [height,cnt] = this.do_list_item(0,dy,circleboxo,p.text,extent,this.fontsize_factor,g)
          dy += height;
          all.push(cnt);
        }
      }
  ...
  fence_to_plaintext(g,ss){
    var text = this.join_para(ss);
    var text = this.uncode(g,text);
    var extent = this.frame_extent;
    if(typeof g.extent === 'number'){
      extent = g.extent;
    }
    var o = this.do_para(0,0,text,extent,this.fontsize_factor,g);
    var [height,cnt] = o;
    height += this.frame_svg_blank;
    return [height,cnt];
  }
