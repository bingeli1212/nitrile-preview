...
`;
  ...
  float_to_itemize(title,label,style,itemize,nblank){
    return this.itemize_to_text(itemize,nblank,0);
  }
  //////////////////////////////////////////////////////////////////////////
  //
  //
  ...
  //
  /////////////////////////////////////////////////////////////////////
  item_dl_to_text(i,item,nblank,nl){
    var o = [];
    ...
    if(item.more && item.more.length){
      item.more.forEach((p) => {
        if(p.lines){
          ...
          o.push(`${this.itemize_to_text(p.itemize,nblank,nl+1)}`);
        }
      });
    }
    o.push(`</dd>`);
  ...
  item_hl_to_text(i,item,nblank,nl){
    var o = [];
    var text = this.uncode(item.style,item.text);
    var cls = (!nblank) ? 'PACK' : 'PARA';
  ...
  item_ol_to_text(i,item,nblank,nl){
    var o = [];
    var text = this.uncode(item.style,item.text);
    var cls = (!nblank) ? 'PACK' : 'PARA';
    if(item.type == 'A'){
  ...
  item_ul_to_text(i,item,nblank,nl){
    var px = 0;
    var py = 0;
    var all = [];
    var o = this.do_plst_item(px,py,this.icon_bullet,item.text,this.frame_extent,this.fontsize_factor,item.style);
    all.push(o);
    if(item.more && item.more.length){
      item.more.forEach((p) => {
        if(p.lines){
          var o = this.untext(p.style,p.lines);
          all.push(o);
        }else if(p.itemize) {
          var o = this.itemize_to_text(p.itemize,nblank,nl+1);
          all.push(o);
        }
      });
    }
  ...
  itemize_to_text(itemize,nblank,nl){
    ...
    switch (bull) {
      case 'DL': {
        items.forEach((item,j) => {
          let o = this.item_dl_to_text(j,item,nblank,nl);
          all.push(o);
        });
        break;
      }
      case 'HL': {
        items.forEach((item,j) => {
          let o = this.item_hl_to_text(j,item,nblank,nl);
          all.push(o);
        });
        break;
      }
      case 'OL': {
        items.forEach((item,j) => {
          let o = this.item_ol_to_text(j,item,nblank,nl);
          all.push(o);
        });
        break;
      }
      case 'UL': {
        items.forEach((item,j) => {
          let o = this.item_ul_to_text(j,item,nblank,nl);
          all.push(o);
        });
        break;
      }
  ...
  fence_to_plaintext(style,body){
    ...
    var o = this.do_para(0,0,text,this.frame_extent,this.fontsize_factor,style);
    var [height,cnt] = o;
    height += this.frame_svg_blank;
    return [height,cnt];
  }
  ...
  do_plst_item(px,py,bullet,text,extent,factor,style){
    ...
    var [tall_txt,cnt_txt] = this.do_para(px+padding,py,this.uncode(style,text),extent-padding,factor,style);
