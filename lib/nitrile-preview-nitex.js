...
class Server extends NitrilePreviewBase {
  ...
  async process(name){
    var mdfname = `${name.slice(0,name.length-path.extname(name).length)}.md`;
    var texfname = `${name.slice(0,name.length-path.extname(name).length)}.tex`;
    if(mdfname){
    ...
    if(0){
      ...
      var ss = lines.split('\n').map(s => s.trimEnd());//remove \r on win
      var s0 = ss[0];
      var s1 = ss[1];
      const re0 = /^%\s*!TEX\s+program\s*=\s*(.*)$/i;
      const re1 = /^%\s*!TEX\s+root\s*=\s*(.*)$/i;
