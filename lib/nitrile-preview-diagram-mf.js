...
class NitrilePreviewDiagramMF extends NitrilePreviewDiagram {
  ...
  p_subtitle(opt,w,h,txt,g){
    ...
    if(g.math){
      ...
      if(opt=='sw'){
        o.push(`draw one shifted (+halfw,+halfh+2pt) shifted (${this.fix(0)}*u,${this.fix(0)}*u) ${this.to_texts(g)};`);
      }else if(opt=='se'){
        o.push(`draw one shifted (-halfw,+halfh+2pt) shifted (${this.fix(w)}*u,${this.fix(0)}*u) ${this.to_texts(g)};`);
      }else{
