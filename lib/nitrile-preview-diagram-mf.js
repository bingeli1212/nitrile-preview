...
class NitrilePreviewDiagramMF extends NitrilePreviewDiagram {
  ...
  do_output() {
    ...
    if (this.background=='grid'){ 
      ...
      all.push(`for i=0 step ${grid} until ${ym}: draw (0,i*u) --- (${xm}*u,i*u) withpen pencircle scaled (0.5) withcolor transparent(1,${gridalpha},black); endfor;`);
      all.push(`for i=0 step ${grid} until ${xm}: draw (i*u,0) --- (i*u,${ym}*u) withpen pencircle scaled (0.5) withcolor transparent(1,${gridalpha},black); endfor;`);
    } 
    all.push(s);
    all.push(`% <-- all done -->`);
