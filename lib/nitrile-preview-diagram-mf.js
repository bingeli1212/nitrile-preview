...
class NitrilePreviewDiagramMF extends NitrilePreviewDiagram {
  ...
  to_diagram(style,ss){
    ...
    this.stretch = this.g_to_stretch_float(style);
  ...
  do_output() {
    ...
    if (this.background=='grid'){ 
      ...
      all.push(`for i=0 step ${grid} until ${ym}: draw (0,i*u) --- (${xm}*u,i*u) withpen pencircle scaled (0.5) withcolor transparent(1,${gridopacity},black); endfor;`);
      all.push(`for i=0 step ${grid} until ${xm}: draw (i*u,0) --- (i*u,${ym}*u) withpen pencircle scaled (0.5) withcolor transparent(1,${gridopacity},black); endfor;`);
    } 
    all.push(s);
    all.push(`% <-- all done -->`);
  ...
  p_subtitle(opt,w,h,txt,g){
    ...
    if(1){
      ...
      if(opt=='sw'){
        o.push(`draw one shifted (+halfw+2pt,+halfh+2pt) shifted (${this.fix(0)}*u,${this.fix(0)}*u) ${this.to_texts(g)};`);
      }else if(opt=='se'){
        o.push(`draw one shifted (-halfw-2pt,+halfh+2pt) shifted (${this.fix(w)}*u,${this.fix(0)}*u) ${this.to_texts(g)};`);
      }else{
        o.push(`draw one shifted (0,+halfh+2pt) shifted (${this.fix(w/2)}*u,${this.fix(0)}*u) ${this.to_texts(g)};`);
      }
      var img = o.join(' ');
    ...
    if(0){
      ...
      var o = [];
      if(opt=='sw'){
        o.push(`pair a ; a := (${this.fix(0)}*u,${this.fix(0)}*u) ;`);
        o.push(`label.urt("${textext}",a) ;`);
      }else if(opt=='se'){
        o.push(`pair a ; a := (${this.fix(w)}*u,${this.fix(0)}*u) ;`);
        o.push(`label.ulft("${textext}",a) ;`);
      }else{
        o.push(`pair a ; a := (${this.fix(w/2)}*u,${this.fix(0)}*u) ;`);
        o.push(`label.top("${textext}",a) ;`);
      }
      var img = o.join(' ')
      var img = `draw image ( ${img} ) ;`;
    }
  ...
  p_image(x,y,w,h,ta,label,g){
    let srcs = this.string_to_array(label);
