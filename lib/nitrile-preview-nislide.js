...
function canvas_redraw(cx,canvas){
  if(canvas_is_visible(cx,canvas) && cx.d){
    if(cx.d.balls){
      canvas_draw_balls(cx,canvas);
    }else if(cx.d.eraser){ 
...
function canvas_draw_balls(cx,canvas){
  ...
  cx.d.balls.forEach((ball) => {
    ...
    if(cx.d.downball===ball){
      //ball_draw_canvas_corners(cx,ball);
      cx.save();
      cx.d.hitcorner = -1;
