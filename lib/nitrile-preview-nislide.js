    ...
    if(1){
      ...
      frame.solutions.forEach((o,i,arr) => {
        if(o.choice){
          ...
          all.push(`<div class='BODY' style='${css("BODY")}' > ${this.icon_writinghand} <i>${this.uncode(o.style,o.title)}</i> ${text} </div>`);
        }else{
          all.push(`<div class='BODY' style='${css("BODY")}' > ${this.icon_writinghand} <i>${this.uncode(o.style,o.title)}</i> </div>`);
        }
      });
      var text2 = all.join('\n');
    }
