...
.FIGURE, .MULTICOLS, .LINES, .LISTING, .EQUATION, .TABLE {
  margin-left: 0pt;
  margin-right: 0pt;
}
.TITLE {
...
class NitrilePreviewSlide extends NitrilePreviewHtml {
  ...
  write_frame_folder(id,frame,subframes){
    ...
    if(1){
      ...
      subframes.forEach((subframe,j,arr) => {
        all.push(`<li style='position:relative;font-decoration:underline;'> <span style='position:absolute;left:-2em' > </span> <a style='color:inherit' href='#frame${id}.${j+1}'> ${this.uncode(subframe.style,subframe.title)} </a> </li>`);
      });
      all.push(`</ul>`);
      all.push('</article>');
      all.push('');
  ...
  write_one_frame(id,order,frame,issub,topframe){
    ...
    if(issub){
      ...
      all.push(`<h2 class='TITLE2' style='${css_subtitle};text-decoration:underline' > ${this.uncode(frame.style,frame.title)} </h2>`);
    }else{
      all.push(`<h1 class='TITLE'    style='${css_title}' > ${order} ${this.uncode(topframe.style,topframe.title)} </h1>`);
      all.push(`<h2 class='TITLE2' style='${css_subtitle}' > &#160; </h2>`);
    }
    ...
    var text2 = '';
    if(x1){
      let bundle = x1.bundles[0];
      if(bundle){
        let itm = this.do_bundle(x1.style,x1.bundles[0]);
        if(itm.img){
          text1 = itm.img;
        }else if(itm.tab){
          text1 = itm.tab;
    ...
    if(1){
      ...
      frame.solutions.forEach((o,i,arr) => {
        if(o.choice){
          ...
          all.push(`<div class='PARAGRAPH' style='${css_paragraph}' > ${this.icon_writinghand} <i>${this.uncode(o.style,o.title)}</i> ${text} </div>`);
        }else{
          all.push(`<div class='PARAGRAPH' style='${css_paragraph}' > ${this.icon_writinghand} <i>${this.uncode(o.style,o.title)}</i> </div>`);
        }
      });
      var text2 = all.join('\n');
    }
    if(text1){
      ...
      all.push('<div style="break-after:column;">');
      all.push(text2);
      all.push('</div>');
      all.push(text1);
      all.push('</div>');
...
`;
  ...
  write_frame_folder(id,frame,subframes){
    ...
    subframes.forEach((subframe,j,arr) => {
      ...
      ///'o' is blocks
      all.push(`\\item \\underline{${this.uncode(subframe.style,subframe.title)}} `);
      if(j+1==arr.length){
        all.push('\\end{list}')
      }
    });  
  ...
  write_one_frame(id,order,frame,issub,topframe) {
    ...
    if(issub){
      ...
      all.push(`\\framesubtitle{\\footnotesize\\underline{${this.uncode(frame.style,frame.title)}}}`);
    }else{
      all.push(`\\frametitle{${order} ${this.uncode(topframe.style,topframe.title)}}`);    
      all.push(`\\framesubtitle{~}`);
    }
...
class NitrilePreviewServer extends NitrilePreviewBase {
  ...
  async run(){
    ...
    if(path.extname(fname)==='.md'){
      ...
      }else if(parser.program=='pdflatex'||parser.program=='xelatex'||parser.program=='lualatex'){
        var texfname = `${fname.slice(0,fname.length-path.extname(fname).length)}.tex`;
