...
console.log("canvas_onfocusin called");
  var ctx = canvas.getContext('2d');
...
class NitrilePreviewSlide extends NitrilePreviewHtml {
  ...
  write_frame_folder(id,frame,subframes){
    ...
    all.push(`<h1 class='TITLE'    style='${css_title}' > <a style='text-decoration:none;color:inherit;' href='#toc'> ${id} ${this.uncode(frame.style,frame.title)} </a> </h1>`);
  ...
  write_one_frame(id,subid,frame,issub,topframe){
    ...
    if(subid){
      all.push(`<h1 class='TITLE'    style='${css_title}' > <a style='text-decoration:none;color:inherit;' href='#frame${id}.'> ${id} ${this.uncode(topframe.style,topframe.title)} </a> </h1>`);
      all.push(`<h2 class='TITLE2' style='${css_subtitle};text-decoration:underline' > <a style='text-decorationb:none;color:inherit;' href='#frame${id}.'> ${id}.${subid} ${this.uncode(frame.style,frame.title)} </a> </h2>`);
    }else{
      all.push(`<h1 class='TITLE'    style='${css_title}' > <a style='text-decoration:none;color:inherit;' href='#toc'> ${id} ${this.uncode(topframe.style,topframe.title)} </a> </h1>`);
      all.push(`<h2 class='TITLE2' style='${css_subtitle}' > &#160; </h2>`);
    }
    //
    ...
    if(wrap_contents.length){
      ...
      wrap_contents.forEach((x,i) => {
        ...
        bundles.forEach((bundle) => {
          if(x.id=='wrapfig'){
            //bundle.g.stretch = 0.9; // this cannot be done here because it affects all future this bundle when it is being restored
            let p = this.do_bundle(x.style,bundle,'img');
            all.push(`<div style='text-align:center'>`)
            all.push(p.img);
            all.push(`</div>`)
...
console.log("color=",v[1]);
    ...
    return super.fence_to_img(style,ss,ssi);
  }
}
////////////////////////////////////////////////////////////////////////////
// require.main === module
