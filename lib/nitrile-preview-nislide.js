...
class NitrilePreviewSlide extends NitrilePreviewHtml {
  ...
  write_one_frame(id,subid,frame,issub,topframe){
    ...
    if(frame.contents.length && frame.contents[0].id=='wrapfig'){
      ...
      frame.contents.slice(1).forEach((x,i) => {
        ///'x' is a block
        let html = this.translate_block(x);
        all.push(html.trim());
      })       
      frame.solutions.forEach((o,i,arr) => {
        if(o.choice){
          ...
          all.push(`<div class='BODY' style='${this.css("BODY")}' > <u>${this.uncode(o.style,o.title)}</u> ${text} </div>`);
        }else{
          all.push(`<div class='BODY' style='${this.css("BODY")}' > <u>${this.uncode(o.style,o.title)}</u> </div>`);
        }
      });
      all.push(`<div style='break-after:column'></div>`);
      frame.contents.slice(0,1).forEach((x,i) => {
        //'x' is a block
      ...
      all.push(`</div>`)
    }else if(frame.contents.length && frame.contents[0].id=='wraptab'){
      all.push(`<div style='column-count:2'>`);
      frame.contents.slice(1).forEach((x,i) => {
        ///'x' is a block
        let html = this.translate_block(x);
