...
const mysvg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 14 14"> <polyline stroke="currentColor" stroke-width="3" fill="none" points="7 1 13 7 7 13"/> <line x1="1" y1="7" x2="13" y2="7" stroke="currentColor" stroke-width="3" /> </svg>`; 
...
function form_onsubmit(form){
  ...
  var palette = document.getElementById(imgid+"-palette");
  event.preventDefault();
  if(img && img.src){
...
class NitrilePreviewSlide extends NitrilePreviewHtml {
  ...
  constructor(parser) {
    ...
    //this.icon_writinghand = '&#x270D;'//WRITING HAND
  ...
  write_one_frame(id,order,frame,issub,topframe){
    ...
    if(x1){
      ...
      if(bundle){
        ...
        var text1 = itm.text;
        if(itm.key=='dia'||itm.key=='ink'){
          var text1 = `<span style="display:inline-block;border:thin solid currentColor;width:100%;">${text1}</span>`;
        }
      }
    }
    if(1){
