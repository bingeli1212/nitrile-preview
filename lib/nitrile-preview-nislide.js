...
class NitrilePreviewSlide extends NitrilePreviewHtml {
  ...
  write_frame_toc(topframes){
    ...
    topframes.forEach((topframe,j,arr) => {
      ...
      let order = j+1;
      all.push(`<li style='position:relative;' > <span style='position:absolute;left:-2.0em;' > ${order} </span> <a style='color:initial' href='#frame${j+1}'> ${this.uncode(topframe.style,topframe.title)} </a> </li>`);
      n++;
    });
    all.push(`</ul>`);
    all.push('</article>');
  ...
  write_frame_folder(id,frame,subframes){
    ...
    if(1){
      ...
      subframes.forEach((subframe,j,arr) => {
        ...
        icon = '';
        all.push(`<li style='position:relative;font-decoration:underline;'> <span style='position:absolute;left:-2em' > ${icon} </span> <a style='color:initial' href='#frame${id}.${j+1}'> ${this.uncode(subframe.style,subframe.title)} </a> </li>`);
      });
      all.push(`</ul>`);
      all.push('</article>');
      all.push('');
