...
const mysvg = `<svg width="12" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 14 14"> <polygon stroke="none" fill="#1010B0" points="1 1 1 13 13 7"/> </svg>`; 
...
function form_onsubmit(form){
  ...
  if(imgsrc){
    ...
    var png = new URL(imgsrc,document.baseURI).pathname;
...
function canvas_onmouseleave(canvas){
  var cx = canvas.getContext("2d");
  canvas.style.cursor = '';
  if(cx.d && cx.d.mouseisdown){
    canvas_end(cx,canvas);
    cx.d.mouseisdown = 0;
  }
}
function canvas_onmousedown(canvas){
console.log('canvas_onmousedown');
  var cx = canvas.getContext("2d");
  if(event.button==0){
    cx.d.posx0 = Math.round((event.clientX-canvas.getBoundingClientRect().x));
    cx.d.posy0 = Math.round((event.clientY-canvas.getBoundingClientRect().y));
    cx.d.posx1 = cx.d.posx0;
    cx.d.posy1 = cx.d.posy0;
    cx.d.posx2 = cx.d.posx1;
    cx.d.posy2 = cx.d.posy1;
    cx.d.shiftKey = event.shiftKey;
    cx.d.altKey   = event.altKey;
    cx.d.mouseisdown = 1;
    cx.d.mouseisdragged = 0;
    cx.d.mousedragcount = 0;
    cx.d.mouseevent = 'mousedown';
    canvas_start(cx,canvas);
  }else{
    canvas_cancel(cx,canvas);
  }
  if(event.button!=0){
    event.preventDefault();
  }
  cx.d.hitball = cx.d.hiliteball;
  canvas_redraw(cx,canvas);
}
function canvas_onmousemove(canvas){
  var cx = canvas.getContext("2d");
  cx.d.posx1 = cx.d.posx2;
  cx.d.posy1 = cx.d.posy2;
  cx.d.posx2 = Math.round((event.clientX-canvas.getBoundingClientRect().x));
  cx.d.posy2 = Math.round((event.clientY-canvas.getBoundingClientRect().y));
  cx.d.shiftKey = event.shiftKey;
  cx.d.altKey   = event.altKey;
  cx.d.mouseevent = 'mousemove';
  if(event.button==0 && cx.d && cx.d.mouseisdown){
    cx.d.mouseisdragged = 1;
    cx.d.mousedragcount += 1;
    canvas_drag(cx,canvas);
  }else{
    cx.d.mouseisdragged = 0;
    canvas_move(cx,canvas);
  }
}
function canvas_onmouseup(canvas){
  var cx = canvas.getContext("2d");
  cx.d.mouseevent = 'mouseup';
  if(event.button==0 && cx.d && cx.d.mouseisdown){
    canvas_end(cx,canvas);
    cx.d.mouseisdown = 0;
    cx.d.mouseisdragged = 0;
...
function canvas_ontouchstart() {
  ...
  for (var i = 0; i < touches.length; i++) {
    ...
    var cx = canvas.getContext("2d");
    cx.d.posx0 = Math.round((touch.clientX-canvas.getBoundingClientRect().x));
    cx.d.posy0 = Math.round((touch.clientY-canvas.getBoundingClientRect().y));
    cx.d.posx1 = cx.d.posx0;
    cx.d.posy1 = cx.d.posy0;
    cx.d.posx2 = cx.d.posx1;
    cx.d.posy2 = cx.d.posy1;
    cx.d.mouseisdown = 1;
    cx.d.shiftKey = evt.shiftKey;
    cx.d.altKey   = evt.altKey;
    if(cx.d.mouseisdown){
      cx.d.mouseisdragged = 0;
      cx.d.mousedragcount = 0;
      canvas_start(cx,canvas);
...
function canvas_ontouchmove(canvas) {
  ...
  for (var i = 0; i < touches.length; i++) {
    ...
    cx.d.posx1 = cx.d.posx2;
    cx.d.posy1 = cx.d.posy2;
    cx.d.posx2 = Math.round((touch.clientX-canvas.getBoundingClientRect().x));
    cx.d.posy2 = Math.round((touch.clientY-canvas.getBoundingClientRect().y));
    cx.d.shiftKey = evt.shiftKey;
    cx.d.altKey   = evt.altKey;
    if(cx.d.mouseisdown){
      cx.d.mouseisdragged = 1;
...
function canvas_cancel(cx,canvas){
  console.log('canvas_cancel');
  if(cx.d.type=='ball'){
    if(cx.d.moveflag){
      cx.d.moveflag = 0;
      cx.d.movedistx = 0;
...
function canvas_start(cx,canvas){
  console.log('canvas_start');
  if(cx.d.type=='image'){
    cx.d.s = cx.getImageData(0,0, canvas.width,canvas.height);
    cx.d.hobby_p = [];
    var x = cx.d.posx0;
    var y = cx.d.posy0;
    cx.d.hobby_p.push([x,y]);
    canvas_redraw(cx,canvas);
  }
  if(cx.d.type=='ball'){
    if(cx.d.moveflag){
      ...
      cx.d.movedistx = 0;
      cx.d.movedisty = 0;
      if(cx.d.hit_transform){
        if(cx.d.hitball){
          cx.d.hitball.transform[0] = cx.d.hit_transform[0];
          cx.d.hitball.transform[1] = cx.d.hit_transform[1];
          cx.d.hitball.transform[2] = cx.d.hit_transform[2];
          cx.d.hitball.transform[3] = cx.d.hit_transform[3];
          cx.d.hitball.transform[4] = cx.d.hit_transform[4];
          cx.d.hitball.transform[5] = cx.d.hit_transform[5];
        }
      }
    }
    canvas_redraw(cx,canvas);
...
function canvas_move(cx,canvas){
  if(cx.d && cx.d.type=='ball'){
    if(cx.d.moveflag){
      cx.d.movedistx = cx.d.posx2 - cx.d.posx0;
      cx.d.movedisty = cx.d.posy2 - cx.d.posy0;
      canvas_redraw(cx,canvas);
    }
  }
}
...
function canvas_init(cx,canvas){
  cx.d = {};
  ...
  if(type=='image'){
    ...
    if(1){
      ...
      var type = 'rect';
      var ball = {points,transform,type};
      cx.d.balls.push(ball); 
...
function canvas_redraw(cx,canvas){
  if(cx.d.type=='ball'){
    canvas_redraw_ball(cx,canvas);
  }else if(cx.d.type=='image'){
    if(cx.d.eraser){ 
      canvas_draw_eraser(cx,canvas);
    }else if(cx.d.smudge){
      canvas_draw_smudge(cx,canvas);
    }else if(cx.d.pencil){
...
function canvas_draw_eraser(cx,canvas){
  ...
  cx.beginPath();
  cx.moveTo(cx.d.posx1,cx.d.posy1);
  cx.lineTo(cx.d.posx2,cx.d.posy2);
  cx.stroke();
  cx.restore();
} 
function canvas_draw_pencil(cx,canvas){
  ...
  cx.d.hobby_p.push([x,y]);
  var tension = 1;
  var knots = makeknots(cx.d.hobby_p, tension, false);
  mp_make_choices(knots[0]);
  cx.putImageData(cx.d.s,0,0);
  cx.beginPath();
...
function canvas_draw_smudge(cx,canvas){
  var colorinputid = canvas.getAttribute('data-colorinputid');
  var colorinput = document.getElementById(colorinputid);
  ...
  do_smudge_between(cx.d.posx1,cx.d.posy1,cx.d.posx2,cx.d.posy2,5,mysmear,cx,canvas);
}
function do_smudge_between(posx0,posy0,posx,posy,lw,mysmear,cx,canvas){
  let dx = posx-posx0;
  let dy = posy-posy0;
  let Dx = Math.abs(dx);
  let Dy = Math.abs(dy);
  ...
  for(i=0; i <= n; ++i){
    let x = posx0 + dx*i;
    let y = posy0 + dy*i;
    do_smudge_rect(x-(lw/2),y-(lw/2),lw,lw,mysmear,cx,canvas);
  }
}
function do_smudge_rect(x,y,w,h,mysmear,cx,canvas){
  ...
  if(w && h){
    ...
    if(x >= 0 && y >= 0 && x < width && y < height){
      var idata = cx.getImageData(x,y,w,h);
      const cutoff = 64;
...
function canvas_oncolorchange(input){
  ...
  var cx = canvas.getContext('2d');
  if(cx.d){
    cx.d.color = input.value;
  }
}
function canvas_redraw_ball(cx,canvas){
  var hiliteball = null;
  cx.clearRect(0,0,canvas.width,canvas.height);
...
console.log(a,b,c,d,e,f);
    ...
    if(ball.rotateflag){
console.log('1');
      cx.translate(ball.rotatex0,ball.rotatey0);
      cx.rotate(ball.rotateangle);
      cx.translate(-ball.rotatex0,-ball.rotatey0);
    }else if(cx.d.hitball==ball && cx.d.moveflag){
console.log('2');
    ...
    if(cx.d.hitball==ball){
      ...
      let e = matrix.e;
      let f = matrix.f;
      cx.d.hit_transform = [a,b,c,d,e,f];
    }
    cx.beginPath();
...
console.log('M',x1,y1);
    ...
    if(ball.type=='line'||ball.type=='polyline'){
      var hitflag = cx.isPointInStroke(cx.d.posx2,cx.d.posy2);
    }else{
      var hitflag = cx.isPointInPath(cx.d.posx2,cx.d.posy2);
    }
    if(hitflag){
      hiliteball = ball;
    }
...
.CANVASSUBMIT, .CANVASRESET, .CANVASCOLOR {
  width:20px;
  height:20px;
  padding:0;
  margin:0;
...
.FIGURE, .MULTICOLS, .ALIGNMENT, .LISTING, .EQUATION, .TABLE {
  margin-left: 0pt;
  margin-right: 0pt;
}
.TITLE {
