...
class NitrilePreviewSlide extends NitrilePreviewHtml {
  ...
  write_one_frame(id,subid,frame,issub,topframe){
    ...
    //NOTE: main contents
    //
    all.push(`<article class='SLIDE' id='frame${id}.${subid}' data-row='${frame.style.row}' style='display:flex;flex-direction:column;' >`);
    if(subid){
    ...
    var x1 = null 
    var xs2 = [];
    if(frame.contents.length && (frame.contents[0].id=='wrapfig' || frame.contents[0].id=='wraptab')){
    ...
    if(1){
      ...
      frame.solutions.forEach((o,i,arr) => {
        if(o.choice){
          ...
          all.push(`<div class='BODY' style='${this.css("BODY")}' > ${this.icon_dag} <i>${this.uncode(o.style,o.title)}</i> ${text} </div>`);
        }else{
          all.push(`<div class='BODY' style='${this.css("BODY")}' > ${this.icon_dag} <i>${this.uncode(o.style,o.title)}</i> </div>`);
        }
      });
      var text2 = all.join('\n');
    }
  ...
  to_titlepagelines(){
    ...
    lines.push(`<div style='${this.css("FRONTMATTERTITLE")}' >${this.smooth(this.style,title)}</div>`);
    lines.push(`<div style='${this.css("FRONTMATTERSUBTITLE")}' >${this.smooth(this.style,subtitle)}</div>`);
    lines.push(`<div style='${this.css("FRONTMATTERINSTITUTE")}' >${this.smooth(this.style,institute)}</div>`);
    lines.push(`<div style='${this.css("FRONTMATTERAUTHOR")}' >${this.smooth(this.style,author)}</div>`);
    lines.push(`<div style='${this.css("FRONTMATTERDATE")}' >${this.smooth(this.style,date)}</div>`);
    lines.push(`</article>`);
    return lines;
  }
  ...
  write_one_frame(id,subid,frame,issub,topframe) {
    ...
    if(1){
      ...
      frame.solutions.forEach((o,i,arr) => {
        if(o.choice){
          let text = this.to_choice(o.style,o.body);
          all.push(`${this.icon_dag} {${this.uncode(o.style,o.title)}} `);
    ...
    if(1){
      ...
      if(1){
        ...
        frame.solutions.forEach((o,i,arr) => {
          all.push('')
          if(o.choice){
            all.push(`{${this.icon_dag}} {\\it ${this.uncode(o.style,o.title)}}\\crlf`);
            let text = this.to_choice(o.style,o.body,'',false);
            all.push(text);
          }else{
            all.push(`{${this.icon_dag}} {\\it ${this.uncode(o.style,o.title)}}`);
          }
        })
        var text2 = all.join('\n');
      }
      if(text1){
        all.push('');
        all.push(`\\placevignette[right]{}{${text1}}`);
        all.push(text2);
      }else{
        all.push('\\vfill');
        all.push('');
