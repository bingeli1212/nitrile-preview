...
function canvas_draw_smudge(cx,canvas){
  ...
  smudge_between(cx.cake.posx1,cx.cake.posy1,cx.cake.posx2,cx.cake.posy2,5,mysmear,cx,canvas);
}
function smudge_between(posx0,posy0,posx,posy,lw,mysmear,ctx,canvas){
  ...
  for(i=0; i <= n; ++i){
    let x = posx0 + dx*i;
    let y = posy0 + dy*i;
    smudgeRect(x-(lw/2),y-(lw/2),lw,lw,mysmear,ctx,canvas);
  }
}
function smudgeRect(x,y,w,h,mysmear,ctx,canvas){
...
.FIGURE, .MULTICOLS, .ALIGNMENT, .LISTING, .EQUATION, .LONGTABU {
  margin-left: 0pt;
  margin-right: 0pt;
}
.TITLE {
...
class NitrilePreviewServer extends NitrilePreviewBase {
  ...
  async run(){
    ...
    if(!fname){
      ...
      var title = this.uncode(this.parser.style,title);
      var xhtml = `\
