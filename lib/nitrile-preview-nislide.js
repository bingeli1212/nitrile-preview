...
`;
  ...
  write_one_frame(id,subid,frame,issub,topframe) {
    ...
    //
    //NOTE: check to see if the first block is a IMG block with wrapfig:right or wrapfig:left
    //
    frame.contents.forEach((x,i) => {
      //'x' is a block
      let latex = this.translate_block(x);
...
`;
  ...
  write_one_frame(id,subid,frame,issub,topframe) {
    ...
    if(1){
      ...
      var x1 = null 
      var xs2 = [];
      if(0&&frame.contents.length && (frame.contents[0].id=='wrapfig' || frame.contents[0].id=='wraptab')){
