...
class NitrilePreviewSlide extends NitrilePreviewHtml {
  ...
  write_frame_toc(topframes){
    ...
    topframes.forEach((topframe,j,arr) => {
      ...
      let order = j+1;
      all.push(`<li style='position:relative;' > <span style='position:absolute;left:-2.0em;' > ${order} </span> <a style='color:inherit' href='#frame${j+1}.'> ${this.polish(topframe.style,topframe.title)} </a> </li>`);
      n++;
    });
    all.push(`</ul>`);
    all.push('</article>');
  ...
  write_frame_folder(id,frame,subframes){
    ...
    all.push(`<h1 class='TITLE'    style='${css_title}' > <a style='text-decoration:none;color:inherit;' href='#toc'> ${id} ${this.polish(frame.style,frame.title)} </a> </h1>`);
    ...
    if(1){
      ...
      subframes.forEach((subframe,k,arr) => {
        all.push(`<li style='position:relative;font-decoration:underline;'> <span style='position:absolute;left:-2em' > </span> <a style='color:inherit' href='#frame${id}.${k+1}'> ${id}.${k+1} ${this.polish(subframe.style,subframe.title)} </a> </li>`);
      });
      all.push(`</ul>`);
      all.push('</article>');
      all.push('');
