...
\\stoptext`;
  ...
  to_titlepage(){
    let fname = this.parser.data['champion.titlepage'];
    if(fname){
      const fs = require('fs');
...
{\\tfd ${this.uncode(this.parser.style,mytitle)}}
\\blank[2cm]
{\\tfa ${this.uncode(this.parser.style,myaddr)}}
\\blank[0.2cm]
{\\tfa ${this.uncode(this.parser.style,myauthor)}}
\\blank[0.2cm]
{\\tfa ${this.uncode(this.parser.style,mydate)}}
\\blank[0.2cm]
\\stopalignment
\\stopstandardmakeup`;
  }
  to_frontmatter(){
    let fname = this.parser.data['champion.frontmatter'];
    if(fname){
      const fs = require('fs');
      let lines = fs.readFileSync(fname,"utf8");
      return lines;
...
\\completecontent`;
  ...
  to_preamble(){
    let fname = this.parser.data['champion.preamble'];
    if(fname){
      const fs = require('fs');
      let lines = fs.readFileSync(fname,"utf8");
      return lines;
  ...
  to_appendices(){
    let fname = this.parser.data['champion.appendices'];
    if(fname){
      const fs = require('fs');
      let lines = fs.readFileSync(fname,"utf8");
      return lines;
