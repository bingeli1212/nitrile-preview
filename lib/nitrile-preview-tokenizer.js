...
class NitrilePreviewTokenizer extends NitrilePreviewBase {
  ...
  constructor (translator) {
    ...
    this.build_reverse_symbol_map(this.cc_symbol_map,this.tex_symbol_map,this.cex_symbol_map);
  }
  ...
  findIdByElement (v,used) {
      ...
      if( (typeof v === 'string' && v.startsWith('\\')) 
        ...
        if(p){
          ...
          if(this.iscex && p.cex){
            ...
            if(p.cex.startsWith('\\')){
              tex = p.cex;
            }else if(p.cex=='unicode'){
              if(p.html.startsWith('&#x')){
                let cc = p.html.slice(3,-1);
                cc = `0x${cc}`;
                tex = String.fromCodePoint(cc);
              }else{
                let cc = p.html.slice(2,-1);
                tex = String.fromCodePoint(cc);
              }            
            }
          }
    ...
    if(this.cc_symbol_map.has(cc)){
      let {tex,html,width,op,id} = this.cc_symbol_map.get(cc);
      width = this.get_width_for_char(width,html);
      width *= this.fs/12;
      op = op||0;
      used.add(id);
  ...
  // in 'smooth()' function
  build_reverse_symbol_map(cc_symbol_map,tex_symbol_map,cex_symbol_map){
