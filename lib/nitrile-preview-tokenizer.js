...
class NitrilePreviewTokenizer extends NitrilePreviewBase {
  ...
  to_svgmath (math,style,from) {
    ...
    var results = top.map( (z) => {
      var used = new Set();
      var pref = {...style, compact:0, frac:0};
      let [w,h,mid,s,g_,q_,last_id,shiftdist] = this.toInnerSvg(z,used,pref); 
      var defs = this.buildup_defs(used);
      s = this.to_outer_svg(s,w,h,mid,defs,from,pref);
      return {w,h,mid,s,defs,last_id,shiftdist};
    });
    if(top.length==1){       
      var used = new Set();
      var pref = {...style, compact:0, frac:0};
      let [w,h,mid,s,g_,q_,last_id,shiftdist] = this.toInnerSvg(top[0],used,pref); 
      var defs = this.buildup_defs(used);
      s = this.to_outer_svg(s,w,h,mid,defs,from,pref);
      return {w,h,mid,s};    
    }else{
      ...
      var s = o.join('\n')
      var defs = this.buildup_defs(used);
      var s = this.to_outer_svg(s,w,h,mid,defs,from,style);
      return {w,h,mid,s};
    }
  }
  ...
  to_innersvg_math(math,used,style){
    ...
    ///change the top to '\\math' so that it will be added an extra width to the right if the last element is a protruded symbol
    l[0] = '\\math';
