...
class NitrilePreviewTokenizer extends NitrilePreviewBase {
        ...
        case '\\leftright':
          ...
          if(v[0]==='\\leftright'){
            ...
            if(v[0] === '\\pipe' || v[0] === '\\math'){
              let protrude = this.get_protrude_by_id(this.my_last_id);
              w += protrude;//increase by 2pt to ensure that the right-hand side of last letter symbols aren't clipped off
              var defs = this.buildup_defs(used);
              var type = v[0];
  ...
  to_frac(m1,m2,frac,isline){
    ...
    o.push(`<g transform='translate(${bot_x*1.333},${bot_y*1.333}) scale(${reduce_rate})'> ${bot_s} </g>`);
    if(isline){
      o.push(`<line x1='0pt' y1='${top_h}pt' x2='${w}pt' y2='${top_h}pt' style='stroke:inherit; fill:none; font-size:${this.fs}pt;'/>`);
    }
    var newv = [w,h,mid,o.join('\n'),0,0,''];
    return newv;
  }
