...
class NitrilePreviewTokenizer extends NitrilePreviewBase {
  ...
  constructor (translator) {
    ...
    this.build_all_symbol_maps(this.name_symbol_map,this.cc_symbol_map);
  }
  ...
  findIdByElement (v,used) {
    ...
    if(v){
      ...
      if(this.cc_symbol_map.has(cc)){
        let {id,width,op,html,name} = this.cc_symbol_map.get(cc);
        width = this.get_width_for_char(width,html);
        op = op||0;
        used.add(id);
        let lmath = this.get_lmath_symbol(name);
  ...
  // in 'smooth()' function
  build_all_symbol_maps(name_symbol_map,cc_symbol_map){
    for (let name in pjson.symbol) {
      ...
      let id = `my_${name}`;
      if(cc && cc > 0x7F){
        cc_symbol_map.set(cc,{tex,cex,lmath,cmath,html,id,op,width,name});
      }
      if(1){
  ...
  get_unicode_width(cc){
    ...
    if(this.cc_symbol_map.has(cc)){
      let width = this.cc_symbol_map.get(cc).width;
      if(width){
        return width;
      }
