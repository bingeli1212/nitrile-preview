...
class NitrilePreviewTokenizer extends NitrilePreviewBase {
  ...
  constructor (translator) {
    ...
    this.bracelevel = 0;
    this.re_svg = /^<svg\b([^<>]*>)(.*)$/s;
    this.re_span = /^<span\b([^<>]*>)(.*)$/s;
    this.re_entity = /^(&#x[0-9A-Za-z]+;|&#[0-9]+;)(.*)$/s;
    this.re_segment = /^(\\;|\\,|\\:|\\begin\{\w+\}|\\end\{\w+\}|\\text\{.*?\}|\\hrule\{.*?\}|&[a-zA-Z][a-zA-Z0-9]*;|\\[a-zA-Z]+|'+|.)(.*)/s;///this needs to have a 's' because the SVG's could have newline in it and without it only part of the SVG will be captured in the second argument
    this.re_firstchar = /^(.)(.*)$/s;
    this.re_begin = /^\\begin\{(\w+)\}$/;
    this.re_end = /^\\end\{(\w+)\}$/;
    this.re_text_brace = /^\\text\{(.*)\}$/;
    this.re_hrule_brace = /^\\hrule\{(.*)\}$/;
    ...
    this.name_symbol_map = new Map();
    this.cc_symbol_map = new Map();
    this.tex_symbol_map = new Map();
    this.cex_symbol_map = new Map();
    this.build_all_symbol_maps(this.name_symbol_map,this.cc_symbol_map,this.tex_symbol_map,this.cex_symbol_map);
  }
  ...
  to_svgmath (math,style,from) {
    ...
    if(top.length==1){       
      ...
      var shiftdist = 0;
      s = this.to_outer_svg(s,w,h,mid,defs,from,pref);
  ...
  findIdByEntity(v,used){
    for (var symb in pjson.symbol) {
      let {html,width,op} = pjson.symbol[symb];
  ...
  findIdByElement (v,used) {
    ...
    if(v){
      ...
      if( (typeof v === 'string' && v.startsWith('\\')) 
        ...
        if(this.name_symbol_map.has(name)){
          let {id,width,op,html} = this.name_symbol_map.get(name);
          width = this.get_width_for_char(width,html);
          op = op||0;
          used.add(id);
          let lmath = this.get_lmath_symbol(name);
          let cmath = this.get_cmath_symbol(name);
          return [id,width,op,lmath,cmath];
        }
      }
      // find in the cc_symbol_map using 'cc' as index
      // because of the input could be a Unicode character
      let cc = v.codePointAt(0);
      if(this.cc_symbol_map.has(cc)){
        let {id,width,op,html,lmath,cmath} = this.cc_symbol_map.get(cc);
        width = this.get_width_for_char(width,html);
        op = op||0;
        used.add(id);
        return [id,width,op,lmath,cmath];
  ...
  to_math_segment_array(str){
    ...
    while(str.length){
      ...
      if((v=this.re_span.exec(str))!==null){
        var hdr = v[1];
        str = v[2];
        var [s,str] = this.extract_span_element(str,1);
        var p = ['\\text',s];
        o.push(p);
        str = str.trimLeft();
        continue;
  ...
  to_cairo_segment_array(str){
    ...
    while(str.length){
      ...
      if((v=this.re_span.exec(str))!==null){
        ...
        str = v[2];
        var [s,str] = this.extract_span_element(str,1);
        let pp = this.to_cairo_segment_array(s);
        for(let p of pp){
          o.push(p);
        }
        continue;
      ...
      if(cc > 0xFFFF){
        //need to extract two characters
        let s = str.substr(0,2)
        str = str.substr(2);
        o.push(['\\cc',cc,0,0,0]);
        continue;
      }else{
        //need to extract one character
        let s = str.substr(0,1);
        str = str.substr(1);
        o.push(['\\cc',cc,0,0,0]);
        continue; 
      }
      ///it should've never got here, but just in case
      break;
    }
    return o;
  }
    ...
    if (v === undefined || v === null) {
      ...
      switch (v[0]) {
        ...
        case '\\hrule':
          ...
          mid = h/2;
          s=(`<line x1='0pt' y1='${dy}pt' x2='${w}pt' y2='${dy}pt' style='stroke:currentColor; fill:none; font-size:${this.fs}pt;'/>`);
          return [w,h,mid,s,0,0,''];
          break;
        ...
        case '\\mathring':
          ...
          var x1 = w/2 + 1.5
          var y1 = this.lower(1.5,id_);
          o.push(`<circle cx='${xm}pt' cy='${ym}pt' r='1.5pt' style='stroke:currentColor; fill:none; font-size:${this.fs}pt;'/>`);
          return [w,h,mid,o.join('\n'),0,0,''];
          break;
        ...
        case '\\dif':
          ...
          if (1) {
            ...
            o.push(`<use fill='currentColor' x='${dx}pt' y='${dy_for_d}pt' xlink:href='#${idd_}'/>`);
            dx += wd_;
        ...
        case '\\pmod':
          ...
          dx += wdlp_;
          dx += 1;
          o.push(`<use fill='currentColor' x='${dx}pt' y='${dy}pt' xlink:href='#${idmod_}' />`);
          dx += wdmod_;
          dx += 2;
        ...
        case '\\dfrac':
          ...
          var compact1 = (pref.displaymath)?0:1;
          var compact2 = (pref.displaymath)?0:1;
        ...
        case '\\dbinom':
          ...
          var [w,h,mid,s,g,q,id] = this.to_frac(m1,m2,0,0);
        ...
        case '\\binom':
          ...
          if(pref.displaymath){
            ...
            var [w,h,mid,s,g,q,id] = this.to_frac(m1,m2,0,1);
          }
          var padding = this.extra_gap_int;//1pt gap
          var o = [];
          var x = 0;
        ...
        case '\\sup':
          ...
          if (!this.isinmatrix && pref.displaymath && (
                  v[1] === '\\sum' || 
                  v[1] === '\\prod' || 
                  v[1] === '\\lim'  )) {
            var o = [];
          ...
          } else if (!this.isinmatrix && pref.displaymath && 
                  v[1] === '\\int' ) {
      ...
      if(this.re_variable.test(v)) {
        let symb = `${v}mathit`;
        var p = pjson.symbol[symb];//this should always work
        let id = 'my_' + symb;
        used.add(id);
        var width = this.get_width_for_char(p.width,p.html);
        var op = p.op||0;
      ...
      if (id === 'myINT') { 
        ...
        var w = width;  
        var dx = 3;
        if(pref.displaymath && !this.isinmatrix){
  ...
  get_tex_symbol(name){
    ...
    if(pjson.symbol[name]){
      ...
      if(v.tex){
        ...
        return `\\ensuremath{${v.lmath}}`;
      }else{
        return '';
        /*
        if(v.html.startsWith('&#x')){
          let cc = v.html.slice(3,-1);
          cc = `0x${cc}`;
          return String.fromCodePoint(cc);
        }else{
          let cc = v.html.slice(2,-1);
          return String.fromCodePoint(cc);
        }
        */
      }
  ...
  get_cex_symbol(name){
    ...
    if(pjson.symbol[name]){
      var v = pjson.symbol[name];
      //TESTING
      if(v.cex && v.cex.length > 0){
        cex = v.cex.trimRight();
        return `{\\switchtobodyfont[]${cex}}`;
      }else if(v.cmath && v.cmath.length > 0){
        cex = v.cmath.trimRight();
        return `\\math{${cex}}`;
      }else{
        if(v.html.startsWith('&#x')){
          let cc = v.html.slice(3,-1);
          cc = `0x${cc}`;
          cex = String.fromCodePoint(cc);
        }else{
          let cc = v.html.slice(2,-1);
          cex = String.fromCodePoint(cc);
        }
        if(v.fonts){
          return `{\\switchtobodyfont[${v.fonts}]${cex}}`
        }else{
          ///interestingly if a unicode character is placed inside a \math{...} then mostly likely it is going to work
          return `\\math{${cex}}`
        }
      }
    }
  ...
  get_cmath_symbol(name){
    ...
    if(pjson.symbol[name]){
      var v = pjson.symbol[name];
      //TESTING
      if(v.cmath && v.cmath.length){
        return v.cmath.trimRight();
      }else if(v.cex && v.cex.length ){
        return `\\text{${v.cex.trimRight()}}`
      }else{
        if(v.html.startsWith('&#x')){
          let cc = v.html.slice(3,-1);
          cc = `0x${cc}`;
  ...
  // in 'smooth()' function
  build_all_symbol_maps(name_symbol_map,cc_symbol_map,tex_symbol_map,cex_symbol_map){
    for (let name in pjson.symbol) {
      let p = pjson.symbol[name];
      let tex   = p.tex;
      let cex   = p.cex;
      let lmath = p.lmath;
      let cmath = p.cmath;
      ...
      let id = `my_${name}`;
      if(cc && cc > 0x7F){
        cc_symbol_map.set(cc,{tex,cex,lmath,cmath,html,id,op,width});
      }
      if(1){
        tex_symbol_map.set(tex,{tex,cex,lmath,cmath,html,id,op,width});
      }
      if(1){
        cex_symbol_map.set(cex,{tex,cex,lmath,cmath,html,id,op,width});
      }
      if(1){
        name_symbol_map.set(name,{tex,cex,lmath,cmath,html,id,op,width});
      }
    }
  }
  ...
  /// is the extracted string, and the second part is the remaining string
  ///
  extract_span_element(str,count){
    var i = 0;
    var v;
  ...
  to_innersvg_math(math,used,style){
    ...
    return this.toInnerSvg(l,used,style);
  }
  ///
  ///all math phrases
  ///
  ...
  to_phrase_sqrt(style,cnt1){
    var used = new Set();
    var m1 = this.to_innersvg_math(cnt1,used,style);
    var m = this.to_sqrt(m1,used);
    var [w,h,mid,s] = m;
    var defs = this.buildup_defs(used,style);
    return this.to_outer_svg(s,w,h,mid,defs,'html',style);
  }
  to_phrase_root(style,cnt1,cnt2){
    var n = cnt2;
    var n = n||'?';
    var used = new Set();
    var m1 = this.to_innersvg_math(cnt1,used,style);
    var m = this.to_root(m1,n,used);
    var [w,h,mid,s] = m;
    var defs = this.buildup_defs(used,style);
    return this.to_outer_svg(s,w,h,mid,defs,'html',style);
  }
  to_phrase_dfrac(style,cnt1,cnt2){
    var cnt2 = cnt2||'?';
    var used = new Set();
    var m1 = this.to_innersvg_math(cnt1,used,style);
    var m2 = this.to_innersvg_math(cnt2,used,style);
    var m = this.to_frac(m1,m2,1,0);
    var [w,h,mid,s] = m;
    var defs = this.buildup_defs(used,style);
    return this.to_outer_svg(s,w,h,mid,defs,'html',style);
  }
  to_phrase_frac(style,cnt1,cnt2){
    var cnt2 = cnt2||'?';
    var used = new Set();
    var m1 = this.to_innersvg_math(cnt1,used,style);
    var m2 = this.to_innersvg_math(cnt2,used,style);
    var m = this.to_frac(m1,m2,1,1);
    var [w,h,mid,s] = m;
    var defs = this.buildup_defs(used,style);
    return this.to_outer_svg(s,w,h,mid,defs,'html',style);
  }
  to_phrase_dbinom(style,cnt1,cnt2){
    var cnt2 = cnt2||'?';
    var used = new Set();
    var m1 = this.to_innersvg_math(cnt1,used,style);
    var m2 = this.to_innersvg_math(cnt2,used,style);
    var m = this.to_frac(m1,m2,0,0);
    ...
    o.push(`<use fill='currentColor' transform='translate(${(x+w1+w)*1.333},${y}) scale(1,${sy})' xlink:href='#${id2}' />`);
    var s = o.join('\n');
    var defs = this.buildup_defs(used,style);
    var w = w1 + w + w2;
    return this.to_outer_svg(s,w,h,mid,defs,'html',style);
  }
  to_phrase_binom(style,cnt1,cnt2){
    var cnt2 = cnt2||'?';
    var used = new Set();
    var m1 = this.to_innersvg_math(cnt1,used,style);
    var m2 = this.to_innersvg_math(cnt2,used,style);
    var m = this.to_frac(m1,m2,0,1);
    var [w,h,mid,s,g,q,id] = m;
    var [id1,w1,op1] = this.findIdByElement('(',used);
    var [id2,w2,op2] = this.findIdByElement(')',used);
    var o = [];
  ...
  to_phrase_overleftrightarrow(style,cnt1){
    var used = new Set();
    var m1 = this.to_innersvg_math(cnt1,used,style);
    var [w_,h_,mid_,s_,g_,q_,id_] = m1;
    var lead = 2.5;
  ...
  to_phrase_overrightarrow(style,cnt1){
    var used = new Set();
    var m1 = this.to_innersvg_math(cnt1,used,style);
    var [w_,h_,mid_,s_,g_,q_,id_] = m1;
    var lead = 2.5;
  ...
  to_phrase_underleftrightarrow(style,cnt1){
    var used = new Set();
    var m1 = this.to_innersvg_math(cnt1,used,style);
    var [w_,h_,mid_,s_,g_,q_,id_] = m1;
    var lead = 2.5; ///the arrow will protrude from either end for 2pt
  ...
  to_phrase_underrightarrow(style,cnt1){
    var used = new Set();
    var m1 = this.to_innersvg_math(cnt1,used,style);
    var [w_,h_,mid_,s_,g_,q_,id_] = m1;
    var lead = 2.5; ///the arrow will protrude from either end for 2pt
  ...
  to_phrase_subsup(style,cnt1,cnt2,cnt3){
    var used = new Set();
    var m1 = this.to_innersvg_math(cnt1,used,style);
    var m2 = this.to_innersvg_math(cnt2,used,style);
    var m3 = this.to_innersvg_math(cnt3,used,style);
    var m = this.to_subsup(m1,m2,m3);
    var [w,h,mid,s] = m;
    var defs = this.buildup_defs(used,style);
    return this.to_outer_svg(s,w,h,mid,defs,'html',style);
  }
  to_phrase_sum(style,cnt1,cnt2,cnt3){
    var used = new Set();
    if(style.displaymath){
      var m1 = this.to_innersvg_math(cnt1,used,{...style,compact:1});
      var m2 = this.to_innersvg_math(cnt2,used,{...style,compact:1});
      var m3 = this.to_innersvg_math(cnt3,used,{...style,compact:0});
      var [id,w] = this.findIdByEntity('&#x2211;',used);
      var s = `<use fill='currentColor' xlink:href='#${id}' />`;
      var m = this.to_limits(w,this.fs,s,m1,m2,2.5,1.5);//stack them up and down
      var m = this.to_sidebyside([m,m3]);
      var [w,h,mid,s] = m;
      var defs = this.buildup_defs(used,style);
      return this.to_outer_svg(s,w,h,mid,defs,'html',style);
    }else{
      var ms = this.to_innersvg_math('&#x2211;',used,style);
      var m1 = this.to_innersvg_math(cnt1,used,{...style,compact:1});
      var m2 = this.to_innersvg_math(cnt2,used,{...style,compact:1});
      var m3 = this.to_innersvg_math(cnt3,used,{...style,compact:0});
      var m = this.to_subsup(ms,m1,m2);
      var m = this.to_sidebyside([m,m3]);
      var [w,h,mid,s] = m;
      var defs = this.buildup_defs(used,style);
      return this.to_outer_svg(s,w,h,mid,defs,'html',style);
  ...
  to_phrase_int(style,cnt1,cnt2,cnt3){
    var used = new Set();
    if(style.displaymath){
      var m1 = this.to_innersvg_math(cnt1,used,{...style,compact:1});
      var m2 = this.to_innersvg_math(cnt2,used,{...style,compact:1});
      var m3 = this.to_innersvg_math(cnt3,used,{...style,compact:0});
      var [idtop,wtop] = this.findIdByEntity('&#x2320;',used);
      var [idbot,wbot] = this.findIdByEntity('&#x2321;',used);
      var s = `<use fill='currentColor' xlink:href='#${idtop}'/><use fill='currentColor' transform='translate(0,${this.fs*1.333})' xlink:href='#${idbot}'/>`;
      var w = Math.max(wtop,wbot);
      var h = this.fs+this.fs;
      var m = this.to_integral(w,h,s,m1,m2,1.0,1.0);
      var m = this.to_sidebyside([m,m3]);
      var [w,h,mid,s] = m;
      var defs = this.buildup_defs(used,style);
      return this.to_outer_svg(s,w,h,mid,defs,'html',style);
