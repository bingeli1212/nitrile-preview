...
class NitrilePreviewTokenizer extends NitrilePreviewBase {
  ...
  to_svgmath_split(math,style){
    let type = (style.name=='equation')?'equation':'svgmath_split';
    ...
    s = this.to_outer_svg(s,w,h,mid,defs,this.fs,0,type,style);
  ...
  to_outer_svg(s,w,h,mid,defs,fs,dy,type,style){
    ///type:='\\pipe' or '\\math'
    mid = mid||'0';
    var vw = w*1.333;
    var vh = h*1.333;
    ...
    if(type=='\\math'){
      ...
      opt += ` style='vertical-align:${VA}em;padding:0.1em 0;'`;
    ...
    if (v === undefined || v === null) {
      ...
      switch (v[0]) {
        ...
        case '\\leftright':
          ...
          if(v[0]==='\\leftright'){
            ...
            if(v[0] === '\\pipe' || v[0] === '\\math'){
              let protrude_w = this.get_protrude_by_id(this.my_last_id);//this depends on the last char being processed
              let protrude_h = 1;//this is always a 1pt.
              protrude_w = parseFloat(protrude_w)||0;
              protrude_h = parseFloat(protrude_h)||0;
              protrude_w *= this.fs/12;
              protrude_h *= this.fs/12;
              w += protrude_w;//increase by 2pt to ensure that the right-hand side of last letter symbols aren't clipped off
              h += protrude_h;//increase by 1pt to ensure that bottom of some characters such as 'g' aren't being cutoff
              var defs = this.buildup_defs(used);
              var type = v[0];
              s = this.to_outer_svg(s,w,h,mid,defs,this.fs,this.dy,type,pref);
            }
        ...
        case '\\sup':
          ...
          if (!this.isinmatrix && this.is_displaymath && (
          ...
          } else if (!this.isinmatrix && this.is_displaymath && v[1] === '\\int') {
            ...
            var dx2 = 4 * this.fs/12;///shift right by 2pt
            var dx3 = 4 * this.fs/12;///shift right by 2pt
            ...
            if (3) { //upper script
              ...
              o.push(`<g transform='translate(${dx3*1.333},${(dy3+exh)*1.333}) scale(${this.sub_rate},${this.sub_rate})'>`);
              o.push(s3_);
              o.push(`</g>`);
              //o.push(`</svg>`);
            }
