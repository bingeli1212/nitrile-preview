...
const re_token = /\\;|\\,|\\:|\\\\|\\\{|\\\}|\\begin\{\w+\}|\\end\{\w+\}|(\\mathcal|\\mathbf|\\mathbb|\\mathrm|\\mathit)\s*\{[A-Za-z0-9]*\}|\\[a-zA-Z]+|'+|./ug;
...
class NitrilePreviewTokenizer extends NitrilePreviewBase {
  ...
  constructor (translator) {
    ...
    this.iscex = 0;
    this.my_last_id = '';
    this.dy = 0.90;//0.74em dy for each text element. If it has been shown that bottom part of
  ...
  to_svgmath (math,style,isdisplaymath) {
    ...
    var top = this.splitArrayForNewline(v);
    var [w,h,mid,s] = this.to_top_svg(top,used,pref);
    var defs = [];
    s = this.to_outer_svg(s,0,0,w*1.333,h*1.333,w,h,mid,defs,this.fs,this.dy,v[0],pref);
  ...
  to_top_svg(top,used,pref){
    ...
    results.forEach( (p) => {
      ...
      let left_ = p[7];
      let x_ = left_w - left_;
      o.push(`<svg x='${x_}pt' y='${y_}pt' > ${s_} </svg>`);
      y_ += h_;
    })
    var s = o.join('\n')
    return [w,h,mid,s,0,0,'',0];
  }

  to_svgmath_array(math,style) {
    this.is_displaymath = 0;
    ...
    for (var i = 0; i < gs.length; ++i) {
      ...
      var used = new Set();
      var pref = {...style, compact:0, frac:0};
      var [w, h, mid, s, op, q, id, shiftdist] = this.toInnerSvg(mg,used,pref);
      //var defs = this.buildup_defs(used);
      var defs = [];///toInnerSvg() would have already builtup the defs
  ...
  to_svgmath_split(math,style){
    //var fontsize = this.to_mathfontsize();
    var d = this.to_svgmath_array(math,style);
    d = d.filter(x => (x.w > 0));
    var {s,w,h,mid,shiftdist,defs} = this.svgmath_array_to_svg(d,style);
    let x = 0;
    let y = 0;
    let vw = w*1.333;
    let vh = h*1.333;
    s = this.to_outer_svg(s,x,y,vw,vh,w,h,mid,defs,this.fs,0,'mathsplit',style);
    return {s,w,h,mid,shiftdist,defs};
  }

  svgmath_array_to_svg(d,style){

    const vspace = 1;///default 2pt separation
  ...
  to_outer_svg(s,x,y,vw,vh,w,h,mid,defs,fs,dy,type,style){
    ///type:='\\pipe' or '\\math'
    mid = mid||'0';
    //var [W,H,MID] = this.to_math_width_height(w,h,mid);
    var opt = '';
    if(type=='\\math' && !this.is_displaymath){
      ///for '\\math' type, which could be called by from
      ///within a Diagram, the style.fontcolor could be set,
      ///in which case the 'fill=' should be set to this color
      var VA = this.translator.to_math_vertical_align(w,h,mid,fs,dy);
      ...
      if(style.svgdiagram){
        ...
        opt += ` style='vertical-align:text-bottom;'`;
      }
      opt += ` class='INLINEMATH'`;
      if(style.svgfontcolor){
        opt += ` fill='${style.svgfontcolor}'`;
      }else{
        opt += ` fill='currentColor'`;
      }
      opt += ` stroke='currentColor'`
    }else if(type=='\\math' && this.is_displaymath){
      ///for '\\math' type, which could be called by from
      ///within a Diagram, the style.fontcolor could be set,
      ///in which case the 'fill=' should be set to this color
      var VA = this.translator.to_math_vertical_align(w,h,mid,fs,dy);
  ...
  buildup_defs(used){
    ...
    for (var mathSymbol of pjson.mathSymbols) {
      ...
      if (used.has(id)) {
        let adjust = 'spacingAndGlyphs';
        if(id=='myINT'||id=='mySUM'){
          adjust='';
        }
        defs.push(`<text id='${id}' dx='${dx}em' dy='${this.dy}em' textLength='${width}pt' lengthAdjust='${adjust}' style='stroke:none;fill:inherit;font-size:${this.fs}pt;font-style:${fontstyle};'>${unicode}</text>`);
      }
    }
    for(var key of my_math_variants_keys){
      for (var mathSymbol of pjson.fontVariants[key]) {
  ...
  splitArrayForAligned (v) {
    ///IMPORANT: the input argument is an item
    var top = [];
    if(Array.isArray(v) && v[0]=='\\brace'){
      var o = [];
      top.push(['\\brace',o,undefined,undefined,'']);
      var pp = v[1];///the second element is the array within the brace
      for (let p of pp) {
        if (typeof p === 'string' && p === ';') {
          o = []; /// create a new row (empty row)
          top.push(['\\brace',o,undefined,undefined,'']); /// add this row to the collection
        } else {
          o.push(p);/// add this new cell to this row
        }
      }
  ...
  splitArrayForNewline (v) {
    ...
    if(Array.isArray(v) && Array.isArray(v[1])){
      ...
      var pp = v[1];///the second element is the array within the brace
      var o = [];
      top.push([name,o,undefined,undefined,'']);
      for (let p of pp) {
        if (typeof p === 'string' && p === '\\newline') {
          o = []; /// create a new row (empty row)
          top.push([name,o,undefined,undefined,'']); /// add this row to the collection
        } else {
          o.push(p);/// add this new cell to this row
        }
      }
  ...
  findIdByElement (v,used) {
    ...
    if(this.unicode_symbol_map.has(cc)){
      let {tex,width,op,id} = this.unicode_symbol_map.get(cc);
      width = width||this.def_symbol_width;
      width *= this.fs/12;
      op = op||0;
      used.add(id);
      return [id,width,op,tex];
      ...
      switch (v[0]) {
        ...
        case '\\beginend': 
          ...
          if(1){
            ...
            return this.to_fenced(m,fence1,fence2,used);
          }  
          break;
        ...
        case '\\leftright':
          ...
          for (i=0; i < results.length; i++){
            ...
            if (i === 0) {
              ...
              if (!compact) {
                if(before_id=='myEQ'){
                  w += (this.extra_gap_eq) * this.fs/12;
                }else{
                  w += (this.extra_gap) * this.fs/12;
                }
              }
          ...
          ///set the shiftdist to the width of the brace
          if(shiftdist < 0){
            shiftdist = w;
          }
          ///
          if(v[0]==='\\leftright'){
            ...
            if(v[0] === '\\pipe' || v[0] === '\\math'){
              let deltah = this.get_protrude_by_id(this.my_last_id)*this.fs/12;//this depends on the last char being processed
              var deltav = 2*this.fs/12; /// this is in the unit of 'pt'
              var defs = this.buildup_defs(used);
              var type = v[0];
              var x = 0;
              var y = 0;
              ...
              var vw = w*1.333;
              var vh = h*1.333;
              s = this.to_outer_svg(s,x,y,vw,vh,w,h,mid,defs,this.fs,this.dy,type,pref);
            }
        ...
        case '\\sqrt':
          ...
          o.push(`<g transform='scale(1,${h/this.fs})' >`)
          ...
          if(1){
            ...
            o.push(`<g transform='translate(${lead*1.333},0) scale(${w_/ws_},1)' >`);
            o.push(`<use xlink:href='#${ids_}'/>`);
            o.push(`</g>`);
          }else{
            var bar_y = 1;
            o.push(`<line x1='${lead}pt' y1='${bar_y}pt' x2='${w}pt' y2='${bar_y}pt' style='stroke:inherit; fill:none; font-size:${this.fs}pt;'/>`);
          }
          return [w,h,mid,o.join('\n'),0,0,''];
        ...
        case '\\frac':
          ...
          var compact1 = (this.is_displaystyle)?false:true;
          var compact2 = (this.is_displaystyle)?false:true;
        ...
        case '\\sup':
          ...
          if (!this.isinmatrix && this.is_displaystyle && (
            ...
            o.push(`<g transform='translate(${subdx*1.333},${(h-nh2_)*1.333}) scale(${this.sub_rate})' >`)
            o.push(s2_);
            o.push(`</g>`);
            //o.push(`</svg>`);
            //o.push(`<svg x='${supdx}pt' width='${nw3_}pt' height='${nh3_}pt' viewBox='0 0 ${w3_*1.333} ${h3_*1.333}'>`);
            o.push(`<g transform='translate(${supdx*1.333},0) scale(${this.sup_rate})' >`);
            o.push(s3_);
            o.push(`</g>`)
            //o.push(`</svg>`);
            return [w,h,mid,o.join('\n'),g1_,q,''];
  ...
  to_frac(m1,m2,frac,isline){
    ...
    o.push(`<g transform='translate(${bot_x*1.333},${bot_y*1.333}) scale(${reduce_rate})'> ${bot_s} </g>`);
    if(isline){
      o.push(`<line x1='0pt' y1='${bar_h}pt' x2='${w}pt' y2='${bar_h}pt' style='stroke:inherit; fill:none; font-size:${this.fs}pt;'/>`);
    }
    var newv = [w,h,mid,o.join('\n'),0,0,''];
    return newv;
  }
  ...
  to_fenced(m,fence1,fence2,used){
    ...
    if(id1=='myLB'){
      ...
      o.push(`<use transform='scale(1,${h/2.0/this.fs})' xlink:href='#${top_id}' />`);
      o.push(`<use transform='translate(0,${h*0.5*1.333}) scale(1,${h/2.0/this.fs})' xlink:href='#${bot_id}' />`);
    }else if(id1=='myLPAREN'){
      ...
      o.push(`<use transform='translate(0,0)              scale(1,${h/2.0/this.fs})' xlink:href='#${top_id}' />`);
      o.push(`<use transform='translate(0,${h*0.5*1.333}) scale(1,${h/2.0/this.fs})' xlink:href='#${bot_id}' />`);
      o.push(`<use transform='translate(${w1h*1.333},0)              scale(1,${h/2.0/this.fs})' xlink:href='#${top_id}' />`);
      o.push(`<use transform='translate(${w1h*1.333},${h*0.5*1.333}) scale(1,${h/2.0/this.fs})' xlink:href='#${bot_id}' />`);
      w1 = w1h + w1h;
    }else{
      o.push(`<use transform='scale(1,${h/h1})' xlink:href='#${id1}' />`);
    }
    if(id2=='myRB'){
      ...
      o.push(`<use transform='translate(${(w1+w)*1.333},0) scale(1,${h/2.0/this.fs})' xlink:href='#${top_id}' />`);
      o.push(`<use transform='translate(${(w1+w)*1.333},${h*0.5*1.333}) scale(1,${h/2.0/this.fs})' xlink:href='#${bot_id}' />`);
    }else if(id2=='myRPAREN'){
      ...
      o.push(`<use transform='translate(${(w1+w)*1.333},0)              scale(1,${h/2.0/this.fs})' xlink:href='#${top_id}' />`);
      o.push(`<use transform='translate(${(w1+w)*1.333},${h*0.5*1.333}) scale(1,${h/2.0/this.fs})' xlink:href='#${bot_id}' />`);
      o.push(`<use transform='translate(${(w1+w+w2h)*1.333},0)              scale(1,${h/2.0/this.fs})' xlink:href='#${top_id}' />`);
      o.push(`<use transform='translate(${(w1+w+w2h)*1.333},${h*0.5*1.333}) scale(1,${h/2.0/this.fs})' xlink:href='#${bot_id}' />`);
      w2 = w2h + w2h;
    }else{
      o.push(`<use transform='translate(${(w1+w)*1.333},0) scale(1,${h/h2})' xlink:href='#${id2}' />`);
    }
  ...
  to_fence_lbrace(x,y,w,h,used){
    var o = [];
    o.push(`<use transform='scale(1,${h/2.0/this.fs})' xlink:href='#myLMOUSTACHE' />`);
    o.push(`<use transform='translate(0,${h*0.5*1.333}) scale(1,${h/2.0/this.fs})' xlink:href='#myRMOUSTACHE' />`);
  ...
  build_unicode_symbol_map(){
    ...
    for (let name in pjson.symbol) {
      ...
      if(cc && cc > 0x7F){
        ...
        map.set(cc,{tex,cex,lmath,cmath,id,op,width});
      }
    }
    return map;
  }
