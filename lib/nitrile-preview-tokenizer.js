...
class NitrilePreviewTokenizer extends NitrilePreviewBase {
  ...
  to_svgmath (math,style,type) {
    ...
    var defs = [];
    s = this.to_outer_svg(s,0,0,w*1.333,h*1.333,w,h,mid,defs,this.fs,this.dy,type,pref);
  ...
  to_outer_svg(s,x,y,vw,vh,w,h,mid,defs,fs,dy,type,style){
    ...
    opt += ` type='${type}'`
    if(type=='inlinemath'||type=='displaymath'){
  ...
  buildup_defs(used,pref){
    ...
    for (var mathSymbol of pjson.mathSymbols) {
      ...
      if (used.has(id)) {
        ...
        defs.push(`<text id='${id}' dx='${dx}em' dy='${this.dy}em' textLength='${width}pt' lengthAdjust='${adjust}' style='stroke:none;fill:${myfill};font-size:${this.fs}pt;font-style:${fontstyle};'>${unicode}</text>`);
      }
    }
    for(var key of my_math_variants_keys){
      for (var mathSymbol of pjson.fontVariants[key]) {
