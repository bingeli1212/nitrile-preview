...
class NitrilePreviewTokenizer extends NitrilePreviewBase {
      ...
      switch (v[0]) {
        ...
        case '\\binom':
          ...
          if(pref.displaymath){
            ...
            var [w,h,mid,s,g,q,id] = this.to_frac(m1,m2,0,1,pref.frac);
          }
          var padding = this.extra_gap_int;//1pt gap
          var o = [];
          var x = 0;
        ...
        case '\\sup':
          ...
          if (!this.isinmatrix && pref.displaymath && (
            ...
            var h = 0;
            var exh = 4.5; /// this is to lower the upper limit (and up the lower) so that it is close to the summation symbol---this is because the symmation symbol is not full height
            ...
            o.push(`<g transform='translate(${dx2_*1.333},${(h-exh)*1.333}) scale(${sub_rate},${sub_rate})'>`);
            o.push(s2_);
            o.push(`</g>`);
            h += nh2_-exh;
            //o.push(`</svg>`);
            return [Math.max(nw1_,nw2_,nw3_),h,nh3_+nmid1_-exh,o.join('\n'),0,1,''];
  ...
  to_phrase_math(style,cnt){
    ...
    var [w,h,mid,s,g,q,last_id,shiftdist] = m;
    if(last_id.startsWith('my_')){
      last_id = last_id.slice(3);
      var p = this.pjson.symbol[last_id];
      if(p && p.p){
        /// protrude flag is set
        w += 1.5;
      }
    }
