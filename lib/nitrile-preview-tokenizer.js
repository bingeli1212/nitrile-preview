...
class NitrilePreviewTokenizer extends NitrilePreviewBase {
  ...
  to_fenced(m,fence1,fence2,used,type){
    ...
    if(0){
      ...
      o.push(`<use fill="currentColor" transform="translate(${tx1*1.333},${ty1*1.333}) scale(${sx1},${sy1})" xlink:href="#${id1}" />`);
    }
    o.push(`<g transform="translate(${(w1+padding)*1.333},0)" > ${s} </g>`);
    if(0){
      this.insert_fence(tx2,0,w2,h,fence2,used,o);
    }else{
      o.push(`<use fill="currentColor" transform="translate(${tx2*1.333},${ty2*1.333}) scale(${sx2},${sy2})" xlink:href="#${id2}" />`);
    }
