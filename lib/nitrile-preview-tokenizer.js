...
class NitrilePreviewTokenizer extends NitrilePreviewBase {
    ...
    if(v){
      ...
      if( (typeof v === 'string' && v.startsWith('\\')) || (typeof v === 'string' && v.startsWith('&') && v.endsWith(';')) ){
        ...
        if(this.name_symbol_map.has(name)){
          var {id,op,html,width} = this.name_symbol_map.get(name);
          width = this.get_width_for_char(width,html);
          op = op||0;
          used.add(id);
  ...
  // might throw an exception
  get_tex_symbol(tex,lmath,cc,fonts){
    ...
    if(this.translator.program=='lualatex'||this.translator.program=='xelatex'){
      if(fonts){
  ...
  // might throw an exception
  get_cex_symbol(cex,cmath,cc,fonts){
    var s = String.fromCodePoint(cc);
    if(cex){
      s = `{${cex}}`
    }else if(cmath){
  ...
  build_all_symbol_maps(name_symbol_map,cc_symbol_map){
    for (let name in pjson.symbol) {
      ...
      if(1){
        name_symbol_map.set(name,{tex,cex,lmath,cmath,html,id,op,width,cc,fonts});
      }
    }
    for (let alt_name in pjson.alt_symbol){
      var name = pjson.alt_symbol[alt_name];
      if(name_symbol_map.has(name)){
        let o = name_symbol_map.get(name);
        name_symbol_map.set(alt_name,o);
      }
    }
  }
