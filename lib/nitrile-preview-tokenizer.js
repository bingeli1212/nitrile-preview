...
class NitrilePreviewTokenizer extends NitrilePreviewBase {
  ...
  constructor (translator) {
    ...
    this.build_all_symbol_maps(this.symbol_name_map,this.symbol_cc_map,this.symbol_group_map);
  }
  ...
  get_cmath_symbol(ctex,cmath,cc,fonts,op){
    if(cmath){
      return cmath.trimRight();
    }else if(ctex){
      return `\\text{${ctex.trimRight()}}`
    }else{
      var s = String.fromCodePoint(cc);
      if(fonts){
  ...
  // in 'smooth()' function
  build_all_symbol_maps(symbol_name_map,symbol_cc_map,symbol_group_map){
