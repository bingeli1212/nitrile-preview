...
class NitrilePreviewTokenizer extends NitrilePreviewBase {
    ...
    if (v === undefined || v === null) {
      ...
      switch (v[0]) {
        ...
        case '\\binom':
          ...
          var x = 0;
          var y = 0;
          o.push(`<use transform='translate(${(x)*1.333},${y})      scale(1 ${h/this.fs})' xlink:href='#${id1}' />`);
          o.push(`<g transform='translate(${(x+w1)*1.333},${y})' > ${s} </g>`);
          o.push(`<use transform='translate(${(x+w1+w)*1.333},${y}) scale(1 ${h/this.fs})' xlink:href='#${id2}' />`);
          var s = o.join('\n');
  ...
  to_fenced(m,fence1,fence2,used){
    ...
    o.push(`<g transform='translate(${w1*1.333},${padding_h})' > ${s} </g>`);
    w += w1 + w2;
    var newv = [w,h+padding_h+padding_h,mid,o.join('\n'),0,0,''];
    return newv;
  }
