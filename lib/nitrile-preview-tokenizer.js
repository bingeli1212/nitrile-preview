...
class NitrilePreviewTokenizer extends NitrilePreviewBase {
  ...
  constructor (translator) {
    ...
    this.build_all_symbol_maps(this.name_symbol_map,this.cc_symbol_map,this.symbol_group_map);
  }
  ...
  // in 'smooth()' function
  build_all_symbol_maps(name_symbol_map,cc_symbol_map,symbol_group_map){
    for (let name in this.pjson.symbol) {
      ...
      let id = `my_${name}`;
      if(cc){
        cc_symbol_map.set(cc,{tex,cex,lmath,cmath,html,id,fonts,op,width,name,comment});
      }
      if(1){
        name_symbol_map.set(name,{tex,cex,lmath,cmath,html,id,op,width,cc,fonts,comment});
      }
      if(group){
