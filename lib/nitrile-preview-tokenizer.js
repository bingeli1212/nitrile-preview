...
class NitrilePreviewTokenizer extends NitrilePreviewBase {
  ...
  to_svgmath (str,style) {
    ...
    var g = this.toCommands(g);
    var g = this.toSubsup(g);
    var [w, h, mid, s, op, q, id, shiftdist] = this.toInnerSvg(g,used,style);
  ...
  to_svgmath_array(str,style) {
    ...
    for (var i = 0; i < gs.length; ++i) {
      var mg = gs[i];
      var used = new Set();
      var [w, h, mid, s, op, q, id, shiftdist] = this.toInnerSvg(mg,used,style);
      //var defs = this.buildup_defs(used);
      var defs = [];///toInnerSvg() would have already builtup the defs
      var o = { s, w, h, mid, defs, shiftdist };
      d.push(o);
  ...
  to_outer_svg(s,x,y,vw,vh,w,h,mid,defs,fs,dy,type,style){
    ...
    if(type=='\\math'){
      ...
      opt += ` style='vertical-align:${VA}em;outline:1px solid orange;'`;
      ...
      opt += ` xmlns:xlink='http://www.w3.org/1999/xlink'`
      opt += ` width='${w/fs}em' height='${h/fs}em'`
      opt += ` style='vertical-align:text-top;outline:1px solid green;'`;
      opt += ` fill='currentColor' stroke='currentColor'`
    } else if(type=='mathsplit'){
      opt += ` xmlns = 'http://www.w3.org/2000/svg'`;
      opt += ` xmlns:xlink='http://www.w3.org/1999/xlink'`
      opt += ` width='${w/fs}em' height='${h/fs}em'`
      opt += ` style='vertical-align:top;outline:1px solid blue;'`;
      opt += ` fill='currentColor' stroke='currentColor'`
    } else {
    ...
    if (v === undefined || v === null) {
      ...
      switch (v[0]) {
        ...
        case '\\frac':
          ...
          var compact1 = (this.is_displaymath)?false:true;
          var compact2 = (this.is_displaymath)?false:true;
        ...
        case '\\sup':
          ...
          if (!this.isinmatrix && this.is_displaymath && (
                  v[1] === '\\sum' || 
                  v[1] === '\\prod' || 
                  v[1] === '\\lim'  )) {
            var o = [];
          ...
          } else if (!this.isinmatrix && this.is_displaymath && 
                  v[1] === '\\int' ) {
            ...
            o.push(`<g transform='translate(${supdx*1.333},${nw3_*1.333}) scale(${this.sup_rate})' >`);
            o.push(s3_);
            o.push(`</g>`)
            //o.push(`</svg>`);
            return [w,h,mid,o.join('\n'),g1_,q,''];
      ...
      if (id === 'myINT') { 
        ...
        var mid = this.fs/2;
        var dx = 3;
        if(this.is_displaymath && !this.isinmatrix){
  ...
  build_unicode_symbol_map(){
    ...
    for (let name in pjson.symbol) {
      ...
      if(cc && cc > 0x7F){
        ...
        map.set(cc,{tex,cex,lmath,cmath});
      }
    }
    return map;
  }
