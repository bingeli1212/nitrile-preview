  ...
  buildup_defs(used){
    ...
    for (var mathSymbol of this.pjson.mathSymbols) {
      ...
      if (used.has(id)) {
        ...
        defs.push(`<text id="${id}" dy="11pt" textLength="${width}pt" lengthAdjust="${adjust}" style="stroke:none;fill:${myfill};font-size:${this.fs}pt;font-style:${fontstyle};">${unicode}</text>`);
      }
    }
    for(var key of my_math_variants_keys){
      for (var mathSymbol of this.pjson.fontVariants[key]) {
