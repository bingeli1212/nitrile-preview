...
class NitrilePreviewTokenizer extends NitrilePreviewBase {
  ...
  ///
  ///The API splits the math based on \cr
  ///
  to_align_math(str,style,used,isdmath,extra_css=''){
    var m = this.to_svgmath(str,style,used,isdmath);
    var {w,h,mid,s} = m;
    var defs = this.buildup_defs(used);
    var text = this.to_outer_svg(s,w,h,mid,defs,'html',extra_css);
    return text;
  }
  ///
  ///This API generates a single line
  ///
  to_literal_math(str,style,used,isdmath,extra_css=''){
    var m = this.to_innersvg_math(str,style,used,isdmath);
    var [w,h,mid,s] = m;
    var defs = this.buildup_defs(used);
    var text = this.to_outer_svg(s,w,h,mid,defs,'html',extra_css);
    return text;
  ...
  ///
  to_innersvg_math(str,style,used,isdmath){
    var l = this.to_math_segment_array(str);//this method is to be derived by each subclass
    var l = this.to_brace_group(l,str);
    ...
    var pref = {...style,isdmath,compact:0,frac:0,array:0};
  ...
  to_outer_svg(s,w,h,mid,defs,type,extra_css=''){
    ///type:='\\pipe' or '\\math'
    var x = 0;
    var y = 0;
    var vw = w*1.333;
    ...
    if (v === undefined || v === null) {
      ...
      switch (v[0]) {
        ...
        case '\\beginend': 
          ...
          /// config parameters
          pref.array += 1;
          const gap_w = 6.0 * this.fs/12;
          const gap_h = 0.0 * this.fs/12;
          var o = [];
        ...
        case '\\leftright':
          ...
          for (i=0; i < results.length; i++){
            ...
            if (i === 0) {
              ...
              // op=4    USed for \sum and \prod operators, where the spaccing is always needed before,
              //         but remove after
              ...
              /// or a plus sign 
              /// actually add a small gap
              w += this.math_gap; /// 1.5pt

            } else if (before_id == 'myLPAREN'){

              /// do not add gap if the previous item is a left parenthesis

            } else if (before_id == 'myLBR'){

              /// do not add gap if the previous item is a left bracket

            } else if (before_id0 == 'myLPAREN' && (before_id=='myMI'||before_id=='myPLUS')) {

              /// do not add gap, this is for (-a, +b)
              w += this.math_gap; /// 1.5pt
            
            } else if (before_id0 == 'myCOMMA' && (before_id=='myMI'||before_id=='myPLUS')) {
              ...
              ///this is for \sum and \prod, where the spacing is needed before but not after
              w += 0;
            ...
            if(id_.startsWith("my_")){
              let myid = id_.slice(3);
              let protrude = this.pjson.protrude[myid]||0;
              if(Number.isFinite(protrude)){
                w += protrude;
              }
            }
          }
        ...
        case '\\hrule':
          ...
          if(v2){
            ...
            if(mymath){
              ...
              let dx = (w-wt_)/2;
              s = `<g transform="translate(${this.fix(dx*1.333)} 0) "> ${s} </g>`;
              o.push(s);
            }
          }
        ...
        case '\\sqrt':
          ...
          o.push(`<use transform="scale(1,${h/this.fh})" fill="currentColor" xlink:href="#${idr_}" />`);
          ...
          if(1){
            ...
            var bar_y = sunk+1;
            o.push(`<line x1="${lead}pt" y1="${bar_y}pt" x2="${w}pt" y2="${bar_y}pt" style="stroke:currentColor; fill:none; font-size:${this.fs}pt;"/>`);
          }
        ...
        case '\\dfrac':
          ...
          var m1 = this.toInnerSvg(v[1],used,{...pref,frac});
          var m2 = this.toInnerSvg(v[2],used,{...pref,frac});
          var reduce = 0;
          return this.to_frac(m1,m2,1,reduce,frac);
          break;
        ...
        case '\\dbinom':
          ...
          var [w,h,mid,s,g,q,id] = this.to_frac(m1,m2,0,reduce,pref.frac);
        ...
        case '\\binom':
          ...
          var [id1,w1,op1] = this.findIdByElement('(',used);
          var [id2,w2,op2] = this.findIdByElement(')',used);
          var [w,h,mid,s,g,q,id] = this.to_frac(m1,m2,0,reduce,frac);
          var padding = this.extra_gap_int;//1pt gap
          var o = [];
          var x = 0;
          var y = 0;
        ...
        case '\\sup':
          ...
          if (    v[1] === '\\dsum' || 
            ...
            var h = 0;
            var exhsup = 4.5; /// this is to lower the upper limit so that it is close to the summation symbol---this is because the symmation symbol is not full height
            var exhsub = 4.5; /// this is to up the lower limit
            var mid = nh3_+nmid1_;

            //o.push(`<svg x="${dx3_}pt" y="${h+exh}pt" width="${nw3_}pt" height="${nh3_}pt" viewBox="0 0 ${w3_*1.333} ${h3_*1.333}">`);
            o.push(`<g transform="translate(${dx3_*1.333},${(h)*1.333}) scale(${sup_rate},${sup_rate})">`);
            o.push(s3_);
            o.push(`</g>`);
            //o.push(`</svg>`);
            h += nh3_;
            ...
            var h = nh1_;
            var mid = nmid1_;
            //var exh = 5 * this.fs/12; /// this is to lower the upper limit so that it is close to the summation symbol---this is because the integration symbol is not full height
            ...
            var supdy = nh3_ - (h1_*0.5) - 1;
            var subdy = nh2_ - (h1_*0.5) - 1;
            /// not less than zero
            ...
            //for SUB
            o.push(`<g transform="translate(${subdx*1.333},${(h-nh2_)*1.333}) scale(${adjustment*this.sub_rate})" >`);///the subscript is raised by 1pt
            o.push(s2_);
            o.push(`</g>`);
            //o.push(`</svg>`);
            //o.push(`<svg x="${supdx}pt" width="${nw3_}pt" height="${nh3_}pt" viewBox="0 0 ${w3_*1.333} ${h3_*1.333}">`);
            //for SUP
            o.push(`<g transform="translate(${supdx*1.333},0) scale(${adjustment*this.sup_rate})" >`);///the superscript is artificially lowered by 1pt
            o.push(s3_);
            o.push(`</g>`)
            //o.push(`</svg>`);
            return [w,h,mid,o.join('\n'),g1_,q,''];
      ...
      if (id === 'myDINT') { 
      ...
      } else if (id === 'myDSUM' || id === 'myDPROD') { /// THIS is \\sum symbol by itself it is enlarged if 'isdstyle' is on
        ...
        //a vertical line
        return this.to_vl_element("",pref);
  ...
  to_vl_element(text,style){

    var w = this.def_symbol_width;
    var h = this.fh;
    var mid = h/2;
    var s = `<line x1="${w/2}pt" y1="0pt" x2="${w/2}pt" y2="${h}pt" style="stroke:currentColor;"/>`;
    return [w, h, mid, s, 0, 0, ''];
  }
  ...
  to_frac(m1,m2,isline,isreduce,nfrac){
    ...
    if(isline){
      o.push(`<line x1="0pt" y1="${top_h}pt" x2="${w}pt" y2="${top_h}pt" stroke="currentColor" />`);
    }
    var s = o.join('\n');
  ...
  to_fenced(m,fence1,fence2,used,type){
    ...
    var padding = 0; //gaps for +
    var sx1 = 1;
    var sy1 = h/this.fs;
    var sx2 = 1;
    var sy2 = h/this.fs;
    var tx1 = 0;
    var ty1 = (this.fs-this.fh);
    var tx2 = w1+padding+w+padding;
    var ty2 = (this.fs-this.fh);
    if(0 && h < this.fh*1.5){
      sy1 = 1;//do not do anything if the array contains a single row
      sy2 = 1;
      ty1 = 0;
      ty2 = 0;
    }
    var o = [];
    if(0){
      this.insert_fence(tx1,0,w1,h,fence1,used,o);
    }else{
      o.push(`<use fill="currentColor" transform="translate(${tx1*1.333},${ty1*1.333*sy1}) scale(${sx1},${sy1})" xlink:href="#${id1}" />`);
    }
    o.push(`<g transform="translate(${(w1+padding)*1.333},0)" > ${s} </g>`);
    if(0){
      this.insert_fence(tx2,0,w2,h,fence2,used,o);
    }else{
      o.push(`<use fill="currentColor" transform="translate(${tx2*1.333},${ty2*1.333*sy2}) scale(${sx2},${sy2})" xlink:href="#${id2}" />`);
    }
  ...
  insert_fence(x,y,w,h,fence,used,o){
    var v = this.fs-1.5;
    var V = (h-this.fs-this.fs+3)/this.fs;
    if(V<0){
      V = 0;
    }
    var m = h/2;
    var M = (h/2)/this.fs;
    if(fence=='\\lbrack'){
      var [id1,w1,op1] = this.findIdByElement('\\lbrackupper',used);
      var [id2,w2,op2] = this.findIdByElement('\\lbrackexten',used);
      var [id3,w3,op3] = this.findIdByElement('\\lbracklower',used);
      o.push(`<use fill="currentColor" transform="translate(${x*1.333},${0*1.333})" xlink:href="#${id1}" />`);
      o.push(`<use fill="currentColor" transform="translate(${x*1.333},${0*1.333}) scale(1,${h/this.fs})" xlink:href="#${id2}" />`);
      o.push(`<use fill="currentColor" transform="translate(${x*1.333},${(h-this.fs)*1.333})" xlink:href="#${id3}" />`);
    }else if(fence=='\\rbrack'){
      var [id1,w1,op1] = this.findIdByElement('\\rbrackupper',used);
      var [id2,w2,op2] = this.findIdByElement('\\rbrackexten',used);
      var [id3,w3,op3] = this.findIdByElement('\\rbracklower',used);
      o.push(`<use fill="currentColor" transform="translate(${x*1.333},${0*1.333})" xlink:href="#${id1}" />`);
      o.push(`<use fill="currentColor" transform="translate(${x*1.333},${0*1.333}) scale(1,${h/this.fs})" xlink:href="#${id2}" />`);
      o.push(`<use fill="currentColor" transform="translate(${x*1.333},${(h-this.fs)*1.333})" xlink:href="#${id3}" />`);
    }else if(fence=='('){
      var [id1,w1,op1] = this.findIdByElement('\\lparenupper',used);
      var [id2,w2,op2] = this.findIdByElement('\\lparenexten',used);
      var [id3,w3,op3] = this.findIdByElement('\\lparenlower',used);
      o.push(`<use fill="currentColor" transform="translate(${x*1.333},${0*1.333})" xlink:href="#${id1}" />`);
      o.push(`<use fill="currentColor" transform="translate(${x*1.333},${v*1.333}) scale(1,${V})" xlink:href="#${id2}" />`);
      o.push(`<use fill="currentColor" transform="translate(${x*1.333},${(h-this.fs)*1.333})" xlink:href="#${id3}" />`);
    }else if(fence==')'){
      var [id1,w1,op1] = this.findIdByElement('\\rparenupper',used);
      var [id2,w2,op2] = this.findIdByElement('\\rparenexten',used);
      var [id3,w3,op3] = this.findIdByElement('\\rparenlower',used);
      o.push(`<use fill="currentColor" transform="translate(${x*1.333},${0*1.333})" xlink:href="#${id1}" />`);
      o.push(`<use fill="currentColor" transform="translate(${x*1.333},${v*1.333}) scale(1,${V})" xlink:href="#${id2}" />`);
      o.push(`<use fill="currentColor" transform="translate(${x*1.333},${(h-this.fs)*1.333})" xlink:href="#${id3}" />`);
    }else if(fence=='\\lvert'){
      var [id2,w2,op2] = this.findIdByElement('\\lbrackexten',used);
      var n = Math.floor(h/10);
      var n = Math.max(n,1);
      var left = h;
      var y = 0;
      while(left > 0){
        if(left < 10){
          o.push(`<use fill="currentColor" transform="translate(${x*1.333},${y*1.333}) scale(1,${left/10})" xlink:href="#${id2}" />`);
        }else{
          o.push(`<use fill="currentColor" transform="translate(${x*1.333},${y*1.333})" xlink:href="#${id2}" />`);
        }
        left -= 10;
        y += 10;
      }
    ...
    }else if(fence=='\\lbrace'){
      var [id1,w1,op1] = this.findIdByElement('\\lbraceupper',used);
      var [id2,w2,op3] = this.findIdByElement('\\lbracemidle',used);
      var [id3,w3,op3] = this.findIdByElement('\\lbracelower',used);
      var [id4,w3,op3] = this.findIdByElement('\\lbraceexten',used);
      var n = Math.floor((h-10)/5/2);
      var m = Math.floor(h - n*2*5)-1;
      var y = -5;
      for(let j=0; j < n; ++j){
        if(j==0){
          o.push(`<use fill="currentColor" transform="translate(${x*1.333},${y*1.333})" xlink:href="#${id1}" />`);
        }else{
          o.push(`<use fill="currentColor" transform="translate(${(x)*1.333},${y*1.3333})" xlink:href="#${id4}" />`);
        }
        y+=5;
      } 
      y += 5
      o.push(`<use fill="currentColor" transform="translate(${x*1.333},${y*1.333}) scale(1,${m/10})" xlink:href="#${id2}" />`);
      y += m-5;
      for(let j=0; j < n; ++j){
        if(j==(n-1)){
          o.push(`<use fill="currentColor" transform="translate(${x*1.333},${y*1.333})" xlink:href="#${id3}" />`);
        }else{
          o.push(`<use fill="currentColor" transform="translate(${(x)*1.333},${y*1.3333})" xlink:href="#${id4}" />`);
        }
        y+=5;
      } 
    }else if(fence=='\\rbrace'){
      var [id1,w1,op1] = this.findIdByElement('\\rbraceupper',used);
      var [id2,w2,op3] = this.findIdByElement('\\rbracemidle',used);
      var [id3,w3,op3] = this.findIdByElement('\\rbracelower',used);
      var [id4,w3,op3] = this.findIdByElement('\\rbraceexten',used);
      var n = Math.floor((h-10)/5/2);
      var m = Math.floor(h - n*2*5)-1;
      var y = -5;
      for(let j=0; j < n; ++j){
      ...
      // o.push(`<use fill="currentColor" transform="translate(${x*1.333},${-5*1.333})" xlink:href="#${id1}" />`);
      // o.push(`<use fill="currentColor" transform="translate(${(x-1)*1.333},${(0)*1.3333})" xlink:href="#${id4}" />`);
      // o.push(`<use fill="currentColor" transform="translate(${(x-1)*1.333},${(5)*1.3333})" xlink:href="#${id4}" />`);
      // o.push(`<use fill="currentColor" transform="translate(${x*1.333},${(15)*1.333}) scale(1,1.2)" xlink:href="#${id2}" />`);
      // o.push(`<use fill="currentColor" transform="translate(${(x-1)*1.333},${(22)*1.3333})" xlink:href="#${id4}" />`);
      // o.push(`<use fill="currentColor" transform="translate(${x*1.333},${(27)*1.333})" xlink:href="#${id4}" />`);
      // o.push(`<use fill="currentColor" transform="translate(${x*1.333},${(32)*1.333})" xlink:href="#${id3}" />`);
      // var gap = (h - 10 - 4 - 4)/2/12;
      // o.push(`<use fill="currentColor" transform="translate(${(x-1)*1.333},${(h-12-5)*1.3333})" xlink:href="#${id4}" />`);
    }
  }
