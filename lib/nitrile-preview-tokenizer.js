...
class NitrilePreviewTokenizer extends NitrilePreviewBase {
  ...
  constructor (translator) {
    ...
    this.re_segment = /^(\\;|\\,|\\:|\\begin\{\w+\}|\\end\{\w+\}|\\text\{.*?\}|&[a-zA-Z][a-zA-Z0-9]*;|\\[a-zA-Z]+|'+|.)\s*(.*)/;
  ...
  to_innersvg_math(math,used,style){
    ...
    var l = this.to_brace_group(l);
    //var l = this.toCleanup(l); // this should not be called because it replies on the fact that if an element is an array it needs to be cleaned by 'to_math_segment_array' method could return an array such as \\svg or \\ensuremath
    var l = this.toCommands(l);
    var l = this.toSubsup(l);//need to be '\\brace' here because it exports the 'used' to the caller, so that it does not need to build 'defs'
