...
const { NitrilePreviewFlowerpot } = require('./nitrile-preview-niflowerpot');
...
class Server extends NitrilePreviewBase {
  constructor() {
    super();
    var inlist = process.argv;    
    var inlist = inlist.slice(2); 
    this.inlist = inlist.map(fname => {return {fname};});
    this.md_map = {};
    if(this.inlist.length==0){
      var entries = fs.readdirSync('.',{withFileTypes:true});
      entries.forEach((p) => {
        if(path.extname(p.name)==".md"){
          let fname = p.name;
          this.inlist.push({fname})
        }
      });
    }
  }
  async run() {
    ...
    app.get('/',(req,res) => {
      if(this.inlist.length){
        var html = this.to_index_html(this.inlist);
        console.log(html);
        res.send(html);
      }else{
