...
class NitrilePreviewPaper {
  ...
  to_top_chapters(blocks) {
    var top = [];
    var o;
    if(blocks.length && blocks[0].sig=='HDGS' && blocks[0].name=='part'){
      o = top;
      o.push(blocks[0]);
    ...
    for (let block of blocks) {
      ...
      if (sig == 'HDGS' && this.translator.name_to_hdgn(name,hdgn) == 0) {
        this.num_chapters += 1;
        o = [];
        top.push(o);
        o.push(block);
  ...
  to_top_sections(blocks) {
    ...
    for (let block of blocks) {
      ...
      if (sig == 'HDGS' && this.translator.name_to_hdgn(name,hdgn) == 1) {
        o = [];
        top.push(o);
        o.push(block);
        continue;
  ...
  to_top_subsections(blocks) {
    ...
    for (let block of blocks) {
      let { sig, hdgn, name } = block;
      if (sig == 'HDGS' && this.translator.name_to_hdgn(name,hdgn) == 2) {
        o = [];
        top.push(o);
        o.push(block);
        continue;
  ...
  to_top_subsubsections(blocks) {
    ...
    for (let block of blocks) {
      let { sig, hdgn, name } = block;
      if (sig == 'HDGS' && this.translator.name_to_hdgn(name,hdgn) >= 3) {
        o = [];
        top.push(o);
        o.push(block);
        continue;
