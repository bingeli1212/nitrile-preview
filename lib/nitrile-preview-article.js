...
class NitrilePreviewArticle extends NitrilePreviewLatex {
  ...
  to_article_section(top) {
    let my = top.shift();
    let all = [];
    all.push(`\\section{${this.uncode(my.style,my.title)}}{\\label{${my.label}}}`);
    top.forEach((o, i) => {
      if (Array.isArray(o)) {
  ...
  to_article_subsection(top) {
    let my = top.shift();
    let all = [];
    all.push(`\\subsection{${this.uncode(my.style,my.title)}}`);
    top.forEach((o,i) => {
      if(Array.isArray(o)){
  ...
  to_article_subsubsection(top) {
    let my = top.shift();
    let all = [];
    all.push(`\\subsubsection{${this.uncode(my.style,my.title)}}`);
    top.forEach((o,i) => {
      if(Array.isArray(o)){
      }else{
        all.push(o.latex);
