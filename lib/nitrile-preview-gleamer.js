...
class NitrilePreviewGleamer extends NitrilePreviewContex {
  ...
  to_beamer_document() {
    this.icon_keypoint = `{\\switchtobodyfont[zapfdingbats]❖}`;
    this.icon_exercise = `{\\switchtobodyfont[zapfdingbats]✎}`;
    this.icon_solution = `{\\switchtobodyfont[zapfdingbats]✍}`;
    this.icon_folder   = `{\\switchtobodyfont[zapfdingbats]♨}`;
...
`;
  ...
  write_frame_toc(topframes){
    ...
    all = [];
    all.push('');
    all.push(`\\startsection[title={${this.icon_folder} Table Of Contents}]`);
  ...
  write_frame_folder(frame,subframes){
    let all = [];
    all.push('');
    all.push(`\\startsection[title={${this.icon_folder} ${frame.title}}]`); 
    all.push(`\\startsubsection[title={}]`);
  ...
  write_frame(frame) {
    ...
    if(1){
      ...
      all.push(`\\startsection[title={${icon} ${frame.title}}]`); 
      all.push(`\\startsubsection[title={}]`);
      frame.contents.forEach((x,i) => {
        //'x' is a block
        let latex = this.translate_block(x);
    ...
    frame.solutions.forEach((o,i,arr) => {     
      ...
      let icon = frame.isex?this.icon_exercise:this.icon_keypoint;
      all.push(`\\startsection[title={${icon} ${frame.title}}]`); 
      all.push(`\\startsubsection[title={{\\it ${o.title}} ~ ${this.untext(o.body,o.style).trim()}}]`);
  ...
  to_titlelines(){
    ...
    if(block && block.sig=='FRNT'){
      ...
      for(let t of data){
        let [key,val] = t;
        if(key=='title'){
          title = `${this.uncode(val,style)}`;
        }
        else if(key=='subtitle'){
          subtitle = `${this.uncode(val,style)}`;
        }
        else if(key=='author'){
          author = `${this.uncode(val,style)}`;
        }
        else if(key=='institute'){
          institute = `${this.uncode(val,style)}`;
        }
      }
    }
    var titlelines = [];
    ...
    titlelines.push(`\\tfd ${this.uncode(title)}`);
    ...
    titlelines.push(`\\bTR \\bTD ${this.uncode(author)} \\eTD \\eTR`);
    titlelines.push(`\\bTR \\bTD ${this.uncode(institute)}   \\eTD \\eTR`);
    titlelines.push(`\\bTR \\bTD ${this.uncode(date)}   \\eTD \\eTR`);
...
module.exports = { NitrilePreviewGleamer }
