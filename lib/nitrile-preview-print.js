...
const widthsMap = require('./nitrile-preview-widthsmap');
const settingsDefaults = { font: 'Arial', size: 100 };

class NitrilePreviewPrint extends NitrilePreviewTranslator {
  ...
  string_pixel_width(string, settings){
  ...
  char_pixel_width(ch,cc, settings){
    const sett = { ...settingsDefaults, ...settings };
    const font = sett.font.toLowerCase();
    const size = sett.size;
    const variant = 0 + (sett.bold ? 1 : 0) + (sett.italic ? 2 : 0);
    if(this.widthsMap.hasOwnProperty(font)){
      if(cc <= 0x1F){
        return 0;
      }
      const widths = map[font][ch] || map[font].x;
      const width = widths[variant];
  ...
  get_string_length(print,startx,maxwidth){
    let x = startx;
    for(var i=0; i < print.str.length; ++i){
      let ch = print.str.charAt(i);
      let cc = print.str.codePointAt(i);
      let w = this.char_pixel_width(ch,cc,settings);
      if(x + w > maxwidth){
        ///move to next line
  ...
  do_layout(print,startx,maxwidth,box){
    if(box.length==0){
      var line = [];
      box.push(line);
      var p = {};
      ...
      p.w = 0;
      p.s = '';
      p.settings = print.settings;
    }else{
      var line = box[box.length-1];
      var p = line[line.length-1];
    }
    let x = startx;
    let i0 = 0;
    for(var i=0; i < print.s.length; ++i){
      let ch = print.s.charAt(i);
      let cc = print.s.codePointAt(i);
      let w = this.char_pixel_width(ch,cc,print.settings);
      if(x + w > maxwidth){
        ///move to next line
        var line = [];
        box.push(line);
  ...
  do_PARA(block) {
    ...
    block.print = this.untext(body,style);
    block.env = {};
    if(nspace){
      block.env.nspace = 1;
    }
  ...
  smooth (unsafe) {
    ...
    /// texts are to be part of a SVG-PARA element, thus any HTML markup
  ...
  phrase_to_math (str,style) {  
    this.style = style;
    var {s} = this.to_svgmath(str,style);
    if(dstyle){
  ...
  para_to_lines(ss,style) {
    ...
    let text = this.itms_to_verse(itms, style);
    return text;
  }
  ...
  itms_to_itemized(itms,style){
    ...
    if (itms.length) {
      ...
      return `<ul class='PARA' >${pp.join('\n')}</ul>`;
    }
    return `<ul class='PARA' ><li></li></ul>`;
  }
  itms_to_verse(itms, style) {
    let pp = itms.map((p, i) => {
      return `<div>${p.text}</div>`;
    })
    return `<dl class='PARA' >\n${pp.join('\n')}\n</dl>`;
  }
  pp_to_multi_itemized(itms,style,name){
    ...
    for (let j = 0,i=0; j < itms.length; i+=1,j+=k) {
      ...
      let fr = frs[i];
      d.push(`<td style='vertical-align:top;width:${fr*w*100}%'><${name} class='PARA' >\n${pp.join('\n')}\n</${name}></td>`);
      if (i < n - 1) {
        d.push(`<td style='width:${gap * 100}%'></td>`);
      }
    }
...
module.exports = { NitrilePreviewPrint }