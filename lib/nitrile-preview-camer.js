...
class NitrilePreviewCamer extends NitrilePreviewContex {
  ...
  to_document() {
    var style_chapter = this.conf_to_string('context.chapter','\\bfd');
    var style_section = this.conf_to_string('context.section','\\bfa');
    var style_subsection = this.conf_to_string('context.subsection','\\bf');
    var style_subsubsection = this.conf_to_string('context.subsubsection','\\bf');
    var style_subsubsubsection = this.conf_to_string('context.subsubsubsection','\\bf');
    ///do translate
    ...
    if (this.conf_to_string('context.twocolumn')) {
    ...
    /// toc lines
    var toclines = [];
    if (this.conf_to_string('context.toc')) {
      if (1) {
    ...
    var p_layout = '';
    var p_bodyfont = '';
    if (this.conf_to_string('context.papersize')) {
      var s = `\\setuppapersize[${this.conf_to_string('context.papersize')}]`;
      p_papersize = s;
    }
    /*
    var s_layout = `\\setuplayout[width = ${ this.conf_to_string('width') }mm,
                      backspace = ${ this.conf_to_string('backspace') }mm,
                      cutspace = ${ this.conf_to_string('cutspace') }mm,
                      topspace = ${ this.conf_to_string('topspace') }mm,
                      height = ${ this.conf_to_string('height') }mm,
                      header = ${ this.conf_to_string('header') }mm,
                      footer = ${ this.conf_to_string('footer') }mm]`;
                      */
    if (this.conf_to_string('context.layout')) {
      var s = this.conf_to_list('context.layout');
      var s = `\\setuplayout[${s.join(',')}]`;
      p_layout = s;
    }
    //\\setupbodyfont[linuxlibertineo, ${ this.conf_to_string('bodyfontsizept') } pt]
    if (this.conf_to_string('context.bodyfont')) {
      var s = this.conf_to_list('context.bodyfont');
