  ...
  toInnerSvg (v, used,pref) {
    ...
    if (v === undefined || v === null) {
        ...
        if(math_cmd_info.verbatim){
          if(math_cmd_info.count==1){
            ...
            if (v1 === undefined){
              ...
              if (v1[0] === '\\brace') {
                //str = v1[4];
                let ss = v1[1];///this is an array
                let pp = ss.map((s) => this.toInnerSvg(s,used,pref));
                pp = pp.map((p) => p[3]); /// each innersvg is an array of four elements: [w,h,mid,s]
                str = pp.join('');
              } else {
      ...
      if(id){
        ...
        /// reverse from 'tex' to 'lmath'
        if(this.tex_symbol_map.has(v)){
          var s = this.tex_symbol_map.get(v).lmath;
          return [w,h,mid,s];
        }
        ...
        if(this.translator.cc_symbol_map.has(cc)){
          var s = this.translator.cc_symbol_map.get(cc).lmath;
          return [w,h,mid,s];
        }
