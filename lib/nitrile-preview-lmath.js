...
class NitrilePreviewLmath extends NitrilePreviewTokenizer {
    ...
    if (v === undefined || v === null) {
      ...
      if( (typeof v === 'string' && v.startsWith('\\')) || (typeof v === 'string' && v.startsWith('&') && v.endsWith(';')) ){
        ...
        if(this.name_symbol_map.has(name)){
          var {id,op,lmath,fonts,cc} = this.name_symbol_map.get(name);
          if(this.translator.program==='lualatex'||this.translator.program==='xelatex'){
            if(fonts){
              var lmath = `\\text{\\${fonts}{}${String.fromCodePoint(cc)}}`;
              if(op){
                lmath = `\\operatorname{${math}}`;
              }
            }
          }
          var s = lmath;
      ...
      if(id){
        var s = v;
        if(name){
          s = this.get_lmath_symbol(name);
          if(this.translator.program==='lualatex'||this.translator.program==='xelatex'){
            var {lmath,op,cc,fonts} = this.name_symbol_map.get(name);
            s = lmath;
            if(fonts){
              s = `\\text{\\${fonts}{}${String.fromCodePoint(cc)}}`;
              if(op){
                s = `\\operatorname{${s}}`;
              }
            }
  ...
  to_phrase_root(style,cnt,cnt2){
    ...
    var m = this.to_innersvg_math(cnt,used,style);
    var [w,h,mid,s] = m;
    return `\\ensuremath{\\sqrt[${n}]{${s}}}`;
  }
  to_phrase_overline(style,cnt) {
    var used = new Set();
    var m = this.to_innersvg_math(cnt,used,style);
    var [w,h,mid,s] = m;
    return `\\ensuremath{\\overline{${s}}}`;
  }
  to_phrase_overleftrightarrow(style,cnt) {
    var used = new Set();
    var m = this.to_innersvg_math(cnt,used,style);
    var [w,h,mid,s] = m;
    return `\\ensuremath{\\overleftrightarrow{${s}}}`;
  }
  to_phrase_overrightarrow(style,cnt) {
    var used = new Set();
    var m = this.to_innersvg_math(cnt,used,style);
    var [w,h,mid,s] = m;
    return `\\ensuremath{\\overrightarrow{${s}}}`;
  }
  to_phrase_underleftrightarrow(style,cnt) {
    var used = new Set();
    var m = this.to_innersvg_math(cnt,used,style);
    var [w,h,mid,s] = m;
    return `\\ensuremath{\\underleftrightarrow{${s}}}`;
  }
  to_phrase_underrightarrow(style,cnt) {
    var used = new Set();
    var m = this.to_innersvg_math(cnt,used,style);
    var [w,h,mid,s] = m;
    return `\\ensuremath{\\underrightarrow{${s}}}`;
  }
  to_phrase_subsup(style,cnt,cnt2,cnt3) {
    ...
    if(cnt2 && cnt3){
      return `\\ensuremath{{${s}}_{${s2}}^{${s3}}}`;
    }else if(cnt2){
      return `\\ensuremath{{${s}}_{${s2}}}`;
    }else if(cnt3){
      return `\\ensuremath{{${s}}^{${s3}}}`;
    }else{
      return `\\ensuremath{${s}}`
    }
  }
  to_phrase_sum(style,cnt,cnt2,cnt3) {
    ...
    var m3 = this.to_innersvg_math(cnt3,used,style);
    var [w3,h3,mid3,s3] = m3;
    return `\\ensuremath{\\sum_{${s}}^{${s2}}{${s3}}}`;
  }
  to_phrase_int(style,cnt,cnt2,cnt3) {
    ...
    var m3 = this.to_innersvg_math(cnt3,used,style);
    var [w3,h3,mid3,s3] = m3;
    return `\\ensuremath{\\int_{${s}}^{${s2}}{${s3}}}`;
  }
