...
\\setuphead[section][margin=0pt,style=\\bfb,number=no,page=yes,margin=-3.5mm,color=titleblue,after={}]
\\setuphead[subsection][style=\\small\\bold,number=no,continue=yes,before={},after={}]
\\setuphead[subsubsection][style=bold,number=no,continue=yes,before={},after={}]
...
`;
  ...
  write_frame_folder(id,frame,subframes){
    let all = [];
    all.push('');
    all.push(`\\startsection[title={${id} ${this.uncode(frame.style,frame.title)}\\\\\\small ~ },reference=${id},bookmark={${id}. ${this.revise(frame.raw)}}]`); 
    ...
    subframes.forEach((subframe,i,arr) => {
      ...
      let label = `${id}.${i+1}`;
      all.push(`\\sym {${label}}~ \\goto{\\it ${title}}[${label}]`);
    });  
    all.push(`\\stopitemize`);
    all.push(`\\stopsection`) 
    var text = all.join('\n');
  ...
  write_one_frame(id,order,icon,frame,issub,topframe) {
    ...
    if(1){
      ...
      all.push('');
      if(issub){
        all.push(`\\startsection[title={${order} ${this.uncode(topframe.style,topframe.title)}\\\\\\small ${this.uncode(frame.style,frame.tytle)}},reference=${id},bookmark={${order} ${this.revise(frame.raw)}}]`); 
        //all.push(`\\startsubsection[title={${this.uncode(frame.style,frame.title)}},reference=,bookmark=]`); 
        order = '';
      }else{
        icon = '';
        all.push(`\\startsection[title={${order} ${this.uncode(topframe.style,topframe.title)}\\\\\\small ~},reference=${id},bookmark={${order}. ${this.revise(topframe.raw)}}]`); 
        //all.push(`\\startsubsection[title={~},reference=,bookmark=]`); 
      }
      ...
      frame.solutions.forEach((o,i,arr) => {
        all.push('')
        if(o.choice){
          all.push(`{${this.icon_solution}} \\underbar{${this.uncode(o.style,o.title)}}`);
    ...
    frame.solutions.forEach((o,i,arr) => {     
      ...
      if(o.choice){
        ...
        let text = this.to_choice(o.style,o.body,o.choice);
        all.push(`\\startsection[title={~}]`);//empty
        all.push(`\\startsubsection[title={${icon} {\\underbar{${title}}}\\\\\\small ${text}},reference=,bookmark={${icon} ${title}}]`);//empty 
