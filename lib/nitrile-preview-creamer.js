...
\\setuphead[subsection][style=bold,number=no,continue=yes,before={},after={}]
...
`;
  ...
  write_one_frame(id,order,icon,frame,issub,topframe) {
    ...
    if(1){
      ...
      if(issub){
        let icon = this.icon_folder;
        all.push(`\\startsection[title={${order} ${this.uncode(topframe.style,topframe.title)} ${icon} \\small ${this.uncode(frame.style,frame.title)}},reference=${id},bookmark={${order} ${this.revise(frame.raw)}}]`); 
