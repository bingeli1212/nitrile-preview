...
\\setuphead[section][style=tfb,number=no,page=yes,margin=-7mm,color=titleblue,after={}]
...
`;
  ...
  write_frame_folder(id,frame,subframes){
    let all = [];
    let s_ad = String.fromCodePoint(0xAD);
    all.push('');
    all.push(`\\startsection[title={${id} ${this.uncode(frame.style,frame.title)}},reference=${id},bookmark={${id}. ${this.revise(frame.raw)}}]`); 
    all.push(`\\godown[-0.25cm]`)
    all.push(`\\startsubsubsection[title={${s_ad}~},reference=,bookmark=]`); 
  ...
  write_one_frame(id,order,icon,frame,issub,topframe) {
    ...
    if(1){
      ...
      if(issub){
        ...
        all.push(`\\godown[-0.25cm]`)
        all.push(`\\startsubsubsection[title={${icon} {\\it ${this.uncode(frame.style,frame.title)}}},reference=,bookmark=]`); 
        order = '';
      }else{
        let icon = '';
        let s_ad = String.fromCodePoint(0xAD);
      ...
      frame.solutions.forEach((o,i,arr) => {
        all.push('')
        if(o.choice){
          all.push(`{${this.icon_solution}} {\\it ${this.uncode(o.style,o.title)}}\\crlf`);
