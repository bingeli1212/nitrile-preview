...
`;
  ...
  write_frame_folder(id,frame,subframes){
    let all = [];
    all.push('');
    all.push(`\\startsection[title={${this.uncode(frame.style,frame.title)}},reference=${id},bookmark={${this.revise(frame.raw)}}]`); 
  ...
  write_frame(id,frame,issub) {
    ...
    if(1){
      ...
      if(issub){
        all.push(`\\startsection[title={${this.icon_subpoint} ${this.uncode(frame.style,frame.title)}},reference=${id},bookmark={${String.fromCodePoint(0x261E)} ${this.revise(frame.raw)}}]`); 
      }else{
        all.push(`\\startsection[title={${this.uncode(frame.style,frame.title)}},reference=${id},bookmark={${this.revise(frame.raw)}}]`); 
      }
