...
`;
    ...
    top.sections.forEach((sect,i,arr) => {
      ...
      if(sect.sections.length){
        ...
        subframes.forEach((subframe,j) => {
          let out = this.write_frame(`${i}:${j}`,subframe,1);
          all_main.push(out);
        });
      }else{
        let out = this.write_frame(`${i}`,topframe,0);
        all_main.push(out);
      }
    });
  ...
  write_frame_folder(id,frame,subframes){
    let all = [];
    all.push('');
    all.push(`\\startsection[title={${this.icon_keypoint} ${this.uncode(frame.style,frame.title)}},reference=${id},bookmark={${this.revise(frame.raw)}}]`); 
