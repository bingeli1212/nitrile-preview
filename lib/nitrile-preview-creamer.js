...
class NitrilePreviewCreamer extends NitrilePreviewContex {
  ...
  to_document() {
    var style_setupbodyfont = this.conf_to_string('bodyfont','linux,10pt');
...
`;
  ...
  write_one_frame(id,order,icon,frame,issub,topframe) {
    ...
    if(1){
      ...
      all.push('');
      if(issub){
        all.push(`\\startsection[title={${order} ${this.uncode(topframe.style,topframe.title)}\\\\{\\small ${icon} ${this.uncode(frame.style,frame.title)}}},reference=${id},bookmark={${order} ${icon} ${this.revise(frame.raw)}}]`); 
        order = '';
      }else{
        icon = '';
