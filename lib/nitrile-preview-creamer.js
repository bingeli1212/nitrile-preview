...
class NitrilePreviewCreamer extends NitrilePreviewContex {
  ...
  to_document() {
    //this.icon_keypoint = `{\\switchtobodyfont[dingbats]❖}`;//U+2756 Black Diamond Minus White X
    //this.icon_exercise = `{\\switchtobodyfont[dingbats]✎}`;//U+270E Down Right Pencil
    //this.icon_folder   = `{\\switchtobodyfont[dingbats]➥}`;//U+27A5 Heavy Black Curved Downwards and Rightwards Arrow
    //this.icon_solution = `{\\switchtobodyfont[dingbats]✍}`;//U+270D Writing Hand
    ...
    var style_setupbodyfont = this.conf_to_string('creamer.setupbodyfont','11pt');
...
`;
  ...
  write_frame(frame) {
    ...
    if(1){
      ...
      all.push(`\\startsection[title={\\directcolored[r=0.0627,g=0.0627,b=0.6902]{${icon} ${this.uncode(frame.style,frame.title)}}},bookmark={${frame.raw}}]`); 
      //all.push(`\\startsubsection[title={}]`);
      frame.contents.forEach((x,i) => {
        //'x' is a block
        let latex = this.translate_block(x);
    ...
    frame.solutions.forEach((o,i,arr) => {     
      all.push('');
      let icon = frame.isex?this.icon_exercise:this.icon_keypoint;
      all.push(`\\startsection[title={\\directcolored[r=0.0627,g=0.0627,b=0.6902]{${icon} ${this.uncode(frame.style,frame.title)}}},bookmark={${frame.raw} - ${o.raw}}]`); 
  ...
  to_titlepage(){
    ...
    all.push(`\\tfd \\color[myblue]{${this.uncode(style,title)}} \\crlf`);
    all.push(`\\blank[1cm]`)
    all.push(`\\tfa ${this.uncode(style,subtitle)} \\crlf`);
    all.push(`\\blank[0.2cm]`)
    all.push(`\\tfa ${this.uncode(style,institute)} \\crlf`);
    all.push(`\\blank[0.2cm]`)
    all.push(`\\tfa ${this.uncode(style,author)} \\crlf`);
    all.push(`\\blank[0.2cm]`)
