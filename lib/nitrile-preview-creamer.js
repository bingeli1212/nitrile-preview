...
`;
  ...
  write_frame(id,frame) {
    ...
    frame.solutions.forEach((o,i,arr) => {     
      ...
      if((v=re_choice.exec(o.title))!==null){
        all.push(`\\startsection[title={${icon} \\it ${this.uncode(o.style,v[1])}},reference=${id}:${i},bookmark={${this.revise(v[1])}}]`); 
        let text = this.to_choice(o.style,o.body,v[2]);
        all.push(`\\startsubsection[title={${text}},bookmark={}]`);
      }else{
        all.push(`\\startsection[title={${icon} \\it ${this.uncode(o.style,o.title)}},reference=${id}:${i},bookmark={${this.revise(o.title)}}]`); 
