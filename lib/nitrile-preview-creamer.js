...
`;
  ...
  write_frame_folder(id,frame,subframes){
    let all = [];
    all.push('');
    all.push(`\\startsection[title={${this.icon_folder} ${this.uncode(frame.style,frame.title)}},reference=${id},bookmark={${frame.raw}}]`); 
  ...
  write_frame(id,frame) {
    ...
    if(1){
      ...
      all.push(`\\startsection[title={${icon} ${this.uncode(frame.style,frame.title)}},reference=${id},bookmark={${frame.raw}}]`); 
    ...
    frame.solutions.forEach((o,i,arr) => {     
      all.push('');
      let icon = frame.isex?this.icon_exercise:this.icon_keypoint;
      all.push(`\\startsection[title={${icon} ${this.uncode(frame.style,frame.title)}},reference=${id}:${i},bookmark={${frame.raw} - ${o.raw}}]`); 
