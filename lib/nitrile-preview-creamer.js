...
\\setuphead[subsection][style=\\small\\bold,number=no,continue=yes,before={}]
...
`;
  ...
  write_one_frame(id,order,icon,frame,issub,topframe) {
    ...
    if(1){
      ...
      all.push('');
      if(issub){
        all.push(`\\startsection[title={${order} ${this.uncode(topframe.style,topframe.title)}\\\\{\\small\\it ${icon} ${this.uncode(frame.style,frame.title)}}},reference=${id},bookmark={${order} ${icon} ${this.revise(frame.raw)}}]`); 
        order = '';
      }else{
        icon = '';
    ...
    frame.solutions.forEach((o,i,arr) => {     
      ...
      if(o.choice){
        ...
        all.push(`\\startsubsection[title={${icon} \\underbar{${title}} ~ ${text}},bookmark={}]`);
      }else{
