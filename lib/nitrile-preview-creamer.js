...
`;
  ...
  write_one_frame(id,order,icon,frame,issub,topframe) {
    ...
    if(1){
      ...
      all.push('');
      if(issub){
        all.push(`\\startsection[title={${order} ${this.uncode(topframe.style,topframe.title)}},reference=${id},bookmark={${order} ${this.revise(frame.raw)}}]`); 
    ...
    frame.solutions.forEach((o,i,arr) => {     
      ...
      if(o.choice){
        ...
        all.push(`\\startsection[title={${icon} {\\underbar{${title}}}},reference=,bookmark={${icon} ${title}}]`);//empty 
        all.push(`\\startsubsubsection[title={${text}},bookmark={}]`);
      }else{
        let title = this.uncode(o.style,o.title).trim();
