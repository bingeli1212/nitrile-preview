...
\\setuphead[section][style=\\bfb,number=no,page=yes,margin=-3.5mm,color=titleblue,after={}]
\\setuphead[subsection][style=\\bold,number=no,continue=yes,before={}]
\\setuphead[subsubsection][style=\\small\\bold,number=no,continue=yes,before={}]
...
`;
  ...
  write_frame_folder(id,frame,subframes){
    ...
    subframes.forEach((subframe,i,arr) => {
      ...
      let label = `${id}.${i+1}`;
      all.push(`\\sym {${icon}} \\goto{\\it ${title}}[${label}]`);
    });  
    all.push(`\\stopitemize`);
    all.push(`\\stopsection`) 
    var text = all.join('\n');
  ...
  write_one_frame(id,order,icon,frame,issub,topframe) {
    ...
    if(1){
      ...
      all.push('');
      if(issub){
        all.push(`\\startsection[title={${order} ${this.uncode(topframe.style,topframe.title)}},reference=${id},bookmark={${order} ${icon} ${this.revise(frame.raw)}}]`); 
        all.push(`\\startsubsection[title={${icon} ${this.uncode(frame.style,frame.title)}},reference=,bookmark=]`); 
        order = '';
      }else{
        icon = '';
        all.push(`\\startsection[title={${order} ${icon} ${this.uncode(topframe.style,topframe.title)}},reference=${id},bookmark={${order} ${icon} ${this.revise(topframe.raw)}}]`); 
      }
      ...
      ///
      if(frame.boardname){
        all.push(`\\definelayer[test]`);
        all.push(`\\setlayer[test][x=40pt,y=20pt]{\\externalfigure[${frame.boardname}.png][width=290pt,height=212pt]}`);
        all.push(`\\flushlayer[test]`);
      }
      ///
      ///contents
      ///
    ...
    frame.solutions.forEach((o,i,arr) => {     
      ...
      all.push(`\\startsection[title={},reference=,bookmark={}]`);//empty 
      if(o.choice){
        let title = this.uncode(o.style,o.title);
        let text = this.to_choice(o.style,o.body,o.choice);
        all.push(`\\startsubsubsection[title={${icon} \\underbar{${title}} ~ ${text}},bookmark={}]`);
      }else{
