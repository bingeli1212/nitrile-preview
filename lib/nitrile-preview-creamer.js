...
class NitrilePreviewCreamer extends NitrilePreviewContex {
  ...
  constructor(parser) {
    ...
    this.icon_exercise = '{\\symbol[martinvogel 2][PointingHand]}';
    this.icon_folder   = '';
...
`;
  ...
  to_body() {
    ...
    top.sections.forEach((sect,i,arr) => {
      ...
      if(sect.sections.length){
        ...
        let out = this.write_frame_folder(`${i}`,topframe,subframes);
        all_main.push(out);
        subframes.forEach((subframe,j) => {
          let out = this.write_frame(`${i}:${j}`,subframe);
          all_main.push(out);
        });
      }else{
        let out = this.write_frame(`${i}`,topframe);
        all_main.push(out);
      }
    });
  ...
  write_frame_folder(id,frame,subframes){
    let all = [];
    all.push('');
    all.push(`\\startsection[title={${this.icon_folder} ${this.uncode(frame.style,frame.title)}},reference=${id},bookmark={${this.revise(frame.raw)}}]`); 
  ...
  write_frame(id,frame) {
    ...
    if(1){
      ...
      all.push(`\\startsection[title={${icon} ${this.uncode(frame.style,frame.title)}},reference=${id},bookmark={${this.revise(frame.raw)}}]`); 
      ...
      frame.solutions.forEach((o,i,arr) => {
        if((v=re_choice.exec(o.title))!==null){
          all.push(`\\sym {${this.icon_solution}} {\\it ${this.uncode(o.style,v[1])}}`);
    ...
    frame.solutions.forEach((o,i,arr) => {     
      all.push('');
      let icon = frame.isex?this.icon_exercise:this.icon_keypoint;
      all.push(`\\startsection[title={${icon} ${this.uncode(frame.style,frame.title)}},reference=${id}:${i},bookmark={}]`); 
      if((v=re_choice.exec(o.title))!==null){
        let text = this.to_choice(o.style,o.body,v[2]);
        all.push(`\\startsubsection[title={{\\it ${this.uncode(o.style,v[1])}} ~ ${text}},bookmark={}]`);
      }else{
  ...
  to_choice(style,body,check){
    ...
    var re_word = /^(\w+)/;
    var v;
    var checks = this.string_to_array(check);
