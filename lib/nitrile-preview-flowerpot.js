...
console.log('board_onsubmitclicked',name,e);
      ...
      if(ctx.cake.myop=='ball'){ //board_redraw ball
        ...
        o.myball.balls.forEach((ball,j) => {
          ...
          if(o.myball.hitball===ball){
            ///DRAW four control points including corner
            ball.corners.forEach((pt,i) => {
              ctx.fillStyle = 'yellow';
    ...
    function board_dup(ctx,canvas){
      ...
      if(ctx.cake.myop=='ball'){
        if(o.myball.hitball){
          ...
          let dup = {points,transform,corner_i,pivot_x,pivot_y,color,w,h};
    ...
    function board_start(ctx,canvas){
      ...
      if(ctx.cake.myop=='ball'){ //board_start ball
        o.myball.altKey = ctx.altKey;
        o.myball.shiftKey = ctx.shiftKey;
        ...
        if(altKey && shiftKey){
          if(o.myball.hitball){
            ...
            ball.pivot_x0 = ball.pivot_x;
            ball.pivot_y0 = ball.pivot_y;
          }else{
            //create a new ball                                     
            var x = ctx.posx0; 
            var y = ctx.posy0; 
            ...
            var color = 'gray';
            var m = Math.PI/180;
            var points = board_get_ball_points('tri',30,30,ctx,canvas);
            var corners = board_get_ball_corners('tri',30,30,ctx,canvas);
            if(points.length){
              let ball = {points,corners,transform,corner_i,pivot_x,pivot_y,color,w:30,h:30};
              o.myball.balls.push(ball);
          ...
          if(o.myball.hitball){
            o.myball.hitball.corner_i = ball_find_hitcorner(ctx.posx0,ctx.posy0, o.myball.hitball,ctx);
            console.log('corner_i',o.myball.hitball.corner_i);
          }
          if(!o.myball.hitball || o.myball.hitball.corner_i < 0){
            o.myball.hitball = ball_find_hitball(ctx.posx0,ctx.posy0, o.myball.balls,ctx);
            if(o.myball.hitball){
              let ball = o.myball.hitball;
              o.myball.hitball.corner_i = ball_find_hitcorner(ctx.posx0,ctx.posy0, o.myball.hitball,ctx);
    ...
    function board_drag(ctx,canvas){
      ...
      if(ctx.cake.myop=='ball'){ //board_drag ball
        if(o.myball.hitball){
          let ball = o.myball.hitball;
          ...
          if(o.myball.istodo=='move'){
            ...
            o.myball.hitball.pivot_x = o.myball.hitball.pivot_x0 + dx;
            o.myball.hitball.pivot_y = o.myball.hitball.pivot_y0 + dy;
          }
          if(o.myball.istodo=='size'){
            let dx = ctx.posx2 - ctx.posx1;
            let dy = ctx.posy2 - ctx.posy1;
            let w = ball.w;
            let h = ball.h;
            ...
            o.myball.hitball.corners.forEach((pt) => {
              pt.x *= vx;
              pt.y *= vy;
            });
            o.myball.hitball.w *= vx;
          ...
          if(o.myball.istodo=='skewX'){
console.log('it is skewX');
            let dx = ctx.posx2 - ctx.posx1;
            let h = ball.h;
            let lambda = (1+dx/h);
console.log('lambda',lambda);
            ...
            o.myball.hitball.points.forEach((pt) => {
              ...
              pt.x *= lambda*pt.y;
              pt.y *= 1;
              pt.cp1x *= lambda*pt.cp1y;
              pt.cp1y *= 1;
              pt.cp2x *= lambda*pt.cp2y;
              pt.cp2y *= 1; 
              pt.x1 *= lambda*pt.y1;
              pt.y1 *= 1; 
              pt.x2 *= lambda*pt.y2;
            ...
            o.myball.hitball.w *= 1;
            o.myball.hitball.h *= 1;
          }
        }
        board_redraw(ctx,canvas);
      }
    ...
    function board_end(ctx,canvas){
      ...
      if(ctx.cake.myop=='ball'){ //board_end ball
        o.myball.altKey = 0;
        o.myball.shiftKey = 0;
        o.myball.istodo = '';
        board_redraw(ctx,canvas);
    ...
    function ball_find_hitcorner(sx,sy, hitball,ctx){
      // re-discover corners only -- no rediscovering of new hit balls
      var success = 0;
      var corner_i = -1;
      ctx.save();
      let [a,b,c,d,e,f] = hitball.transform;
      let {pivot_x,pivot_y} = hitball;
      ctx.resetTransform();
      ctx.translate(pivot_x,pivot_y);
      ctx.transform(a,b,c,d,e,f);
      hitball.corners.forEach((pt,i) => {
        let {x,y} = pt;
        if(Number.isFinite(x) && Number.isFinite(y)){
          let path2d = new Path2D();
          const hit_size = 4;
