...
//console.log('x-intercept','y0=',y0,'yi=',yi,'x0=',x0,'xii=',xii,'sign0=',sign0,'sign=',sign);
    ...
    function ball_start(ctx,canvas){
      ...
      if(ctx.note.downball && ctx.note.downcorner >= 0 && ctx.note.downcorner != ctx.note.hitcorner){
        ...
        ctx.note.rotate_ang = 0;
        let [x0,y0] = ball_getcornerxy(ctx.note.downball,ctx.note.downcorner);
        ctx.note.corner_x = x0;
        ctx.note.corner_y = y0;
    ...
    function ball_drag(ctx,canvas){
      if(ctx.note.rotateflag){
        let angle0 = Math.atan2(ctx.posy0-ctx.note.corner_y, ctx.posx0-ctx.note.corner_x);
        let angle1 = Math.atan2(ctx.posy2-ctx.note.corner_y, ctx.posx2-ctx.note.corner_x);
        ctx.note.rotate_ang = angle1-angle0;
      }else if(ctx.note.moveflag){
        ctx.note.translate_x = ctx.posx2 - ctx.posx0;
        ctx.note.translate_y = ctx.posy2 - ctx.posy0;
      }
      ball_redraw(ctx,canvas);
    }
    function ball_end(ctx,canvas){
      if(ctx.note.rotateflag && ctx.note.rotate_ang != 0){
        ctx.save();
    ...
    function ball_getcornerxy(ball,corner){
      let x = ball.points[corner].x;
      let y = ball.points[corner].y;
      return [x,y];
    }
    function ball_redraw(ctx,canvas){
      ...
      ctx.note.balls.forEach((ball) => {
        ...
        if(ctx.note.rotateflag && ball == ctx.note.downball){
          ctx.translate(ctx.note.corner_x,ctx.note.corner_y);
          ctx.rotate(ctx.note.rotate_ang);
          ctx.translate(-ctx.note.corner_x,-ctx.note.corner_y);
        ...
        if(ctx.note.downball===ball){
          ...
          ball.points.forEach((pt,i) => {
            ...
            if( ctx.isPointInPath(ctx.posx2,ctx.posy2) ){
