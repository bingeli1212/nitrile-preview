...
`;
  ...
  write_frame_folder(id,frame,subframes,toc,pages){
    ...
    let img_id = `img${id}`;
    let edit_contents = this.to_edit_contents(board_id,img_id,frame.boardname);
    all.push(`<div class='BOTTOMCONTAINER'>`);
    all.push(`<input class='EDITBUTTON' type='checkbox'></input>`);
    all.push(`<canvas class='EDITBOARD' id='${board_id}' data-type='board' width='441' height='320'></canvas>`);
  ...
  write_one_frame(id,order,icon,frame,issub,parentid,toc,pages){
    ...
    all.push(`<input class='EDITBUTTON' type='checkbox'></input>`);
    all.push(`<canvas class='EDITBOARD' id='${board_id}' data-type='board' width='441' height='320'></canvas>`);
    all.push(`<form class='EDITFORM' action='/echo' method='post' onsubmit='board_submit(this,"${board_id}","${boardname}")'> ${edit_contents} </form>`);
    all.push(`<div class='PAGENUM'> / </div>`);
    all.push(`</div>`);
    all.push(`<div class='SIDECONTAINER'><img width='90' height='360' style='background:var(--side${id}) 0% 0% no-repeat' usemap='#map${id}' /></div>`);
    ...
    frame.solutions.forEach((o,i,arr) => {
      ...
      pages.push(1);
      let board_id = `board${id}a${i}`;
      let img_id = `img${id}a${i}`;
      let edit_contents = this.to_edit_contents(board_id,img_id,frame.boardname);
      all.push(`<div class='BOTTOMCONTAINER'>`);
      all.push(`<input class='EDITBUTTON' type='checkbox'></input>`);
      all.push(`<canvas class='EDITBOARD' id='${board_id}' data-type='board' width='441' height='320'></canvas>`);
  ...
  to_edit_contents(canvasid,imgid,boardname){
    ...
    return `\
       <select name='${canvasid}' onchange='select_change_line(this.value,this.name)'>
          <optgroup label='Pencil'>
           <option value='line/1.5'>Light Pencil</option>
           <option value='line/2.5'>Med Pencil</option>
           <option value='line/4'>Heavy Pencil</option>
          </optgroup>
          <optgroup label='Smudge'>
           <option value='smudge/1.5'>Light Smudge</option>
           <option value='smudge/2.5'>Med Smudge</option>
           <option value='smudge/4'>Heavy Smudge</option>
          </optgroup>
          <optgroup label='Erasers'>
            <option value='eraser/1.5'>Light Eraser</option>
            <option value='eraser/2.5'>Med Eraser</option>
       ...
       <textarea class='EDITDATAURI' name='textarea' style='display:none'>
       </textarea>
       <textarea class='EDITFILENAME' name='filename' style='display:none'>
       </textarea>
       <img id='${imgid}' src='${imgsrc}' style='display:none'/>
       <button type='button' onclick='board_load_png("${canvasid}","${imgid}")'>Load</button>
       <button>Save</button>
       `;
  }
  ////////////////////////////////////////////////////////////////////////
  ...
  to_setup_script(){
    ...
    function board_draw_dot(evt,ctx,canvas,first){
      ...
      ctx.d.myposx = (evt.clientX-canvas.getBoundingClientRect().x)*sx;
      ctx.d.myposy = (evt.clientY-canvas.getBoundingClientRect().y)*sy;
      if(1){
        if(myop == 'eraser'){
          ///ERASER
          if(myline==0){
          ...
          ///SMUDGE
          var lw = myline*sx;
          var idata = ctx.getImageData(ctx.d.myposx-(lw/2),ctx.d.myposy-(lw/2),lw,lw);
          if(first){
            smudgeRect(ctx.d.myposx-(lw/2),ctx.d.myposy-(lw/2),lw,lw,mysmear,ctx);
          }else{
            smudge_between(myposx0,myposy0,ctx.d.myposx,ctx.d.myposy,lw,mysmear,ctx);
          }
        }else if(myop == 'line'){
          ///PENCIL
          var lw = myline*sx;
          ...
          if(first){
            ...
            draw_between(myposx0,myposy0,ctx.d.myposx,ctx.d.myposy,lw,ctx);
          }
        }
      }
    }
    ...
    function board_load_png(canvasid,imgid){
      console.log('james','borad_load_png',canvasid);
      var img = document.getElementById(imgid);
      let canvas = document.getElementById(canvasid);
      let ctx = canvas.getContext("2d");
      ctx.drawImage(img,0,0,canvas.getAttribute('width'),canvas.getAttribute('height'));
    }
    function __board_save_png__(canvasid){
      console.log('james','borad_save_png',canvasid);
    ...
    function board_submit(form,canvasid,boardname){
      let canvas = document.getElementById(canvasid);
      let dataURL = canvas.toDataURL('image/png');
