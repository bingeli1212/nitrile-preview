  ...
  to_setup_script(){
    ...
    function board_setup_canvas(canvas){
      ...
      if(1){
        ...
        ctx.myfs = 12;
        ctx.mytext = '';
        var myo = board_create_myo(ctx,canvas);
        ctx.myundo.push(myo);
        ///
        board_setstrokeid(0,ctx,canvas);
        board_setcolorid(0,ctx,canvas);
        board_set_op('pencil',ctx,canvas);
        board_set_shape('line',ctx,canvas);
      }
    }
    function board_create_myo(ctx,canvas){ 
      let stroke = board_curr_stroke(ctx,canvas);
      let color = board_curr_color(ctx,canvas);
      let sh = ctx.mysh;
      let x = ctx.posx0;
      let y = ctx.posy0;
      let myfig = {x,y,w:0,h:0,rotate:0,skewX:0,skewY:0,stroke,color,sh};
      ...
      var s = ctx.getImageData(0,0, canvas.getAttribute('width'),canvas.getAttribute('height'));
	    var mode = ctx.mode;
      var myo = {s,mybasis,myop,myfig,mysh,mytext,myfs,mode};
      ///MISC
      myo.zoom = 1;
      myo.grid = 30;
      myo.origin_x = Math.round(canvas.width/2);
      myo.origin_y = Math.round(canvas.height/2);
      myo.action = 0;
      myo.scale_factor = 1.0;
      myo.rotate_ang = 0;//in degrees clockwise-rotation is positive
      myo.move_x = 0.0;//pixels
      myo.move_y = 0.0;//pixels
      myo.pointisinhandle = 0;
      myo.val_real = 2;
      myo.val_imag = 1;
      myo.info_text = '';
      myo.info_xi = 0;
      ...
      myo.corner_size = 4;
      myo.hit_ball = null;
      myo.hit_transform = [1,0,0,1,0,0];
      myo.ctx_transform = [1,0,0,1,0,0];
      return myo;          
    }
    ...
    function board_redraw(ctx,canvas){
      var n = ctx.myundo.length;
      var o = ctx.myundo[n-1];  
      ctx.resetTransform();
...
console.log('x-intercept','y0=',y0,'yi=',yi,'x0=',x0,'xii=',xii,'sign0=',sign0,'sign=',sign);
    ...
    function board_undo(ctx,canvas){
      if(ctx.myundo.length>1){
        let o = ctx.myundo.pop();
        ctx.myredo.push(o);
      }
      let n = ctx.myundo.length;
      let o = ctx.myundo[n-1];
      ctx.setTransform(...o.hit_transform);
    }
    function board_revundo(ctx,canvas){
      if(ctx.myredo.length>0){
        let o = ctx.myredo.pop();
        ctx.myundo.push(o);
      }
      let n = ctx.myundo.length;
      let o = ctx.myundo[n-1];
    ...
    function board_reset(ctx,canvas){
      ...
      while(ctx.myundo.length > 1){
        ctx.myundo.pop();
      }
      ctx.myredo = [];
    }
    function board_action(ctx,canvas){
      var o = board_last_cake(ctx,canvas); 
      o.action++;
      if(o.myop=='cplane'){
        o.rotate_ang = 0;
        o.scale_factor = 1;
    ...
    function board_setcolorid(v,ctx,canvas){
      ...
      if(o.myop=='ball'){
        o.colorid = v;
        if(o.hit_ball){
          o.hit_ball.color = board_curr_color(ctx,canvas);
        }
      }
      if(1){
        let color = board_curr_color(ctx,canvas);
    ...
    function board_draw_pencil(ctx,canvas,first){
      ...
      let posx2 = ctx.posx2;
      let posy2 = ctx.posy2;
console.log('pos1,pos2',posx1,posy1,posx2,posy2);
    ...
    ////////////////////////////////////////////
    ////////////////////////////////////////////
    function board_last_cake(ctx,canvas){
      let n = ctx.myundo.length;
      let o = ctx.myundo[n-1]
console.log('n=',n);
      return o;
    }
    function board_new_cake(ctx,canvas){
      if(1){
        let o = board_create_myo(ctx,canvas);
        ctx.myundo.push(o);
        ctx.myredo = [];
        ctx.setTransform(...o.ctx_transform);
      }
      return board_last_cake(ctx,canvas);
    }
    ////////////////////////////////////////////
    ////////////////////////////////////////////
    ////////////////////////////////////////////
    ...
    function board_start(ctx,canvas){
      if(ctx.myop=='pencil'){ //pencil draw shape
        let o = board_new_cake(ctx,canvas);
        if(ctx.shiftKey){
          o.myfig.sh = o.mysh;
          o.myfig.x = ctx.posx0;
          o.myfig.y = ctx.posy0;
      ...
      if(ctx.myop=='select'){ //board_start
        ...
        if( (ctx.posx0 > o.mybasis.x) &&
          ...
          ///hilite-only 
          o.mybasis.s = board_get_face(ctx,canvas);
          o.mybasis.a = null;
      ...
      if(ctx.myop=='cplane'){ //board_start
        let o = board_last_cake(ctx,canvas);
        if(o.myop=='cplane'){
        }else{
          o = board_new_cake(ctx,canvas);
        }
      ...
      if(ctx.myop=='ball'){ //board_start
        ...
        console.log('hit_ball=',o.hit_ball);
        board_redraw(ctx,canvas);
      }
      if(ctx.myop=='xyplane'){ //board_start
        ...
        for(var j=0; j < o.y_intercepts.length; ++j){
          ...
          if(ctx.isPointInPath(p.pathyi,x,y)){
            ...
            o.info_xi = 0;
            o.info_yi = p.yi;
console.log('o.info_text',o.info_text,o.info_xi,o.info_yi);
          } 
        }
    ...
    function board_drag(ctx,canvas){
      ...
      if(o.myop=='select'){//board_drag
        ...
        if(o.mybasis.a){
          ...
          if(o.mybasis.a=='move'){
            ...
            o.mybasis.s = ctx.getImageData(0,0, canvas.getAttribute('width'),canvas.getAttribute('height'));
            ///draw hilite
            o.mybasis.dx = dx;
            o.mybasis.dy = dy;
            note_draw_hilite(o,ctx,canvas);
      ...
      if(o.myop=='ball'){//board_drag
        if(ctx.shiftKey && o.hit_ball){
          ...
          o.rotate_ang = (angle1-angle0)/Math.PI*180;
        }else if(o.hit_ball){
          o.move_x = ctx.posx2 - ctx.posx1;
          o.move_y = ctx.posy2 - ctx.posy1;
        }
        board_redraw(ctx,canvas);
      }
