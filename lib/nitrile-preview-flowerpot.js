...
`;
  ...
  write_one_frame(id,order,icon,frame,issub,parentid,toc,pages,topframe){
    ...
    all.push(`<div class='SIDECONTAINER'><img width='88' height='360' style='content:var(--side${id})' usemap='#map${id}' /></div>`);
    if(issub){
      all.push(`<h1 class='${title_css}'>${order} <a href='#frame${parentid}'>${icon}</a> <b>${this.uncode(topframe.style,topframe.title)}<br/><span style='font-size:0.5em'>${this.uncode(frame.style,frame.title)}</span></b> </h1>`);
    }else{
      all.push(`<h1 class='${title_css}'>${order} <a href='#frame${parentid}'>${icon}</a> <b>${this.uncode(topframe.style,topframe.title)}</b> </h1>`);
    }
