...
`;
  ...
  to_edit_contents(boardid,buttonopid,buttonshid,buttonclid){
    return `\
       <select name='${boardid}' style='margin-left:1px;margin-right:1px;' onchange='board_onchangemode(this.value,this.name)'>
           <option value='note'>Notes</option>
           <option value='cplane'>Complex Plane</option>
           <option value='xyplane'>Cartesian Plane</option>
           <option value='ball'>Balls</option>
       </select>
       <input type='text' name='${boardid}' style='margin-left:1px;margin-right:1px;' onchange='board_change_text(this.value,this.name)'/>
       ...
       <div style='position:relative'>
         ...
         <div class='DROPMENU' style='display:flex;flex-direction:column;align-items:flex-start;justify-content:center;' >
           <div style='display:flex;flex-direction:row;align-items:center;justify-content:flex-start;' >
             <button class='ONECL' name='${boardid}'  data-cl='0'  onclick="board_onecolorclicked(this.name,0)"></button>
             <button class='ONECL' name='${boardid}'  data-cl='1'  onclick="board_onecolorclicked(this.name,1)"></button>
             <button class='ONECL' name='${boardid}'  data-cl='2'  onclick="board_onecolorclicked(this.name,2)"></button>
             <button class='ONECL' name='${boardid}'  data-cl='3'  onclick="board_onecolorclicked(this.name,3)"></button>
             <button class='ONECL' name='${boardid}'  data-cl='4'  onclick="board_onecolorclicked(this.name,4)"></button>
             <button class='ONECL' name='${boardid}'  data-cl='5'  onclick="board_onecolorclicked(this.name,5)"></button>
             <button class='ONECL' name='${boardid}'  data-cl='6'  onclick="board_onecolorclicked(this.name,6)"></button>
             <button class='ONEST' name='${boardid}'               onclick="board_onestrokeclicked(this.name)"></button>
           </div>
         </div>
       </div>
  ...
  to_setup_script(){
    ...
    all_strokes = [1.6,3.0];
    all_canvas = null;
    all_op_icons = {'pencil':'&#x270E;','select':'&#x2B1A;','smudge':'&#x271C;','eraser':'&#x232B;'};
    ...
    console.log('board_oneopclicked',v);
      var canvas = document.getElementById(name);
      var ctx = canvas.getContext('2d');
      if(v=='select'){
        board_select(ctx,canvas);                   
      }else if(v=='smudge'){
        board_smudge(ctx,canvas);                   
      }else if(v=='eraser'){
        board_eraser(ctx,canvas);                   
      }else if(v=='pencil'){
        board_pencil(ctx,canvas);                   
      }
    }
    function board_oneshclicked(name,v){
    console.log('board_oneshclicked',v);
      var canvas = document.getElementById(name);
      var ctx = canvas.getContext('2d');
      board_shape(v,ctx,canvas);                   
    }
    function board_onecolorclicked(name,v){
    console.log('board_onecolorclicked',v);
      var canvas = document.getElementById(name);
      var ctx = canvas.getContext('2d');
      board_setcolorid(parseInt(v),ctx,canvas);
    }
    function board_onestrokeclicked(name){
    console.log('board_onestrokeclicked');
      var canvas = document.getElementById(name);
      var ctx = canvas.getContext('2d');
    ...
    function board_setup_canvas(canvas){
      ...
      if(1){
        ...
        ctx.note.myfig = {x:0,y:0,w:0,h:0,rotate:0,skewX:0,skewY:0,mystroke:0,mycolor:''};
        ctx.note.mybasis = {x:0,y:0,w:0,h:0,dx:0,dy:0,dw:0,dh:0,s:null,a:null};
        ctx.note.myundo = [];
        ctx.note.myredo = [];
        ctx.note.all_op = 'pencil';
        ctx.note.all_sh = 'line';
        ctx.note.mystrokeid = 0;
        ctx.note.mycolorid = 0;
        ctx.note.all_fs = 12;
        ...
        var mysh = ctx.note.all_sh;
	var mode = ctx.mode;
        ctx.note.myundo.push({s,mybasis,myop,mysh,myfig,mode});
        ///MISC
        ctx.note.zoom = 1;
        ctx.note.grid = 30;
        ...
        ctx.note.rotate_ang = 0;//in degrees clockwise-rotation is positive
        ctx.note.move_x = 0.0;//pixels
        ctx.note.move_y = 0.0;//pixels
        ...
        ctx.note.corner_size = 4;
        ctx.note.hit_ball = null;
        ctx.note.hit_transform = [1,0,0,1,0,0];
        ///
        board_setstrokeid(0,ctx,canvas);
        board_setcolorid(0,ctx,canvas);
        board_pencil(ctx,canvas);
        board_shape('line',ctx,canvas);
      }
    }
    function board_draw_canvas(canvas){
    }
    function board_curr_color(ctx,canvas){
      var id = ctx.note.mycolorid;
      id = Math.abs(id);
      id = id % all_colors.length;
      return all_colors[id]; 
    }
    function board_curr_stroke(ctx,canvas){
      var id = ctx.note.mystrokeid;
      id = Math.abs(id);
    ...
    function board_redraw(ctx,canvas){
      if(ctx.mode=='note'||ctx.mode=='ball'||ctx.mode=='cplane'){
    ...
    function board_undo(ctx,canvas){
      if(ctx.mode=='note'){
        note_undo(ctx,canvas);
      }else if(ctx.mode=='cplane'){
        cplane_undo(ctx,canvas);
      }else if(ctx.mode=='xyplane'){
        xyplane_undo(ctx,canvas);
      }else if(ctx.mode=='ball'){
        ball_undo(ctx,canvas);
      }
    }
    function board_revundo(ctx,canvas){
      if(ctx.mode=='note'){
        note_revundo(ctx,canvas);
      }else if(ctx.mode=='cplane'){
        cplane_revundo(ctx,canvas);
      }else if(ctx.mode=='xyplane'){
        xyplane_revundo(ctx,canvas);
      }else if(ctx.mode=='ball'){
        ball_revundo(ctx,canvas);
      }
    }
    function board_reset(ctx,canvas){
      note_reset(ctx,canvas);
    }
    function board_toggle_action(ctx,canvas){
      if(ctx.mode=='note'){
        note_toggle_action(ctx,canvas);
      }else if(ctx.mode=='cplane'){
        cplane_toggle_action(ctx,canvas);
      }else if(ctx.mode=='xyplane'){
        xyplane_toggle_action(ctx,canvas);
      }else if(ctx.mode=='ball'){
        ball_toggle_action(ctx,canvas);
      }
    }
    function board_setcolorid(v,ctx,canvas){
      if(ctx.mode=='note'){
        note_setcolorid(v,ctx,canvas);
      }else if(ctx.mode=='cplane'){
        cplane_setcolorid(v,ctx,canvas);
      }else if(ctx.mode=='xyplane'){
        xyplane_setcolorid(v,ctx,canvas);
      }else if(ctx.mode=='ball'){
        ball_setcolorid(v,ctx,canvas);
      }
      if(1){
        let mycolor = board_curr_color(ctx,canvas);
        let button_elem = document.getElementById(canvas.getAttribute('data-buttonclid'));
        //button_elem.style.backgroundColor = mycolor;
        //button_elem.style.color = 'White';
        button_elem.style.color = mycolor;
      }
    }
    function board_setstrokeid(v,ctx,canvas){
      ctx.note.mystrokeid = v;
      if(ctx.mode=='note'){
        note_setstrokeid(v,ctx,canvas);
      }else if(ctx.mode=='cplane'){
        cplane_setstrokeid(v,ctx,canvas);
      }else if(ctx.mode=='xyplane'){
        xyplane_setstrokeid(v,ctx,canvas);
      }else if(ctx.mode=='ball'){
        ball_setstrokeid(v,ctx,canvas);
      }
      if(1){
        let mystroke = board_curr_stroke(ctx,canvas);
        let button_elem = document.getElementById(canvas.getAttribute('data-buttonclid'));
        button_elem.innerHTML = "&#x2B24;";
        if(v==0){
	  button_elem.style.fontSize = "6px";
        }else{
	  button_elem.style.fontSize = "10px";
        }
      }
    }
    function board_toggle_strokeid(ctx,canvas){
      let v = ctx.note.mystrokeid;
      v = (v+1) % 2;
      board_setstrokeid(v,ctx,canvas);    
    }
    function board_select(ctx,canvas){
      ctx.note.all_op='select';
      if(1){
        let button_elem = document.getElementById(canvas.getAttribute('data-buttonopid'));
        let icon = all_op_icons['select'];
        button_elem.innerHTML = icon;
      }
    }
    function board_smudge(ctx,canvas){
      ctx.note.all_op='smudge';
      if(1){
        let button_elem = document.getElementById(canvas.getAttribute('data-buttonopid'));
        let icon = all_op_icons['smudge'];
        button_elem.innerHTML = icon;
      }
    }
    function board_eraser(ctx,canvas){
      ctx.note.all_op='eraser';
      if(1){
        let button_elem = document.getElementById(canvas.getAttribute('data-buttonopid'));
        let icon = all_op_icons['eraser'];
        button_elem.innerHTML = icon;
      }
    }
    function board_pencil(ctx,canvas){
      ctx.note.all_op='pencil';
      if(1){
        let button_elem = document.getElementById(canvas.getAttribute('data-buttonopid'));
        let icon = all_op_icons['pencil'];
        button_elem.innerHTML = icon;
      }
    }
    function board_shape(v,ctx,canvas){
      ctx.note.all_sh=v;
      if(ctx.mode=='note'){
        note_shape(v,ctx,canvas);
      }else if(ctx.mode=='cplane'){
        cplane_shape(v,ctx,canvas);
      }else if(ctx.mode=='xyplane'){
        xyplane_shape(v,ctx,canvas);
      }else if(ctx.mode=='ball'){
        ball_shape(v,ctx,canvas);
      }
      if(1){
        let button_elem = document.getElementById(canvas.getAttribute('data-buttonshid'));
        let icon = all_sh_icons[v];
        button_elem.innerHTML = icon;
      }
    }
    ...
    function board_move(vx,vy,ctx,canvas){
      if(ctx.mode=='note'){
        note_move(vx,vy,ctx,canvas);
      }else if(ctx.mode=='cplane'){
        cplane_move(vx,vy,ctx,canvas);
      }else if(ctx.mode=='xyplane'){
        xyplane_move(vx,vy,ctx,canvas);
      }else if(ctx.mode=='ball'){
        ball_move(vx,vy,ctx,canvas);
      }
    }
    function board_rotate(v,ctx,canvas){
      if(ctx.mode=='note'){
        note_rotate(v,ctx,canvas);
      }else if(ctx.mode=='cplane'){
        cplane_rotate(v,ctx,canvas);
      }else if(ctx.mode=='xyplane'){
        xyplane_rotate(v,ctx,canvas);
      }else if(ctx.mode=='ball'){
        ball_rotate(v,ctx,canvas);
      }
    }
    function board_remove(ctx,canvas){
      if(ctx.mode=='note'){
        note_remove(ctx,canvas);
      }else if(ctx.mode=='cplane'){
        cplane_remove(ctx,canvas);
      }else if(ctx.mode=='xyplane'){
        xyplane_remove(ctx,canvas);
      }else if(ctx.mode=='ball'){
        ball_remove(ctx,canvas);
      }
    }
    function board_add(ctx,canvas){
      if(ctx.mode=='note'){
        note_add(ctx,canvas);
      }else if(ctx.mode=='cplane'){
        cplane_add(ctx,canvas);
      }else if(ctx.mode=='xyplane'){
        xyplane_add(ctx,canvas);
      }else if(ctx.mode=='ball'){
        ball_add(ctx,canvas);
      }
    }
    function board_dup(ctx,canvas){
      if(ctx.mode=='note'){
        ...
        cplane_dup(ctx,canvas);
      }else if(ctx.mode=='xyplane'){
        xyplane_dup(ctx,canvas);
      }else if(ctx.mode=='ball'){
        ball_dup(ctx,canvas);
      }
    }
    ...
    function board_handle_keydown(shiftKey,keycode,ctx,canvas){
      ...
      ///FOR TRI   
      case 'Digit3':
        board_shape('tri',ctx,canvas);
        break;
      ///FOR RECT  
      case 'Digit4':
        board_shape('rect',ctx,canvas);
        break;
      case 'Digit5':
        board_shape('pentagon',ctx,canvas);
        break;
      case 'Digit6':
        board_shape('hexagon',ctx,canvas);
        break;
      case 'Digit7':
        board_shape('circle',ctx,canvas);
        break;
      case 'Digit8':
        board_select(ctx,canvas);
        break;
      ...
      case 'Slash':
        if(shiftKey){
          ...
          board_toggle_action(ctx,canvas);
        }
        break;
      case 'BracketLeft':
        if(shiftKey){
      ...
      case 'Comma':
        if(shiftKey){
          ...
          ///decrement color id
          let id = ctx.note.mycolorid;
          id--;
          id += all_colors.length;//ensure it does not go below zero
          id = id % all_colors.length;
          board_setcolorid(id,ctx,canvas);
        }
        break;
      case 'Period':
        if(shiftKey){
      ...
      if(evt.button==0){
        ...
        if(ctx.mouseisdown){
          if(ctx.mode=='note'){
            ...
            ball_drag(ctx,canvas);
          }
        }else{
          if(ctx.mode=='note'){
            note_hover(ctx,canvas);
          }else if(ctx.mode=='cplane'){
            cplane_hover(ctx,canvas);
          }else if(ctx.mode=='xyplane'){
            xyplane_hover(ctx,canvas);
          }else if(ctx.mode=='ball'){
            ball_hover(ctx,canvas);
          }
        }
      }
    }
...
console.log('board_draw_pencil',first,lineWidth,strokeStyle);
      let posx1 = ctx.posx1;
      let posy1 = ctx.posy1;
      let posx2 = ctx.posx2;
      let posy2 = ctx.posy2;
    ...
    function board_draw_smudge(ctx,canvas){
      ...
      let mystroke = board_curr_stroke(ctx,canvas);
      let mycolor = board_curr_color(ctx,canvas);        
      let mysmear = all_smears[mycolor];
      var lw = 3*mystroke;
      lw = Math.max(lw,1);
    ...
    function board_clear_basis(ctx){
...
console.log('note_start','mode=',ctx.mode,'all_op=',ctx.note.all_op,'all_sh=',ctx.note.all_sh);
      if(ctx.note.all_op=='pencil' && ctx.note.all_sp){ //note_start
        board_clear_basis(ctx);
        let mystroke = board_curr_stroke(ctx,canvas);
        let mycolor = board_curr_color(ctx,canvas);        
        let x = ctx.posx0;
        let y = ctx.posy0;
      ...
      if(ctx.note.all_op=='select'){ //note_start
        if( (ctx.posx0 > ctx.note.mybasis.x) &&
            ...
            (ctx.posy0 < ctx.note.mybasis.y+ctx.note.mybasis.h+ctx.note.mybasis.dh) ){ ///note_start
          ///move/copy        
          ctx.note.mybasis.s = ctx.note.myface;
          if(ctx.shiftKey){
            ctx.note.mybasis.a = 'copy';
          }else{
            ctx.note.mybasis.a = 'move';
          }
        }else{
          ///hilite-only 
          board_clear_basis(ctx);
      ...
      if(ctx.note.all_op=='smudge'){ //note_start
        board_clear_basis(ctx);
        ctx.clearRect(0,0, canvas.getAttribute('width'),canvas.getAttribute('height'));
        ctx.putImageData(ctx.note.myface, 0,0);
        board_draw_smudge(ctx,canvas,true);
      }
      if(ctx.note.all_op=='eraser'){ //note_start
        board_clear_basis(ctx);
        ctx.clearRect(0,0, canvas.getAttribute('width'),canvas.getAttribute('height'));
        ctx.putImageData(ctx.note.myface, 0,0);
        board_draw_eraser(ctx,canvas,true);
      }
      if(ctx.note.all_op=='pencil'){ //note_start
        board_clear_basis(ctx);
        ctx.clearRect(0,0, canvas.getAttribute('width'),canvas.getAttribute('height'));
        ctx.putImageData(ctx.note.myface, 0,0);
    ...
    function note_hover(ctx,canvas){
      //EMPTY
    }
    function note_drag(ctx,canvas){
console.log('note_drag','mode=',ctx.mode,'all_op=',ctx.note.all_op,'all_sh=',ctx.note.all_sh);
      if(ctx.note.all_op=='pencil' && ctx.note.all_sh=='circle' && ctx.note.myfig){
        //DRAG  
        let posx0 = ctx.posx0;
        let posy0 = ctx.posy0;
        let posx2 = ctx.posx2;
        ...
        ctx.lineWidth   = ctx.note.myfig.mystroke;         
      ...
      if(ctx.note.all_op=='pencil' && ctx.note.all_sh=='hexagon' && ctx.note.myfig){
        //DRAG  
        let posx0 = ctx.posx0;
        let posy0 = ctx.posy0;
        let posx2 = ctx.posx2;
      ...
      if(ctx.note.all_op=='pencil' && ctx.note.all_sh=='pentagon' && ctx.note.myfig){
        //DRAG  
        let posx0 = ctx.posx0;
        let posy0 = ctx.posy0;
        let posx2 = ctx.posx2;
      ...
      if(ctx.note.all_op=='pencil' && ctx.note.all_sh=='rect' && ctx.note.myfig){
        //DRAG  
        let posx0 = ctx.posx0;
        let posy0 = ctx.posy0;
        let posx2 = ctx.posx2;
      ...
      if(ctx.note.all_op=='pencil' && ctx.note.all_sh=='tri' && ctx.note.myfig){
        //DRAG  
        let posx0 = ctx.posx0;
        let posy0 = ctx.posy0;
        let posx2 = ctx.posx2;
      ...
      if(ctx.note.all_op=='pencil' && ctx.note.all_sh=='line' && ctx.note.myfig){
        //DRAG  
        let x1 = ctx.posx0;
        let y1 = ctx.posy0;
        let x2 = ctx.posx2;
      ...
      if(ctx.note.all_op=='select'){
        ...
        if(ctx.note.mybasis.a){
          ...
          if(ctx.note.mybasis.h==0){
            ctx.note.mybasis.h = 1;//ensure that the hilite box is always to be drawn
          }
          if(ctx.note.mybasis.w==0){
            ctx.note.mybasis.w = 1;//ensure that the hilite box is always to be drawn
          }
          ctx.putImageData(ctx.note.myface, 0,0);
          note_draw_hilite(ctx,canvas);
        }
...
console.log('note_end','mode=',ctx.mode,'all_op=',ctx.note.all_op,'all_sh=',ctx.note.all_sh);
      if(ctx.note.all_op=='pencil'){
        //'pencil with shape'
        let s        = ctx.note.myface;
	let mode     = ctx.mode;
	...
	let mode     = ctx.mode;
        if(ctx.note.mybasis.a && ctx.note.mybasis.w && ctx.note.mybasis.h){
          ///normalize ctx.note.mybasis
          ctx.note.mybasis.x += ctx.note.mybasis.dx;
          ctx.note.mybasis.y += ctx.note.mybasis.dy;
          ctx.note.mybasis.dx = 0;
    ...
    function note_rotate(v,ctx,canvas){
      var n = ctx.note.myundo.length;
      var o = ctx.note.myundo[n-1];
      o.myfig.rotate += v;
    }
    ...
    function note_setcolorid(v,ctx,canvas){
      ctx.note.mycolorid = v;
      //let n = ctx.note.myundo.length;
      //let o = ctx.note.myundo[n-1];
      //o.myfig.mycolor = board_curr_color(ctx,canvas);
    }
    function note_setstrokeid(v,ctx,canvas){
      //let n = ctx.note.myundo.length;
      //let o = ctx.note.myundo[n-1];
      //o.myfig.mystroke = board_curr_stroke(ctx,canvas);
    }
    function note_shape(v,ctx,canvas){
    }
    function note_reset(ctx,canvas){
      //remove all 'myundo' except for the first one
      while(ctx.note.myundo.length > 1){
        ctx.note.myundo.pop();
      }
      ctx.note.myredo = [];
      note_redraw(ctx,canvas);
    ...
    function note_revundo(ctx,canvas){
      ///reverse Previous Undo
      if(ctx.note.myredo.length>0){
        let o = ctx.note.myredo.pop();
        if( o.myop=='pencil' && o.mysh ){
          o.s = ctx.note.myface;
        }
...
console.log('note_redraw');
      ...
      if(o.mode=='note' && o.myop=='pencil' && o.mysh=='circle'){
        //REDRAW
        let myfig = o.myfig;
        ctx.lineWidth = myfig.mystroke;
      ...
      if(o.mode=='note' && o.myop=='pencil' && o.mysh=='hexagon'){
        //REDRAW
        let myfig = o.myfig;
        ctx.lineWidth = myfig.mystroke;
        ctx.strokeStyle = myfig.mycolor;
      ...
      if(o.mode=='note' && o.myop=='pencil' && o.mysh=='pentagon'){
        //REDRAW
        let myfig = o.myfig;
        ctx.lineWidth = myfig.mystroke;
        ctx.strokeStyle = myfig.mycolor;
      ...
      if(o.mode=='note' && o.myop=='pencil' && o.mysh=='rect'){
        //REDRAW
        let myfig = o.myfig;
        ctx.lineWidth = myfig.mystroke;
        ctx.strokeStyle = myfig.mycolor;
      ...
      if(o.mode=='note' && o.myop=='pencil' && o.mysh=='tri'){
        //REDRAW
        let myfig = o.myfig;
        ctx.lineWidth = myfig.mystroke;
        ctx.strokeStyle = myfig.mycolor;
      ...
      if(o.mode=='note' && o.myop=='pencil' && o.mysh=='line'){
        //REDRAW
        let myfig = o.myfig;
        ctx.lineWidth = myfig.mystroke;
        ctx.strokeStyle = myfig.mycolor;
      ...
      if(o.mode=='ball'){
        //REDRAW
        var corner_size = ctx.note.corner_size;
        ctx.resetTransform();
        ctx.globalAlpha = 1.0;
        ctx.strokeStyle = 'black';
        ctx.lineWidth = corner_size;
        ctx.note.balls.forEach((ball,j) => {
          ctx.save();
          if(ball == ctx.note.hit_ball){
            ctx.translate(ctx.note.move_x,ctx.note.move_y); ///IMPORTANT: the translate operation has to happen before rotation 
            var [a,b,c,d,e,f] = ball.transform;
            ctx.transform(a,b,c,d,e,f);
            var [cornerx,cornery] = ball_getcornerxy(ball);
            ctx.translate(cornerx,cornery);
            ctx.rotate(ctx.note.rotate_ang/180*Math.PI);
            ctx.translate(-cornerx,-cornery);
            var {a,b,c,d,e,f} = ctx.getTransform();
            ball.transform = [a,b,c,d,e,f];
            ball.pivot_x += ctx.note.move_x;//updated during move
            ball.pivot_y += ctx.note.move_y;
      ...
      if(o.mode=='cplane'){
        //REDRAW
        if(ctx.note.rotate_ang!=0 || ctx.note.scale_factor!=1|| ctx.note.move_x!=0|| ctx.note.move_y!=0){
          var double_flag = 1;
        }else{
          var double_flag = 0;
        }
  ...
  //console.log('val_real=',val_real,'val_imag=',val_imag,'which=',which);
        ...
        let arrow_x = (val_real)*ctx.note.grid ;
        let arrow_y = -(val_imag)*ctx.note.grid ;
        arrow_x = (val_real)*ctx.note.grid ;
        arrow_y = -(val_imag)*ctx.note.grid ;
        ...
        ctx.beginPath();
        var r = 1.6*2;
        if((ctx.note.action%2)==0){
          ctx.arc(0,0,r,0,Math.PI*2);
        }else if((ctx.note.action%2)==1){
        ...
        if(double_flag){
          ctx.globalAlpha = 0.5;
          if(ctx.note.scale_factor < 1){
            let delta = 1 - ctx.note.scale_factor;//always greater than 0
          ...
          ctx.translate(ctx.note.move_x,ctx.note.move_y);
          ctx.beginPath();
          ctx.strokeStyle = 'gray';
          ctx.fillStyle = 'gray';
          ctx.lineWidth = 0.5;
        ...
        //console.log(ctx.note.origin_x,ctx.note.origin_y, ctx.note.move_x, ctx.note.move_y, ctx.note.scale_factor, ctx.note.rotate_ang);
        ctx.note.myface = ctx.getImageData(0,0, canvas.getAttribute('width'),canvas.getAttribute('height'));
        ctx.resetTransform();
      }
    }
    ...
    function cplane_hover(ctx,canvas){
      ///EMPTY
    }
...
//console.log('diff_y=',diff_y,'diff_x=',diff_x,'dy=',dy,'dx=',dx);
        ...
        if((ctx.note.action%2)==1){
          ctx.note.rotate_ang = Math.atan2(diff_y, diff_x)/Math.PI*180;
          ctx.note.scale_factor = Math.fround(Math.sqrt(diff_y*diff_y + diff_x*diff_x)/ctx.note.grid);
        }else if((ctx.note.action%2)==0){
          ctx.note.move_x = diff_x ;
          ctx.note.move_y = diff_y ;
        }
      }else{
    ...
    function cplane_rotate(v,ctx,canvas){
    }
    ...
    function cplane_setcolorid(v,ctx,canvas){
    }
    ...
    function cplane_toggle_action(ctx,canvas){
      ctx.note.action++;
      ctx.note.action = ctx.note.action % 2;
      ctx.note.scale_factor = 1;
      ctx.note.rotate_ang = 0;
      ctx.note.move_x = 0;
      ctx.note.move_y = 0;
    ...
    function xyplane_rotate(v,ctx,canvas){
    }
    ...
    function xyplane_setcolorid(v,ctx,canvas){
    }
...
//console.log('x-intercept','y0=',y0,'yi=',yi,'x0=',x0,'xii=',xii,'sign0=',sign0,'sign=',sign);
    ...
    ///////////////////////////////////////////////////////
    ///////////////////////////////////////////////////////
    function ball_start(ctx,canvas){
      var corner_size = ctx.note.corner_size;
      var hit_size = ctx.note.corner_size+4;
      if(ctx.altKey){
      }else if(ctx.shiftKey){
      }else{
        var success = 0;
        if(ctx.note.hit_ball){
          ///DETERMINE hit_ball corner
          let [a,b,c,d,e,f] = ctx.note.hit_ball.transform;
          ctx.setTransform(a,b,c,d,e,f);
          ctx.note.hit_ball.points.forEach((pt,i) => {
            let {op,x,y,cp1x,cp1y,cp2x,cp2y,x1,y1,x2,y2,r} = pt;
            ...
            if(ctx.isPointInPath(path2d, ctx.posx0,ctx.posy0)){
              ctx.note.hit_ball.corner_i = i;
              ctx.note.hit_ball.pivot_x = ctx.posx0;
              ctx.note.hit_ball.pivot_y = ctx.posy0;
              success = 1;
            }
          });
        }
      ...
      console.log('hit_ball=',ctx.note.hit_ball);
      board_redraw(ctx,canvas);
    }
    function ball_hover(ctx,canvas){
    }
    function ball_drag(ctx,canvas){
      if(ctx.shiftKey && ctx.note.hit_ball){
        let [x,y] = ball_getpivotxy(ctx.note.hit_ball);
        let angle0 = Math.atan2(ctx.posy1-y, ctx.posx1-x);
        let angle1 = Math.atan2(ctx.posy2-y, ctx.posx2-x);
        ctx.note.rotate_ang = (angle1-angle0)/Math.PI*180;
      }else if(ctx.note.hit_ball){
        ctx.note.move_x = ctx.posx2 - ctx.posx1;
        ctx.note.move_y = ctx.posy2 - ctx.posy1;
      }
      board_redraw(ctx,canvas);
    }
    function ball_end(ctx,canvas){
    }
    ...
    function ball_scale(vx,vy,ctx,canvas){
      if(ctx.note.hit_ball){
        ctx.note.hit_ball.points.forEach((pt) => {
          //let {op,x,y,cp1x,cp1y,cp2x,cp2y,x1,y1,x2,y2,rx,ry,sAngle,eAngle,anticlockflag} = pt;
          pt.x *= vx;
          pt.y *= vy;
          pt.cp1x *= vx;
    ...
    function ball_rotate(v,ctx,canvas){
      if(ctx.note.hit_ball){
        ctx.note.rotate_ang = v;
      }
    }
    function ball_remove(ctx,canvas){
      if(ctx.note.hit_ball){
        let i = ctx.note.balls.indexOf(ctx.note.hit_ball);
        if(i>=0){
          ctx.note.balls.splice(i,1);
        }
        ctx.note.hit_ball = null;
      }
    }
    function ball_add(ctx,canvas){
      //////////////////////////////////////////////////////////////////
      ...
      // ctx.note.balls.push(ball);
      ///////////////////////////////////////////////////
      var v = ctx.note.all_sh;
      var x = ctx.posx0; 
      var y = ctx.posy0; 
      var points = [];
      ...
      if(ctx.note.hit_ball){
        ctx.note.hit_ball.points = points;//{points,transform,corner_i,pivot_x,pivot_y,color};
        ctx.note.hit_ball.corner_i = 0;
      }else{
        var ball = {points,transform,corner_i,pivot_x,pivot_y,color};
        ctx.note.balls.push(ball);
        ctx.note.hit_ball = ball;//making it the latest hit ball
      }
    }
    function ball_dup(ctx,canvas){
      if(ctx.note.hit_ball){
        var ball = ctx.note.hit_ball;
        ...
        let ball_dup = {points,transform,corner_i,pivot_x,pivot_y,color};
        ctx.note.balls.push(ball_dup);
      }
    }
    function ball_mode_changed(ctx,canvas){
      note_dup(ctx,canvas);
    }
    function ball_setcolorid(v,ctx,canvas){
      ctx.note.mycolorid = v;
      if(ctx.note.hit_ball){
        ctx.note.hit_ball.color = board_curr_color(ctx,canvas);
      }
    }
