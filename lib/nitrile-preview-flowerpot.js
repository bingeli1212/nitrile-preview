...
`;
  ...
  to_setup_script(){
    ...
    function board_enter(ctx,canvas){
      ...
      || o.myop=='shape'    //board_enter shape
    ...
    function board_redraw(ctx,canvas){
      ...
      if(ctx.cake.myop=='cplane'){ //board_redraw cplane
        ...
        if(1){
          ...
          ctx.lineWidth = board_curr_stroke(ctx,canvas)*1.4*(1/reduce_rate);
          let arrow_x = (val_real)*o.grid ;
          let arrow_y = -(val_imag)*o.grid ;
          arrow_x = (val_real)*o.grid ;
          arrow_y = -(val_imag)*o.grid ;
          ...
          //
          ctx.beginPath();
          var r = board_curr_stroke(ctx,canvas)*2*(1/reduce_rate);
    ...
    function board_addpolypt(x0,y0, pt, ctx,canvas){
      var o = ctx.cake;
      ...
      x += x0;
      y += y0;
      for(let i=0; i < o.polypts.length; ++i){
        if(o.polypts[i].x == x && o.polypts[i].y == y){
          dup_flag = 1;
          break;
        }
      } 
    ...
    function board_addpolypt_square(myop,mysquare, ctx,canvas){
      let x0 = mysquare.x;
      let y0 = mysquare.y;
      if(myop=='square'){
        board_addpolypt(x0,y0, mysquare.points[0], ctx,canvas);
        board_addpolypt(x0,y0, mysquare.points[1], ctx,canvas);
        board_addpolypt(x0,y0, mysquare.points[2], ctx,canvas);
        board_addpolypt(x0,y0, mysquare.points[3], ctx,canvas);
      }else if(myop=='triangle'){
        board_addpolypt(x0,y0, mysquare.points[0], ctx,canvas);
        board_addpolypt(x0,y0, mysquare.points[2], ctx,canvas);
        board_addpolypt(x0,y0, mysquare.points[3], ctx,canvas);
      }
    }
    function board_findpolypt(pt, ctx,canvas){
      var o = ctx.cake;          
      var [sx,sy] = pt;
      var dist = Infinity;
      var tx = 0;
    ...
    function board_start(ctx,canvas){
      ...
      if(ctx.cake.myop=='poly'){ //board_start
        ...
        let y2 = ctx.posy0;
        if(ctx.altKey){
          [x1,y1] = board_findpolypt([x1,y1], ctx,canvas);
          [x2,y2] = board_findpolypt([x2,y2], ctx,canvas);
        }
        o.mypoly.x1 = x1;
        o.mypoly.y1 = y1;
        o.mypoly.x2 = x2;
      ...
      || ctx.cake.myop=='ellipse' ){  //board_start ellipse
        ...
        if(o.mysquare.altKey && o.mysquare.shiftKey){
          ...
          //add to polypts
          board_addpolypt_square(o.myop,o.mysquare, ctx,canvas);
        ...
        if(ctx.altKey){
          ...
          if(ctx.shiftKey){
            ...
            if(Math.abs(dx) > Math.abs(dy)){
              posy1 = ctx.posy0; //draw a horizontal line
              posy2 = ctx.posy0;
            }else{
              posx1 = ctx.posx0; //draw a vertical line
              posx2 = ctx.posx0;
            }
            ctx.lineWidth = board_curr_stroke(ctx,canvas);
            ctx.strokeStyle = board_curr_color(ctx,canvas);           
    ...
    function board_end(ctx,canvas){
      ...
      if(ctx.cake.myop=='poly'){ //board_end
        if(ctx.mouseisdragged){
          ...
          let x2 = o.mypoly.x2;
          let y2 = o.mypoly.y2;
          board_addpolypt(0,0, [x1,y1], ctx,canvas);
          board_addpolypt(0,0, [x2,y2], ctx,canvas);
        }
      }
