  ...
  to_sidesvg_for_id(id,toc){
    ...
    toc.forEach((p,j) => {
      if(p.id==id){
        ...
        all.push(`<text id="svg${p.id}" x="5" y="${60+j*12}" stroke="none" fill="#DDEEFF" font-size="7pt" >${p.order} ${p.icon} ${this.smooth(p.title)}</text>`)
      }else{
        all.push(`<text id="svg${p.id}" x="5" y="${60+j*12}" stroke="none" fill="#124A47" font-size="7pt" >${p.order} ${p.icon} ${this.smooth(p.title)}</text>`)
      }
    })
    all.push(`</svg>`);
    return all.join('\n') 
