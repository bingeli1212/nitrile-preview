...
`;
  ...
  write_one_frame(id,order,icon,frame,issub,parentid,toc,pages,topframe){
    ...
    if(issub){
      all.push(`<div class='TOPCONTAINER'> 
                ...
                <h2 class='SLIDESUBTITLE'> <b> ${this.uncode(frame.style,frame.title)}</b> </h2>
                </div>`);
      order = '';
    }else{
      order = order + '.';
  ...
  to_sidesvg_for_id(id,toc,max_n=30){
    ...
    toc.forEach((p,j) => {
      ...
      all.push(`<text id="svg${p.id}" x="${x}" y="${y-dy+8}" stroke="none" fill="${fontcolor}" font-size="5pt" >${p.order} ${this.smooth(p.title)} ${p.posticon} </text>`)
    })
    var text = all.join('\n');
  ...
  to_tool_contents(boardid){
    return `\
         ...
         <div id='tool-${boardid}' class='TOOLMENU' style='display:flex;flex-direction:row;align-items:flex-start;justify-content:flex-start;flex-wrap:wrap;width:340px;height:34px;' >
