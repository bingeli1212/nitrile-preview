...
`;
  ...
  to_setup_script(){
    ...
    function board_draw_hilite(ctx,canvas){
      var o = ctx.cake;
      const {x,y,w,h} = o.mybasis;
      if(w && h){
        const gap = Math.min(10,0.25*w);
        const dep = Math.min(10,0.25*h);
        ctx.save();
        ctx.beginPath();
        ctx.lineWidth = 1;   
        ctx.strokeStyle = 'black';
    ...
    function board_start(ctx,canvas){
      ...
      if(ctx.cake.myop=='select'){ //board_start select
        ...
        if(o.mybasis.altKey && o.mybasis.shiftKey){//this keycombination makes the current clipboard permanent in current location
          if(o.mybasis.face){
            //copy from face
          ...
          o.mybasis.w = 0;
          o.mybasis.h = 0;
        }else if(o.mybasis.shiftKey){
          if(o.mybasis.face){
            //copy from face, but keep old clipboard content
            o.mybasis.s = o.mybasis.face;
            o.mybasis.face = null;
          }
        }else{
