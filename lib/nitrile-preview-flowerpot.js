...
`;
  ...
  write_frame_folder(id,frame,subframes,toc,pages){
    ...
    if(boardpng){
      all.push(`<img class='EDITIMG' id='${imgid}' data-type='board' data-imgid='${imgid}' data-boardname='${boardname}' width='441' height='320' src='${boardpng}'></img>`);
    }
  ...
  to_setup_script(){
    ...
    function cplane_setup_canvas(canvas){
      ...
      ctx.d.origin_x = 300;
      ctx.d.origin_y = 300;
      ctx.d.handle_x = ctx.d.step_x;
      ctx.d.handle_y = 0;
      ctx.d.translate_x = 0.0;
      ctx.d.translate_y = 0.0;
      ctx.d.scale_x = 1.0;
      ctx.d.scale_y = 1.0;
      ctx.d.rotate_ang = 0;
      ctx.d.is_mouse_down = 0;
      ctx.d.action_flag = 'multiply';
      ctx.d.val_real = 0;
      ctx.d.val_imag = 0;
    ...
    function cplane_doZoomoutClicked(){
      ...
      ctx.d.step_x -= 3;
      ctx.d.step_y -= 3;
      ctx.d.step_x = Math.max(ctx.d.step_x,10);
      ctx.d.step_y = Math.max(ctx.d.step_y,10);
      ctx.d.step_x = Math.min(ctx.d.step_x,100);
      ctx.d.step_y = Math.min(ctx.d.step_y,100);
      ///recompute handle_x/handle_y
      ctx.d.handle_x = (ctx.d.step_x * ctx.d.scale_x) * Math.cos(ctx.d.rotate_ang);
      ctx.d.handle_y = (ctx.d.step_y * ctx.d.scale_y) * Math.sin(ctx.d.rotate_ang);
      cplane_redraw(canvas);
    }
    function cplane_doZoominClicked(){
      ...
      ctx.d.step_x = Math.min(ctx.d.step_x,100);
      ctx.d.step_y = Math.min(ctx.d.step_y,100);
      ///recompute ctx.d.handle_x/ctx.d.handle_y
      ctx.d.handle_x = (ctx.d.step_x * ctx.d.scale_x) * Math.cos(ctx.d.rotate_ang);
      ctx.d.handle_y = (ctx.d.step_y * ctx.d.scale_y) * Math.sin(ctx.d.rotate_ang);
      cplane_redraw(canvas);
    }
    function cplane_onMouseDown(){
      var canvas = event.target;
      var ctx = canvas.getContext('2d');
      var r = 4*Math.sqrt(ctx.d.scale_x);
      if(getDist(ctx.d.handle_x,(event.offsetX-ctx.d.origin_x),
                 ctx.d.handle_y,(event.offsetY-ctx.d.origin_y)) <= r){
        ctx.d.is_mouse_down = 1;
      }
    }
    function cplane_onMouseMove(){
      ...
      var ctx = canvas.getContext('2d');
      var r = 4*Math.sqrt(ctx.d.scale_x);
      if(ctx.d.is_mouse_down && ctx.d.action_flag=='multiply'){
        ctx.d.handle_x = (event.offsetX-ctx.d.origin_x);
        ctx.d.handle_y = (event.offsetY-ctx.d.origin_y);
        var diff_x = ctx.d.handle_x;
        var diff_y = ctx.d.handle_y;
        if(1){
          //snap to grid within 2px
          var v = diff_y / ctx.d.step_y;
          var v = v - Math.round(v);
          var v = Math.abs(v);
          if(v < r/ctx.d.step_x){
            diff_y = Math.round(diff_y / ctx.d.step_y) * ctx.d.step_y;
            ctx.d.handle_y = diff_y;
          }
        }
        if(1){
          //snap to grid within 2px
          var v = diff_x / ctx.d.step_x;
          var v = v - Math.round(v);
          var v = Math.abs(v);
          if(v < r/ctx.d.step_x){
            diff_x = Math.round(diff_x / ctx.d.step_x) * ctx.d.step_x;
            ctx.d.handle_x = diff_x;
          }
        }
        ctx.d.rotate_ang = Math.atan2(diff_y, diff_x);
        ctx.d.scale_x = Math.fround(Math.sqrt(diff_y*diff_y + diff_x*diff_x))/ctx.d.step_x;
        ctx.d.scale_y = ctx.d.scale_x;
        cplane_redraw(canvas);
      }
      if(ctx.d.is_mouse_down && ctx.d.action_flag=='add'){                      
        ctx.d.handle_x = (event.offsetX-ctx.d.origin_x);
        ctx.d.handle_y = (event.offsetY-ctx.d.origin_y);
        var diff_x = ctx.d.handle_x;
        var diff_y = ctx.d.handle_y;
        ...
        ctx.d.translate_x = Math.fround(diff_x);
        ctx.d.translate_y = Math.fround(diff_y);
        cplane_redraw(canvas);
      }
    }
    ...
    function cplane_redraw(canvas){
      ...
      ctx.translate(+ctx.d.origin_x,+ctx.d.origin_y);
      var w = canvas.getAttribute('width');
      var h = canvas.getAttribute('height');
      for (var x=0; x < +w/2; x += ctx.d.step_x) {
        ctx.beginPath();
      ...
      for (var y=0; y < +h/2; y += ctx.d.step_y) {
        ctx.beginPath();
        ctx.strokeStyle = 'rgba(0,0,0,0.2)';
        ctx.lineWidth = 0.5;
        ctx.moveTo(-w/2,y);
      ...
      if(1){
        /// draw handle
        ctx.save();
        ctx.translate(ctx.d.handle_x,ctx.d.handle_y);
        ctx.beginPath();
        var r = 4 * Math.sqrt(ctx.d.scale_x);
      ...
      if(1){
        ...
        for(var x=0; x < +w/2; x+=ctx.d.step_x){
          var myx = ""+Math.round(x/ctx.d.step_x);
          ctx.font = '12px';
      ...
      if(1){
        ...
        for(var y=0; y < +h/2; y+=ctx.d.step_y){
          if(y==0) continue;
          var label = ""+Math.round(-y/ctx.d.step_y);
          ctx.font = '12px';
      ...
      ///draw blue grids
      ///////////////////////////////////////////////
      ctx.rotate(ctx.d.rotate_ang);
      ctx.scale(ctx.d.scale_x,ctx.d.scale_y);
      //ctx.translate(-ctx.d.origin_x,-ctx.d.origin_y);
      ctx.translate(ctx.d.translate_x,ctx.d.translate_y);
      for (var x=0; x < +w/2; x += ctx.d.step_x) {
        ctx.beginPath();
      ...
      ctx.strokeStyle = 'red';
      ctx.fillStyle   = 'red';
      var val_real = parseFloat(ctx.d.val_real);
      var val_imag = parseFloat(ctx.d.val_imag);
      if(1){
        let arrow_x = Math.round(val_real)*ctx.d.step_x ;
        let arrow_y = -Math.round(val_imag)*ctx.d.step_y ;
        arrow_x = Math.round(val_real)*ctx.d.step_x ;
        arrow_y = -Math.round(val_imag)*ctx.d.step_y ;
