...
`;
  ...
  to_setup_script(){
    ...
    function smudge_fill(x,y,stacksize,n,sx,sy,mysmear,ctx){
      x = Math.floor(x);
      y = Math.floor(y);
      let limit = 0;
      if(stacksize<2000 && n < 5000){
        var count = smudgeRect(x,y,1,1,mysmear,ctx,limit);
        n += count;
        if(count){
          n = smudge_fill(x,y-1,stacksize+1,n,sx,sy,mysmear,ctx);
          n = smudge_fill(x,y+1,stacksize+1,n,sx,sy,mysmear,ctx);
          n = smudge_fill(x-1,y,stacksize+1,n,sx,sy,mysmear,ctx);
          n = smudge_fill(x+1,y,stacksize+1,n,sx,sy,mysmear,ctx);
          ///
          n = smudge_fill(x-1,y-1,stacksize+1,n,sx,sy,mysmear,ctx);
          n = smudge_fill(x-1,y+1,stacksize+1,n,sx,sy,mysmear,ctx);
          n = smudge_fill(x+1,y-1,stacksize+1,n,sx,sy,mysmear,ctx);
          n = smudge_fill(x+1,y+1,stacksize+1,n,sx,sy,mysmear,ctx);
        }
    ...
    function board_draw_smudge_fill(x1,y1,sx,sy,ctx,canvas){
      let mysmear = ctx.d.mysmear;
      let mystroke= ctx.d.mystroke;
      let n = smudge_fill(x1,y1,0,0,sx,sy,mysmear,ctx);
    }
