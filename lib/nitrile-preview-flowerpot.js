...
`;
  ...
  to_setup_script(){
    ...
    function note_start(ctx,canvas){
      if(ctx.note.action==1){ //DRAG_START
        if( (ctx.posx0 > ctx.note.mybasis.x) &&
            ...
            (ctx.posy0 < ctx.note.mybasis.y+ctx.note.mybasis.h+ctx.note.mybasis.dh) ){ ///DRAG_START
          ///move/copy        
      ...
      if(ctx.note.action==0 && ctx.note.all_op=='shape'){ //DRAG_START
        board_clear_basis(ctx);
        let mystroke = board_curr_stroke(ctx,canvas);
        let mycolor = board_curr_color(ctx,canvas);        
        let x = ctx.posx0;
      ...
      if(ctx.note.action==0 && ctx.note.all_op=='pencil'){ //DRAG_START
        board_clear_basis(ctx);
        ctx.clearRect(0,0, canvas.getAttribute('width'),canvas.getAttribute('height'));
        ctx.putImageData(ctx.note.myface, 0,0);
        if(ctx.altKey){
