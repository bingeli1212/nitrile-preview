...
class NitrilePreviewFlowerpot extends NitrilePreviewHtml {
  ...
  to_peek_document() {
    ...
    var main_html = this.to_main_html(toc);
  ...
  to_document() {
    ...
    var main_html = this.to_main_html(toc);
    var main_html = this.replace_all_refs(main_html);
    var side_svg = this.to_side_svg(toc);
    console.log(side_svg);
    ...
    @media print {
      ...
      .SLIDE {
        background-image: url("image-slide.png"), url("data:image/svg+xml,${encodeURIComponent(side_svg)}"), url("data:image/svg+xml,${encodeURIComponent(this.bgsvg)}");
        background-repeat: no-repeat, no-repeat, no-repeat;
        background-position: top left, top left, top left;
        background-size: 88px 40px, 88px 400px, cover;
      ...
      .SLIDE.SLIDE0 {
        background-image: url("data:image/svg+xml,${encodeURIComponent(this.bgsvg0)}");
...
`;
  ...
  to_main_html(toc) {
    ...
    top.sections.forEach((sect,i,arr) => {
      ...
      if(sect.sections.length){
        ...
        let out = this.write_frame_folder(`${i+1}`,topframe,subframes,toc);
        d.push(out);
        topframe.subframes.forEach((subframe,j) => {
          let id = `${i+1}.${j+1}`;
          let order = '';
          let icon = this.icon_folder;
          let out = this.write_one_frame(id,order,icon,subframe,1,toc);
          d.push(out);
        });
      }else{
        let id = `${i+1}`;
        let order = i+1;
        let icon = '';
        let out = this.write_one_frame(id,order,icon,topframe,0,toc);
        d.push(out);
      }
    });
    var text = d.join('\n');
    ...
    //
    var table_of_contents_frame = this.write_frame_toc(topframes);
    //
    // put together
    //
    var d = [];
  ...
  write_frame_folder(id,frame,subframes,toc){
    let slidetitle_css = this.css('SLIDETITLE');
    ...
    toc.push({order:id,icon:'',style:frame.style,title:frame.title})
  ...
  write_one_frame(id,order,icon,frame,issub,toc){
    let slidetitle_css = this.css('SLIDETITLE');
    let itemize_css = this.css('ITEMIZE');
    let paragraph_css = this.css('PARAGRAPH');
    let ul_css = this.css('PARA UL');
    ...
    toc.push({order,icon,style:frame.style,title:frame.title})
  ...
  to_titlepage(){
    ...
    let data = `<article row='0' class='SLIDE SLIDE0'>
  ...
  to_side_svg(toc){
    ...
    toc.forEach((p,j) => {
      all.push(`<text x="5" y="60" dy="${j*12}" stroke="none" fill="#124A47" font-size="8pt" >${p.order} ${p.icon} ${this.smooth(p.title)}</text>`)
    })
    all.push(`</svg>`);
    return all.join('\n') 
  }
