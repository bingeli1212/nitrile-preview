...
class NitrilePreviewFlowerpot extends NitrilePreviewSlide {
  ...
  to_document() {
    ...
    :root {
      ...
      --onedot: url("data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHZpZXdCb3g9IjAgMCAxMDAgMTAwIj4NCiAgPGNpcmNsZSBjeD0iNzUiIGN5PSIyNSIgcj0iMTAiLz4gDQo8L3N2Zz4NCg==");
    }
    :root {
${allsidesvgs.join('\n')}
    }
...
`;
  ...
  to_edit_contents(boardid,buttonopid,buttonclid){
    return `\
       <select name='${boardid}' style='margin-left:1px;margin-right:1px;' onchange='board_change_type(this.value,this.name)'>
       ...
       <div style='position:relative'>
         <button name='${boardid}' id='${buttonclid}' class="BUTTON" role="button" onclick='board_onstrokeclicked(this.name)' ></button>
         <div class='DROPMENU' style='display:flex;flex-direction:column;align-items:flex-start;justify-content:center;' >
          <div style='display:flex;flex-direction:row;align-items:center;justify-content:flex-start;' >
          </div>
         </div>
  ...
  to_setup_script(){
    ...
    function board_onestrokechanged(value,name){
      var boardid = name;
      var canvas = document.getElementById(boardid); 
      var ctx = canvas.getContext('2d');
      board_setstrokeid(value,ctx,canvas);
    }
    function board_onstrokeclicked(name){
      var boardid = name;
      var canvas = document.getElementById(boardid); 
      var ctx = canvas.getContext('2d');
      board_changestrokeid(ctx,canvas);
    }
    function board_onshowdropmenu(){
event.target.focus();
console.log('board_onshowdropmenu',name);
    ...
    function board_changeaction(ctx,canvas){
      if(ctx.mode=='note'){
        note_changeaction(ctx,canvas);
      }else if(ctx.mode=='cplane'){
        cplane_changeaction(ctx,canvas);
      }else if(ctx.mode=='xyplane'){
        xyplane_changeaction(ctx,canvas);
      }else if(ctx.mode=='ball'){
        ball_changeaction(ctx,canvas);
      }
    }
    ...
    function board_changestrokeid(ctx,canvas){
    ...
    function board_handle_keydown(shiftKey,keycode,ctx,canvas){
      ...
      case 'KeyZ':
        ...
        break;
      case 'KeyX':
        board_setcolorid(1,ctx,canvas);
        break;
      case 'KeyC':
        board_setcolorid(2,ctx,canvas);
        break;
      case 'KeyV':
        board_setcolorid(3,ctx,canvas);
        break;
      case 'KeyB':
        board_setcolorid(4,ctx,canvas);   
        break;
      case 'KeyN':
        board_setcolorid(5,ctx,canvas);   
        break;
      case 'KeyM':
        board_setcolorid(6,ctx,canvas);   
        break;
      ///STROKES
      case 'Backquote':
        board_changestrokeid(ctx,canvas);
...
console.log('all_op=',ctx.note.all_op);
    ...
    function note_changeaction(ctx,canvas){
    }
...
//console.log('diff_y=',diff_y,'diff_x=',diff_x,'dy=',dy,'dx=',dx);
    ...
    function cplane_changeaction(ctx,canvas){
      ctx.note.action++;
...
//console.log('val_real=',val_real,'val_imag=',val_imag,'which=',which);
    ...
    function xyplane_changeaction(ctx,canvas){
    }
...
//console.log('x-intercept','y0=',y0,'yi=',yi,'x0=',x0,'xii=',xii,'sign0=',sign0,'sign=',sign);
    ...
    function ball_changeaction(ctx,canvas){
    }
    ...
    function ball_redraw(ctx,canvas){
      ...
      ctx.note.balls.forEach((ball,j) => {
        ...
        if(ball == ctx.note.hit_ball){
          ctx.translate(ctx.note.move_x,ctx.note.move_y); ///IMPORTANT: the translate operation has to happen before rotate operation because of the nature of the transform matrix of canvas
