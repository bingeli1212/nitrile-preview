    ...
    return `\
       <select name='${boardid}' onchange='board_change_type(this.value,this.name)'>
        <optgroup label='Board'>
           ...
           <option value='xyplane'>XY Plane</option>
        </optgroup>
       </select>
       `;
  }
  ...
  to_setup_script(){
    ...
    function board_setup_canvas(canvas){
      ...
      if(1){
        ...
        ctx.xyplane.zoom = 1;
        ctx.xyplane.grid = 30;
        ctx.xyplane.origin_x = Math.round(canvas.width/2);
        ctx.xyplane.origin_y = Math.round(canvas.height/2);
        ctx.xyplane.expr = 'x';
      }
    }
    ...
    function board_handle_keydown(shiftKey,keycode,ctx,canvas){
      ...
      case 'Equal':
        if(ctx && ctx.mode=='note'){
          ...
          ctx.cplane.zoom *= 1.05;
          cplane_redraw(ctx,canvas);
        }else if(ctx && ctx.mode=='xyplane'){
          ctx.xyplane.zoom *= 1.05;
          xyplane_redraw(ctx,canvas);
        }
        break;
    ...
    function board_onmouseenter(evt){
      var canvas = evt.target;
      all_canvas = canvas;
      console.log('board_onmouseenter',all_canvas);
      var ctx = canvas.getContext("2d");
      if(ctx.mouseisdown){
        if(ctx.mode=='note'){
          note_end(ctx,canvas);
    ...
    function board_onmouseleave(evt){
      var canvas = evt.target;
      all_canvas = canvas;
      console.log('board_onmouseleave',all_canvas);
      var ctx = canvas.getContext("2d");
      if(ctx.mouseisdown){
        if(ctx.mode=='note'){
          note_end(ctx,canvas);
    ...
    function cplane_start(ctx,canvas){
      ...
      ctx.cplane.pointisinhandle = flag;
    }
    function cplane_drag(ctx,canvas){
      if(ctx.cplane.pointisinhandle){
        var diff_x = ctx.posx2 - ctx.cplane.origin_x;
        var diff_y = ctx.posy2 - ctx.cplane.origin_y;
        diff_x /= ctx.cplane.zoom;
        diff_y /= ctx.cplane.zoom;
        if(1){
        ...
        ctx.cplane.origin_x += diff_x;
        ctx.cplane.origin_y += diff_y;
      }
      cplane_redraw(ctx,canvas);
    }
    ...
    function cplane_redraw(ctx,canvas){
      ...
      ctx.scale(ctx.cplane.zoom,ctx.cplane.zoom);
      var w = canvas.width;
      var h = canvas.height;
      ...
      ///////////////////////////////////////////////
      if(1){
        var fs = Math.min(ctx.cplane.grid,10);
        var fs = ""+fs+"px";
        ctx.font = fs;
        ctx.fillStyle = 'black';
      ...
      if(1){
        ctx.beginPath();
        var r = 2;
        var f = ctx.cplane.zoom * ctx.cplane.scale_factor;
        if(f < 1){
          r /= f;
        }
    ...
    function xyplane_drag(ctx,canvas){
      if(0){
        ...
        var diff_x = ctx.posx2 - ctx.posx1;
        var diff_y = ctx.posy2 - ctx.posy1;
        ctx.xyplane.origin_x += diff_x;
        ctx.xyplane.origin_y += diff_y;
      }
      xyplane_redraw(ctx,canvas);
    }
    ...
    function xyplane_redraw(ctx,canvas){
      ...
      ctx.globalAlpha = 1.0;
      ctx.fillStyle = '#555588';
      ctx.translate(+ctx.xyplane.origin_x,+ctx.xyplane.origin_y);
      ctx.scale(ctx.xyplane.zoom,ctx.xyplane.zoom);
      var w = canvas.width;
      var h = canvas.height;
      var grid = ctx.xyplane.grid;
      ///////////////////////////////////////////////
      ///draw gray grids
      ///////////////////////////////////////////////
      if(1){
      ...
      ///////////////////////////////////////////////
      if(1){
        var fs = Math.min(ctx.xyplane.grid,10);
        var fs = ""+fs+"px";
        ctx.font = fs;
        ctx.fillStyle = 'black';
        ///draw horizontal labels
	...
	return [myval,s,phrase];
      ...
      if (this.diagram && (v=this.re_scalar_complex.exec(s))!==null) {
	/// a complex number: '$a.re', or '$a.im'
	let phrase = v[0];
	let symbol = v[1];
	let re_im = v[2];
	s = v[3];
    ...
    function exec_complex_func(func_name,func_args,g,z=0) {
