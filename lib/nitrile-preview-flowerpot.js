  ...
  to_setup_script(){
    ...
    function board_setup_canvas(canvas){
      ...
      if(1){
        ...
        ctx.note.all_stroke = all_sml_stroke;
        ctx.note.all_arrow = all_sml_arrow;
    ...
    function board_handle_keydown(shiftKey,keycode,ctx,canvas){
      ...
      case 'Minus':
        ...
        break;
      case 'Slash':
        if(ctx.note.all_stroke==all_big_stroke){
          ctx.note.all_stroke = all_sml_stroke;
          ctx.note.all_arrow = all_sml_arrow;
        }else{
          ctx.note.all_stroke = all_big_stroke;
          ctx.note.all_arrow = all_big_arrow;
        }
        break;
      case 'BracketLeft':
        ///PENCIL-SMALL
        ctx.note.all_fs -= 1;
        ctx.note.all_fs = Math.max(2,ctx.note.all_fs);
        break;
      case 'BracketRight':
        ///PENCIL-LARGE      
        ctx.note.all_fs += 1;
        ctx.note.all_fs = Math.min(30,ctx.note.all_fs);
        break;
      case 'KeyK':
        ///move shape up
    ...
    function board_onmousemove(evt){
      ...
      if(evt.button==0){
        ...
        if(ctx.mouseisdown){
          ...
          if(ctx.mode=='note'){
            note_move(ctx,canvas);
          }else if(ctx.mode=='cplane'){
            cplane_move(ctx,canvas);
          }else if(ctx.mode=='xyplane'){
            xyplane_move(ctx,canvas);
          }
        }
      }
    }
    ...
    function note_redraw(ctx,canvas){
    ...
    function cplane_move(ctx,canvas){
      ///EMPTY
    }
    function cplane_drag(ctx,canvas){
      ...
      if(ctx.note.pointisinhandle){
        ...
        if(ctx.shiftKey){
          ...
          dx %= ctx.note.grid;
          dy %= ctx.note.grid;
          if(dx < 0) dx += ctx.note.grid;
          if(dy < 0) dy += ctx.note.grid;
          let snap_dist = 5;//snap to grid within 10px
//console.log('diff_y=',diff_y,'diff_x=',diff_x,'dy=',dy,'dx=',dx);
          let r = Math.sqrt(diff_x*diff_x+diff_y*diff_y);
          if( (!ctx.note.snap_type) && (dy<=snap_dist||(ctx.note.grid-dy)<=snap_dist) && (dx<=snap_dist||(ctx.note.grid-dx)<=snap_dist) ){
            ctx.note.snap_type = 'point';
          }
          if( (!ctx.note.snap_type) && (Math.abs(r-grid)<=snap_dist) ){
            ctx.note.snap_type = 'circle';
          }
          if( (!ctx.note.snap_type) && (dx<=snap_dist||(ctx.note.grid-dx)<=snap_dist) ){
            ctx.note.snap_type = 'vline';
          }
          if( (!ctx.note.snap_type) && (dy<=snap_dist||(ctx.note.grid-dy)<=snap_dist) ){
            ctx.note.snap_type = 'hline';
          }
          if(ctx.note.snap_type=='circle'){
            let rate = r/grid;
            diff_x /= rate;
            diff_y /= rate;
          }else if(ctx.note.snap_type=='point'){
            diff_y = Math.round(diff_y / ctx.note.grid) * ctx.note.grid;
            diff_x = Math.round(diff_x / ctx.note.grid) * ctx.note.grid;
          }else if(ctx.note.snap_type=='vline'){
            diff_x = Math.round(diff_x / ctx.note.grid) * ctx.note.grid;
          }else if(ctx.note.snap_type=='hline'){
            diff_y = Math.round(diff_y / ctx.note.grid) * ctx.note.grid;
          }
        }else{
...
//console.log('val_real=',val_real,'val_imag=',val_imag,'which=',which);
      ///////////////////////////////////////////////
      ///draw gray grids
      ///////////////////////////////////////////////
      ...
      //
      ctx.beginPath();
      ctx.arc(0,0,1*ctx.note.grid,0,Math.PI*2);
      ctx.stroke();
      ...
      //gray arrow
      //
      ctx.lineWidth = ctx.note.all_stroke*1.4;
      ...
      if(Number.isFinite(arrow_x)&&
         ...
         if(arrow_x == 0 && arrow_y == 0) {
          ...
          drawArrow(ctx,0,0,arrow_x,arrow_y,3,1,Math.PI/8,ctx.note.all_arrow);
         }
      }
      //
      //gray handle 
      ...
      if(ctx.note.rotate_ang!=0 ||
        ...
        ctx.beginPath();
        ctx.strokeStyle = ctx.note.all_color;
        ctx.fillStyle = ctx.note.all_color;
        ctx.lineWidth = 0.5;
        for (var x=-50; x <= 50; x++) {
          ctx.moveTo(x*grid,0);
          ctx.lineTo(x*grid,-50*grid);   
    ...
    function xyplane_move(ctx,canvas){
    }
...
//console.log('left=',left,'right=',right,'D=',D);
      ...
      if(1){
        ctx.fillStyle = ctx.note.all_color;
        ctx.strokeStyle = ctx.note.all_color;
        ctx.lineWidth = ctx.note.all_stroke;
        var g = {};
        var r = 1;
        var s = ctx.note.text.trim();
        var x0 = NaN;
