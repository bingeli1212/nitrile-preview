  ...
  to_setup_script(){
    ...
    function board_setup_canvas(canvas){
      ...
      for(let i=0; i < allnames.length; ++i){
        ...
        if(typeof hexc == 'string' && hexc.length==6){
          ...
          g = parseInt(g);
          b = parseInt(b);
          ctx.d.rgbs[name]=[r,g,b];
        }
      }
      ctx.d.myop = 'line';
      ctx.d.myline = 1.5;
      ctx.d.mycolor = 'DarkSlateBlue';
      ctx.d.mysmear = ctx.d.rgbs['DarkSlateBlue'];
    }
        ...
        if(myop == 'eraser'){
          ...
          for(let i=0; i < idata.data.length; i+=4){
            if(idata.data[i+3] <= cutoff){
              idata.data[i+0] = idata.data[i+0]*0.5 + mysmear[0]*0.5;
              idata.data[i+1] = idata.data[i+1]*0.5 + mysmear[1]*0.5;
              idata.data[i+2] = idata.data[i+2]*0.5 + mysmear[2]*0.5;
              let alpha = Math.floor(Math.random()*cutoff);
              idata.data[i+3] = idata.data[i+3]*0.5 + alpha*0.5;
            }
          }
          ctx.putImageData(idata,ctx.d.myposx-(lw/2),ctx.d.myposy-(lw/2));
        }else if(myop == 'line'){
          ///PENCIL
          var lw = myline*sx;
    ...
    function select_change_color(value,name){
      ...
      if(canvas && canvas.getAttribute("data-type")=='board'){
        var ctx = canvas.getContext("2d");
        ctx.d.mycolor = value;
        ctx.d.mysmear = ctx.d.rgbs[value];
      }
    }
