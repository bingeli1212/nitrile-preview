    ...
    return `\
       <select name='${canvasid}' onchange='select_change_line(this.value,this.name)'>
          ...
          <optgroup label='Smudge'>
           <option value='smear/1.5'>Light Smudge</option>
           <option value='smear/2.5'>Med Smudge</option>
           <option value='smear/4'>Heavy Smudge</option>
          </optgroup>
        ...
        if(myop == 'eraser'){
          ...
          for(let i=0; i < idata.data.length; i+=4){
            if(idata.data[i+3] <= cutoff){
              idata.data[i+0] = mysmear[0]||0;
              idata.data[i+1] = mysmear[1]||0;
              idata.data[i+2] = mysmear[2]||0;
              idata.data[i+3] = cutoff;
            }
          }
