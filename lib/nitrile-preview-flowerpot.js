  ...
  to_setup_script(){
    ...
    function board_setup_canvas(canvas){
      ...
      if(1){
        ...
        ctx.note.transform = [1,0,0,1,0,0];
        ///TESTING
        if(1){
          ...
          points.push({op:'z'});
          let ball = {points,transform,corner};
...
//console.log('x-intercept','y0=',y0,'yi=',yi,'x0=',x0,'xii=',xii,'sign0=',sign0,'sign=',sign);
    ...
    function ball_drag(ctx,canvas){
      if(ctx.note.rotateflag){
        ...
        let angle0 = Math.atan2(ctx.posy0-y, ctx.posx0-x);
    ...
    function ball_end(ctx,canvas){
      if(ctx.note.rotateflag){
        ///SAVE the rotate transform back to hitball
        ctx.note.hitball.transform = [...ctx.note.transform];///make a copy
        ctx.note.rotateflag = 0;
        ctx.note.transform = [1,0,0,1,0,0];
      }else if(ctx.note.moveflag){
        let dx = ctx.posx2-ctx.posx0;
        ...
        if(1){
          ...
          ctx.note.hitball.transform = [a,b,c,d,e,f];
        }
        ctx.restore();
        ctx.note.moveflag = 0;
      }else if(ctx.note.hitball && ctx.note.corner >= 0){
        ctx.note.downcorner = ctx.note.corner;
      }else if(ctx.note.insideflag){
        ctx.note.insideflag = 0;
        ctx.note.corner = -1;
        ctx.note.downcorner = -1;
      }else{
        ctx.note.hitball = ctx.note.hitball;
        ctx.note.corner = -1;
        ctx.note.downcorner = -1;
      }
      ctx.note.rotateflag = 0;
    ...
    function ball_redraw(ctx,canvas){
      ...
      ctx.note.balls.forEach((ball) => {
        ...
        let [a,b,c,d,e,f] = ball.transform;
        ctx.setTransform(a,b,c,d,e,f);
        let [x,y] = ball_getcornerxy(ball);
        if(ctx.note.rotateflag && ball == ctx.note.hitball){
          ctx.translate(x,y);
          ctx.rotate(ctx.note.rotate_ang);
          ctx.translate(-x,-y);
          let matrix = ctx.getTransform();
          let {a,b,c,d,e,f} = matrix;
          ctx.note.transform = [a,b,c,d,e,f];
        }else if(ctx.note.moveflag && ball == ctx.note.hitball){
          let matrix = ctx.getTransform();
          let a = matrix.a;
          let b = matrix.b;
          let c = matrix.c;
