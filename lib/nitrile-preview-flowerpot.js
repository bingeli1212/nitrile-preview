...
`;
  ...
  to_sidesvg_for_id(id,toc,max_n=32){
    ...
    toc.forEach((p,j) => {
      if(p.id==id){
        where_j = j;
        all.push(`<rect x="0" y="${j*10}" width="88" height="10" stroke="none" fill="#124A47"/>`);
        all.push(`<text id="svg${p.id}" x="5" y="${j*10+8}" stroke="none" fill="#DDEEFF" font-size="5pt" >${p.order} ${p.icon} ${this.smooth(p.title)}</text>`)
      }else{
        all.push(`<text id="svg${p.id}" x="5" y="${j*10+8}" stroke="none" fill="#124A47" font-size="5pt" >${p.order} ${p.icon} ${this.smooth(p.title)}</text>`)
      }
    })
    var text = all.join('\n');
    if(where_j > max_n){
