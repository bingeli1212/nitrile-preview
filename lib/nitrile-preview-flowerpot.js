...
`;
  ...
  write_frame_folder(id,frame,subframes,toc,pages){
    ...
    all.push(`<h1 class='${slidetitle_css}'> ${id} <b>${this.uncode(frame.style,frame.title)}</b> </h1>`);
  ...
  write_one_frame(id,order,icon,frame,issub,parentid,toc,pages,topframe){
    ...
    if(issub){
      order = '';
      all.push(`<h1 class='${title_css}'>${order} <a href='#frame${parentid}'></a> <b>${this.uncode(topframe.style,topframe.title)}<br/><span style='font-size:0.5em'>${icon} ${this.uncode(frame.style,frame.title)}</span></b> </h1>`);
    }else{
      icon = '';
      all.push(`<h1 class='${title_css}'>${order} <a href='#frame${parentid}'></a> <b>${this.uncode(topframe.style,topframe.title)}</b> </h1>`);
    }
    ...
    frame.solutions.forEach((o,i,arr) => {
      ...
      if(o.choice){
        ...
        all.push(`<h2 class='${subtitle_css}'> ${icon} <u>${title}</u> &#160; ${text} </h2>`)
      }else{
        let icon = this.icon_solution;
        let title = this.uncode(o.style,o.title).trim();
