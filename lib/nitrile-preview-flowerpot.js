...
`;
  ...
  write_frame_folder(id,frame,subframes,toc,pages){
    ...
    let edit_contents = this.to_edit_contents(board_id);
  ...
  write_one_frame(id,order,icon,frame,issub,parentid,toc,pages){
    ...
    all.push(`<input class='EDITBUTTON' type='checkbox'></input>`);
    all.push(`<canvas class='EDITBOARD' id='${board_id}' data-type='board' width='441' height='320'></canvas>`);
    all.push(`<form class='EDITFORM' action='/echo' method='post' onsubmit='board_submit(this,"${board_id}")'> ${edit_contents} </form>`);
    all.push(`<div class='PAGENUM'> / </div>`);
    all.push(`</div>`);
    all.push(`<div class='SIDECONTAINER'><img width='90' height='360' style='background:var(--side${id}) 0% 0% no-repeat' usemap='#map${id}' /></div>`);
  ...
  to_edit_contents(canvasid){
    return `\
       ...
       <textarea name='textarea' style='display:none'>
       </textarea>
       <textarea name='filename' style='display:none'>
       </textarea>
       <button onclick='board_download_png("${canvasid}")'>Download</button>
       `;
  }
  ////////////////////////////////////////////////////////////////////////
  //
  ...
  to_setup_script(){
    ...
    function board_download_png(canvasid){
      console.log('james','borad_download_as_png',canvasid);
    ...
    function board_submit(form,canvasid){
      let canvas = document.getElementById(canvasid);
      let dataURL = canvas.toDataURL('image/png');
