  ...
  to_setup_script(){
    ...
    function setup(){
      ...
      */
      document.onkeypress=function(evt){
        ...
        return;
        var type = evt.target.getAttribute('data-type');
        var cursor = evt.target.style.cursor;
        if(mycanvas){           
          if(evt.code=='Digit0'){
            mycursor = 0;
    ...
    function ball_setup_canvas(canvas){
      ...
      if(json){
        ...
        canvas.addEventListener("mouseenter", ball_onmouseenter, false);
        canvas.addEventListener("mouseleave", ball_onmouseleave, false);
        ctx.d = {};
        ctx.d.dirty = 1;
        ctx.d.balls = balls;
        ctx.d.currmousex = -1;
    ...
    function ball_onmouseleave(evt){
      var canvas = evt.target;
      canvas.style.cursor = '';
    }
    function ball_onmouseenter(evt){
      var canvas = evt.target;
