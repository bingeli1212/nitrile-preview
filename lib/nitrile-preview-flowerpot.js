    ...
    function board_drag(ctx,canvas){
      ...
      if(ctx.cake.myop=='ball'){ //board_drag
        if(o.myball.hitball && !o.myball.isnewlyball && !o.myball.isnewlycorner){
          if(ctx.altKey){                 
            if(o.myball.hitball.corner_i==0 && ctx.shiftKey) {
              //skewX
              let [x,y] = ball_getpivotxy(o.myball.hitball);//grow/shrink vertical dimension only
              ...
              if(rotate_ang > 180){
                rotate_ang = 360-rotate_ang;
              }
              if(rotate_ang < -180){
                rotate_ang = 360+rotate_ang;
              }
              let multiple = rotate_ang/360;//360-deg is 2x size larger and 180-deg is 1.5x size larger
              multiple = Math.max(-1,multiple);
              multiple = Math.min(multiple,1);
              if(multiple>0){//grow
                board_skewX(multiple*4, ctx,canvas);
              }else{//shrink
                board_skewX(multiple*4, ctx,canvas);
              }
              ...
              //scaleY
              let [x,y] = ball_getpivotxy(o.myball.hitball);//grow/shrink vertical dimension only
              let distt0 = [ctx.posy1-y, ctx.posx1-x];
              let distt1 = [ctx.posy2-y, ctx.posx2-x];
