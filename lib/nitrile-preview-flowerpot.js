...
`;
  ...
  to_setup_script(){
    ...
    function board_setup_canvas(canvas){
      ...
      if(1){
        ...
        if(1){
          ...
          let color = '';
          points = [];
          points.push({op:'M',x:150,y:50,x1:150,y1:50});
          points.push({op:'L',x:200,y:50,x1:200,y1:50});
          points.push({op:'L',x:200,y:100,x1:200,y1:100});
          points.push({op:'L',x:150,y:100,x1:150,y1:100});
          points.push({op:'z'});
          ball = {points,transform,corner_i,pivot_x,pivot_y,color};
        ...
        if(1){
          ...
          let color = '';
          points = [];
          points.push({op:'M',x:50,y:50,x1:50,y1:50});
          points.push({op:'L',x:100,y:100,x1:100,y1:100});
          points.push({op:'L',x:100,y:50,x1:100,y1:50});
          points.push({op:'z'});
    ...
    function board_scale(v,ctx,canvas){
      if(ctx.mode=='note'){
        note_scale(v,ctx,canvas);
      }else if(ctx.mode=='cplane'){
        cplane_scale(v,ctx,canvas);
      }else if(ctx.mode=='xyplane'){
        xyplane_scale(v,ctx,canvas);
      }else if(ctx.mode=='ball'){
        ball_scale(v,ctx,canvas);
      }
    }
    ...
    function note_scale(v,ctx,canvas){
      let n = ctx.note.myundo.length;
      let o = ctx.note.myundo[n-1];
      o.myfig.w *= v;
      o.myfig.h *= v;
...
//console.log('diff_y=',diff_y,'diff_x=',diff_x,'dy=',dy,'dx=',dx);
    ...
    function cplane_scale(v,ctx,canvas){
    }
...
//console.log('val_real=',val_real,'val_imag=',val_imag,'which=',which);
    ...
    function xyplane_scale(v,ctx,canvas){
    }
...
//console.log('x-intercept','y0=',y0,'yi=',yi,'x0=',x0,'xii=',xii,'sign0=',sign0,'sign=',sign);
    ...
    function ball_scale(v,ctx,canvas){
    }
    ...
    function ball_add(ctx,canvas){
      ...
      let color = '';
      points = [];
      points.push({op:'M',x:x,y:y,x1:x,y1:y});
      points.push({op:'L',x:x+100,y:y,x1:x+100,y1:y});
      points.push({op:'L',x:x+50,y:y-40,x1:x+50,y1:y-40});
      points.push({op:'z'});
      let ball = {points,transform,corner_i,pivot_x,pivot_y,color};
      ctx.note.balls.push(ball);
    }
    ...
    function ball_redraw(ctx,canvas){
      ...
      ctx.note.balls.forEach((ball,j) => {
        ...
        if(ball == ctx.note.hit_ball){
          ///IMPORTANT: the translate operation has to happen before rotate operation because of the nature of the transform matrix of canvas
          ctx.translate(ctx.note.move_x,ctx.note.move_y);
