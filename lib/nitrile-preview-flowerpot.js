...
//console.log('x-intercept','y0=',y0,'yi=',yi,'x0=',x0,'xii=',xii,'sign0=',sign0,'sign=',sign);
    ...
    function ball_start(ctx,canvas){
      ...
      var hitball = ctx.note.hitball;
      if(hitball){
        console.log('hitball',hitball);
      }
      ///check for switching to a new ball
      if(ctx.shiftKey && hitball && hitball.corner_i>=0){
        ///SETUP rotate flag
        ctx.note.rotate_ang = 0;
        ctx.note.rotateflag = 1;
      } else if(ctx.shiftKey && hitball && (hitball.corner_i<0)){
        ///SETUP move flag
        ctx.note.translate_x = 0;
        ctx.note.translate_y = 0;
        ctx.note.moveflag = 1;
        ...
        hitball.points.forEach((pt,i) => {
          ...
          if(ctx.isPointInPath(path2d, ctx.posx0,ctx.posy0)){
            hitball.corner_i = i;
            hitball.pivot_x = ctx.posx0;
            hitball.pivot_y = ctx.posy0;
            console.log('hitball=',hitball);
          }
        });
      } else {
        ///DETERMIND if hitball
...
console.log('pivot=',x,y,'pos1=',ctx.posx1,ctx.posy1,'pos2=',ctx.posx2,ctx.posy2);
        let angle0 = Math.atan2(ctx.posy1-y, ctx.posx1-x);
        let angle1 = Math.atan2(ctx.posy2-y, ctx.posx2-x);
        ctx.note.rotate_ang = angle1-angle0;
console.log('angle0=',angle0,'angle1=',angle1,'rotate_ang=',ctx.note.rotate_ang); 
      }else if(ctx.note.moveflag){
        //DETERMINE MOVE DISTANCE
        ctx.note.translate_x = ctx.posx2 - ctx.posx1;
        ctx.note.translate_y = ctx.posy2 - ctx.posy1;
        console.log('ctx.note.translate_x/y=',ctx.note.translate_x,ctx.note.translate_y);              
      }
      ball_redraw(ctx,canvas);
    }
    ...
    function ball_redraw(ctx,canvas){
      ...
      ctx.note.balls.forEach((ball,j) => {
        ...
        if(ball == ctx.note.hitball){
          ...
          ctx.translate(ctx.note.translate_x,ctx.note.translate_y);
          ...
          ball.pivot_x += ctx.note.translate_x;//updated during move
          ball.pivot_y += ctx.note.translate_y;
          ctx.note.rotate_ang = 0;
          ctx.note.translate_x = 0;
          ctx.note.translate_y = 0;
        }else{
