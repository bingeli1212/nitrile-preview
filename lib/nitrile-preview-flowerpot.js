...
`;
  ...
  write_frame_folder(id,frame,subframes,toc,pages){
    ...
    all.push(`<article class='SLIDE' id='frame${id}' row='${frame.style.row}' style='--boardimg:url("");background-image:var(--boardimg),var(--logosvg),var(--bgsvg);background-repeat:no-repeat;background-position:88px 40px,top left,top left;background-size:441px 320px,88px 40px,cover;' >`);
    pages.push(1);
    ...
    all.push(`<canvas class='EDITBOARD' id='${boardid}' data-type='board' data-imgid='${imgid}' data-boardname='${boardname}' width='441' height='320'></canvas>`);
    ...
    return `\
       <select name='${boardid}' onchange='select_change_line(this.value,this.name)'>
          <optgroup label='Pencil'>
           <option value='pencil/1.0'>Light Pencil</option>
           <option value='pencil/2.0'>Heavy Pencil</option>
          </optgroup>
          <optgroup label='Smudge'>
           <option value='smudge/1.0'>Light Smudge</option>
           <option value='smudge/2.0'>Heavy Smudge</option>
          </optgroup>
          <optgroup label='Erasers'>
            <option value='eraser/1.0'>Light Eraser</option>
            <option value='eraser/2.0'>Heavy Eraser</option>
            <option value='eraser/0'>Clear All</option>
          </optgroup>
          <optgroup label='Lines'>
            <option value='line/1.0'>Light Line</option>
            <option value='line/2.0'>Heavy Line</option>
          </optgroup>
          <optgroup label='Tools'>
            <option value='selection/1.0'>Selection</option>
          </optgroup>
       </select>
  ...
  to_setup_script(){
    ...
    function board_onmousedown(evt){
      ...
      board_start(evt.shiftKey,evt.altKey,ctx,canvas);
    }
    function board_onmousemove(evt){
      ...
      if(ctx.d.mymouseisdown){
        board_move(evt.shiftKey,evt.altKey,ctx,canvas);
      }
    }
    function board_onmouseup(evt){
      ...
      var ctx = canvas.getContext("2d");
      if(ctx.d.mymouseisdown){
        board_end(evt.shiftKey,evt.altKey,ctx,canvas);
      }
    ...
    function board_ontouchstart(evt) {
      ...
      for (var i = 0; i < touches.length; i++) {
        ...
        ctx.d.mymouseisdown = 1;
        if(ctx.d.mymouseisdown){
          board_start(evt.shiftKey,evt.altKey,ctx,canvas,true,false);
        }
      }
    }
    ...
    ////////////////////////////////////////////
    ////////////////////////////////////////////
    function board_draw_copy(ctx,canvas,isfirst,islast){
      ctx.globalAlpha = 1;
      ctx.lineWidth = 1;
      ...
      myposx0 = Math.floor(myposx0);
      myposy0 = Math.floor(myposy0);
      myposx2 = Math.floor(myposx2);
      myposy2 = Math.floor(myposy2);
      if(ctx.d.myselect){       
        let p = ctx.d.myselect;
        ctx.clearRect(0,0, 441,320);
        ctx.putImageData(p, 0,0);
        let x = ctx.d.myselectx;
        let y = ctx.d.myselecty;
        let w = ctx.d.myselectw;
        let h = ctx.d.myselecth;
        let p1 = ctx.getImageData(x,y, w,h);
        x += myposx2 - myposx0;
        y += myposy2 - myposy0;
        ctx.putImageData(p1, x,y);
        ctx.d.mycopy = ctx.getImageData(0,0, 441,320);
        ctx.d.mycopyx = x;
        ctx.d.mycopyy = y;
        ctx.strokeRect(x,y, w,h);
      }
    }
    function board_draw_move(ctx,canvas,isfirst,islast){
      ctx.globalAlpha = 1;
      ctx.lineWidth = 1;
      let myposx0 = ctx.d.myposx0;
      let myposy0 = ctx.d.myposy0;
      ...
      if(ctx.d.myselect){       
        ...
        y += myposy2 - myposy0;
        ctx.putImageData(p1, x,y);
        ctx.d.mymove = ctx.getImageData(0,0, 441,320);
        ctx.d.mymovex = x;
        ctx.d.mymovey = y;
        ctx.strokeRect(x,y, w,h);
      }
    }
    function board_draw_select(ctx,canvas,isfirst,islast){
      ctx.globalAlpha = 1;
      ctx.lineWidth = 1;
      let myposx0 = ctx.d.myposx0;
      let myposy0 = ctx.d.myposy0;
      ...
      if(ctx.d.myselect){  
        ...
        ctx.putImageData(p, 0,0);
        if(isfirst){
          ctx.d.myselectx = myposx0;
          ctx.d.myselecty = myposy0;
          ctx.d.myselectw = 0;
          ctx.d.myselecth = 0
        }else{
    ...
    function board_draw_line(ctx,canvas,isfirst){
      ...
      let boundwidth = canvas.getBoundingClientRect().width;
      let boundheight = canvas.getBoundingClientRect().height;
      let sx = canvaswidth/boundwidth;
      let sy = canvasheight/boundheight;
      ctx.globalAlpha = 1;
      ctx.lineWidth = ctx.d.mystroke*sx;
      ctx.strokeStyle = ctx.d.mycolor;
      ///LINE  
      if(ctx.d.myselect){  
        let p = ctx.d.myselect;
        ctx.clearRect(0,0, 441,320);
        ...
        ctx.stroke();
        ctx.d.myline = ctx.getImageData(0,0, 441,320);
      }
    }
    ...
    function board_draw_smudge(ctx,canvas,first){
      let myposx1 = ctx.d.myposx1;
      let myposy1 = ctx.d.myposy1;
      let myposx2 = ctx.d.myposx2;
      let myposy2 = ctx.d.myposy2;
    ...
    ////////////////////////////////////////////
    ////////////////////////////////////////////
    function board_start(shiftKey,altKey,ctx,canvas){
      if(ctx.d.mymouseisdown){
        if(ctx.d.myop=='selection'){
          if(ctx.d.myselect){
          }else{
          ...
          if(shiftKey && altKey){
            ...
            board_draw_move(ctx,canvas,true,false);
          }else{
            board_draw_select(ctx,canvas,true,false);
          }
    ...
    function board_move(shiftKey,altKey,ctx,canvas){
      if(ctx.d.myop=='selection'){
        if(shiftKey && altKey){
          board_draw_copy(ctx,canvas,false,false);
        }else if(altKey){
          board_draw_move(ctx,canvas,false,false);
        }else{
          board_draw_select(ctx,canvas,false,false);
        }
      }else if(ctx.d.myop=='line'){
    ...
    function board_end(shiftKey,altKey,ctx,canvas){
      if(ctx.d.myop=='selection'){
        if(shiftKey && altKey){
          board_draw_copy(ctx,canvas,false,true);
          if(ctx.d.mycopy){
            let p = ctx.d.mycopy;
            ctx.d.mysave.push(p);
            ctx.d.myredo = [];
            ctx.d.myselect = ctx.d.mycopy;
            ctx.d.myselectx = ctx.d.mycopyx;
            ctx.d.myselecty = ctx.d.mycopyy;
          }
        }else if(altKey){
          board_draw_move(ctx,canvas,false,true);
          if(ctx.d.mymove){
            let p = ctx.d.mymove;
            ctx.d.mysave.push(p);
            ctx.d.myredo = [];
            ctx.d.myselect = ctx.d.mymove;
            ctx.d.myselectx = ctx.d.mymovex;
            ctx.d.myselecty = ctx.d.mymovey;
          }
        }else{
          board_draw_select(ctx,canvas,false,true);
        }
      }else if(ctx.d.myop=='line'){
        let p = ctx.d.myline;
        if(p){
        ...
        if(1){
          let p = ctx.getImageData(0,0, 441,320);
          ctx.d.mysave.push(p);
          ctx.d.myredo = [];
        }
      }
    ...
    function board_undo(boardid){
      ...
      if(ctx.d.mysave.length>0){
        if(ctx.d.mysave.length>1){
          let p = ctx.d.mysave.pop();
          ctx.d.myredo.push(p);
        }
        var n = ctx.d.mysave.length;
        var mysave = ctx.d.mysave[n-1];
        ctx.clearRect(0,0,441,320);
