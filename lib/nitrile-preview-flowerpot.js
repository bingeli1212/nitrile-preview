...
`;
    ...
    return `\
       <div class='DROPDOWNCONTAINER' style='position:static'>
         ...
         <div class='DROPMENU' style='display:flex;flex-direction:row;align-items:flex-start;justify-content:flex-start;flex-wrap:wrap;width:85px;height:119px;z-index:10;' >
             ...
             <button class='ONEAT'          name='1-${boardid}'  data-at='2'                      onclick="board_one_atclicked(this.name,2,this)" />        
             <button class='ONEAT'          name='1-${boardid}'  data-at='3'                      onclick="board_one_atclicked(this.name,3,this)" />        
             <button class='ONEAT'          name='1-${boardid}'  data-at='4'                      onclick="board_one_atclicked(this.name,4,this)" />        
  ...
  to_setup_script(){
    ...
    function board_draw_hilite(ctx,canvas){
      var o = ctx.cake;
      const x = o.mybasis.x+o.mybasis.dx;
      const y = o.mybasis.y+o.mybasis.dy; 
      const w = o.mybasis.w+o.mybasis.dw;
      const h = o.mybasis.h+o.mybasis.dh;
      const gap = Math.min(10,w);
      const dep = Math.min(10,h);
      ctx.save();
      ctx.fillStyle = 'black';
      ...
      ctx.strokeStyle = 'black';
      ///
      ctx.moveTo(x,     y); ctx.lineTo(x+gap, y);
      ctx.moveTo(x,     y); ctx.lineTo(x,     y+dep);
      ///
      ctx.moveTo(x+w,     y); ctx.lineTo(x+w-gap, y);
      ctx.moveTo(x+w,     y); ctx.lineTo(x+w,     y+dep);
      ///
      ctx.moveTo(x+w,     y+h); ctx.lineTo(x+w-gap, y+h);
      ctx.moveTo(x+w,     y+h); ctx.lineTo(x+w,     y+h-dep);
      ///
      ctx.moveTo(x,     y+h); ctx.lineTo(x+gap, y+h);
      ctx.moveTo(x,     y+h); ctx.lineTo(x,     y+h-dep);
      ///
      ctx.stroke();
      ctx.restore();
    }
      ...
      || ctx.cake.myop=='ellipse' ){  //board_start
        ...
        if(o.mysquare.altKey && o.mysquare.shiftKey){
          if(o.mysquare.face){
            ...
            let y0 = o.mysquare.y;
            if(o.myop=='square'){
              board_addpolypt(x0,y0, o.mysquare.points[0], ctx,canvas);
              board_addpolypt(x0,y0, o.mysquare.points[1], ctx,canvas);
              board_addpolypt(x0,y0, o.mysquare.points[2], ctx,canvas);
              board_addpolypt(x0,y0, o.mysquare.points[3], ctx,canvas);
            }else if(o.myop=='triangle'){
              board_addpolypt(x0,y0, o.mysquare.points[0], ctx,canvas);
              board_addpolypt(x0,y0, o.mysquare.points[2], ctx,canvas);
              board_addpolypt(x0,y0, o.mysquare.points[3], ctx,canvas);
            }
          ...
          if(o.mysquare.face && o.mysquare.corner_i == 0){
            ...
            //copy from face, but keep old dimension
            o.mysquare.s = o.mysquare.face;
            o.mysquare.face = null;
          }
        }
