...
class NitrilePreviewFlowerpot extends NitrilePreviewHtml {
  ...
  to_document() {
    ...
    var allsidesvgs = [];
    toc.forEach((p) => {
      let s = this.to_sidesvg_for_id(p.id,toc,26);
    ...
    var allsidemaps = [];
    toc.forEach((p,i) => {
      let s = this.to_sidemap_for_id(p.id,toc,i,26);
      allsidemaps.push(s);
    })
...
`;
  ...
  write_frame_folder(id,frame,subframes,toc,pages){
    ...
    pages.push(1);
    all.push(`<div class='PAGENUMCONTAINER'><div class='PAGENUM'> / </div></div>`);
    all.push(`<div class='SIDECONTAINER'><img width='88' height='360' style='background:var(--side${id}) 0% 0% no-repeat' usemap='#map${id}' /></div>`);
  ...
  write_one_frame(id,order,icon,frame,issub,parentid,toc,pages){
    ...
    frame.solutions.forEach((o,i,arr) => {
      ...
      pages.push(1);
      all.push(`<div class='PAGENUMCONTAINER'><div class='PAGENUM'> / </div></div>`);
      all.push(`<div class='SIDECONTAINER'><img width='88' height='360' style='background:var(--side${id}) no-repeat' usemap='#map${id}' /></div>`);
  ...
  to_sidesvg_for_id(id,toc,max_n=26){
    var all = [];
    var where_j = 0;
    ...
    toc.forEach((p,j) => {
      if(p.id==id){
        where_j = j;
        all.push(`<rect x="0" y="${j*12}" width="88" height="12" stroke="none" fill="#124A47"/>`);
        all.push(`<text id="svg${p.id}" x="5" y="${j*12+10}" stroke="none" fill="#DDEEFF" font-size="5pt" >${p.order} ${p.icon} ${this.smooth(p.title)}</text>`)
      }else{
        all.push(`<text id="svg${p.id}" x="5" y="${j*12+10}" stroke="none" fill="#124A47" font-size="5pt" >${p.order} ${p.icon} ${this.smooth(p.title)}</text>`)
      }
    })
    var text = all.join('\n');
    if(where_j > max_n){
    ...
    text = (`<svg xmlns="http://www.w3.org/2000/svg" width="88" height="400" viewBox="0 0 88 400"> ${text} </svg>`);
    return text
  }
  to_sidemap_for_id(id,toc,where_j,max_n){
    ...
    toc.forEach((p,j) => {
      // 508 <map name='shapesmap'><area shape='rect' coords='0 0 88 12' href='http:www.google.com'/> </map>
      if(where_j > max_n){
        let dn = where_j - max_n;
        all.push(`<area shape='rect' coords='0 ${(j-dn)*12} 88 ${(j-dn)*12+12}' href='#frame${p.id}'/>`);
      }else{
        all.push(`<area shape='rect' coords='0 ${j*12} 88 ${j*12+12}' href='#frame${p.id}'/>`);
      }
    })
    all.push(`</map>`);
    return all.join('\n')
