...
`;
  ...
  write_one_frame(id,order,icon,frame,issub,parentid,toc,pages){
    ...
    frame.solutions.forEach((o,i,arr) => {
      ...
      all.push(`<article class='SLIDE' onmouseenter='myslide=this' onmouseleave='myslide=null' row='${o.style.row}' style='background-image:var(--logosvg),var(--bgsvg);background-repeat:no-repeat;background-position:top left;background-size:88px 40px,cover;' >`);
      pages.push(1);
      all.push(`<div class='PAGENUMCONTAINER'><div class='PAGENUM'> / </div></div>`);
  ...
  to_setup_script(){
    ...
    function board_setup_canvas(canvas){
      ...
      ctx.d = {};
      canvas.addEventListener("mousedown",  board_onmousedown,  true);
    ...
    function ball_setup_canvas(canvas){
      ...
      if(json){
        ...
        canvas.addEventListener("mousedown",  ball_onmousedown,  true);
