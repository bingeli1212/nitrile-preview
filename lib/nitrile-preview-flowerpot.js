    ...
    function board_draw_move(evt,ctx,canvas,isfirst,islast){
      ...
      if(ctx.d.mysave.length){  
        if(isfirst){
          ...
          ctx.putImageData(mymove,myposx2,myposy2);
          mysave = ctx.getImageData(0,0, 441,320);
          ctx.d.mysave.push(mysave);
          ctx.d.mymovex = myposx2;
          ctx.d.mymovey = myposy2;
          ctx.strokeRect(myposx2,myposy2,w,h);
        }else{
          let mysave = ctx.d.mysave[ctx.d.mysave.length-1];
          ctx.clearRect(0,0, 441,320);
          ctx.putImageData(mysave, 0,0);
