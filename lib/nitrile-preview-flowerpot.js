...
`;
  ...
  to_tool_contents(boardid){
    return `\
         ...
         <div id='tool-${boardid}' class='TOOLMENU' style='display:flex;flex-direction:row;align-items:flex-start;justify-content:flex-start;flex-wrap:wrap;width:391px;height:34px;z-index:10;' >
             ...
             <button class='ONEWG DOT'      name='${boardid}'  data='0'                      onclick="board_onewgclicked(this.name,this)" />         
             <button class='ONEWG'          name='${boardid}'  data='1'                      onclick="board_onewgclicked(this.name,this)" />         
             <button class='ONEWG'          name='${boardid}'  data='2'                      onclick="board_onewgclicked(this.name,this)" />         
             <button class='ONEWG'          name='${boardid}'  data='3'                      onclick="board_onewgclicked(this.name,this)" />         
  ...
  to_setup_script(){
    ...
    function setup(){
      ...
      var select = document.querySelectorAll('.ONEWG');
      for(var i=0; i < select.length; i++){
        var button = select[i];
        var wg = button.getAttribute('data');
        var wg = parseInt(wg);
        var icon = all_wg_icons[wg];
        button.innerHTML = icon;
        if(button.classList.contains('DOT')){
          button.click();
        }
...
*/
    ...
    function board_onewgclicked(name,e){
      var canvas = document.getElementById(name);
      var ctx = canvas.getContext('2d');
      ctx.cake.mywiggleid = e.getAttribute('data');
      board_wiggleidchanged(ctx,canvas);
      board_redraw(ctx,canvas);
      var sibs = getSiblings(e);
      sibs.forEach((v)=>{
        if(v.classList.contains('ONEWG')){
          v.classList.remove('DOT');
        }
      });
      e.classList.add('DOT');
    ...
    function board_wiggleidchanged(ctx,canvas){
      var o = ctx.cake;          
      if(o.myop=='pencil'){
        o.mypencil.wiggleid = o.mywiggleid;
      } 
    }
