...
.PARAGRAPH, .ITEMIZE, .EQUATION, .FIGURE, .TABLE, .LONGTABLE, .LISTING, .COLUMNS {
  text-indent: initial;
  margin-top: 0pt;   
  margin-bottom: 6pt;   
  margin-left: 0px;
...
`;
  ...
  write_frame_folder(id,frame,subframes,toc,pages){
    ...
    pages.push(1);
    all.push(`<div class='TOPCONTAINER'> 
              <h1 class='TITLE'> <b>${id}. ${this.uncode(frame.style,frame.title)}</b> </h1>
              <h2 class='SUBTITLE'> <b></b> </h2>
              </div>`);
  ...
  write_one_frame(id,order,frame,issub,parentid,toc,pages,topframe){
    ...
    if(issub){
      all.push(`<div class='TOPCONTAINER'> 
                <h1 class='TITLE'><b> ${order} ${this.uncode(topframe.style,topframe.title)}</b> </h1>
                <h2 class='SUBTITLE'> <b> ${icon} ${this.uncode(frame.style,frame.title)}</b> </h2>
                </div>`);
      order = '';
    }else{
      order = order + '.';
    ...
    frame.solutions.forEach((o,i,arr) => {
      ...
      if(1){
        if(o.choice){
          ...
          all.push(`<div class='PARAGRAPH' style='font-size:0.8em;font-weight:bold;'> ${this.icon_solution} <u>${title}</u> &#160; ${text} </div>`)
        }else{
          let title = this.uncode(o.style,o.title).trim();
