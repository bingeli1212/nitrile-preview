...
`;
  ...
  to_setup_script(){
    ...
    function board_oneopclicked(name,v){
      ...
      var s = ctx.getImageData(0,0, canvas.width,canvas.height);
      ctx.myop = v;
      ctx.myundo.push(ctx.cake);
      ctx.myredo = [];
      ctx.cake = board_create_cake(ctx,canvas);
      ctx.cake.s = s;
      ctx.setTransform(...ctx.cake.ctx_transform);
      board_op(ctx,canvas);                   
    ...
    function board_setup_canvas(canvas){
      ...
      if(1){
        ...
        ctx.cake.s = null;
        ///
        board_op(ctx,canvas);///this needs to go first to create a new 'cake'
        board_strokeid(ctx,canvas);
        board_colorid(ctx,canvas);
        board_shape(ctx,canvas);
      }
...
console.log('3',a,b,c,d,e,f);
    ...
    function board_undo(ctx,canvas){
      if(ctx.myundo.length>0){
        ...
        ctx.cake = board_create_cake(ctx,canvas);
        ctx.cake.s = null;
        ctx.cake.ctx_transform = [1,0,0,1,0,0];
        ctx.setTransform(...ctx.cake.hit_transform);
      }
    }
