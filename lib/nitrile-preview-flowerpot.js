...
`;
  ...
  to_setup_script(){
    ...
    function smudge_fill(x,y,mysmear,ctx){
      ...
      x = Math.floor(x);
      y = Math.floor(y);
      var ret_val = smudgeRect(x,y,1,1,mysmear,ctx,rand,limit);
      if(ret_val){
    ...
    function smudge_between(myposx0,myposy0,myposx,myposy,lw,mysmear,ctx){
      ...
      for(i=0; i <= n; ++i){
        ...
        let rand = 0.5+Math.random()*0.5;
        let limit = 127;
        smudgeRect(x-(lw/2),y-(lw/2),lw,lw,mysmear,ctx,rand,limit);
      }
    }
    function smudgeRect(x,y,w,h,mysmear,ctx,rand,limit){
