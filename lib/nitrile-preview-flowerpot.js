...
`;
  ...
  to_setup_script(){
    ...
    function board_handle_keydown(shiftKey,keycode,ctx,canvas){
      //var n = ctx.note.myundo.length;
      //var o = ctx.note.myundo[n-1];
      console.log(keycode);
      switch(keycode){
      ///FOR SMUDGE
      case 'Backquote':
    ...
    function board_drag(ctx,canvas){
      ...
      if (all_op=='shape' && all_arg=='circle' && ctx.note.myfig){
        //DRAG  
      ...
      if(all_op=='shape' && all_arg=='rect' && ctx.note.myfig){
        //DRAG  
        let myposx0 = ctx.note.myposx0;
        let myposy0 = ctx.note.myposy0;
        let myposx2 = ctx.note.myposx2;
      ...
      if(all_op=='shape' && all_arg=='tri' && ctx.note.myfig){
        //DRAG  
        let myposx0 = ctx.note.myposx0;
        let myposy0 = ctx.note.myposy0;
        let myposx2 = ctx.note.myposx2;
      ...
      if(all_op=='shape' && all_arg=='line' && ctx.note.myfig){
        //DRAG  
    ...
    function ball_setup_canvas(canvas){
      ...
      if(json){
        ...
        ctx.note = {};
        ctx.note.dirty = 1;
        ctx.note.balls = balls;
        ctx.note.currmousex = -1;
        ctx.note.currmousey = -1;
        ctx.note.hitball = null;//the ball being hit the last time
        ctx.note.downball = null;//the current ball being pressed down
        ctx.note.hitcorner = -1;// the corner last hit by the mouse
        ctx.note.downcorner = -1;//the index for the corner that is down
        ctx.note.showmode = 0;//0=corners,1=centroid
        ctx.note.rotateflag = 0;
        ctx.note.moveflag = 0;
        ctx.note.insideflag = 0;
        ctx.note.movedistx = 0;
        ctx.note.movedisty = 0;
        ctx.note.downmousex = 0;
        ctx.note.downmousey = 0;
        ctx.note.initmousex = 0;
        ctx.note.initmousey = 0;
        ctx.note.rotateangle = 0;
        ctx.note.rotatex0 = 0;
        ctx.note.rotatey0 = 0;
      }
    }
    ...
    function ball_draw_canvas(canvas){
      var ctx = canvas.getContext('2d');
      if(ctx && ctx.note && ctx.note.dirty){
        ctx.note.dirty = 0;
        ctx.clearRect(0,0,500,500);
        ctx.note.hitball = null;
        ctx.note.balls.forEach((ball) => {
          ctx.save();
          let [a,b,c,d,e,f] = ball.transform;
          ctx.setTransform(a,b,c,d,e,f);
          if(ctx.note.rotateflag && ball == ctx.note.downball){
            ctx.translate(ctx.note.rotatex0,ctx.note.rotatey0);
            ctx.rotate(ctx.note.rotateangle);
            ctx.translate(-ctx.note.rotatex0,-ctx.note.rotatey0);
          }else if(ctx.note.moveflag && ball == ctx.note.downball){
            ...
            ctx.translate(ctx.note.movedistx,ctx.note.movedisty);
          ...
          if(1||ball.type=='line'||ball.type=='polyline'){
            var hitflag = ctx.isPointInStroke(ctx.note.currmousex,ctx.note.currmousey);
          }else{
            var hitflag = ctx.isPointInPath(ctx.note.currmousex,ctx.note.currmousey);
          }
          if(hitflag){
            ctx.note.hitball = ball;
          }
          if(hitflag){
            ctx.strokeStyle = 'blue';
            ctx.stroke();
          }else if(ctx.note.downball===ball){
            ctx.strokeStyle = 'blue';
            ctx.stroke();
          }else{
            ctx.stroke();
          }
          if(ctx.note.downball===ball){
    ...
    function ball_draw_canvas_corners(ctx,ball){
      ...
      ball.points.forEach((pt,i) => {
        ...
        let hitflag = ctx.isPointInPath(ctx.note.currmousex,ctx.note.currmousey);
        if(hitflag){
          ctx.note.hitcorner = i;
        }
        if(hitflag){
          ctx.lineWidth = '2';
        }
        if(ctx.note.downcorner >= 0 && ctx.note.downcorner == i){
          ctx.fillStyle = 'red';
        }
        ctx.fill();
        ctx.stroke();
    ...
    function ball_onmousemove(evt){
      ...
      let [sx,sy] = board_calc_sx_and_sy(canvas);
      ctx.note.dirty = 1;
      ctx.note.currmousex = evt.offsetX*sx;
      ctx.note.currmousey = evt.offsetY*sy;
      if(ctx.note.rotateflag){
        let angle0 = Math.atan2(ctx.note.initmousey-ctx.note.downmousey,ctx.note.initmousex-ctx.note.downmousex);
        let angle1 = Math.atan2(ctx.note.currmousey-ctx.note.downmousey,ctx.note.currmousex-ctx.note.downmousex);
        ctx.note.rotateangle = angle1-angle0;
      }else if(ctx.note.moveflag){
        ctx.note.movedistx = ctx.note.currmousex - ctx.note.initmousex;
        ctx.note.movedisty = ctx.note.currmousey - ctx.note.initmousey;
      }
    }
    function ball_onmousedown(evt){
      ...
      if(ctx.note.hitball && ctx.note.downball && ctx.note.hitball != ctx.note.downball){
        ctx.note.downball = ctx.note.hitball;
        ctx.note.hitcorner = -1;
        ctx.note.downcorner = -1;
      } else if(ctx.note.hitball && !ctx.note.downball && ctx.note.hitball != ctx.note.downball){
        ctx.note.downball = ctx.note.hitball;
        ctx.note.hitcorner = -1;
        ctx.note.downcorner = -1;
      }
      ///check for corners of existing ball
      if(ctx.note.downball && ctx.note.downcorner >= 0 && ctx.note.downcorner != ctx.note.hitcorner){
        //start rotate
        ctx.note.rotateflag = 1;
        ctx.note.rotateangle = 0;
        let [x0,y0] = getBallPointXY(ctx.note.downball,ctx.note.downcorner);
        ctx.note.rotatex0 = x0;
        ctx.note.rotatey0 = y0;
        ctx.note.initmousex = evt.offsetX*sx;
        ctx.note.initmousey = evt.offsetY*sy;
      }else if(ctx.note.downball && ctx.note.hitball && ctx.note.downball == ctx.note.hitball){
        //start move
        ctx.note.moveflag = 1;
        ctx.note.movedistx = 0;
        ctx.note.movedisty = 0;
        ctx.note.initmousex = evt.offsetX*sx;
        ctx.note.initmousey = evt.offsetY*sy;
      }else if(ctx.note.downball && ctx.note.hitcorner >= 0){
        ctx.note.insideflag = 1;
        ctx.note.initmousex = evt.offsetX*sx;
        ctx.note.initmousey = evt.offsetY*sy;
        ctx.note.rotateflag = 0;
        ctx.note.moveflag = 0;
    ...
    function ball_onmouseup(evt){
      ...
      ctx.note.currmousex = evt.offsetX*sx;
      ctx.note.currmousey = evt.offsetY*sy;
      if(ctx.note.rotateflag && ctx.note.rotateangle != 0){
        ctx.save();
        let [a,b,c,d,e,f] = ctx.note.downball.transform;
        ctx.setTransform(a,b,c,d,e,f);
        ctx.translate(ctx.note.rotatex0,ctx.note.rotatey0);
        ctx.rotate(ctx.note.rotateangle);
        ctx.translate(-ctx.note.rotatex0,-ctx.note.rotatey0);
        if(1){
          ...
          ctx.note.downball.transform = [a,b,c,d,e,f];
        }
        ctx.restore();
        ctx.note.rotateflag = 0;
      }else if(ctx.note.moveflag && (ctx.note.movedistx != 0 || ctx.note.movedisty != 0)){
        ctx.save();
        let [a,b,c,d,e,f] = ctx.note.downball.transform;
        ctx.setTransform(1,0,0,1,0,0);
        ctx.translate(ctx.note.movedistx,ctx.note.movedisty);
        ...
        ctx.note.downcorner = ctx.note.hitcorner;
        ctx.note.downmousex = evt.offsetX*sx;
        ctx.note.downmousey = evt.offsetY*sy;
      }else if(ctx.note.insideflag){
        ctx.note.insideflag = 0;
        ctx.note.hitcorner = -1;
        ctx.note.downcorner = -1;
      }else{
    ...
    ////////////////////////////////////////////
    ////////////////////////////////////////////
    function cplane_setup_canvas(canvas){
      var ctx = canvas.getContext('2d');
      ctx.lineCap = "round";
      ctx.lineJoin = "round";
      ctx.cplane = {};
      ctx.cplane.step_x = 30;
      ctx.cplane.step_y = 30;
      ctx.cplane.origin_x = 300;
      ctx.cplane.origin_y = 300;
      ctx.cplane.handle_x = ctx.cplane.step_x;
      ctx.cplane.handle_y = 0;
      ctx.cplane.translate_x = 0.0;
      ctx.cplane.translate_y = 0.0;
      ctx.cplane.scale_x = 1.0;
      ctx.cplane.scale_y = 1.0;
      ctx.cplane.rotate_ang = 0;
      ctx.cplane.is_mouse_down = 0;
      ctx.cplane.action_flag = 'multiply';
      ctx.cplane.val_real = 0;
      ctx.cplane.val_imag = 0;
      cplane_redraw(canvas);
    }
    function cplane_doInputReal(val){
      var input = event.target;
      var canvasid = input.getAttribute('data-canvasid'); 
      var canvas = document.getElementById(canvasid);
      var ctx = canvas.getContext('2d');
      ctx.cplane.val_real = val;
      cplane_redraw(canvas);
    }
    function cplane_doInputImag(val){
      var input = event.target;
      var canvasid = input.getAttribute('data-canvasid'); 
      var canvas = document.getElementById(canvasid);
      var ctx = canvas.getContext('2d');
      ctx.cplane.val_imag = val;
      cplane_redraw(canvas);
    }
    function cplane_doAddButtonClicked(){
      var input = event.target;
      var canvasid = input.getAttribute('data-canvasid'); 
      var canvas = document.getElementById(canvasid);
      var ctx = canvas.getContext('2d');
      ctx.cplane.action_flag = 'add';
      cplane_reset(canvas);
    }
    function cplane_doMultiplyButtonClicked(){
      var input = event.target;
      var canvasid = input.getAttribute('data-canvasid'); 
      var canvas = document.getElementById(canvasid);
      var ctx = canvas.getContext('2d');
    ...
    function cplane_doResetClicked(){
      var input = event.target;
      var canvasid = input.getAttribute('data-canvasid'); 
      var canvas = document.getElementById(canvasid);
      cplane_reset(canvas);
    }
    function cplane_doZoomoutClicked(){
      var input = event.target;
      var canvasid = input.getAttribute('data-canvasid'); 
      var canvas = document.getElementById(canvasid);
      var ctx = canvas.getContext('2d');
      ctx.cplane.step_x -= 3;
      ctx.cplane.step_y -= 3;
      ctx.cplane.step_x = Math.max(ctx.cplane.step_x,10);
      ctx.cplane.step_y = Math.max(ctx.cplane.step_y,10);
      ctx.cplane.step_x = Math.min(ctx.cplane.step_x,100);
      ctx.cplane.step_y = Math.min(ctx.cplane.step_y,100);
      ///recompute handle_x/handle_y
      ctx.cplane.handle_x = (ctx.cplane.step_x * ctx.cplane.scale_x) * Math.cos(ctx.cplane.rotate_ang);
      ctx.cplane.handle_y = (ctx.cplane.step_y * ctx.cplane.scale_y) * Math.sin(ctx.cplane.rotate_ang);
      cplane_redraw(canvas);
    }
    function cplane_doZoominClicked(){
      var input = event.target;
      var canvasid = input.getAttribute('data-canvasid'); 
      var canvas = document.getElementById(canvasid);
      var ctx = canvas.getContext('2d');
      ...
      ctx.cplane.step_x = Math.min(ctx.cplane.step_x,100);
      ctx.cplane.step_y = Math.min(ctx.cplane.step_y,100);
      ///recompute ctx.cplane.handle_x/ctx.cplane.handle_y
      ctx.cplane.handle_x = (ctx.cplane.step_x * ctx.cplane.scale_x) * Math.cos(ctx.cplane.rotate_ang);
      ctx.cplane.handle_y = (ctx.cplane.step_y * ctx.cplane.scale_y) * Math.sin(ctx.cplane.rotate_ang);
      cplane_redraw(canvas);
    }
    function cplane_onMouseDown(){
      var canvas = event.target;
      var ctx = canvas.getContext('2d');
      var r = 4*Math.sqrt(ctx.cplane.scale_x);
      if(getDist(ctx.cplane.handle_x,(event.offsetX-ctx.cplane.origin_x),
                 ctx.cplane.handle_y,(event.offsetY-ctx.cplane.origin_y)) <= r){
        ctx.cplane.is_mouse_down = 1;
      }
    }
    function cplane_onMouseMove(){
      var canvas = event.target;
      var ctx = canvas.getContext('2d');
      var r = 4*Math.sqrt(ctx.cplane.scale_x);
      if(ctx.cplane.is_mouse_down && ctx.cplane.action_flag=='multiply'){
        ctx.cplane.handle_x = (event.offsetX-ctx.cplane.origin_x);
        ctx.cplane.handle_y = (event.offsetY-ctx.cplane.origin_y);
        var diff_x = ctx.cplane.handle_x;
        var diff_y = ctx.cplane.handle_y;
        if(1){
          //snap to grid within 2px
          var v = diff_y / ctx.cplane.step_y;
          var v = v - Math.round(v);
          var v = Math.abs(v);
          if(v < r/ctx.cplane.step_x){
            diff_y = Math.round(diff_y / ctx.cplane.step_y) * ctx.cplane.step_y;
            ctx.cplane.handle_y = diff_y;
          }
        }
        if(1){
          //snap to grid within 2px
          var v = diff_x / ctx.cplane.step_x;
          var v = v - Math.round(v);
          var v = Math.abs(v);
          if(v < r/ctx.cplane.step_x){
            diff_x = Math.round(diff_x / ctx.cplane.step_x) * ctx.cplane.step_x;
            ctx.cplane.handle_x = diff_x;
          }
        }
        ctx.cplane.rotate_ang = Math.atan2(diff_y, diff_x);
        ctx.cplane.scale_x = Math.fround(Math.sqrt(diff_y*diff_y + diff_x*diff_x))/ctx.cplane.step_x;
        ctx.cplane.scale_y = ctx.cplane.scale_x;
        cplane_redraw(canvas);
      }
      if(ctx.cplane.is_mouse_down && ctx.cplane.action_flag=='add'){                      
        ctx.cplane.handle_x = (event.offsetX-ctx.cplane.origin_x);
        ctx.cplane.handle_y = (event.offsetY-ctx.cplane.origin_y);
        var diff_x = ctx.cplane.handle_x;
        var diff_y = ctx.cplane.handle_y;
        ...
        ///change only translate_x/y
        ctx.cplane.translate_x = Math.fround(diff_x);
        ctx.cplane.translate_y = Math.fround(diff_y);
        cplane_redraw(canvas);
      }
    }
    function cplane_onMouseUp(){
      var canvas = event.target;
      var ctx = canvas.getContext('2d');
      if(ctx.cplane.is_mouse_down){
        cplane_redraw(canvas);
        ctx.cplane.is_mouse_down = 0;
      }
    }
    function cplane_reset(canvas){
      var ctx = canvas.getContext('2d');
      ctx.cplane.scale_x = 1;
      ctx.cplane.scale_y = 1;
      ctx.cplane.rotate_ang = 0;
      ctx.cplane.translate_x = 0;
      ctx.cplane.translate_y = 0;
      if(document.getElementById('myMove').checked){
        ctx.cplane.handle_x = 0;
        ctx.cplane.handle_y = 0;
      }else if(document.getElementById('myScale').checked){
        ctx.cplane.handle_x = ctx.cplane.step_x;
        ctx.cplane.handle_y = 0;
      }
      cplane_redraw(canvas);
    }
    function cplane_redraw(canvas){
      var ctx = canvas.getContext('2d');
      ctx.resetTransform();
      ...
      for (var x=0; x < +w/2; x += ctx.cplane.step_x) {
        ctx.beginPath();
      ...
      for (var y=0; y < +h/2; y += ctx.cplane.step_y) {
        ctx.beginPath();
        ctx.strokeStyle = 'rgba(0,0,0,0.2)';
        ctx.lineWidth = 0.5;
        ctx.moveTo(-w/2,y);
      ...
      if(1){
        /// draw handle
        ctx.save();
        ctx.translate(ctx.cplane.handle_x,ctx.cplane.handle_y);
        ctx.beginPath();
        var r = 4 * Math.sqrt(ctx.cplane.scale_x);
      ...
      if(1){
        ...
        for(var x=0; x < +w/2; x+=ctx.cplane.step_x){
          var myx = ""+Math.round(x/ctx.cplane.step_x);
          ctx.font = '12px';
          ctx.fillText(myx,x,0);
        }
        for(var x=0; x > -w/2; x-=ctx.cplane.step_x){
          var myx = ""+Math.round(x/ctx.cplane.step_x);
          ctx.font = '12px';
      ...
      if(1){
        ...
        for(var y=0; y < +h/2; y+=ctx.cplane.step_y){
          if(y==0) continue;
          var label = ""+Math.round(-y/ctx.cplane.step_y);
          ctx.font = '12px';
          ctx.fillText(label+"\u{1D456}",0,y);
        }
        for(var y=0; y > -h/2; y-=ctx.cplane.step_y){
          if(y==0) continue;
          var label = ""+Math.round(-y/ctx.cplane.step_y);
      ...
      ctx.scale(ctx.cplane.scale_x,ctx.cplane.scale_y);
      //ctx.translate(-ctx.cplane.origin_x,-ctx.cplane.origin_y);
      ctx.translate(ctx.cplane.translate_x,ctx.cplane.translate_y);
      for (var x=0; x < +w/2; x += ctx.cplane.step_x) {
        ctx.beginPath();
      ...
      if(1){
        let arrow_x = Math.round(val_real)*ctx.cplane.step_x ;
        let arrow_y = -Math.round(val_imag)*ctx.cplane.step_y ;
        arrow_x = Math.round(val_real)*ctx.cplane.step_x ;
        arrow_y = -Math.round(val_imag)*ctx.cplane.step_y ;
