...
`;
  ...
  to_setup_script(){
    ...
    ////////////////////////////////////////////
    ////////////////////////////////////////////
    function board_draw_selection(ctx,canvas,isfirst,islast){
      ...
      let dd = Math.sqrt(w*w + h*h);
      let s     = ctx.d.myselect;
      let myop  = ctx.d.myop;
      let myselection = {x,y,w,h,dd};
      var o = {s,myop,myselection};
      board_redraw(o,ctx,canvas);
      ctx.d.myselection = myselection;
    }
    function board_draw_move(ctx,canvas,isfirst,islast){
      ...
      let myposx2  = ctx.d.myposx2;
      let myposy2  = ctx.d.myposy2;
      let myselection = ctx.d.myselection;
      myposx0 = Math.floor(myposx0);
      myposy0 = Math.floor(myposy0);
      myposx2 = Math.floor(myposx2);
      myposy2 = Math.floor(myposy2);
      ...
      if(ctx.d.shiftKey){
        ...
        let w = mymove.w;
        let h = mymove.h;
        let dx = myposx2 - myposx0;
        let dy = myposy2 - myposy0;
        mymove.dx += dx;
        mymove.dy += dy;
        mymove.dd = Math.sqrt(mymove.dx*mymove.dx + mymove.dy*mymove.dy);
        let s = ctx.d.myselect;
        ctx.clearRect(0,0, 441,320);
        ctx.putImageData(s, 0,0);
        let p1 = ctx.getImageData(x,y, w,h);
        ctx.clearRect(x,y, w,h);
        ctx.putImageData(p1, x+dx,y+dy);
        ///draw hilite
        ctx.save();
        ctx.globalCompositeOperation = 'xor';
    ...
    function board_start(ctx,canvas){
      if(ctx.d.mymouseisdown){
        if(ctx.d.myop=='basis'){
          ...
          let w = 0;
          let h = 0;
          ctx.d.mybasis = {x,y,w,h};
        }else if(ctx.d.myop=='move'){
          let dx = 0;
          let dy = 0;
          let dd = 0;
          ctx.d.mymove = {dx,dy,dd};
    ...
    function board_move(ctx,canvas){
      if(ctx.d.myop=='basis'){
        if(ctx.d.mybasis){
          ...
          let w = ctx.d.myposx2 - ctx.d.myposx0;
          let h = ctx.d.myposy2 - ctx.d.myposy0;
          ctx.d.mybasis.w = w;
          ctx.d.mybasis.h = h;
          let s = ctx.d.myselect;
          ctx.clearRect(0,0, 441,320);
    ...
    function board_end(ctx,canvas){
      if(ctx.d.myop=='basis'){
        if(ctx.d.mybasis){
          ...
          let s        = ctx.d.myselect;
          let myop     = ctx.d.myop;
          if(s && mybasis && myop && (mybasis.w && mybasis.h) ){
            ctx.d.mysave.push({s,myop,mybasis});
            ctx.d.myredo = [];
            var n = ctx.d.mysave.length;
            var o = ctx.d.mysave[n-1];
            board_redraw(o,ctx,canvas);
        ...
        if(ctx.d.mybasis && ctx.d.mymove){
          ...
          let s        = ctx.d.myselect;
          let myop     = ctx.d.myop;
          if(s && mybasis && mymove && myop && (mymove.dx||mymove.dy)){
            ctx.d.mysave.push({s,myop,mybasis,mymove});
            ctx.d.myredo = [];
            var n = ctx.d.mysave.length;
            var o = ctx.d.mysave[n-1];
            board_redraw(o,ctx,canvas);
            ctx.d.mybasis = {...mybasis};
            ctx.d.mybasis.x += mymove.dx;
            ctx.d.mybasis.y += mymove.dy;
            ctx.d.mymove = null;
          }
        }
    ...
    function board_redraw(o,ctx,canvas){
      ...
      if(o.myop=='circle'){
        ...
        ctx.stroke();
        ctx.d.myselect = ctx.getImageData(0,0, 441,320);
      } else if(o.myop=='basis'){
        let mybasis = o.mybasis;
        let x = mybasis.x;
        let y = mybasis.y;
        let w = mybasis.w;
        let h = mybasis.h;
