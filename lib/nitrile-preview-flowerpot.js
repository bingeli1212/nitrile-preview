...
`;
  ...
  write_frame_folder(id,frame,subframes,toc,pages){
    ...
    all.push(`<article class='SLIDE' id='frame${id}' row='${frame.style.row}' style='--boardimg:url("${boardimg}");background-image:var(--boardimg),var(--logosvg),var(--bgsvg);background-repeat:no-repeat;background-position:88px 40px,top left,top left;background-size:441px 320px,88px 40px,cover;' >`);
    pages.push(1);
  ...
  write_one_frame(id,order,icon,frame,issub,parentid,toc,pages){
    ...
    frame.solutions.forEach((o,i,arr) => {
      if(o.choice){
        ...
        all.push(`<div style='${paragraph_css}'> ${this.icon_solution} <i>${this.uncode(o.style,o.title)}</i> ${text} </div>`);
      }else if(o.details){
        all.push(`<div style='${paragraph_css}'> <details><summary>${this.uncode(o.style,o.title)}</summary><p>${this.untext(o.style,o.body)}</p></details> </div>`);
      }else{
        all.push(`<div style='${paragraph_css}'> ${this.icon_solution} <i>${this.uncode(o.style,o.title)}</i> </div>`);
      }
    });
    all.push('</article>');
    all.push('');
    // 
    //NOTE: filter out the ones that are details
    //
    frame.solutions = frame.solutions.filter((p) => !p.details);
    // 
    ...
    return `\
       <select name='${boardid}' onchange='select_change_line(this.value,this.name)'>
          ...
          <optgroup label='Select'>
            ...
            <option value='move/1.0'>Move</option>
          </optgroup>
       </select>
       ...
       <button type='button' onclick='board_undo("${boardid}")'>Undo</button>
       <button>Save</button>
       `;
  }
  ////////////////////////////////////////////////////////////////////////
  ...
  to_setup_script(){
    ...
    function board_setup_canvas(canvas){
      ...
      if(img){
console.log('james','ctx.drawImage','imgid=',imgid);
    ...
    function board_onmousedown(evt){
      ...
      if(mycursor.length){
        ...
        if(ctx.d.myop=='move' && ctx.d.mymove){
    ...
    function board_draw_move(evt,ctx,canvas,isfirst,islast){
      ...
      myposx1 = Math.floor(myposx1);
      myposy1 = Math.floor(myposy1);
      myposx2 = Math.floor(myposx2);
      myposy2 = Math.floor(myposy2);
      if(ctx.d.mysave.length){  
        if(isfirst){
        }else if(islast){
          let mymoveto = ctx.d.mymoveto;
          ctx.clearRect(0,0, 441,320);
          ctx.putImageData(mymoveto, 0,0);
          ctx.d.mysave.push(mymoveto);
          if(ctx.d.mysave.length > 20){
            ctx.d.mysave.shift();
          }
          let tox = ctx.d.mymovetox;
          let toy = ctx.d.mymovetoy;
          let w = ctx.d.mymovew;
          let h = ctx.d.mymoveh;
          ctx.strokeRect(tox,toy,w,h);
          ctx.d.mymovex = tox;
          ctx.d.mymovey = toy;
        }else{
          let mysave = ctx.d.mysave[ctx.d.mysave.length-1];
          ctx.clearRect(0,0, 441,320);
          ...
          let w = ctx.d.mymovew;
          let h = ctx.d.mymoveh;
          let mymove = ctx.d.mymove;
          ctx.clearRect(x,y,w,h);
          x += myposx2 - myposx0;
          y += myposy2 - myposy0;
          ctx.putImageData(mymove,x,y);
          ctx.d.mymoveto = ctx.getImageData(0,0, 441,320);
          ctx.d.mymovetox = x;
          ctx.d.mymovetoy = y;
    ...
    function board_draw_select(evt,ctx,canvas,isfirst,islast){
      ...
      if(ctx.d.mysave.length){  
        ...
        if(isfirst){
          ...
          if(w > 0 && h > 0){
            ctx.d.mymove = ctx.getImageData(myposx0,myposy0,w,h);
            ctx.d.mymovex = myposx0;
            ctx.d.mymovey = myposy0;
            ctx.d.mymovew = w;
            ctx.d.mymoveh = h
            ctx.strokeRect(myposx0,myposy0,w,h);
          }
        }
      }
    }
