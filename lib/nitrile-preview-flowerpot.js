...
`;
    ...
    return `\
       <select name='${boardid}' onchange='board_change_op(this.value,this.name)'>
        ...
        <optgroup label='Selection'>
           ...
           <option value='select/zoom'>Zoom</option>
        </optgroup>
       </select>
  ...
  to_setup_script(){
    ...
    function board_move(ctx,canvas){
      ...
      if(ctx.d.myop=='select'){
        //MOVE  
console.log('ctx.d.mybasis',ctx.d.mybasis);
        if(ctx.d.mybasis.a){
...
console.log('dx=',dx,'dy=',dy);
          ...
          if(ctx.d.mybasis.a=='move'){
            ...
            let w = p1.width;
            let h = p1.height;
            let p2 = resizeImageData(p1, w+dx, h+dy, ctx);
            ctx.clearRect(ctx.d.mybasis.x,ctx.d.mybasis.y, ctx.d.mybasis.w,ctx.d.mybasis.h);
...
console.log('resizeImageData',owidth,oheight,odata.data.length,iwidth,iheight,idata.data.length);
      for(var row = 0; row < iheight; row++) {
        for(var col = 0; col < iwidth; col++) {
          let i = row*iwidth + col;
          let j = row*owidth + col;
          idata.data[i*4 + 0] = odata[j*4 + 0];
          idata.data[i*4 + 1] = odata[j*4 + 0];
          idata.data[i*4 + 2] = odata[j*4 + 0];
          idata.data[i*4 + 3] = odata[j*4 + 0];
        }
      }
      return idata;
    }
