...
`;
    ...
    topframes.forEach((topframe,j,arr) => {
      ...
      if(n==0){
        ...
        all.push(`<div class='TOPCONTAINER'> 
                  <h1 class='TITLE'> <b>Table Of Contents</b> </h1>
                  <h2 class='SUBTITLE'> <b></b> </h2>
                  </div>`);
                  */
  ...
  write_frame_folder(id,frame,subframes,toc,pages){
    ...
    pages.push(1);
    all.push(`<div class='TOPCONTAINER'> 
              <h1 class='TITLE'> ${id} ${this.uncode(frame.style,frame.title)} </h1>
    ...
    all.push(`<canvas class='BOARD CAKE' id='${boardid}' data-boardpng='${boardpng}' data-frameid='${frameid}' width='${this.board_width}' height='${this.board_height}' style='position:absolute;left:7mm;top:0mm;background-image:url("${boardpng}");background-repeat:no-repeat;background-size:114mm 80mm;' onmouseenter="board_onmouseenter()" onmoueleave="board_onmouseleave()" onmousedown="board_onmousedown()" onmousemove="board_onmousemove()" onmouseup="board_onmouseup()" ontouchstart="board_ontouchstart()" ontouchmove="board_ontouchmove()" ontouchend="board_ontouchend()" ontouchcancel="board_ontouchcancel()" ></canvas>`);
    if(1){
      let order = id;
      ...
      subframes.forEach((subframe,j,arr) => {
        ...
        //let icon = `<span style='position:absolute;right:100%;top:50%;transform:translate(-1.12ex,-50%);'>${j+1}</span>`;
        let icon = this.icon_folder;
        all.push(`<li class='PACK LI' style='position:relative;font-style:oblique;'> ${icon} <b> <a href='#frame-${id}x${j+1}'> ${this.uncode(subframe.style,subframe.title)} </a> </b> </li>`);
        if(j+1==arr.length){
          all.push(`</ul>`);
        }
      });
  ...
  write_one_frame(id,order,frame,issub,parentid,toc,pages,topframe){
    ...
    if(issub){
      all.push(`<div class='TOPCONTAINER'> 
      <h1 class='TITLE'> ${order} ${this.uncode(topframe.style,topframe.title)} </h1>
      <h2 class='SUBTITLE'> ${icon} ${this.uncode(frame.style,frame.title)} </h2>
      </div>`);
      order = '';
    }else{
      order = order;
    ...
    frame.solutions.forEach((o,i,arr) => {
      ...
      all.push(`<div class='BOTTOMCONTAINER'> <div class='EDITFORM' > ${tool_contents} </div> <div class='PAGENUM'> / </div> </div>`);
      all.push(`<div class='INSIDECONTAINER'>`);
      all.push(`<canvas class='BOARD CAKE' id='${boardid}' data-boardpng='${boardpng}' data-frameid='${frameid}' width='${this.board_width}' height='${this.board_height}' style='position:absolute;left:7mm;top:0mm;background-image:url("");background-repeat:no-repeat;background-size:114mm 80mm;' onmouseenter="board_onmouseenter()" onmoueleave="board_onmouseleave()" onmousedown="board_onmousedown()" onmousemove="board_onmousemove()" onmouseup="board_onmouseup()" ontouchstart="board_ontouchstart()" ontouchmove="board_ontouchmove()" ontouchend="board_ontouchend()" ontouchcancel="board_ontouchcancel()" ></canvas>`);
      if(1){
        if(o.choice){
          let title = this.uncode(o.style,o.title).trim();
          let text = this.to_choice(o.style,o.body,o.choice).trim();
          all.push(`<div class='PARAGRAPH' style='font-size:0.8em;font-weight:bold;'> ${this.icon_solution} <i>${title}</i> &#160; ${text} </div>`)
        }else{
          let title = this.uncode(o.style,o.title).trim();
  ...
  to_setup_script(){
    ...
    function board_bg2fg(ctx,canvas){
      board_bgimport(ctx,canvas);
    }
    function board_bgimport(ctx,canvas){
      var o = ctx.cake;          
      var img = new Image();
      img.crossOrigin = 'anonymous';
      img.onload = function(){
      ...
      let url = canvas.style.backgroundImage;
      url = url.slice(5,-2)
      console.log('url=',url);
      let re = /^url\(\"(.*)\"\)$/;
      let v = re.exec(url);
