...
`;
  ...
  to_setup_script(){
    ...
    function smudge_fill(x,y,sx,sy,mysmear,ctx){
      x = Math.floor(x);
      y = Math.floor(y);
      if((Math.abs(x-sx) < 50) && (Math.abs(y-sy) < 50)){
        ///ONLY smudge within the range of 50x50 
        let limit = 0;
        var ret_val = smudgeRect(x,y,1,1,mysmear,ctx,limit);
        if(ret_val){
          smudge_fill(x-1,y,sx,sy,mysmear,ctx);
          smudge_fill(x,y-1,sx,sy,mysmear,ctx);
          smudge_fill(x+1,y,sx,sy,mysmear,ctx);
          smudge_fill(x,y+1,sx,sy,mysmear,ctx);
        }
      }
    ...
    function board_draw_smudge_fill(x1,y1,sx,sy,ctx,canvas){
      let mysmear = ctx.d.mysmear;
      let mystroke= ctx.d.mystroke;
      smudge_fill(x1,y1,sx,sy,mysmear,ctx);
    }
