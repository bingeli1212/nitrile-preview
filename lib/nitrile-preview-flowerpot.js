...
//console.log('board_start','op=',ctx.cake.myop,'id=',canvas.id,'altKey=',ctx.altKey,'shiftKey=',ctx.shiftKey);
      ...
      if(ctx.cake.myop=='ball'){ //board_start
        ...
        if(ctx.shiftKey && ctx.altKey && !o.hit_ball){
          ...
          if(success==0){
            let old_hit_ball = o.hit_ball;//check to see if we have switched to a different hit ball
            ///DETERMIND hit_ball
            o.hit_ball = null;
            o.balls.forEach((ball) => {
              ctx.save();
            ...
            o.myball.washitball = (old_hit_ball && o.hit_ball && old_hit_ball === o.hit_ball);
          }
        } 
        board_redraw(ctx,canvas);
      }
    ...
    function board_drag(ctx,canvas){
      ...
      if(ctx.cake.myop=='ball'){ //board_drag
        if(o.hit_ball && !o.myball.isnewlyball){
          if(ctx.altKey){                 
            if(o.hit_ball.corner_i==0 && ctx.shiftKey) {
              //skewX
            ...
            }else if(o.hit_ball.corner_i > 0 && ctx.shiftKey) {
              //scaleY
              let [x,y] = ball_getpivotxy(o.hit_ball);//grow/shrink vertical dimension only
              let angle0 = Math.atan2(ctx.posy1-y, ctx.posx1-x);
              let angle1 = Math.atan2(ctx.posy2-y, ctx.posx2-x);
