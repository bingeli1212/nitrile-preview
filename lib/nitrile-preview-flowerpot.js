...
`;
  ...
  to_setup_script(){
    ...
    function smudge_fill(x,y,stacksize,n,mysmear,ctx){
      x = Math.floor(x);
      y = Math.floor(y);
      let limit = 0;
      if(stacksize<5000 && n < 5000){
        var ret_val = smudgeRect(x,y,1,1,mysmear,ctx,limit);
        if(ret_val){
          n++;
          n = smudge_fill(x-1,y,stacksize+1,n,mysmear,ctx);
          n = smudge_fill(x,y-1,stacksize+1,n,mysmear,ctx);
          n = smudge_fill(x+1,y,stacksize+1,n,mysmear,ctx);
          n = smudge_fill(x,y+1,stacksize+1,n,mysmear,ctx);
        }
      }
    ...
    function board_draw_smudge_fill(x1,y1,sx,sy,ctx,canvas){
      let mysmear = ctx.d.mysmear;
      let mystroke= ctx.d.mystroke;
      let n = smudge_fill(x1,y1,0,0,mysmear,ctx);
console.log('n=',n);
    }
