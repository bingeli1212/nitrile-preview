    ...
    function board_setup_canvas(canvas){
      ...
      if(1){
        ...
        if(1){
          ...
          points.push({op:'z'});
          ball = {points,transform,corner_i,pivot_x,pivot_y};
        ...
        if(1){
          ...
          points.push({op:'z'});
          let ball = {points,transform,corner_i,pivot_x,pivot_y};
          ctx.note.balls.push(ball);
        }
      }
    }
      ...
      if(ctx.altKey){
        ...
        ctx.note.balls.forEach((ball) => {
          ...
          if(ctx.isPointInStroke(ctx.posx0,ctx.posy0)){
            ...
            ///DETERMINE hitball corner
            let [a,b,c,d,e,f] = ctx.note.hitball.transform;
            ctx.setTransform(a,b,c,d,e,f);
            ctx.note.hitball.points.forEach((pt,i) => {
              ...
              if(ctx.isPointInPath(path2d, ctx.posx0,ctx.posy0)){
                ctx.note.hitball.corner_i = i;
                ctx.note.hitball.pivot_x = ctx.posx0;
                ctx.note.hitball.pivot_y = ctx.posy0;
              }
            });
          }
        });
      } 
      console.log('hitball=',ctx.note.hitball);
      ball_redraw(ctx,canvas);
    }
    function ball_move(ctx,canvas){
    }
    function ball_drag(ctx,canvas){
      if(ctx.altKey && ctx.note.hitball){
        ctx.note.move_x = ctx.posx2 - ctx.posx1;
        ctx.note.move_y = ctx.posy2 - ctx.posy1;
      }else if(ctx.shiftKey && ctx.note.hitball){
        let [x,y] = ball_getpivotxy(ctx.note.hitball);
