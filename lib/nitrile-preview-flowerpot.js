...
`;
  ...
  to_setup_script(){
    ...
    function xyplane_move(ctx,canvas){
      var x = ctx.posx2;
      var y = ctx.posy2;
//console.log('xyplane_move',ctx.note.x_intercepts.length,x,y);
        ...
        for (var h=left; h <= right; h++) {
          for (var d=0; d < D; ++d) {
            ...
            if(Number.isFinite(x) && Number.isFinite(y)){
              ctx.beginPath();
              if(x==0){
                ctx.arc(x*grid,y*grid,r+2,0,Math.PI*2);
              }else{
            ...
            if(Number.isFinite(x0) && Number.isFinite(y0) && 
              ...
              if((sign0 < 0 && sign > 0) || (sign0 > 0 && sign < 0 ) || (sign == 0) ){
                let diff = (0 - y0)/(y - y0);
                let xi = (x - x0)*diff + x0;  
//console.log('y0=',y0,'y=',y,'x0=',x0,'x=','sign0=',sign0,'sign=',sign,'xi=',xi);
                if(Number.isFinite(xi)){
                  var pathxi = new Path2D();
                  pathxi.arc(xi*grid,0,r+2,0,Math.PI*2);
                  ctx.fill(pathxi);
                  ctx.note.x_intercepts.push({pathxi,xi});
                }
              }
            }
            x0 = x;
