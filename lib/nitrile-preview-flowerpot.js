...
`;
  ...
  write_frame_folder(id,frame,subframes,toc,pages){
    ...
    let butt_contents = this.to_butt_contents(boardid);
    ...
    pages.push(1);
    all.push(`<div class='BOTTOMCONTAINER'>`);
    all.push(`<div class='EDITFORM' > ${butt_contents} </div>`);
    all.push(`<div class='PAGENUM'> / </div>`);
    all.push(`</div>`);
    all.push(`<div class='SIDECONTAINER'><img width='88' height='360' style='content:var(--side${id})' usemap='#map${id}' /> ${tool_contents} </div>`);
    ...
    all.push(`</div>`);
    all.push(`<img class='IMG' id='imgid-1-${boardid}' style='display:none' src='${boardpng}' crossOrigin='Anonymous' />`);
    all.push(`<canvas class='CANVAS CAKE' id='1-${boardid}' data-frameid='${frameid}' width='441' height='320' data-boardpng='${boardpng}' ></canvas>`);
    all.push('</article>');
    all.push('');
    //
    //NOTE: end
  ...
  to_tool_contents(boardid){
    return `\
         <div id='tool-1-${boardid}' class='TOOLMENU' style='display:flex;flex-direction:row;align-items:flex-start;justify-content:flex-start;flex-wrap:wrap;width:85px;height:119px;z-index:10;' >
             <input  class='ONETE'          name='1-${boardid}'  type='text'                      onchange="board_one_te_changed(this.value,this.name)" />

             <button class='ONEOP DOT'      name='1-${boardid}'  data-op='pencil'                 onclick="board_oneopclicked(this.name,'pencil'    ,this)" />
             ...
             <form name='1-${boardid}' id='form-1-${boardid}' action='/echo' method='post' style='display:none' onsubmit='board_onsubmitclicked(this.name,this)' >
               ...
               <textarea style='display:none' id='png-1-${boardid}' name='png'></textarea>
               <textarea style='display:none' id='url-1-${boardid}' name='url'></textarea>
            </form>
       `;
  }
  ////////////////////////////////////////////////////////////////////////
  ...
  to_setup_script(){
    ...
    all_smears = {};
    all_colors = ['SlateGray', 'RoyalBlue', 'OliveDrab', 'DarkOrchid', 'Turquoise', 'DarkOrange', 'Red'];
    all_strokes = [1,1.6,3.0];
    all_canvas = null;
    all_op_icons = {'pencil':'&#x270E;','poly':'&#x269E;','square':'&#x25B1;','triangle':'&#x25B3;','ellipse':'&#x2B2D;','eraser':'&#x2B1A;','smudge':'&#x25CD;','select':'&#x26F6;','cplane':'&#x1D456;','ball':'&#x269D;','xyplane':'&#x1D465;&#x1D466;','hand':'&#x270B;'};
    ...
    function setup(){
      ...
      for(var i=0; i < select.length; i++){
        ...
        if(1){
          ...
          if(boardpng){
            var img = document.getElementById("imgid-"+canvas.id); 
...
console.log(buttonid,button);
    ...
    function board_oncakeclicked(name,e){
      var canvas = document.getElementById(name);
      var butt = document.getElementById("op-"+name);
...
console.log('board_onsubmitclicked',name,e);
      ...
      var boardpng = canvas.getAttribute('data-boardpng');
      if(boardpng){
        var e_png  = document.getElementById("png-"+name);
        var e_url  = document.getElementById("url-"+name);
    ...
    function board_leave(ctx,canvas){
      ...
      || o.myop=='ellipse' ){ //board_leave
        if(o.mysquare.face){
          ...
          o.mysquare.face = null;
          board_addpolypt_square(o.myop,o.mysquare, o.mypoly,ctx,canvas);
        }
        if(o.mysquare.s){
          ctx.clearRect(0,0, canvas.width,canvas.height);
    ...
    function board_addpolypt_square(myop,mysquare, mypoly, ctx,canvas){
      switch(myop){
        case 'square': {
          board_addpolypt_xy([mysquare.points[0][0]+mysquare.x,mysquare.points[0][1]+mysquare.y], mypoly, ctx,canvas);
          board_addpolypt_xy([mysquare.points[1][0]+mysquare.x,mysquare.points[1][1]+mysquare.y], mypoly, ctx,canvas);
          board_addpolypt_xy([mysquare.points[2][0]+mysquare.x,mysquare.points[2][1]+mysquare.y], mypoly, ctx,canvas);
          board_addpolypt_xy([mysquare.points[3][0]+mysquare.x,mysquare.points[3][1]+mysquare.y], mypoly, ctx,canvas);
          break;
        }
        case 'triangle': {
          board_addpolypt_xy([mysquare.points[0][0]+mysquare.x,mysquare.points[0][1]+mysquare.y], mypoly, ctx,canvas);
    ...
    function board_addpolypt_xy(pt, mypoly, ctx,canvas){
      ///check for duplicates
      var dup_flag = 0;
      var [x,y] = pt;
      for(let i=0; i < mypoly.polypts.length; ++i){
        if(mypoly.polypts[i].x == x && mypoly.polypts[i].y == y){
          dup_flag = 1;
          break;
        }
      } 
      if(!dup_flag){
        var mypt = {x,y};
        mypoly.polypts.push(mypt);
      }
    }
    function board_findpolypt(mypoly,pt, ctx,canvas){
      var o = ctx.cake;          
      var [sx,sy] = pt;
      var dist = Infinity;
      var tx = 0;
      ...
      var n = 0;
      for(let i=0; i < mypoly.polypts.length; ++i){
        let x = mypoly.polypts[i].x;
        let y = mypoly.polypts[i].y;
    ...
    function board_start(ctx,canvas){
      ...
      if(ctx.cake.myop=='poly'){ //board_start
        ...
        let y2 = ctx.posy0;
        if(ctx.altKey){
          [x1,y1] = board_findpolypt(o.mypoly,[x1,y1], ctx,canvas);
          [x2,y2] = board_findpolypt(o.mypoly,[x2,y2], ctx,canvas);
        }
        o.mypoly.x1 = x1;
        o.mypoly.y1 = y1;
        o.mypoly.x2 = x2;
    ...
    function board_drag(ctx,canvas){
      ...
      if(ctx.cake.myop=='pencil'){ //board_drag pencil
        if(ctx.altKey){
          ...
          if(ctx.shiftKey){
            if(Math.abs(dx)<=2 && Math.abs(dy)<=2){
              //nothing do not draw a line yet
              x1 = x0; 
              y1 = y0;
              x2 = x0; 
              y2 = y0;
      ...
      || ctx.cake.myop=='ellipse' ){ //board_drag ellipse
        let {isnewlycorner,isnewsketch,istomove,isresize,corner_i,w,h} = o.mysquare;
        let {mousedragcount} = ctx;
        if(isnewsketch && mousedragcount < 10){
        }else if(isnewlycorner && o.mysquare.corner_i==0){
          let dx = ctx.posx2 - ctx.posx1;
          let dy = ctx.posy2 - ctx.posy1;
          if(ctx.shiftKey){
    ...
    function board_end(ctx,canvas){
      ...
      if(ctx.cake.myop=='pencil'){ //board_end
        let {altKey,shiftKey} = o.mypencil;
        if(altKey && shiftKey){
          ctx.clearRect(0,0, canvas.width,canvas.height);
        }
      }
      if(ctx.cake.myop=='poly'){ //board_end
        if(ctx.mouseisdragged){
          ...
          let x2 = o.mypoly.x2;
          let y2 = o.mypoly.y2;
          board_addpolypt_xy([x1,y1], o.mypoly, ctx,canvas);
          board_addpolypt_xy([x1,y1], o.mypoly, ctx,canvas);
        }
      }
