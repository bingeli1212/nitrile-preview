...
`;
  ...
  to_tool_contents(boardid){
    return `\
         <div id='tool-1-${boardid}' class='TOOLMENU' style='display:flex;flex-direction:row;align-items:flex-start;justify-content:flex-start;flex-wrap:wrap;width:85px;height:136px;z-index:10;' >
             ...
             <button class='ONEAT'          name='1-${boardid}'  data-at='rotcl'                  onclick="board_one_atclicked(this.name,this)" />        
             <button class='ONEAT'          name='1-${boardid}'  data-at='rotcr'                  onclick="board_one_atclicked(this.name,this)" />        
  ...
  to_setup_script(){
    ...
    all_st_icons = ['&#x2758;','&#x2759;','&#x275A;'];
    all_at_icons = {'clear':'&#x2BBD;','cropp':'&#x26CB;','fopen':'&#x1F4C2;','dload':'&#x2728;','zoomo':'&#x2315;-','zoomi':'&#x2315;+','rotcl':'&#x2B6F;','rotcr':'&#x2B6E;','acton':'&#x2318;','cutbd':'&#x2702;','paste':'&#x26E8;','rsize':'&#x26DE;','echos':'&#x2BB8;'};
...
console.log('board_onsubmitclicked',name,e);
    ...
    function board_redraw(ctx,canvas){
      ...
      if(ctx.cake.myop=='ball'){ //board_redraw ball
        ...
        var corner_size = 4;
        if(o.myball.hitball){
          o.myball.hitball.shape = o.myaction%4;//pick up the latest shape from the action            
          o.myball.hitball.rotation += o.rotation; o.rotation = 0;
        }
        o.myball.balls.forEach((ball,j) => {
          ...
          ctx.translate(center_p.x,center_p.y);
      ...
      if(ctx.cake.myop=='ball'){ //board_start ball
        ...
        if(altKey && shiftKey){
          if(o.myball.hitball){
            ...
            var m = Math.PI/180;
            if(1){                  
              let ball = {shape:0,corner_i,color,x,y,w:30,h:30,skewX:0,skewY:0,rotation:0};
              o.myball.balls.push(ball);
    ...
    function board_drag(ctx,canvas){
      ...
      if(ctx.cake.myop=='ball'){ //board_drag ball
        let ball = o.myball.hitball;
        if(ball){
          if((ball.corner_i == 0) && (ball.corner_t == 2)){
            let w = ball.w;
            let h = ball.h;
            //skewX
            let dx = ctx.posx2 - ctx.posx1;
          ...
          }else if((ball.corner_i == 0) && (ball.corner_t == 1)){
            //rotate
            let [x,y] = ball_getpivotxy(ball);//rotate
          ...
          }else if((ball.corner_i > 0) && (ball.corner_t == 2)){
            //scaleY
            let dx = ctx.posx2 - ctx.posx1;
            if(dx >= 1){ //expand
