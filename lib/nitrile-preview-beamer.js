   ...
   let cdigits = [ '\\ding{202}',
    ...
    if(frame.solutions.length){
      ...
      frame.solutions.forEach((o,i,arr) => {
        if(o.ischoices){
          all.push('\\bgroup');
          all.push(`${o.title} \\rule{20pt}{0.4pt}`);
          all.push('\\egroup')
        }
      })
      all.push(`\\end{frame}`);
      ...
      frame.solutions.forEach((o,i,arr) => {     
        ...
        all.push('');
        if(o.ischoices){
          all.push(`\\textsl{${o.title}} ${o.text}`);
          all.push('');
          ///search the frame.contents for a block that its fenceid is set to 'choices'
          for(let b of frame.contents){
            if(b.sig=='TEXT'){
              let {fenceid,ss,style} = this.ss_to_fenced_ss(b.body);
              if(fenceid=='choices'){
                style = {...style,selected:o.raw};
                let text = this.para_to_choices(ss,style);
                console.log(text);
                all.push(text);
              }
            }
