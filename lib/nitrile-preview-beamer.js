...
`;
  ...
  write_frame(frame) {
    ...
    frame.topics.forEach((o,i,arr) => {     
      all.push(`\\begin{frame}[t]`);
      all.push(`\\frametitle{${++this.frameid} ~ ${title} ${endash} \\textsl{${o.title}} }`);
      all.push('');
    ...
    frame.solutions.forEach((o,i,arr) => {     
      all.push(`\\begin{frame}[t]`);
      all.push(`\\frametitle{${++this.frameid} ~ ${title} ${endash} \\underline{${o.title}} }`);
      all.push('');
      if(o.text){
        all.push(`${o.text}`);
      }
