...
`;
  ...
  write_frame(frame, icon) {
    ...
    let checkmark = this.uncode('&checkmark;');
    let box = this.uncode('&BallotBox;');
    let cbox = this.uncode('&BallotBoxWithCheck;');
    let chevrondown = '\\faChevronDown';
    let chevronright = '\\faChevronRight';
    let edit = this.uncode('&Edit;');
    let dtri = '\\MoveDown';
    ...
    if(block && block.sig=='FRNT'){
      ...
      for(let t of data){
        ...
        if(key=='title'){
          ...
          titlelines.push(`\\subtitle{${this.uncode(val)}}`);
        }
        else if(key=='author'){
          titlelines.push(`\\author{${this.uncode(val)}}`);
        }
        else if(key=='institute'){
          titlelines.push(`\\institute{${this.uncode(val)}}`);
        }
      }
    }
    return titlelines;
  ...
  para_to_listing(ss,style){
    ...
    let opts = [];
    opts.push(`caption={${this.uncode(caption)||'~'}}`);///when caption is empty the Listing: won't show
    if(label){
