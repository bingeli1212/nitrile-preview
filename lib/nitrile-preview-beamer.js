    ...
    if(sig=='HDGS'){
      ...
      if(d.length > 0){
        ...
        if(w.length){
          ...
          w.forEach((o,i) => {
            all.push(`\\item[\\ding{51}] \\textsl{${o.title}}`);
          });
      ...
      w.forEach((o,i) => {
        all.push(`\\begin{frame}[t]`);
        all.push(`\\frametitle{${my_id} ${my_title} - \\textsl{${o.title}}}`);
      ...
      w.forEach((o,i) => {     
        ...
        w.forEach((p,j) => {
          ...
          if(i==j){
            all.push(`\\item[${dtri}] \\textbf{\\textsl{${o.title}}} ${o.text.length?'-':''} ${o.text}`);
            all.push('');
            all.push(o.contents.map(x=>x.latex).join('\n'));
            all.push('');
          }else{
            all.push(`\\item ${p.title}`);
          }
        });
        all.push(`\\end{itemize}`);
        all.push(`\\end{frame}`);
