  ...
  write_frame(frame) {
    ...
    if(frame.solutions.length){
      ...
      frame.solutions.forEach((o,i,arr) => {     
        ...
        all.push('');
        if(1){
          all.push(`${cbox}~~\\textsl{${o.title}} ~ ${o.text}`);
        }
      ...
      frame.topics.forEach((o,i,arr) => {
        ...
        if(i==0){
          all.push('');
          all.push(`\\begin{list}{}{\\setlength\\itemsep{0pt}\\setlength\\parsep{0pt}\\setlength\\leftmargin{12pt}}`);
        }
      ...
      frame.topics.forEach((o,i,arr) => {     
        ...
        all.push(`\\frametitle{${++this.frameid} ~ ${title} }`);
        all.push('');
        all.push(`${cdigits[i]}~~\\textsl{${o.title}}`);
        all.push('');
        o.contents.forEach((x,i) => {
          all.push('');
          all.push(x.latex.trim());
