...
`;
  ...
  to_beamer_frame(top) {
    ...
    if(d.length==0){
      ...
      all.push(`\\frametitle{${my_id} ${this.unmask(my.title)}}`);    
      ...
      if(w.length){
        all.push('\\begin{itemize}');
        w.forEach((o,i) => {
          all.push(`\\item[$\\Square$] {${this.unmask(o.title)}}`);
        });
        all.push('\\end{itemize}')
      }
      all.push(`\\end{frame}`);
      ...
      w.forEach((o,i) => {
        all.push(`\\begin{frame}[t]`);
        all.push(`\\frametitle{${my_id} ${this.unmask(my.title)} - ${this.unmask(o.title)}}`);
        all.push(d.join('\n'));
        all.push(`\\begin{itemize}`);
        let sym = '$\\CheckedBox$';
        if(o.contents.length && o.contents[0].sig=='TEXT'){
          all.push(`\\item[${sym}] ${this.unmask(o.title)} - ${this.unmask(this.join_para(o.contents[0].body))}`);
