...
class NitrilePreviewBeamer extends NitrilePreviewLatex {
  ...
  write_frame(frame) {
    ...
    frame.solutions.forEach((o,i,arr) => {
      ...
      all.push(`\\item[${the_bull}] \\textit{${this.uncode(o.style,o.title)}} `);
      if(i+1==arr.length){
        all.push('\\end{list}')
      }
    })
    ...
    frame.solutions.forEach((o,i,arr) => {     
      ...
      all.push(`\\begin{frame}${this.frameopt}`);
      all.push(`\\frametitle{${icon} ${this.uncode(frame.style,frame.title)}}`);
      all.push(`\\framesubtitle{\\textit{${this.uncode(o.style,o.title)}} ~ \\bfseries ${this.uncode(o.style,this.join_para(o.body)).trim()}}`);
