...
class NitrilePreviewBeamer extends NitrilePreviewLatex {
  ...
  to_beamer_document() {
    ...
    var p_fonts_layout = this.to_fonts_layout(p_latex_program);
...
`;
  ...
  to_beamer_frame(top) {
    ...
    if(1){
      ///set it so that the second and forward TEXT block will have a vspace:6 set
      let textblocks = d.filter(x => x.style && x.style.sig=='TEXT');
    ...
    if(sigs[0]=='HDGS'){
      ...
      if(d.length > 0){
        ...
        all.push(`\\frametitle{${++this.my_frameid}. ${my_title} }`);
        d.forEach((x,i) => {
          all.push('');
      ...
      w.forEach((o,i) => {
        all.push(`\\begin{frame}[t]`);
        all.push(`\\frametitle{${++this.my_frameid}. \\textsl{${my_title}}}`);
        all.push('');
        all.push(`\\begin{enumerate}`)
        all.push(`\\item[${checkmark}] ${o.title}`);
        all.push('');
