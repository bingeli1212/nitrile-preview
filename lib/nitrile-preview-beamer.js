  ...
  write_frame(frame) {
    ...
    frame.solutions.forEach((o,i,arr) => {
      if((v=re_choice.exec(o.title))!==null){
        let choices = this.to_choices(o.style,o.body);
        if(v[1]){
          all.push(`${this.icon_solution} \\textit{${this.uncode(o.style,v[1])}}\\\\`);
        }
        choices.forEach((s) => {
          all.push(`${s}\\\\`);
        });
      }else{
        all.push(`${this.icon_solution} \\textit{${this.uncode(o.style,o.title)}} `);
      }
    })
    ...
    frame.solutions.forEach((o,i,arr) => {     
      ...
      all.push(`\\begin{frame}${this.frameopt}`);
      all.push(`\\frametitle{${icon} ${this.uncode(frame.style,frame.title)}}`);
      all.push(`\\framesubtitle{\\bfseries`);
      if((v=re_choice.exec(o.title))!==null){
        let choices = this.to_choices(o.style,o.body,v[2]);
        if(v[1]){
          all.push(`\\textit{${this.uncode(o.style,v[1])}} \\\\`);
        }
        choices.forEach((s) => {
          all.push(`${s} \\\\`);
        });
      }else{
        all.push(`\\textit{${this.uncode(o.style,o.title)}} ~ \\bfseries ${this.untext(o.style,o.body).trim()}`);
      }
      let s = all.pop();
      if(s.endsWith('\\\\')){
        s = s.slice(0,-2)
