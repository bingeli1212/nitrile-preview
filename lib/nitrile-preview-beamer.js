   ...
   let cdigits = [ '\\ding{202}',
    ...
    if(frame.solutions.length){
      ...
      frame.solutions.forEach((o,i,arr) => {
        ...
        all.push(`\\item[${box}] \\textsl{${o.title}} ~ \\rule{40pt}{0.4pt}`);
        if(i+1==arr.length){
          all.push('\\end{list}')
        }
      })
      ...
      frame.solutions.forEach((o,i,arr) => {     
        ...
        if(o.ischeckbox){
          ...
          all.push(`\\item[${cbox}] \\textsl{${o.title}} ~ \\underline{${o.selection}}`);
          all.push('');
          ///search the frame.contents for a block that its fenceid is set to 'choices'
          let b = frame.contents[frame.contents.length-1];
          if(b.sig=='TEXT'){
            let style = {...b.style, selection:o.selection};
          ...
          all.push('\\begin{list}{}{\\setlength\\itemsep{0pt}}')
          if(o.text){
            all.push(`\\item[${cbox}] \\textsl{${o.title}} ~ \\underline{${o.text}}`);
          }else{
            all.push(`\\item[${cbox}] \\textsl{${o.title}} \\rule{40pt}{0.4pt}`);
          }
