...
class NitrilePreviewBeamer extends NitrilePreviewLatex {
  ...
  to_beamer_document() {
    ...
    // this.keypoint_icon = this.conf_to_string('beamer.keypoint-icon','{\\faLeaf}');
    // this.exercise_icon = this.conf_to_string('beamer.exercise-icon','{\\faPencil}');
    //required setups
...
`;
  ...
  write_contents_frame(topframes){
    ...
    var text = outs.join('\n')
    ///ASSEMBLE
    let frameopt = this.conf_to_string('beamer.frameopt');
    outs = [];
    if(frameopt){
