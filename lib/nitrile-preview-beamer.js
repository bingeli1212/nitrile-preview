    ...
    if(sigs[0]=='HDGS'){
      ...
      if(d.length > 0){
        ...
        if(w.length){
          ...
          w.forEach((o,i) => {
            all.push(`\\item[${checkmark}] \\textsl{${o.title}}`);
          });
      ...
      w.forEach((o,i) => {
        all.push(`\\begin{frame}[t]`);
        all.push(`\\frametitle{${my_id} ${my_title} ${endash} \\textsl{${o.title}}}`);
        all.push('');
      ...
      if(w.length){
        ...
        w.forEach((o,i) => {
          let sym = '$\\Square$';
          all.push(`\\item \\underline{${o.title}}`);
        });
      ...
      w.forEach((o,i) => {     
        ...
        all.push(`\\begin{itemize}`);
        if(o.text){
          all.push(`\\item[${dtri}] \\underline{${o.title}:} ~ {${o.text}}`);
        }else{
          all.push(`\\item[${dtri}] \\underline{${o.title}}`);
        }
        all.push('');
