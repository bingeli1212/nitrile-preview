  ...
  to_beamer() {
    ...
    if(main_frames.length){
      ...
      main_frames.forEach((frame,i,arr) => {
        if(i==0){
          ...
          all.push(`\\frametitle{Topics}`);    
          all.push(`\\begin{itemize}`);    
        }
        all.push(`\\item[${asterisk}] ${frame.title}`);
        if(i+1==arr.length){
    ...
    if(ex_frames.length){
      ...
      ex_frames.forEach((frame,i,arr) => {
        ...
        all.push(`\\item[${pencil}] ${frame.title}`);
        if(i+1==arr.length){
          all.push(`\\end{itemize}`);    
          all.push(`\\end{frame}`);
        }
   ...
   let cdigits = [ '\\ding{202}',
    ...
    if(frame.solutions.length){
      ...
      frame.solutions.forEach((o,i,arr) => {
        ...
        all.push(`\\item \\textsl{${o.title}} ${chevrondown}`);
        if(i+1==arr.length){
          all.push('\\end{list}')
        }
      })
      ...
      frame.solutions.forEach((o,i,arr) => {     
        ...
        if(o.text){
          all.push(`\\item \\textsl{${o.title}} : \\underline{${o.text.trim()}}`);
        }else{
          all.push(`\\item \\textsl{${o.title}} : `);
        }
