...
class NitrilePreviewNicreamer {
  ...
  async run(){
    ...
    if(!fname){
        ...
        const exe = require('child_process').spawn('creamer.bat', [texfile]); 
        exe.stdout.on('data',(out) => process.stderr.write(out)); 
        exe.on('close',(code) => { console.log('creamer.bat finished') }); 
    }else{
        throw "File does not end with .md"
    }
  }
}
var server = new NitrilePreviewNicreamer();
server.run(()=>{});
