...
class NitrilePreviewCreamer extends NitrilePreviewContex {
  constructor(parser) {
    ...
    this.contex_details_ul_symbol = '\\symbol[martinvogel 2][Forward]';
  }
  to_document() {
...
`;
  ...
  write_one_frame(id,subid,frame,issub,topframe) {
    ...
    if(issub){
      ...
      all.push(`\\startsubsection[title={\\small \\underbar{${id}.${subid} ${this.uncode(frame.style,frame.title)}}},reference={${id}.${subid}},bookmark={${id}.${subid} ${frame.title}}]`); 
      all.push(`\\godown[-0.05cm]`)
    }else{
      all.push(`\\startsection[title={${id}. ${this.uncode(topframe.style,topframe.title)}},reference=${id},bookmark={${id} ${topframe.title}}]`); 
      all.push(`\\godown[-0.05cm]`)
      all.push(`\\startsubsection[title={\\small \\ }]`); 
      all.push(`\\godown[-0.05cm]`)
    }
    ///
    ...
    frame.solutions.forEach((o,i,arr) => {     
      ...
      if(o.choice){
        ...
        all.push(`\\startsection[title={}]`);
        all.push(`\\godown[-0.05cm]`)
        all.push(`\\startsubsection[title={\\underbar{${title}} \\small ${text}},reference=,bookmark={${o.title}}]`);//empty 
        all.push(`\\godown[-0.05cm]`)
      }else{
        let title = this.uncode(o.style,o.title).trim();
  ...
  to_titlepage(){
    ...
    all.push(`{\\tfb \\color[titleblue]{${this.smooth(this.parser.style,title)}}}\\\\`);
    all.push(`\\godown[+0.05cm]`);
    all.push(`${this.smooth(this.parser.style,subtitle)}\\\\`);
    all.push(`\\godown[+0.05cm]`);
    all.push(`${this.smooth(this.parser.style,institute)}\\\\`);
    all.push(`\\godown[+0.05cm]`);
    all.push(`${this.smooth(this.parser.style,author)}\\\\`);
    all.push(`\\godown[+0.05cm]`);
    all.push(`${this.smooth(this.parser.style,date)}`);
    all.push(`\\stopalignment`);
    all.push(`\\vfill`);
