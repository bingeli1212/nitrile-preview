...
`;
  ...
  write_frame_folder(id,frame,subframes){
    ...
    subframes.forEach((subframe,i,arr) => {
      ...
      let subid = `${i+1}`;
      all.push(`\\sym {${subid}.} \\underbar{${title}}`);
    });  
    all.push(`\\stopitemize`);
    all.push(`\\stopsection`) 
    var text = all.join('\n');
...
*/
  ...
  to_titlepage(){
    ...
    all.push(`\\tfc \\color[titleblue]{${this.smooth(this.parser.style,title)}}\\\\`);
    all.push(`\\tfa ${this.smooth(this.parser.style,subtitle)}\\\\`);
    all.push(`\\tfa ${this.smooth(this.parser.style,institute)}\\\\`);
    all.push(`\\tfa ${this.smooth(this.parser.style,author)}\\\\`);
    all.push(`\\tfa ${this.smooth(this.parser.style,date)}`);
    all.push(`\\stopalignment`);
    all.push(`\\vfill`);
