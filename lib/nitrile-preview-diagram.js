...
class NitrilePreviewDiagram extends NitrilePreviewBase {
  ...
  initialize_env(line){
    var env = {};
    var ss = this.string_to_array(line);
    for(let s of ss){
      var [key,val] = s.split('/');
      key = key||'';
      val = val||'';
      env[key] = this.read_scalar(val,env);
    }
    return env;
  }
  ...
  exec_body(style,body,env){
    ...
    while (lines.length) {
      ...
      if((v=this.re_set_command.exec(line))!==null){
        ...
        let val = v[5];
        if(this.re_is_symbolname.test(key)){
          if(que && this.config.hasOwnProperty(key)){
            //dont do anything
          }else if(eq==':'){
            let s = val;
            this.config[key] = s;
            ...
            let [numbers] = this.read_array(val,env);//returns an array 'Complex' number
            this.config[key] = this.array_to_string(numbers);
          }else{
            let a = this.read_scalar(val,env);//returns a Complex or a string
            if(a instanceof Complex){
              this.config[key] = a.re;
          ...
          this.do_comment(`*** set ${key} = ${this.config[key]}`);          
        }else{
          this.do_comment(`*** [ERROR] '${key}' is not a valid symbol name`);  
        }
        continue;
