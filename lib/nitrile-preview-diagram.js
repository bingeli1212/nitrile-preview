    ...
    while (lines.length) {
      ...
      if((v=this.re_var_command.exec(line))!==null){
        ...
        if(s.startsWith('[') && s.endsWith(']')){
          ...
          env[name] = numbers;
          this.do_comment(`*** ${name} = [ ${this.numbers_to_string(numbers,precision)} ]`);
        }else{
          // var a = 12.3
          let [numbers] = this.read_complex_numbers(s);//returns an array 'Complex' number
          env[name] = numbers[0];
          this.do_comment(`*** ${name} = ${this.numbers_to_string(numbers.slice(0,1),precision)}`);
        }
        continue;
      }  
      //
      ...
      if((v=this.re_show_command.exec(line))!==null) {
        ...
        if(s.startsWith('@')){
          ...
          if(m && Array.isArray(m)){
            let arr = m;
            let str = this.numbers_to_string(arr,precision);
            console.log(str);
          }else if(m){
            let arr = [m];
            let str = this.numbers_to_string(arr,precision);
