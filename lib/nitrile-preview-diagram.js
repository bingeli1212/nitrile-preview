...
class NitrilePreviewDiagram extends NitrilePreviewBase {
  ...
  offset_and_scale_coords(coords,offsetx,offsety,xscale,yscale){
    var newcoords = [];
    for(let i=0; i < coords.length; ++i){
      let pt = coords[i];
      let newpt = this.offset_and_scale_pt(pt,offsetx,offsety,xscale,yscale);
      newcoords.push(newpt);
    }
    return newcoords;
  }

  offset_and_scale_pt(pt,offsetx,offsety,xscale,yscale){
    var newpt = [];
