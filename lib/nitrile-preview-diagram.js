  ...
  exec_body(style,body,env){
    ...
    while (lines.length) {
      ...
      if((v=this.re_var_command.exec(line))!==null){
        ...
        if(index.length){
          // var a[] = 12.3 12.4 12.5
          let [numbers] = this.read_complex_numbers(s,env);//returns an array 'Complex' number
          env[name] = numbers;
          this.do_comment(`***var ${name}[] = ${this.numbers_to_string(numbers)}`);
        }else{
          // var a = 12.3
          let [numbers] = this.read_complex_numbers(s,env);//returns an array 'Complex' number
          env[name] = numbers[0];
          this.do_comment(`***var ${name} = ${this.numbers_to_string(numbers.slice(0,1))}`);
        }
        continue;
      }  
      //
