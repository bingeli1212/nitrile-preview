...
class NitrilePreviewDiagram extends NitrilePreviewBase {
  ...
  constructor(translator) {
    ...
    */
    ...
    this.re_aux   = /^(x:|y:|X:|Y:|sx:|sy:|s:|up:|down:|left:|right:|at:|lastpt:|hints:)(\S*)\s*(.*)$/;
    this.re_relative = /^\[(.*?)\]\s*(.*)$/;
    ...
    while (line.length) {
      ...
      if ((v = this.re_aux.exec(line))!==null) {
        ...
        if (key === 'hints:'){
          ...
          else if(key=='X:')      offset_x=this.viewport_width-num;
          else if(key=='Y:')      offset_y=this.viewport_height-num;
