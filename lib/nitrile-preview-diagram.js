...
class NitrilePreviewDiagram extends NitrilePreviewBase {
  ...
  constructor(translator) {
    ...
    */
    ...
    this.re_number_is_filter             = /^<(.*)>$/;
    this.re_number_is_spread             = /^([^!]+)!([^!]+)!([^!]+)$/;
    this.re_number_is_range_expr2        = /^([^~]+)~([^~]+)$/;
    this.re_number_is_range_expr3        = /^([^~]+)~([^~]+)~([^~]+)$/;
    // path construction
    this.re_dashdot = /^(~|\.\.)\s*(.*)$/;
    this.re_coord = /^\((.*?)\)\s*(.*)$/;
  ...
  read_numbers(line) {
    ...
    while(line.length) {
      ...
      if((v=this.re_number_is_filter.exec(expr))!==null){
        let ss = v[1].split(':');
        if(ss.length==2){
          filters.push(ss);
        }
      ...
      if((v=this.re_number_is_range_expr2.exec(expr))!==null){
        /// '1~10'
      ...
      if((v=this.re_number_is_range_expr3.exec(expr))!==null){
        /// '1~3~10'
        let [a1]   = this.expr.extract_next_expr(v[1],this.my_var_g);//returns a 'Complex'
        let [a2] = this.expr.extract_next_expr(v[2],this.my_var_g);//If empty it returns (0,0*I)
      ...
      if((v=this.re_number_is_spread.exec(expr))!==null){
        /// '1!3!10'
        let [a1] = this.expr.extract_next_expr(v[1],this.my_var_g);//returns a 'Complex'
        let [a2] = this.expr.extract_next_expr(v[2],this.my_var_g);//If empty it returns (0,0*I)
  ...
  copy_numbers(o,all,filters){
    if(filters.length){
      all.forEach((a) => {
        var [a,keep] = this.apply_number_filters(a,filters);
        if(keep){
          o.push(a);
        }
      })
  ...
  apply_number_filters(a,filters){
    /// return an array of two, the first is the number itself, and the second one is a flag, where 1 means
    /// that the number is kept and 0 if not
    var keep = 1;
    for(let j=filters.length; keep && j > 0; j--){
      let filter = filters[j-1];
      let key = filter[0];
      let fn = filter[1];
      if(key=='map'){
        a = this.expr.exec_complex_func(fn,[a],this.my_var_g,0);
      }else if(key=='keep'){
        let d = this.expr.exec_complex_func(fn,[a],this.my_var_g,0);
        keep = (d >= 1) ? 1 : 0;
      }
    }
    return [a,keep];
  ...
  do_box(opts,g,txt,coords){
    ...
    var showid = this.g_to_showid_int(g);
    var tspan = this.g_to_tspan_int(g);
  ...
  g_to_tspan_int(g){
    g = this.update_style_with_group(g);
    return this.get_int_prop(g,'tspan',0);
  }
  g_to_shift_float(g){
    g = this.update_style_with_group(g);
