...
class NitrilePreviewDiagram extends NitrilePreviewBase {
          ...
          if (val.length == 2) {
            ...
            if (Number.isFinite(dx) && Number.isFinite(dy)) {
              ...
              if((Math.sign(dx)>0 && Math.sign(dy)>0)||
                ...
                x1 = this.lastpt_x + dx*rat;
                y1 = this.lastpt_y;
                x2 = x;
                y2 = y - dy*rat;
              }else{
                x1 = this.lastpt_x;
                y1 = this.lastpt_y + dy*rat;
                x2 = x - dx*rat;
                y2 = y;
              }
  ...
  read_scalar(line,env) {
    ...
    if((v=re_format.exec(line))!==null){
      ...
      return s2;
    }else if((v=re_atsign.exec(line))!==null){
      let s2 = v[1];
      ...
      let symbol = line.slice(1);
      let p = this.get_path_from_symbol(symbol);
      let s2 = this.coords_to_string(p);
      return s2;
    }else{
