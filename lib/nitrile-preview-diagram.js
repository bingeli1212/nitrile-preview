  ...
  exec_line(line,env) {
    ...
    if ((v = this.re_path_command.exec(line)) !== null) {
      ...
      if((v=re.exec(symbol))!==null){
        ...
        this.commands.push(this.do_comment(`***ERR: ${symbol} is not a valid variable`));
      }
      return false;
    }
    ///
    ...
    if ((v = this.re_set_command.exec(line)) !== null) {
      ...
      if (key === 'refxy') {
        ...
        this.debug = this.assert_int(val, this.def_debug);
      }
      return false;
    }
    ///NOTE: fn
  ...
  */
  ...
  do_label(opts,g,txt,ts,coords){
    ...
    for (var i = 0; i < coords.length; ++i){
      ...
      var label = this.replace_label(label,g);
  ...
  replace_label(label,g){
    var arr = this.string_to_array(g.replace);
    console.log('james','arr',arr);
