...
class NitrilePreviewDiagram extends NitrilePreviewBase {
  ...
  exec_body(style,body,env){
    ...
    while (lines.length) {
      ...
      if((v=this.re_for_command.exec(line)) !== null) {
        ...
        var loopbody = this.extract_loop_body(style,lines);
        var max_n = 0;
      ...
      //
      this.do_line(style,line);
    }
  }
  extract_loop_body(style,lines){
    var body=[];
    var re_leading = /^(\s+)(.*)$/;
    var n = 0;
    var v;
  ...
  exec_action_command(style,line) {
  ...
  do_record_command(style,symbol,line){
  ...
  do_playback_command(style,symbol){
    if(this.my_record_map.has(symbol)){
      var ss = this.my_record_map.get(symbol);
      for(var s of ss){
        this.do_line(style,s);
      }
    }
  }
  ...
  do_line(style,s) {
    var s = this.exec_action_command(style,s);
    if(s){
      this.commands.push(s);
    }
  }
