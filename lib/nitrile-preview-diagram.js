...
class NitrilePreviewDiagram extends NitrilePreviewBase {
  ...
  exec_action_command(style,line,env) {
    ...
    if (cmd == 'subtitle'){
      ...
      let x = 0;
      let y = 0;
      o.push(this.do_subtitle(opt,x,y,w,h,g,txts));///own method
    }
    ///
    ///NOTE: 
    ///
  ...
  do_header(opt,x,y,w,h,g,txts){
    var o = [];
    var txt = this.fetch_label_at(txts,0,g);
    o.push(this.p_header(opt,x,y,w,h,txt,g));
    return o.join('\n');
  }
  do_footer(opt,x,y,w,h,g,txts){
    var o = [];
    var txt = this.fetch_label_at(txts,0,g);
    o.push(this.p_footer(opt,x,y,w,h,txt,g));
    return o.join('\n');
  }
  do_subtitle(opt,x,y,w,h,g,txts){
    var o = [];
    var txt = this.fetch_label_at(txts,0,g);
    o.push(this.p_subtitle(opt,x,y,w,h,txt,g));
    return o.join('\n');
  }
  do_image(opt,x,y,w,h,g,txts){
    ...
    let imgsrc = this.translator.choose_image_file(txts);
  ...
  do_drawslopedlabel(opts,g,txts,coords){
    ...
    for (var j = 0; j < linesegs.length; ++j){
      ...
      x2 = x2_;
      y2 = y2_;
      o.push(this.p_slopedlabel(x1_,y1_,x2_,y2_,label,ta,g));
      if(mar){
        ...
        if(xcol){
          ...
          xcols.forEach((p,i) => {
            ...
            labels.forEach((label,j) => {
              let px = x + (p[0]+p[1])/2;
              let py = y + h - (j+0.5)*dy;    
              o.push(this.p_label(px,py,label,'ctr',g));
            });
          });
        }
      }
