  ...
  read_numbers(line) {
    ...
    while(line.length) {
      if((v=this.re_numbers_range.exec(line))!==null){
        ...
        let pp = range.split(':');
        if(pp.length==3){
          let [a1] = this.expr.extract_next_expr(pp[0],this.my_var_g);//returns a 'Complex'
          let [a2] = this.expr.extract_next_expr(pp[1],this.my_var_g);//If empty it returns (0,0*I)
          let [a3] = this.expr.extract_next_expr(pp[2],this.my_var_g);
          let all = [];
