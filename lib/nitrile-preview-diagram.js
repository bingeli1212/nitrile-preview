...
class NitrilePreviewDiagram extends NitrilePreviewBase {
  ...
  constructor(translator) {
    ...
    */
    ...
    this.re_numbers_nonblanks             = /^(\S+)\s*(.*)$/;
    this.re_numbers_is_fn                 = /^\^fn:(\S+)$/;
    ...
    this.re_coords_style    = /^\s*\{(.*?)\}\s*(.*)$/;
    this.re_coords_label    = /^\s*\"([^"]*)\"\s*(.*)$/;
    //
  ...
  exec_body(body,env){
    ...
    while (lines.length) {
      ...
      if((v=this.re_array_command.exec(line))!==null){
        let name = v[1];
        let line = v[2];
        let [numbers] = this.read_complex_numbers(line);//this returns an array 'Complex' number
      ...
      if((v=this.re_for_command.exec(line)) !== null) {
        ...
        while(line.length){
          if((v=re_for_one.exec(line))!==null) {
            ...
            let name = v[1];
            let range = v[2];
            let [numbers] = this.read_complex_numbers(range);
  ...
  exec_command_line(line) {
    ...
    ///
      ...
      o.push(this.do_cartesian(subcmd,opts,g,txts,numbers));///own method
    }
    ///
  ...
  read_coords(line) {
    ...
    while (line.length) {
      ...
      if ((v = this.re_coords_aux.exec(line))!==null) {
        //right:2, up:2, down:2, left:2, x:2, X:2, y:2, Y:2, s:2, sx:2, sy:2, at:&a, at:center, mark:a, veer:-20 hints:1
        line = v[3];
        let val = v[2];
        let key = v[1];
      ...
      v1 = this.re_coords_point.exec(line);
      v2 = this.re_coords_flow.exec(line);
      if(v1 || v2){
        if(v1){
          v = v1;
          ///(0,0)
      ...
      if ((v = this.re_coords_relative.exec(line)) !== null) {
        ...
        if (fun_name.length) {
          ...
          if (key === 'angledist') {
            ...
            var val = val.map(x => x.trim());
            if (val.length == 3) {
              var abr = parseFloat(val[0]);
              dx =      parseFloat(val[1]);
              dy =      parseFloat(val[2]);
              if (Number.isFinite(abr) && Number.isFinite(dx) && Number.isFinite(dy)) {
                x = this.my_lastpt_x + dx;
                y = this.my_lastpt_y + dy;
                var hints = lasthints;
                lasthints = 0;
                lastjoin = 'line';
                let [pt0,pt1,pt2] = this.abr_to_qbezier(abr, this.my_lastpt_x,this.my_lastpt_y,0, x,y,0);  
                let pt = [x,y,'Q', pt1[0],pt1[1],'','', 0,0,0,0,0, hints];
                coords.push(pt);
                this.my_lastpt_x = x;
                this.my_lastpt_y = y;
  ...
  read_complex_numbers(line) {
    ...
    var fns = [];
    while(line.length) {
      v=this.re_numbers_nonblanks.exec(line); 
      if(v){
        var expr = v[1];
        line = v[2];
      }else{
      ...
      if((v=this.re_numbers_is_fn.exec(expr))!==null){
        let fn = v[1];
        fns.push(fn);
        continue;
      }
  ...
  copy_numbers(o,all,fns){
    all = all.map((a) => {
      a = this.map_number(a,fns);
      return a;
    });
    all.forEach((a) => {
      o.push(a);
  ...
  do_cartesian(subcmd,opts,g,txts,numbers) {
