...
class NitrilePreviewDiagram extends NitrilePreviewBase {
  ...
  exec_action_command(style,line) {
    ...
    ///
    if (cmd == 'text'){
      let [coords,g,txts] = this.read_coords(data);
    ...
    ///
      ...
      let [numbers,g,txts] = this.read_complex_numbers(data);
  ...
  read_complex_numbers(line) {
    ...
    while(line.length) {
      ...
      /// 1:4:10
      /// 0!10!1
      if((v=this.re_numbers_nonblanks.exec(line))!==null){ 
        var expr = v[1];
        line = v[2];
        ...
        if((v=this.re_numbers_is_range_expr2.exec(expr))!==null){
          ...
          this.copy_numbers(numbers,all,fns);
          continue; 
        }
        ...
        if((v=this.re_numbers_is_array.exec(expr))!==null){
          ...
          this.copy_numbers(numbers,arr,fns);
          continue;
        }
        if(1){
          ...
          let [a] = this.expr.extract_next_expr(expr,this.my_var_g);//this returns a 'Complex' number
          this.copy_numbers(numbers,[a],fns);
        }
        continue;
      }
      break;
    ...
    if(1){
      let s0 = this.numbers_to_string(numbers);
  ...
  do_path_command(symbol,line){
    ...
    let [coords] = this.read_coords(line);
    ///NOTE: for "[a,b,c]"
