...
class NitrilePreviewDiagram extends NitrilePreviewBase {
    ...
    ss.forEach((val,i,arr) => {
      if(val.startsWith('left:')){
        ...
        if(symbol.startsWith('&')){
          ...
          switch(symbol){
            case 'center': this.origin_x = this.viewport_width/2, this.origin_y = this.viewport_height/2; break;
            case 'north':  this.origin_x = this.viewport_width/2, this.origin_y = this.viewport_height; break;
            case 'south':  this.origin_x = this.viewport_width/2, this.origin_y = 0; break;
            case 'west':   this.origin_x = 0, this.origin_y = this.viewport_height/2; break;
            case 'east':   this.origin_x = this.viewport_width, this.origin_y = this.viewport_height/2; break;
            case 'northwest': this.origin_x = 0, this.origin_y = this.viewport_height; break;
            case 'northeast': this.origin_x = this.viewport_width, this.origin_y = this.viewport_height; break;
            case 'southwest': this.origin_x = 0, this.origin_y = 0; break;
            case 'southeast': this.origin_x = this.viewport_width, this.origin_y = 0; break;
          }
        } 
  ...
  get_path_from_symbol(symbol) {
    ...
    if (this.my_path_map.has(symbol)) {
      ...
      if(Number.isFinite(index)){
        ...
        if(index < p.length){
          let pt = p[index];
          pt[2] = '';//clear the 'join' field so that it may be assigned to 'L' by the caller
          return [pt];
        }else{
          return [];
        }
  ...
  read_coords(line) {
    ...
    while (line.length) {
      ...
      if((v=this.re_parse_pathfunc.exec(line))!==null){
        var [from,line] = this.parse_pathfunc(line);
      ...
      if(v1 || v2){
        ...
        if (lastjoin === 'hobby') {
          ...
          lastjoin = 'line';///reset it to 'line' so that it needs another .. to make it 'hobby'
        } else {
          ///turn it off
          if(hobby_p.length){
            hobby_p = [];
          ...
          if (key === 'angledist') {
            ...
            if (val.length == 3) {
              ...
              if (Number.isFinite(cx) && Number.isFinite(cy) && Number.isFinite(angle)) {
                ...
                lasthints = 0;
                lastjoin = 'arc';
                let pt = [x, y, 'A', '','','','', r,r,Phi,bigarcflag,sweepflag, hints];
                coords.push(pt);
                this.my_lastpt_x = x;
                this.my_lastpt_y = y;
              }
      ...
      case 'text'://CARTESIAN
        for( var i=0,j=0; i < floats.length; i+=2,j+=1 ) {
          ...
          if(Number.isFinite(x)&&Number.isFinite(y)){
            ...
            if(math){
              o.push(this.p_math(x,y,label,ta,g));
            }else{
              o.push(this.p_text(x,y,label,ta,g));
            }
          }
        }
        break;
  ...
  do_node(opts,g,txts,coords){
    ...
    for(let one of all){
      ...
      if(math){
        o.push(this.p_math(x,y,label,'ctr',g));
      }else{
        o.push(this.p_text(x,y,label,'ctr',g));
      }      
    }
    return o.join('\n');
  }
  ...
  do_box(opts,g,txts,coords){
    ...
    for (let one of all) {
      ...
      var {label,math} = this.fetch_label_at(txts,j,g);
      if(math){
        o.push(this.p_math(x+w/2,y+h/2,label,'ctr',g));
      }else{
        o.push(this.p_text(x+w/2,y+h/2,label,'ctr',g));
      }
    }
    return o.join('\n');
  }
