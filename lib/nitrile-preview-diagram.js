...
class NitrilePreviewDiagram extends NitrilePreviewBase {
  ...
  read_coords(line) {
    ...
    while (line.length) {
      ...
      if ((v = this.re_flow.exec(line)) !== null) {
        ///<1_s,5,-3> 
        n++
        var hints = 0;
        let flow = v[1];
        line = v[2];
        let pt = [0, 0, `<${flow}>`, '','','','', 0,0,0,0,0, hints];
        coords.push(pt);
        continue;
      }
  ...
  do_walk(opts,g,txt,coords){
    ...
    for(var pt of coords) {
      let v = re.exec(pt[2]);
      if(v){
        var [s,dx,dy,op,cx1,cy1,cx2,cy2] = v[1].split(',').map(s=>s.trim());
        var [id,anchor] = this.string_to_id_and_anchor(s);
  ...
  do_flow(opts,g,txt,coords){
    ...
    for(var pt of coords) {
      let v = re.exec(pt[2]);
      if(v){
        var [s,dx,dy] = v[1].split(',').map(s=>s.trim());
        var [id,anchor] = this.string_to_id_and_anchor(s);
        if(this.my_box_map.has(id)){
