...
class NitrilePreviewDiagram extends NitrilePreviewBase {
  ...
  read_action_style_label(line) {
    ...
    if((v=this.re_action_label.exec(line))!==null) {
      ...
      var txt = v[2].trim();
      g.math = 1;
      console.log('james','g.math',g.math);
      line = v[3];    
    }
    return [g,txt,line];
  }
  ...
  exec_line(line,env) {
    ...
    ///
      ...
      if (key === 'refxy') {
        ...
        this.nodeid = this.assert_int(val, this.ref_nodeid, 0, this.MAX);
      }
    }
    ///NOTE: fn
  ...
  */
  ...
  do_node(opts,g,txt,coords){
    ...
    var dx = this.get_float_prop(g,'dx',0);
    var dy = this.get_float_prop(g,'dy',0);
    var autonodeid = this.get_int_prop(g,"autonodeid",0);
    coords.forEach((pt,i,arr) => {
      ...
      if(name){
        ...
        var nodeid = this.assert_int(this.nodeid,1,0,this.MAX);
        name = `${nodeid}`
        this.my_node_map.set(name,v);
        this.nodeid = ++nodeid;
      }
