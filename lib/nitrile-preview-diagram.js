...
class NitrilePreviewDiagram extends NitrilePreviewBase {
  ...
  constructor(translator) {
    ...
    this.re_arr_command = /^\\arr\s+([A-Za-z][A-Za-z0-9]*)\s*=\s*(.*)$/;
  ...
  exec_body(style,body,env){
    ...
    while (lines.length) {
      ...
      //
      // \arr a = 12.3 12.4 12.5
      //
      if((v=this.re_arr_command.exec(line))!==null){
        let key = v[1];
        let val = v[2];
        let [numbers] = this.read_array(val,env);//returns an array 'Complex' number
        env[key] = numbers;
        let s = this.array_to_string(numbers);
        this.do_comment(`*** [arr] ${key} = ${s}`);          
        continue;
      }  
      //
      // \var a = 12.3
