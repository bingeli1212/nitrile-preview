...
class NitrilePreviewDiagram extends NitrilePreviewBase {
  ...
  read_action_floats(line) {
    ...
    var v;
    while(line.length) {
      if((m=re_range_expr.exec(line))!==null){
        // [1:3:10]
        line = m[2];
        v = m[1];
        v = v.split(':');
      ...
      else if((m=re_float.exec(line))!==null){
        // 12
        v = m[1];
        line = m[2];
        v = parseFloat(v);
  ...
  exec_line(line,env) {
    ...
    if((v=this.re_env_string.exec(line))!==null){
      let env_name = v[1];
      let env_string = v[2];
      if(this.re_symbol_complete.test(env_name)){
        [env_string] = this.line_replace_env(env_string,env);
        env[env_name] = env_string;
      }
      return false;
    }else if((v=this.re_env_expr.exec(line))!==null){
      let env_name = v[1];
      let env_expr = v[2];
      if(this.re_symbol_complete.test(env_name)){
