...
class NitrilePreviewDiagram extends NitrilePreviewBase {
  ...
  exec_action_command(style,line,env) {
    ...
    if (cmd == 'image'){
      ...
      o.push(this.do_image(opts,w,h,g,txt));///own method
    }
    ///
    ///NOTE: 
    ///
    if (cmd == 'subtitle'){
      ...
      const re_2 = /^\{(.*)\}\s*\"(.*)\"$/;
      let v;
      if((v=re_2.exec(data))!==null){
        g = this.string_to_style(v[1],g);
        txt = v[2];
      }else if((v=re_1.exec(data))!==null){
        txt = v[1];
      }else{
        txt = data;
      }
      ...
      let w = this.viewport_width;
      let h = this.viewport_height;
      o.push(this.do_subtitle(opt,w,h,g,txt));///own method
    }
    ///
    ///NOTE: 
    ///
      ...
      o.push(this.do_drawimage(opts,g,txts,coords));///own method
    }
    ///
    ///NOTE: 
    ///
    else if (cmd == 'drawlabel' || cmd=='drawtext'){
      let [coords,g,txts] = this.read_coords(data,env);
    ...
    ///
      ...
      o.push(this.do_barchart(opts,g,txts,numbers));///own method
    }
    ///
  ...
  extract_pathfunc_args(s,env,lastpt_x,lastpt_y){
    ...
    while(s.length){
      if((v=re_cma.exec(s))!==null){
        ...
        s = v[2];
        if(s1=='*'){
          let p = [lastpt_x,lastpt_y,'M', '','','','', 0,0,0,0,0, 0];
          pt.coords.push(p);
        }else{
          s1 = `[${s1}]`;
          var [val] = this.read_array(s1,env);
          var val = this.numbers_to_floats(val);
          let [x,y] = val;
          if(Number.isFinite(x) && Number.isFinite(y)){
            let p = [x,y,'M', '','','','', 0,0,0,0,0, 0];
            pt.coords.push(p);
          }
        }
      }
  ...
  do_barchart(opts,g,txts,numbers) {
        ...
        for( var i=0,j=0; i < floats.length; i+=1,j+=1 ) {
          ...
          if (Number.isFinite(num)){
            ...
            if(Number.isFinite(x)&&Number.isFinite(y)){
              ...
              p.push(this.p_label(x,y,label,'bot',g));
            }
          }
        }
        break;
  ...
  do_drawimage(opts,g,txts,coords){
    var o = [];
    ...
    for (var j = 0; j < coords.length; ++j){
      ...
      o.push(this.p_image(x,y,w,h,ta,label,g));
    }
    return o.join('\n');
  }
  do_image(opt,w,h,g,txt){
    var o = [];
    var ta = '';
    if(opt=='contain'){
      g = {...g,fit:'contain'};
    ...
    o.push(this.p_image(0,0,w,h,ta,txt,g));
    return o.join('\n');
  }
  do_subtitle(opt,w,h,g,txt){
    var o = [];
    o.push(this.p_subtitle(opt,w,h,txt,g));
    return o.join('\n');
  }
  ...
  do_drawpango(opts,g,txts,coords){
    ...
    var label = this.fetch_label_at(txts,0,g);
    var o = [];
    ...
    for(var j=0; j < linesegs.length; ++j){
      ...
      if(this.is_valid_point(p0)&&this.is_valid_point(p1)&&this.is_valid_point(q0)&&this.is_valid_point(q1)&&this.is_same_pt(p1,q0)){
        ...
        if (label) {
          ...
          o.push(this.p_label(labelx,labely,label,'ctr',g));
        }
      }
    }
    return o.join('\n');
    ...
    for(var j=0; j < linesegs.length; ++j){
      ...
      if(this.is_valid_point(p0)&&this.is_valid_point(p1)&&this.is_valid_point(q0)&&this.is_valid_point(q1)&&this.is_same_pt(p1,q0)){
        ...
        if(bartype=='triple'){
          ...
          o.push(this.p_stroke(q3,g));
          o.push(this.p_stroke(q2,g));
          o.push(this.p_stroke(q1,g));
  ...
  do_node(opts,g,txts,coords){
    ...
    all.forEach((one,j,arr) => {
      ...
      o.push(this.p_label(x+tx,y+ty,label,'ctr',g));    
    })
    return o.join('\n');
  }
  do_edge(opts,g,txts,coords){
    ...
    for(let j=1; j < opts.length; ++j){
      var name1 = opts[j-1];
      var name2 = opts[j];
      var label = this.fetch_label_at(txts,j-1,g);
      if(this.my_node_map.has(name1)&&this.my_node_map.has(name2)){
        ...
        if(name1==name2){
          ...
          if(label){
            o.push(this.p_label(mylabelx,mylabely,label,'ctr',g));
          }
        }else{
  ...
  do_box(opts,g,txts,coords){
    ...
    all.forEach((one,j,arr) => {
      ...
      o.push(this.p_draw(q,g));
      var label = this.fetch_label_at(txts,j,g);
      o.push(this.p_label(x+w/2+tx,y+h/2+ty,label,'ctr',g));
    });
    return o.join('\n');
  }
  ...
  g_to_linecolor_string(g){
    ...
    if(g.hints & this.hint_stroke1){
      return this.assert_string(this.config.stroke1,this.stroke1);
    }
    if(g.hints & this.hint_stroke2){
      return this.assert_string(this.config.stroke2,this.stroke2);
    }
    if(g.hints & this.hint_stroke3){
      return this.assert_string(this.config.stroke3,this.stroke3);
    }
    return this.get_string_prop(g,'linecolor','');
  }
  g_to_gridopacity_float(g){
    g = this.update_style_with_group(g);
    return this.get_float_prop(g,'gridopacity',0.2,0,1);
  }
  g_to_fillcolor_string(g){
    ...
    if(g.hints & this.hint_fill0){
      return this.assert_string(this.config.fill0,this.fill0);
    }
    if(g.hints & this.hint_fill1){
      return this.assert_string(this.config.fill1,this.fill1);
    }
    if(g.hints & this.hint_fill2){
      return this.assert_string(this.config.fill2,this.fill2);
    }
    if(g.hints & this.hint_fill3){
      return this.assert_string(this.config.fill3,this.fill3);
    }
    if(g.hints & this.hint_fill4){
      return this.assert_string(this.config.fill4,this.fill4);
    }
    if(g.hints & this.hint_fill5){
      return this.assert_string(this.config.fill5,this.fill5);
    }
    if(g.hints & this.hint_fill6){
      return this.assert_string(this.config.fill6,this.fill6);
    }
    if(g.hints & this.hint_fill7){
      return this.assert_string(this.config.fill7,this.fill7);
    }
    if(g.hints & this.hint_fill8){
      return this.assert_string(this.config.fill8,this.fill8);
    }
    if(g.hints & this.hint_fill9){
      return this.assert_string(this.config.fill9,this.fill9);
    }
  ...
  g_to_rx_float(g){
    g = this.update_style_with_group(g);
    return this.get_float_prop(g,'rx',this.r,0);
  }
  g_to_ry_float(g){
    g = this.update_style_with_group(g);
    return this.get_float_prop(g,'ry',this.r,0);
  }
  g_to_r_float(g){
    g = this.update_style_with_group(g);
