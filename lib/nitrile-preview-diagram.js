  ...
  */
  ...
  do_lego(opts,g,txt,ts,floats) {
    ...
    if (!this.my_lego[id]) {
      ...
      A.max_y = 4;
      A.max_z = 4;
      A.q = [[0,0],[1,0,'L'],[1,1,'L'],[0,1,'L'],[0,0,'cycle'],
             [0,1],[1,1,'L'],[1.3,1.3,'L'],[0.3,1.3,'L'],[0,0,'cycle'],
             [1,0],[1,1,'L'],[1.3,1.3,'L'],[1.3,0.3,'L'],[0,0,'cycle']];
      A.all = new Set();
    }
    var A = this.my_lego[id];
    var [opt,subopt] = opts;
    switch( opt ) {
      case 'setup'://LEGO
        // lego.setup 0 0 4 4 4
        A.xorigin = this.assert_float(floats[0],A.xorigin);
        A.yorigin = this.assert_float(floats[1],A.yorigin);
        A.max_x = this.assert_float(floats[2],A.max_x);
        A.max_y = this.assert_float(floats[3],A.max_y);
        A.max_z = this.assert_float(floats[4],A.max_z);
        break;
      ...
      case 'add_z'://LEGO
        for( var j=0; j < floats.length; j++ ) {
          var z = floats[j];
          if(z >= 0 && z < A.max_z) {
            for(var y=0; y < A.max_y; y++) {
              for(var x=0; x < A.max_x; x++) {
                var key = `${x}/${y}/${z}`;
                A.all.add(key);
                console.log('james','key',key);
              }
            }
          }
        }
      ...
      case 'delete_yz'://LEGO
        for( var j=0; j < floats.length; j+=2 ) {
          ...
          for(var x=0; x < A.max_x; x++){
            ...
            if(A.all.has(key)){
              A.all.delete(key);
              console.log('james','delete key',key);
            }
          }
        }

      case 'delete_xyz'://LEGO
        for( var j=0; j < floats.length; j+=3 ) {
          ...
          if(A.all.has(key)){
            A.all.delete(key);
            console.log('james','remove key',key);
          }
        }
        break;
          ...
          for(var y=max_y; y >= 0; y--) {
            for(var x=0; x <= max_x; x++) {
              ...
              if(A.all.has(key)){
                console.log('james','key',key);
                var q = A.q;
                var offsetx = (1 * x) + (0.3 * y);
                var offsety = 0.3 * y + (1 * z);
