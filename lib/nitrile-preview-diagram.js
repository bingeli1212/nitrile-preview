...
class NitrilePreviewDiagram extends NitrilePreviewBase {
  ...
  constructor(translator) {
    ...
    */
    ...
    this.re_numbers_arr           = /^&([A-Za-z0-9_]+)\s*(.*)$/;
    this.re_numbers_fn            = /^fn:(\w+)\s*(.*)$/;
    this.re_numbers_range         = /^\[(\S+)\]\s*(.*)$/;
    // path construction
    this.re_dashdot = /^(~|\.\.)\s*(.*)$/;
    this.re_coord = /^\((.*?)\)\s*(.*)$/;
  ...
  read_numbers(line) {
    ...
    var fn;
    while(line.length) {
      if((v=this.re_numbers_range.exec(line))!==null){
        let range = v[1];
        line = v[2];
        let pp_bar = range.split('|');
        if(pp_bar.length==3){
          let [a1] = this.expr.extract_next_expr(pp_bar[0],this.my_var_g);//returns a 'Complex'
          let [a2] = this.expr.extract_next_expr(pp_bar[1],this.my_var_g);//If empty it returns (0,0*I)
          let [a3] = this.expr.extract_next_expr(pp_bar[2],this.my_var_g);
          let all = [];
      ...
      }else if((v=this.re_numbers_fn.exec(line))!==null){
        fn = v[1];
        line = v[2];
      }else if((v=this.re_numbers_arr.exec(line))!==null){
        let symbol = v[1];
        line = v[2];
