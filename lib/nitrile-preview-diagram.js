  ...
  exec_body(style,body,env){
    ...
    while (lines.length) {
      ...
      if((v=this.re_set_command.exec(line))!==null){
        ...
        if(eq==':'){
          this.config[name] = s1;
          this.do_comment(`*** set ${name} : ${s1}`);     
        }else if(this.re_is_symbolname.test(name)){
          if(arrflag){
            let [numbers] = this.read_array(s1,env);//returns an array 'Complex' number
            this.config[name] = this.array_to_string(numbers);
            this.do_comment(`*** set ${name} = ${this.array_to_string(numbers)}`);
          }else{
            let a = this.read_scalar(s1,env);//returns a Complex or a string
            if(a instanceof Complex){
              this.config[name] = a.re;
            }else if(typeof a == 'string'){
              this.config[name] = a;
            }else{
              this.config[name] = `${a}`;
            }
            this.do_comment(`*** set ${name} = ${a}`);          
          }
        }else{
          this.do_comment(`***ERROR '${name}' is not a valid symbol name`);  
        }
