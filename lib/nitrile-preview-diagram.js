...
class NitrilePreviewDiagram extends NitrilePreviewBase {
  ...
  exec_body(style,body,env){
    ...
    while (lines.length) {
      ...
      if((v=this.re_array_command.exec(line))!==null){
        let name = v[1];
        let line = v[2];
        let [numbers] = this.read_complex_numbers(line,env);//returns an array 'Complex' number
      ...
      if((v=this.re_let_command.exec(line))!==null){
        let name = v[1];
        let expr = v[2];
        let [a] = this.expr.extract_next_expr(expr,env);//this returns a 'Complex' number
        env[name] = a;
      ...
      if((v=this.re_for_command.exec(line)) !== null) {
        ...
        while(line.length){
          if((v=re_for_one.exec(line))!==null) {
            ...
            let name = v[1];
            let range = v[2];
            let [numbers] = this.read_complex_numbers(range,env);
    ...
    while(line.length) {
      ...
      if((v=this.re_numbers_nonblanks.exec(line))!==null){ 
        ...
        if((v=this.re_numbers_is_range_expr2.exec(expr))!==null){
          /// '1:10'
          let [a1] = this.expr.extract_next_expr(v[1],env);//returns a 'Complex'
          let [a2] = this.expr.extract_next_expr(v[2],env);//If empty it returns (0,0*I)
          let all = [];
        ...
        if((v=this.re_numbers_is_range_expr3.exec(expr))!==null){
          ...
          let [a1]   = this.expr.extract_next_expr(v[1],env);//returns a 'Complex'
          let [a2] = this.expr.extract_next_expr(v[2],env);//If empty it returns (0,0*I)
          let [a3]   = this.expr.extract_next_expr(v[3],env);//If empty it returns (0,0*I)
          let all = [];
        ...
        if(1){
          ...
          ///'12.3', '12.3+10i', '12.3-10i', '1i', '-1i', etc.
          let [a] = this.expr.extract_next_expr(expr,env);//this returns a 'Complex' number
  ...
  drop_numbers(all,f){
    all = all.filter( (x) => {
      let a = this.expr.exec_complex_func(f,[x],env,0);
