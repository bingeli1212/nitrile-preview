...
class NitrilePreviewDiagram extends NitrilePreviewBase {
  ...
  constructor(translator) {
    ...
    /// all attrflags
    this.attrflag_linedashed = 0x1 << 0;
  ...
  get_path_from_variable(a) {
    ...
    if(a=='apple'){
      return this.read_coords_line(this.string_to_builtin_path('apple')).coords;
    }
    if(a=='basket'){
      return this.read_coords_line(this.string_to_builtin_path('basket')).coords;
    }
    if(a=='crate'){
      return this.read_coords_line(this.string_to_builtin_path('crate')).coords;
    }
    if(a=='tree'){
      return this.read_coords_line(this.string_to_builtin_path('tree')).coords;
    }
    if(a=='balloon'){
      return this.read_coords_line(this.string_to_builtin_path('balloon')).coords;
    }
  ...
  read_coords_line(line) {
    ...
    while (line.length) {
      ...
      if ((v = this.re_coord.exec(line))!==null) {
        ...
        var d0 = fun_name[0];
        var d1 = fun_name[1];
        var attrflag = parseInt(fun_name[2])||0;
        if (!d0) {
          x = lastpt[0];
        } else {
          x = parseFloat(d0);
