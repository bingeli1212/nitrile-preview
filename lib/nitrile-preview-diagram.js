...
class NitrilePreviewDiagram extends NitrilePreviewBase {
      ...
      for(var s of lines){
        if((v=this.re_is_buf_start.exec(s))!==null){
          ...
          if((v=this.re_is_buf_paste.exec(s))!==null){
            let name = v[1];
            if(this.translator.buffers.hasOwnProperty(name)){
              let {ss} = this.translator.buffers[name];
              o = o.concat(ss);
            }
          }
          continue;
  ...
  exec_body(style,body,env){
    ...
    while (lines.length) {
      ...
      if((v=this.re_arr_command.exec(line))!==null){
        ...
        this.my_arr_map.set(key,numbers);
  ...
  exec_action_command(style,line,env) {
    ...
    ///NOTE:
    ///
    else if (cmd=='drawplot'){
      let [coords,g,txts] = this.read_coords(data,env);
      o.push(this.p_plot(coords,g));
    }
    ///
    ///NOTE:
    ///
  ...
  extract_pathfunc_args(s,env){
    ...
    d.forEach((pt) => {
      ...
      else if((v=re_avr.exec(s))!==null){
        let s1 = v[1];
        if(this.my_arr_map.has(s1)){
          let l = this.my_arr_map.get(s1);
          l.forEach((x) => {
            pt.array.push(x);
          })
        }        
