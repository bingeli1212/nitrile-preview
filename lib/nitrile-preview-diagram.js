  ...
  */
  ...
  do_box(opts,g,txt,coords){
    ...
    for (var j = 0; j < coords.length; j++) {
      ...
      if(boxtype=='hexgon'){
        ...
        if(g1){
          o.push(this.p_rhombus(x1,y1,w,h,rdx,rdy,g1));
        }
    ...
    if(coords.length==0){
      for(var j=0; j < opts.length; j++){
        ...
        if(this.my_box_map.has(id)){
          ...
          var [x,y,w,h,rdx,rdy,boxtype] = v;
          var [x,y] = this.movexy(x,y);
          var x1, y1, g1;
          var p = [x,y,w,h,rdx,rdy,g, boxtype, x1,y1,g1];
          all.push(p);
        }
      }
    }
    all.forEach((p) => {
      var [x,y,w,h,rdx,rdy,g, boxtype, x1,y1,g1] = p;
      if(boxtype=='hexgon'){
        if(g1){
          o.push(this.p_hexgon(x1,y1,w,h,g1));
        }
  ...
  g_to_opacity_float(g){
    ...
    if(g.hints & this.hint_shadow){
      return this.get_float_prop(g,'shadowopacity',0.15,0,1);
    }
