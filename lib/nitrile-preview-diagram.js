...
class NitrilePreviewDiagram extends NitrilePreviewBase {
  ...
  exec_command_line(line) {
    ...
    ///
      ...
      if(opt){
        ...
        o.push(this.draw_text_between_points(dx,dy,shift,txt,g,coords));
      }
    }
    else if (cmd=='fill'){
      this.move_coords(coords,this.refx,this.refy,this.refsx,this.refsy,0);
  ...
  parse_pathfunc(line){
    ...
    var v = this.re_parse_pathfunc.exec(line);
    var name = v[1];
    var line = v[2];
    var ret_val = [];
    var j = this.translator.extract(line,0);
    var scan_text = line.slice(0,j);
    var line = line.slice(j).trimLeft();
...
*/
  ...
  */
  ...
  draw_text_between_points(dx,dy,shift,txt,g,coords){
    var o = [];
    if(coords.length){
      for(var j=0; j < coords.length; j++){
        var i = j-1;
