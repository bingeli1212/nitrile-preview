...
class NitrilePreviewDiagram extends NitrilePreviewBase {
  ...
  constructor(translator) {
    ...
    this.re_var_command = /^\\(?:var|set)\s+([A-Za-z][A-Za-z0-9]*)(\[\]|)\s*(\??)([=:])\s*(.*)$/;
  ...
  init_internals(style={}){
    ...
    if(typeof style=='object'){
      this.env = {...this.env,...style}; 
    }
    /// setup the replacement label
    let replace = this.env.replace||'';
    let ss = this.string_to_array(replace);
    this.replace = ss.map((s) => {
      let [key,val] = s.split('/');
      key = key||'';
      val = val||'';
      return [key,val];      
  ...
  fetch_label_at(txts,n,g){
    ...
    label = this.replace_label(label,this.replace);
    return label;
  }
