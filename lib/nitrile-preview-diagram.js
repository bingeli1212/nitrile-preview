...
class NitrilePreviewDiagram extends NitrilePreviewBase {
  ...
  exec_command_line(line) {
    ...
    else if (cmd == 'textpath'){
      o.push(this.do_textpath(opts,g,txt,coords));///own method
    }
    ///
        ...
        if (fun_name.length) {
          ...
          /// [a:rx,ry,angle,bigarcflag,sweepflag,dx,dy]
            ...
            if(val=='last'){
              //NOTE:[offset:last]
              let pt = [0,0];
              pt[0] = this.my_offset[0];
              pt[1] = this.my_offset[1];
              coords.push(pt);
              lastpt[0] = pt[0];
              lastpt[1] = pt[1];
            }else if(val.startsWith('&')){
            ...
            if(this.re_symbol_name.test(val)){
              ...
              pt[0] = lastpt[0];
              pt[1] = lastpt[1];
              this.my_lastpt_map.set(val,pt);
              this.do_comment(`***save lastpt: ${val}=(${pt[0]},${pt[0]})`);
            }
          }
          else if (key === 'load') {
            //NOTE:[load:a]
            if(this.my_lastpt_map.has(val)){
              let pt = this.my_lastpt_map.get(val);
              lastpt[0] = pt[0];
              lastpt[1] = pt[1];
              this.do_comment(`***load lastpt: ${val}=(${pt[0]},${pt[0]})`);
            }
          }
        }
        continue;
  ...
  */
  ...
  do_textpath(opts,g,txt,coords){
    ...
    for (var i = 0; i < linesegs.length; ++i){
      ...
      var x2 = lineseg.q1[0];
      var y2 = lineseg.q1[1];
      o.push(this.p_textpath(x1+dx,y1+dy,x2+dx,y2+dy,txt,ta,g));
    }
    return o.join('\n');
  }
