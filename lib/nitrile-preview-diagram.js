...
class NitrilePreviewDiagram extends NitrilePreviewBase {
  ...
  exec_body(style,body,env){
    ...
    while (lines.length) {
      ...
      if((v=this.re_var_command.exec(line))!==null){
        ...
        if(index.length){
          // var a[] = 12.3 12.4 12.5
          let [numbers] = this.read_complex_numbers(s);//returns an array 'Complex' number
          env[name] = numbers;
          this.do_comment(`*** ${name}[] = ${this.numbers_to_string(numbers)}`);
        }else{
          // var a = 12.3
          let [numbers] = this.read_complex_numbers(s);//returns an array 'Complex' number
          env[name] = numbers[0];
          this.do_comment(`*** ${name} = ${this.numbers_to_string(numbers.slice(0,1))}`);
        }
        continue;
      }  
      //
      ...
      if((v=this.re_let_command.exec(line))!==null){
        ...
        let [a] = m;
        env[name] = a;
        this.do_comment(`*** ${name} = ${a.toString()}`);
        continue;
      }  
      //
      // for-loop
    ...
    while(line.length) {
      ...
      if((v=this.re_numbers_nonblanks.exec(line))!==null){ 
        ...
        if(1){
          ...
          ///'12.3', '12.3+10i', '12.3-10i', '1i', '-1i', '1.234E-10', etc.
  ...
  do_path_command(symbol,line){
    ...
    if(symbol=='_'){
      ...
      this.do_comment(`***${symbol} = ${this.coords_to_string(this.my_path_map.get(symbol))}`);
      ...
      for (var i = 0; i < segs.length; ++i) {
        ...
        if (this.re_path_name.test(seg)) {
          ...
          this.do_comment(`***${seg} = ${this.coords_to_string(this.my_path_map.get(seg))}`);
        }
      } 
