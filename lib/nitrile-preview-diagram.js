...
class NitrilePreviewDiagram extends NitrilePreviewBase {
  ...
  exec_body(style,body,env){
    ...
    while (lines.length) {
      ...
      var [line,flag] = this.replace_line_env_variables(style,line,env);
      //
      //send to console
      //
      //
    ...
    while (line.length) {
      ...
      if ((v = this.re_coords_is_relative.exec(line)) !== null) {
        ...
        if (key === 'angledist') {
          ...
          if (val.length == 2) {
            var tx = this.lastpt_x + val[0];
            var ty = this.lastpt_y + val[1];
          ...
          if (val.length == 6) {
            x1 = this.lastpt_x + val[0];
            y1 = this.lastpt_y + val[1];
            x2 = this.lastpt_x + val[2];
            y2 = this.lastpt_y + val[3];
            x = this.lastpt_x + val[4];
            y = this.lastpt_y + val[5];
          ...
          if (val.length == 4) {
            ...
            x2 = this.lastpt_x + val[0];
            y2 = this.lastpt_y + val[1];
            x = this.lastpt_x + val[2];
            y = this.lastpt_y + val[3];
            if (Number.isFinite(x1) &&
              Number.isFinite(y1) &&
              Number.isFinite(x2) &&
              Number.isFinite(y2) &&
          ...
          /// [a:rx,ry,angle,bigarcflag,sweepflag,dx,dy]
          if (val.length == 7) {
            let X1 = this.lastpt_x;
            let Y1 = this.lastpt_y;
            let X2 = this.lastpt_x + val[5];
            let Y2 = this.lastpt_y + val[6];
            let Rx = val[0];
            let Ry = val[1];
            let Phi = val[2];
  ...
  replace_line_env_variables(style,line,env){
    /// replace env variable before everything else
    let flag = 0;
    const re_replace = /\$\{(.*?)\}/g;
    const re_is_config_name = /^%([A-Za-z][A-Za-z0-9]*)%$/g;
    const re_is_scalar_name = /^([A-Za-z][A-Za-z0-9]*)$/g;
    let v;
    line = line.replace(re_replace,(match,p1) => {
      flag = 1;
      if((v=re_is_config_name.exec(p1))!==null){
        p1 = v[1];
        if(style.hasOwnProperty(p1)){
          p1 = style[p1];
        }else{
          p1 = "";
        }
        return p1;
      }else if((v=re_is_scalar_name.exec(p1))!==null){
        p1 = v[1];
        if(env.hasOwnProperty(p1)){
          let m = env[p1];
          if(Array.isArray(m)){
            p1 = this.array_to_string(m);
          }else if(m instanceof Complex){
            p1 = m.toString();
          }else{
            p1 = ""+m;
