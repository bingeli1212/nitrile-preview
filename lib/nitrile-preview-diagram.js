...
class NitrilePreviewDiagram extends NitrilePreviewBase {
  ...
  constructor(translator) {
    ...
    this.re_var_command = /^\\var\s+([A-Za-z][A-Za-z0-9]*)(\[\w*\]|)\s*=\s*(.*)$/;
    this.re_var2_command = /^\\var\s+\(([A-Za-z][A-Za-z0-9]*),([A-Za-z][A-Za-z0-9]*)\)\s*=\s*(.*)$/;
  ...
  exec_body(style,body,env){
    ...
    while (lines.length) {
      ...
      //
      // \var (cx,cy) = (&O) // this sets 'cx' to x-coord and 'cy' to y-coord of (&O) point
      //
      if((v=this.re_var2_command.exec(line))!==null){
        let cx = v[1];
        let cy = v[2];
        let val = v[3];
        ...
        switch (key){
          ...
          default: {
            if(Number.isFinite(num)){
              ...
              else if(key=='up')     lastoffset_y += num;
