...
class NitrilePreviewDiagram extends NitrilePreviewBase {
  ...
  exec_command_line(line) {
    ...
    if (cmd == 'text'){
      ...
      if(this.g_to_math_int(g)){
        o.push(this.do_math(opts,g,txts,coords));///own method
      }else{
    ...
    ///
      ...
      o.push(this.do_cartesian(subcmd,opts,g,txts,data));///own method
    }
    ///
    ...
    ///
      ...
      o.push(this.do_barchart(subcmd,opts,g,txts,floats));///own method
    }
    ///
    ...
    ///
      ...
      let [floats] = this.read_floats(data);
      o.push(this.do_lego(subcmd,opts,g,txts,floats));///own method
    }
    ///
    ...
    ///
      ...
      o.push(this.do_argand(subcmd,opts,g,txts,data));///own method
    }
    ///
    ...
    ///
      ...
      //let [numbers] = this.read_complex_numbers(data);
      o.push(this.do_trump(subcmd,subsubcmd,opts,g,txts,data));///own method
    }
    ///
  ...
  do_barchart(subcmd,opts,g,txts,floats) {
    var o = [];
    var id = 0;
      ...
      case 'xlabel'://BARCHART
        for( var j=0,i=0; j < floats.length; j+=1,i++ ) {
          ...
          if (Number.isFinite(num)){
            ...
            var y = A.yorigin;
            if(Number.isFinite(x)&&Number.isFinite(y)){
              var label = this.fetch_label_at(txts,j,g);
  ...
  do_cartesian(subcmd,opts,g,txts,data) {
    var o = [];
    var id = 0;
    let [floats] = this.read_floats(data);
  ...
  do_lego(subcmd,opts,g,txts,floats) {
    var o = [];
    var id = 0;
    if (!this.my_lego[id]) {
      this.my_lego[id] = {};
  ...
  do_argand(subcmd,opts,g,txts,data) {
    var o = [];
    var id = 0;
      ...
      case 'label': {
        ...
        for( var j=0; j < numbers.length; j++ ) {
          ...
          if(Number.isFinite(x)&&Number.isFinite(y)){
            let [opt] = opts;
            var ta = this.assert_ta(opt);
            let label = this.fetch_label_at(txts,j,g);
            if(!label){
              label = a1.toString();
            }
  ...
  do_trump(subcmd,subsubcmd,opts,g,txts,data) {
    var o = [];
    var id = 0;
  ...
  do_math(opts,g,txts,coords){
    ...
    for (var j = 0; j < coords.length; ++j){
      ...
      o.push(this.p_math(x+dx,y+dy,label,ta,g));
    }
    return o.join('\n');
  }
  do_label(opts,g,txts,coords){
    var o = [];
    var dx = this.g_to_dx_float(g);
    var dy = this.g_to_dy_float(g);
    var shift = this.g_to_shift_float(g);
  ...
  do_text(opts,g,txts,coords){
    ...
    for (var j = 0; j < coords.length; ++j){
      ...
      var ta = this.assert_ta(opt);
      var [x,y] = this.linear_shift_pt(shift,ta,x,y);
      o.push(this.p_text(x+dx,y+dy,label,ta,g));
    }
    return o.join('\n');
  }
  ...
  do_node(opts,g,txts,coords){
    ...
    for(let one of all){
      ...
      if(math){
        o.push(this.p_math(x+dx,y+dy,label,'ctr',g));
      }else{
        o.push(this.p_text(x+dx,y+dy,label,'ctr',g));
      }      
    }
    return o.join('\n');
  }
  ...
  do_box(opts,g,txts,coords){
    ...
    for (let one of all) {
      ...
      o.push(this.p_draw(q,g));
      var label = this.fetch_label_at(txts,j,g);
      o.push(this.p_text(x+w/2+dx,y+h/2+dy,label,'ctr',g));
    }
    return o.join('\n');
  }
