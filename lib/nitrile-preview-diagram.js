...
class NitrilePreviewDiagram extends NitrilePreviewBase {
  ...
  constructor(translator) {
    ...
    */
    ...
    this.re_aux   = /^(x:|y:|sx:|sy:|s:|up:|down:|left:|right:|at:|save:|hints:)(\S*)\s*(.*)$/;
    this.re_relative = /^\[(.*?)\]\s*(.*)$/;
  ...
  do_reset() {
    ...
    this.def_offset_x = this.offset_x = 0;
    this.def_offset_y = this.offset_y = 0;
    this.def_offset_sx = this.offset_sx = 1;
    this.def_offset_sy = this.offset_sy = 1;
    this.def_origin_x = this.origin_x = 0;
    this.def_origin_y = this.origin_y = 0;
    this.def_scale_xy = this.scale_xy = 1;
  }
  do_set_command(key,val) {
    if (key === 'id' ) {
      ...
      /// set offset center
      /// set offset origin
      /// set offset left:2 right:2 up:2 down:2
      ///
      ss.forEach((val,i,arr) => {
        if(val.startsWith('left:')){
          let s = val.substr(5);
  ...
  move_coords(coords){
    this.offset_coords(coords,this.origin_x,this.origin_y,this.scale_xy,this.scale_xy,0);
  }
  move_xy(x,y){
    var pt = [x,y];
    this.offset_pt(pt,this.origin_x,this.origin_y,this.scale_xy,this.scale_xy,0);
    return pt;
  }
  scale_dist(r){
    return r*this.scale_xy;
  }
  ...
  read_coords(line) {
    ...
    var offset_sx = this.offset_sx;
    var offset_sy = this.offset_sy;
    var dx = 0;
    var dy = 0;
    var x = 0;
    var y = 0;
    ...
    while (line.length) {
      ...
      if ((v = this.re_aux.exec(line))!==null) {
        ...
        if (key === 'hints:'){
          ...
          if(key=='up:')          offset_y+=num;
          else if(key=='down:')   offset_y-=num;
          else if(key=='left:')   offset_x-=num;
          else if(key=='right:')  offset_x+=num;
          else if(key=='x:')      offset_x=num;
          else if(key=='y:')      offset_y=num;
          else if(key=='sx:')     offset_sx=num;
          else if(key=='sy:')     offset_sy=num;
          else if(key=='s:')      offset_sx=num,offset_sy=num;
        }
        lastjoin = '';
        continue;
      }
      ...
      if((v=this.re_parse_pathfunc.exec(line))!==null){
        ...
        for (var i = 0; i < ret_val.length; ++i) {
          ...
          this.offset_pt(pt,offset_x,offset_y,offset_sx,offset_sy);
          if(lastjoin=='line'){
            pt[2]='L';
          }
          lastjoin='';
    ...
    for(let i=1; i < opts.length; ++i){
      ...
      if(this.my_node_map.has(name1)&&this.my_node_map.has(name2)){
        ...
        if(name1==name2){
          ...
          this.my_lastpt[0] = p0[0];
          this.my_lastpt[1] = p0[1];
          this.my_lastpt[0] = p2[0];
          this.my_lastpt[1] = p2[1];
          if(coords.length){
            var q = [];
