...
class NitrilePreviewDiagram extends NitrilePreviewBase {
  ...
  constructor(translator) {
    ...
    this.re_set_command = /^\\(?:set|var)\s+([A-Za-z][A-Za-z0-9]*)(\[\w*\]|)\s*(\??)([=:])\s*(.*)$/;
    this.re_set2_command = /^\\set\s+\(([A-Za-z][A-Za-z0-9]*),([A-Za-z][A-Za-z0-9]*)\)\s*=\s*(.*)$/;
    this.re_origin_command = /^\\origin\s+(.*)$/;
    this.re_show_command = /^\\show\s+(.*)$/;
    this.re_subtitle_command = /^\\subtitle\s+(.*)$/;
    this.re_image_command = /^\\image\s+(.*)$/;
  ...
  init_internals(style={}){
    ...
    this.env = {...style};
    /// my subtitle
    this.my_subtitle = style.subtitle||'';
      ...
      for(var s of lines){
        if((v=this.re_is_buf_start.exec(s))!==null){
          ...
          if((v=this.re_is_buf_paste.exec(s))!==null){
            let name = v[1];
            if(style['$'].buffers.hasOwnProperty(name)){
              let {ss} = style['$'].buffers[name];
              o = o.concat(ss);
            }
          }
          continue;
  ...
  exec_body(style,body,env){
    ...
    while (lines.length) {
      ...
      // \set a[i] = 12.3 12.4 12.5
      // \set a = @"%d-%d-%d" 1 2 3
      // \set a ?= 3  // this will set a to 3 only when a is not already set
      //
      if((v=this.re_set_command.exec(line))!==null){
        ...
        if(this.re_is_symbolname.test(key)){
          if(que && env.hasOwnProperty(key)){
            ...
            let [numbers] = this.read_array(val,env);//returns an array 'Complex' number
            let i = arrflag.slice(1,-1);
            if(this.re_is_symbolname.test(i)){
              i = parseInt(env[i])||0;
            }else{
              i = parseInt(i)||0;//maybe it is already a number such as '2'
            }
            if(i < numbers.length){
              env[key] = numbers[i];
            }else{
      ...
      //
      // \set (cx,cy) = (&O) // this sets 'cx' to x-coord and 'cy' to y-coord of (&O) point
      //
      if((v=this.re_set2_command.exec(line))!==null){
        let cx = v[1];
        let cy = v[2];
        let val = v[3];
      ...
      // image
      //
      if((v=this.re_image_command.exec(line))!==null) {
        let s = v[1];
        this.do_image(s);
        continue;
      } 
      ...
      if((v=this.re_for_command.exec(line)) !== null) {
        ...
        var s = v[1];
        var iter_array = [];
        const re_for_one = /^([A-Za-z][A-Za-z0-9]*)\s*in\s*(.*?)\s*;\s*(.*)$/;
        while(s.length){
  ...
  do_subtitle(s) {
    var v;
    var o = [];
    while((v=this.re_coords_is_label.exec(s))!==null){
      let label = v[1].trim();
      s = v[2];    
      o.push(label);
    }
    this.my_subtitle = o.join(' ');
  }
  do_image(s){
    var v;
    var o = [];
