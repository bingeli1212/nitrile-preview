...
class NitrilePreviewDiagram extends NitrilePreviewBase {
  ...
  constructor(translator) {
    ...
    this.re_range_three = /^([^:]+):([^:]+):([^:]+)$/;
    /// regular expression
    this.re_var_command = /^var\b\s*(\w+)\s*=\s*(.*)$/;
    ...
    */
    ...
    // parse
    this.re_parse_pathfunc      = /^&([A-Za-z][A-Za-z0-9]*)\{(.*)$/;
    this.re_pathvar_indexed     = /^&([A-Za-z][A-Za-z0-9]*)\[(\d+)\]\s*(.*)$/;
    this.re_pathvar_underscored = /^&([A-Za-z][A-Za-z0-9]*)_(\d+)\s*(.*)$/;
    this.re_pathvar_single      = /^&([A-Za-z]\w*)\s*(.*)$/;
    this.re_dashdot = /^(~|\.\.)\s*(.*)$/;
    this.re_coord = /^\((.*?)\)\s*(.*)$/;
  ...
  exec_body(lines,env){
    ...
    while (lines.length) {
      ...
      if((v = this.re_group_command.exec(line)) !== null) {
        var id = v[1];
        var {style:g} = this.string_to_style(v[2]);
  ...
  exec_command_line(line) {
    ...
    ///
    if(suffix=='._') {
      var id = this.assert_int(this.id,0,0);
      this.id = ++id;
      opt = `${id}`
      opts = [opt];
      suffix = `.${id}`;
    ...
    else if (cmd=='fillregion'){
      o.push(this.do_fillregion(opts,g,txt,coords));//own method
    }
    ///
    ///NOTE: prodofprimes
    ///
    ...
    ///
      ...
      this.move_coords(coords,this.refx,this.refy,this.refsx,this.refsy,0);
      let s0 = this.coords_to_string(coords);
      this.do_comment(`***move_coords=${s0} refx=${this.refx} refy=${this.refy} refsx=${this.refsx} refsy=${this.refsy}`);
    ...
    switch (name) {
      ...
      case 'circle': {
        ...
        this.is_arg_scalar(args[1])) {
          ...
          let r = args[1].scalar; 
          ret_val.push([z[0]+r,z[1],'', '','','','', 0,0,0,0,0, z[12]]);
      ...
      case 'ellipse': {
        ...
        ret_val.push([x1,y1, '', '','','','', 0,0,0,0,0, z0[12]]);
      ...
      case 'parallelogram': {
        ...
        if(SX >= 0){
          ...
          ret_val.push([x,y, '', '','','','', 0,0,0,0,0, pt[12]]);
          ...
          ret_val.push([x+SX,y, '', '','','','', 0,0,0,0,0, pt[12]]);
      ...
      case 'rhombus': {
        ...
        let W = w/2;
        let H = h/2;
        ret_val.push([x+W,y+0, '', '','','','', 0,0,0,0,0, pt[12]]);
        ret_val.push([x+w,y+H, 'L']);
        ret_val.push([x+W,y+h, 'L']);
      ...
      case 'roundrectangle': {
        ...
        let dx = 0.2*w; 
        let dy = 0.2*h; 
        ret_val.push([x+0,    y+dy,   '', '','','','', 0,0,0,0,0, pt[12] ]);
      ...
      case 'triangle': {
        ...
        if(args.length == 3 &&
          ...
          this.is_arg_coords(args[2])) {
            ...
            let z1 = this.point(args[1].coords,0);
            let z2 = this.point(args[2].coords,0);
            ret_val.push([z0[0],z0[1],'', '','','','', 0,0,0,0,0, z0[12]]);
      ...
      case 'polyline': {
        ...
        args.forEach((arg,i,arr) => {
          if(this.is_arg_coords(arg)){
            ...
            if(i==0){
              ret_val.push([z[0],z[1],'', '','','','', 0,0,0,0,0, z[12]]);
            }else{
      ...
      case 'arctravel': {
        ...
        this.is_arg_scalar(args[2]) )
        {
          ...
          ret_val.push([px,py,'', '','','','', 0,0,0,0,0, c[12]]);
      ...
      case 'cylinder': {
        ...
        ret_val.push([z0[0]-rx,z0[1]+ht], '', '','','','', 0,0,0,0,0, z0[12]);
      ...
      case 'equilateraltriangle': {
        ...
        angles.forEach((angle,i,arr) => {
          ...
          if(i==0){
            ret_val.push([x,y,'', '','','','', 0,0,0,0,0, z[12]]);
          }else{
            ret_val.push([x,y,'L']);
          }
        });
  ...
  parse_pathparm(s){
    ...
    while(s.length){
      if((v=re_point.exec(s))!==null){
        ...
        let s1 = v[1];
        s = v[2];
        let coords = this.get_path_from_variable(s1);
        d.push({coords});
        if (s.charAt(0) == ',') {
          s = s.slice(1).trimLeft();
        }
  ...
  read_coords(line) {
    ...
    while (line.length) {
      ...
      if ((v = this.re_pathfunc.exec(line)) !== null) {
        ...
        var fun_parm = v[2];
        line = v[3];
        var ret_val = this.read_path_func(fun_name,fun_parm);
        for (var i = 0; i < ret_val.length; ++i) {
          let pt = this.point(ret_val, i);
      ...
      */
      ...
      if ((v = this.re_pathvar_indexed.exec(line)) !== null) {
        let symbol = v[1];
        let index = v[2];
        line = v[3];
        let i = parseInt(index);
        let from = this.get_path_from_variable(symbol);
        if (from.length && Number.isFinite(i) && i < from.length) {
 ...
 /*
      if ((v = this.re_pathvar_underscored.exec(line)) !== null) {
        let symbol = v[1];
        let index = v[2];
        line = v[3];
        let i = parseInt(index);
      ...
      if ((v = this.re_relative.exec(line)) !== null) {
        ...
        if (fun_name.length) {
          ...
          /// [a:rx,ry,angle,bigarcflag,sweepflag,dx,dy]
            ...
            if(val=='last'){
              //NOTE:[offset:last]
              offset[0] = this.my_offset[0];
              offset[1] = this.my_offset[1];
            }else if(val.startsWith('&')){
              //NOTE:[offset:&a]
              let symbol = val.slice(1);
              let p = this.get_path_from_variable(symbol);
              if(p.length){
                let pt = p[0]; 
                offset[0] = pt[0];
                offset[1] = pt[1];
              ...
              var val = val.split(',');
              var val = val.map(x => x.trim());
              var val = val.map(x => parseFloat(x)||0);
              var val = val.filter(x => Number.isFinite(x));
              if (val.length == 2) {
                offset[0] += val[0];
                offset[1] += val[1];
                /// 'lastpt' is not changed
    ...
    if((v=re.exec(symbol))!==null){
      ...
      for (var i = 0; i < segs.length; ++i) {
        ...
        if (this.re_path_name.test(seg)) {
          ...
          this.do_comment(`***SAVED: ${seg}=${this.coords_to_string(this.my_path_map.get(seg))}`);
        }
      } 
      ...
      this.do_comment(`***SAVED: ${symbol}=${this.coords_to_string(this.my_path_map.get(symbol))}`);
  ...
  */
  ...
  do_label(opts,g,txt,coords){
    ...
    for (var i = 0; i < coords.length; ++i){
      ...
      var ta = this.fetch_opt_at(opts,i,g);
  ...
  g_to_lineseg_int(g){
    g = this.update_style_with_group(g);
    return this.get_int_prop(g,'lineseg',0);
  }
  ///
  ///
  ///
  ...
  draw_text_between_points(txt,g,coords){
    ...
    if(coords.length){
      for(var j=0; j < coords.length; j++){
        ...
        if(i>=0 && j>=0){
          var label = this.fetch_label_at(txt,i,g);
          var [x1,y1,join1] = this.point(coords,i);
          var [x2,y2,join2] = this.point(coords,j);
          if(join1=='cycle'){
            var [x1,y1] = this.point(coords,0);
          }
          if(join2=='cycle'){
            var [x2,y2] = this.point(coords,0);
          }
          var x = (x1 + x2)*0.5;
          var y = (y1 + y2)*0.5;
          if(shift){
  ...
  get_line_segs(pt,r,linesegs){
    var w = this.viewport_width;
    var h = this.viewport_height;
    var all = [];
    console.log('james','pt=',pt);
    console.log('james','r=',r)
    console.log('james','linesegs=',linesegs)
    ...
    for(var i=0; i < 360; i++){
      ...
      for(var j=-4; j < linesegs.length; ++j){
        ...
        if(Math.abs(theta-ang_rad) < this.MIN && dist < dist_saved){
          dist_saved = dist;
          j_saved = j;
          x_saved = x;
          y_saved = y;
      ...
      ///add to the database
      all.push([ang_deg,dist_saved,j_saved, x_saved,y_saved]);
    }
    console.log('james, all=',all);
