...
class NitrilePreviewDiagram extends NitrilePreviewBase {
  ...
  constructor(translator) {
    ...
    this.re_range_three = /^([^:]+):([^:]+):([^:]+)$/;
    /// regular expression
    this.re_foreach_loop = /^foreach\s*\[(.*?)\]\s*\[(.*?)\]\s*:$/;
    ...
    this.re_longdiv_command = /^longdiv(\.[\.\w]+|)\s+(.*)$/;
    ...
    this.re_symbol_complete = /^[A-Za-z][A-Za-z0-9]*$/;
  ...
  exec_body(lines,env){
    ...
    while (lines.length) {
      ...
      if ((v = this.re_foreach_loop.exec(line)) !== null) {
        ...
        this.exec_foreach_body(var_array, seq_array, body, env);
        continue;
      }
  ...
  exec_foreach_body(var_array,seq_array,body,env){
  ...
  get_int_prop(g, name, def_v, min=null, max=null) {
    let val;
    if(g && g.hasOwnProperty(name)){
      val = g[name];
    }else if(this.config.hasOwnProperty(name)){
    ...
    return this.assertInt(val,def_v,min,max);
  }
  get_string_prop(g, name, def_v) {
    ...
    return this.assertString(val,def_v);
  }

  assertInt(val, def_v, min=null, max=null) {
    val = parseInt(val);
  ...
  fetch_arg(s){
    let {coords} = this.read_coords_line(s);
  ...
  read_coords_line(line) {
    ...
    while (line.length) {
      ...
      if((v=this.re_action_label1.exec(line))!==null){
        txt=v[2];
        ts=1;
        line=v[3];
        continue;
      ...
      if((v=this.re_action_label2.exec(line))!==null){
        txt = v[2];
        ts = 2;
        line = v[3];    
        continue;
      ...
      if ((v = this.re_action_style.exec(line)) !== null) {
        let g1 = this.string_to_style(v[1].trim());
        g = {...g, ...g1};
        line = v[2];
        continue;
      }  
  ...
  read_action_coords(line) {
    var {coords,g,txt,ts,line} = this.read_coords_line(line);
    for(var j=0; j < coords.length; ++j){
      var join = coords[2];
  ...
  exec_line(line,env) {
    ...
    if((v=re_env.exec(line))!==null){
      let env_name = v[1];
      let env_expr = v[2];
      if(this.re_symbol_complete.test(env_name)){
    ...
    if ((v = this.re_set_command.exec(line)) !== null) {
      ...
      if (key === 'refx') {
        ...
        if (val[0] === '/') { /// '/3'
          val = val.slice(1);
          val = this.assertFloat(val, this.def_refx, 0, this.viewport_width);
        } else if (val[val.length - 1] === '/') { /// '3/'
          val = val.slice(0, val.length - 1);
          val = this.viewport_width - this.assertFloat(val, this.def_refx, 0, this.viewport_width);
        } else {
          val = this.assertFloat(val, this.def_refx, -this.viewport_width, this.viewport_width);
        }
        this.refx = val;
      } ///such as 12.5
      else if (key === 'refy') {
        ...
        if (val[0] === '/') { /// '/3'
          val = val.slice(1);
          val = this.viewport_height - this.assertFloat(val, this.def_refy, 0, this.viewport_height);
        } else if (val[val.length - 1] === '/') { /// '3/'
          val = val.slice(0, val.length - 1);
          val = this.assertFloat(val, this.def_refy, 0, this.viewport_height);
        } else {
          val = this.assertFloat(val, this.def_refy, -this.viewport_height, this.viewport_height);
        }
        this.refy = val;
      } ///such as 12.5
      else if (key === 'refs') { 
        this.refsx = this.assertFloat(val, this.def_refsx, this.MIN, this.MAX); 
        this.refsy = this.assertFloat(val, this.ref_refsy, this.MIN, this.MAX);
      } 
      else if (key === 'refsx') {
        this.refsx = this.assertFloat(val, this.def_refsx, this.MIN, this.MAX); 
      }
    ...
    if ((v = this.re_label_command.exec(line)) !== null) {
      ...
      var {coords,g} = this.read_action_coords(line);
      this.commands.push(this.do_label(opt,g,coords));///own method
      return false;
    }
    ///
    ...
    if ((v = this.re_drawanglearc_command.exec(line)) !== null) {
      ...
      //var [g,line] = this.readActionStyle(line);
      var {coords,g} = this.read_action_coords(line);
      this.commands.push(this.do_drawanglearc(opt,g,coords));//own method
      return false;
    }
    ///
    ///NOTE: rec
    ...
    if ((v = this.re_rec_command.exec(line)) !== null) {
      ...
      if(opt){
        ...
        if(cmd=='playback'){
          //var [txt,ts,line] = this.readActionLabel(line);
          //var [g,line] = this.readActionStyle(line);
          var {coords,g} = this.read_action_coords(line);///path is to be modified by refx/refy/refsx/refsy    
          this.commands.push(this.to_playback_rec(rec,g,coords));//own method
        }else{
          //var [txt,ts,line] = this.readActionLabel(line);
          //var [g,line] = this.readActionStyle(line);
          var {coords,g} = this.read_coords_line(line);//path is not modified by current settings of refx/refy/refsx/refsy    
          let a = {};
          a.cmd = cmd;
          a.cmdopt = cmdopt;
          a.p = coords;
    ...
    if ((v = this.re_prodofprimes_command.exec(line)) !== null) {
      ...
      //var [txt,ts,line] = this.readActionLabel(line);
      //var [g,line] = this.readActionStyle(line);
      var {coords,g,txt} = this.read_action_coords(line);
    ...
    ///NOTE: longdiv
    ///
    if ((v = this.re_longdiv_command.exec(line)) !== null) {
      var opt = this.remove_first_dot(v[1]);
      var line = v[2];
      //var [txt,ts,line] = this.readActionLabel(line);
      //var [g,line] = this.readActionStyle(line);
      ...
      var x = z[0];
      var y = z[1];
      this.commands.push(this.to_longdiv(x,y,g,txt));//own method
      return false;
    }
    ///
    ///NOTE: multiws
    ...
    if ((v = this.re_node_command.exec(line)) !== null) {
      ...
      //var [g,line] = this.readActionStyle(line);
      var {coords,g} = this.read_action_coords(line);
      this.commands.push(this.do_node(opts,g,coords));///own method
      return false;
    }
    if ((v = this.re_edge_command.exec(line)) !== null) {
      ...
      //var [txt,ts,line] = this.readActionLabel(line);
      //var [g,line] = this.readActionStyle(line);
      var {coords,g,txt,ts} = this.read_action_coords(line);
    ...
    if ((v = this.re_box_command.exec(line)) !== null) {
      ...
      //var [g,line] = this.readActionStyle(line);
      var {coords,g} = this.read_action_coords(line);
      this.commands.push(this.do_box(opt,g,coords));///own method
      return false;
    }
    ///
    /// Scalar commands   
  ...
  */
  ...
  exec_scalar_func(func_name,func_args,z=0) {
    switch(func_name) {
      ...
      case 'sin':
        ...
        break;
      case 'Cos':
        return Math.cos(func_args[0]/180*Math.PI);
        break;
      case 'Sin':
        return Math.sin(func_args[0]/180*Math.PI);
        break;
      case 'sqrt':
  ...
  do_cartesian(opt,txt,ts,g,floats) {
    ...
    switch( cmd ) {
      ...
      case 'arc'://CARTESIAN
        ...
        var start_a = this.assertFloat(floats[3],0);
        var span_a  = this.assertFloat(floats[4],45);
  ...
  do_label(opt,g,coords){
    ...
    for (var i = 0; i < coords.length; ++i){
      ...
      var txt = pt.txt;
      var ts = pt.ts;
      o.push(this.p_label(x,y,txt,ts,opt,g));
    }
    return o.join('\n');
  }
  ...
  to_playback_rec(rec,g,coords){
    ...
    for (var i = 0; i < coords.length; i++) {
      ...
      var theta = this.assertFloat(g.theta, 0);
      for(let a of rec){
        let g = a.g;
        let p = a.p;
  ...
  do_drawanglearc(opt,g,coords){   
    var o = [];
    var r = this.assertFloat(g.r,0.75,0,this.MAX);
    var gap = this.assertFloat(g.gap,1,0,this.MAX);
  ...
  do_node(opts,g,coords){
    ...
    coords.forEach((pt,i) => {
      ...
      if(txt){
        ...
        ///obscured
        let label = this.fetch_label_at(txt,i);
        if(label){
          o.push(this.p_label(pt[0],pt[1],label,ts,'ctr',g));
        }
      }
      if(name){
    ...
    for(let i=1; i < opts.length; ++i){
      ...
      if(this.my_nodes.has(name1)&&this.my_nodes.has(name2)){
        ...
        if(name1==name2){
          ...
          let [p0,p1,p2,p3] = this.dir_to_cbezier_loop(dir, node1.pt[0],node1.pt[1],node1.r);
          ...
          var dir = this.assertFloat(g.dir,0);
          let [p0,p1,p2] = this.dir_to_qbezier(dir, node1.pt[0],node1.pt[1],node1.r, node2.pt[0],node2.pt[1],node2.r);        
          if(dir){
  ...
  dir_to_cbezier_loop(dir, x1,y1,r1){
    var p0 = [0,0];
    var p1 = [0,0];
    var p2 = [0,0];
    var p3 = [0,0];
    var r2 = r1+r1+r1;
    var sw = 22.5;
    p1[0] = x1 + r2*Math.cos((dir-sw)/180*Math.PI);
    p1[1] = y1 + r2*Math.sin((dir-sw)/180*Math.PI);
    p2[0] = x1 + r2*Math.cos((dir+sw)/180*Math.PI);
    p2[1] = y1 + r2*Math.sin((dir+sw)/180*Math.PI);
    p0[0] = x1 + r1*Math.cos((dir-sw)/180*Math.PI);
    p0[1] = y1 + r1*Math.sin((dir-sw)/180*Math.PI);
    p3[0] = x1 + r1*Math.cos((dir+sw)/180*Math.PI);
    p3[1] = y1 + r1*Math.sin((dir+sw)/180*Math.PI);
    return [p0,p1,p2,p3];
  }
  ...
  do_box(opt,g,coords){
    var o = [];
    var baseskip = this.get_float_prop(g,"baseskip",1);
    var box = this.get_string_prop(g,"box","");
    ...
    for (var j = 0; j < coords.length; j++) {
      ...
      if (txt) {
        ...
        var n = txt_ss.length;
        var multi = this.to_multiline(n,labelx,labely,'ctr',baseskip);
  ...
  fetch_pic_from_notes(id){
    if(this.translator.notes.has('pic')) {
      let pics = this.translator.notes.get('pic');
      ///'pics' is an JS Array
      for(let pic of pics){
  ...
  to_angle_str(g){
    ...
    return this.assertFloat(parseFloat(angle),0,0,90);
  }
  to_shade_str(g){
    let shade = g.shade;
    if(!shade){
  ...
  to_linedashed_str(g){
    let linedashed = g.linedashed;
    if(!linedashed){
      linedashed = this.config.linedashed;
    }
    return linedashed;
  }
  ...
  to_multiws(x,y,g,txt){
    ...
    var answerfontcolor = this.get_string_prop(g,'answerfontcolor','');
    var decimalpoint = this.get_string_prop(g,'decimalpoint','');
    var decimalpoints = this.string_to_int_array(decimalpoint);
    var decimalpointsize = this.get_int_prop(g,'decimalpointsize',2);
    var ss = this.string_to_array(txt);
    var number = parseInt(ss[0])||'';
    var divisor = parseInt(ss[1])||'';
    var d = [];
    ...
    if(number && divisor){
      ...
      ///decimal points
      let q1 = decimalpoints[0]||0;
      let q2 = decimalpoints[1]||0;
      if(q1){
        /// draw the decimal point for the first number
        let g1 = {...g, dotsize:decimalpointsize}
        d.push(this.p_dot_circle(x+n-q1,y-1+0.2,g1));
      }
      if(q2){
        /// draw the decimal point for the second number
        let g2 = {...g, dotsize:decimalpointsize}
        d.push(this.p_dot_circle(x+n-q2,y-2+0.3,g2));
      }
      ///if we need also to provide answer
      if(answer){
        let q3 = q1 + q2;
        let ga = {...g,fontcolor:answerfontcolor};
        ...
        for(let j=m;j>0;j--,dx++){
          ...
          for(let i=n;i>0;i--){
            ...
            d.push(this.draw_letter_string_flushright(x+i-dx,y-dy,p,ga));
          }
        }
        if(dy>2){
          dy++;
          ...
          sum = `${sum}`;
          d.push(this.draw_letter_string_flushright(x+n,y-dy,sum,ga));
          d.push(this.p_line(x-m,y-dy,x+n,y-dy,ga));
          if(q3){
            /// draw the decimal point for the second number
            let g3 = {...g, dotsize:decimalpointsize, dotcolor:answerfontcolor}
            d.push(this.p_dot_circle(x+n-q3,y-dy-1+0.3,g3));
          }
        }else{
          if(q3){
            /// draw the decimal point for the second number
            let g3 = {...g, dotsize:decimalpointsize, dotcolor:answerfontcolor}
            d.push(this.p_dot_circle(x+n-q3,y-3+0.3,g3));
          }
        }
      }
    }
