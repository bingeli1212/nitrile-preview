...
class NitrilePreviewDiagram extends NitrilePreviewBase {
  ...
  constructor(translator) {
    ...
    this.re_range_three = /^([^:]+):([^:]+):([^:]+)$/;
    /// regular expression
    this.re_let_command = /^let(\.[\.\w]+|)\b\s*(.*)$/;
    this.re_foreach_loop = /^for(\.[\.\w]+|)\b\s*(.*)$/;
    this.re_word = /^[A-Za-z][A-Za-z0-9]*$/;
    this.re_commentline = /^\%/;
  ...
  exec_line(line,env) {
    ...
    if ((v = this.re_let_command.exec(line)) !== null) {
      var opts = this.string_to_command_opts(v[1]);
      var line = v[2];
      var [a,line] = this.read_action_token(line);
      var [op,line] = this.read_action_token(line);
      var [quan] = this.extract_next_expr(line,{});
      env[a] = quan;
      return false;
    }
    ///
