...
class NitrilePreviewDiagram extends NitrilePreviewBase {
  ...
  exec_line(line,env) {
    ...
    var coords = this.read_action_coords(data);
    var floats = this.read_action_floats(data);
    var [opt] = opts;
    ///
    ...
    ///
      ...
      o.push(this.do_flow(opts,g,txt,coords));///own method
    }
    ///
  ...
  exec_path_func(line, fun_str, args) {
    ...
    switch (fun_str) {
      ...
      case 'scale': {
        ...
        Z = this.offset_and_scale_coords(Z,0,0,sx,sy,0);
        for(let z of Z){
          ret_val.push(z);
        }
        break;
      ...
      case 'rotate': {
        ...
        Z = this.offset_and_scale_coords(Z,0,0,1,1,rotate);
        for(let z of Z){
          ret_val.push(z);
        }
        break;
      ...
      case 'translate': {
        ...
        Z = this.offset_and_scale_coords(Z,offsetX,offsetY,1,1,0);
        for(let z of Z){
          ret_val.push(z);
        }
        break;
  ...
  */
  ...
  do_box(opts,g,txt,coords){
    ...
    for (var j = 0; j < coords.length; j++) {
      ...
      var x = pt[0];
      var y = pt[1];
      var [x,y] = this.align_box(x,y,w,h,boxalign);
      var v = [x,y,w,h,boxtype];
      if(boxtype=='hexgon'){
  ...
  do_flow(opts,g,txt,coords){
    ...
    for(var j=1; j<opts.length; ++j){
      var [id1,anchor1] = this.string_to_id_and_anchor(opts[j-1]);
      var [id2,anchor2] = this.string_to_id_and_anchor(opts[j]);
      var s = this.to_flowed_lines(id1,anchor1,id2,anchor2);
      o.push(s); 
    }
    return o.join('\n');
  }
  ...
  g_to_dx_float(g){
    return this.get_string_prop(g,'dx',0);
  }
  g_to_dy_float(g){
    return this.get_string_prop(g,'dy',0);
  }
