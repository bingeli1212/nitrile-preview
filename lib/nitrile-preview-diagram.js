...
class NitrilePreviewDiagram extends NitrilePreviewBase {
  ...
  constructor(translator) {
    ...
    this.re_else_command = /^else\s+(.*)\s*then$/;
    this.re_elif_command = /^elif\s+(.*)\s*then$/;
    this.re_if_command = /^if\s+(.*)\s*then$/;
    ...
    /// %...
    this.re_is_commentline = /^(\%+)(.*)$/;
    /// @"%d %f %s"
    this.re_is_formattingtemplate = /^@\"(.*?)\"\s*(.*)$/;
    /// a, a0, a0a
    ...
    this.re_numbers_is_array         = /^@([A-Za-z][A-Za-z0-9]*)\s*(.*)$/;
  ...
  join_body_lines(style,lines) {
    ...
    const re_buf_stop = /^(%%%)/;
    for(var s of lines){
      if((v=this.re_is_commentline.exec(s))!==null){
  ...
  exec_body(style,body,env){
    ...
    while (lines.length) {
      ...
      if((v=this.re_var_command.exec(line))!==null){
        ...
        if(this.re_is_symbolname.test(name)){
          if(atsign.length){
            ...
            env[name] = numbers;
            this.do_comment(`*** env @${name} = ${this.array_to_string(numbers)}`);
          }else if((v=this.re_is_formattingtemplate.exec(s1))!==null){
            let fmt = v[1];
            let args = this.string_to_array(v[2]);
            let s2 = this.do_format(fmt,args,env);
            env[name] = s2;
            this.do_comment(`*** env ${name} = \"${s2}\"`);          
          }else{
            let [a] = this.expr.extract_next_expr(s1,env);
            env[name] = a;
            this.do_comment(`*** env ${name} = ${this.array_to_string([a])}`);          
          }
        }else{
      ...
      if((v=this.re_if_command.exec(line)) !== null) {
        ...
        let thebody = this.eval_ifthen_hash(thehash,env);//it could return null
        if(thebody){
          thebody = this.trim_samp_body(thebody);
      ...
      if((v=this.re_for_command.exec(line)) !== null) {
        ...
        for(var i=0; i < max_n; ++i){
          ...
          for(var j=0; j < iter_array.length; ++j){
            ...
            let key = iter.name;
            if(i < iter.seq.length){
              /// update the 'env' variable only if we still have leftover items
              let val = iter.seq[i];
              env[key] = val;
  ...
  eval_ifthen_hash(hash,env){
    var ret_val = null;
    hash.forEach((l) => {
      let [key,expr,body] = l;
      if(key=='if' || key=='elif'){
        let [flag] = this.expr.extract_next_expr(expr,env);
        if(flag.re == 1){
          ret_val = body;
          return;
        }
  ...
  extract_ifthen_hash(style,lines,expr){
    ...
    while(lines.length) {
      ...
      if((v=this.re_elif_command.exec(line))!==null){
        expr = v[1];
        body = [];
        hash.push(['elif',expr,body]);
        continue;
      }
      if((v=this.re_else_command.exec(line))!==null){
        expr = v[1];
        body = [];
        hash.push(['else',expr,body]);
        continue;
      }
      body.push(line);
      lines.shift();//remove the first line
  ...
  exec_action_command(style,line,env) {
    ...
    ///
    ///NOTE: replace box.@.@.@ with box.0.1.2 where '0','1','2' are auto-assigned IDs
    ///
    opts = opts.map((x) => {
      if(x === '_'){
    ...
    ///
      ...
      o.push(this.do_cartesian(opts,g,txts,numbers,env));///own method
    }
    ///
  ...
  get_path_from_symbol(symbol) {
    ...
    if(symbol=='lastpt'){
      return [[this.my_lastpt_x,this.my_lastpt_y,'M']];
    }
    if(symbol=='northwest'){
    ...
    switch (name) {
      ...
      case 'rotatepoint': {
        ...
        if (args.length == 4 && 
          ...
          let z0 = this.point(args[0].coords, 0);
          let z1 = this.point(args[1].coords, 0);
          let rot  = args[2].scalar;
          let dist = args[3].scalar;
          if (this.isvalidpt(z0) && this.isvalidpt(z1) && this.isvalidnum(dist)) {
            let z0x = parseFloat(z0[0]);
            let z0y = parseFloat(z0[1]);
            let z1x = parseFloat(z1[0]);
            let z1y = parseFloat(z1[1]);
      ...
      case 'circlecircleintersectclip': {
        ///constructs a new path that is the intersection of two circles;
      ...
      case 'circlecirclediffclip': {
        ///constructs a new path with the remains of the first
      ...
      case 'rectanglecircleholeclip': {
        ///constructs a new path with the remains of the rectangle
  ...
  __read_style_and_label(line) {
    ...
    while(line.length){
      if((v=this.re_coords_is_style.exec(line))!==null) {
        ...
        style = {...style,...g};
      }else if((v=this.re_coords_is_label.exec(line))!==null) {
        //parse text
        line = v[2];    
        let s = v[1].trim();
  ...
  extract_pathfunc_args(s,env){
    ...
    const s0 = s;
    const re_pnt = /^(\(.*?\))\s*(.*)$/;
    const re_pat = /^(&[A-Za-z]\w*)\s*(.*)$/;
    const re_elm = /^([\+\-]?[A-Za-z][A-Za-z0-9]*\[\d+\])\s*(.*)$/;
    const re_var = /^([\+\-]?[A-Za-z][A-Za-z0-9]*)\s*(.*)$/;
    ...
    while(s.length){
      ...
      }else if((v=re_cma.exec(s))!==null){
        s = v[2];
        pt = {coords:[],scalar:NaN}
        d.push(pt);
      }else{
        break;
      }
  ...
  read_coords(line,env) {
    ...
    while (line.length) {
      ...
      if ((v = this.re_coords_is_aux.exec(line))!==null) {
        //^right:2, ^up:2, ^down:2, ^left:2, ^x:2, ^X:2, ^y:2, ^Y:2, ^s:2, ^sx:2, ^sy:2, ^at:center, ^pt:a, ^hint:linedashed
        let key   = v[1];
        let colon = v[2];
        let val   = v[3];
        line      = v[4];
        ...
        switch (key){
          ...
          case 'lasty': {
            //lasty:2
            if(Number.isFinite(num)){
              this.my_lastpt_y = num;
            }
            break;
          }
          case 'lastpt': {
            // 'lastpt:a' - save the lastpt 
            let symbol = val;
            if(this.re_is_symbolname.test(symbol)){
              let pt = [0,0,'M','','','','', 0,0,0,0,0, 0];
      ...
      if((v=this.re_coords_is_point.exec(line))!==null){
        ...
        }else if(s.startsWith('@cartesian.')){    
          /// '@cartesian.1,-2,1'    
          s = s.slice(11);
          var [id,dx,dy] = s.split(',').map((x)=>x.trim());
          if(this.my_cartesian_map.has(id)){
            var p = this.my_cartesian_map.get(id); 
        ...
        if (dashdot.charAt(0) === '.') {
          ...
          for (var pt of from){
            ...
            hobby_p = [];
            hobby_p.push([x,y]);          
            let [pt0,pt1,pt2] = this.abr_to_qbezier(abr, this.my_lastpt_x,this.my_lastpt_y, x,y);  
            pt[2] = 'Q';
            pt[3] = pt1[0];
            pt[4] = pt1[1];
            pt[5] = '';
      ...
      if ((v = this.re_coords_is_relative.exec(line)) !== null) {
        ...
        if (key === 'angledist') {
          ...
          if (val.length == 2) {
            var tx = this.my_lastpt_x + val[0];
            var ty = this.my_lastpt_y + val[1];
          ...
          if (val.length == 6) {
            x1 = this.my_lastpt_x + val[0];
            y1 = this.my_lastpt_y + val[1];
            x2 = this.my_lastpt_x + val[2];
            y2 = this.my_lastpt_y + val[3];
            x = this.my_lastpt_x + val[4];
            y = this.my_lastpt_y + val[5];
          ...
          var lastjoin = '';
          if(coords.length){
            lastjoin = coords[coords.length-1][2];
          }
          if (val.length == 4) {
            if (lastjoin === 'Q') {
            ...
            x2 = this.my_lastpt_x + val[0];
            y2 = this.my_lastpt_y + val[1];
            x = this.my_lastpt_x + val[2];
            y = this.my_lastpt_y + val[3];
            if (Number.isFinite(x1) &&
              Number.isFinite(y1) &&
              Number.isFinite(x2) &&
              Number.isFinite(y2) &&
          ...
          if (val.length == 7) {
            ...
            let X2 = this.my_lastpt_x + val[5];
            let Y2 = this.my_lastpt_y + val[6];
            let Rx = val[0];
            let Ry = val[1];
            let Phi = val[2];
          ...
          if (val.length == 2) {
            x = this.my_lastpt_x + val[0];
            y = this.my_lastpt_y + val[1];
            if (lastjoin === 'Q') {
              [x1,y1] = this.MIRROR([x1,y1],[this.my_lastpt_x,this.my_lastpt_y]);
            } else if (lastjoin === 'C') {
              [x1,y1] = this.MIRROR([x2,y2],[this.my_lastpt_x,this.my_lastpt_y]);
  ...
  read_array(line,env) {
    ...
    while(line.length) {
      ...
      /// @a
      if((v=this.re_numbers_is_array.exec(line))!==null){
        let symbol = v[1];
        line = v[2];
  ...
  do_path_command(symbol,coords,env){
    ...
    const re_special = /^@([A-Za-z])$/;
    var v;
    if((v=re_special.exec(symbol)) !== null){
      var c = v[1];
      var i = parseInt(env['@'])||0;
      symbol = String.fromCodePoint(c.codePointAt(0)+i);
      if(this.re_is_symbolname.test(symbol)){
  ...
  do_cartesian(opts,g,txts,numbers,env) {
    let floats = this.numbers_to_floats(numbers);
    ...
    var A = {};
    if (this.my_cartesian_map.has(id)) {
      A = this.my_cartesian_map.get(id);
    }else{
      this.my_cartesian_map.set(id,A);
      A.xorigin = 0;
      A.yorigin = 0;
      A.xgrid = 1;
      A.ygrid = 1;
  ...
  do_lego(subcmd,opts,g,txts,numbers) {
    ...
    switch( subcmd ) {
      ...
      case 'show2'://LEGO
        ...
        A.q1 = [[0,wh],[0-wx,wh-wy,'L'],[0,wh-wy-wh,'L'],[0+wx,wh-wy,'L'],[0,0,'z']];//top
        A.q2 = [[0-wx,0-wy],[0-wx,wh-wy,'L'],[0,wh-wy-wy,'L'],[0,0-wy-wy,'L'],[0,0,'z']];//left
        A.q3 = [[0+wx,0-wy],[0+wx,wh-wy,'L'],[0,wh-wy-wy,'L'],[0,0-wy-wy,'L'],[0,0,'z']];//right
