...
class NitrilePreviewDiagram extends NitrilePreviewBase {
  ...
  exec_body(style,body,env){
    ...
    while (lines.length) {
      ...
      if((v=this.re_arr_command.exec(line))!==null){
        let key = v[1];
        let val = v[2];
        let numbers = this.expr.read_arr(val,env);//returns an array 'Complex' number
        env[key] = numbers;
  ...
  extract_pathfunc_args(s,env){
    ...
    while(s.length){
      if((v=re_cma.exec(s))!==null){
        ...
        let s1 = v[1];
        s = v[2];
        let q = this.expr.read_arr(s1,env);
        q.forEach((x) => {
          pt.array.push(x);
        })
      }
  ...
  string_to_range(s){
    ...
    var is_colon = 0;
    var v;
    if ((v=this.re_is_range_two.exec(s))!==null) {
      var base = v[1];
      var limit = v[2];
      var step = 1;
      is_colon = 1;
    } else if ((v = this.re_is_range_three.exec(s)) !== null) {
      var base = v[1];
      var step = v[2];
      var limit = v[3];
      is_colon = 1;
