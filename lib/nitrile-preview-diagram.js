...
class NitrilePreviewDiagram extends NitrilePreviewBase {
  ...
  read_coords(line) {
    ...
    while (line.length) {
      ...
      if ((v = this.re_flow.exec(line)) !== null) {
        ...
        n++
        var hints = 0;
        let [Rx,Ry] = v[1].split(',').map(s => s.trim()).map(s => parseFloat(s));
        line = v[2];
        Rx = Rx||0;
        Ry = Ry||0;
        let pt = [0, 0, '<flow>', '','','','', Rx,Ry,0,0,0, hints];
        coords.push(pt);
        continue;
      }
  ...
  do_flow(opts,g,txt,coords){
    ...
    var [id2,anchor2] = this.string_to_id_and_anchor(opts[1]||'');
    var q = [];
    var lastx = 0;
    var lasty = 0;
    if(this.my_box_map.has(id1)){
      var v1 = this.my_box_map.get(id1);
      var [x1,y1,w1,h1,rdx1,rdy1,boxtype1] = v1;
    ...
    if(this.my_box_map.has(id2)){
      var v2 = this.my_box_map.get(id2); 
      var [x2,y2,w2,h2,rdx2,rdy2,boxtype2] = v2;
