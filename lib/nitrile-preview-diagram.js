...
class NitrilePreviewDiagram extends NitrilePreviewBase {
    ...
    while (line.length) {
      ...
      if((v=this.re_coords_is_point.exec(line))!==null){
        ...
        if(s.startsWith('(')&&s.endsWith(')')){
          ...
          ///<box.a:nw,10,10> 
          n++
          let flow = s.slice(1,-1);
          line = v[2];
          var [ac,dx,dy] = flow.split(',').map(s=>s.trim());
          var [article,id,anchor] = this.string_to_article_id_anchor(ac);
    ...
    for(let j=1; j < opts.length; ++j){
      ...
      if(this.my_node_map.has(name1)&&this.my_node_map.has(name2)){
        ...
        if(name1==name2){
          ...
          p0 = this.update_node_point(p0[0],p0[1],p1[0],p1[1],r1,nodetype1);
          p2 = this.update_node_point(p2[0],p2[1],p1[0],p1[1],r2,nodetype2);
  ...
  update_node_point(x1,y1,x2,y2,r,nodetype){
    var dy = y2-y1;
    var dx = x2-x1;
  ...
  to_node_anchor_relative(angd,r,nodetype){
  ...
  to_node_anchor_point(p,anchor){
    ...
    var [dx,dy] = this.to_node_anchor_relative(angd,r,nodetype);
    x += dx;
    y += dy;
    return [x,y];
  }
