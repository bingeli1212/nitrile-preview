...
class NitrilePreviewDiagram extends NitrilePreviewBase {
  ...
  constructor(translator) {
    ...
    /// all ss maps
    this.global_ss_map = translator.global_ss_map;
    this.MAX = 2048;
    this.MIN = 0.000001;
  ...
  init_internals(){
    ...
    this.def_dotsize = 5;
    this.def_barlength = 0.40;
    /// add following as built-in path
    this.my_path_map.set('apple',this.read_coords_line(this.string_to_builtin_path('apple')).coords)
    this.my_path_map.set('basket',this.read_coords_line(this.string_to_builtin_path('basket')).coords)
    this.my_path_map.set('crate',this.read_coords_line(this.string_to_builtin_path('crate')).coords)
    this.my_path_map.set('tree',this.read_coords_line(this.string_to_builtin_path('tree')).coords)
    this.my_path_map.set('balloon',this.read_coords_line(this.string_to_builtin_path('balloon')).coords)
  }
  ...
  exec_path_func(line, fun_str, args) {
    ...
    switch (fun_str) {
      ...
      case 'grid': {
        //NOTE: &grid{width,height,xstep,ystep}
        if(args.length == 4 &&
  ...
  rotate_point(x,y,rotation){
    var cos_theta = Math.cos(rotation/180*Math.PI);
    var sin_theta = Math.sin(rotation/180*Math.PI);
    var newx=[x*cos_theta - y*sin_theta]
    var newy=[x*sin_theta + y*cos_theta]
    return [newx,newy];
  }

  offset_and_scale_coords(coords,offsetx,offsety,xscale,yscale,rotation){
    var newcoords = [];
    for(let i=0; i < coords.length; ++i){
      let pt = coords[i];
      let newpt = this.offset_and_scale_pt(pt,offsetx,offsety,xscale,yscale,rotation);
      newcoords.push(newpt);
    }
    return newcoords;
  }

  offset_and_scale_pt(pt,offsetx,offsety,xscale,yscale,rotation){
    var newpt = [];
  ...
  read_action_style(line) {
    ...
    if ((v = this.re_action_style.exec(line)) !== null) {
      var s = v[1].trim();
      var s = `{${s}}`;
      var {style:g} = this.string_to_style(s);
      line = v[2];
    }
    return [g,line];
  }
      ...
      if ((v = this.re_relative.exec(line)) !== null) {
        ...
        if (fun_name.length) {
          ...
          if (key === 'angledist') {
            ...
            var val = val.map(x => x.trim());
            if (val.length == 2) {
              var last_angle = this.compute_last_clock(coords);
              var angle = parseFloat(val[0]);
              var angle = last_angle + angle;
              var dist = parseFloat(val[1]);
              dx = dist * Math.sin(angle / 180 * Math.PI);
              dy = dist * Math.cos(angle / 180 * Math.PI);
              if (!Number.isFinite(dx)) { dx = 0; }
              if (!Number.isFinite(dy)) { dy = 0; }
              x = lastpt[0] + dx;
              y = lastpt[1] + dy;
  ...
  exec_line(line,env) {
      ...
      if((v=re.exec(symbol))!==null){
        ...
        for (var i = 0; i < segs.length; ++i) {
          ...
          if (this.re_path_name.test(seg)) {
            ...
            this.commands.push(this.do_comment(`saved ${seg}=${this.coords_to_string(this.my_path_map.get(seg))}`));
          }
        } 
        ...
        this.commands.push(this.do_comment(`saved ${symbol}=${this.coords_to_string(this.my_path_map.get(symbol))}`));
      } else {
    ...
    if ((v = this.re_source_command.exec(line)) !== null) {
      let id = v[1];
      let source = this.fetch_source_from_ss_map(id);
      if(source) {
        this.exec_body(source,env);
      } else {
        this.commands.push(this.do_comment(`ERR:source does not exist: ${id}`));
      }
      return false;
    }
    ///
    ...
    if ((v = this.re_debug_command.exec(line)) !== null) {
      var opts = this.string_to_command_opts(v[1]);
      var line = v[2];
      this.commands.push(this.do_comment(line));///primitive method
      return false;
    }
    ///
    ...
    if ((v = this.re_drawlinesegcong_command.exec(line)) !== null) {
      var opts = this.string_to_command_opts(v[1]);
      var line = v[2];
      var [g,line] = this.read_action_style(line);
      var [txt,ts,line] = this.read_action_label(line);
      var coords = this.read_action_coords(line);
      this.commands.push(this.do_drawlinesegcong(opts,g,txt,ts,coords));//own method
      return false;
    }
    ///
    ///NOTE: rec
    ...
    if ((v = this.re_draw_command.exec(line)) !== null) {
      ...
      if(opt){
        ///is 'opt' a known path?
        if(this.my_path_map.has(opt)){
          let p = this.my_path_map.get(opt);
    ...
    ///
    if ((v = this.re_lego_command.exec(line)) !== null) {
      var opts = this.string_to_command_opts(v[1]);
      var line = v[2];
      var [g,line] = this.read_action_style(line);
      var [txt,ts,line] = this.read_action_label(line);
      var floats = this.read_action_floats(line);
      this.commands.push(this.do_lego(opts,g,txt,ts,floats));///own method
      return false;
    }
    return false;
  }
  ...
  extract_next_scalar(s,g,z=0) {
    ...
    if ((v=this.re_scalar_path.exec(s))!==null) {
      ...
      s = v[4];
      var num = 0;
      if(this.my_path_map.has(pathvar)){
        let coords = this.my_path_map.get(pathvar);
  ...
  */
      ...
      case 'ytick'://BARCHART
        ...
        for( var j=0; j < floats.length; j+=1 ) {
          ...
          if (Number.isFinite(num)){
            ...
            [x,y] = this.scalexy(x,y);
            if(Number.isFinite(x)&&Number.isFinite(y)){
              o.push(this.p_label(x+dx,y+dy,`${num}`,ts,'lft',g));
        ...
        for( var j=0,i=0; j < floats.length; j+=1,i++ ) {
          ...
          if (Number.isFinite(num)){
            ...
            if(Number.isFinite(x)&&Number.isFinite(y)){
              ...
              o.push(this.p_label(x+dx,y+dy,label,ts,'bot',g));
            }
          }
        }
        break;
    ...
    switch( opt ) {
      ...
      case 'xtick'://CARTESIAN
        for (var j=0; j < floats.length; ++j) {
          ...
          if(Number.isFinite(x)&&Number.isFinite(y)){
            o.push(this.p_label(x+dx,y+dy,`${num}`,ts,'bot',g));
          }
        }
        break;
      ...
      case 'label'://CARTESIAN
        for( var j=0,i=0; j < floats.length; j+=2,i++ ) {
          ...
          if(Number.isFinite(x)&&Number.isFinite(y)){
            var ta = subopt||'';
            o.push(this.p_label(x+dx,y+dy,txt,ts,ta,g));
          }
        }
        break;
  ...
  do_lego(opts,g,txt,ts,floats) {
    ...
    switch( opt ) {
      ...
      case 'show2'://LEGO
        var wx = this.assert_float(floats[0],0.7 );
        var wy = this.assert_float(floats[1],0.4 );
        var wh = this.assert_float(floats[2],0.6 );
        ...
        A.q = [[0,wh],[0-wx,wh-wy,'L'],[0,wh-wy-wh,'L'],[0+wx,wh-wy,'L'],[0,0,'cycle'],
               [0-wx,0-wy],[0-wx,wh-wy,'L'],[0,wh-wy-wy,'L'],[0,0-wy-wy,'L'],[0,0,'cycle'],
               [0+wx,0-wy],[0+wx,wh-wy,'L'],[0,wh-wy-wy,'L'],[0,0-wy-wy,'L'],[0,0,'cycle']];
          ...
          for(var x=1; x <= max_x; x++) {
            for(var y=1; y <= max_y; y++) {
              ...
              if(A.all.has(key)){
                ...
                if(1){
                  var q = this.offset_and_scale_coords(A.q,offsetx,offsety,1,1,0);
      ...
      case 'show'://LEGO
        ...
        var wh = this.assert_float(floats[2],1.00);
        var origin_x = A.origin_x;
        var origin_y = A.origin_y;
        var max_z = A.max_z;
        var max_y = A.max_y;
        ...
        A.q = [[0,wh],[wh,wh,'L'],[wh-wx,wh-wy,'L'],[0-wx,wh-wy,'L'],[0,0,'cycle'],
               [wh,wh],[wh,0,'L'],[wh-wx,0-wy,'L'],[wh-wx,wh-wy,'L'],[0,0,'cycle'],
               [0-wx,0-wy],[0-wx,wh-wy,'L'],[wh-wx,wh-wy,'L'],[wh-wx,0-wy,'L'],[0,0,'cycle']];
        A.f1 = [[0,wh],[wh,wh,'L'],[wh-wx,wh-wy,'L'],[0-wx,wh-wy,'L'],[0,0,'cycle']];
        A.f2 = [[wh,wh],[wh,0,'L'],[wh-wx,0-wy,'L'],[wh-wx,wh-wy,'L'],[0,0,'cycle']];
        A.f3 = [[0-wx,0-wy],[0-wx,wh-wy,'L'],[wh-wx,wh-wy,'L'],[wh-wx,0-wy,'L'],[0,0,'cycle']];
        ///export commands for each cube
        for(var z=1; z <= max_z; z++) {
          for(var x=1; x <= max_x; x++) {
            for(var y=1; y <= max_y; y++) {
              ...
              if(A.all.has(key)){
                var q = A.q;
                var offsetx = origin_x + ((y-1)*wh) - ((x-1)*wx);
  ...
  do_label(opts,g,txt,ts,coords){
    ...
    for (var i = 0; i < coords.length; ++i){
      ...
      o.push(this.p_label(x+dx,y+dy,label,ts,ta,g));
    }
    return o.join('\n');
  }
  do_text(opts,g,txt,ts,coords){
    ...
    for (var i = 0; i < coords.length; ++i){
      ...
      var x = pt[0];
      var y = pt[1];
      o.push(this.p_text(x+dx,y+dy,txt,ts,ta,g));
    }
    return o.join('\n');
  }
  to_playback_rec(rec,g,coords){
    ...
    for (var i = 0; i < coords.length; i++) {
      ...
      var x = z0[0];
      var y = z0[1];
      var sx = this.get_float_prop(g,'sx',1);
      var sy = this.get_float_prop(g,'sy',1);
      var rotation = this.get_float_prop(g,'rotation',0);
      for(let a of rec){
        let g = a.g;
        let p = a.p;
        let q = this.offset_and_scale_coords(p,x,y,sx,sy,rotation);
        let operation = a.cmd;
        if(operation=='fill'){
  ...
  to_shape_of_path(p,g,coords,operation){
    ...
    for (var i = 0; i < coords.length; i++) {
      ...
      var sy = this.get_float_prop(g,'sy',1);
      var rotation = this.get_float_prop(g,'rotation',0);
      var q = this.offset_and_scale_coords(p,x,y,sx,sy,rotation);
      if(operation=='fill'){
        o.push(this.p_fill(q,g));
      }else if(operation=='stroke'){
        o.push(this.p_stroke(q,g));
  ...
  do_drawanglearc(opts,g,txt,ts,coords){   
    ...
    var gap = this.get_float_prop(g,'gap',1,0,this.MAX);
    var dx = this.get_float_prop(g,'dx',0);
    var dy = this.get_float_prop(g,'dy',0);
    for(var j=0; j < coords.length-2; ++j){
      ...
      if (label) {
        ...
        o.push(this.p_label(labelx+dx,labely+dy,label,ts,'ctr',g));
      }
    }
    return o.join('\n');
  }

  do_drawlinesegcong(opts,g,txt,ts,coords){
    var o = [];
  ...
  do_node(opts,g,txt,ts,coords){
    ...
    coords.forEach((pt,i,arr) => {
      ...
      if(g.dot){
        ...
        if(txt){
          o.push(this.p_label(pt[0]+dx,pt[1]+dy,txt,ts,'ctr',g));
        }else if(showname && name){
          o.push(this.p_label(pt[0]+dx,pt[1]+dy,name,ts,'ctr',g));
        }
      }
      if(name){
        this.my_node_map.set(name,v);
      }else if(this.config.nodeid){
        var nodeid = parseInt(this.config.nodeid);
    ...
    for(let i=1; i < opts.length; ++i){
      ...
      if(this.my_node_map.has(name1)&&this.my_node_map.has(name2)){
        ...
        if(name1==name2){
          ...
          o.push(this.p_label(mylabelx+dx,mylabely+dy,txt,ts,'ctr',g));
        }else{
          ...
          if(shift){
            ...
            o.push(this.p_label(p1[0]+dx,p1[1]+dy,txt,ts,'ctr',g));
          }else{
            o.push(this.p_label(p1[0]+dx,p1[1]+dy,txt,ts,'ctr',g));
          }
        }
  ...
  do_box(opts,g,txt,ts,coords){
    ...
    for (var j = 0; j < coords.length; j++) {
      ...
      if (txt) {
        ...
        var ctr_y = y + h/2;
        var label = this.fetch_label_at(txt,j);
        o.push(this.p_label(ctr_x+dx,ctr_y+dy,label,ts,'ctr',g));
      }
    }
    return o.join('\n');
  }
  ...
  fetch_source_from_ss_map(name0){
    if (this.global_ss_map.has(name0)){
      let ss = this.global_ss_map.get(name0);
      return ss;
    }
    return null;
  }
