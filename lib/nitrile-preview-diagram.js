...
class NitrilePreviewDiagram extends NitrilePreviewBase {
  ...
  constructor(translator) {
    ...
    this.re_set_command = /^\\(?:set|var)\s+([A-Za-z][A-Za-z0-9]*)(\[\]|)\s*(\??)([=:])\s*(.*)$/;
    this.re_origin_command = /^\\origin\s+(.*)$/;
    this.re_show_command = /^\\show\s+(.*)$/;
    this.re_clear_command = /^\\clear\b/;
    this.re_output_command = /^\\output\b/;
    // when reading numbers
    this.re_numbers_is_fn            = /^\^fn:(\w+)\s*(.*)$/;
    this.re_numbers_is_list          = /^\[(.*?)\]\s*(.*)$/;
    this.re_numbers_is_solid         = /^(\S+)\s*(.*)$/;
    // when reading coords
    this.re_coords_is_remark         = /^\/([^\/]*)\/\s*(.*)$/;
    this.re_coords_is_label          = /^([-+][.\d]+|)([-+][.\d]+|)\"([^"]*)\"\s*(.*)$/;
  ...
  init_internals(style={}){
    ...
    this.save = {};
    this.save.lastpt_x = 0;
    this.save.lastpt_y = 0;
    ...
    /// viewport
    this.viewport_width = 12;//width is 12 grids total
    this.viewport_height = 7;//height is 7 grids total
    this.viewport_unit = 5;//5mm width/height for each grid
    this.viewport_grid = 1;//draw grid line for each one grid separation
    /// origin
    this.origin_x = 0;
    this.origin_y = 0;
    this.origin_sx = 1;
    ...
    ss.forEach((s,i,arr) => {
      ...
      switch(name){
        ...
        case 'x':
          ...
          break;
        case 'Y':
          this.origin_y = this.viewport_height-num;
          break;
        case 'X':
          this.origin_x = this.viewport_width-num;
          break;
        case 'pt':
          //NOTE: pt:a
    ...
    while (lines.length) {
      ...
      if((v=this.re_set_command.exec(line))!==null){
        ...
        if(this.re_is_symbolname.test(key)){
          if(que && env.hasOwnProperty(key)){
            //dont do anything
          }else if(eq==':'){
            env[key] = val;
          }else if(arrflag){
            let [numbers] = this.read_array(val,env);//returns an array 'Complex' number
            env[key] = numbers;
          }else{
            let a = this.read_scalar(val,env);
            env[key] = a;
          }
          if(Array.isArray(env[key])){
            let s = this.array_to_string(env[key]);
            this.do_comment(`*** ${key}:${s}`);          
          }else{
            let s = env[key];
            this.do_comment(`*** ${key}:${s}`);          
          }
        }else{
          this.do_comment(`*** [ERROR] '${key}' is not a valid symbol name`);  
        }
        continue;
      }  
      //
      ...
      // clear    
      //
      if((v=this.re_clear_command.exec(line))!==null) {
        this.do_clear();
        this.do_comment(`*** [clear]`)
        continue;
      } 
      //
      // output    
      //
      if((v=this.re_output_command.exec(line))!==null) {
        this.do_output();
        this.do_comment(`*** [output]`)
        continue;
      } 
      //
      // if-then-end
  ...
  exec_action_command(style,line,env) {
    ...
    if (cmd == 'image'){
      ...
      let x = 0;
      let y = 0;
      o.push(this.do_image(opts,x,y,w,h,g,txts));///own method
    }
    ///
    ///NOTE: 
    ///
    ...
    ///
      ...
      for (var i = 1; i < coords.length; i+=2) {
        ...
        if(1){
          ...
          if(label){
            o.push(this.p_label(x1+tx,y1+ty,label,'ctr',g));
          }
        }
        if(1){
          var {label,tx,ty} = this.fetch_label_at(txts,i,g);
          if(label){
            o.push(this.p_label(x2+tx,y2+ty,label,'ctr',g));
          }
        }
      }
    }
    ...
    ///
      ...
      for (var i = 0; i < coords.length; i++) {
        ...
        var x = pt[0];
        var y = pt[1];
        var q = q = this.to_ARC(x,y,r,start,span);
        if(arrowhead==1){
    ...
    ///
      ...
      let [coords,g,txts] = this.read_coords(data,env);
      let pt1 = this.point_at(coords,0); 
      let pt2 = this.point_at(coords,1); 
      let pt3 = this.point_at(coords,2); 
      o.push(this.p_qbezier_line(pt1[0],pt1[1],pt2[0],pt2[1],pt3[0],pt3[1],g));
    }
      ...
      let pt2 = this.point_at(coords,1); 
      let pt3 = this.point_at(coords,2); 
      let pt4 = this.point_at(coords,3); 
      o.push(this.p_cbezier_line(pt1[0],pt1[1],pt2[0],pt2[1],pt3[0],pt3[1],pt4[0],pt4[1],g));
    }
    ///
    ///NOTE: specialized draw/fill commands
    ///
  ...
  extract_pathfunc_args(s,env,lastpt_x,lastpt_y){
    //NOTE: parm='(0,0),4,5'
    //NOTE: parm='&a,4,5'
    //NOTE: parm='&a_0,5,6'
    //NOTE: para='@xarr,@yarr'
    ...
    while(s.length){
      if((v=re_cma.exec(s))!==null){
        ...
        let s1 = v[1];
        s = v[2];
        let [q] = this.read_coords(s1,env,lastpt_x,lastpt_y);
        if(q.length){
          pt.coords.push(q[0]);
        }
      }
  ...
  read_coords(line,env,lastpt_x=0,lastpt_y=0) {
    ...
    while (line.length) {
      ...
      if ((v=this.re_coords_is_label.exec(line))!==null) {
        let tx = parseFloat(v[1])||0;
        let ty = parseFloat(v[2])||0;
        let label = v[3].trim();
        line = v[4];    
      ...
      if ((v = this.re_coords_is_aux.exec(line))!==null) {
        ...
        switch (key){
          ...
          case 'pt': {
            ...
            let symbol = val;
            if(this.re_is_symbolname.test(symbol)){
              let pt = [lastpt_x,lastpt_y,'M','','','','', 0,0,0,0,0, 0];
          ...
          case 'save': {
            this.save.lastpt_x = lastpt_x;
            this.save.lastpt_y = lastpt_y;
            this.save.lastoffset_x = lastoffset_x;
            this.save.lastoffset_y = lastoffset_y;
            this.save.lastoffset_sx = lastoffset_sx;
            this.save.lastoffset_sy = lastoffset_sy;
            this.save.lastoffset_xmin = lastoffset_xmin;
            this.save.lastoffset_ymin = lastoffset_ymin;
            break;
          }
          case 'restore': {
            lastpt_x = this.save.lastpt_x;
            lastpt_y = this.save.lastpt_y;
            lastoffset_x = this.save.lastoffset_x;
            lastoffset_y = this.save.lastoffset_y;
            lastoffset_sx = this.save.lastoffset_sx;
            lastoffset_sy = this.save.lastoffset_sy;
            lastoffset_xmin = this.save.lastoffset_xmin;
            lastoffset_ymin = this.save.lastoffset_ymin;
            break;
          }
      ...
      if((v=this.re_coords_is_pathfunc.exec(line))!==null){
        ...
        var parm = v[2];
        line = v[3];
        var args = this.extract_pathfunc_args(parm,env,lastpt_x,lastpt_y);
      ...
      if((v=this.re_coords_is_point.exec(line))!==null){
        ...
        if(s=='*'){
          ...
          if(Number.isFinite(dx)&&Number.isFinite(dy)){
            let pt = [lastpt_x+dx,lastpt_y+dy,'M', '','','','', 0,0,0,0,0, 0];
            lastpt_x = pt[0];
            lastpt_y = pt[1];
            this.offset_pt(pt,lastoffset_x,lastoffset_y,lastoffset_sx,lastoffset_sy,lastoffset_xmin,lastoffset_ymin);
            if(lasthint){
        ...
        if (dashdot.charAt(0) === '.') {
          ...
          for (var pt of from){
            ...
            hobby_p = [];
            hobby_p.push([x,y]);     
            let [pt0,pt1,pt2] = this.abr_to_qbezier(abr, lastpt_x,lastpt_y, x,y);  
            pt[2] = 'Q';
            pt[3] = pt1[0];
            pt[4] = pt1[1];
            pt[5] = '';
          ...
          for (var pt of from){
            ...
            hobby_p = [];
            hobby_p.push([x,y]);          
            x1 = lastpt_x + this.assert_float(dashdot_floats[0],0);
            y1 = lastpt_y + this.assert_float(dashdot_floats[1],0);  
            pt[2] = 'Q';
            pt[3] = x1;
            pt[4] = y1;
            pt[5] = '';
          ...
          for (var pt of from){
            ...
            x1 = lastpt_x + this.assert_float(dashdot_floats[0],0);
            y1 = lastpt_y + this.assert_float(dashdot_floats[1],0);  
            x2 = x + this.assert_float(dashdot_floats[2],0);
            y2 = y + this.assert_float(dashdot_floats[3],0);  
            pt[2] = 'C';
            pt[3] = x1;
            pt[4] = y1;
            pt[5] = x2;
      ...
      if ((v = this.re_coords_is_relative.exec(line)) !== null) {
        ...
        /// <ellipse:dx,dy,sweepflag> /// draws a curve from the current to the point dx/dy away in the shape of a quarter ellipse
        /// <l:dx,dy> /// line
        /// <h:dx> /// line
        /// <v:dy> /// line
        ...
        if (key === 'angledist') {
          if (val.length == 2) {
            ...
            this.offset_pt(pt,lastpt_x,lastpt_y,1,1);
            lastpt_x = pt[0];
            lastpt_y = pt[1];
            this.offset_pt(pt,lastoffset_x,lastoffset_y,lastoffset_sx,lastoffset_sy,lastoffset_xmin,lastoffset_ymin);
            coords.push(pt);
          ...
          if (val.length == 2) {
            ...
            if (coords.length > 1 && Number.isFinite(tx) && Number.isFinite(ty)) {
              ...
              var st = coords[n - 2];
              var pt = coords[n - 1];
              var stx = st[0] - lastpt_x;
              var sty = st[1] - lastpt_y;
              var ptx = pt[0] - lastpt_x;
              var pty = pt[1] - lastpt_y;
          ...
          if (val.length == 4) {
            if (glue === 'Q') {
              [x1,y1] = this.MIRROR([x1,y1],[lastpt_x,lastpt_y]);
            } else if (glue === 'C') {
              [x1,y1] = this.MIRROR([x2,y2],[lastpt_x,lastpt_y]);
            } else {
              x1 = lastpt_x;
              y1 = lastpt_y;
            }
          ...
          if(Number.isFinite(dx)&&Number.isFinite(dy)){
            ...
            this.offset_pt(pt,lastoffset_x,lastoffset_y,lastoffset_sx,lastoffset_sy,lastoffset_xmin,lastoffset_ymin);
            if(coords.length){
              let pt1 = coords[coords.length-1];
              if(pt1[2]=='M'){
                coords.pop();
              }
            }
  ...
  read_array(line,env) {
    ...
    while(line.length) {
      ...
      // /0,0,text/
      if ((v=this.re_coords_is_remark.exec(line))!==null) {
        line = v[2];    
        let ss = v[1].trim().split(',').map(s=>s.trim());
        let tx = parseFloat(ss[0])||0;
        let ty = parseFloat(ss[1])||0;
        let label = ss[2]||'';
        txts.push({label,tx,ty});
        continue;
      }
      ...
      if((v=this.re_numbers_is_list.exec(line))!==null){ 
        ...
        if((v=re_range3.exec(expr))!==null){
          if(v[2].startsWith('!') && v[2].endsWith('!')){
            /// '1:!4!:10'
            let [a1] = this.expr.extract_next_expr(v[1],env);
            let [a2] = this.expr.extract_next_expr(v[2].slice(1,-1),env);
  ...
  do_path_command(symbol,coords,env){
    ...
    if((v=re_destructuring.exec(symbol))!==null){
      ...
      segs.forEach((x,i,arr) => {
        ...
        }else if (this.re_is_symbolname.test(x)) {
  ...
  do_image(opt,x,y,w,h,g,txts){
    ...
    let ss = txts.map((p) => p.label);
    let imgsrc = this.translator.choose_image_file(ss);
    o.push(this.p_image(x,y,w,h,ta,imgsrc,g));
    return o.join('\n');
  }
  do_drawlabel(opts,g,txts,coords){
    var o = [];
    var shift = this.g_to_shift_float(g);
    var tx0    = this.g_to_tx_float(g);
    var ty0    = this.g_to_ty_float(g);
    for (var j = 0; j < coords.length; ++j){
      var pt = this.point_at(coords, j);
      if (!this.is_valid_point(pt)) continue;
      var x = pt[0];
      ...
      o.push(this.p_label(x+tx0+tx,y+ty0+ty,label,ta,g));
    }
    return o.join('\n');
  }
    ...
    for(var j=0; j < linesegs.length; ++j){
      ...
      if(this.is_valid_point(p0)&&this.is_valid_point(p1)&&this.is_valid_point(q0)&&this.is_valid_point(q1)&&this.is_same_pt(p1,q0)){
        ...
        if (label) {
          ...
          o.push(this.p_label(labelx+tx,labely+ty,label,'ctr',g));
        }
      }
    }
    return o.join('\n');
  ...
  do_drawslopedlabel(opts,g,txts,coords){
    ...
    for (var j = 0; j < linesegs.length; ++j){
      ...
      x2 = x2_;
      y2 = y2_;
      o.push(this.p_slopedlabel(x1_+tx0+tx,y1_+ty0+ty,x2_+tx0+tx,y2_+ty0+ty,label,ta,g));
      if(mar){
  ...
  do_chart(opts,g,txts,coords){
    ...
    var xcol = this.g_to_xcol_string(g);
    var yrow = this.g_to_yrow_string(g);
    for (var j = 0; j < coords.length; j++) {
      var pt = this.point_at(coords,j);
      if(!this.is_valid_point(pt)) continue;
      var id = opts[j]||'';
      var x = pt[0];
      var y = pt[1];
      var p = {x,y,xmin,xmax,ymin,ymax,xtick,ytick,xcol,yrow,w,h};
      if(id){
    ...
    all.forEach((one,j,arr) => {
      let {x,y,xmin,xmax,ymin,ymax,xtick,ytick,xcol,yrow,w,h,id} = one;
      if(1){
        var p = [];
        p.push([x,y,'M']);
        p.push([x+w,y,'L']);
        ...
        if(xtick){
          ...
          for(let label of labels){
            ...
            if(Number.isFinite(a)){
              ...
              o.push(this.p_text(px,py,label,'bot',g));
            }
          }
        }
        if(ytick){
          ...
          for(let label of labels){
            ...
            if(Number.isFinite(a)){
              ...
              o.push(this.p_text(px,py,label,'lft',g));
            }
          }
        }
        if(xcol){
          let xcols = this.string_to_array(xcol);
          let n = 0;
          xcols = xcols.map((x) => {
            let d = parseFloat(x);
            let n1 = n;
          ...
          let dy = parseFloat(yrow); 
          xcols.forEach((p,i) => {
            let {label,tx,ty} = this.fetch_label_at(txts,i,g);
            let labels = label.split('\\\\');
            labels.forEach((label,j) => {
              let px = x + (p[0]+p[1])/2;
              let py = y + h - (j+0.5)*dy;    
              o.push(this.p_label(px+tx,py+ty,label,'ctr',g));
            });
          });
        }
      }
  ...
  do_car(opts,g,txts,coords){
    ...
    all.forEach((one,j,arr) => {
      ...
      if(xaxis){
        ...
        var x1 = this.assert_float(ss[0],0);
        var x2 = this.assert_float(ss[1],0);
        var arrowhead = this.assert_float(ss[2],0);
        x1 /= xgrid;
        x2 /= xgrid;
        x1 += xorigin;
        x2 += xorigin;
      ...
      if(yaxis){
        ...
        var y1 = this.assert_float(ss[0],0);
        var y2 = this.assert_float(ss[1],0);
        var arrowhead = this.assert_float(ss[2],0);
        y1 /= ygrid;
        y2 /= ygrid;
        y1 += yorigin;
  ...
  do_node(opts,g,txts,coords){
    ...
    all.forEach((one,j,arr) => {
      ...
      o.push(this.p_label(x+tx0+tx,y+ty0+ty,label,'ctr',g));    
    })
    return o.join('\n');
  }
  do_edge(opts,g,txts,coords){
    ...
    const tx0 = this.g_to_tx_float(g);
    const ty0 = this.g_to_ty_float(g);
    var o = [];
    for(let j=1; j < opts.length; ++j){
      ...
      if(this.my_node_map.has(name1)&&this.my_node_map.has(name2)){
        ...
        if(name1==name2){
          ...
          if(label){
            o.push(this.p_label(mylabelx+tx,mylabely+ty,label,'ctr',g));
          }
        }else{
          ...
          if(label){
            ...
            o.push(this.p_label(pt[0],pt[1],label,'ctr',g));
          }
        }
      }
    }
  ...
  do_box(opts,g,txts,coords){
    ...
    all.forEach((one,j,arr) => {
      ...
      o.push(this.p_drawpath(q,g));
      var {label,tx,ty} = this.fetch_label_at(txts,j,g);
      o.push(this.p_label(x+w/2+tx0+tx,y+h/2+ty0+ty,label,'ctr',g));
    });
    return o.join('\n');
  }
