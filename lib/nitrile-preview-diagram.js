  ...
  do_lego(opts,g,txt,ts,floats) {
    ...
    switch( opt ) {
      case 'setup'://LEGO
        ...
        A.wx = this.assert_float(floats[2],A.wx);
        A.wy = this.assert_float(floats[3],A.wy);
        A.shade = this.assert_int(floats[4],A.shade);
        var wx = A.wx;
        var wy = A.wy;
          ...
          for(var x=1; x <= max_x; x++) {
            for(var y=1; y <= max_y; y++) {
              ...
              if(A.all.has(key)){
                ...
                if(A.shade & 0x1){
                  var q = this.offset_and_scale_coords(A.f1,offsetx,offsety,1,1,0);
                  g.fillcolor = 'gray';
