...
class NitrilePreviewDiagram extends NitrilePreviewBase {
      ...
      case 'ytick'://BARCHART
        for( var j=0; j < floats.length; j+=1 ) {
          ...
          if (Number.isFinite(num)){
            ...
            if(Number.isFinite(x)&&Number.isFinite(y)){
              o.push(this.p_text(x+dx,y+dy,`${num}`,'lft',g));
        ...
        for( var i=0,j=0; i < floats.length; i+=1,j+=1 ) {
          ...
          if (Number.isFinite(num)){
            ...
            if(Number.isFinite(x)&&Number.isFinite(y)){
              ...
              if(math){
                p.push(this.p_math(x+dx,y+dy,label,'bot',g));
              }else{
                o.push(this.p_text(x+dx,y+dy,label,'bot',g));
              }
            }
          }
        }
    ...
    switch( subcmd ) {
      ...
      case 'xtick'://CARTESIAN
        for (var j=0; j < floats.length; ++j) {
          ...
          o.push(this.p_text(x+dx,y+dy,`${num}`,'bot',g));
        }
        break;
    ...
    switch( subcmd ) {
      ...
      case 'ytick': {
        ...
        for (var j=0; j < floats.length; ++j) {
          ...
          var y = A.yorigin + num/A.grid;
          o.push(this.p_rhbar(x,y,g));
          o.push(this.p_text(x+dx,y+dy,`${num}i`,'lft',g));
        }
        break;
      }
  ...
  do_cairo(opts,g,txts,coords){
    ...
    for (var i = 0; i < coords.length; ++i){
      ...
      var x = pt[0];
      var y = pt[1];
      o.push(this.p_cairo(x+dx,y+dy,label,extent,g));
    }
    return o.join('\n');
  }
  do_slopedtext(opts,g,txts,coords){
    ...
    for (var i = 0; i < linesegs.length; ++i){
      ...
      var x2 = lineseg.q1[0];
      var y2 = lineseg.q1[1];
      o.push(this.p_slopedtext(x1+dx,y1+dy,x2+dx,y2+dy,label,ta,g));
      if(mar){
    ...
    for(var j=0; j < linesegs.length; ++j){
      ...
      if(this.isvalidpt(p0)&&this.isvalidpt(p1)&&this.isvalidpt(q0)&&this.isvalidpt(q1)&&this.is_same_pt(p1,q0)){
        ...
        if (label) {
          ...
          if(math){
            o.push(this.p_math(labelx+dx,labely+dy,label,'ctr',g));
          }else{
            o.push(this.p_text(labelx+dx,labely+dy,label,'ctr',g));
          }
        }
      }
    }
    ...
    for(let j=1; j < opts.length; ++j){
      ...
      if(this.my_node_map.has(name1)&&this.my_node_map.has(name2)){
        ...
        if(name1==name2){
          ...
          if(label){
            o.push(this.p_text(mylabelx+dx,mylabely+dy,label,'ctr',g));
          }
        }else{
          ...
          if(label){
            ...
            if(math){
              o.push(this.p_math(p1[0]+dx,p1[1]+dy,label,'ctr',g));
            }else{
              o.push(this.p_text(p1[0]+dx,p1[1]+dy,label,'ctr',g));
            }
          }
        }
      }
  ...
  do_box(opts,g,txts,coords){
    ...
    for (let one of all) {
      ...
      var {label,math} = this.fetch_label_at(txts,j,g);
      if(math){
        o.push(this.p_math(x+w/2+dx,y+h/2+dy,label,'ctr',g));
      }else{
