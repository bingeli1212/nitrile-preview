...
class NitrilePreviewDiagram extends NitrilePreviewBase {
    ...
    if ((v = this.re_rec_command.exec(line)) !== null) {
      ...
      if(opt){
        ...
        if(cmd=='playback'){
          ...
          var [txt,ts,line] = this.readActionLabel(line);
          var [g,line] = this.readActionStyle(line);
          var coords = this.readCoordsLine(line);//path is not modified by current settings of refx/refy/refsx/refsy    
          let a = {};
          a.cmd = cmd;
          a.cmdopt = cmdopt;
          a.p = coords;
  ...
  extract_next_scalar(s,g,z=0) {
    ...
    if ((v=this.re_scalar_func.exec(s))!==null) {
      ...
      var phrase = s0.substr(0,s0.length - s.length);
      //console.log('james','extract_next_scalar', 'scalar_func', 'num',num, 's',s, 'g',g, 'phrase',phrase);
      return [num,s,phrase];
    } 
    ...
    switch( cmd ) {
      ...
      case 'yplot'://CARTESIAN
        ...
        if(g.plot=='line'){
          ...
          mypts = mypts.join('..');
          let coords = this.readCoordsLine(mypts);
