...
class NitrilePreviewDiagram extends NitrilePreviewBase {
  ...
  constructor(translator) {
    ...
    ///the buffers will hold a list of named buffers, this will preserve
    ///  through difference instances of diagram, allowing buffers to be
    ///  created and shared across invocations of diagram
    this.buffers = {};
    /// all ss maps
    this.MAX_ARRAY = 256;
    this.MAX_FLOAT = 2048;
    ...
    this.re_let_command = /^let\s+(\w+)\s*:=\s*(.*)$/;
    this.re_for_command = /^for\s+(.*)$/;
    ...
    */
    ...
    this.re_aux   = /^(x:|y:|X:|Y:|sx:|sy:|s:|up:|down:|left:|right:|at:|mark:|hints:)(\S*)\s*(.*)$/;
    this.re_flow     = /^<(.*?)>\s*(.*)$/;
    this.re_relative = /^\[(.*?)\]\s*(.*)$/;
  ...
  init_internals(){
    ...
    /// all vars defined by 'var' command
    this.my_var_g = {};
    /// all vars defined by 'array' command
    this.my_array_g = {};
  ...
  do_origin_command(val) {
    ...
    ss.forEach((val,i,arr) => {
      if(val.startsWith('left:')){
        ...
        if(Number.isFinite(s)){
          this.origin_r = s;
          console.log('james','this.origin_r',this.origin_r);
        }
  ...
  join_body_lines(lines) {
    ...
    if(1){
      ///also remove comments and take care of buffers
      lines = o;
      o =[];
      var save_buf = null;
      var v;
      ...
      for(var s of lines){
        if((v=this.re_commentline.exec(s))!==null){
          save_buf = null;
          let d = v[1];
          if((v=re_buf_save.exec(d))!==null){
            let name = v[1];
            save_buf = [];
            this.buffers[name] = save_buf;
          }else if((v=re_buf_load.exec(d))!==null){
            let name = v[1];
            let load_buf = this.buffers[name];
        ...
        if(save_buf && Array.isArray(save_buf)){
          save_buf.push(s);
        }
        o.push(s);
      }
  ...
  exec_body(body,env){
    var v;
    var max_n;
    var lines = this.join_body_lines(body);//also made a new copy of it
    while (lines.length) {
      var line = lines.shift();//modify 'lines' variable
      //
      // replace this line with env variables 
      //
      var [line] = this.replace_line_env_variables(line,env);
      //
      //send to console
      //
      ...
      if((v=this.re_var_command.exec(line))!==null){
        let name = v[1];
        let expr = v[2];
        let my = this.expr.extract_next_expr(expr,this.my_var_g);//this returns a 'Complex' number
        let [a] = my;
        this.my_var_g[name] = a;
        this.do_comment(`***var: ${name}=${a.toString()}`);
        continue;
      }  
      //
      // array
      //
      if((v=this.re_array_command.exec(line))!==null){
        let name = v[1];
        let line = v[2];
        let [numbers] = this.read_numbers(line);//this returns an array 'Complex' number
        this.my_array_g[name] = numbers;
        this.do_comment(`***array: ${name}=${this.numbers_to_string(numbers)}`);
        continue;
      }  
      //
      ...
      if((v=this.re_show_command.exec(line))!==null) {
        var s = v[1].trim();
        if(s.startsWith('@')){
          let arr = this.get_array_from_symbol(s.slice(1));
          let str = this.numbers_to_string(arr);
          console.log(str);
        }else if(s.startsWith('&')){
          let coords = this.get_path_from_symbol(s.slice(1));
          let str = this.coords_to_string(coords);
          console.log(str);
        }else{
          console.log(s);
        }
      ...
      if((v=this.re_format_command.exec(line))!==null){
        ...
        let s = this.format_args(fmt,args);
        env[name] = s;
        this.do_comment(`***format: ${name}=${s}`);
        continue;
      }  
      //
      // let
      //
      if((v=this.re_let_command.exec(line))!==null){
        let name = v[1];
        let expr = v[2];
        let [a] = this.expr.extract_next_expr(expr,this.my_var_g);//this returns a 'Complex' number
        let quan = a.re;
        env[name] = quan;
        this.do_comment(`***let: ${name}=${a.toString()}`);
        continue;
      }  
      //
      // for-loop
      ...
      if((v=this.re_for_command.exec(line)) !== null) {
        ...
        var iter_array = [];
        const re_for_one = /^(\w+)\s*:=\s*\[(.*?)\]\s*(.*)$/;
        while(line.length){
          if((v=re_for_one.exec(line))!==null) {
            var line = v[3];
            let name = v[1];
            let range = v[2];
  ...
  exec_command_line(line) {
    ...
    if (cmd == 'label'){
      ...
      let [coords] = this.read_coords(data);
      o.push(this.do_cairo(opts,g,txt,coords));///own method
    }
    ///
    ...
    ///
      ...
      let [coords] = this.read_coords(data);
      o.push(this.do_drawcontrolpoints(opts,g,txt,coords));//own method
    }
    else if (cmd=='drawlinesegarc'){
      let [coords] = this.read_coords(data);
      o.push(this.do_drawlinesegarc(opts,g,txt,coords));///own method
    }
    ...
    ///
      ...
      o.push(this.do_cartesian(subcmd,opts,g,txt,data));///own method
    }
    ///
    ...
    ///
      ...
      o.push(this.do_barchart(subcmd,opts,g,txt,floats));///own method
    }
    ///
    ...
    ///
      ...
      let [floats] = this.read_floats(data);
      o.push(this.do_lego(subcmd,opts,g,txt,floats));///own method
    }
    ///
    ///NOTE: the argand command
    ///
    else if (cmd=='argand'){
      //let [numbers] = this.read_complex_numbers(data);
      o.push(this.do_argand(subcmd,opts,g,txt,data));///own method
    }
    ///
    ...
    ///
      ...
      //let [numbers] = this.read_complex_numbers(data);
      o.push(this.do_trump(subcmd,subsubcmd,opts,g,txt,data));///own method
    }
    ///
  ...
  //
  //get_array_from_symbol()
  //
  get_array_from_symbol(symbol) {

    ///***NOTE: this function should return an array, not a float or a Complex number
    ...
    if (this.my_array_g.hasOwnProperty(symbol)) {
      var p = this.my_array_g[symbol];
      var p = this.dup_arr(p);
      if(Number.isFinite(index)){
        if(index < p.length){
          let a = p[index];
          return [a];
    ...
    return []; //by default resturns an array without any points in it
  }
  ...
  read_coords(line) {
    ...
    while (line.length) {
      ...
      if ((v = this.re_aux.exec(line))!==null) {
        //right:2, up:2, down:2, left:2, at:&a, save:a, hints:1
        line = v[3];
        let val = v[2];
        let key = v[1];
        let num = parseFloat(v[2]);
        let dec = parseInt(v[2]);
        if (key === 'hints:'){
          //NOTE:hints:1
      ...
      v1 = this.re_coord.exec(line);
      v2 = this.re_flow.exec(line);
      if(v1 || v2){
        if(v1){
          v = v1;
          ///(0,0)
          ...
          var x = parseFloat(ss[0])||0;
          var y = parseFloat(ss[1])||0;
          let pt = [x,y];
          this.offset_pt(pt,lastoffset_x,lastoffset_y,lastoffset_sx,lastoffset_sy);
          x = pt[0];
          y = pt[1];
          ...
          if(article=='box' && this.my_box_map.has(id)){
            var [x,y,w,h,rdx,rdy,boxtype] = this.my_box_map.get(id); 
            var [x,y] = this.to_box_anchor_point(x,y,w,h,anchor);
            dx = parseFloat(dx)||0;
            dy = parseFloat(dy)||0;
            x += dx;
            y += dy;
          }else if(article=='node' && this.my_node_map.has(id)){
            var {x,y,r} = this.my_node_map.get(id); 
            var [x,y] = this.to_node_anchor_point(x,y,r,anchor);
            dx = parseFloat(dx)||0;
            dy = parseFloat(dy)||0;
            x += dx;
            y += dy;
        ...
        if (lastjoin === 'hobby') {
          ...
          if(lastjoin==='curve2'){
            let [pt0,pt1,pt2] = this.abr_to_qbezier(30, this.my_lastpt_x,this.my_lastpt_y,0, x,y,0);  
            let pt = [x,y,'Q', pt1[0],pt1[1],'','', 0,0,0,0,0, hints];
            coords.push(pt);
          }else if(lastjoin==='curve3'){
            let [pt0,pt1,pt2] = this.abr_to_qbezier(48, this.my_lastpt_x,this.my_lastpt_y,0, x,y,0);  
            let pt = [x,y,'Q', pt1[0],pt1[1],'','', 0,0,0,0,0, hints];
            coords.push(pt);
          }else if(lastjoin==='curve4'){
            let [pt0,pt1,pt2] = this.abr_to_qbezier(60, this.my_lastpt_x,this.my_lastpt_y,0, x,y,0);  
            let pt = [x,y,'Q', pt1[0],pt1[1],'','', 0,0,0,0,0, hints];
            coords.push(pt);
          }else if(lastjoin==='line'){
        ...
        if (fun_name.length) {
          ...
          if (key === 'angledist') {
            ...
            if (val.length == 2) {
              ...
              var hints = lasthints;
              lasthints = 0;
              let pt = [x, y, 'L', x1, y1, x2, y2, 0,0,0,0,0, hints];
              coords.push(pt);
              this.my_lastpt_x = x;
              this.my_lastpt_y = y;
            ...
            if (val.length == 6) {
              ...
              if (Number.isFinite(x1) &&
                ...
                var hints = lasthints;
                lasthints = 0;
                let pt = [x, y, 'C', x1, y1, x2, y2, 0,0,0,0,0, hints];
                coords.push(pt);
                this.my_lastpt_x = x;
                this.my_lastpt_y = y;
              }
          ...
          /// [a:rx,ry,angle,bigarcflag,sweepflag,dx,dy]
            ...
            if (val.length == 7) {
              ...
              if (Number.isFinite(X1) &&
                ...
                var hints = lasthints;
                lasthints = 0;
                let pt = [X2, Y2, 'A', '','','','', Rx,Ry,Phi,bigarcflag,sweepflag, hints];
                coords.push(pt);
                this.my_lastpt_x = X2;
                this.my_lastpt_y = Y2;
              }
            ...
            if (val.length == 4) {
              ...
              if (Number.isFinite(x1) &&
                ...
                var hints = lasthints;
                lasthints = 0;
                let pt = [x, y, 'Q', x1,y1,'','', 0,0,0,0,0, hints];
                coords.push(pt);
                this.my_lastpt_x = x;
                this.my_lastpt_y = y;
            ...
            if (val.length == 1) {
              ...
              if(Number.isFinite(r)){
                coords.push([x+r,y,'M', '','','','', 0,0,0,0,0, hints]);
            ...
            if(val.startsWith('&')){
              ...
              if(this.isvalidpt(pt)){
                ...
                if(coords.length && coords[coords.length-1][2]=='M'){
                  ...
                  if(hints) {
                    coords[coords.length-1][12] = hints;
                  }
                }else{
  ...
  read_floats(line) {
    ...
    var [numbers,line] = this.read_numbers(line);
    var floats = numbers.map((x) => x.re);
    return [floats,line];
  }

  read_numbers(line) {
    /// 5 5 0.5 0.5 \pi a (1+2*I) (2+1*I)
    /// fn:f 5 5 0.5 0.5 \pi a (1+2*I) (2+1*I)
    var o = [];
    line = line.trimLeft();
    var v;
    var filters = [];
    while(line.length) {
      v=this.re_nonblanks.exec(line); 
      if(v){
        var expr = v[1];
        line = v[2];
      }else{
      ...
      if((v=this.re_numbers_is_filter.exec(expr))!==null){
        let ss = v[1].split(':');
        if(ss.length==2){
          filters.push(ss);
        }
      ...
      if((v=this.re_numbers_is_range_expr2.exec(expr))!==null){
        /// '1~10'
        let [a1] = this.expr.extract_next_expr(v[1],this.my_var_g);//returns a 'Complex'
        let [a2] = this.expr.extract_next_expr(v[2],this.my_var_g);//If empty it returns (0,0*I)
        let all = [];
        ...
        this.copy_numbers(o,all,filters);
        continue; 
      }
      if((v=this.re_numbers_is_range_expr3.exec(expr))!==null){
        ...
        let [a1]   = this.expr.extract_next_expr(v[1],this.my_var_g);//returns a 'Complex'
        let [a2] = this.expr.extract_next_expr(v[2],this.my_var_g);//If empty it returns (0,0*I)
        let [a3]   = this.expr.extract_next_expr(v[3],this.my_var_g);//If empty it returns (0,0*I)
        let all = [];
      ...
      if((v=this.re_numbers_is_spread.exec(expr))!==null){
        ...
        let [a1] = this.expr.extract_next_expr(v[1],this.my_var_g);//returns a 'Complex'
        let [a2] = this.expr.extract_next_expr(v[2],this.my_var_g);//If empty it returns (0,0*I)
        let [a3] = this.expr.extract_next_expr(v[3],this.my_var_g);
        let all = [];
        if(a1.isFinite()&&a2.isFinite()&&a3.isFinite()){
      ...
      if((v=this.re_numbers_is_array.exec(expr))!==null){
        /// '@a', '@a_1', '@a_0'
        let symbol = v[1];
        let arr = this.get_array_from_symbol(symbol);
        this.copy_numbers(o,arr,filters);
        continue;
      }
      if(1){
        ///a single number, such as:
        ///'12.3', 'pow(5,2)', '@a_1', '&a_1.x', '&a_1.y'
        let [a] = this.expr.extract_next_expr(expr,this.my_var_g);//this returns a 'Complex' number
        this.copy_numbers(o,[a],filters);
      }
    }
    if(1){
      let s0 = this.numbers_to_string(o);
  ...
  copy_numbers(o,all,filters){
    all = this.filter_numbers(all,filters);
    all.forEach((x) => o.push(x));
  }

  filter_numbers(all,filters){
    for(let j=filters.length; j > 0; j--){
      let filter = filters[j-1];
      let key = filter[0];
      let f = filter[1];
      if(key==='map'){
        all = this.map_numbers(all,f);
      }else if(key==='keep'){
        all = this.keep_numbers(all,f);
      }else if(key==='drop'){
        all = this.drop_numbers(all,f);
      }
    }
    return all;
  }
  ...
  drop_numbers(all,f){
    all = all.filter( (x) => {
      let a = this.expr.exec_complex_func(f,[x],this.my_var_g,0);
      return (a.re >= 1) ? false : true; 
    })
    return all;
  }
  ...
  do_barchart(subcmd,opts,g,txt,floats) {
    var o = [];
    var id = 0;
  ...
  do_cartesian(subcmd,opts,g,txt,data) {
    var o = [];
    var id = 0;
    let [floats] = this.read_floats(data);
    ...
    switch( subcmd ) {
      ...
      case 'yplot'://CARTESIAN
        ...
        if(this.g_to_plottype_string(g)=='line'){
          ...
          let y0 = NaN;
          for(var x of floats){
            var y = this.expr.exec_float_func(f,[x],this.my_var_g);
            x /= A.grid;
            y /= A.grid;
            x += A.xorigin;
            y += A.yorigin;
      ...
      case 'xplot'://CARTESIAN
        ...
        if(this.g_to_plottype_string(g)=='line'){
          ...
          let y0 = NaN;
          for(var y of floats){
            var x = this.expr.exec_float_func(f,[y],this.my_var_g);
            x /= A.grid;
            y /= A.grid;
            x += A.xorigin;
            y += A.yorigin;
  ...
  do_lego(subcmd,opts,g,txt,floats) {
    var o = [];
    var id = 0;
    if (!this.my_lego[id]) {
      this.my_lego[id] = {};
  ...
  do_argand(subcmd,opts,g,txt,data) {
    var o = [];
    var id = 0;
    ...
    switch( subcmd ) {
      ...
      case 'ytick': {
        ...
        for (var j=0; j < floats.length; ++j) {
          ...
          var y = A.yorigin + num/A.grid;
          o.push(this.p_rhbar(x,y,g));
          o.push(this.p_label(x+dx,y+dy,`${num}i`,'lft',g));
        }
        break;
      }
      ...
      case 'dot': {
        //ARGAND
        let [numbers] = this.read_numbers(data);
      ...
      case 'label': {
        ...
        for( var j=0; j < numbers.length; j++ ) {
          ...
          if(Number.isFinite(x)&&Number.isFinite(y)){
            let [opt] = opts;
            var ta = this.assert_ta(opt);
            let label = this.fetch_label_at(txt,j,g);
            if(!label){
              label = a1.toString();
            }
  ...
  do_trump(subcmd,subsubcmd,opts,g,txt,data) {
    var o = [];
    var id = 0;
  ...
  do_cairo(opts,g,txt,coords){
    ...
    for (var i = 0; i < coords.length; ++i){
      ...
      var x = pt[0];
      var y = pt[1];
      o.push(this.p_cairo(x+dx,y+dy,txt,extent,g));
    }
    return o.join('\n');
  }
  ...
  do_drawlinesegarc(opts,g,txt,coords){
    var [opt] = opts;
    var o = [];
  ...
  do_node(opts,g,txt,coords){
    ...
    for (var j = 0; j < coords.length; j++) {
      ...
      all.push(this.p_circle(x,y,r,g));
      if(txt){
        var label = this.fetch_label_at(txt,j,g);
        all.push(this.p_label(x+dx,y+dy,label,'ctr',g));
      }else if(showid){
        all.push(this.p_label(x+dx,y+dy,id,'ctr',g));
      }
    }
    ///
    ...
    for(let i=1; i < opts.length; ++i){
      ...
      if(this.my_node_map.has(name1)&&this.my_node_map.has(name2)){
        ...
        if(name1==name2){
          ...
          if(label){
            o.push(this.p_label(mylabelx+dx,mylabely+dy,label,'ctr',g));
          }
        }else{
          ...
          if(coords.length){
            var q = [];
            this.add_move_pt(q,p0);
            this.add_coords(q,coords);
            this.add_line_pt(q,p2);
          ...
          if(label){
            if(shift){
              ...
              o.push(this.p_label(p1[0]+dx,p1[1]+dy,label,'ctr',g));
            }else{
              o.push(this.p_label(p1[0]+dx,p1[1]+dy,label,'ctr',g));
            }
          }
  ...
  do_box(opts,g,txt,coords){
    ...
    var dx = this.g_to_dx_float(g);
    var dy = this.g_to_dy_float(g);
    var rdx = this.g_to_rdx_float(g,w);
    var rdy = this.g_to_rdy_float(g,h);
    for (var j = 0; j < coords.length; j++) {
      var pt = this.point(coords, j);
      if (!this.isvalidpt(pt)) continue;
      var id = opts[j]||'';
    ...
    all.forEach((p,j,arr) => {
      ...
      let q = null;
      if(boxtype=='HEX'){
        q = this.to_p_HEX(x,y,w,h);
      }else if(boxtype=='TRI'){
        q = this.to_p_TRI(x,y,w,h);
      }else if(boxtype=='DOWNTRI'){
        q = this.to_p_DOWNTRI(x,y,w,h);
      }else if(boxtype=='PGRAM'){
        q = this.to_p_PGRAM(x,y,w,h,rdx);
      }else if(boxtype=='RHOMBUS'){
        q = this.to_p_RHOMBUS(x,y,w,h);
      }else if(boxtype=='RRECT'){
        q = this.to_p_RRECT(x,y,w,h,rdx,rdy);
      }else{
        q = this.to_p_rect(x,y,w,h);
      }
      if(this.g_to_shadow_int(g)){
  ...
  g_to_start_float(g){
    g = this.update_style_with_group(g);
    return this.get_float_prop(g,'start',this.span);
  }
  g_to_abr_float(g){
    g = this.update_style_with_group(g);
  ...
  to_flowed_lines(id1,anchor1,id2,anchor2,g){
    ...
    if(this.my_box_map.has(id1)&&this.my_box_map.has(id2)){
      ...
      var [x1,y1,w1,h1] = v1;
      var [x2,y2,w2,h2] = v2;
      var [x1,y1] = this.to_box_anchor_point(x1,y1,w1,h1,anchor1);
      var [x2,y2] = this.to_box_anchor_point(x2,y2,w2,h2,anchor2);
      q.push([x1,y1,'M']);
      q.push([x2,y2,'L']);
      o.push(this.p_arrow(q,g));
    }
  ...
  to_box_anchor_point(x,y,w,h,anchor){
  ...
  to_node_anchor_point(x,y,r,anchor){
    ///'anchor' is one of the following string:
    //   "n", "w", "e", "w", "sw", "se", "nw", "ne"
    //  return [x,y]
    if(anchor=='n'){
  ...
  to_trump_card(x,y,mypath,d,g,mycolor){
    ...
    if(d==='10'){
      ///
      all.push(this.p_label(x+0.5*scaleX,y+(0.5+5)*scaleY,'10','ctr',g))
      ...
      ///
      all.push(this.p_label(x+0.5*scaleX,y+(0.5+5)*scaleY,'9','ctr',g))
      all.push(this.p_fill(q1,g));
      ///
      this.offset_coords(q,1*scaleX,1*scaleY);
      all.push(this.p_fill(q,g));
      ...
      ///
      all.push(this.p_label(x+0.5*scaleX,y+(0.5+5)*scaleY,'8','ctr',g))
      all.push(this.p_fill(q1,g));
      ///
      this.offset_coords(q,1*scaleX,1*scaleY);
      all.push(this.p_fill(q,g));
      ...
      ///
      all.push(this.p_label(x+0.5*scaleX,y+(0.5+5)*scaleY,'7','ctr',g))
      all.push(this.p_fill(q1,g));
      ///
      this.offset_coords(q,1*scaleX,1*scaleY);
      all.push(this.p_fill(q,g));
      ...
      ///
      all.push(this.p_label(x+(0.5*scaleX),y+(0.5+5)*scaleX,'6','ctr',g))
      all.push(this.p_fill(q1,g));
      ///
      this.offset_coords(q,1*scaleX,1*scaleY);
      all.push(this.p_fill(q,g));
      ...
      ///
      all.push(this.p_label(x+0.5*scaleX,y+(0.5+5)*scaleY,'5','ctr',g))
      all.push(this.p_fill(q1,g));
      ///
      this.offset_coords(q,1*scaleX,1*scaleY);
      all.push(this.p_fill(q,g));
      ...
      ///
      all.push(this.p_label(x+0.5*scaleX,y+(0.5+5)*scaleY,'4','ctr',g))
      all.push(this.p_fill(q1,g));
      ///
      this.offset_coords(q,1*scaleX,1*scaleY);
      all.push(this.p_fill(q,g));
      ...
      ///
      all.push(this.p_label(x+0.5*scaleX,y+(0.5+5)*scaleY,'3','ctr',g))
      all.push(this.p_fill(q1,g));
      ///
      ...
      ///
      all.push(this.p_label(x+0.5*scaleX,y+(0.5+5)*scaleY,'2','ctr',g))
      all.push(this.p_fill(q1,g));
      ///
      this.offset_coords(q,2*scaleX,1*scaleY);
      all.push(this.p_fill(q,g));
      ...
      ///
      all.push(this.p_label(x+0.5*scaleX,y+(0.5+5)*scaleY,'A','ctr',g))
      all.push(this.p_fill(q1,g));
      ///
      all.push(this.p_fill(q2,g));
    }else if(d==='J'){
      ///
      all.push(this.p_label(x+0.5*scaleX,y+(0.5+5)*scaleY,'J','ctr',g))
      all.push(this.p_fill(q1,g));
      ///
      ...
      all.push(this.p_label(x+2.5*scaleX,y+3*scaleY,"JACK",'bot',g)) 
    }else if(d==='Q'){
      ///
      all.push(this.p_label(x+0.5*scaleX,y+(0.5+5)*scaleY,'Q','ctr',g))
      all.push(this.p_fill(q1,g));
      ///
      all.push(this.p_stroke(sq,{...g,linesize:0,linecolor:'lightgray'}));      
      ///
      ...
      this.offset_coords(jack,0,2*scaleY);
      all.push(this.p_draw(jack,g));      
      all.push(this.p_label(x+2.5*scaleX,y+3*scaleY,"QUEEN",'bot',g)) 
    }else if(d==='K'){
      ///
      all.push(this.p_label(x+0.5*scaleX,y+(0.5+5)*scaleY,'K','ctr',g))
      all.push(this.p_fill(q1,g));
      ///
      all.push(this.p_stroke(sq,{...g,linesize:0,linecolor:'lightgray'}));      
      ///
      ...
      this.offset_coords(jack,0,2*scaleY);
      all.push(this.p_draw(jack,g));     
      all.push(this.p_label(x+2.5*scaleX,y+3*scaleY,"KING",'bot',g)) 
    }
    return all.join('\n');
  }
  ///////////////////////////////////////////////////////////////////////////////////
