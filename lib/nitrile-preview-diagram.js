...
class NitrilePreviewDiagram extends NitrilePreviewBase {
  ...
  exec_line(line,env) {
    ...
    if ((v = this.re_set_command.exec(line)) !== null) {
      ...
      if (key === 'refxy') {
        ...
        if(val === 'center'){
          ...
          let vals = this.string_to_float_array(val);
          if(vals.length==2){
            this.refx = vals[0];
            this.refy = vals[1];
          }
    ...
    if ((v = this.re_text_command.exec(line)) !== null) {
      ...
      //var [g,line] = this.readActionStyle(line);
      var {coords,g} = this.read_action_coords(line);
      this.commands.push(this.do_text(opts,g,coords));///own method
      return false;
    }
    ///
  ...
  */
    ...
    for(let i=1; i < opts.length; ++i){
      ...
      if(this.my_nodes.has(name1)&&this.my_nodes.has(name2)){
        ...
        if(name1==name2){
          ...
          var dir = this.assert_float(g.dir,0);
  ...
  to_angle_str(g){
    ...
    return this.assert_float(parseFloat(angle),0,0,90);
  }
  to_shade_str(g){
    let shade = g.shade;
    if(!shade){
  ...
  string_to_command_opts(s){
    var opts = this.remove_first_dot(s).split('.');
    return opts;
  }
}
