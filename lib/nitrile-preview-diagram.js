...
class NitrilePreviewDiagram extends NitrilePreviewBase {
  ...
  constructor(translator) {
    ...
    */
    ...
    this.re_shift = /^(up:|down:|left:|right:)([0-9\.\-\+]*)\s*(.*)$/;
    this.re_offset = /^\/(.*?)\/\s*(.*)$/;
    this.re_relative = /^\[(.*?)\]\s*(.*)$/;
  ...
  get_next_id(){
    if(typeof this.id === 'number'){
      var id = Math.abs(Math.round(this.id));
      this.id = id+1;
  ...
  exec_command_line(line) {
    ...
    else if (cmd == 'drawtextpath'){
      o.push(this.do_drawtextpath(opts,g,txt,coords));///own method
    }
  ...
  exec_path_func(name,args) {
    ...
    switch (name) {
      ...
      case 'asaTriangle': {
        ...
        let a = this.assert_arg_scalar(args,1);
        let B = this.assert_arg_scalar(args,2);
  ...
  read_coords(line) {
    ...
    while (line.length) {
      ...
      if ((v = this.re_shift.exec(line))!==null) {
        //right:2, up:2, down:2, left:2
        line = v[3];
        let key = v[1];
        let val = parseFloat(v[2]);
        if(Number.isFinite(val)){
          if(key=='up:')          offset[1]+=val;
          else if(key=='down:')   offset[1]-=val;
          else if(key=='left:')   offset[0]-=val;
          else if(key=='right:')  offset[0]+=val;
        }
        lastjoin = '';
        continue;
      }
        ...
        if (fun_name.length) {
          ...
          /// [a:rx,ry,angle,bigarcflag,sweepflag,dx,dy]
            ...
            if(this.re_symbol_name.test(val)){
              ...
              this.do_comment(`***path ${val}=(${pt[0]},${pt[0]})`);
            }
          }
          else if (key === 'load') {
            //NOTE:[load:a]
            if(this.my_path_map.has(val)){
              let p = this.my_path_map.get(val);
              if(p.length){
                let pt = p[0];
                if(this.isvalidpt(pt)){
                  lastpt[0] = pt[0];
                  lastpt[1] = pt[1];
                  this.do_comment(`***lastpt: ${val}=(${pt[0]},${pt[0]})`);
                }
              }
            }
          }
  ...
  do_path_command(symbol,line){
    ...
    }else if((v=re_symbols.exec(symbol))!==null){
      var segs = v[1].split(',');
  ...
  */
  ...
  do_drawtextpath(opts,g,txt,coords){
    this.move_coords(coords,this.refx,this.refy,this.refsx,this.refsy,0);
    var ta = opts[0]||'';
    var o = [];
    var dx = this.g_to_dx_float(g);
