...
class NitrilePreviewDiagram extends NitrilePreviewBase {
      ...
      if((v=this.re_var_command.exec(line))!==null){
        ...
        if(this.re_is_symbolname.test(name)){
          if(prefix==='@'){
            ...
            let s2 = this.read_string(s1,env);
            env[name] = s2;
  ...
  substitute_dollar_expressions(style,line,env){
    ...
    let flag = 0;
    const re_replace = /\$\{(.*?)\}/g;
    const re_is_config_name = /^\^([A-Za-z][A-Za-z0-9]*)$/;
    const re_is_array_name = /^@([A-Za-z][A-Za-z0-9]*)$/;
    const re_is_string_name = /^#([A-Za-z][A-Za-z0-9]*)$/;
    let v;
    line = line.replace(re_replace,(match,p1) => {
      ...
      if((v=re_is_config_name.exec(p1))!==null){
        p1 = v[1];
        let s = '';
        if(this.config.hasOwnProperty(p1)){
          s = this.config[p1];
        }
        return s;
      }else if((v=re_is_array_name.exec(p1))!==null){
        p1 = v[1];
        let s = '';
        if(env.hasOwnProperty(p1)){
          let m = env[p1];
          if(Array.isArray(m)){
            s = this.array_to_string(m);
          }
        }
        return s;
      }else if((v=re_is_string_name.exec(p1))!==null){
        p1 = v[1];
        let s = '';
        if(env.hasOwnProperty(p1)){
          let m = env[p1];
  ...
  read_string(s,env){
    ...
    var fmt = '';
    var args = [];
    if((v=re_format.exec(s))!==null){
      fmt = v[1];
      [args] = this.read_array(v[2],env);
    }
    while(fmt.length){
