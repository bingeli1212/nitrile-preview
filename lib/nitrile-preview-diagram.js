...
class NitrilePreviewDiagram extends NitrilePreviewBase {
  ...
  constructor(translator) {
    ...
    this.re_range_three = /^([^:]+):([^:]+):([^:]+)$/;
    /// regular expression
    this.re_var_expr = /^var\b\s*(\w+)\s*=\s*(.*)$/;
    ...
    */
    // RE
    this.re_pathfunc        = /^&([A-Za-z]\w*)\{(.*?)\}\s*(.*)$/;
    this.re_pathvar_range   = /^&([A-Za-z]\w*)\[(.*?)\]\s*(.*)$/;
    ...
    this.re_var_name = /^([A-Za-z]\w*)(.*)$/;
    this.re_path_name = /^[A-Za-z]\w*$/;
    this.re_colorfunction = /^([A-Za-z]+)\((.*)\)$/;
    // range, [1:2], [1:2:7]
  ...
  exec_body(lines,env){
    ...
    while (lines.length) {
      ...
      // console
      //
      if((v=this.re_console_command.exec(line))!==null) {
        var line = v[1];
        console.log(line);
        continue;
      }
      ...
      if((v=this.re_show_command.exec(line))!==null) {
        var line = v[1];
        let [p] = this.read_coords_line(line);
      ...
      // var
      //
      if((v=this.re_var_expr.exec(line))!==null){
        let name = v[1];
        let expr = v[2];
      ...
      if((v=this.re_for_all.exec(line)) !== null) {
        ...
        while(line.length){
          if((v = this.re_for_one.exec(line))!==null) {
            ...
            let name = v[1];
            let range = v[2];
            let [seq] = this.read_floats_line(range);
  ...
  exec_line(line,env) {
    ...
    const re_action_command = /^([A-Za-z]+)(\.[\.\w]+|)\b\s*(.*)$/;
    var v = re_action_command.exec(line);
    if(v===null){
      return '';
    }
    var o = [];
    ...
    var data = v[3];
    var [g,txt,data] = this.read_action_style_label(data);
    var [coords] = this.read_coords_line(data);
    //this.move_coords(coords,this.refx,this.refy,this.refsx,this.refsy,0);
    var [floats] = this.read_floats_line(data);
    var [opt] = opts;
    ///
    ///NOTE: move to refx/refy/refsx/refsy
    ///
    ///
    ...
    ///
    ///NOTE: send input line to console
    ///
    if(this.config.verbose){
      console.log(line);
    }
    ///
    ///NOTE: send to comment
    ///
    var line = this.p_comment(line);
  ...
  get_path_from_variable(a) {
    ...
    if(s){
      var [coords] = this.read_coords_line(this.name_to_path_string(a));
      return coords;
    }
  ...
  exec_path_func(line, fun_str, args) {
    ...
    switch (fun_str) {
      ...
      case 'rectangle': {
        ...
        let [x,y] = this.assert_arg_pt(args,0);
    ...
    while (line.length) {
      ...
      if ((v = this.re_pathfunc.exec(line)) !== null) {
        ...
        for (var i = 0; i < ret_val.length; ++i) {
          ...
          lastjoin='';
          pt[12] = hints;//always set the hint for a path func generated coords
          coords.push(pt);
          lastpt[0] = pt[0];
          lastpt[1] = pt[1];
        }
      ...
      if ((v = this.re_offset.exec(line)) !== null) {
        ...
        if(fun_name=='last'){
          ...
          let idx = 0;
          if((v=this.re_pathvar_range.exec(fun_name))!==null){
            a = v[1];
            idx = parseInt(v[2])||0;
          }
          let a_coords = this.get_path_from_variable(a);
          let a_pt = this.point(a_coords,idx);
          ...
          /// [a:rx,ry,angle,bigarcflag,sweepflag,dx,dy]
            ...
            if (val.length == 1) {
              ...
              if(Number.isFinite(r)){
                coords.push([x+r,y, '',  '','','','', 0,0,0,0,0, hints]);
                coords.push([x-r,y, 'A', '','','','', r,r,0,0,0, hints]);
                coords.push([x+r,y, 'A', '','','','', r,r,0,0,0, hints]);          
    ...
    if((v=re.exec(symbol))!==null){
      ...
      for (var i = 0; i < segs.length; ++i) {
        ...
        if (this.re_path_name.test(seg)) {
          ...
          if(this.config.verbose){
            console.log(`***SAVED: ${seg}=${this.coords_to_string(this.my_path_map.get(seg))}`);
          }
        }
      } 
      ...
      console.log(`***SAVED: ${symbol}=${this.coords_to_string(this.my_path_map.get(symbol))}`);
    } else {
      console.log(`***ERROR: ${symbol} is not a valid variable`);
    }
  }
      ...
      if(opts.length){
        ...
        for(var j=0; j<opts.length; ++j){
          ...
          if(this.my_box_map.has(id1)&&this.my_box_map.has(id2)){
            ...
            var [q] = this.read_coords_line(my);
            this.move_coords(q,this.refx,this.refy,this.refsx,this.refsy,0);
