    ...
    while (line.length) {
      ...
      if ((v = this.re_aux.exec(line))!==null) {
        ...
        if (key === 'hints:'){
          ...
          if(key=='up:')          this.my_lastpt_y+=num;
          else if(key=='down:')   this.my_lastpt_y-=num;
          else if(key=='left:')   this.my_lastpt_x-=num;
          else if(key=='right:')  this.my_lastpt_x+=num;
          else if(key=='x:')      this.my_lastpt_x=num;
          else if(key=='y:')      this.my_lastpt_y=num;
          else if(key=='X:')      this.my_lastpt_x=this.viewport_width-num;
          else if(key=='Y:')      this.my_lastpt_y=this.viewport_height-num;
        }
        lastjoin = '';
        lastoffset_x = this.my_lastpt_x;
        lastoffset_y = this.my_lastpt_y;
        continue;
      }
      ...
      if((v=this.re_parse_pathfunc.exec(line))!==null){
        ...
        for (var i = 0; i < ret_val.length; ++i) {
          ...
          this.offset_pt(pt,lastoffset_x,lastoffset_y);
          if(lastjoin=='line'){
            pt[2]='L';
          }
          lastjoin='';
