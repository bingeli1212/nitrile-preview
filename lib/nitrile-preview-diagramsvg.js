...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  p_label(x,y,txt,ta,g) {
    ...
    if (this.g_to_math_int(g)) {
      ...
      if(this.has_texts(g)){
          let s1 = `<g transform='translate(${x},${y}) rotate(-${rotate}) translate(${dx},${dy})'><svg width='${width}' height='${height}' fill='inherit' stroke='inherit' viewBox='0 0 ${vw} ${vh}'><defs>${defs.join('\n')}</defs>${s}</svg></g>`;
          d.push(s1);
      }
    } else {
      ...
      var rotate = this.g_to_rotate_float(g);
      if(this.has_texts(g)){
        d.push(`<text transform='translate(${x} ${y}) scale(${factor}) rotate(${-rotate})' font-family='${fontfamily}' font-style='${fontstyle}' font-size='${this.tokenizer.fs}pt' text-anchor='${anchor}' ${this.g_to_svg_textdraw_str(g)} dy='${this.tokenizer.text_dy_pt}pt'>${txt}</text>`);
      }
    }
    return d.join('\n');
  }
