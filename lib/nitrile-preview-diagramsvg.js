...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  do_finalize(text,style) {
    ...
    if (style && style.width && style.height){
      ///both width and height will be changed, the preserveAspectRatio will be set to 'none'
      width = this.translator.string_to_css_length(style.width);
      height = this.translator.string_to_css_length(style.height);
      css_style.push(`width:${width}`);
      css_style.push(`height:${height}`)
      text = `<svg xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' fill='currentColor' stroke='currentColor' viewBox='0 0 ${this.fix(vw)} ${this.fix(vh)}' preserveAspectRatio='none' style='${css_style.join(";")}' >${text}</svg>`;
    }
    else if (style && style.width) {
      ///use specified width and leave height empty, such that it will scale
      width = this.translator.string_to_css_length(style.width);
      css_style.push(`width:${width}`);
      text = `<svg xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' fill='currentColor' stroke='currentColor' viewBox='0 0 ${this.fix(vw)} ${this.fix(vh)}' style='${css_style.join(";")}' >${text}</svg>`;
    } 
    else {
      ///use native width/height
      text = `<svg xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' fill='currentColor' stroke='currentColor' viewBox='0 0 ${this.fix(vw)} ${this.fix(vh)}' style='${css_style.join(";")}' width='${this.fix(vw)}' height='${this.fix(vh)}' >${text}</svg>`;
      width=`${vw}px`
      height=`${vh}px`;
    }
    let s = text;
  ...
  p_label(x, y, txt, ts, ta, g) {
    ...
    if (ts==2) {
      //math text
      var style = {};
      var {s,w,h,defs} = this.tokenizer.to_svgmath(txt,style);
      ...
      d.push(`<svg x='${x}' y='${y}' width='${vw*factor}' height='${vh*factor}' fill='inherit' stroke='inherit' viewBox='0 0 ${vw} ${vh}'><defs>${defs.join('\n')}</defs>${s}</svg>`);
    } else {
      //literal text
      if(ts==1){
