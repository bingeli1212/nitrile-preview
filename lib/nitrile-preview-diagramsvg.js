...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  do_finalize(text,style) {
    ...
    var gridcolor = this.g_to_gridcolor_string(this.conf_to_stringig);
    ...
    if(this.conf_to_stringig.gridlines=='none'){
  ...
  p_cairo(x,y,txt,extent,g){
    ...
    var segment_array = this.tokenizer.to_cairo_segment_array(txt);
    var pp = this.cairo_segment_array_to_pp(segment_array,extent,x,y);
    var all = [];
    pp.forEach((p,i,arr) => {
      if(p.key=='latin' || p.key=='punc' || p.key=='unicode'){
        ...
        var w = p.w;
        var txt = p.txt;
        all.push(`<text transform='translate(${this.fix(x)} ${this.fix(y)}) scale(${factor})' font-family='${fontfamily}' font-style='${fontstyle}' font-size='${this.tokenizer.fs}pt' text-anchor='${anchor}' ${this.g_to_svg_textdraw_str(g)} dy='${this.tokenizer.text_dy_pt}pt' textLength='${this.fix(w)}' lengthAdjust='spacingAndGlyphs'>${txt}</text>`);
      }else if(p.key=='svg'){
        var x = p.x;
        var y = p.y;
        var w = p.w;
  ...
  /// user coordinates; 
  ///
  cairo_segment_array_to_pp(segment_array,extent,x,y){
    ...
    for(let segment of segment_array){
      let type = segment[0];
      let cc = segment[1];
      let w = segment[2];
      let h = segment[3];
      ...
      if(type=='\\cc'){
        ...
        if(cc <= 0xFF){
          ///LATIN-1
          let type = this.get_latin_type(cc);
          let islatin = (type==this.CC_LETTER)||(type==this.CC_NUMBER)||(type==this.CC_SYMBOL)?true:false;
          let ispunc = (type==this.CC_PUNC);
