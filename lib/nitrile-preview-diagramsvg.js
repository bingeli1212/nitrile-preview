...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  do_finalize(text,style) {
    ...
    o.push('<defs>');
    o.push(`<pattern id="${smallgridid}" width="${u}" height="${u}" patternUnits="userSpaceOnUse"><path d="M ${u} 0 L 0 0 0 ${u}" fill="none" stroke="rgb(200,200,200)" /></pattern>`);
    ...
    if(this.config.grid=='none'){
      ...
      o.push(`<rect width="${this.fix(vw)}" height="${this.fix(vh)}" stroke="rgb(200,200,200)" fill="none" />`);
    }
    else{ 
      for(var j = 0; j <= this.viewport_height; j++){
        var y = j*u;
        o.push(`<line stroke="rgb(200,200,200)" x1="0" y1="${y}" x2="${vw}" y2="${y}" />`)
      }
      for(var i = 0; i <= this.viewport_width; i++){
        var x = i*u;
        o.push(`<line stroke="rgb(200,200,200)" x1="${x}" y1="0" x2="${x}" y2="${vh}" />`)
      }
      ///o.push(`<rect width="${this.fix(vw)}" height="${this.fix(vh)}" stroke="rgb(200,200,200)" fill="none" />`);
    }
    o.push(text);    
  ...
  p_label(x, y, txt, ts, ta, g) {
    ...
    if (ts==2) {
      ...
      var {s,w,h,defs} = this.tokenizer.to_svgmath(txt,{...g,svgdiagram:1});
