...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  do_one_frame(style,ss,env) {
    ...
    if(width && height){
      ...
      var mywidth = `${width}`
      var myheight = `${width*(this.viewport_height/this.viewport_width)}`
      var w = width*this.MM_TO_PX;
    }else if(height){
      var myheight = `${height}`
      var mywidth = `${height*(this.viewport_width/this.viewport_height)}`
      var w = height*(this.viewport_width/this.viewport_height)*this.MM_TO_PX;
    }else{
      var mywidth = `${this.viewport_width*this.viewport_unit}`;
      var myheight = `${this.viewport_height*this.viewport_unit}`;
    ...
    ///
    ///THE OLD WAY
    ///var img = `<svg style="${css}" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="currentColor" stroke="currentColor" viewBox="0 0 ${this.fix(vw)} ${this.fix(vh)}" >${img}</svg>`;
    ///
    var ENCODE = 1;
    if(ENCODE){
      ///IMPORTANT: it has to be placed inside a <object>, otherwise when 'img' is placed inside a <table> element the iBook app for iPAD1.0 just wont show the picture
      ...
      let s_svg = `<svg style='${css}' width='${this.fix(vw)}' height='${this.fix(vh)}' xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' fill='currentColor' stroke='currentColor' viewBox='0 0 ${this.fix(vw)} ${this.fix(vh)}' >${img}</svg>`;
      img = s_svg;
      var subc = this.translator.uncode(style,this.subtitle);
    }
    return { img, subc, w };
