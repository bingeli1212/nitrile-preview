...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  do_finalize(text,style) {
    ...
    if(this.config.grid=='none'){
      ...
      o.push(`<rect width="${this.fix(vw)}" height="${this.fix(vh)}" stroke="rgb(200,200,200)" fill="url(#${smallgridid})" />`);
    }
    o.push(text);    
    var text = o.join('\n');
  ...
  p_label(x, y, txt, ts, ta, g) {
    ...
    if (ts==2) {
      ...
      if(1){
        ...
        //
        if(valign == 'upper') {
          y -= 2*1.333; // y reduce by 3pt, moving text up
