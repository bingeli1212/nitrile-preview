...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  string_to_svg_color(s,hints,def='none') {
    ...
    if(typeof(s)==='string'){
      s = this.w3color_to_hexstring(s,hints);
      s = s.slice(1);
  ...
  to_textdraw_str(g) {
    ...
    if (fontcolor) {
      d.push(`fill='${this.string_to_svg_color(fontcolor,g.hints)}'`);
    }
    d.push(`stroke='none'`);
    return d.join(' ');
  }
  ...
  to_dot_str(g){
    ...
    if (dotcolor) {
      o.push(`fill='${this.string_to_svg_color(dotcolor,g.hints)}'`);
    } else {
      o.push(`fill='inherit'`);
    }
    o.push(`stroke='none'`);
  ...
  to_svg_colorbox (g) {
    ...
    var h = g.height;
    var color = g.data;;
    o.push(`<rect x='0' y='0' width='${this.fix(w)}mm' height='${this.fix(h)}mm' stroke='none' fill='${this.string_to_svg_color(color,g.hints)}' />`);
    var s = o.join('\n');
    return {s,w,h};
  }
  to_svg_vbarchart (g) {
