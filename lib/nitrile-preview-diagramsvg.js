...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  constructor(translator) {
    ...
    this.my_path_array = [];
    ///this will not be used and could be deprecated
  ...
  p_textpath(x1,y1,x2,y2,txt,ta,g){
    if(!txt) return '';
    var o = [];
    ...
    if(this.has_texts(g)){
      ...
      var height = this.tokenizer.fh*1.333;
      var dx = height*Math.cos(ang);
      var dy = height*Math.sin(ang);
      //x += dx;
      //y -= dy;//for SVG negative is going up
      var fontfamily = this.g_to_svg_fontfamily_str(g);
      var fontstyle = this.g_to_svg_fontstyle_str(g);
      var txt = this.translator.flatten(txt);
      let id = this.get_css_id();
      var txt = `<textPath href='#${id}'>${txt}</textPath>`;
      o.push(`<text x='${x}' y='${y}' font-family='${fontfamily}' font-style='${fontstyle}' font-size='${this.tokenizer.fs}pt' text-anchor='middle' ${this.g_to_svg_textdraw_str(g)} dy='-0.5ex'>${txt}</text>`);
