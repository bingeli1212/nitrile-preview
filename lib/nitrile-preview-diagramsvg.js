...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  coordsToD(coords) {
    ///***NOTE: returns a list of objects, each object has three members: {d,iscicled,attrflag}
    ...
    for (var i in coords) {
      ...
      if (o.length == 0) {
        ...
        d = o.join(' ');
        o = [];
        items.push({iscycled,d,attrflag});
        continue;
      }
      else if(join == 'L'){
        ///NOTE: line
  ...
  p_label(x,y,txt,ta,g) {
    ...
    let fontsize = this.to_fontsize_float(g);
    ...
    if (this.to_math_int(g)) {
      ...
      var width = w*1.333;
      var height = h*1.333;
      let factor = fontsize/this.tokenizer.fs;
      width *= factor;
      height *= factor;
  ...
  p_text(x,y,txt,ta,g){
    ...
    */
    ...
    var y = this.localy(y);
    var height = this.tokenizer.fh*1.333;
    var fontsize = this.to_fontsize_float(g);
    var factor = fontsize/this.tokenizer.fs;
    ...
    var ss = ss.map((s,i,arr) => {
      if(i==0){
        return `<tspan x='0' dy='${this.tokenizer.dy}em'>${s}</tspan>`;
      }else{
        return `<tspan x='0' dy='1em'>${s}</tspan>`;
      }
    });
    var txt = ss.join('\n');
    var d = [];
    //adjust y
    var height = fontsize*1.333*ss.length;
    if(valign == 'upper'){
      y -= height;
    ...
    var [x,y] = this.to_adjusted_xy_for_label(x,y,valign,anchor)
    d.push(`<text transform='translate(${x} ${y}) scale(${factor})' font-size='${this.tokenizer.fs}pt' text-anchor='${anchor}' ${this.to_textdraw_str(g)} dy='${this.tokenizer.dy}em'>${txt}</text>`);
    return d.join('\n');
  }
  ...
  p_fill(coords,g){
    ...
    var o = [];
    var shade = this.get_string_prop(g,'shade','');
    var fillcolor = this.get_string_prop(g,'fillcolor','');
  ...
  p_draw(coords,_g){
    var items = this.coordsToD(coords,true);
    var shade = this.get_string_prop(_g,'shade','');
    var fillcolor = this.get_string_prop(_g,'fillcolor','');
    var o = [];
    for(var item of items){
      var {iscycled,d,attrflag} = item; 
      if(!d) continue;
      var g = {..._g};
  ...
  p_arrow(coords,_g){
    ...
    for(var item of items){
      ...
      o.push(`<path d='${d}' ${this.to_drawonly_str(g)} marker-end='url(#${id1})' />`);
    }
    return o.join('\n');
  }
  p_revarrow(coords,_g){
    ...
    for(var item of items){
      ...
      o.push(`<path d='${d}' ${this.to_drawonly_str(g)} marker-start='url(#${id2})' />`);
    }
    return o.join('\n');
  }
  p_dblarrow(coords,_g){
    ...
    for(var item of items){
      ...
      let id1 = this.create_arrowhead('end',g);
      let id2 = this.create_arrowhead('start',g);
      o.push(`<path d='${d}' ${this.to_drawonly_str(g)} marker-end='url(#${id1})' marker-start='url(#${id2})' />`);
    }
    return o.join('\n');
  }
  ...
  p_hexgon(x,y,w,h,g){
    ...
    var w = this.localdist(w);
    var h = this.localdist(h);
    o.push(`<path d='M ${this.fix(x+gap)} ${this.fix(y)} L ${this.fix(x+w-gap)} ${this.fix(y)} L ${this.fix(x+w)} ${this.fix(y-0.5*h)} L ${this.fix(x+w-gap)} ${this.fix(y-h)} L ${this.fix(x+gap)} ${this.fix(y-h)} L ${this.fix(x)} ${this.fix(y-0.5*h)} z' ${this.to_fillonly_str(g)} />`);
    o.push(`<path d='M ${this.fix(x+gap)} ${this.fix(y)} L ${this.fix(x+w-gap)} ${this.fix(y)} L ${this.fix(x+w)} ${this.fix(y-0.5*h)} L ${this.fix(x+w-gap)} ${this.fix(y-h)} L ${this.fix(x+gap)} ${this.fix(y-h)} L ${this.fix(x)} ${this.fix(y-0.5*h)} z' ${this.to_drawonly_str(g)} />`);
    return o.join('\n');
  }
  ...
  p_triangle(x,y,w,h,g){
    ...
    var w = this.localdist(w);
    var h = this.localdist(h);
    o.push(`<path d='M ${this.fix(x)} ${this.fix(y)} L ${this.fix(x+w)} ${this.fix(y)} L ${this.fix(x+0.5*w)} ${this.fix(y-h)} z' ${this.to_fillonly_str(g)} />`);
    o.push(`<path d='M ${this.fix(x)} ${this.fix(y)} L ${this.fix(x+w)} ${this.fix(y)} L ${this.fix(x+0.5*w)} ${this.fix(y-h)} z' ${this.to_drawonly_str(g)} />`);
    return o.join('\n');
  }
  ...
  p_line(x1,y1,x2,y2,g){
    ...
    if(g.arrowhead==1){
      let id1 = this.create_arrowhead('end',g);
      o.push(`<line x1='${this.fix(x1)}' y1='${this.fix(y1)}' x2='${this.fix(x2)}' y2='${this.fix(y2)}' ${this.to_drawonly_str(g)} marker-end='url(#${id1})' />`);
    }else if(g.arrowhead==2){
      let id2 = this.create_arrowhead('start',g);
      o.push(`<line x1='${this.fix(x1)}' y1='${this.fix(y1)}' x2='${this.fix(x2)}' y2='${this.fix(y2)}' ${this.to_drawonly_str(g)} marker-start='url(#${id2})' />`);
    }else if(g.arrowhead==3){
      let id1 = this.create_arrowhead('end',g);
      let id2 = this.create_arrowhead('start',g);
      o.push(`<line x1='${this.fix(x1)}' y1='${this.fix(y1)}' x2='${this.fix(x2)}' y2='${this.fix(y2)}' ${this.to_drawonly_str(g)} marker-end='url(#${id1})' marker-start='url(#${id2})' />`);
    }else{
  ...
  p_qbezier_line(x0,y0, x1,y1, x2,y2, g){
    ...
    if(g.arrowhead==1){
      let id1 = this.create_arrowhead('end',g);
      o.push(`<path d='M ${this.fix(x0)},${this.fix(y0)} Q ${this.fix(x1)},${this.fix(y1)},${this.fix(x2)},${this.fix(y2)}' ${this.to_drawonly_str(g)} marker-end='url(#${id1})' />`);
    }else if(g.arrowhead==2){
      let id2 = this.create_arrowhead('start',g);
      o.push(`<path d='M ${this.fix(x0)},${this.fix(y0)} Q ${this.fix(x1)},${this.fix(y1)},${this.fix(x2)},${this.fix(y2)}' ${this.to_drawonly_str(g)} marker-start='url(#${id2})' />`);
    }else if(g.arrowhead==3){
      let id1 = this.create_arrowhead('end',g);
      let id2 = this.create_arrowhead('start',g);
      o.push(`<path d='M ${this.fix(x0)},${this.fix(y0)} Q ${this.fix(x1)},${this.fix(y1)},${this.fix(x2)},${this.fix(y2)}' ${this.to_drawonly_str(g)} marker-end='url(#${id1})' marker-start='url(#${id2})' />`);
    }else{
  ...
  p_cbezier_line(x0,y0, x1,y1, x2,y2, x3,y3, g){
    ...
    if(g.arrowhead==1){
      let id1 = this.create_arrowhead('end',g);
      o.push(`<path d='M ${this.fix(x0)},${this.fix(y0)} C ${this.fix(x1)},${this.fix(y1)},${this.fix(x2)},${this.fix(y2)},${this.fix(x3)},${this.fix(y3)}' ${this.to_drawonly_str(g)} marker-end='url(#${id1})' />`);
    }else if(g.arrowhead==2){
      let id2 = this.create_arrowhead('start',g);
      o.push(`<path d='M ${this.fix(x0)},${this.fix(y0)} C ${this.fix(x1)},${this.fix(y1)},${this.fix(x2)},${this.fix(y2)},${this.fix(x3)},${this.fix(y3)}' ${this.to_drawonly_str(g)} marker-start='url(#${id2})' />`);
    }else if(g.arrowhead==3){
      let id1 = this.create_arrowhead('end',g);
      let id2 = this.create_arrowhead('start',g);
      o.push(`<path d='M ${this.fix(x0)},${this.fix(y0)} C ${this.fix(x1)},${this.fix(y1)},${this.fix(x2)},${this.fix(y2)},${this.fix(x3)},${this.fix(y3)}' ${this.to_drawonly_str(g)} marker-end='url(#${id1})' marker-start='url(#${id2})' />`);
    }else{
  ...
  p_arc(x1,y1,x2,y2,Rx,Ry,bigarcflag,g){
    ...
    if(g.arrowhead==1){
      ...
      let id1 = this.create_arrowhead('end',g);
      return(`<path d='M${x1},${y1} A${Rx},${Ry},0,${bigarcflag},0,${x2},${y2}' ${this.to_drawonly_str(g)} marker-end='url(#${id1})' />`);
    }else if(g.arrowhead==2){
      ///back-pointing-arrow-head
      let id2 = this.create_arrowhead('start',g);
      return(`<path d='M${x1},${y1} A${Rx},${Ry},0,${bigarcflag},0,${x2},${y2}' ${this.to_drawonly_str(g)} marker-start='url(#${id2})' />`);
    }else if(g.arrowhead==3){
      ///front-and-back-pointing-arrow-heads
      let id1 = this.create_arrowhead('end',g);
      let id2 = this.create_arrowhead('start',g);
      return(`<path d='M${x1},${y1} A${Rx},${Ry},0,${bigarcflag},0,${x2},${y2}' ${this.to_drawonly_str(g)} marker-end='url(#${id1})' marker-start='url(#${id2})' />`);
    }else{
      ///no-arrow-head
