...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  do_finalize(text,style) {
    ...
    this.my_linegradient_array.forEach(p => {
      /*
     ...
     */
     o.push(`<linearGradient id="${p.id}"><stop offset="0%" stop-color="${p.startcolor}"/><stop offset="50%" stop-color="${p.midcolor}"/><stop offset="100%" stop-color="${p.stopcolor}" x1="${p.x1}" y1="${p.y1}" x2="${p.x2}" y2="${p.y2}"/></linearGradient>`);
    })
    o.push('</defs>');
...
//    css_style.push(`background-color:white`)
  ...
  to_shadeonly_str(g,isinherit) {
    g = g||{};
    var d = [];
    let s1 = this.to_startcolor_str(g);
    let s2 = this.to_midcolor_str(g);
    let s3 = this.to_stopcolor_str(g);
    ...
    if(shade=='line'){
      ...
      this.my_linegradient_array.push(p);
  ...
  to_startcolor_str(g){
    if (g.startcolor) {
      return(`${this.string_to_color_name(g.startcolor)}`);
    } 
    if(this.config.startcolor){
      return(`${this.string_to_color_name(this.config.startcolor)}`);
    }
    return '';
  }
  to_midcolor_str(g){
    if (g.midcolor) {
      return(`${this.string_to_color_name(g.midcolor)}`);
    } 
    if(this.config.midcolor){
      return(`${this.string_to_color_name(this.config.midcolor)}`);
    }
    return '';
  }
  to_stopcolor_str(g){
    if (g.stopcolor) {
      return(`${this.string_to_color_name(g.stopcolor)}`);
    } 
    if(this.config.stopcolor){
      return(`${this.string_to_color_name(this.config.stopcolor)}`);
    }
    return '';
  }
  to_fillcolor_str(g){
