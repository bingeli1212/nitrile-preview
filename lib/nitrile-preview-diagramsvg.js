...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  do_finalize(text,style) {
    ...
    if (style.aspectratio){
      ///if user need to set a new aspect ratio, make sure to turn the keepAspectRatio attribute off and set it to 'none', otherwise, the css
      /// aspect-ratio: won't work
  ...
  p_fillclipath(_coords, g) {
    ...
    names.forEach(x => {
      if(this.my_paths.has(x)){
        ...
        path = this.offset_and_scale_coords(path,this.refx,this.refy,this.refs,this.refs,0);
