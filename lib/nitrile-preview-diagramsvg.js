...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  do_finalize(s,style) {
    ...
    var text = o.join('\n');
    if(style.flex){
      var css_width = `width:${style.flex*this.viewport_width*this.viewport_unit}mm;`;
      var css_height = `height:${style.flex*this.viewport_height*this.viewport_unit}mm;`;
  ...
  p_slopedtext(x1,y1,x2,y2,txt,ta,g){
    ...
    if(this.has_texts(g)){
      ...
      if(ta=='bot'){
        o.push(`<text font-family='${fontfamily}' font-style='${fontstyle}' font-weight='${fontweight}' font-size='${fontsize}' text-anchor='middle' ${this.g_to_svg_textdraw_str(g)} dx='${linelength/2}' dy='+1.00em'>${txt}</text>`);
      }else{
  ...
  p_image(imagefile){
    ...
    o.push(`<image href='${imagefile}' width='${width}' height='${height}' x='0' y='0' />`);
    return o.join('\n');
  }
