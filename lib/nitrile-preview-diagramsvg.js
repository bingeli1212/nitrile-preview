...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  p_label(x, y, txt, ts, ta, g) {
    ...
    if (ts==2) {
      ...
      var [x,y] = this.to_adjusted_xy_for_label(x,y,valign,anchor)
      d.push(`<text transform='translate(${x} ${y}) scale(${factor})' font-size='12pt' text-anchor='${anchor}' ${this.to_textdraw_str(g)} dy='${this.tokenizer.dy}em'>${txt}</text>`);
    }
    return d.join('\n');
  }
