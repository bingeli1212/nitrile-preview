...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  do_finalize(text,style) {
    ...
    text = `<svg style='${css}' xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' fill='currentColor' stroke='currentColor' viewBox='0 0 ${this.fix(vw)} ${this.fix(vh)}' >${text}</svg>`;
    return {text};
  }
  ...
  p_cairo(x,y,txt,extent,g){
    if(!txt) return '';
    [x,y] = this.move_xy(x,y);//move according to the current origin settings
    var x = this.localx(x);
    var y = this.localy(y);
    ...
    var segment_array = this.tokenizer.to_cairo_segment_array(txt,g);
    var pp = this.cairo_segment_array_to_pp(segment_array,extent,x,y,factor);
    var all = [];
    pp.forEach((p,i,arr) => {
      if(p.key=='latin' || p.key=='punc' || p.key=='unicode'){
        ...
        all.push(`<text transform='translate(${this.fix(x)} ${this.fix(y)}) scale(${factor})' font-family='${fontfamily}' font-style='${fontstyle}' font-weight='${fontweight}' font-size='${this.tokenizer.fs}pt' text-anchor='${anchor}' ${this.g_to_svg_textdraw_str(g)} dy='${this.tokenizer.text_dy_pt}pt' textLength='${this.fix(textlength)}' lengthAdjust='spacingAndGlyphs'>${txt}</text>`);
      }else if(p.key=='svg'){
        var x = p.x;
        var y = p.y;
        var w = p.w;
        ...
        all.push(`<svg x='${this.fix(x)}' y='${this.fix(y)}' ${opt} >${p.svg}</svg>`);
      }
    });
    return all.join('\n');
  }
  ...
  /// user coordinates; 
  ///
  cairo_segment_array_to_pp(segment_array,extent,x,y,factor){
    ...
    for(var p of pp){
      var w1 = w + p.w;
      if(w==0){
        //first element of first line, always honored
        p.x = 0;
        p.seq = seq;
        w = p.w;
      }
    ...
    ///at this stage we will add in the input x/y so that all segments
    // are translated this much distance
    pp.forEach((p) => {
      p.x += x;
      p.y += y;
    })
