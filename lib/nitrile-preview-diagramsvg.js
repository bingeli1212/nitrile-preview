...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  do_finalize(text,style) {
    ...
    this.my_lineargradient_array.forEach(p => {
      ...
      */
      if(p.ss.length==0){
        o.push(`<linearGradient id="${p.id}" x1="${p.x1}" y1="${p.y1}" x2="${p.x2}" y2="${p.y2}"> <stop offset="0%" stop-color=""/> <stop offset="100%" stop-color=""/> </linearGradient>`);
      }else if(p.ss.length==1){
        o.push(`<linearGradient id="${p.id}" x1="${p.x1}" y1="${p.y1}" x2="${p.x2}" y2="${p.y2}"> <stop offset="0%" stop-color="${p.ss[0]}"/> <stop offset="100%" stop-color=""/> </linearGradient>`);
    ...
    this.my_radialgradient_array.forEach(p => {
      ...
      */
      if(p.ss.length==0){
        o.push(`<radialGradient id="${p.id}" cx="${p.cx}" cy="${p.cy}" fx="${p.fx}" fy="${p.fy}" r="${p.r}"> <stop offset="0%" stop-color=""/> <stop offset="100%" stop-color=""/> </radialGradient>`);
      }else if(p.ss.length==1){
        o.push(`<radialGradient id="${p.id}" cx="${p.cx}" cy="${p.cy}" fx="${p.fx}" fy="${p.fy}" r="${p.r}"> <stop offset="0%" stop-color="${p.ss[0]}"/> <stop offset="100%" stop-color=""/> </radialGradient>`);
      }else if(p.ss.length==2){
        ...
        o.push(`<radialGradient id="${p.id}" cx="${p.cx}" cy="${p.cy}" fx="${p.fx}" fy="${p.fy}" r="${p.r}"> <stop offset="0%" stop-color="${p.ss[0]}"/> <stop offset="50%" stop-color="${p.ss[1]}"/> <stop offset="100%" stop-color="${p.ss[2]}"/> </radialGradient>`);
      }
    });
    o.push('</defs>');
...
//    css_style.push(`background-color:white`)
  ...
  p_label(x, y, txt, ts, ta, g) {
    ...
    if (ts==2) {
      //math text
      if(g.fontcolor){
        g.svgfontcolor = this.string_to_color_name(g.fontcolor);
      }
