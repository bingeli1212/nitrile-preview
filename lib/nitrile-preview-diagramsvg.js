...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  do_finalize(s,style) {
    ...
    css += css_height;
    if(1){
      let gw = this.viewport_grid*this.viewport_unit*this.MM_to_PX;
      let s_svg = `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="${gw}" height="${gw}"><rect fill="none" stroke="rgb(235,235,235)" x="0" y="0" width="${gw}" height="${gw}"/></svg>`;
      css += `; background-image:url("data:image/svg+xml;charset=UTF-8,${encodeURIComponent(s_svg)}"); background-repeat:repeat`;
    }
    ///
    ///THE OLD WAY
    ...
    if(1){
      ...
      sub = this.translator.uncode(style,this.sub);
    }
    return { img, sub };
  }
  ...
  p_rect(x,y,w,h,g){
    ...
    if(this.has_strokes(g)){
      d.push(`<rect x="${this.fix(x)}" y="${this.fix(y-h)}" width="${this.fix(w)}" height="${this.fix(h)}" ${this.g_to_svg_drawonly_str(g)} />`);
    }
    return d.join('\n');
  }
  ///
  ...
  p_triangle(x1,y1,x2,y2,x3,y3,g){
    ...
    if(this.has_strokes(g)){
      d.push(`<polygon points="${this.fix(x1)},${this.fix(y1)} ${this.fix(x2)},${this.fix(y2)} ${this.fix(x3)},${this.fix(y3)}" ${this.g_to_svg_drawonly_str(g)} />`);
    }
    return d.join('\n');
  }
  ///
