...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  p_label(x,y,txt,ta,g) {
    ...
    if (this.to_math_int(g)) {
      ...
      var [x,y] = this.to_adjusted_xy_for_label(x,y,valign,anchor)
      d.push(`<text transform='translate(${x} ${y}) scale(${factor})' font-size='${this.tokenizer.fs}pt' text-anchor='${anchor}' ${this.to_textdraw_str(g)} dy='${this.tokenizer.text_dy_pt}pt'>${txt}</text>`);
    }
    return d.join('\n');
  }
  p_text(x,y,txt,ta,g){
