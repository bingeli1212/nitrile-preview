...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  do_finalize(text,style) {
    ...
    this.my_arrowhead_array.forEach((p) => {
     ...
     if(p.position=='end'){
        if(linesize){
          // none-zero linesize
          o.push(`<marker id='${p.id}' markerWidth='3' markerHeight='4' refX='3' refY='2' orient='auto'> <path d='M0.5,0 L3,1.5 L3,2.5 L0.5,4 L1.25,2.5 L1.25,1.5 z' fill='${this.string_to_color_name(p.linecolor,'inherit')}' stroke='none'/> </marker>`)
        }else{
          // zero linesize
          o.push(`<marker id='${p.id}' markerWidth='3' markerHeight='4' refX='3' refY='2' orient='auto'> <path d='M0.5,0 L3,1.75 L3,2.25 L0.5,4 L1.25,2.5 L1.25,1.5 z' fill='${this.string_to_color_name(p.linecolor,'inherit')}' stroke='none'/> </marker>`)
        }
      }else if(p.position=='start'){
        if(linesize){
          // non-zero linesize
          o.push(`<marker id='${p.id}' markerWidth='3' markerHeight='4' refX='0' refY='2' orient='auto'> <path d='M2.5,0 L1.75,1.5 L1.75,2.5 L2.5,4 L0,2.5 L0,1.5 z' fill='${this.string_to_color_name(p.linecolor,'inherit')}' stroke='none'/> </marker>`)
        }else{
          // zero linesize
          o.push(`<marker id='${p.id}' markerWidth='3' markerHeight='4' refX='0' refY='2' orient='auto'> <path d='M2.5,0 L1.75,1.5 L1.75,2.5 L2.5,4 L0,2.25 L0,1.75 z' fill='${this.string_to_color_name(p.linecolor,'inherit')}' stroke='none'/> </marker>`)
        }
      }
    })
  ...
  string_to_color_name(s,def='none') {
    if (!s) {
      return def;
    } 
  ...
  to_shadeonly_str(g,isinherit) {
    ...
    let angle = this.to_angle_float(g);//this should be  number
    if(shade=='linear'){
      let p = {};
      p.x1 = 0;
      p.y1 = 0;
  ...
  to_dot_str(g){
    ...
    if (dotcolor) {
      o.push(`fill='${this.string_to_color_name(dotcolor)}'`);
    } else {
      o.push(`fill='inherit'`);
    }
    o.push(`stroke='none'`);
  ...
  to_dotsize_radius(g){
    let dotsize = this.get_float_prop(g,'dotsize',this.def_dotsize);
    return (dotsize/2*1.333);
  }

  to_fontsize_str(g){
    var fontsize = this.get_string_prop(g,'fontsize',this.def_fontsize);
    return fontsize;
  }
  ...
  to_fillcolor_str(g){
    let fillcolor = this.get_string_prop(g,'fillcolor','');
    if (fillcolor) {
      return(`${this.string_to_color_name(fillcolor)}`);
    } 
    return '';
  }

  to_barlength_length(g){
    let barlength = this.get_float_prop(g,'barlength',this.def_barlength);
    return barlength;
  }
