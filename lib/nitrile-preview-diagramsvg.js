...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
    ...
    this.my_arrowhead_array.forEach((p) => {
      ...
      */
      if(p.position=='end'){
        ...
        let lw = linesize * 1.333; /// convert to px
        let hw = 0.5;
        o.push(`<marker id='${p.id}' markerWidth='3' markerHeight='4' refX='3' refY='2' orient='auto'> <path d='M0.5,0 L3,1.5 L3,2.5 L0.5,4 L1.25,2.5 L1.25,1.5 z' stroke='none' fill='${this.string_to_color_name(p.linecolor)}'/> </marker>`)
      }else if(p.position=='start'){
        o.push(`<marker id='${p.id}' markerWidth='3' markerHeight='4' refX='0' refY='2' orient='auto'> <path d='M2.5,0 L1.75,1.5 L1.75,2.5 L2.5,4 L0,2.5 L0,1.5 z' stroke='none' fill='${this.string_to_color_name(p.linecolor)}'/> </marker>`)
      }
    })
  ...
  p_label(x, y, txt, ts, ta, g) {
    ...
    if (ts==2) {
      ...
      var anchor = 'middle', dy='0.3em', valign = '';
      if (ta==='lrt') {
        anchor = 'start', dy='0.8em', valign = 'lower';
      } else if (ta==='bot') {
        anchor = 'middle', dy='0.8em', valign = 'lower';
      } else if (ta==='llft') {
        anchor = 'end', dy='0.8em', valign = 'lower';
      } else if (ta==='urt') {
        anchor = 'start', dy='-0.2em', valign = 'upper';
      } else if (ta==='top') {
        anchor = 'middle', dy='-0.2em', valign = 'upper';
      } else if (ta==='ulft') {
        anchor = 'end', dy='-0.2em', valign = 'upper';
      } else if (ta==='rt') {
