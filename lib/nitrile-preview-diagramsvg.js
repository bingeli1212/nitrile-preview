...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  do_finalize(text,style) {
    ...
    text = `<svg innersvg='${this.fix(vw)} ${this.fix(vh)} ${this.fix(vh)}' style='${css}' xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' fill='currentColor' stroke='currentColor' viewBox='0 0 ${this.fix(vw)} ${this.fix(vh)}' >${text}</svg>`;
    return {text};
  }
  ...
  p_cairo(x,y,txt,extent,g){
    ...
    var indent = 0;
    var hanging = 0;
    var pp = this.cairo_segment_array_to_pp(segment_array,extent,x,y,factor,align,indent,hanging);
    var all = [];
    pp.forEach((p,i,arr) => {
  ...
  /// @indent: integer
  /// @hanging: integer
  cairo_segment_array_to_pp(segment_array,extent,x,y,factor,align,indent,hanging){
