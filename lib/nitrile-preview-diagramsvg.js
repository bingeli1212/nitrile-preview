...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  do_finalize(text,style) {
    ...
    text = `<svg xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' fill='currentColor' stroke='currentColor' viewBox='0 0 ${this.fix(vw)} ${this.fix(vh)}' width='${this.fix(vw*zoom)}' height='${this.fix(vh*zoom)}' preserveAspectRatio='none' >${text}</svg>`;
    width=`${vw*zoom}px`
    height=`${vh*zoom}px`;
