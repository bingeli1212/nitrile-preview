  ...
  p_cairo(x,y,txt,extent,g){
    ...
    pp.forEach((p,i,arr) => {
      if(p.key=='latin' || p.key=='punc' || p.key=='unicode'){
        ...
        all.push(`<text transform='translate(${this.fix(x)} ${this.fix(y)}) scale(${factor})' font-family='${fontfamily}' font-style='${fontstyle}' font-size='${this.tokenizer.fs}pt' text-anchor='${anchor}' ${this.g_to_svg_textdraw_str(g)} dy='${this.tokenizer.text_dy_pt}pt' textLength='${this.fix(textlength)}' lengthAdjust='spacingAndGlyphs'>${txt}</text>`);
      }else if(p.key=='svg'){
        var x = p.x;
        var y = p.y;
        var w = p.w;
