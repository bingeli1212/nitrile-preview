...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  p_textpath(x1,y1,x2,y2,txt,ta,g){
    ...
    if(this.has_texts(g)){
      ...
      o.push(`<text x='${x}' y='${y}' font-family='${fontfamily}' font-style='${fontstyle}' font-size='${fontsize}' text-anchor='middle' ${this.g_to_svg_textdraw_str(g)} dy='-0.50em'>${txt}</text>`);
  ...
  g_to_svg_fontsize_str(g) {
    ...
    console.log('james','fontsize=',s)
    var percent = this.string_to_fontsize_percent(s);
    if(percent){
      console.log('james','fontsize=',percent)
      return percent;
    }
    var number = parseFloat(s);
    if(Number.isFinite(number)){
      number = `${number}pt`;
      console.log('james','fontsize=',number)
      return number;
    }
    return '';
  }
