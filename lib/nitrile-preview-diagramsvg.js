...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  do_finalize(s,style) {
    ...
    this.my_arrowhead_array.forEach((p) => {
     ...
     if(p.position=='end'){
        ...
        o.push(`<marker id='${p.id}' markerWidth='4' markerHeight='4' refX='${4-Dx}' refY='2' orient='auto'> <path d='M0.5,0 L4,2 L0.5,4 z' fill='${this.string_to_svg_color(p.linecolor,'','inherit')}' stroke='none'/> </marker>`)
      }else if(p.position=='start'){
        let Dx = linesize/2/4*3.5;
        o.push(`<marker id='${p.id}' markerWidth='4' markerHeight='4' refX='${0+Dx}' refY='2' orient='auto'> <path d='M3.5,0 L0,2 L3.5,4 z' fill='${this.string_to_svg_color(p.linecolor,'','inherit')}' stroke='none'/> </marker>`)
      }
    })
    ...
    if(this.my_filter_blur_id){
      ...
      */
      o.push(`<filter id="${this.my_filter_blur_id}"> <feGaussianBlur stdDeviation="0.111"/> </filter>`);
    }
    o.push('</defs>');
    ...
    var text = o.join('\n');
    if(style.stretch){
      var css_width = `width:${100*style.stretch}%;`;
      var css_height = '';
    }else if(style.width && style.height){
      var css_width = `width:${style.width}mm;`;
      var css_height = `height:${style.height}mm;`;
    }else if(style.width){
      var css_width = `width:${style.width}mm;`;
      var css_height = `height:${style.width/this.viewport_width*this.viewport_height}mm;`;
    }else if(style.height){
      var css_width = `width:${style.height/this.viewport_height*this.viewport_width}mm;`;
      var css_height = `height:${this.height}mm;`;
    }else{
    ...
    ///note that w, h, and mid are in pts
  ...
  ___do_drawcontrolpoints(opts,g,txts,coords){    
    var s = [];
  ...
  p_slopedtext(x1,y1,x2,y2,txt,ta,g){
    ...
    if(this.has_texts(g)){
      ...
      if(ta=='bot'){
        o.push(`<text font-family='${fontfamily}' font-style='${fontstyle}' font-weight='${fontweight}' font-size='${fontsize}' text-anchor='middle' ${this.g_to_svg_textdraw_str(g)} dx='${linelength/2}' dy='+0.50em'>${txt}</text>`);
      }else if(ta=='top'){
        o.push(`<text font-family='${fontfamily}' font-style='${fontstyle}' font-weight='${fontweight}' font-size='${fontsize}' text-anchor='middle' ${this.g_to_svg_textdraw_str(g)} dx='${linelength/2}' dy='-0.50em'>${txt}</text>`);
      }else{
        o.push(`<text font-family='${fontfamily}' font-style='${fontstyle}' font-weight='${fontweight}' font-size='${fontsize}' text-anchor='middle' ${this.g_to_svg_textdraw_str(g)} dx='${linelength/2}' dy=''>${txt}</text>`);
      }
  ...
  p_slopedmath(x1,y1,x2,y2,txt,ta,g){
    ...
    if(this.has_texts(g)){
      ...
      if(this.has_texts(g)){
        ...
        let s1 = `<g transform='translate(0,${-fh*0.2}) translate(${x},${y}) rotate(${-rotate}) translate(${dx},${dy})'><svg width='${width}' height='${height}' fill='${svgfontcolor}' stroke='${svgfontcolor}' viewBox='0 0 ${vw} ${vh}'><defs>${defs.join('\n')}</defs>${s}</svg></g>`;
        o.push(s1);
      }
    }
    return o.join('\n');
  ...
  p_rect(x,y,w,h,g){
    ...
    y = y - h;
    if(this.has_shade(g)){
      d.push(`<rect x='${this.fix(x)}' y='${this.fix(y)}' width='${this.fix(w)}' height='${this.fix(h)}' ${this.g_to_svg_shadeonly_str(g)} />`);
    }else if(this.has_fills(g)){
  ...
  p_ellipse(x,y,Rx,Ry,angle,g){
    ...
    ///anti-clockwise
    if(this.has_shades(g)){
      o.push(`<path d='M${x1},${y1} A${Rx},${Ry},${-angle},1,0,${x2},${y2}' ${this.g_to_svg_shadeonly_str(g)} />`);//anti-clockwise fill
      o.push(`<path d='M${x1},${y1} A${Rx},${Ry},${-angle},1,1,${x2},${y2}' ${this.g_to_svg_shadeonly_str(g)} />`);//clockwise fill
    }else if(this.has_fills(g)){
