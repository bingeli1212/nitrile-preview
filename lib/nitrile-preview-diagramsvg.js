...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  string_to_svg_color(s,def='none') {
    if (!s) {
      return def;
    } 
    else if (typeof s === 'string') {
      s = this.w3color_to_hexstring(s);
      s = s.slice(1);
  ...
  to_textdraw_str(g) {
    ...
    if (fontcolor) {
      d.push(`fill='${this.string_to_svg_color(fontcolor)}'`);
    }
    d.push(`stroke='none'`);
    return d.join(' ');
  }
  ...
  ticks() {
    var s = [];
    if (this.tickcolor) {
      s.push(`stroke='${this.string_to_svg_color(this.tickcolor)}'`);
    } else {
  ...
  p_label(x, y, txt, ts, ta, g) {
    ...
    if (ts==2) {
      ...
      var fontcolor = this.get_string_prop(g,'fontcolor','');
      if(fontcolor){
        g.svgfontcolor = this.string_to_svg_color(fontcolor);
      }else{
        g.svgfontcolor = '';
      }
      ...
      var [x,y] = this.to_adjusted_xy_for_label(x,y,valign,anchor)
      d.push(`<text transform='translate(${x} ${y}) scale(${factor})' font-size='12pt' text-anchor='${anchor}' ${this.to_textdraw_str(g)} dy='0.78em'>${txt}</text>`);
    }
    return d.join('\n');
  }
  p_text(x,y,txt,ts,ta,g){
    ...
    */
    ...
    if(1){
      ...
      //
      // For horizontal spacings,
      // we adding a nbsp before and after the text to allow some room 
      // otherwise the label is too close to the left and right anchor point
      //
      // txt = `&#160;${txt}&#160;`;
      //
      ...
      //
      if(valign == 'upper') {
        y -= 3*1.333; // y reduce by 3pt, moving text up
      }else if(valign == 'lower') {
        y += 3*1.333; // y increase by 3pt, moving text down
      }
    }
    if(1){
    ...
    // increase or decrease dy based on the number of lines
    if(ss.length > 1){
      let extra_lines = ss.length - 1;
      if(valign == 'upper'){
        dy -= extra_lines; // dy is in 'em'
      }else if(valign == 'lower'){
        dy = dy; // no change for 'lower'
      }else{
        // for each additional line move 0.5em up
        dy -= extra_lines * 0.5;
      }
    }
    ...
    var ss = ss.map((s,i,arr) => {
      if(i==0){
        return `<tspan x='${x}' dy='${dy}em'>${s}</tspan>`;
      }else{
        return `<tspan x='${x}' dy='1em'>${s}</tspan>`;
      }
    });
    var txt = ss.join('\n');
    var d = [];
    //d.push(`<text style='outline:1px solid currentColor;padding:4pt;' font-size='${fontsize?fontsize:12}pt' text-anchor='${anchor}' ${this.to_textdraw_str(g)} x='${x}' y='${y}' dy='${dy}'>${txt}</text>`);
    d.push(`<text font-size='${fontsize?fontsize:12}pt' text-anchor='${anchor}' ${this.to_textdraw_str(g)} x='${x}' y='${y}' dy='${dy}em'>${txt}</text>`);
    return d.join('\n');
  }
  ...
  to_drawonly_str(g) {
    ...
    if (linecolor) {
      d.push(`stroke='${this.string_to_svg_color(linecolor)}'`);
    } else {
  ...
  to_fillonly_str(g,isinherit) {
    ...
    if(fillcolor == 'none'){
      ...
      d.push(`fill='${this.string_to_svg_color(fillcolor)}'`);
    } 
    else if (isinherit) {
      d.push(`fill='inherit'`);
    } 
  ...
  to_dot_str(g){
    ...
    if (dotcolor) {
      o.push(`fill='${this.string_to_svg_color(dotcolor)}'`);
    } else {
      o.push(`fill='inherit'`);
    }
    o.push(`stroke='none'`);
  ...
  to_svg_shadecolor_ss(g){
    ...
    ss = ss.map(s => this.string_to_svg_color(s));
    return ss;
  }
  ...
  to_svg_colorbox (g) {
    ...
    var h = g.height;
    var color = g.data;;
    o.push(`<rect x='0' y='0' width='${this.fix(w)}mm' height='${this.fix(h)}mm' stroke='none' fill='${this.string_to_svg_color(color)}' />`);
    var s = o.join('\n');
    return {s,w,h};
  }
  to_svg_vbarchart (g) {
  ...
  to_adjusted_xy_for_label(x,y,valign,anchor){
    if(1){
      ...
      //
      if(valign == 'upper') {
        y -= 3.5; // y reduce by approx. 2.5*1.333, moving text up
      }else if(valign == 'lower') {
        y += 3.5; // y increase by approx 2.5*1.333, moving text down
      }else{
        y += 0;
      }
    }
