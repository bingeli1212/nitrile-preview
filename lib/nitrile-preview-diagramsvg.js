...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  do_one_frame(style,ss,env) {
    ...
    o.push(s);    
    ///CSS
    var css = this.translator.css('DIA');
    ...
    if(this.imgs.length){
      let s = this.imgs[0];
      let srcs = this.to_img_list(s);
      imgsrc = this.translator.choose_image_file(srcs);
      ...
      let s_data = encodeURIComponent(s_svg);
      css += `; background-image:url("data:image/svg+xml;charset=UTF-8,${s_data}")`;
      css += `; background-size:fill`;
      var img = `<img style='${css}' width='${vw}' height='${vh}' src='${imgsrc}' id='${imgid}' />`;
    }else{
      let s_svg = `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="currentColor" stroke="currentColor" viewBox="0 0 ${this.fix(vw)} ${this.fix(vh)}" >${img}</svg>`;
      let s_data = encodeURIComponent(s_svg);
      var img = `<object style='${css}' width='${vw}' height='${vh}' data='data:image/svg+xml;charset=UTF-8,${s_data}' id='${imgid}'></object>`;
    }
    var subc = this.translator.uncode(style,this.subtitle);
    return { img, subc, w, h, vw,vh,svgtext,imgsrc,imgid };
  }
  localx(x) {
