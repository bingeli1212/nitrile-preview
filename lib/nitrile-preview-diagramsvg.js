...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  do_finalize(text,style) {
    ...
    this.my_arrowhead_array.forEach((p) => {
      ...
      */
      if(p.position=='end'){
        o.push(`<marker id='${p.id}' markerWidth='3' markerHeight='4' refX='3' refY='2' orient='auto'> <path d='M0,0 L3,2 L0,4 z' stroke='none' fill='${this.string_to_color_name(p.color)}'/> </marker>`)
      }else if(p.position=='start'){
        o.push(`<marker id='${p.id}' markerWidth='3' markerHeight='4' refX='0' refY='2' orient='auto'> <path d='M3,0 L3,4 L0,2 z' stroke='none' fill='${this.string_to_color_name(p.color)}'/> </marker>`)
      }
    })
...
//    css_style.push(`background-color:white`)
  ...
  p_arrow(coords,g){
    ...
    for(var item of items){
      var {iscycled,d} = item; 
      if(!d) continue;
      let id1 = this.create_arrowhead('end',this.get_string_prop(g,'linecolor'));
  ...
  p_revarrow(coords,g){
    ...
    for(var item of items){
      var {iscycled,d} = item; 
      if(!d) continue;
      let id2 = this.create_arrowhead('start',this.get_string_prop(g,'linecolor'));
  ...
  create_arrowhead(position,color){
