...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  p_cairo(x,y,txt,ta,g){
    ...
    */
    ...
    /// (x,y) is now in SVG-coordinates, 
    var max_w = this.assert_float(g.extent,100);
    var pp = this.cairo_segment_array_to_pp(segment_array,max_w,x,y);
    var all = [];
    pp.forEach((p,i,arr) => {
      if(p.key=='text'){
        var anchor = 'start';
        ...
        all.push(`<text transform='translate(${x} ${y}) scale(${factor})' font-family='${fontfamily}' font-style='${fontstyle}' font-size='${this.tokenizer.fs}pt' text-anchor='${anchor}' ${this.g_to_svg_textdraw_str(g)} dy='${this.tokenizer.text_dy_pt}pt'>${txt}</text>`);
      }else if(p.key=='svg'){
        //TODO
      }
    });
  ...
  cairo_segment_array_to_pp(segment_array,extent,x,y){
    ...
    var gap_w = 4;
    var pp = [];
    var p_latin = {key:'latin',text:'',w:0,h:0,mid:0};
    var p_svg = {key:'svg',svg:'',w:0,h:0,mid:0};
    var p_symbol = {key:'symbol',text:'',w:0,h:line_h,mid:line_mid}
      ...
      if(type=='\\cc'){
        ...
        if(cc <= 0xFF){
          ...
          if(p && p.key=='latin'){
            ...
            p = {...p_latin};
            p.text = String.fromCodePoint(cc);
            p.w = w;
            p.h = h;
            p.mid = mid;
            pp.push(p);
    ...
    // to decide the vertical distance between lines
    var w = 0;
    var seq = 0;
    for(let p of pp){
      var w1 = w + p.w;
