  ...
  p_cairo(x,y,txt,extent,g){
    ...
    pp.forEach((p,i,arr) => {
      if(p.key=='latin' || p.key=='punc' || p.key=='unicode'){
        ...
        var y = p.y + p.dy;
        var txt = p.txt;
        all.push(`<text transform='translate(${x} ${y}) scale(${factor})' font-family='${fontfamily}' font-style='${fontstyle}' font-size='${this.tokenizer.fs}pt' text-anchor='${anchor}' ${this.g_to_svg_textdraw_str(g)} dy='${this.tokenizer.text_dy_pt}pt' lengthAdjust='spacingAndGlyphs'>${txt}</text>`);
      }else if(p.key=='svg'){
        //TODO
      }
    });
