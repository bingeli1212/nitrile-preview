...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  do_finalize(s,style) {
    ...
    this.my_arrowhead_array.forEach((p) => {
     ...
     if(p.position=='end'){
        let dx = linesize/2/4*3;
        o.push(`<marker id='${p.id}' markerWidth='3' markerHeight='4' refX='${3-dx}' refY='2' orient='auto'> <path d='M0,0 L3,2 L0,4 L0.5,2 z' fill='${this.string_to_svg_color(p.linecolor,'','inherit')}' stroke='none'/> </marker>`)
      }else if(p.position=='start'){
        let dx = linesize/2/4*3;
        o.push(`<marker id='${p.id}' markerWidth='3' markerHeight='4' refX='${0+dx}' refY='2' orient='auto'> <path d='M3,0 L0,2 L3,4 L2.5,2 z' fill='${this.string_to_svg_color(p.linecolor,'','inherit')}' stroke='none'/> </marker>`)
      }
    })
