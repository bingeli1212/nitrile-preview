...
class NitrilePreviewBase {
  ...
  replace_leading_blanks_with(str, c) {
    ...
    if(newstr.length<str.length){
      let k = str.length-newstr.length;
      if(typeof c=='string'){
        return c.repeat(k)+str;
      }
    }
    return str;
  }
  ...
  body_is_details(ss){
    return ss.length && this.re_body_is_details.test(ss[0]);
  }
  body_is_itemize(ss){
    return ss.length && this.re_is_fullline.test(ss[0]) && this.re_body_is_itemize.test(ss[0]);
  }
  body_is_verbatim(ss){
    return ss.length && this.re_body_is_verbatim.test(ss[0]);
  }
  body_is_bundle(ss){
    var n = ss.length;
    var s0 = ss[0];
  ...
  to_formatted_text(s,fmt){
    ...
    if((v=re_fgroup.exec(fmt))!==null){
      ...
      switch(f_letter){
        case 's': {
          s = ""+s;//convert it to string because it could be a Complex number
          break;
        }
        case 'o': {
