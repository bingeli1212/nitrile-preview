...
class NitrilePreviewBase {
  ...
  string_to_style(s, style) {
    ...
    var style1 = {};
    if(style.switches){
      style1.switches = style.switches;
    }
    if(style.vmap){
      style1.vmap = style.vmap;
    }
    ss.forEach((s,i,arr) => {
      var kk = s.split(':');
      var key = '';
  ...
  /// Choose an image file based on their extentions
  ///
  choose_contex_image_file(ss){
    const supported = ['.pdf','.png','.jpg','.mps','.jpeg','.jbig2','.jb2','.svg','.PDF','.PNG','.JPG','.JPEG','.JBIG2','.JB2','.SVG'];
    for(let s of ss){
      var extname = path.extname(s);
      if(supported.indexOf(extname)>=0){
        return s;
  ...
  update_css_style(html,s){
    ...
    if((v=re_svg.exec(html))!==null){
      var attrs = this.string_to_html_attrs(v[1]);
      var cnt = v[2];
      attrs = attrs.map((p) => {
        let key = p[0];
        let val = p[1];
      ...
      return `<svg ${attrs.map(p => `${p[0]}="${p[1]}"`).join(' ')}> ${cnt} </svg>`;
    }
    if((v=re_img.exec(html))!==null){
      var attrs = this.string_to_html_attrs(v[1]);
      var cnt = v[2];
      attrs = attrs.map((p) => {
        let key = p[0];
      ...
      return `<img ${attrs.map(p => `${p[0]}="${p[1]}"`).join(' ')}> ${cnt} </img>`;
    }
    if((v=re_table.exec(html))!==null){
      var attrs = this.string_to_html_attrs(v[1]);
      var cnt = v[2];
      attrs = attrs.map((p) => {
        let key = p[0];
      ...
      return `<table ${attrs.map(p => `${p[0]}="${p[1]}"`).join(' ')}> ${cnt} </table>`;
    }
    return html;
  }
  ///
