...
class NitrilePreviewBase {
  ...
  join_line (s0, s1) {
    ...
    s1 = s1 || '';
    s1 = s1.trimLeft();
    ///IMPORTANT: this prevents something like the following where
    /// 's0' is "Example: The operator minus (−) is not associative since" and 's1' is "    "
    ///
    ...
    /// - Example: \(a + b\)
    ///   and this is good
    if (s0 && s1 && this.is_cjk_cc(s0.charCodeAt(s0.length-1)) && this.is_cjk_cc(s1.charCodeAt(0))) {
      return s0 + s1;
    } else {
      return s0 + ' ' + s1;
    }
