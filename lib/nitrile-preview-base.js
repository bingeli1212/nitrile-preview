...
class NitrilePreviewBase {
  ...
  constructor(){
    ...
    this.re_body_is_details  = /^(\d+|\+|-|\*)\)\s+(.*)$/u;
    this.re_body_is_checklist  = /^\*\s+\[(.*?)\]\s*(.*)$/u;
    this.re_body_is_argumentlist  = /^\*\s+\{(.*?)\}\s*(.*)$/u;
    this.re_body_is_four_space  = /^\s\s\s\s(.*)$/u;
    this.re_body_is_one_space  = /^\s(.*)$/u;
    this.re_body_is_tilda = /^(~)(\s+)(.*)$/u;
    this.re_body_is_lesss = /^(<)(\s+)(.*)$/u;
  ...
  ///
  g_to_text_fontfamily_flag(g){
    var ff = this.letter_to_array(g.text).map(s=>s.toLowerCase()).filter(s => (s=='t'));
    return ff[0]||'';
  }
  g_to_text_fontstyle_flag(g){
    var ff = this.letter_to_array(g.text).map(s=>s.toLowerCase()).filter(s => (s=='i'));
    return ff[0]||'';
  }
  g_to_text_fontweight_flag(g){
    var ff = this.letter_to_array(g.text).map(s=>s.toLowerCase()).filter(s => (s=='b'));
    return ff[0]||'';
  }
  g_to_text_textdecoration_flag(g){
    var ff = this.letter_to_array(g.text).map(s=>s.toLowerCase()).filter(s => (s=='s'));
    return ff[0]||'';
  }
  ...
  body_is_checklist(ss){
    return ss.length && this.re_is_fullline.test(ss[0]) && this.re_body_is_checklist.test(ss[0]);
  }
  body_is_argumentlist(ss){
    return ss.length && this.re_is_fullline.test(ss[0]) && this.re_body_is_argumentlist.test(ss[0]);
  }
  ...
  string_to_ww_array(template,textalign) {
    ...
    var aa = this.string_to_array(textalign);
    ww = ww.map((w,i) => {
      let ta = aa[i]||'';
      let pw = w;
      let p = {pw,ta};
  ...
  to_formatted_text(s,fmt){
    const re_fgroup = /^%(\-?)(\+?)(\s?)(0?)([1-9][0-9]*)(\.?)([1-9]*)([A-Za-z%])$/;
