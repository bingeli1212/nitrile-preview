...
class NitrilePreviewBase {
  ...
  g_to_text_fontfamily_flag(g){
    var ff = this.string_to_array(g.text).map(s=>s.toLowerCase()).filter(s => (s=='t'));
    return ff[0]||'';
  }
  g_to_text_fontstyle_flag(g){
    var ff = this.string_to_array(g.text).map(s=>s.toLowerCase()).filter(s => (s=='i'));
    return ff[0]||'';
  }
  g_to_text_fontweight_flag(g){
    var ff = this.string_to_array(g.text).map(s=>s.toLowerCase()).filter(s => (s=='b'));
    return ff[0]||'';
  }
  g_to_text_textdecoration_flag(g){
    var ff = this.string_to_array(g.text).map(s=>s.toLowerCase()).filter(s => (s=='s'));
    return ff[0]||'';
  }
