...
class NitrilePreviewDiagramMP extends NitrilePreviewDiagram {
  ...
  do_drawcontrolpoints(opts,g,txt,coords){
    ...
    for (var i = 0; i < coords.length    ; i++) {
      ...
      if (join==='C') {
        ...
        s.push(`fill unitsquare scaled(${dm}) shifted(${-r},${-r}) shifted(${x}*u,${y}*u) ${this.g_to_mp_dotcolor(g)};`);
        if (typeof x0 === 'number' && typeof y0 === 'number') {
          s.push(`fill unitsquare scaled(${dm}) shifted(${-r},${-r}) shifted(${x0}*u,${y0}*u) ${this.g_to_mp_dotcolor(g)};`);
          x0 = null;
          y0 = null;
        }
        s.push(`fill fullcircle scaled(${dm}) shifted(${i3}*u,${i4}*u) ${this.g_to_mp_dotcolor(g)};`);
        s.push(`fill fullcircle scaled(${dm}) shifted(${i5}*u,${i6}*u) ${this.g_to_mp_dotcolor(g)};`);
      } if (join==='Q') {
        var i3 = this.localx(z0[3]);
        var i4 = this.localy(z0[4]);
        s.push(`fill unitsquare scaled(${dm}) shifted(${-r},${-r}) shifted(${x}*u,${y}*u) ${this.g_to_mp_dotcolor(g)};`);
  ...
  to_texts(g) {
    var s = '';
    var fontcolor = this.g_to_fontcolor_string();
    if (fontcolor) {
  ...
  g_to_mp_dotcolor(g){
    ...
    if(s){
      return(`withcolor ${this.string_to_mp_color(s)}`);
    }
    return '';
  }
