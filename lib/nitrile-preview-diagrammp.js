...
class NitrilePreviewDiagramMP extends NitrilePreviewDiagram {
  ...
  p_label(x,y,txt,ts,ta,g){
    ...
    var fontsize = this.to_fontsize_str(g);
  ...
  to_fillcolor_str(g){
    let s = '';
    if (g.fillcolor) {
      s=(`${this.to_colors(g.fillcolor)}`);
    } else if(this.config.fillcolor){
      s=(`${this.to_colors(this.config.fillcolor)}`);
    } 
    if(s){
      if(typeof g.opacity==='number'){
        s = `transparent(1,${g.opacity},${s})`;
      }else if(typeof g.opacity==='string' && g.opacity.length){
        s = `transparent(1,${g.opacity},${s})`;
      }else if(typeof this.config.opacity==='number'){
        s = `transparent(1,${this.config.opacity},${s})`;
  ...
  to_shadecolor_arr(g) {
    var ss = [];
    if (g.shadecolor) {
      var ss = this.string_to_array(g.shadecolor)
    }else if (this.config.shadecolor) {
      var ss = this.string_to_array(this.config.shadecolor)
    }
  ...
  to_draws(g) {
    ...
    if (linecolor) {
      o.push(`withcolor ${this.to_colors(linecolor)}`);
    }
    return o.join(' ');
  }
  ...
  to_texts(g) {
    ...
    if (fontcolor) {
      var withcolor = `withcolor ${this.to_colors(fontcolor)}`;
    }
    return withcolor;
  }
