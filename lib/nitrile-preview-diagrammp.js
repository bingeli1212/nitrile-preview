...
class NitrilePreviewDiagramMP extends NitrilePreviewDiagram {
  ...
  p_shape(x,y,p,g){
    ...
    for( var item of items ) {
      ...
      if(!d) continue;
      if(iscycled){
        o.push(`fill (${d}) xscaled(${sx}) yscaled(${sy}) shifted(${x},${y}) scaled(u) ${this.to_fills(g,1)};`);
      }
      o.push(`${this.to_leads(g)} draw (${d}) xscaled(${sx}) yscaled(${sy}) shifted(${x},${y}) scaled(u) ${this.to_draws(g)};`);
    }
    return o.join('\n');
  }
