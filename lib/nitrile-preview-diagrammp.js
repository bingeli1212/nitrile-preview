...
class NitrilePreviewDiagramMP extends NitrilePreviewDiagram {
  ...
  do_finalize(s,style) {
    ...
    if (style && style.width && style.height){
      ...
      if (ht_percent) {
        d.push(`textheight := textheight*${ht_percent};`);
        d.push(`pu := textheight/${ym};`);
        d.push(`ratio_h := pu/u*fr;`);  
      }else{
      ...
      d.push(`draw wheel xscaled(ratio_w) yscaled(ratio_h);`);
      ...
      d.push(`draw wheel scaled(ratio_w);`);
    }
    d.push('\\stopMPcode{}');
    var text = d.join('\n');
    ///if float:right
