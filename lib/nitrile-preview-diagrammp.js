...
class NitrilePreviewDiagramMP extends NitrilePreviewDiagram {
  ...
  do_drawarc(opt,txt,g,coords){
    ...
    for (var i = 1; i < coords.length; ++i) {
      ...
      if (this.sweepflag) {
        o.push(`${this.to_leads(g)} draw subpath (${this.fix(ang2/45)},${this.fix(ang1/45)}) of fullcircle xscaled(${2 * Rx}) yscaled(${2 * Ry}) rotated(${Phi}) scaled(u) shifted(${Cx}*u,${Cy}*u);`);
      } else {
        o.push(`${this.to_leads(g)} draw subpath (${this.fix(ang1/45)},${this.fix(ang2/45)}) of fullcircle xscaled(${2 * Rx}) yscaled(${2 * Ry}) rotated(${Phi}) scaled(u) shifted(${Cx}*u,${Cy}*u);`);
      }
    }
    return o.join('\n');
  }
  ...
  p_text(x,y,txt,ta,g) {
    ...
    if(ta=='top'||ta=='ulft'||ta=='urt'){
      ...
      ss.forEach((s,j,arr) => {
        ...
        o.push(`draw pic[${j+1}] shifted (${dx}*bbwidth(pic[${j+1}]),${dy}*bbheight(pic[${j+1}])) shifted (${x}*u,${y}*u) withcolor ${fontcolor};`)
        dy -= 1;
      })
  ...
  p_slopedtext(x1,y1,x2,y2,txt,ta,g){
    ...
    */
    ...
    //d.push(`font=${fs}`);
    var s = this.translator.flatten(txt);
    if(fontsize){
      s = `\\switchtobodyfont[${fontsize}pt]${s}`        
    }else{
      s = `\\switchtobodyfont[${this.fontsize}pt]${s}`        
    ...
    if(this.has_texts(g)){
      ...
      if(ta=='bot'){
        //'bot'
        o.push(`draw one shifted (0,-bbheight(one)) zscaled (${this.fix(zsx)},${this.fix(zsy)}) shifted (${x}*u,${y}*u) withcolor ${fontcolor};`);
      }else{
        //'top'
        o.push(`draw one shifted (0,+bbheight(one)) zscaled (${this.fix(zsx)},${this.fix(zsy)}) shifted (${x}*u,${y}*u) withcolor ${fontcolor};`);
      }
  ...
  p_arc(cx,cy,r,start_a,span_a,g){
    ...
    var y0 = Y1;
    if (sweepflag) {
      o.push(`draw ((${this.fix(x0)},${this.fix(y0)})--(subpath (${this.fix(ang2 / 45)},${this.fix(ang1 / 45)}) of fullcircle xscaled(${this.fix(2*Rx)}) yscaled(${this.fix(2*Ry)}) rotated(${Phi}) shifted(${this.fix(Cx)},${this.fix(Cy)}))) scaled(u) ${this.to_draws(g)};`);
    } else {
      o.push(`draw ((${this.fix(x0)},${this.fix(y0)})--(subpath (${this.fix(ang1 / 45)},${this.fix(ang2 / 45)}) of fullcircle xscaled(${this.fix(2*Rx)}) yscaled(${this.fix(2*Ry)}) rotated(${Phi}) shifted(${this.fix(Cx)},${this.fix(Cy)}))) scaled(u) ${this.to_draws(g)};`);
    }
    return o.join('\n')
  }
  is_valid_ta(s){
