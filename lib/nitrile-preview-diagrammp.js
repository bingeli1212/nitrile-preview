...
class NitrilePreviewDiagramMP extends NitrilePreviewDiagram {
  ...
  to_tex_label(txt,ts,fontsize){
    ...
    if (ts==2) {
      // math text
      let style = {};
      var s = this.translator.phrase_to_math(txt,style);
  ...
  to_shadecolor_arr(g) {
    ...
    let opacity = this.to_opacity_str(g);
    if(opacity){
      ss = ss.map(s => {
  ...
  to_shadeonlys(g,isinherit) {
    var d = [];
    var angle = this.to_angle_str(g);//between 0-90
