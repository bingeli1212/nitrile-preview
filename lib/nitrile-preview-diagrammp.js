...
class NitrilePreviewDiagramMP extends NitrilePreviewDiagram {
  ...
  do_finalize(s,style) {
    ...
    ///special situation when it is inside a side
    if(style.side >= 0){
      let n = parseInt(style.n);
      let gap = 0.02;
      var frs = this.translator.string_to_frac(style.fr,n,gap);
      var fr = parseFloat(frs[style.side])||1;
    } else {
      var fr = 1;
    }
    ///aspectratio is specified
    /// the return value could be a NAN
    var aspectratio = this.translator.aspect_ratio_to_float(style.aspectratio);
    ...
    d.push(`numeric pu; pu := u;`)
    d.push(`numeric fr; fr := ${fr};`)
    ...
    if (style && style.width && aspectratio){
      ...
      if (style && style.zoom) {
        d.push(`ratio_w := ratio_w * ${style.zoom};`)
        d.push(`ratio_h := ratio_h * ${style.zoom};`)
      }
  ...
  do_drawarc(opt,txt,g,coords){
    ...
    for (var i = 1; i < coords.length; ++i) {
      ...
      if (this.sweepflag) {
        o.push(`${this.to_leads(g)} draw subpath (${ang2 / 45},${ang1 / 45}) of fullcircle xscaled(${2 * Rx}) yscaled(${2 * Ry}) rotated(${Phi}) scaled(u) shifted(${Cx}*u,${Cy}*u);`);
      } else {
        o.push(`${this.to_leads(g)} draw subpath (${ang1 / 45},${ang2 / 45}) of fullcircle xscaled(${2 * Rx}) yscaled(${2 * Ry}) rotated(${Phi}) scaled(u) shifted(${Cx}*u,${Cy}*u);`);
      }
    }
    return o.join('\n');
  }
  ...
  p_fill(coords,g){
    ...
    for(var item of items) {
      ...
      if(g.shade){
        o.push(`fill (${d}) scaled(u) ${this.to_shadeonlys(g,1)};`);
      }else{
        o.push(`fill (${d}) scaled(u) ${this.to_fills(g,1)};`);
      }
    }
    return o.join('\n');
  }
  p_draw(coords,g){
    ...
    for(var item of items) {
      ...
      if(g.dblarrow){
        o.push(`${this.to_leads(g)} drawdblarrow (${d}) scaled(u) ${this.to_draws(g)};`);
      }else if(g.revarrow){
        o.push(`${this.to_leads(g)} drawarrow reverse(${d}) scaled(u) ${this.to_draws(g)};`);
      }else if(this.get_int_prop(g,"arrow",0)){
        o.push(`${this.to_leads(g)} drawarrow (${d}) scaled(u) ${this.to_draws(g)};`);
      }else{
        o.push(`${this.to_leads(g)} draw (${d}) scaled(u) ${this.to_draws(g)};`);
      }
    }
    return o.join('\n');
  }
  ...
  p_circle(x,y,radius,g){
    ...
    o.push(`${this.to_leads(g)} draw fullcircle scaled(${radius*2}) shifted(${x},${y}) scaled(u) ${this.to_draws(g)};`);
    return o.join('\n');
  }
  ...
  p_tvbar(x,y,g){
    var o = [];
    var delta_y = this.to_barlength_float(g)/2;
  ...
  p_label(x,y,txt,ta,g){
    ...
    var math = this.to_math_int(g);
    var tex_label = this.to_tex_label(txt,math,fontsize);
  ...
  p_pie(x,y,radius,angle1,angle2,g){
    ...
    o.push(`${this.to_leads(g)} draw ((${x1},${y1})..(${xm},${ym})..(${x2},${y2})--(0,0)--cycle) scaled(${radius}) shifted(${x},${y}) scaled(u) ${this.to_draws(g)};`);
    return o.join('\n');
  }
  ...
  p_chord(x,y,radius,angle1,angle2,g){
    ...
    return(`${this.to_leads(g)} draw ((${x1},${y1})--(${x2},${y2})) scaled(${radius}) shifted(${x},${y}) scaled(u) ${this.to_draws(g)};`);
  }
  ...
  _p_arc(x,y,radius,angle1,angle2,g){
    ...
    ym = this.fix(ym);
    radius = this.fix(radius);
    return(`${this.to_leads(g)} draw ((${x1},${y1})..(${xm},${ym})..(${x2},${y2})) scaled(${radius}) shifted(${x},${y}) scaled(u) ${this.to_draws(g)};`);
  }
  ...
  p_cseg(x,y,radius,angle1,angle2,g){
    ...
    o.push(`${this.to_leads(g)} draw ((${x1},${y1})..(${xm},${ym})..(${x2},${y2})--cycle) scaled(${radius}) shifted(${x},${y}) scaled(u) ${this.to_draws(g)};`);
    return o.join('\n');
  }
  to_mp_shadecolor_ss(g) {
    ...
    ss = ss.map(s => this.string_to_mp_color(s,g.user));
  ...
  to_draws(g) {
    ...
    var d = this.to_linesize_float(g);
    if(d){
      o.push(`withpen pencircle scaled ${d}`);
    }
    let linecolor = this.to_linecolor_string(g);
    if (linecolor) {
      o.push(`withcolor ${this.string_to_mp_color(linecolor)}`);
    }
    return o.join(' ');
  }
  ...
  to_shadeonlys(g,isinherit) {
    var d = [];
    var angle = this.to_angle_float(g);//between 0-90
    var shade = this.to_shade_string(g);
    ...
    if (shade=='linear') {
      ...
      d.push(`withshadedomain(0,1.5)`)
      if(ss.length==0){
        d.push(`withshadestep( withshadefraction .25 withshadecolors(\\MPcolor{white},\\MPcolor{gray}) )`)
        d.push(`withshadestep( withshadefraction  1 withshadecolors(\\MPcolor{gray},\\MPcolor{black}) )`)
      }else{
        d.push(`withshadestep( withshadefraction .25 withshadecolors(\\MPcolor{white},${ss[0]}) )`)
        d.push(`withshadestep( withshadefraction  1 withshadecolors(${ss[0]},\\MPcolor{black}) )`)
      }
    }
    return d.join(' ');
  ...
  to_fills(g,isinherit) {
    ...
    let s = '';
    if (fillcolor) {
      s=(`${this.string_to_mp_color(fillcolor)}`);
    }
    if(opacity < 1){
  ...
  has_fills(g){
    var d = this.to_fillcolor_string(g);
    return (d)?true:false;
  }

  to_texts(g) {
    var s = '';
    var fontcolor = this.to_fontcolor_string();
    if (fontcolor) {
      s = `withcolor ${this.string_to_mp_color(fontcolor)}`;
    }
    return s;
  }
  ...
  p_ellipse(x,y,Rx,Ry,angle,g){
    ...
    o.push(`${this.to_leads(g)} draw fullcircle xscaled(${this.fix(2*Rx)}*u) yscaled(${this.fix(2*Ry)}*u) rotated(${Phi}) shifted(${this.fix(Cx)}*u,${this.fix(Cy)}*u) ${this.to_draws(g)};`);
    return o.join('\n'); 
  }
  p_dot_square(x,y,g){
    var x = this.localx(x);    
    ...
    let r = r2 * 0.5;
    return(`fill unitsquare scaled(${r2}) shifted(${-r},${-r}) shifted(${x}*u,${y}*u) ${this.to_dotcolors(g)};`);
  }
  p_dot_circle(x,y,g){
    var x = this.localx(x);    
    var y = this.localy(y);    
    ...
    ///   because there is no path before it
    let r2 = this.to_dotsize_diameter(g);
    return(`fill fullcircle scaled(${r2}) shifted(${x}*u,${y}*u) ${this.to_dotcolors(g)};`);
  }
  ...
  to_dotcolors(g){
    if (g.dotcolor) {
      return(`withcolor ${this.string_to_mp_color(g.dotcolor)}`);
    }
    return '';
  }
  p_arc(x1,y1,x2,y2,Rx,Ry,bigarcflag,g){
    ...
    var y0 = Y1;
    if (sweepflag) {
      o.push(`draw ((${this.fix(x0)},${this.fix(y0)})--(subpath (${ang2 / 45},${ang1 / 45}) of fullcircle xscaled(${this.fix(2*Rx)}) yscaled(${this.fix(2*Ry)}) rotated(${Phi}) shifted(${this.fix(Cx)},${this.fix(Cy)}))) scaled(u) ${this.to_draws(g)};`);
    } else {
      o.push(`draw ((${this.fix(x0)},${this.fix(y0)})--(subpath (${ang1 / 45},${ang2 / 45}) of fullcircle xscaled(${this.fix(2*Rx)}) yscaled(${this.fix(2*Ry)}) rotated(${Phi}) shifted(${this.fix(Cx)},${this.fix(Cy)}))) scaled(u) ${this.to_draws(g)};`);
    }
    return o.join('\n')
  }
  is_valid_ta(s){
  ...
  to_mp_colorbox (g) {
    ...
    var height = g.height;
    var data = g.data;
    o.push(`fill unitsquare xscaled(${width}mm) yscaled(${height}mm) withcolor ${this.string_to_mp_color(data)};`);
    return o.join('\n');
  }
  to_mp_xyplot(g){
    ///
