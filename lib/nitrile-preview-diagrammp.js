...
class NitrilePreviewDiagramMP extends NitrilePreviewDiagram {
  ...
  p_text(x,y,txt,ta,g) {
    ...
    if(ta=='top'||ta=='ulft'||ta=='urt'){
      ...
      ss.forEach((s,j,arr) => {
        ...
        o.push(`draw pic[${j+1}] shifted (${dx}*bbwidth(pic[${j+1}]),${dy}*bbheight(pic[${j+1}])) shifted (${x}*u,${y}*u);`)
        dy -= 1;
      })
  ...
  p_slopedtext(x1,y1,x2,y2,txt,ta,g){
    /*
    picture one; one := outlinetext.f("{\switchtobodyfont[8pt]Hello}") (withcolor black);
    draw one shifted (-bbwidth(one)/2,0) zscaled (-0.7,0.7) shifted (3*u,3*u);
    */
    if(!txt) return '';
    var d = [];
    if(ta=='bot'){
    ...
    fontcolor = this.string_to_mp_color(fontcolor); 
    //let fs = this.g_to_mp_font_attr(g);
    //d.push(`font=${fs}`);
    var tex_label = this.to_tex_text(txt,g);
    var x = (x1+x2)/2;
    var y = (y1+y2)/2;
    var zsx = Math.cos(Math.atan2(y2-y1,x2-x1));
    var zsy = Math.sin(Math.atan2(y2-y1,x2-x1));
    var o = [];
    if(this.has_texts(g)){
      o.push(`picture one; one := outlinetext.f("${tex_label}") (withcolor ${fontcolor});`);
      if(ta=='bot'){
        //'bot'
        o.push(`draw one shifted (-bbwidth(one)/2,-bbheight(one)-1mm) zscaled (${this.fix(zsx)},${this.fix(zsy)}) shifted (${x}*u,${y}*u);`);
      }else{
        //'top'
        o.push(`draw one shifted (-bbwidth(one)/2,1mm) zscaled (${this.fix(zsx)},${this.fix(zsy)}) shifted (${x}*u,${y}*u);`);
      }
    }
    return o.join('\n')
  }
