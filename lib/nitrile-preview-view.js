  ...
  constructor(data) {
    ...
    this.disposables.add(
      ...
      atom.commands.add(this.element, {
        ...
        'nitrile-preview:to-report': () => {
          this.toReport();
        },
        'nitrile-preview:to-beamer': () => {
          this.toBeamer();
        },
        'nitrile-preview:to-context': () => {
          this.toContext();
        },
        'nitrile-preview:to-page': () => {
          this.toPage();
        },
...
///console.log(`image ${fsrc} changed`);
  ...
  doScrollTop (row,column) {
    ...
    if(0){
      for (var block of this.parser.blocks) {
        ...
        if (row == style.row) {
          var node = document.querySelector(`[row='${row}']`);
          if (node) {
    ...
    if(0){
      ...
      for(var i=0; i < nodes.length; ++i){
        var node = nodes[i];
        if(row == node.getAttribute('row')){
          the_i = i;
          break;
        }else if(row > node.getAttribute('row')){
          the_i = i;
          continue;
        }else{
          break;
  ...
  doHilite (needscroll) {
    ...
    if(the_i && nodes.length){
      ...
      if(this.hilite_node && this.hilite_node===node){
        ...
        this.hilite_node.style.outline = '1px solid black';
  ...
  refreshView() {
    ...
    var fontsize = this.get_body_font_size_pt();
    ...
    var opts = [];
    opts.push(`font-size:${fontsize}pt`);
    opts.push(`font-family:roman`);
    opts.push(`color:black`);
    //opts.push(`background-color:white`);
    //opts.push(`width:8.5in`);
    opts.push(`padding-left:0.5in`);
    opts.push(`padding-right:0.5in`);
    opts.push(`padding-top:0.5in`);
    opts.push(`padding-bottom:0.5in`);
    opts.push(`margin:0`);
    opts.push(`box-sizing:border-box`);

    var html = '';
    if(this.parser.conf_to_string('doc')=='slide'){
...
// console.log('jumpToEditor')
  ...
  async saveAs (savefilepath) {
    ...
    try {
      ...
      else if (this.saveastype === 'context') {

        const translator = new NitrilePreviewContex(this.parser);
