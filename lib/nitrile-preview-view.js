  ...
  constructor(data) {
    ...
    this.disposables.add(
      ...
      atom.commands.add(this.element, {
        ...
        'nitrile-preview:to-pdflatex': () => {
          this.toPdflatex();
        },
        'nitrile-preview:to-lualatex': () => {
          this.toLualatex();
        },
        'nitrile-preview:to-xelatex': () => {
          this.toXelatex();
        },
...
///console.log(`image ${fsrc} changed`);
  ...
  doScrollTop (row,column) {
    ...
    for (var block of this.parser.blocks) {
      ...
      if (row >= row1 && row < row2) {
        var node = document.querySelector(`span[rows~='${row1}']`);
        if(!node){
  ...
  refreshView() {
    ...
    var html = translator.to_peek_document();
  ...
  toHeadings (chaps,isarticle) {
    ...
    var tt = pp.map( x => `<li><a href='#${x[0]}'>${x[1]} &#160; ${this.parser.polish(x[2])}</a></li>` );
    var out = tt.join('\n');
    var out = `<ul>${out}</ul>`;
