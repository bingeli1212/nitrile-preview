...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  fence_to_tab(style,ss,ssi) {
    ...
    if(rows.title){
      all.push(`<caption>${this.polish(style,rows.title)}</caption>`);
    }
    ...
    step.forEach((p,i,arr) => {
      if(i==0){
        if(p.value=='+'){
          ...
          all.push(`<dt><b>${this.uncode(style,p.term)}</b></dt>`);
  ...
  float_to_description(title,label,style,subtitles,body,bodyrow){
    let ss = body.map((s)=>{
      ...
      if(style.value==':'){
        return `<dt><strong>${this.polish(style,dt)}</strong></dt><dd>${this.uncode(style,dd)}</dd>`;
      }
      return;
    });
    var text = ss.join('\n');
