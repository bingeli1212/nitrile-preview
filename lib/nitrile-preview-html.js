...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  fence_to_tab(style,ss,ssi) {
    ...
    var ww = this.string_to_textalign_array(style.textalign);
    ...
    if(style.head && rows.length){
      ...
      let ss = pp.map((p,i) => {
        ...
        let css = `padding:${td_padding};text-align:${p.ta}`;
    ...
    var width = ww.map(w => w.pw).reduce( (acc,cur) => acc+cur, 0);
    var itm = {};
    var subtitle = this.to_fig_subtitle(style);
    var width_css = stretch?`${stretch*100}%`:`${width}mm`;
