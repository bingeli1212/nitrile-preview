...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  item_ol_to_text(style,i,item,nblank){
    ...
    if(item.type == 'A'){
      ...
      o.push(`<li class='${li_cls}' type=''  data-row='${item.row}' value='${item.value}' style='--bull:"${value}."' > ${text}`)
    }else if(item.type == 'a'){
      let value = i+1;
      value = this.int_to_letter_a(value);
      o.push(`<li class='${li_cls}' type=''  data-row='${item.row}' value='${item.value}' style='--bull:"${value}."' > ${text}`)
      ...
      o.push(`<li class='${li_cls}'          data-row='${item.row}' value='${item.value}' style='--bull:"${value}."' > ${text}`);
    }else if(item.bullet=='*'){
      let value = i+1;
      o.push(`<li class='${li_cls}'          data-row='${item.row}' value='${value}'      style='--bull:"${value}."' > ${text}`);
    }else{
      let value = i+1;
      value = this.int_to_letter_i(value);
      o.push(`<li class='${li_cls}' type=''  data-row='${item.row}' value='${item.value}' style='--bull:"${value}."' > ${text}`);
  ...
  item_ul_to_text(style,i,item,nblank){
    ...
    o.push(`<li class='${li_cls}' data-row='${item.row}' style='--bull:"&#x2022;"' > ${text}`);
    if(item.more && item.more.length){
      var style = {...style};
      item.more.forEach((p) => {
  ...
  float_to_indent(title,label,style,ss,ssi){
  ...
  float_to_longtabu(title,label,style,ss,ssi){
    ...
    var titlec = this.to_caption_text(title,label,style,"table");
    if(style.stretch){
      var fig = `<table class='LONGTABU' ${frame} ${rules} cellpadding='0' cellspacing='0' style='display:table' width='100%' data-row='${style.row}' >\n<caption align='top' class='CAPTION'>${titlec}</caption>\n${colgroup}\n${text}\n</table>`;
    }else{
      var fig = `<table class='LONGTABU' ${frame} ${rules} cellpadding='0' cellspacing='0' style='display:table'              data-row='${style.row}' >\n<caption align='top' class='CAPTION'>${titlec}</caption>\n${colgroup}\n${text}\n</table>`;
    }
    return fig;
  }
