...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  float_to_stamp(title,label,style,subtitles,body,bodyrow){
    ...
    if(img){
      return `<p id='${label}' class='STAMP' style='${this.css("STAMP")}'> ${img} ${text} </p>`;
    }else if(tab){
      return `<p id='${label}' class='STAMP' style='${this.css("STAMP")}'> ${tab} ${text} </p>`;
    }else{
      return `<p id='${label}' class='STAMP' style='${this.css("STAMP")}'> ${text} </p>`;
    }
  }
