    ...
    itms.forEach((p,i,arr) => {
      ...
      if(p.type=='UL'){
        ...
        if(p.bull.trim().length || this.is_in_list(p.start,check_ss)){
          //filled CB
          d.push(`<label  style='${css}' > <input type='checkbox' disabled='yes' checked='yes'  ></input> ${p.text}</label> `);
        }else{
          //empty CB
          d.push(`<label  style='${css}' > <input type='checkbox' disabled='yes'  ></input> ${p.text}</label> `);
        }
      }
      else if(p.type == 'RB'){
        if(p.bull.trim().length || this.is_in_list(p.start,check_ss)){
          //filled CB
          d.push(`<label  style='${css}' > <input type='radio' disabled='yes' checked='yes'  ></input> ${p.text}</label> `);
        }else{
          //empty CB
          d.push(`<label  style='${css}' > <input type='radio' disabled='yes'  ></input> ${p.text}</label> `);
        }
      }
      else {
