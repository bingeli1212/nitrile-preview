  ...
  rows_to_tabular(rows,style){
    ...
    for (var j = 0; j < k; ++j) {
      ...
      pp.forEach((x, i) => {
        ...
        let pad=`0 ${lr}`;
        let color = '';
        if(cellcolor_map.has(x)){
          color = cellcolor_map.get(x);
        }
        mypp.push(`<td style='width:${frs[i]};padding:${pad};border-top:${border_top};border-bottom:${border_bot};border-left:${border_left};border-right:${border_right};background-color:${color};'>${x}</td>`);        
      });
      var p = mypp.join('');
      var p = `<tr>${p}</tr>`;
      border_top = '';
