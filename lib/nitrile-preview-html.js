...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  phrase_to_framed(fname,id,body,g){
    if(id){
  ...
  phrase_to_diagram(fname,id,body,g){
    if(id){
      var {body} = this.fetch_from_notes(id);
    }
    if(body){
  ...
  itms_to_figure(itms,style) {
    ...
    itms.forEach( (p,i,arr) => {
      let {type,fname,id,body,g,sub} = p;
      sub = this.unmask(sub);
      g.width = `100%`;///reset its width to be the percentage of the current window
      var s = '';
      if(type=='framed'){
        var s = this.phrase_to_framed(fname,id,body,g);
      }else if(type=='diagram'){
        var s = this.phrase_to_diagram(fname,id,body,g);
      }else if(type=='img'){
