...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  hdgs_to_section(title,label,hdgn,level,style){
    ...
    if(hdgn==1){
      return(`<h2 id='${label}' data-row='${style.row}' class='${cls}' style='${css}' > ${leader} <span title='${this.polish(style,style.note)}:${1+style.row}'>${this.uncode(style,title)}</span> </h2>`);
    }else if(hdgn==2){
      return(`<h3 id='${label}' data-row='${style.row}' class='${cls}' style='${css}' > ${leader} <span title='${this.polish(style,style.note)}:${1+style.row}'>${this.uncode(style,title)}</span> </h3>`);
    }else{
      return(`<h4 id='${label}' data-row='${style.row}' class='${cls}' style='${css}' > ${leader} <span title='${this.polish(style,style.note)}:${1+style.row}'>${this.uncode(style,title)}</span> </h4>`);
    }
  }
  //////////////////////////////////////////////////////////////////////////
  //
  ...
  item_ol_to_text(style,i,item,nblank){
    ...
    var css = this.css('LI');
    if(item.bullet=='*'){
      o.push(`<li class='LI' style='${css}' type='1'  data-row='${item.row}' value='${item.value}' > <b>${this.uncode(style,item.key)}</b> &#160; ${text}`);
    }else if(item.type == 'A'){
      let value = i+1;
  ...
  fence_to_ink(style,ss,ssi){
    ...
    var s_style = this.css('SUBTITLE');
    var subtitle = this.to_fig_subtitle(style);
    var svg  = `\n<svg data-bundle="INK" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="currentColor" stroke="none" viewBox="0 0 ${this.fix(itm.vw)} ${this.fix(itm.vh)}" >${svgtext}</svg>\n`;
    var { imgsrc, imgid } = this.to_request_svgid(svg);
    itm.img = `<img style="border:${itm.border};width:${itm.width}mm;height:${itm.height}mm" src="${imgsrc}" id="${imgid}" onload="img_onload(this)"/>`;
    itm.tab = '';
    itm.fig = `\n<table class='COLUMN INK' style='${this.css("COLUMN INK")}' cellpadding='0' cellspacing='0' data-row='${style.row}' >\n<caption align='bottom' class='SUBTITLE' style='${s_style}' > ${subtitle} </caption>\n<tr><td>${itm.img}</td></tr>\n</table>`;
    itm.wlt = `<img align="left" style="border:${itm.border};width:${itm.width}mm;height:${itm.height}mm" src="${imgsrc}" id="${imgid}" onload="img_onload(this)"/>`;
    itm.wrt = `<img align="right" style="border:${itm.border};width:${itm.width}mm;height:${itm.height}mm" src="${imgsrc}" id="${imgid}" onload="img_onload(this)"/>`;
    itm.style = style;
    itm.subtitle = subtitle;
  ...
  fence_to_dia(style,ss,ssi){
    ...
    if(type=='canvas'){
      ...
      itm.img  = `<div class='CANVASFORM' style='position:relative;' data-imgsrc='' data-imgid='${imgid}' data-paletteid='${imgid}-palette' > ${canvaswidget} ${formwidget} ${imgwidget} </div>`
      itm.tab  = '';
      itm.fig  = `\n<table class="COLUMN DIA" style="${this.css("COLUMN DIA")}" cellpadding="0" cellspacing="0" data-row="${style.row}" >\n<caption align="bottom" class="SUBTITLE" style="${s_style}" >${subtitle}</caption>\n<colgroup/><thead/><tbody><tr><td>${itm.img}</td></tr></tbody>\n</table>`;
    }else if(type=='ball'){
      var svg = itm.svg;
  ...
  fence_to_tab(style,ss,ssi) {
    ...
    itm.wlt  = `<table class='WRAPLT TAB' style='${this.css("WRAPLT TAB")};${fontsize_css}' ${frame} ${rules} cellpadding='0' cellspacing='0' data-row='${style.row}' >\n<caption align='bottom' class='SUBTITLE' style='${s_style}'>${subtitle}</caption>                      \n${text}\n</table>`;
    itm.wrt  = `<table class='WRAPRT TAB' style='${this.css("WRAPRT TAB")};${fontsize_css}' ${frame} ${rules} cellpadding='0' cellspacing='0' data-row='${style.row}' >\n<caption align='bottom' class='SUBTITLE' style='${s_style}'>${subtitle}</caption>                      \n${text}\n</table>`;
    itm.style = style;
    itm.subtitle = subtitle;
    return itm;
  }
  fence_to_par(style,ss,ssi){
    ...
    itm.wlt  = `\n<table class='WRAPLT PAR' style='${this.css("WRAPLT PAR")};${fontsize_css}' ${frame} ${rules} cellpadding='0' cellspacing='0' data-row='${style.row}' >\n<caption align='bottom' class='SUBTITLE' style='${s_style}'>${subtitle}</caption>\n<colgroup/><thead/><tbody><tr><td style='vertical-align:bottom;'>${text}</td></tr></tbody>\n</table>`;
    itm.wrt  = `\n<table class='WRAPRT PAR' style='${this.css("WRAPRT PAR")};${fontsize_css}' ${frame} ${rules} cellpadding='0' cellspacing='0' data-row='${style.row}' >\n<caption align='bottom' class='SUBTITLE' style='${s_style}'>${subtitle}</caption>\n<colgroup/><thead/><tbody><tr><td style='vertical-align:bottom;'>${text}</td></tr></tbody>\n</table>`;
    itm.subtitle = subtitle;
    itm.style = style;
    return itm;
  }
  fence_to_vtm(style,ss,ssi){
    ...
    itm.wlt = `\n<table class='WRAPLT VTM' style='${this.css("WRAPLT VTM")};${fontsize_css}' ${frame} ${rules} cellpadding='0' cellspacing='0' data-row='${style.row}' >\n<caption align='bottom' class='SUBTITLE' style='${s_style}'>${subtitle}</caption>\n<colgroup/><thead/><tbody><tr><td>${text}</td></tr></tbody>\n</table>`;
    itm.wrt = `\n<table class='WRAPRT VTM' style='${this.css("WRAPRT VTM")};${fontsize_css}' ${frame} ${rules} cellpadding='0' cellspacing='0' data-row='${style.row}' >\n<caption align='bottom' class='SUBTITLE' style='${s_style}'>${subtitle}</caption>\n<colgroup/><thead/><tbody><tr><td>${text}</td></tr></tbody>\n</table>`;
    itm.style = style;
    itm.subtitle = subtitle;
    return itm;
  }
  ...
  float_to_example(title,label,style,splitid,body,bodyrow){
    ...
    var text = ss.join('<br/>\n');
    let css = this.css('EXAMPLE');
    return `<p class='EXAMPLE' style='${css}' > <span style='position:absolute;left:20pt;'> &#x2023; </span> ${text} </p>`;
  }
  float_to_verbatim(title,label,style,splitid,body,bodyrow){
    ...
    if(hew>1){
      ...
      return `<p class='VERBATIM' style='${css};column-count:${hew};font-size:${fontsize_css};' >${text}</p>`;
    }else{
      var text = ss.join('<br/>\n');
      var css = this.css('VERBATIM');
      return`<p class='VERBATIM' style='${css};font-size:${fontsize_css};' data-row='${style.row}' >${text}</p>`;
    }
  }
  float_to_body(title,label,style,splitid,body,bodyrow){
    ...
    let css = this.css('BODY');
    return `<p class='BODY' style='${css}' > ${text} </p>`;
  }
  float_to_details(title,label,style,splitid,body,bodyrow){
    ...
    step.forEach((p,i,arr) => {
      if(i==0){
        if(p.value=='+'){
          all.push(`<dl class='DETAILS DL' style='${this.css("DETAILS DL")}' data-row='${style.row}'>`);
          all.push(`<dt class='DT' style='${this.css("DT")}'><b>${this.polish(style,p.term)}</b></dt>`);
          all.push(`<dd class='DD' style='${this.css("DD")}'>${this.uncode(style,this.join_para(p.lines))}</dd>`);
        }else{
          all.push(`<ol class='DETAILS OL' style='${this.css("DETAILS OL")}' data-row='${style.row}'>`);
        ...
        if(p.value=='+'){
          all.push(`<dd class='DETAILSROW DD' style='${this.css("DETAILSROW DD")}'>${text}</dd>`);
        }else{
          all.push(`<span class='DETAILSROW' style='${this.css("DETAILSROW")}'>${text}</span>`);
        }
      }
      if(i+1==arr.length){
        if(p.value=='+'){
  ...
  float_to_wrapfig(title,label,style,splitid,body,bodyrow){
    ...
    itms.forEach((p,j,arr) => {
      if(p.type=='bundle'){
        if(p.img){
          if(style.align=='left'){
            var img = this.add_css_style('float:left',p.img);
          }else{
            var img = this.add_css_style('float:right',p.img);
          }
          o.push(img);
        }
      }else if(p.type=='\\\\'){
  ...
  float_to_wraptab(title,label,style,splitid,body,bodyrow){
    ...
    itms.forEach((p,j,arr) => {
      if(p.type=='bundle'){
        if(p.tab){
          if(style.align=='left'){
            var tab = this.add_css_style('float:left',p.tab);
          }else{
            var tab = this.add_css_style('float:right',p.tab);
          }
          o.push(tab);
        }
      }else if(p.type=='\\\\'){
  ...
  float_to_columns(title,label,style,splitid,body,bodyrow){
    ...
    var n = itms.length;
    var all = [];
    all.push(`<figure class='COLUMNS' style='${this.css("COLUMNS")};column-count:${n};' data-row='${style.row}' >`);
    itms.forEach((p,j,arr) => {
      let fig = p.fig;
      all.push(fig);
    });
  ...
  float_to_lines(title,label,style,splitid,body,bodyrow){
    ...
    if(bundles.length){
      ...
      all.push(`<figure class='LINES' style='${this.css("LINES")};text-align:${align};column-count:${hew};' data-row='${style.row}' >`);
  ...
  add_css_style(cs,s){
    ...
    if((v=re.exec(s))!==null){
      ...
      return `<${tag} ${this.tag_attributes_to_string(attributes)}>${content}</${tag}>`;
    }
    return s;
  }
  string_to_tag_attributes(s){
  ...
  tag_attributes_to_string(attributes){
