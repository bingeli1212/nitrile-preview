...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  fence_to_img(style,ss,ssi){
    ...
    if(type=='canvas'){
      ...
      var imgwidget = `<img style="border:${border};aspect-ratio:${width}/${height};" src="${imgsrc}" id="${imgid}" onload="img_onload(this)"> </img>`;
      var imgid = imgid;
      ...
      itm.fig = `<span style='position:relative;border:${border};' data-imgsrc='' data-imgid='${imgid}' data-paletteid='${imgid}-palette' > ${canvaswidget} ${formwidget} ${imgwidget} </span>`
    }else if(type=='ball'){
      var itm = this.diagram.to_diagram(style,ss);
      ...
      var fig = `<svg style="border:${border}" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="currentColor" stroke="currentColor" viewBox="0 0 ${this.fix(vw)} ${this.fix(vh)}">${svgtext}</svg>`;
      itm.img = img;
      itm.fig = fig;
    }
    itm.width = width;
  ...
  fence_to_tab(style,ss,ssi) {
    ...
    itm.tab  = `<table width='${width}' class='TAB' style='display:inline-table;margin:0;page-break-inside:avoid;table-layout:fixed;font-size:${textsmaller};' ${frame} ${rules} cellpadding='0' cellspacing='0' data-row='${style.row}' >${text}</table>`;
    return itm;
  }
  ...
  float_to_flushleft(title,label,style,splitid,body,bodyrow){
    ...
    itms.forEach((p,j,arr) => {
      ...
      if(p.type=='\\\\' || j+1==arr.length){
        d.push(`<div style='display:flex;flex-direction:row;justify-content:flex-start;'>${one.join("&#160;")}</div>`);
        one = [];
      }
    });
    var text = d.join('<br/>\n');
  ...
  float_to_center(title,label,style,splitid,body,bodyrow){
    ...
    itms.forEach((p,j,arr) => {
      ...
      if(p.type=='\\\\' || j+1==arr.length){
        d.push(`<div style='display:flex;flex-direction:row;justify-content:center;'>${one.join("&#160;")}</div>`);
        one = [];
      }
    });
    var text = d.join('<br/>\n');
  ...
  float_to_multicol(title,label,style,splitid,body,bodyrow){
    var bundles = this.body_to_all_bundles(style,body,bodyrow);
