...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  fence_to_displaymath(style,ss,ssi) {
    ...
    p.width = '';
    p.img = `\n<table class='DISPLAYMATH' style='${css1}' ${frame} ${rules} cellpadding='0' cellspacing='0' width='${p.width}' data-row='${style.row}' >\n                                                                                    \n<colgroup/><thead/><tbody><tr><td>${text}</td></tr></tbody>\n</table>`;
    p.fig = `\n<table class='DISPLAYMATH' style='${css2}' ${frame} ${rules} cellpadding='0' cellspacing='0' width='${p.width}' data-row='${style.row}' >\n<caption align='bottom' class='SUBCAPTION' style='${s_style}' > ${p.subc} </caption>\n<colgroup/><thead/><tbody><tr><td>${text}</td></tr></tbody>\n</table>`;
    var o = [];
    o.push(p);
    return o;
  }
  ...
  phrase_to_dia(style,cnt){
    ...
    if(o.length){
      ...
      if(style.width){
        ...
        s_svg = `<svg width='100%' xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="currentColor" stroke="currentColor" viewBox="0 0 ${this.fix(p.vw)} ${this.fix(p.vh)}" >${p.svgtext}</svg>`;
      }
    }
    return s_svg;
  }
