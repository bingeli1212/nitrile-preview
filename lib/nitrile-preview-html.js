...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  float_to_equation(title,label,style,splitid,body,bodyrow){
    ...
    bundles.forEach((bundle,i,arr) => {
      ...
      all.push(`<span class='EQUATIONROW' style='${this.css("EQUATIONROW")}'> <span style='margin-left:auto;margin-right:auto'>${fml}</span> <span style='font-size:smaller;'> (${eqnnum}${sub}) </span> </span>`);
    })
    all.push(`</figure>`);
    return all.join('\n');
  }
