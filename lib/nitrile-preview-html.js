...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  hdgs_to_part(title,label,style){
    var style = this.update_style_from_switches(style,'part')
    return(`<h1>Part. ${this.uncode(title)}</h1>`);
  }
  hdgs_to_chapter(title,label,style){
    return(`<h1>${this.uncode(title)}</h1>`);
  }
  hdgs_to_section(title,label,style){
    return(`<h2>${this.uncode(title)}</h2>`);
  }
  hdgs_to_subsection(title,label,style){
    return(`<h3>${this.uncode(title)}</h3>`);
  }
  hdgs_to_subsubsection(title,label,style){
    return(`<h4>${this.uncode(title)}</h4>`);
  }
  /////////////////////////////////////////////////////////////////////
  //
  /////////////////////////////////////////////////////////////////////
  item_dl_to_text(i,item,style,item_labels){
    ...
    if(item.more && item.more.length){
      item.more.forEach((p) => {
        let { lines, itemize } = p;
        if(lines){
          o.push(`<div class='PARA DIV' style='text-indent:initial'> ${this.untext(lines,style)} </div>`);
        }else if(itemize) {
  ...
  item_hl_to_text(i,item,style){
    var o = [];
    o.push(`<li class='PARA LI'> ${this.uncode(item.text,style)} `);
    if(item.more && item.more.length){
      item.more.forEach((p) => {
        let { lines, itemize } = p;
        if(lines){
  ...
  do_PARA(block){
    ...
    var text = this.untext(body,style);
    var css = '';
    css += this.style_to_css_margin(style);
  ...
  smooth (line) {
    ...
    while(line.length){
      if((v=re_entity.exec(line))!==null){
        ...
        line=v[2];
        try{
          let s = this.tokenizer.get_html_symbol(symbol);
          safe+=s;
        }catch(e){
          safe+=symbol;
        }
  ...
  fence_to_parbox(style,ss) {
    ...
    var texts = ss.map((s) => this.uncode(s,style));
    ...
    css += this.style_to_css_fontfamily(style);
    css += this.style_to_css_fontstyle(style);
  ...
  fence_to_framed(style,ss){
    ...
    var { s, vw, vh, text } = this.diagram.to_framed(style,ss);
    var css = '';
    css += `box-sizing:border-box;`;
    css += `text-indent:0;`;
    css += this.style_to_css_width(style,vw);
    css += this.style_to_css_height(style,'auto');
    css += this.style_to_css_outline(style);
    if(style.wrap){
      css += this.style_to_css_wrap(style);
    }
    let ENCODE = 0;
    if(ENCODE){
  ...
  fence_to_verbatim(style,ss){
    ...
    var ss = ss.map(x => this.polish_verb(x,style));
  ...
  float_to_math(title,label,style,ss){
    ...
    var text = ss.join('\n');
    var {s:text} = this.tokenizer.to_svgmath(text,style,'html');
    text = `<div style='text-align:center'>${text}</div>`
    return `<figure class='MATH' > ${text} </figure>`;
  }
  float_to_equation(title,label,style,ss){
    ...
    var o = [];
    var text = `<figure class='EQUATION'  > $text} </figure>`;
    return text;
  }
  float_to_listing(title,label,style,ss){
    ...
    if(1){
      var ss = ss.map((x, i) => {
        ...
        var line = this.polish(line,style);
        if(style.numbers){
  ...
  float_to_table(title,label,style,ss){
    ...
    var text = `<figure class='TABLE'  > ${this.to_caption_text(title,label,style,"table")} ${text} </figure>`;
    return text;
  }
  ...
  float_to_framed(title,label,style,ss) {
    var style = this.update_style_from_switches(style,'framed')
    var { s, framewidth, frameheight, text } = this.diagram.to_framed(style,ss);
    let css_style = [];
    ...
    var text = `<figure class='FRAMED'> ${text} </figure>`;
    return text;
  }
  float_to_verbatim(title,label,style,ss){
    var style = this.update_style_from_switches(style,'verbatim')
    var text = this.fence_to_verbatim(style,ss);
    var text = `<figure class='VERBATIM'> ${text} </figure>`;
    return text;
  }
  ...
  to_caption_text(title,label,style,key){
    let caption_text = this.uncode(title,style).trim();
  ...
  rows_to_tabular(rows,style){
    ...
    /// add the title
    if(style.title){
      var title = this.uncode(style.title,style);
  ...
  to_subfigure_html(img,sub){
    return `<table border='0' style='table-layout:fixed;display:inline-table;'><caption align="bottom" style="line-height:1;font-size:smaller;">${sub}</caption><tr><td style='width:1px;white-space:nowrap;text-align:center;'>${img}</td></tr></table>`;
  }
  ///
  /// literal_to_double
  ///
