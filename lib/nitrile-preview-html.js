...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  polish_verb(unsafe){
    ...
    unsafe = this.replace_all_blanks_with(unsafe,'&#160;')
    return unsafe;
  }
  polish(unsafe) {
    unsafe = unsafe
  ...
  fence_to_vbox(ss,style){
    var text = this.unmask(this.join_para(ss),style);
  ...
  ss_to_framed(ss,style){
    var { s, vw, vh, text } = this.framed.to_framed(ss,'100%');
    let css_style = [];
    css_style.push(`box-sizing:border-box`);
    css_style.push(`outline:1px solid`);
    css_style.push(`padding:1px`);
    let width = this.string_to_css_width(style.width,style.zoom,'100%');
    if(width){
      css_style.push(`width:${width}`);
    }else{
      css_style.push(`width:${vw}px`)
    ...
    if(ENCODE){
      ...
      css_style.push(`color:black`);
      css_style.push(`background-color:white`);
      s = `<svg style='${css_style.join(';')}' xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' fill='currentColor' stroke='' viewBox='0 0 ${this.fix(vw)} ${this.fix(vh)}' >${text}</svg>`;
    }
    return s;
  }
  ...
  phrase_to_framed(fname,id,g){
    if(id){
      let {body,style} = this.fetch_from_notes(id);
      if(body){
        return this.ss_to_framed(body,g);
      }
    }
    return '';
  }
  phrase_to_diagram(fname,id,g){
    if(id){
      let {body,style} = this.fetch_from_notes(id);
      if(body){
        return this.ss_to_diagram(body,g);
      }
    }
    return '';
  }
  phrase_to_img(fname,id,g) {
    if(fname){
      let src = fname;
      ...
      css_style.push(`box-sizing:border-box`);
      css_style.push(`vertical-align:middle`);
      let width = this.string_to_css_width(g.width,g.zoom);
      if(width){
        css_style.push(`width:${width}`)
      }
      let height = this.string_to_css_height(g.height,g.zoom);
      if(height){
        css_style.push(`height:${height}`)
      }
      if(g.frame==1){
  ...
  fence_to_table(ss, style) {
    ...
    o.push(`<table style='${css_style}' border='0'>`);
    if(caption){
      let caption_text = this.unmask(caption);
      o.push(`<tr><td colspan='${n}'>${caption_text}</td></tr>`)
    }
    o.push(text);
    o.push(`</table>`)
    text = o.join('\n');  
  ...
  para_to_verb(ss,style){
    ss = ss.map(s => this.polish(s));
    var text = ss.join('<br/>');
    return `<pre class='PARA VERBATIM' >${text}</pre>`;
  }
  para_to_equation(ss,style){
    let labels = this.string_to_array(style.label||'');
    ...
    clusters.forEach((ss,i) => {
      ...
      if(label){
        let idnum = this.label_to_idnum(label,'equation');///should be overriden by subclasses
  ...
  para_to_framed(ss,style){
    var text = this.fence_to_framed(ss,style);
    return `<p class='PARA FRAMED' >${text}</p>`;
  }
  ...
  to_caption_text(caption,style,name){
    ...
    let idnum = this.label_to_idnum(label,name);
    if(name=='figure'){
      var title = "Figure";
    }else if(name=='table'){
      var title = 'Table';
    ...
    if(label || caption){
      return `<figcaption>${title} <span class='IDNUM'>${idnum}</span> ${this.unmask(caption)}</figcaption>`;
    }else if(label){
      return `<figcaption>${title} <span class='IDNUM'>${idnum}</span> </figcaption>`;
    }else if(caption){
  ...
  itms_to_bull(itms,style){
    ...
    const box = '&#x2610;'
    const cbox = '&#x2611;' 
    let pp = itms.map((p,i,arr) => {
      let text = this.unmask(p.text);
      let value = this.unmask(p.value);
      let ischeckbox = style.checkbox?1:0;
      let ischecked = ischeckbox && this.string_has_item(style.selection,p.bull);
      if(p.type=='UL'){
        return `<label class='ITEM'>${bullet}${nbsp}${text}</label>`;
      }
      if(p.type=='OL'){
        return `<label class='ITEM'>${i+1}.${nbsp}${text}</label>`  
      }
      if(p.type == '0'){
        return `<label class='ITEM'>${value}${p.ending}${nbsp}${text}</label>`;
      }
      if(p.type == 'a'){
        return `<label class='ITEM'>${this.to_a_letter(value)}${p.ending}${nbsp}${text}</label>`;
      }
      if(p.type == 'A'){
        return `<label class='ITEM'>${this.to_A_letter(value)}${p.ending}${nbsp}${text}</label>`;
      }
      if(p.type == 'DL'){
        return `<label class='ITEM'><b>${value}</b><br/>${text}</label>`;
      }
      if(p.type == 'LL'){
        return `<label class='ITEM'>${text}</label>`
      }
      if(p.type == 'CB'){
        if(p.bull=='_'){
          return `<input type='checkbox' class='ITEM'>${text}</input>`;
        }else{
          return `<input type='checkbox' class='ITEM'>${p.bull}. ${text}</input>`;
        }
      }
      return `<label class='ITEM'>${text}</label>`
    });
    return `<form style='font-size:${this.string_to_fontsize_percent(style.fontsize)}' >${pp.join('<br/>')}</form>`;
  }
  ...
  itms_to_figure(itms,style) {
    ...
    itms.forEach( (p,i,arr) => {
      let {type,fname,id,g,sub} = p;
      sub = this.unmask(sub);
      g.width = `100%`;///reset its width to be the percentage of the current window
      var s = '';
      if(type=='framed'){
        var s = this.phrase_to_framed(fname,id,g);
      }else if(type=='diagram'){
        var s = this.phrase_to_diagram(fname,id,g);
      }else if(type=='img'){
        var s = this.phrase_to_img(fname,id,g);
      }
      s = `<td style='width:${wd}%;'>${s}<span class='SUB'>${sub}</span></td>`;
      p.s = s;
    });
    while(itms.length){
