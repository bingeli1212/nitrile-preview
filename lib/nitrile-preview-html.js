...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  constructor(parser) {
    ...
    this.icon_triangledown = '&#x25BD;'//WHITE DOWN POINTING TRIANGLE
    ...
    step.forEach((p,i,arr) => {
      if(i==0){
        ...
        if(this.re_body_is_fence.test(p.lines[0])){
          ...
          let lines = this.body_to_body(p.lines,this.re_body_is_gthan).map(x=>`${\\small this.icon_triangledown}&#x2002; ${this.uncode(style,x)}`);
  ...
  float_to_example(title,label,style,subtitles,body,bodyrow){
    let ss = body.map((s) => this.uncode(style,s)).map((s) => `<dt>${\\small this.icon_triangledown}&#x2003;${s}</dt>`);
    var text = ss.join('\n');
