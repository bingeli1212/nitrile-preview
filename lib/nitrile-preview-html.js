...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  do_PLST(block) {
    ...
    for (var item of items) {
      ...
      if(more && more.length){
        more.forEach((plitem) => {
          let {lines} = plitem;
          extra_text += `<div class='PARA'> ${this.untext(lines,style)} </div>`;
        });
      }
      switch (bull) {
        case 'OL': {
        ...
        case 'LI': {
          if(type=='hanginglist'){
            ...
            o.push(`<li style='text-indent:-1em;' ><div class='PARA'>${text}</div></li>${extra_text}`);
          }
          else if(type=='description'){
            text = this.uncode(text,style);
            o.push(`<dt><div class='PARA'><b>${text}</b></div></dt> <dd>${extra_text}</dd>`);
          }
          else{
            text = this.uncode(text,style);
            if(bullet=='*'){
            ...
            o.push(`<li type='${item.type}' value='${value}' ><div class='PARA'>${text}</div></li>${extra_text}`);
          }
          break;
        }
        case '/OL': {
  ...
  do_PRIM(block) {
    ...
    o.push(`<div class='PARA PRIM PRIM${hdgn}' >${text}</div>`);
    o.push('');
    block.html = o.join('\n');
  }
  do_PARA(block) {
  ...
  fence_to_tabbing(ss,style){
    ...
    var css = '';
    css += this.to_css_fontsize(style);  
    return `<table style='100%;${css}' class='TABBING' > ${text} </table>`;
  }
  ...
  text_to_typeface(text,type) {
    ...
    switch (type) {
      ...
      case 'frac': {
        ...
        css += `display:inline-block;`
        css += `vertical-align:middle;`
        return `<table style='${css}'><tr><td style='text-align:center;border-bottom:1px solid;'>${s0}</td></tr><tr><td style='text-align:center;border-top:1px solid;'>${s1}</td></tr></table>`;
        break;
      }
      case 'sup': {
        var ss = text.split('/');
        ...
        var s1 = ss[1]||'';
        var css = '';
        return `<span>${s0}</span><sup>${s1}</sup>`;
        break;
      }
      default: {
  ...
  to_circled_number_svg(num){
    return `<svg xmlns = 'http://www.w3.org/2000/svg' width='0.75em' height='0.75em' viewBox='0 0 16 16' fill='currentColor' stroke='currentColor' style='vertical-align:baseline;'> \
    <text transform='translate(8 1.5) scale(0.65)' style='stroke:none;fill:inherit;font-size:12pt;font-style:;' \
    textLength='13' lengthAdjust='spacing' \
    text-anchor='middle' dy='${this.tokenizer.text_dy_pt}pt'>${num}</text> \
    <circle cx='8' cy='8' r='7' stroke='inherit' fill='none'/> </svg>`;
  }
}
