...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  fence_to_verbatim(style,ss,ssi){
    ...
    var cov = `\n<table class='          VERBATIM' ${frame} ${rules} cellpadding='0' cellspacing='0' width='100%' data-row='${style.row}' >\n<caption align='bottom' class='SUBCAPTION'> ${subc} </caption>\n<colgroup/><thead/><tbody><tr><td>${text}</td></tr></tbody>\n</table>`;
    var o = [];
    o.push({img,subc,stretch,fig,cov,style});
    return o;
  }
  ...
  fence_to_tabular(style,ss,ssi) {
    ...
    var cov = `<table class='          TABULAR' ${flt} ${frame} ${rules} cellpadding='0' cellspacing='0' data-row='${style.row}' >\n<caption align='bottom' class='SUBCAPTION' style='${s_style}' >       </caption>\n${colgroup}\n${text}\n</table>`;
    var o = [];
    o.push({img,subc,w,h,stretch:0,fig,cov,style});
    return o;
  }
  ...
  cove_to_text(style,cove){
    ...
    if(n > 2 && p0.text.startsWith('```') && pn.text.startsWith('```')){
      ...
      if(o.length){
        let cov = o[0].fig;
        cov = `<span class='COVE' style='--bull:"&#x25B8;";--pad:-1.0em;display:inline-flex;'> ${cov} </span>`;
        all.push(cov);
      }
      var text = all.join('\n');
    }else{
      ...
      var text = all.join('\n');
      text = `<span class='COVE' style='--bull:"&#x25B8;";--pad:-1.0em;' > ${text} </span>`;
    }
    text = `<p class='PARAGRAPH INDENTING' data-row='${style.row}' > ${text} </p>`;
    return text;
  }
  ////////////////////////////////////////////////////////////////////////////////
  //
