...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  do_PLST(block) {
    ...
    for (var item of items) {
      var {bull,bullet,value,text,keys,row1,row2,more} = item;
      text = text || '';
      let extra_text = '';
      if(more && more.length){
        more.forEach((plitem) => {
  ...
  do_NOTE(block) {
    if(block.title=='note'){
      ///the first line of body is the name of the note
      let name = block.body[0]||'';
      name = name.trim();
      let body = block.body.slice(1);
      block.notes.set(name,body);
    }
  }
  smooth (unsafe) {
  ...
  fence_to_diagram(ss,style){
    ...
    if(style.width){
      let width = this.string_to_css_length(style.width);
  ...
  para_to_longtable(ss, style) {
    ...
    return `<div class='TEXT ${style.nspace?'NSPACE':''}' >${text}</div>`;
  }
  ...
  para_to_blockquote(ss,style) {
    ...
    return `<div class='TEXT BLOCKQUOTE ${style.nspace?'NSPACE':''}' >${text}</div>`;
  }
  ...
  make_line_svg(math,style){
    ...
    var d = this.tokenizer.to_svgmath_array(math,style);
    d = d.filter(x => (x.w > 0));
