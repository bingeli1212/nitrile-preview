...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
      ...
      switch (bull) {
        ...
        case 'LI': {
          if(bullet[0]=='+'){
            ...
            text = `<p>${text}</p>`
            o.push(`<dt class='PLST' ><b>${text}</b></dt><dd class='PLST'>${extra_text}</dd>`);
          }
          else{
            text = this.unmask(text);
            text = `<p>${text}</p>`
            ...
            o.push(`<li class='PLST' type='${item.type}' value='${value}' >${text}${extra_text}</li>`);
          }
          break;
        }
        case '/OL': {
  ...
  fence_to_vbox(ss,style){
    ...
    if(style.width){
      var width = this.string_to_css_width(style.width);
    }else{
      var width = '100%'
    }
    var o = [];
    o.push(`<span style='display:inline-block;width:${width};text-align:justify;hyphens:auto;'>`)
    o.push(text);
    o.push(`</span>`);
    text = o.join('\n');
    return text;
  ...
  fence_to_diagram(ss,style){
    ...
    if(style.height){
      let height = this.string_to_css_height(style.height,zoom);
      if(height){
  ...
  phrase_to_img(cnt) {
    ...
    if(g._.substr(0,1)=='#'){
      ...
      css_style.push(`box-sizing:border-box`);
      if(1){
        let width = this.string_to_css_width(g.width,zoom);
        css_style.push(`width:${width}`)
      }
      if(g.height){
  ...
  para_to_multi(ss,style){
    ...
    rows.forEach((pp,j) => {
      ...
      if(style.head && j==0){
        pp = pp.map((x,i) => `<th style='width:${frs[i]};text-align:${haligns[i]};padding:0 0.5em'>${x.text}</th>`);
        d.push('<tr>')
        d.push(pp.join(gap));
        d.push('</tr>')
      }else {
        pp = pp.map((x,i) => `<td style='width:${frs[i]};text-align:${haligns[i]};padding:0 0.5em'>${x.text}</td>`);
        d.push('<tr>')
        d.push(pp.join(gap));
        d.push('</tr>')
      }
      d.push(`<tr><td class='RIFT' colspan='${n}'></tr>`);
      has_rift = 1;
    });
    if(has_rift){
      d.pop();
    ...
    d.push('</table>')
    var text = d.join('\n')
    return `<p class='TEXT MULTI' >${text}</p>`;
  }
  para_to_tabbing(ss,style){
    var itms = this.ss_to_tabbing_itms(ss,style);
    var n = itms.reduce((acc,cur) => Math.max(acc,cur.length),0);
    var d = [];
    var text = d.join('\n')
    var k = Math.ceil(ss.length/n);
    d.push('');
    d.push(`<table style='width:100%' border='0'>`);
    let gap = 100/n;
    for(let j=0; j < itms.length; ++j){
      var pp = itms[j];
      var pp = pp.map((x) => this.unmask(x));
      var pp = pp.map((x) => `<td style='width:${gap}%;text-align:left;'>${x}</td>`)
      let x = pp.join('');
      d.push(`<tr>${x}</tr>`);
    }
    /*
    129 \begin{tabbing}
    130 \hspace{0.5\linewidth}\=\kill
    131 Hello\>World\\
    132 \end{tabbing}
    */
    d.push('</table>')
    text = d.join('\n')
    return `<p class='TEXT TABBING' >${text}</p>`;
  }
  para_to_quote(ss,style) {
    let text = ss.join('\n').trim();
    text = this.unmask(text);
    return `<p class='TEXT QUOTE' >${text}</p>`;
  }
  para_to_plaintext(ss, style) {
    let text = ss.join('\n');
    text = this.unmask(text, style);
    return `<p class='TEXT PLAINTEXT' >${text}</p>`;
  }
  phrase_to_br() {
    let text = '<br/>';
    return text;
  ...
  itms_to_bull(itms,style){
    let bullet = '&#x2022;'
    const nbsp = '&#160;&#160;'
    if(this.flags.p_bullet){
      bullet = this.flags.p_bullet;
    }
    if(itms.length){
      let pp = itms.map((p,i) => {
        ...
        if(p.type=='UL'){
          return `<div class='ITEM'>${bullet}${nbsp}${text}</div>`;
        }
        if(p.type=='OL'){
          return `<div class='ITEM'>${i+1}.${nbsp}${text}</div>`  
        }
        if(p.type == '0'){
          return `<div class='ITEM'>${value}${p.ending}${nbsp}${text}</div>`;
        }
        if(p.type == 'a'){
          return `<div class='ITEM'>${this.to_a_letter(value)}${p.ending}${nbsp}${text}</div>`;
        }
        if(p.type == 'A'){
          return `<div class='ITEM'>${this.to_A_letter(value)}${p.ending}${nbsp}${text}</div>`;
        }
        if(p.type == 'DL'){
          return `<div class='ITEM'><b>${value}</b><br/>${text}</div>`;
        }
        return `<div class='ITEM0'>${text}</div>`
      });
      return `<div>${pp.join('\n')}</div>`;
    }
    return `<div></div>`;  
  ...
  itms_to_figure(itms,style) {
    ...
    itms.forEach( p => {
      ...
      let s = this.string_from_style(g);
      s = this.phrase_to_img(s);
