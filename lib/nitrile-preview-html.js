...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  float_to_wrapfig(title,label,style,ss){
    ...
    itms.forEach((p,j,arr) => {
      if(p.type=='bundle'){
        let img = p.img;
        if(style.location=='left'){
          img = `<figure class='WRAPFIG LEFT'> ${p.subfigure} </figure>`;
        }else{
          img = `<figure class='WRAPFIG RIGHT'> ${p.subfigure} </figure>`;
        }
        all.push(img);
      }
    });
