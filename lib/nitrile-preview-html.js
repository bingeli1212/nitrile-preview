    ...
    if(style.subfigure){
      ...
      itms.forEach( (p,i,arr) => {
        ...
        if(type=='framed'){
          ...
          p.img = this.to_subfigure(this.fence_to_framed(ss,g),p.sub,g.width,style);
        }else if(type=='diagram'){
          p.sub = this.uncode(p.sub,style);
          seqno++;
          p.sub = `(${this.to_a_letter(seqno)}) ${p.sub}`;
          p.img = this.to_subfigure(this.fence_to_diagram(ss,g),p.sub,g.width,style);
        }else if(type=='img'){
          p.sub = this.uncode(p.sub,style);
          seqno++;
          p.sub = `(${this.to_a_letter(seqno)}) ${p.sub}`;
          p.img = this.to_subfigure(this.fence_to_img(ss,g),p.sub,g.width,style);
        }else{
          p.img = '<br/>'
        }
      });
