...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  fence_to_ink(style,ss){
    ...
    var inkw = inkwidth*this.MM_TO_PX;
    var inkh = inkheight*this.MM_TO_PX;
    let ENCODE = 1;
    if(ENCODE){
      let s_svg = `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="currentColor" stroke="currentColor" viewBox="0 0 ${this.fix(inkw)} ${this.fix(inkh)}" >${img}</svg>`;
      img = `<object style='${css}' width='${w}' height='${h}' data='data:image/svg+xml;charset=UTF-8,${encodeURIComponent(s_svg)}'></object>`;
    }else{
      img = `<svg style='${css}' width='${this.fix(w)}' height='${this.fix(h)}' xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' fill='currentColor' stroke='none' viewBox='0 0 ${this.fix(inkw)} ${this.fix(inkh)}' >${img}</svg>`;
    }
    var o = [];
    var subc = '';
  ...
  fence_to_dia(style,ss){
    ...
    o = o.map((p) => {
      ...
      if(p.images.length){
        ...
        let s_svg = `<svg style="${css}" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="currentColor" stroke="currentColor" viewBox="0 0 ${this.fix(p.vw)} ${this.fix(p.vh)}" >${p.svgtext}</svg>`;
        //var img = `<object style='${css}' width='${p.vw}' height='${p.vh}' data='data:image/svg+xml;charset=UTF-8,${encodeURIComponent(s_svg)}' id='${imgid}' ></object>`;
        var img = s_svg;
      }else{
        let s_svg = `<svg style="${css}" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="currentColor" stroke="currentColor" viewBox="0 0 ${this.fix(p.vw)} ${this.fix(p.vh)}" >${p.svgtext}</svg>`;
  ...
  fence_to_img(style,ss) {
    return this.fence_to_dia(style,ss);
  }
