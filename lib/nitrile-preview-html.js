...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  fence_to_displaymath(style,ss) {
    ...
    var text = ss.join('\n');
    var {w,h,mid,s,defs} = this.tokenizer.to_svgmath(text,style);
    s = this.tokenizer.to_outer_svg(s,w,h,mid,defs,'html');
  ...
  fence_to_lines(style,ss) {
    ...
    itms.forEach((p,i,arr) => {
      ...
      if(p.type=='UL'){
        d.push(`<div  class='PACK'   >${bullet}${nbsp}${p.text}</div>`);
      }
      else if(p.type=='OL'){
        d.push(`<div  class='PACK'   >${i+1}.${nbsp}${p.text}</div>`);  
      }
      else if(p.type == '1'){
        if(!p.ending) p.ending = '.';
        d.push(`<div  class='PACK'   >${p.value}${p.ending}${nbsp}${p.text}</div>`);
      }
      else if(p.type == 'a'){
        if(!p.ending) p.ending = '.';
        d.push(`<div  class='PACK'   >${this.to_a_letter(p.value)}${p.ending}${nbsp}${p.text}</div>`);
      }
      else if(p.type == 'A'){
        if(!p.ending) p.ending = '.';
        d.push(`<div  class='PACK'   >${this.to_A_letter(p.value)}${p.ending}${nbsp}${p.text}</div>`);
        ...
        if(p.bull.trim().length || this.is_in_list(p.start,check_ss)){
          //filled CB
          d.push(`<div  class='PACK'   > ${squarebox} &#160; ${p.text}</div> `);
        }else{
          //empty CB
          d.push(`<div  class='PACK'   > ${squareboxo} &#160; ${p.text}</div> `);
        }
      }
      else if(p.type == 'RB'){
        if(p.bull.trim().length || this.is_in_list(p.start,check_ss)){
          //filled CB
          d.push(`<div  class='PACK'   > ${circlebox} &#160; ${p.text}</div> `);
        }else{
          //empty CB
          d.push(`<div  class='PACK'   > ${circleboxo} &#160; ${p.text}</div> `);
        }
      }
    ...
    if(style.wrap){
      ...
      itms.forEach((p,j,arr) => {
        ...
        if(p.type=='fence'){
          ...
          all.pop();
          let img = this.to_subfigure_img(p.img,p.sub);
          img = this.update_css_class(img,'SUBFIGURE');
          all.push(img);
          all.push(end);
        }else{
          all.push(begin);
  ...
  rows_to_tabular(rows,style){
    ...
    rows.forEach((pp,j) => {
      ...
      pp.forEach((x,i) => {
        ...
        var ht = height;
        var css = '';
        css += `text-align:${haligns[i].ta};`;
        if(haligns[i].pw){
          css += `width:${haligns[i].pw};`
        }else{
          text = `<nobr>${text}</nobr>`;
        }
        css += `height:${ht};`
  ...
  to_subfigure_img(img,sub){
    ...
    var s_height = this.extract_css_style(img,'height');
    //img = this.update_css_style(img,'width','100%');
    //img = this.update_css_style(img,'height','');
    var all = [];
    all.push(`<table style='width:${s_width};height:${s_height};' >`);
    if(sub){
      all.push(`<caption align="bottom" style="line-height:1;font-size:smaller;">${sub}</caption>`);
    }
    all.push(`<tr><td style='width:${s_width};padding:0;text-align:center;'>${img}</td></tr></table>`);
    img = all.join('\n');
    return img;
  }
  //////////////////////////////////////////////////////////////////////////////////////
