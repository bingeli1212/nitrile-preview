...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
      ...
      switch (bull) {
        ...
        case 'LI': {
          if(type=='hanginglist'){
            ...
            o.push(`<li style='${css}'>${text}</li>${extra_text}`);
          }
          else if(type=='description'){
            text = this.uncode(text,style);
            item_labels.push(text);
            text = item_labels.join('<br/>')
            var css = '';
            var css_dd = '';
            if(count>1){
              css += `margin:${this.margin_li_inner} 0;`;
  ...
  item_dl_to_text(i,item,g){
    var o = [];
    o.push(`<dt class='DL'> ${this.uncode(item.text,g)} </dt> <dd class='DL'>`);
    if(item.more && item.more.length){
      item.more.forEach((p) => {
        let { lines, itemize } = p;
        if(lines){
          o.push(`<div class='DL' style='text-indent:initial'> ${this.untext(lines,g)} </div>`);
        }else if(itemize) {
  ...
  item_hl_to_text(i,item,g){
    var o = [];
    o.push(`<li class='HL'> ${this.uncode(item.text,g)} `);
    if(item.more && item.more.length){
      item.more.forEach((p) => {
        let { lines, itemize } = p;
        if(lines){
          o.push(`<div class='HL' style='text-indent:initial'> ${this.untext(lines,g)} </div>`);
        }else if(itemize) {
          o.push(`${this.itemize_to_text(itemize,g)}`);
        }
      });
  ...
  item_ol_to_text(i,item,g){
    ...
    if(item.type == 'A'){
      o.push(`<li class='OL' type='A' value='${item.value}'> ${text}`)
    }else if(item.type == 'a'){
      o.push(`<li class='OL' type='A' value='${item.value}'> ${text}`)
    }else if(item.type == 'I'){
      o.push(`<li class='OL' type='I' value='${item.value}'> ${text}`)
    }else if(item.type == 'i'){
      o.push(`<li class='OL' type='i' value='${item.value}'> ${text}`)
    }else if(item.value) {
      o.push(`<li class='OL' value='${item.value}'> ${text}`);
    }else if(item.bullet=='*'){
      let value = i+1;
      o.push(`<li class='OL' value='${value}'> ${text}`);
    }else{
      o.push(`<li class='OL' type='I' value='${item.value}'> ${text}`);
    }
    if(item.more && item.more.length){
      item.more.forEach((p) => {
        let { lines, itemize } = p;
        if(lines){
          o.push(`<div class='OL' style='text-indent:initial'> ${this.untext(lines,g)} </div>`);
        }else if(itemize) {
          o.push(`${this.itemize_to_text(itemize,g)}`);
        }
      });
  ...
  item_ul_to_text(i,item,g){
    var o = [];
    var text = this.uncode(item.text,g);
    o.push(`<li class='UL'> ${text}`);
    if(item.more && item.more.length){
      item.more.forEach((p) => {
        let { lines, itemize } = p;
        if(lines){
          o.push(`<div class='UL' style='text-indent:initial'> ${this.untext(lines,g)} </div>`);
        }else if(itemize) {
          o.push(`${this.itemize_to_text(itemize,g)}`);
        }
      });
  ...
  itemize_to_text(itemize,g){
    ...
    switch (bull) {
      ...
      case 'DL': {
        o.push(`<dl class='DL'>`);
        items.forEach((item,j) => {
          let text = this.item_dl_to_text(j,item,g);
          o.push(text);
        });
        o.push(`</dl>`)
        break;
  ...
  fence_to_combination(ss,g){
    var g = this.update_style_from_switches(g,'combination')
    ...
    itms.forEach((p,j,arr) => {
      ...
      if(id=='diagram' || id=='framed' || id=='img'){
        p.sub = `(${this.to_a_letter(++seqno)}) ${this.uncode(p.sub)}`;
        p.img = this.s_to_phrase(id,cnt,g);
        pp.push({img:p.img,sub:p.sub});
      }else{
        pp = [];
        all.push(pp);
      }
  ...
  float_to_equation(title,ss,g){
    ...
    let o = [];
    let figcss = this.to_figure_css(g);
  ...
  float_to_listing(caption,ss,g){
    var style = this.update_style_from_switches(g,'listing')
    var text = this.untext(ss,g);
  ...
  float_to_figure(title,ss,g){
    ...
    var text = this.untext(ss,g);
    var figcss = this.to_figure_css(g);
    var text = `<figure style='${figcss}'  > ${this.to_caption_text(title,g,"figure")} ${text} </figure>`;
    return text;
  }
  ...
  to_subfigure_html(img,sub,g){
    return `<table border='0' style='display:inline-table'><caption align="bottom">${sub}</caption><tr><td>${img}</td></tr></table>`;
  }
  to_figure_css(style){
    var css = '';
