...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
    ...
    for (var item of items) {
      ...
      if(more && more.length){
        more.forEach((plitem) => {
          ...
          extra_text += `<p class='PARA'> ${this.untext(lines)} </p>`;
        });
      }
  ...
  fence_to_list(ss,style) {
    ...
    return `<div class='PARA BULL' >${text}</div>`;
  }
  ///
  /// paras
  ///
  ...
  itms_to_subfigures(itms,style) {
    ...
    itms.forEach( (p,i,arr) => {
      ...
      if(type=='framed'){
        ...
        p.img = this.fence_to_framed(ss,g,p.sub);
      }else if(type=='diagram'){
        p.sub = this.unmask(p.sub);
        subid++;
        p.sub = `(${this.to_a_letter(subid)}) ${p.sub}`;
        p.img = this.fence_to_diagram(ss,g,p.sub);
      }else if(type=='img'){
        p.sub = this.unmask(p.sub);
        subid++;
        p.sub = `(${this.to_a_letter(subid)}) ${p.sub}`;
        p.img = this.fence_to_img(ss,g,p.sub);
      }else{
        p.img = '<br/>'
      }
    });
