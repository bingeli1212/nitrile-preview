...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  fence_to_tab(style,ss,ssi) {
    ...
    var rows = this.ss_to_tab_rows(ss,ssi,ww.length);
    rows.forEach((pp,j) => {
      pp.forEach((p,i) => {
    ...
    if(1){//colgroup has to come before everything else after caption
      ...
      let empty = '';
      if(style.side){
        let s1 = ww.map((x,i,arr) => `<col width='${(x.pw)?(x.pw*this.MM_TO_PX):(empty)}'/>`).slice(0,1).join('\n');
        let s2 = ww.map((x,i,arr) => `<col width='${(x.pw)?(x.pw*this.MM_TO_PX):(empty)}'/>`).slice(1).join('\n');
        s = `<colgroup>\n${s1}\n</colgroup>\n<colgroup>\n${s2}\n</colgroup>`;      
      }else{
        let s2 = ww.map((x,i,arr) => `<col width='${(x.pw)?(x.pw*this.MM_TO_PX):(empty)}'/>`).join('\n');
        s = `<colgroup>\n${s2}\n</colgroup>`;
      }
      if(hew>1){
        let s0 = s;
  ...
  float_to_flushleft(title,label,style,splitid,body,bodyrow){
    ...
    var text = all.join('&#160;\n');
    return `<div class='FLUSHLEFT' style='${this.css("FLUSHLEFT")};display:flex;flex-direction:row;justify-content:flex-start;align-items:flex-start;' data-row='${style.row}' >${text}</div>`;
  }
  float_to_center(title,label,style,splitid,body,bodyrow){
    ...
    var text = all.join('&#160;\n');
    return `<figure class='CENTER' style='${this.css("CENTER")};display:flex;flex-direction:row;justify-content:center;align-items:center;' data-row='${style.row}' >${text}</figure>`;
  }
  ...
  float_to_columns(title,label,style,splitid,body,bodyrow){
    ...
    var n = itms.length;
    var all = [];
    all.push(`<figure class='COLUMNS' style='${this.css("COLUMNS")};display:grid;grid-template-columns:repeat(${n},${100/n}%);grid-auto-rows:auto;' data-row='${style.row}' >`);
    itms.forEach((p,j,arr) => {
      if(p.fml){
        all.push(p.fml);
      }else if(p.img){
  ...
  float_to_tabbing(title,label,style,splitid,body,bodyrow){
    ...
    if(bundles.length) {
      ...
      ss =  ss.map((s,j,arr) => {
        let xx = s.split(/\s{3,}/);
        maxn = xx.length;
        let isdhline = this.re_is_dhline.test(s);
        if(j==0){
          if(isdhline){
            ww = xx.map(x=>x.length);
            return '';
          }else{
            ww = xx.map(x=>10);
          }
        }
        xx = xx.map(x => this.uncode(bundle.g,x));
        xx = xx.map(x => this.to_fontstyled_text(x,bundle.g.fontstyle));
        xx = xx.map(x => this.to_fontsized_text(x,bundle.g.fontsize));
        xx = xx.map((x,i) => `<td style='width:${ww[i]?''+ww[i]+'ex':''};padding:0 1ex;'>${x}</td>`);
        s = xx.join('');
        s = `<tr>${s}</tr>`;
        return s;
      });
      all.push(`<div class='TABBING' style='${this.css("TABBING")};display:flex;flex-direction:column;align-items:flex-start;' data-row='${style.row}' >`);
      all.push(`<table>`);
      ss.forEach(s => {
        if(s){
          all.push(s);
