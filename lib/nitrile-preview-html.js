...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  do_HRLE(block) {
    var {id,row1,row2,sig,style,text} = block;
    var o = []; 
    text = this.uncode(text,style);
  ...
  fence_to_framed(ss,style){
    ...
    css += this.to_css_height(style,vh);
  ...
  itms_to_bull(itms,style){
    var bullet = '&#x2022;'
    const nbsp = '&#160;&#160;'
    const squareboxo = this.uncode('&SquareBoxO;',style);
    const squarebox = this.uncode('&SquareBox;',style);
    const circleboxo = this.uncode('&CircleBoxO;',style);
    const circlebox = this.uncode('&CircleBox;',style); 
    if(style.bullet){
      try{
  ...
  itms_to_lines(itms,style){
    ...
    for (let j = 0, i = 0; j < itms.length; i += 1, j += k) {
      var pp = itms.slice(j, j + k).map(p => this.uncode(p.text,style))
      cols.push(pp);
    }
