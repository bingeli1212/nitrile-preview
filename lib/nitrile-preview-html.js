...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  float_to_equation(title,label,style,bundles){
    ...
    itms.forEach((p,j,arr) => {
      o.push(`<span class='EQUATIONROW' style='${this.css("EQUATIONROW")}'> ${p.text} <span style='margin-left:auto;font-size:smaller;'> (${p.eqnnum}) </span> </span>`);
    })
    o.push(`</figure>`);
    return o.join('\n');
  }
