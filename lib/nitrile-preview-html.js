...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
    ...
    if(style.wrap){
      ...
      itms.forEach((p,j,arr) => {
        if(p.type=='bundle'){
          ...
          let img = this.to_subfigure_img(p.img,p.sub,p.w);
          row.push(img);
        }else{
          row = [];
          rows.push(row);
  ...
  to_caption_text(title,label,style,key){
    ...
    if(none && !caption_text){
      ...
      let caption_css = this.css('CAPTION')
  ...
  to_subfigure_img(img,sub,w){
    let subfigurecaption_css = this.css('SUBCAPTION');
    let o = [];
    if(w){
      o.push(`<table style='display:inline-table;width:${w};'>`);
    }else{
      o.push(`<table style='display:inline-table;width:100%;'>`);
    }
    if(sub){
      o.push(`<caption align='bottom' style='${subfigurecaption_css}'>${sub}</caption>`);
    }
