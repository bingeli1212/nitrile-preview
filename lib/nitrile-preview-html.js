...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  float_to_tabbing(title,label,style,ss,ssi){
    style = {...style,float:'tabbing'};
    var cols = this.ss_to_clusters(ss);
  ...
  float_to_columns(title,label,style,ss,ssi){
    var n = this.assert_int(style.n,2,2,2);
    ...
    rows.forEach((row) => {
      all.push(`<div class='COLUMNS' style='column-count:2' >`);
      for(let i=0; i < n; ++i){
        all.push(`<div style='break-before:column;'>`);
        if(i < row.length){
          let p = row[i];
          all.push(p.img);         
        }else{
