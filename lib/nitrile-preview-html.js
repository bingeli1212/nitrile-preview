...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  fence_to_dia(style,ss){
    ...
    o = o.map((p) => {
      ...
      if(p.imgsrc){
        ...
        ///THE BACKGROUND SVG
        css += `; background-image:url("${p.imgsrc}")`;
        ...
        let s_svg = `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="currentColor" stroke="currentColor" viewBox="0 0 ${this.fix(p.vw)} ${this.fix(p.vh)}" >${p.svgtext}</svg>`;
        var img = `<object style='${css}' width='${p.vw}' height='${p.vh}' data='data:image/svg+xml;charset=UTF-8,${encodeURIComponent(s_svg)}' id='${p.imgid}'></object>`;
      }
      p.img = img;
  ...
  fence_to_img(style,ss) {
    ...
    if(background=='grid'){
      var gridcolor = this.diagram.g_to_gridcolor_string(style);      
      var gridcolor = this.diagram.string_to_svg_color(gridcolor,0);
    ...
    ///NOTE that for a <img> element the width= and height= attribute should be set because iBOOK on iPAD1 seems to not be able to set the image size correctly w/o it
    var w = mywidth*this.MM_TO_PX;
    var h = myheight*this.MM_TO_PX;
    var img = `<img style='${css}' width='${w}' height='${h}' src='${imgsrc}' id='${imgid}' alt='${imgsrc}'></img>`;
  ...
  fence_to_verbatim(style,ss){
    ...
    var img = `<table ${frame} ${width} style='${css}'> <tr><td>${img}</td></tr> </table>`;
    var o = [];
    var subc = '';
    o.push({img,subc,w});
    return o;
