...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  float_to_example(title,label,style,body,bodyrow){
    ...
    var text = ss.join('<br/>\n');
    let css = this.css('PARAGRAPH EXAMPLE');
    return `<p class='PARAGRAPH EXAMPLE' style='${css}' > <span style='position:absolute;left:20pt;'> &#x2023; </span> ${text} </p>`;
  }
  float_to_sample(title,label,style,body,bodyrow){
    var ss = body.map((s,i,arr) => {
      ...
      return `<span data-row='${p.n}'><code class='CODE'>${x}</code></span>`;
    })
    let hew = this.g_to_hew_int(style);
    let n = ss.length;
    let fontsize_css = '';
    if(hew>1){
      ...
      return `<p class='PARAGRAPH SAMPLE' style='${css};column-count:${hew};${fontsize_css};' >${text}</p>`;
    }else{
      var text = ss.join('<br/>\n');
      var css = this.css('PARAGRAPH SAMPLE');
      return`<p class='PARAGRAPH SAMPLE' style='${css};${fontsize_css};' data-row='${style.row}' >${text}</p>`;
    }
  }
