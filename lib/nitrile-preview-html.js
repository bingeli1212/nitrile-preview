...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  rows_to_tabular(rows,style){
    ...
    let vcols = this.string_to_int_array(style.vline||'');
    let border_top = '';
    let border_bot = '';
    let table_top = '';
    let table_bot = '';
    ...
    if(this.pp_is_hline(rows[rows.length-1])){
      table_bot = 1;
    ...
    for (var j = 0; j < k; ++j) {
      ...
      if (this.pp_is_hline(pp)) {
        ...
        d.push(`<tr><td style='border-top:1px solid;border-bottom:1px solid;' colspan='${n}'></td></tr>`);
        continue;
      }
      if(j==0 && table_top == 1) border_top = '1px solid';
      if(j==k-1 && table_bot == 1) border_bot = '1px solid';
      pp = pp.map((x, i) => {
        const lr='1ex';
        const sp='2ex';    
        let border_left = (vcols.indexOf(i) >= 0)?'1px solid':'';
        let border_right= (vcols.indexOf(i+1) >= 0)?'1px solid':'';
        let pad=`0 ${lr}`;
        return (`<td style='width:${frs[i]};padding:${pad};border-top:${border_top};border-bottom:${border_bot};border-left:${border_left};border-right:${border_right};'>${x}</td>`);
      });
      var p = pp.join('');
      var p = `<tr>${p}</tr>`;
      border_top = '';
  ...
  rows_to_tabulate(rows,style,islongtable){
    ...
    if(border==1){
      ...
      let border_left = '';
      let border_right = '';
      let vcols = this.string_to_int_array(vline);
      for (var j = 0; j < nrows; ++j) {
        var pp = rows[j];
        if (this.pp_is_hline(pp)) {
          border_top = '1px solid';
        ...
        pp = pp.map((x, i) => {
          border_left = (vcols.indexOf(i) >= 0)?'1px solid':'';
          border_right= (vcols.indexOf(i+1) >= 0)?'1px solid':'';
          if (i==0)                var pad=`0 ${lr} 0 0`;
          else if (i+1==pp.length) var pad=`0 0 0 ${lr}`;
          else                     var pad=`0 ${lr}`;
          return (`<td style='width:${frs[i]};padding:${pad};border-top:${border_top};border-left:${border_left};border-right:${border_right};'>${x}</td>`);
        });
        var p = pp.join('');
        var p = `<tr>${p}</tr>`;
        border_top = '';
    ...
    var css_style = [];
    if(islongtable){
      css_style.push('display:table');///always inline-block so that it can be placed side-by-side
      css_style.push('width:100%');
    }else{
  ...
  itms_to_bull(itms,style){
    ...
    if(itms.length){
      let pp = itms.map((p,i) => {
        if(p.bull=='--'){
          return `<div class='ITEM'>${bullet} ${p.text}</div>`;
        }
        if(p.bull=='**'){
          return `<div class='ITEM'>${i+1}. ${p.text}</div>`  
        }
        return `<div class='ITEM'>${p.term} &#160; ${p.text}</div>`
      });
      return `<div>${pp.join('\n')}</div>`;
    }
    return `<div></div>`;  
