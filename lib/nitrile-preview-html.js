...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  do_PLST(block) {
    ...
    for (var item of items) {
      ...
      if(more && more.length){
        more.forEach((plitem) => {
          let {lines} = plitem;
          extra_text += `<div class='P'> ${this.untext(lines,style)} </div>`;
        });
      }
      switch (bull) {
        case 'OL': {
          ...
          o.push(`<ol class='P PLST OL ${inner}' >`);
          break;
        }
        case 'UL': {
          count++;
          let inner = (count>1)? 'INNER' : '';
          o.push(`<ul class='P PLST UL ${inner}' >`);
          break;
        }
        case 'DL': {
          count++;
          let inner = (count>1)? 'INNER' : '';
          o.push(`<dl class='P PLST DL ${inner}' >`);
          break;
        }
        case 'HL': {
          count++;
          let inner = (count>1)? 'INNER' : '';
          o.push(`<ul class='P PLST HL ${inner}' >`);
          break;
        }
        case 'LI': {
          if(type=='hanginglist'){
            text = this.uncode(text,style);
            o.push(`<li><div class='P'>${text}</div></li>${extra_text}`);
          }
          else if(type=='description'){
            text = this.uncode(text,style);
            o.push(`<dt><div class='P'><b>${text}</b></div></dt> <dd>${extra_text}</dd>`);
          }
          else{
            text = this.uncode(text,style);
            if(bullet=='*'){
            ...
            o.push(`<li type='${item.type}' value='${value}' ><div class='P'>${text}</div></li>${extra_text}`);
          }
          break;
        }
        case '/OL': {
  ...
  do_PRIM(block) {
    ...
    var text = `<div class='P PRIM ${rank}' > ${text} </div>`;
    block.html = o.join('\n');
  }
  do_PARA(block){
  ...
  fence_to_blockquote(ss,g){
    ...
    return `<div style='${css}' class='BLOCKQUOTE'> ${text} </div>`;
  }
  fence_to_verbatim(ss,g){
    ...
    css += `text-indent:0;`;
    css += this.to_css_fontsize(g);
    return `<div style='${css}' class='VERBATIM' > ${text} </div>`;
  }
  ...
  fence_to_list(ss,g) {
    ...
    itms.forEach((p,i,arr) => {
      ...
      if(p.type=='UL'){
        d.push(`<label  style='display:list-item;list-style-type:none;padding-left:2em;text-indent:-2em;' >${bullet}${nbsp}${p.text}</label>`);
      }
      else if(p.type=='OL'){
        d.push(`<label  style='display:list-item;list-style-type:none;padding-left:2em;text-indent:-2em;' >${i+1}.${nbsp}${p.text}</label>`);  
      }
      else if(p.type == '1'){
        if(!p.ending) p.ending = '.';
        d.push(`<label  style='display:list-item;list-style-type:none;padding-left:2em;text-indent:-2em;' >${p.value}${p.ending}${nbsp}${p.text}</label>`);
      }
      else if(p.type == 'a'){
        if(!p.ending) p.ending = '.';
        d.push(`<label  style='display:list-item;list-style-type:none;padding-left:2em;text-indent:-2em;' >${this.to_a_letter(p.value)}${p.ending}${nbsp}${p.text}</label>`);
      }
      else if(p.type == 'A'){
        if(!p.ending) p.ending = '.';
        d.push(`<label  style='display:list-item;list-style-type:none;padding-left:2em;text-indent:-2em;' >${this.to_A_letter(p.value)}${p.ending}${nbsp}${p.text}</label>`);
        ...
        d.push(`<label style='display:list-item;list-style-type:none;padding-left:2em;text-indent:-2em;' ><b>${p.value}</b><br/>${p.text}</label>`)
      }
      else if(p.type == 'CB'){
        if(!p.bull.trim().length){
          d.push(`<label  style='display:list-item;list-style-type:none;padding-left:2em;text-indent:-2em;' >${squareboxo} ${p.text}</label>`);
        }else{
          d.push(`<label  style='display:list-item;list-style-type:none;padding-left:2em;text-indent:-2em;' >${squarebox} ${p.text}</label>`);
        }
      }
      else if(p.type == 'RB'){
        if(!p.bull.trim().length){
          d.push(`<label  style='display:list-item;list-style-type:none;padding-left:2em;text-indent:-2em;' >${circleboxo} ${p.text}</label>`);
        }else{
          d.push(`<label  style='display:list-item;list-style-type:none;padding-left:2em;text-indent:-2em;' >${circlebox} ${p.text}</label>`);
        }
      }
      else {
        d.push(`<label  style='display:list-item;list-style-type:none;padding-left:2em;text-indent:-2em;' >${p.text}</label>`);
      }
    });
    if(parsep){
    ...
    css += this.to_css_fontstyle(g);
    css += this.to_css_leftmargin(g);
    text = `<div style='${css}' class='LIST' > ${text} </div>`;
    return text;
  }
  /////////////////////////////////////////////////////////////////////
  ///
  ...
  para_to_equation(caption,ss,style){
    ...
    let o = [];
    o.push(`<figure class='P EQUATION' >`);
    o.push(text);
    o.push(`</figure>`);
    text = o.join('\n');
    return text;
  ...
  para_to_listing(caption,ss,style){
    ...
    return `<figure class='P LISTING' > ${this.to_caption_text(caption,style,"listing")} ${text} </figure>`;
  }
  para_to_figure(caption,ss,style){
    ...
    var text = `<figure class='P FIGURE' > ${this.to_caption_text(caption,style,"figure")} ${text} </figure>`;
    return text;
  }
  para_to_table(caption,ss,style){
    ...
    var text = `<figure class='P TABLE' > ${this.to_caption_text(caption,style,"table")} ${text} </figure>`;
    return text;
  }
  para_to_longtable(caption,ss,style){
    ...
    var text = `<figure class='P LONGTABLE' > ${this.to_caption_text(caption,style,"table")} ${text} </figure>`;
    return text;
  }
  para_to_columns(caption,ss,style){
    ...
    var text = `<div class='P COLUMNS'> ${text} </div>`;  
    return text;
  }
  /////////////////////////////////////////////////////////////////////
  /// samp  
  ...
  samp_to_framed(ss,style) {
    ...
    if(ENCODE){
      ///It is important to convert it to inline IMG because then the 'css_style' will take effect
      var text = `<img class='IMG' alt='diagram' style='${css_style.join(';')}' src="data:image/svg+xml;charset=UTF-8,${encodeURIComponent(s)}" />`;
    }else{
    ...
    var text = `<div class='P SAMP FRAMED'> ${text} </div>`;
    return text;
  }
  samp_to_verbatim(ss,style) {
    ...
    var text = ss.join('<br/>');
    var text = `<pre class='P SAMP VERBATIM'> ${text} </pre>`;
    return text;
  }
  /////////////////////////////////////////////////////////////////////
  /// text typefaces
