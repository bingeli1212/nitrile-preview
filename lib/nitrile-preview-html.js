...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  float_to_wrapfig(title,label,style,splitid,body,bodyrow){
    ...
    if(style.align=='left'){
      return `<figure id='${label}' class='WRAPFIG' style='${this.css("WRAPFIG")};float:left' data-row='${style.row}' > ${text} </figure>`;
    }else{
      return `<figure id='${label}' class='WRAPFIG' style='${this.css("WRAPFIG")};float:right' data-row='${style.row}' > ${text} </figure>`;
    }
  }
