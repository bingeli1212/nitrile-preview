...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  fence_to_dia(style,ss,ssi){
    ...
    if(type=='canvas'){
      ...
      var palettewidget = `<div class='CANVASPALETTE' style='z-index:2;position:absolute;top:100%;left:0;visibility:hidden;' id='${imgid}-palette' ><input class='CANVASSUBMIT' type='submit'/><input class='CANVASRESET' type='reset'/><button type='button' data-canvasid='${imgid}-canvas' onclick='canvas_onplus(this)'>+</button><button type='button' data-canvasid='${imgid}-canvas' onclick='canvas_onminus(this)'>-</button><input class='CANVASCOLOR' type='color' value='#476fc7' onchange='canvas_oncolorchange(this)' data-canvasid='${imgid}-canvas' id='${imgid}-colorinput' list='${imgid}-datalist' /> ${datalist} </div>`;
      var img = `<form class='CANVASFORM' action='/action.php' method='post' onreset='form_onreset(this)' onsubmit='form_onsubmit(this)' style='border:${border};position:relative;' data-imgsrc='' data-imgid='${imgid}' data-paletteid='${imgid}-palette' data-canvasid='${imgid}-canvas' > ${imgwidget} ${canvaswidget} ${palettewidget} </form>`
      itm.img  = img;
