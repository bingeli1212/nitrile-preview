...
<?xml version="1.0" standalone="no"?>
  <svg width="6cm" height="5cm" viewBox="0 0 600 500" xmlns="http://www.w3.org/2000/svg" onclick="svg_click(this)" >
    <desc>Example script01 - invoke an ECMAScript function from an onclick event
    </desc>
    <!-- ECMAScript to change the radius with each click -->
    <script type="application/ecmascript"> <![CDATA[
      function circle_click(circle) {
        console.log(event.offsetX, event.offsetY);
        var currentRadius = circle.getAttribute("r");
        if (currentRadius == 100) {
          circle.setAttribute("r", currentRadius*2);
          circle.setAttribute("transform", "translate(+300,+225) scale(1,2) translate(-300,-225)");     
          var i = circle.getAttribute("data")||"";
          i += "1";
          circle.setAttribute("data",i);
        }
        else {
          circle.setAttribute("r", currentRadius*0.5);
          circle.setAttribute("transform", "");     
          var j = circle.getAttribute("data");
          console.log("j=",j);
        }
      }
      function svg_click(rect) {
        console.log("svg_click",event.offsetX, event.offsetY, event.shiftKey, event.altKey);
      }
      function rect_click(rect) {
        console.log("rect_click",event.offsetX, event.offsetY, event.shiftKey, event.altKey);
        if(event.altKey){
          var x = parseFloat(rect.getAttribute("x"));
          var y = parseFloat(rect.getAttribute("y"));
          var rot = rect.getAttribute("rot")||"";
          var rot = parseFloat(rot)||0;
          rot += 5;
          rect.setAttribute("rot",rot);
          rect.setAttribute("transform", "translate("+x+","+y+") rotate("+rot+") translate("+(-x)+","+(-y)+")");     
      ...
      function rect_mousemove(rect) {
        console.log("mousemove",event.offsetX, event.offsetY, event.shiftKey, event.altKey);
        if(event.shiftKey){
          var dx = parseInt(event.offsetX)-parseInt(rect.getAttribute("data-mousedown-x"));
          var dy = parseInt(event.offsetY)-parseInt(rect.getAttribute("data-mousedown-y"));
console.log("dx=",dx,"dy=",dy);
          if(Number.isFinite(dx)&&Number.isFinite(dy)){
            ...
            rect.setAttribute("data-mousedown-x",event.offsetX);
            rect.setAttribute("data-mousedown-y",event.offsetY);
          }
        }
      }
      function rect_mousedown(rect) {
        console.log("mousedown",event.offsetX, event.offsetY, event.shiftKey, event.altKey);
        rect.setAttribute("data-mousedown-x",event.offsetX);
        rect.setAttribute("data-mousedown-y",event.offsetY);
        rect.setAttribute("data-mouseisdown","1");
      }
      function rect_mouseup(rect) {
        console.log("mousedown",event.offsetX, event.offsetY, event.shiftKey, event.altKey);
        rect.setAttribute("data-mousedown-x",event.offsetX);
        rect.setAttribute("data-mousedown-y",event.offsetY);
        rect.setAttribute("data-mouseisdown","");
      }
    ]]> </script>
    <!-- Outline the drawing area with a blue line -->
    <rect x="1" y="1" width="598" height="498" fill="none" stroke="blue"/>
    <!-- Act on each click event -->
    <rect onclick="rect_click(this)" onmousemove="rect_mousemove(this)" onmousedown="rect_mousedown(this)" x="100" y="100" width="100" height="100" fill="red"/>
...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  fence_to_ink(style,ss,ssi){
    ...
    var subtitle = style.subtitle||'';
    itm.style = style;
    var text = `\n<svg data-bundle="INK" data-label="${style.label}" width="${this.fix(itm.width*this.MM_TO_PX)}" height="${this.fix(itm.height*this.MM_TO_PX)}" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="currentColor" stroke="none" viewBox="0 0 ${this.fix(itm.vw)} ${this.fix(itm.vh)}" >${text}</svg>\n`;
