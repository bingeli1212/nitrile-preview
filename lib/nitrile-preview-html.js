...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  fence_to_math(style,ss) {
    ...
    var text = `<div class='MATH' > ${text} </div>`;
    var o = [];
    o.push({img:text,subc:'',stretch:1,subfigure:text});
    return o;
  }
  ...
  fence_to_tabular(style,ss) {
    ...
    var subfigure = `<table ${attrs.join(' ')} style='display:inline-table' class='TABULAR'>\n<caption align='bottom' class='SUBCAPTION'>${subc}</caption>\n${colgroup}\n${text}\n</table>`;
    var o = [];
    ...
    if(style.wrap){
      itms.forEach((p,j,arr) => {
        if(p.type=='bundle'){
          let img = p.img;
          if(style.wrap=='left'){
            img = `<figure class='FIGURE WRAPLEFT'> ${p.subfigure} </figure>`;
          }else{
            img = `<figure class='FIGURE WRAPRIGHT'> ${p.subfigure} </figure>`;
          }
          all.push(img);
        }
      });
      ...
      itms.forEach((p,j,arr) => {
        if(p.type=='bundle'){
          ...
          let seq = p.seq;
          if(style.subfigure){
            img = this.to_subfigure_num(img,seq);
          }
          all.push(img);
        }else if(p.type=='\\\\'){
          all.push(`<br/>`);
  ...
  phrase_to_framebox(style,cnt){
    return `<span style='display:inline-block;box-sizing:border-box;border:1px solid currentColor;width:2cm;'> &#160; </span>`;
  }
