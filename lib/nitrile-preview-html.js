...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  do_HDGS(block) {
    ...
    if(hdgn==0) {
      o.push(`<h1 id='${id}'>${idnum} ${title}</h1>`);
    } 
    else if(hdgn==1) {
      o.push(`<h2 id='${id}'>${idnum} ${title}</h2>`);
    } 
    else if(hdgn==2) {
      o.push(`<h3 id='${id}'>${idnum} ${title}</h3>`);
    } 
    else {
      o.push(`<h4 id='${id}'>${idnum} ${title}</h4>`);
    }
    o.push('');
    block.html = o.join('\n');
  }
  ...
  do_SAMP(block) {
    ...
    o.push(`<div class='SAMP'><pre style='margin:0'>${text}</pre></div>`);
    o.push('');
    block.html = o.join('\n');
  }
  do_HRLE(block) {
  ...
  do_PRIM(block) {
    ...
    o.push(`<div class='PRIM'>${text}</div>`);
    o.push('');
    block.html = o.join('\n');
  }
  do_TEXT(block) {
    ...
    if(nspace){
      o.push(`<div class='TEXT NSPACE'>${text}</div>`);
    }else{
      o.push(`<div class='TEXT'>${text}</div>`);
    }
    o.push('');
    block.html = o.join('\n');
  }
  do_FLOA(block) {
    var text;
    let id = this.get_refmap_value(block.style,'id');
    switch(block.style.floatname){
      case 'Figure': {
        ...
        if (1) {
          ...
          let o = [];
          o.push(`<figure id='${id}' class='Figure'>`);
      ...
      case 'Tabulate': {
        ...
        if (1) {
          let style = block.style;
          let o = [];
          o.push(`<figure id='${id}' class='Tabulate'>`);
          o.push(this.to_caption_text(style));
          o.push(text);
          o.push('</figure>');
          text = o.join('\n');
      ...
      case 'Longtable': {
        ...
        if (1) {
          let style = block.style;
          let o = [];
          o.push(`<figure id='${id}' class='Longtable'>`);
          o.push(this.to_caption_text(style));
          o.push(text);
          o.push('</figure>');
          text = o.join('\n');
      ...
      case 'Equation': {
        ...
        if(1){
          let o = [];
          o.push(`<figure id='${id}' class='Equation' style='position:relative;'>`);
          //o.push(`<span style='position:absolute;top:50%;transform:translate(0,-50%);'>${idnum}</span>`);
          o.push(`<div style='margin-left:${step}em;text-align:center;'>${text}</div>`);
          o.push(`<span class='idnum'>${idnum}</span>`);
          o.push(`</figure>`);
          text = o.join('\n');
        }
        break;
      ...
      case 'Listing': {
        ...
        if(1){
          let style = block.style;
          let o = [];
          o.push(`<figure class='Listing' id='${block.id}'>`);
          o.push(this.to_caption_text(style));
          o.push(text);
          o.push('</figure>');
          text = o.join('\n');
  ...
  to_caption_text(style){
    ...
    if(idnum && chnum){
      return `<figcaption class='${floatname}'><strong>${title} ${chnum}.${idnum}</strong> &#160; ${this.unmask(caption)}</figcaption>`;  
    }else if(idnum){
      return `<figcaption class='${floatname}'><strong>${title} ${idnum}</strong> &#160; ${this.unmask(caption)}</figcaption>`;
    }else{
      return `<figcaption class='${floatname}'><strong>${title} </strong> &#160; ${this.unmask(caption)}</figcaption>`;
    }
  }
