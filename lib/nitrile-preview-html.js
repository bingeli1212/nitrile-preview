...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  fence_to_canvas(style,ss,ssi){
    ...
    var canvaswidget = `<canvas style='position:relative;z-index:20;' width='${p.width}' height='${p.height}' onmouseenter="canvas_onmouseenter(this)" onmouseleave="canvas_onmouseleave(this)" onmousedown="canvas_onmousedown(this)" onmousemove="canvas_onmousemove(this)" onmouseup="canvas_onmouseup(this)" onfocusin='canvas_onfocusin(this)' onfocusout='canvas_onfocusout(this)' data-imgid='${imgid}' data-paletteid='${imgid}-palette' id='${imgid}-canvas' tabIndex='1' > </canvas>`;
    var palettewidget = `<div style='position:absolute;top:100%;left:0;visibility:hidden;' id='${imgid}-palette' ><button type='submit'>S</button><button type='button'>D</button><button type='button'>E</button></div>`;
    //var submitwidget = `<input style='position:absolute;top:100%;left:0;' type='submit' id='${imgid}-submit' />`;
    p.subc = style.subtitle||'';
    p.subc = this.uncode(style,p.subc);
    p.style = style;
    var frame = (style.frame)?`frame='box'`:``;
    var s_style = this.css('SUBTITLE');
    var text = `<form action='/post' method='post' onsubmit='canvas_onsubmit(this)' style='position:relative;' data-imgsrc='${imgsrc}' data-imgid='${imgid}' data-canvasid='${imgid}-canvas' > ${imgwidget} ${canvaswidget} ${palettewidget} </form>`
  ...
  phrase_to_url(style,cnt){
    return `<span class='URL' style='${this.css("URL")}'>${cnt}</span>`
  }
  //////////////////////////////////////////////////////////////////////////////////////
  /// 
  /// translator function
