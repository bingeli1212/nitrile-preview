...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  fence_to_tab(style,ss,ssi) {
    ...
    if(1){//colgroup has to come before everything else after caption
      ...
      if(style.side){
        let s1 = ww.map((x,i,arr) => `<col width='${(x.pw)?(x.pw*this.MM_TO_PX):(def)}'/>`).slice(0,1).join('\n');
        let s2 = ww.map((x,i,arr) => `<col width='${(x.pw)?(x.pw*this.MM_TO_PX):(def)}'/>`).slice(1).join('\n');
        s = `<colgroup>\n${s1}\n</colgroup>\n<colgroup>\n${s2}\n</colgroup>`;      
      }else{
        let s2 = ww.map((x,i,arr) => `<col width='${(x.pw)?(x.pw*this.MM_TO_PX):(def)}'/>`).join('\n');
        s = `<colgroup>\n${s2}\n</colgroup>`;
      }
      if(hew>1){
        let s0 = s;
