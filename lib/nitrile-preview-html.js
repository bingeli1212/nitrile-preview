...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  constructor(parser) {
    ...
    this.icon_triangledown = '&#x25BF;'//WHITE DOWN-POINTING SMALL TRIANGLE
  ...
  hdgs_to_chapter(title,label,style){
    return(`<h2 id='${label}' class='CHAPTER' style='${this.css("CHAPTER")}' data-partnum='${style.__partnum}' data-chapnum='${style.__chapnum}' > ${this.uncode(style,title)} </h2>`);
  }
  hdgs_to_section(title,label,level,style){
    return(`<h3 id='${label}' class='SECTION' style='${this.css("SECTION")}' data-partnum='${style.__partnum}' data-chapnum='${style.__chapnum}' data-level='${level}' > ${this.uncode(style,title)} </h3>`);
  }
  hdgs_to_subsection(title,label,level,style){
    return(`<h4 id='${label}' class='SUBSECTION' style='${this.css("SUBSECTION")}' data-partnum='${style.__partnum}' data-chapnum='${style.__chapnum}' data-level='${level}' > ${this.uncode(style,title)} </h4>`);
  }
  hdgs_to_subsubsection(title,label,level,style){
    return(`<h5 id='${label}' class='SUBSUBSECTION' style='${this.css("SUBSUBSECTION")} data-partnum='${style.__partnum}' data-chapnum='${style.__chapnum}' data-level='${level}' > ${leader} ${this.uncode(style,title)} </h5>`);
  }
  /////////////////////////////////////////////////////////////////////
  //
  /////////////////////////////////////////////////////////////////////
  ...
  fence_to_tab(g,ss) {
    ...
    var tab  = `<table width='${width}' class='TAB' style='display:inline-table;margin:0;page-break-inside:avoid;table-layout:fixed;font-size:${textsmaller};' ${frame} ${rules} cellpadding='0' cellspacing='0'>${text}</table>`;
    var itm = {};
    itm.tab = tab;
    itm.out = tab;
    return itm;
  ...
  fence_to_par(g,ss){
    ...
    var par = `<span style='width:${w};height:${h};border:${f};display:inline-block;page-break-inside:avoid;text-align:${a};'>${text}</span>`;
    var itm = {};
    itm.par = par;
    itm.out = par;
    return itm;
  ...
  phrase_to_hrule(style,cnt){
    ...
    return `<span class='hrule' style='display:inline-block;box-sizing:border-box;border-bottom:thin solid;padding:1px;min-width:${wd};text-align:center;'> ${s} </span>`;
  }
  //////////////////////////////////////////////////////////////////////////////////////
  /// 
  /// translator function
  ...
  float_to_details(title,label,style,subtitles,body,bodyrow){
    ...
    step.forEach((p,i,arr) => {
      if(i==0){
        if(p.value=='+'){
          all.push(`<dl class='DETAILS' style='${this.css("DETAILS")}'>`);
          all.push('');
          ...
          all.push(`<ul class='DETAILS' style='${this.css("DETAILS")}'>`);
          all.push('');
          ...
          all.push(`<ol class='DETAILS' style='${this.css("DETAILS")}'>`);
          all.push('');
        ...
        if(this.re_body_is_fence.test(p.lines[0])){
          ...
          var itms = bundles.map((bundle) => this.do_bundle(bundle));
          let d = [];
          ...
          let lines = this.body_to_body(p.lines,this.re_body_is_gthan).map((x,i)=>{
            if(i==0){
              return `${this.icon_triangledown}&#x2002; ${this.uncode(style,x)}`;
            }else{
              return `<span style='opacity:0'>${this.icon_triangledown}</span>&#x2002; ${this.uncode(style,x)}`;
            }
          });
          ...
          var text = lines.join('<br/>\n')
          all.push(`<p class='DETAILSROW' style='${this.css("DETAILSROW")}'>${text}</p>`);
        }else if(this.re_body_is_lthan.test(p.lines[0])){
          let body0 = body;
          body = this.body_to_body(body0,this.re_body_is_lthan,2);
          bodyrow = this.body_to_bodyrow(body0,this.re_body_is_lthan);
          let rows = this.ss_to_tabbing_rows(body,style);
          let ww = bodyrow.map(x=>x.length);
          let d = rows.map((pp)=>{
            let ss1 = pp.map(p=>p.text);
            ss1 = ss1.map(x=>this.uncode(style,x));
            let template = ww.map(x=>`${x}fr`).join(' ');
  ...
  float_to_alignment(title,label,style,subtitles,body,bodyrow){
    ...
    var text = d.join('<br/>\n');
    return `<figure class='ALIGNMENT' style='${this.css("ALIGNMENT")};text-align:${align};' data-row='${style.row}' >${text}</figure>`;
  }
  float_to_equation(title,label,style,subtitles,body,bodyrow){
    ...
    var all = bundles.map((bundle,i,arr) => {
      let itm = this.do_bundle(bundle,'fml');
      var fml = itm.fml;
      var sub = '';
      if(arr.length>1){
    ...
    var text = all.join("\n");
    return `<p id='${label}' class='EQUATION' style='${this.css("EQUATION")}' data-row='${style.row}' >${text}</p>` ;
  }
  float_to_figure(title,label,style,subtitles,body,bodyrow){
    var bundles = this.body_to_all_bundles(style,body);
    var itms = bundles.map((bundle) => this.do_bundle(bundle,'img'));
  ...
  float_to_body(title,label,style,subtitles,body,bodyrow){
        ...
        if(this.re_body_is_fence.test(p.lines[0])){
          ...
          itms.forEach((p,i,arr)=>{
            if(p.type=='bundle'){
              if(p.img){
                img = p.img;
                img = this.add_css_style('float:right;margin-left:1em;',img);
              }else if(p.tab){
                tab = p.tab;
                tab = this.add_css_style('float:right;margin-left:1em;',tab);
              }
            }
          });
        }
    ...
    if(img){
      return `<p id='${label}' class='BODY' style='${this.css("BODY")}'> ${img} ${text} </p>`;
    }else if(tab){
      return `<p id='${label}' class='BODY' style='${this.css("BODY")}'> ${tab} ${text} </p>`;
    }else{
      return `<p id='${label}' class='BODY' style='${this.css("BODY")}'> ${text} </p>`;
    }
  }
  ...
  float_to_table(title,label,style,subtitles,body,bodyrow){
    ...
    var all = [];
    bundles.forEach((bundle,j,arr) => {
      let itm = this.do_bundle(bundle,'tab');
      var splitid = bundle.splitid||0;
      var onerow = [];
      onerow.push(itm.tab);
  ...
  float_to_lines(title,label,style,subtitles,body,bodyrow){
    var bundles = this.body_to_all_bundles(style,body);
    bundles = this.merge_all_bundles(style,bundles);
    var ss = [];
    var ssi = 0;
  ...
  float_to_tabbing(title,label,style,subtitles,body,bodyrow){
    var n = body.length;
    var rows = this.ss_to_tabbing_rows(body,style);
    var ww = bodyrow.map(x=>x.length);
    var all = [];
    rows.forEach((pp)=>{
      let ss1 = pp.map(p=>p.text);
      ss1 = ss1.map(x=>this.uncode(style,x));
