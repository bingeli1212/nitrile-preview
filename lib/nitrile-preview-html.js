    ...
    itms.forEach((p,i,arr) => {
      ...
      if(p.type=='UL'){
        ...
        if(p.bull.trim().length || this.is_in_list(p.start,check_ss)){
          //filled CB
          d.push(`<div  style='${css}' > <input type='checkbox' checked disabled></input> <label>${p.text}</label> </div>`);
        }else{
          //empty CB
          d.push(`<div  style='${css}' > <input type='checkbox' disabled></input> <label>${p.text}</label> </div>`);
        }
      }
      else if(p.type == 'RB'){
        if(p.bull.trim().length || this.is_in_list(p.start,check_ss)){
          //filled CB
          d.push(`<div  style='${css}' > <input type='radio' checked disabled></input> <label>${p.text}</label> </div>`);
        }else{
          //empty CB
          d.push(`<div  style='${css}' > <input type='radio' disabled></input> <label>${p.text}</label> </div>`);
        }
      }
      else {
        d.push(`<label  style='${css}' >${bullet}${nbsp}${p.text}</label>`);
