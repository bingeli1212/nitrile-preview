...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  fence_to_tab(style,ss,ssi) {
    ...
    var subtitle = this.to_fig_subtitle(style);
    var fontsize_css = '';
    var width_css = stretch?`width:${stretch*100}%`:``;
    let s_style = this.css('SUBTITLE');
    let t_style = `${s_style};text-align:left`;
    itm.img  = ``;
    itm.tab  = `<table class='TAB' style='${this.css("TAB")};${fontsize_css};${width_css}' ${frame} ${rules} cellpadding='0' cellspacing='0' data-row='${style.row}' >\n<caption align='bottom' class='SUBTITLE' style='${t_style}'>${this.to_tab_subcaptions(style)}</caption>\n${text}\n</table>`;
    itm.style = style;
    itm.subtitle = subtitle;
    return itm;
  }
  fence_to_par(style,ss,ssi){
    ...
    var text = `<span style='margin-left:auto;margin-right:auto;display:inline-block;width:100%;text-align:${talign};'> ${text} </span>`;
    var itm = {};
    itm.img = '';
    itm.tab  = `\n<table class='PAR' style='${this.css("PAR")}' ${frame} ${rules} cellpadding='0' cellspacing='0' data-row='${style.row}' >\n<caption align='bottom' class='SUBTITLE' style='${this.css("SUBTITLE")}'>${subtitle}</caption>\n<colgroup/><thead/><tbody><tr><td style='vertical-align:bottom;'>${text}</td></tr></tbody>\n</table>`;
    itm.subtitle = subtitle;
    itm.style = style;
    return itm;
  }
  ...
  float_to_flushleft(title,label,style,splitid,body,bodyrow){
    ...
    var text = all.join('\n');
    return `<figure class='FLUSHLEFT' style='${this.css("FLUSHLEFT")};display:flex;flex-direction:column;align-items:flex-start;' data-row='${style.row}' >${text}</figure>`;
  }
  float_to_center(title,label,style,splitid,body,bodyrow){
    ...
    var text = all.join('\n');
    return `<figure class='CENTER' style='${this.css("CENTER")};display:flex;flex-direction:column;align-items:center;' data-row='${style.row}' >${text}</figure>`;
  }
    ...
    itms.forEach((p,j,arr) => {
      if(p.type=='bundle'){
        if(p.img){
          ...
          let subtitle = `<span style='text-align:center;font-size:smaller;'>${p.subtitle}</span>`;
      ...
      if(p.type=='\\\\'||j+1==arr.length){
        let n = onerow.length;
        if(n){
          all.push(`<span style='display:inline-grid;grid-template-columns:${onerow.map(s=>s[0]).join(' ')};' > ${onerow.map(s=>s[1]).join('')} ${onerow.map(s=>s[2]).join('')} </span>`);
          onerow = [];
        }
      }
    });
  ...
  float_to_columns(title,label,style,splitid,body,bodyrow){
    ...
    var n = itms.length;
    var all = [];
    all.push(`<figure class='COLUMNS' style='${this.css("COLUMNS")};display:grid;grid-template-columns:repeat(${n},auto);' data-row='${style.row}' >`);
    itms.forEach((p,j,arr) => {
      if(p.img){
        all.push(p.img);
      }else if(p.tab){
