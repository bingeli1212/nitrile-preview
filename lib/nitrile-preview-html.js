...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  fence_to_dia(style,ss,ssi){
    ...
    o = o.map((p) => {
      ...
      p.img = `\n<table class='PARAGRAPH DIA' ${frame} cellpadding='0' cellspacing='0' width='${p.width}' data-row='${style.row}' >\n<caption align='bottom' class='SUBCAPTION' style='${s_style}' >         </caption>\n<colgroup/><thead/><tbody><tr><td>${s_svg}</td></tr></tbody>\n</table>`;
      p.fig = `\n<table class='          DIA' ${frame} cellpadding='0' cellspacing='0' width='${p.width}' data-row='${style.row}' >\n<caption align='bottom' class='SUBCAPTION' style='${s_style}' >${p.subc}</caption>\n<colgroup/><thead/><tbody><tr><td>${s_svg}</td></tr></tbody>\n</table>`;
      return p;
    });
    return o;
  }
  ...
  fence_to_tabular(style,ss,ssi) {
    ...
    var all = [];
    let lower = '';
    let padding = '0.2835em 0.4252em';
    let t_style = '';
    if(style.small){
      t_style += ';font-size:smaller';
    }
    if(style.head && rows.length){
      ...
      pp.forEach((p,i) => {
        let {raw,text} = p;
        let css = '';
        css += `padding:${padding}; text-align:${ww[i].ta}`;
        if(pp.top){
    ...
    var img = `<table class='PARAGRAPH TABULAR' ${frame} ${rules} cellpadding='0' cellspacing='0' data-row='${style.row}' style='${t_style}' >\n<caption align='bottom' class='SUBCAPTION' style='${s_style}' >       </caption>\n${colgroup}\n${text}\n</table>`;
    var fig = `<table class='          TABULAR' ${frame} ${rules} cellpadding='0' cellspacing='0' data-row='${style.row}' style='${t_style}' >\n<caption align='bottom' class='SUBCAPTION' style='${s_style}' >${subc}</caption>\n${colgroup}\n${text}\n</table>`;
    var cov = `<table class='          TABULAR' ${frame} ${rules} cellpadding='0' cellspacing='0' data-row='${style.row}' style='${t_style}' >\n<caption align='bottom' class='SUBCAPTION' style='${s_style}' >       </caption>\n${colgroup}\n${text}\n</table>`;
    var o = [];
  ...
  float_to_figure(title,label,style,ss,ssi){
    ...
    if(style.type=='right'){
      return `<figure class='PARAGRAPH FIGURE' data-row='${style.row}' style='float:right'> ${text} </figure>`;
    }else if(style.type=='left'){
      return `<figure class='PARAGRAPH FIGURE' data-row='${style.row}' style='float:left'> ${text} </figure>`;
