...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  fence_to_canvas(style,ss){
    ...
    var dd = this.string_to_array(viewport);
    var viewport_width = this.assert_int(dd[0],12);
    var viewport_height = this.assert_int(dd[1],7);
    var viewport_unit = this.assert_int(dd[2],5);//mm
    let stretch = this.g_to_stretch_float(style);
    let width = this.g_to_width_float(style);
    let height = this.g_to_height_float(style);
    if(stretch){
    ...
    var img = `<canvas width='${viewport_width*viewport_unit*this.MM_to_PX}' height='${viewport_height*viewport_unit*this.MM_to_PX}' style='${css}' json='${json}'></canvas>`;
    var sub = '';
    var o = [];
    o.push({img,sub});
    return o;
  ...
  fence_to_verse(style,ss) {
    ...
    itms.forEach((p,i,arr) => {
      ...
      if(p.type=='UL'){
        ...
        if(!p.ending) p.ending = '.';
        d.push(`<div  style='${css}'   >${this.to_a_letter(p.value)}${p.ending} ${p.text}</div>`);
      }
      else if(p.type == 'A'){
        if(!p.ending) p.ending = '.';
        d.push(`<div  style='${css}'   >${this.to_A_letter(p.value)}${p.ending} ${p.text}</div>`);
