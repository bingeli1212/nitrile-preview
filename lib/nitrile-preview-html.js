...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  fence_to_ink(style,ss){
    ...
    if(ENCODE){
      ...
      img = `<object style='${css}' width='${w}' height='${h}' data='data:image/svg+xml;charset=UTF-8,${encodeURIComponent(s_svg)}'></object>`;
    }else{
      img = `<svg style='${css}' width='${this.fix(w)}' height='${this.fix(h)}' xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' fill='currentColor' stroke='none' viewBox='0 0 ${this.fix(vw)} ${this.fix(vh)}' >${img}</svg>`;
    }
    var o = [];
    var subc = '';
  ...
  fence_to_dia(style,ss){
    ...
    o = o.map((p) => {
      ...
      let s_svg = `<svg style="${css}" width='${w}' height='${h}' xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="currentColor" stroke="currentColor" viewBox="0 0 ${this.fix(p.vw)} ${this.fix(p.vh)}" >${p.svgtext}</svg>`;
      //var img = `<object style='${css}' width='${p.vw}' height='${p.vh}' data='data:image/svg+xml;charset=UTF-8,${encodeURIComponent(s_svg)}' ></object>`;
      var img = s_svg;
      p.img = img;
  ...
  fence_to_verbatim(style,ss){
    ...
    var img = `<table ${frame} style='${css}'> <tr><td>${img}</td></tr> </table>`;
    var o = [];
    var subc = '';
  ...
  float_to_paragraph(title,label,style,body){
    ...
    all.push(text);
    if(title){
      var css = this.css('PARAGRAPH PRIMARY');
    }else{
      var css = this.css('PARAGRAPH');
    }
    if(style.rank==1){
  ...
  to_subfigure_img(img,sub,w,stretch,seq,subfigure){
    ...
    let o = [];
    if(stretch){
      o.push(`<table cellpadding='0' cellspacing='0' style='${subfigure_css}' width='100%' >`);
    }else if(w){
