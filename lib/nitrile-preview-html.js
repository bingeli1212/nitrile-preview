...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  do_FLOA(block) {
    ...
    switch(block.floatname){
      ...
      case 'Equation': {
        ...
        if(this.refmap.has(label)){
          let g = this.refmap.get(label);
          idnum = g.idnum||'';
          chnum = g.chnum||'';
        }
        if(chnum){
        ...
        text = `<div class='Equation' style='position:relative;'><div style='position:absolute;top:50%;transform:translate(0,-50%);'>${idnum}</div><div style='margin-left:${step}em;text-align:center;'>${text}</div></div>`;
        break;
      }
      case 'Note': {
        block.notes.set(block.label,block.body);
        break;
      }
      case 'Vocabulary': {
  ...
  phrase_to_ref (cnt){
    if(this.refmap && this.refmap.has(cnt)){      
      let g = this.refmap.get(cnt);
  ...
  to_caption_text(style){
    ...
    if(idnum && chnum){
      return `<figcaption><strong>${title} ${chnum}.${idnum}</strong> &#160; ${this.unmask(caption)}</figcaption>`;  
    }else if(idnum){
      return `<figcaption><strong>${title} ${idnum}</strong> &#160; ${this.unmask(caption)}</figcaption>`;
    }else{
      return `<figcaption><strong>${title} </strong> &#160; ${this.unmask(caption)}</figcaption>`;
    }
  }
