...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
    ...
    for (var item of items) {
      ...
      switch (bull) {
        case 'OL': {
          ...
          o.push(`<ol class='PLST OL ${inner}' >`);
          break;
        }
        case 'UL': {
          count++;
          let inner = (count>1)? 'INNER' : '';
          o.push(`<ul class='PLST UL ${inner}' >`);
          break;
        }
        case 'DL': {
          count++;
          let inner = (count>1)? 'INNER' : '';
          o.push(`<dl class='PLST DL ${inner}' >`);
          break;
        }
        case 'HL': {
          count++;
          let inner = (count>1)? 'INNER' : '';
          o.push(`<ul class='PLST HL ${inner}' >`);
          break;
        }
        case 'LI': {
          if(type=='hanginglist'){
            text = this.uncode(text,style);
            o.push(`<li><div class='PARA'>${text}</div></li>${extra_text}`);
          }
  ...
  para_to_listing(caption,ss,style){
    ...
    return `<figure class='LISTING' > ${this.to_caption_text(caption,style,"listing")} ${text} </figure>`;
  }
  para_to_figure(caption,ss,style){
    ...
    return `<figure class='FIGURE' > ${this.to_caption_text(caption,style,"figure")} ${text} </figure>`;
  }
  para_to_table(caption,ss,style){
    ...
    return `<figure class='TABLE' > ${this.to_caption_text(caption,style,"table")} ${text} </figure>`;
  }
