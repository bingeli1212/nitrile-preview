...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  hdgs_to_section(title,label,level,style){
    ...
    if(style.chapnum){
      return(`<h2 id='${label}' data-row='${style.row}' class='SECTION' style='${css}' > ${style.chapnum}.${style.level} <span title='${style.note}:${1+style.row}'>${this.uncode(style,title)}</span> </h2>`);
    }else{
      return(`<h2 id='${label}' data-row='${style.row}' class='SECTION' style='${css}' > ${style.level} <span title='${style.note}:${1+style.row}'>${this.uncode(style,title)}</span> </h2>`);
    }
  }
  /////////////////////////////////////////////////////////////////////
  //
  ...
  hdgs_to_subsection(title,label,level,style){
    ...
    if(style.chapnum){
      return(`<h3 id='${label}' data-row='${style.row}' class='SUBSECTION' style='${css}' > ${style.chapnum}.${style.level} <span title='${style.note}:${1+style.row}'>${this.uncode(style,title)}</span> </h3>`);
    }else{
      return(`<h3 id='${label}' data-row='${style.row}' class='SUBSECTION' style='${css}' > ${style.level} <span title='${style.note}:${1+style.row}'>${this.uncode(style,title)}</span> </h3>`);
    }
  }
  /////////////////////////////////////////////////////////////////////
  //
  ...
  hdgs_to_subsubsection(title,label,level,style){
    ...
    if(style.chapnum){
      return(`<h4 id='${label}' data-row='${style.row}' class='SUBSECTION' style='${css}' > ${style.chapnum}.${style.level} <span title='${style.note}:${1+style.row}'>${this.uncode(style,title)}</span> </h4>`);
    }else{
      return(`<h4 id='${label}' data-row='${style.row}' class='SUBSECTION' style='${css}' > ${style.level} <span title='${style.note}:${1+style.row}'>${this.uncode(style,title)}</span> </h4>`);
    }
  }
  /////////////////////////////////////////////////////////////////////
  /////////////////////////////////////////////////////////////////////
  ...
  fence_to_img(style,ss,ssi){
    ...
    if(1){
      var imgwidget = `<img style='position:absolute;visibility:hidden;' width='${p.width}' height='${p.height}' src='${imgsrc}' data-canvasid='${imgid}-canvas' id='${imgid}' alt='${imgsrc}' />`;
      var canvaswidget = `<canvas style='position:relative;z-index:1;width:${p.width}mm;${p.height}mm;' width='${p.vw}' height='${p.vh}' onmouseenter="canvas_onmouseenter(this)" onmouseleave="canvas_onmouseleave(this)" onmousedown="canvas_onmousedown(this)" onmousemove="canvas_onmousemove(this)" onmouseup="canvas_onmouseup(this)" ontouchstart="canvas_ontouchstart(this)" ontouchmove="canvas_ontouchmove(this)" ontouchend="canvas_ontouchend(this)" ontouchcancel="canvas_ontouchcancel(this)" onfocusin="canvas_onfocusin(this)" onfocusout="canvas_onfocusout(this)" onkeydown="canvas_onkeydown(this)" data-imgid='${imgid}' data-paletteid='${imgid}-palette' data-colorinputid='${imgid}-colorinput' id='${imgid}-canvas' data-type='${type}' data-ball='${ball}' data-unit='${unit}' tabIndex='1' > </canvas>`;
      var datalist = `<datalist id='${imgid}-datalist'> ${itms.color?(itms.color.map(s=>"<option>"+s+"</option>").join("")):''} </datalist>`;
      var palettewidget = `<div class='CANVASPALETTE' style='z-index:2;position:absolute;top:100%;left:0;visibility:hidden;' id='${imgid}-palette' ><button class='CANVASSUBMIT' type='submit'>x</button><button class='CANVASRESET' type='reset'>r</button><input class='CANVASCOLOR' type='color' value='#476fc7' onchange='canvas_oncolorchange(this)' data-canvasid='${imgid}-canvas' id='${imgid}-colorinput' list='${imgid}-datalist' /> ${datalist} </div>`;
    }
    p.subc = style.subtitle||'';
    p.subc = this.uncode(style,p.subc);
    p.style = style;
