    ...
    for (var j = 0; j < k; ++j) {
      ...
      pp.forEach((x, i) => {
        ...
        if(frs){
          ...
          css += `text-align:${haligns[i].halign};width:${haligns[i].pw};`
        }
        css += `height:${ht};`
        css += `padding:${pad};`;
...
${j+n >= itms.length && hlines.indexOf('b') >= 0 ? 'border-bottom:1px solid;' : ''} '>${x}</td>`);
  ...
  string_to_td_haligns(halign,n) {
    ...
    pp = pp.map((x) => {
      if(re_lcr.test(x)){
        if(x=='l') return {halign:'left',pw:''};
        if(x=='r') return {halign:'right',pw:''};
        return            {halign:'center',pw:''};  
      }else if(re_pw.test(x)){
        var w = re_pw.exec(x)[1];
        var w = this.string_to_css_width(w,1,'');
        if(w){
          return {halign:'left',pw:w};
        }else{
          return {halign:'left',pw:''};
        }
      }else{
        return null;
      }
