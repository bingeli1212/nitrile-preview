...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  fence_to_center(style,ss){
    ...
    return `<div class='CENTER' style='text-align:center'> ${text} </div>`;
  }
  fence_to_flushright(style,ss){
    var text = this.join_para(ss);
    var text = this.uncode(style,text);
    return `<div class='CENTER' style='text-align:right'> ${text} </div>`;
  }
    ...
    itms.forEach((p,i,arr) => {
      ...
      if(p.type=='UL'){
        ...
        if(p.bull.trim().length || this.is_in_list(p.start,check_ss)){
          //filled CB
          d.push(`<label  style='${css}' > <input type='checkbox' checked                 ></input> ${p.text}</label> `);
        }else{
          //empty CB
        ...
        if(p.bull.trim().length || this.is_in_list(p.start,check_ss)){
          //filled CB
          d.push(`<label  style='${css}' > <input type='radio' checked                 ></input> ${p.text}</label> `);
        }else{
          //empty CB
