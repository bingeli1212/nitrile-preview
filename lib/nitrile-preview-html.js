...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  ///
  /////////////////////////////////////////////////////////////////////
  float_to_paragraph(title,label,style,ss,ssi){
    var style = {...style,title,label,float:'paragraph'};
    var issamp = this.is_samp(ss);
    var iscase = this.is_case(ss);
    var isplst = this.is_plst(ss);
    if(issamp){
      let samps = this.ss_to_samps(ss,ssiw);
      var text = this.samps_to_text(style,samps);
    }else if(iscase){
      let cases = this.ss_to_cases(ss,ssi);
      var text = this.cases_to_text(style,cases);
    }else if(isplst){
      let plitems = this.ss_to_plitems(ss,ssi);
      let itemize = this.plitems_to_itemize(plitems);
      var text = this.itemize_to_text(style,itemize);  
    }else{
      var text = this.untext(style,ss);
    }
    text = text.trim();
    var all = [];
    all.push('');
