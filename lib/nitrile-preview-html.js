...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  fence_to_parbox(ss,g) {
    ...
    css += this.style_to_css_width(g,'100%');
    css += this.style_to_css_fontsize(g);
    css += this.style_to_css_fontfamily(g);
    css += this.style_to_css_fontstyle(g);
  ...
  fence_to_tabular(ss,g) {
    ...
    css += `display:inline-block;`
    css += `text-indent:0;`
    css += this.style_to_css_bordercollapse(g);
    if(g.wrap){
      css += this.to_css_float(g);
    }
    css += this.style_to_css_fontsize(g);  
    css += this.style_to_css_width(g,'');  
