...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
    ...
    for (var item of items) {
      ...
      if(more && more.length){
        more.forEach((plitem) => {
          ...
          extra_text += `<p class='PARA'> ${this.untext(lines,switches)} </p>`;
        });
      }
      switch (bull) {
        case 'OL': {
  ...
  fence_to_blockquote(ss,style){
    var text = this.uncode(this.join_para(ss),style);
    var css = '';
  ...
  to_caption_text(caption,style,name){
    ...
    let caption_text = this.uncode(caption);
    if(idnum){
      var lead = `${tag} ${idnum}:`
    }else{
      var lead = `${tag}:`
  ...
  rows_to_tabular(rows,style){
    ...
    // add the title
    if(style.title){
      let caption_text = this.uncode(style.title);
  ...
  itms_to_bull(itms,style){
    var bullet = '&#x2022;'
    const nbsp = '&#160;&#160;'
    const squareboxo = this.uncode('&SquareBoxO;');
    const squarebox = this.uncode('&SquareBox;');
    const circleboxo = this.uncode('&CircleBoxO;');
    const circlebox = this.uncode('&CircleBox;'); 
    if(style.bullet){
      try{
  ...
  itms_to_lines(itms,style){
    ...
    for (let j = 0, i = 0; j < itms.length; i += 1, j += k) {
      var pp = itms.slice(j, j + k).map(p => this.uncode(p.text))
      cols.push(pp);
    }
  ...
  itms_to_figure(itms,style) {
    ...
    itms.forEach( (p,i,arr) => {
      ...
      var s = '';
      if(type=='framed'){
        var s = this.fence_to_framed(ss,g);
      }else if(type=='diagram'){
        var s = this.fence_to_diagram(ss,g);
      }else if(type=='img'){
        var s = this.fence_to_img(ss,g);
      }
...
${j+n >= itms.length && hlines.indexOf('b') >= 0 ? 'border-bottom:1px solid;' : ''} '>${x}</td>`);
  ...
  to_css_height(style){
    var s = '';
    if(style.height){
      s = this.string_to_css_height(style.height,style.zoom);
      s = `height:${height};`;
    }
    return s;
  }
