...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  fence_to_ink(style,ss){
    ...
    css += `text-indent:0;`;
    css += this.style_to_css_width(style,`${vw}pt`); //pt -> mm
    css += this.style_to_css_height(style,'');
  ...
  float_to_sample(title,label,style,body){
    ...
    clusters.forEach((ss)=>{
      ss = this.trim_samp_body(ss);
      ss = ss.map((x) => this.polish_verb(style,x));
      ss = ss.map((x) => `<code>${x}</code>`);
      var text = ss.join('<br/>');
  ...
  float_to_listing(title,label,style,ss){
    ...
    var ss = ss.map((line, i) => {
      var line = this.polish(style,line);
      return `<code class='LINE' style='white-space:pre'> ${line} </code>`;
    });
    var text = ss.join('<br/>\n');
    var text = `<div class='BODY'> ${text} </div>`;
  ...
  ///
  //////////////////////////////////////////////////////////////////////////////////////
  to_templated_css(n_para,n_pack,n_half){
    return `
      table {
