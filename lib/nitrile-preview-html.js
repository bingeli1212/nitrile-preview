...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  do_HDGS(block) {
    ...
    ///note that 'hdgn' could be set to 'part'
    if(hdgn=='part'){
  ...
  do_PLST(block) {
    ...
    for (var item of items) {
      ...
      let nested = (count>1)? 'NESTED' : 'TOP';
      switch (bull) {
        case 'OL': {
          count++;
          let nested = (count>1)? 'NESTED' : 'TOP';
          o.push(`<ol class='PLST ${nested}' >`);
          break;
        }
        case 'UL': {
          count++;
          let nested = (count>1)? 'NESTED' : 'TOP';
          o.push(`<ul class='PLST ${nested}' >`);
          break;
        }
        case 'DL': {
          count++;
          let nested = (count>1)? 'NESTED' : 'TOP';
          o.push(`<dl class='PLST ${nested}' >`);
          break;
        }
        case 'LI': {
          if(bullet[0]=='+'){
            text = this.unmask(text);
            o.push(`<dt class='PLST ${nested}' ><b>${text}</b></dt><dd class='PLST ${nested}'>${extra_text}</dd>`);
          }
          else{
            text = this.unmask(text);
            if(bullet=='*'){
            ...
            o.push(`<li class='PLST ${nested}' type='${item.type}' value='${value}' >${text}</li>${extra_text}`);
          }
          break;
        }
        case '/OL': {
  ...
  phrase_to_img(s) {
    ...
    if(src.charAt(0)=='#'){
      ...
      let height = this.string_to_css_height(g.height,zoom);
      if(height){
        css_style.push(`height:${height}`)
      }
      if(g.frame==1){
  ...
  para_to_quote(ss,style) {
    let text = ss.join('\n').trim();
    text = this.unmask(text);
    return `<p class='PARA QUOTE' >${text}</p>`;
  }
  para_to_plaintext(ss, style) {
    if(style.nspace){
      var text = ss.join('\n');
      text = this.unmask(text,style);
      return `<blockquote class='PARA PLAINTEXT' >${text}</blockquote>`;
    }else{
      let text = ss.join('\n');
      text = this.unmask(text,style);
  ...
  itms_to_bull(itms,style){
    ...
    let pp = itms.map((p,i,arr) => {
      ...
      if(p.type=='UL'){
        if(ischeckbox){
          return `<input type='checkbox' class='ITEM'>${bullet}${nbsp}${text}</input>`
        }else{
      ...
      if(p.type=='OL'){
        if(ischeckbox){
          return `<input type='checkbox' class='ITEM'>${i+1}.${nbsp}${text}</input>`  
        }else{
      ...
      if(p.type == '0'){
        if(ischeckbox){
          return `<input type='checkbox' class='ITEM'>${value}${p.ending}${nbsp}${text}</input>`;
        }else{
      ...
      if(p.type == 'a'){
        if(ischeckbox){
          return `<input type='checkbox' class='ITEM'>${this.to_a_letter(value)}${p.ending}${nbsp}${text}</input>`;
        }else{
      ...
      if(p.type == 'DL'){
        if(ischeckbox){
          return `<input type='checkbox' class='ITEM'><b>${value}</b><br/>${text}</input>`;
        }else{
  ...
  itms_to_figure(itms,style) {
    ...
    itms.forEach( (p,i,arr) => {
      ...
      let opt = this.string_from_style(g);
      let s = this.phrase_to_img(`[${opt}]${src}`);
