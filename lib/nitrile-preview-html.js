...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  rows_to_tabular(rows,style){
    ...
    let vcols = this.string_to_array(style.vline||'');
    let border_top = '';
    let border_bot = '';
    let table_top = '';
    let table_bot = '';
    ...
    for (var j = 0; j < k; ++j) {
      ...
      pp = pp.map((x, i) => {
        const lr='1ex';
        const sp='2ex';    
        let border_left = (vcols.indexOf('*')>=0 || vcols.indexOf(`${i}`)>=0)?'1px solid':'';
        let border_right= (vcols.indexOf('*')>=0 || vcols.indexOf(`${i+1}`)>=0)?'1px solid':'';
        let pad=`0 ${lr}`;
