...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
    ...
    switch(block.floatname){
      case 'Figure': {
        ...
        if (1) {
          ...
          o.push(this.to_caption_text(style.floatname, style.chnum, style.idnum, style.caption));
          o.push(text);
          o.push('</figure>');
          text = o.join('\n');
        }
      ...
      case 'Equation': {
        ...
        text = `<div class='Equation' style='position:relative;'><div style='position:absolute;top:50%;transform:translate(0,-50%);'>${mynum}</div><div style='margin-left:${step}em;text-align:center;'>${text}</div></div>`;
        break;
      }
      case 'Note': {
      ...
      case 'Listing': {
        ...
        if(1){
          ...
          o.push(this.to_caption_text(block.style.floatname,block.style.chnum,block.style.idnum,block.style.caption));
          o.push(text);
          o.push('</figure>');
          text = o.join('\n');
        }
  ...
  phrase_to_ref (cnt){
    if(this.ref_map && this.ref_map.has(cnt)){      
      ...
      if(g.floatname){
        if(g.chnum){
          return `<a href='#${id}'>${g.floatname}&#160;${secsign}${g.chnum}.${g.idnum}</a>`;
        }else{
          return `<a href='#${id}'>${g.floatname}&#160;${secsign}${g.idnum}</a>`;
        }
      }else{
        if(g.chnum){
  ...
  to_caption_text(floatname,chnum,idnum,caption){
    if(chnum){
      return `<figcaption><strong>${floatname} ${chnum}.${idnum}</strong> &#160; ${this.unmask(caption)}</figcaption>`;  
    }else{
