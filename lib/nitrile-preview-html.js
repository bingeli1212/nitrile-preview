...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  phrase_to_frac(cnt){
    ...
    if(ss.length>1){
      ...
      top = this.smooth(top);
      bot = this.smooth(bot);
      return `<table border='0' style='vertical-align:middle;display:inline-block;border-collapse:collapse'><tr><td style='text-align:center;border-bottom:1px solid;padding:0;font-size:0.68em'>${top}</td></tr><tr><td style='text-align:center;border-top:1px solid;padding:0;font-size:0.68em'>${bot}</td></tr></table>`
    }else{
      cnt = this.smooth(cnt);
      return `<table border='0' style='vertical-align:middle;display:inline-block;border-collapse:collapse'><tr><td style='text-align:center;border-bottom:1px solid;padding:0;font-size:0.68em'>1</td></tr><tr><td style='text-align:center;border-top:1px solid;padding:0;font-size:0.68em'>${cnt}</td></tr></table>`
    }
  }
  phrase_to_sfrac(cnt){
    var re = /\s*(?:\/|$)\s*/;
  ...
  itms_to_lines(itms,style){
    ...
    for(let i = 0; i < k; ++i){
      ...
      rr = rr.map((s,j) => `<td style='vertical-align:top;width:${frs[j]*100}%'>${s}</td>`);
