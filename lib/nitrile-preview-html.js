...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  hdgs_to_part(title,label,partnum,chapternum,level,style){
    var cls = 'PART';
    return(`<h1 data-row='${style.row}' class='${cls}'> Part ${partnum} &#160; ${this.uncode(style,title)} </h1>`);
  }
  hdgs_to_chapter(title,label,partnum,chapternum,level,style){
    var cls = 'CHAPTER';
    this.addto_refmap(label,`${chapternum}`);
    return(`<h1 data-row='${style.row}' class='${cls}'> Chapter ${chapternum} &#160; ${this.uncode(style,title)} </h1>`);
  }
  ...
  item_ol_to_text(style,i,item,nblank){
    ...
    if(item.type == 'A'){
      ...
      o.push(`<li class='${li_cls}' type=''  data-row='${item.row}' value='${item.value}' style='--bull:"${value}.";--pad:-1.6em;' > ${text}`)
    }else if(item.type == 'a'){
      let value = i+1;
      value = this.int_to_letter_a(value);
      o.push(`<li class='${li_cls}' type=''  data-row='${item.row}' value='${item.value}' style='--bull:"${value}.";--pad:-1.6em;' > ${text}`)
      ...
      o.push(`<li class='${li_cls}'          data-row='${item.row}' value='${item.value}' style='--bull:"${value}.";--pad:-1.6em;' > ${text}`);
    }else if(item.bullet=='*'){
      let value = i+1;
      o.push(`<li class='${li_cls}'          data-row='${item.row}' value='${value}'      style='--bull:"${value}.";--pad:-1.6em;' > ${text}`);
    }else{
      let value = i+1;
      value = this.int_to_letter_i(value);
      o.push(`<li class='${li_cls}' type=''  data-row='${item.row}' value='${item.value}' style='--bull:"${value}.";--pad:-1.6em;' > ${text}`);
  ...
  item_ul_to_text(style,i,item,nblank){
    ...
    o.push(`<li class='${li_cls}' data-row='${item.row}' style='--bull:"&#x2022;";--pad:-1.6em;' > ${text}`);
    if(item.more && item.more.length){
      var style = {...style};
      item.more.forEach((p) => {
  ...
  item_hl_to_text(style,i,item,nblank){
    var o = [];
    var bullet = item.bullet;
    var text = this.untext(style,item.body);
    var li_cls = '';
    if(nblank) li_cls = ('PARA LI');
    else       li_cls = ('PACK LI');
    o.push(`<li class='${li_cls}' data-row='${item.row}' style='--bull:"&#x25B8;";--pad:-1.0em;' > ${text}`);
    if(item.more && item.more.length){
      var style = {...style};
      item.more.forEach((p) => {
        style.row = p.row;///renew style.row to be that of the more cluster
  ...
  itemize_to_text(style,itemize){
    ...
    switch (bull) {
      ...
      case 'OL': {
        ...
        o.push(`<ol class='${ol_cls}' style='padding-left:1.6em;list-style-type:none;'>`);
        items.forEach((item,j) => {
          style.row = item.row;
      ...
      case 'UL': {
        ...
        o.push(`<ul class='${ul_cls}' style='padding-left:1.6em;list-style-type:none;'>`);
        items.forEach((item,j) => {
          style.row = item.row;           
      ...
      case 'HL': {
        ...
        o.push(`<ul class='${ul_cls}' style='padding-left:1.0em;list-style-type:none;'>`);
        items.forEach((item,j) => {
          style.row = item.row;           
  ...
  float_to_paragraph(title,label,style,body){
    ...
    if(iscove){
      ...
      return `<div class='${cls}' data-row='${style.row}' > ${text} </div>`;  
    } else {
      var all = [];
      all.push('');
      if(title){
