...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  fence_to_displaymath(style,ss) {
    ...
    var s = `<span class='DISPLAYMATH' > ${s} </span>`;
    return s;
  }
  ...
  fence_to_img(style,ss) {
    ...
    var text = `<img class='IMG' style='${css}' src='${imgsrc}' id='${imgid}' alt='${imgsrc}'></img>`;
    return text;
  }
  ...
  fence_to_tabular(style,ss) {
    ...
    return `<table class='TABULAR' style='${css}'  > ${text} </table>`;
  }
    ...
    itms.forEach((p,i,arr) => {
      ...
      if(p.type=='UL'){
        ...
        if(p.bull.trim().length || this.is_in_list(p.start,check_ss)){
          //filled CB
          d.push(`<label  style='${css}' > <input type='checkbox' checked disabled        ></input> ${p.text}</label> `);
        }else{
          //empty CB
          d.push(`<label  style='${css}' > <input type='checkbox' disabled        ></input> ${p.text}</label> `);
        }
      }
      else if(p.type == 'RB'){
        if(p.bull.trim().length || this.is_in_list(p.start,check_ss)){
          //filled CB
          d.push(`<label  style='${css}' > <input type='radio' checked disabled        ></input> ${p.text}</label> `);
        }else{
          //empty CB
          d.push(`<label  style='${css}' > <input type='radio' disabled        ></input> ${p.text}</label> `);
        }
      }
      else {
  ...
  text_to_sample(style,body){
    if(style.type=='paragraph'){
      ...
      var text = `<span class='SAMPLE'> ${text} </span>`;
      return text;
  ...
  float_to_equation(title,label,style,ss){
    ...
    var text = `<figure class='PARA EQUATION'  > ${text} </figure>`;
    return text;
  }
  float_to_listing(title,label,style,ss){
    ...
    return `<figure class='PARA LISTING'  > ${this.to_caption_text(title,label,style,"listing")} ${text} </figure>`;
  }
  float_to_figure(title,label,style,ss){
    var itms = this.ss_to_figure_itms(ss,style);
    var all = [];
    if(style.wrap){
      var float = style.wrap=='left' ? 'left' : 'right';
      itms.forEach((p,j,arr) => {
        if(p.type=='fence'){
          p.img = this.do_fence(p.key,p.style,p.body);
          p.img = this.update_css_style(p.img,`float:${float}`);
          p.img = this.update_css_class(p.img,`WRAP`);
          all.push(p.img);
        }
      });
      var text = all.join('\n');
      ...
      var text = all.join('\n');
      return `<figure class='PARA FIGURE'  > ${this.to_caption_text(title,label,style,"figure")} ${text} </figure>`;
    }
  }
  float_to_table(title,label,style,ss){
    ...
    if(style.subfigure){
      ...
      var text = all.join('\n');
      return `<figure class='PARA TABLE'  > ${this.to_caption_text(title,label,style,"table")} ${text} </figure>`;
    }
    else{
      itms.forEach((p,j,arr) => {
        if(p.type=='fence'){
  ...
  float_to_longtable(title,label,style,ss){
    ...
    var text = `<figure class='PARA LONGTABLE'  > ${this.to_caption_text(title,label,style,"table")} ${text} </figure>`;
    return text;
  }
  float_to_tabbing(title,label,style,body){
    ...
    return `<div class='PARA TABBING' > ${text} </div>`
  }
  float_to_columns(title,label,style,ss){
    ...
    var text = o.join('\n');
    var text = `<figure class='PARA COLUMNS' > ${text} </figure>`;  
    return text;
  }
  float_to_framed(title,label,style,ss) {
    ...
    if(ENCODE){
      ///It is important to convert it to inline IMG because then the 'css_style' will take effect
      var text = `<img  alt='diagram' style='${css_style.join(';')}' src="data:image/svg+xml;charset=UTF-8,${encodeURIComponent(s)}"></img>`;
    }else{
    ...
    var text = `<figure class='PARA FRAMED'> ${text} </figure>`;
    return text;
  }
  float_to_blockquote(title,label,style,ss){
    ...
    if(1){
      all.push('');
      all.push(`<figure class='PARA BLOCKQUOTE'>`);
      all.push(`<blockquote>`)
      d.forEach((s) => {
