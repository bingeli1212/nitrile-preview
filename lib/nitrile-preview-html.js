...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  constructor(parser) {
    ...
    this.html_line_paddingleft = '1.5em';
  ...
  float_to_figure(title,label,idnum,idcap,style,ss,ssi){
    ...
    if(style.swrap){
      ...
      let all = [];
      let subrow_css = this.css('SUBGRID');
      let span_css = '';
      if(this.html_caption_align=='l'){                               
        subrow_css += ';justify-content:flex-start';
      ...
      itms.forEach((p,j,arr) => {
        if(p.type=='bundle'){
          let fig = p.img;
          let seq = p.seq;
          let sub = p.style.subtitle||'';
          onerow.push(fig);
          sub = `<span style='${span_css}'> ${this.uncode(style,sub)} </span>`;
          if(style.senum){
            let sidnum = this.int_to_letter_a(seq);
            sub = `(${sidnum}) ${sub}`;
          }
          onesub.push(sub);
        }else if(p.type=='\\\\'){
          let n = onerow.length;
          all.push(`<span class='SUBGRID' style='${subrow_css};grid-template-columns:repeat(${n},auto)' > ${onerow.join('')} ${onesub.join('')} </span>`);
          onerow = [];
          onesub = [];
        }
      });
      if(onerow.length && onesub.length){
        let n = onerow.length;
        all.push(`<span class='SUBGRID' style='${subrow_css};grid-template-columns:repeat(${n},auto)' > ${onerow.join('')} ${onesub.join('')} </span>`);
      }
      var text = all.join('\n');
