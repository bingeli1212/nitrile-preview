    ...
    itms.forEach((p,j,arr) => {
      if(p.type=='bundle'){
        if(p.img){
          ...
          let tab = `<table style='width:${w}'><tr><td>${img}</td></tr><caption class='FIGSUBCAPTION' style='${this.css("FIGSUBCAPTION")}'>${subtitle}</caption></table>`;
          onerow.push([w,tab]);
        }
      }
      if(p.type=='\\\\'||j+1==arr.length){
        let n = onerow.length;
        if(n){
          all.push(`<div class='FIGUREROW' style='${this.css("FIGUREROW")};grid-template-columns:${onerow.map(s=>s[0]).join(' ')}' > ${onerow.map(s=>s[1]).join('')} </div>`);
          onerow = [];
        }
      }
    });
