...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  do_HDGS(block) {
    ...
    if(hdgn==0) {
      o.push(`<h1 id='${id}' rows='${this.rows(block)}' >${idnum} ${title}</h1>`);
    } 
    else if(hdgn==1) {
      o.push(`<h2 id='${id}' rows='${this.rows(block)}' >${idnum} ${title}</h2>`);
    } 
    else if(hdgn==2) {
      o.push(`<h3 id='${id}' rows='${this.rows(block)}' >${idnum} ${title}</h3>`);
    } 
    else {
      o.push(`<h4 id='${id}' rows='${this.rows(block)}' >${idnum} ${title}</h4>`);
    }
    o.push('');
    block.html = o.join('\n');
  }
