...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  do_PRIM(block) {
    ...
    if(hdgn==0){
      var text = `<div class='P PRIM' > ${text} </div>`;
    }else{
      var text = `<div class='P SECO' > ${text} </div>`;
    }
    block.html = o.join('\n');
  }
  do_PARA(block){
  ...
  text_to_typeface(type,cnt,cnt2) {
    ...
    switch (type) {
      ...
      case 'frac': {
        ...
        css += `text-indent:0;`;
        css += `vertical-align:middle;`
        return `<table style='${css}'><tr><td style='text-align:center;border-bottom:1px solid;'>${cnt}</td></tr><tr><td style='text-align:center;border-top:1px solid;'>${cnt2}</td></tr></table>`;
        break;
      }
      case 'sup': {
        return `<sup>${cnt}</sup>`;
  ...
  rows_to_tabular(rows,style){
    ...
    for (var j = 0; j < k; ++j) {
      ...
      ///hline:+ option
      if(this.string_has_item(style.hrule,'-') && j > 0){
        border_top = '1px solid';
      }
      var mypp = [];
      pp.forEach((x, i) => {
        ...
        const lr='1ex';
        if(i>0 && vbars[i]==='||'){
          mypp.push(`<td style='border-left:1px solid;border-right:1px solid;border-top:${border_top};border-bottom:${border_bot};'></td>`);
        }
        ...
        if(style.head && j==0){
          ht = '';
          mypp.push(`<th style='${css}'> ${text} </th>`);        
        }else{
          mypp.push(`<td style='${css}'> ${text} </td>`);        
        }
      });
      var p = mypp.join('');
      var p = `<tr>${p}</tr>`;
