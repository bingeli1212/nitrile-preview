...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  float_to_figure(title,label,style,ss){
    ...
    if(style.wrap){
      ...
      return `<figure style='${figure_css}' > ${this.to_caption_text(title,label,style,"figure")} <div style='${combinationrow_css}'> ${text} </div> </figure>`;
    }
    else{
      itms.forEach((p,j,arr) => {
        if(p.type=='bundle'){
  ...
  to_subfigure_img(img,sub){
    ...
    <caption align='bottom'>${sub}</caption>
