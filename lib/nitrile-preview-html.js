...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  item_ul_to_text(style,i,item,nblank){
    ...
    o.push(`<li style='${li_css}'> <i>${key}</i> ${sep} ${text}`);
    if(item.more && item.more.length){
      item.more.forEach((p) => {
        if(p.lines){
          let div_css = this.css('PARA');
  ...
  fence_to_displaymath(style,ss) {
    ...
    if(style.float){
      ...
      var text = `<div style='${displaymath_css}; text-align:center;' > ${text} </div>`;
    }
    var o = [];
    o.push({img:text,sub:''});
    return o;
  ...
  fence_to_img(style,ss) {
    ...
    if(stretch){
      css += `; width:${100*stretch}%`;
      css += `; aspect-ratio:${viewport_width}/${viewport_height}`;
    } else if (width && height) {
      css += `; width:${width}mm`;
      css += `; height:${height}mm`;
    } else if (width) {
      css += `; width:${width}mm`;
      css += `; height:${width*(viewport_height/viewport_width)}mm`;
    } else if (height) {
      css += `; height:${height}mm`;
      css += `; width:${height*(viewport_width/viewport_height)}mm`;      
    } else {
      css += `; width:${viewport_width*viewport_unit}mm`;
      css += `; height:${viewport_height*viewport_unit}mm`;
    }
  ...
  fence_to_verbatim(style,ss){
    ...
    var ss = ss.map(x => `<tr><td>${x}</td></tr>`);
    var text = ss.join('\n');
    var css = this.css('VERBATIM');
    css += '; display:inline-table';//this is mandatory for it to observe the text-indent of the container
    css += this.style_to_css_border(style);
    var text = `<table style='${css};' width='100%' cellpadding='0' cellspacing='0' > ${text} </table>`;
    var o = [];
    o.push({img:text,sub:''});
    return o;
  }
  ...
  fence_to_tabular(style,ss) {
    ...
    css += `; display:inline-table`;//it has to be inline-block, and not inline-block, because otherwise the "frame=" and "hrules=" attribute which is targetted at a table won't work and we need these attributes to work for setting up table rules.
    if(style.small){
  ...
  float_to_paragraph(title,label,style,body){
    ...
    if(button_text){
      var text = `<button onclick='this.nextElementSibling.style.visibility="visible"'>${button_text}</button> <span style='visibility:hidden'>${text}</span>`;
    }else{
    ...
    if(style.wrap){
      itms.forEach((p,j,arr) => {
        if(p.type=='bundle'){
          let img = p.img;
          if(style.wrap=='left'){
            img = this.update_css_style(img,'float','left');
            img = this.update_css_style(img,'margin-right','1em');
          }else{
            img = this.update_css_style(img,'float','right');
            img = this.update_css_style(img,'margin-left','1em');
          }
          all.push(img);
        }
      });
      ...
      itms.forEach((p,j,arr) => {
        if(p.type=='bundle'){
          p.sub = `(${this.int_to_letter_a(p.seq)}) ${p.sub}`
      ...
      for(row of rows){
        if(row.length){
          let css = this.css('COMBINATION');
          all.push(`<div name='COMBINATION' style='${css}' >`);
      ...
      let combinationrow_css = this.css('COMBINATIONROW')
      return `<figure style='${figure_css}; font-size:smaller;' > ${this.to_caption_text(title,label,style,"figure")} <div name='COMBINATIONROW' style='${combinationrow_css}'> ${text} </div> </figure>`;
    }
    else{
      itms.forEach((p,j,arr) => {
        if(p.type=='bundle'){
  ...
  float_to_table(title,label,style,ss){
    ...
    if(style.wrap){
      ...
      let combinationrow_css = this.css('COMBINATIONROW')
      return `<figure style='${figure_css}'  > ${this.to_caption_text(title,label,style,"table")} <div style='${combinationrow_css}'> ${text} </div> </figure>`;
    }
    else{
      itms.forEach((p,j,arr) => {
        if(p.type=='bundle'){
  ...
  float_to_columns(title,label,style,ss){
    ...
    for(let j=0; j < total; j+=n){
      ...
      o.push(`<div style='${para_css}; columns:${n}'>`);
      for(let i=0; i < n; ++i){
  ...
  to_caption_text(title,label,style,key){
    ...
    if(none && !caption_text){
      ...
      let figcaption_css = this.css('FIGCAPTION')
      return `<figcaption style='${figcaption_css}' >${lead} </figcaption>`;
    }else{
  ...
  to_subfigure_img(img,sub){
    let subfigurecaption_css = this.css('SUBFIGCAPTION')
    var img = `<table style='display:inline-table'>
    <caption align='bottom' style='${subfigurecaption_css}'>${sub}</caption>
    <tr><td>${img}</td></tr></table>`
    return img;
  }
  __to_subfigure_img(img,sub){
    var s_width = this.extract_css_style(img,'width');
    img = this.update_css_style(img,'width','100%');
    var all = [];
    let subfigure_css = this.css('SUBFIGURE')
    all.push(`<span style='${subfigure_css}'>`);
    all.push(`${img}`);
    if(sub){
      let subfigurecaption_css = this.css('SUBFIGCAPTION')
      all.push(`<footer style='${subfigurecaption_css}'>${sub}</footer>`);
    }
    all.push(`</span>`);
    img = all.join(' ');
  ...
  to_css_map(){
    ...
    this.add_css_map_entry(
      map,
      'PARAGRAPH, PRIMARY, INDENT, ITEMIZE, EQUATION, TABBING, FIGURE, TABLE, LONGTABLE, LISTING, COLUMNS',[
        'text-indent: initial',
  ...
  to_script_canvas(){
    return this.canvas.to_script_canvas();
  }
}
