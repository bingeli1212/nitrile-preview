...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  float_to_equation(title,ss,g){
    ...
    var text = ss.join('\n');
    var {s:text} = this.tokenizer.to_svgmath(text,g,'math');
    var label = g.label||'';
    if(label.length){
  ...
  float_to_figure(title,ss,g){
    ...
    itms.forEach((p,j,arr) => {
      ...
      if(id=='diagram' || id=='framed' || id=='img'){
        seq++;
        p.sub = `(${this.to_a_letter(seq)}) ${this.uncode(p.sub)}`;
        p.img = this.s_to_phrase(id,cnt,g);
        all.push(this.to_subfigure_html(p.img,p.sub,g));
      }else{
        all.push(`<br/>`);
      }
    });
    var text = all.join('\n');
    var figcss = this.to_figure_css(g);
    return `<figure style='${figcss}'  > ${this.to_caption_text(title,g,"figure")} ${text} </figure>`;
  }
  ...
  to_subfigure_html(img,sub,g){
    return `<table border='0' style='display:inline-table'><caption align="bottom"><small>${sub}</small></caption><tr><td>${img}</td></tr></table>`;
  }
