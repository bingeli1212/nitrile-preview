...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  fence_to_canvas(style,ss,ssi){
    ...
    var imgwidget = `<img style='position:absolute;' width='${p.width}' height='${p.height}' src='${imgsrc}' id='${imgid}' alt='${imgsrc}' />`;
    var canvaswidget = `<canvas style='position:relative;z-index:20;' width='${p.width}' height='${p.height}' onclick='canvas_onclick(this)' onfocusin='canvas_onfocusin(this)' onfocusout='canvas_onfocusout(this)' data-imgid='${imgid}' id='${imgid}-canvas' tabIndex='1' > </canvas>`;
    ...
    var frame = (style.frame)?`frame='box'`:``;
    var s_style = this.css('SUBTITLE');
    var text = `<form action='/post' method='post' onsubmit='canvas_onsubmit(this)' style='position:relative;' data-imgsrc='${imgsrc}' data-imgid='${imgid}' data-canvasid='${imgid}-canvas' data-paletteid='${imgid}-palette' > ${imgwidget} ${canvaswidget} ${palettewidget} </form>`
