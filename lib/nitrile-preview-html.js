...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  do_PLST(block) {
    ...
    for (var item of items) {
      var {bull,bullet,value,text,dl,ds,row1,row2,more} = item;
      text = text || '';
      let extra_text = '';
      if(more && more.length){
        more.forEach((plitem) => {
          let {lines,row1,row2} = plitem;
          extra_text += `<div class='PLST' rows='${this.rows(row1,row2)}' >${this.untext(lines)}</div>`;
        });
      }
      switch (bull) {
        case 'OL': {
        ...
        case 'LI': {
          if(dl){
            ...
            dt = this.polish(dt);
            dd = this.unmask(dd);
            o.push(`<dt class='PLST' rows='${this.rows(row1,row2)}' >${dt}</dt><dd class='PLST'>${dd}${extra_text}</dd>`);
          }
          else if(ds){
            ...
            o.push(`<li class='PLST' rows='${this.rows(row1,row2)}' >${text}${extra_text}</li>`);
          }else{
            text = this.unmask(text);
            o.push(`<li class='PLST' value='${value}' rows='${this.rows(row1,row2)}' >${text}${extra_text}</li>`);
          }
          break;
        }
        case '/OL': {
  ...
  do_PRIM(block) {
    ...
    o.push(`<div class='PRIM' rows='${this.rows(row1,row2)}' >${text}</div>`);
    o.push('');
    block.html = o.join('\n');
  }
  do_TEXT(block) {
    var {body,nspace,style} = block;
    var o = []; 
    let text = this.untext(body,style);
    let {row1,row2} = style;
    if(nspace){
      o.push(`<div class='TEXT NSPACE' rows='${this.rows(row1,row2)}' >${text}</div>`);
    }else{
      o.push(`<div class='TEXT' rows='${this.rows(row1,row2)}' >${text}</div>`);
    }
    o.push('');
    block.html = o.join('\n');
  }
  do_FLOA(block) {
    ...
    var text;
    switch(block.style.floatname){
      case 'Figure': {
        text = this.untext(block.body,block.style);
        if (1) {
          let style = block.style;
          let o = [];
          o.push(`<figure class='Figure' rows='${this.rows(row1,row2)}' >`);
          o.push(this.to_caption_text(style));
          o.push(text);
          o.push('</figure>');
          text = o.join('\n');
        }
      ...
      case 'Tabulate': {
        ...
        if (1) {
          let style = block.style;
          let o = [];
          o.push(`<figure class='Tabulate' rows='${this.rows(row1,row2)}' >`);
          o.push(this.to_caption_text(style));
          o.push(text);
          o.push('</figure>');
          text = o.join('\n');
      ...
      case 'Longtable': {
        ...
        if (1) {
          let style = block.style;
          let o = [];
          o.push(`<figure class='Longtable' rows='${this.rows(row1,row2)}' >`);
          o.push(this.to_caption_text(style));
          o.push(text);
          o.push('</figure>');
          text = o.join('\n');
      ...
      case 'Equation': {
        let clusters = this.ss_to_clusters(block.body);
        let d = [];
        clusters.forEach(ss => {
          let text = this.untext(ss,block.style);
          text = `<div>${text}</div>`
          d.push(text);
        })
        text = d.join('');
        ...
        if(1){
          let o = [];
          o.push(`<figure class='Equation' rows='${this.rows(row1,row2)}' >`);
          o.push(this.to_caption_text(style));
          o.push(text);
          o.push(`</figure>`);
          text = o.join('\n');
          //o.push(`<span style='position:absolute;top:50%;transform:translate(0,-50%);'>${idnum}</span>`);
        }
        break;
      }
      case 'Listing': {
        ...
        if(1){
          let style = block.style;
          let o = [];
          o.push(`<figure class='Listing' rows='${this.rows(row1,row2)}' >`);
          o.push(this.to_caption_text(style));
          o.push(text);
          o.push('</figure>');
          text = o.join('\n');
      ...
      case 'Note': {
        block.notes.set(block.style.label,block.body);
        break;
      }
      case 'Vocabulary': {
        this.addto_vmap(block.body,block.style,block.vmap);
        break;
      }
    }
    if(text){
  ...
  phrase_to_ref (label){
    ...
    if(g){
      ...
      let id = g.id||'';
      let saveas = g.saveas||'';
      if(g.idnum && g.floatname){
        if(g.chnum){
          return `<a href='${saveas}#${id}'>${g.title}&#160;${secsign}${g.chnum}.${g.idnum}</a>`;
        }else{
          return `<a href='${saveas}#${id}'>${g.title}&#160;${secsign}${g.idnum}</a>`;
        }
      }
      if(g.idnum){
        if(g.chnum){
          return `<a href='${saveas}#${id}'>Section&#160;${secsign}${g.chnum}.${g.idnum}</a>`;  
        }else{
          return `<a href='${saveas}#${id}'>Section&#160;${secsign}${g.idnum}</a>`;
        }
      }
    }
  ...
  phrase_to_img(g) {
    if(g.src){
      var src = g.src;
      var width = this.string_to_css_length(g.width);
      var height = this.string_to_css_length(g.height);
      this.imgs.push(src);
      ...
      if(g.zoom){
        css_style.push(`zoom:${g.zoom*100}%`)
      }
      if(g.frame==1){
  ...
  phrase_to_frac(cnt){
    ...
    if(ss.length>1){
      ...
      top = this.smooth(top);
      bot = this.smooth(bot);
      return `<table border='0' style='vertical-align:middle;display:inline-block;border-collapse:collapse'><tr><td style='text-align:center;border-bottom:1px solid;padding:0;font-size:smaller'>${top}</td></tr><tr><td style='text-align:center;border-top:1px solid;padding:0;font-size:smaller'>${bot}</td></tr></table>`
    }else{
      cnt = this.smooth(cnt);
      return `<table border='0' style='vertical-align:middle;display:inline-block;border-collapse:collapse'><tr><td style='text-align:center;border-bottom:1px solid;padding:0;font-size:smaller'>1</td></tr><tr><td style='text-align:center;border-top:1px solid;padding:0;font-size:smaller'>${cnt}</td></tr></table>`
    }
  }
  phrase_to_sfrac(cnt){
    var re = /\s*(?:\/|$)\s*/;
  ...
  svgarray_to_svg(d,style){

    const vspace = parseInt(style.vspace)||6;
  ...
  make_line_svg(math,style){
    ...
    d = d.filter(x => (x.w > 0));
    var {s,w,h,mid,shiftdist,defs} = this.svgarray_to_svg(d,style);
  ...
  rows_to_tabulate(rows,style,islongtable){
    ...
    if(border==1){
      ...
      for (var j = 0; j < nrows; ++j) {
        ...
        pp = pp.map((x, i) => {
          if(i==0){
            return `<td style='width:${frs[i]};padding:0 ${lr};border-left:1px solid;border-top:${border_top};border-bottom:${border_bot}'>${x}</td>`;
          }else if(i==pp.length-1){
            return `<td style='width:${frs[i]};padding:0 ${lr};border-right:1px solid;border-top:${border_top};border-bottom:${border_bot}'>${x}</td>`;
          }else{
            return `<td style='width:${frs[i]};padding:0 ${lr};border-top:${border_top};border-bottom:${border_bot}'>${x}</td>`;
          }
        });
        var p = pp.join('');
        var p = `<tr>${p}</tr>`;
      ...
      for (var j = 0; j < nrows; ++j) {
        ...
        let gap = `<td style='padding:0 ${sp};border-top:${border_top};border-bottom:${border_bot};'></td>`;
        var p = pp.join(gap);
        var p = `<tr>${p}</tr>`;
        d.push(p);
      }
      ...
      for (var j = 0; j < nrows; ++j) {
        ...
        let gap = `<td style='padding:0 ${sp};border-top:${border_top};'></td>`;
        var p = pp.join(gap);
        var p = `<tr>${p}</tr>`;
        border_top = '';
        d.push(p);
  ...
  itms_to_lines(itms,style){
    ...
    for(let i = 0; i < k; ++i){
      ...
      rr = rr.map(s => `<td style='vertical-align:top;width:${frs[i]*100}%'>${s}</td>`);
      let text = rr.join(`<td style='width:2%'></td>`);
      text = '<tr>' + text + '</tr>';
      d.push(text);
    }
    d.push(`</table>`);
  ...
  pp_to_multi_itemized(itms,style,name){
    ...
    for (let j = 0,i=0; j < itms.length; i+=1,j+=k) {
      ...
      let fr = frs[i];
      d.push(`<td style='vertical-align:top;width:${fr*w*100}%'><${name} class='TEXT' >\n${pp.join('\n')}\n</${name}></td>`);
      if (i < n - 1) {
        d.push(`<td style='width:${gap * 100}%'></td>`);
      }
    }
  ...
  imgrid_to_htmltable(itms,style) {
    ...
    itms.forEach( p => {
      ...
      let img = this.phrase_to_img(g);
      let txt = `<span style='display:inline-block;text-align:center;vertical-align:top;width:${wd}%;'>${sub}</span>`;
      p.img = img;
      p.txt = txt;
    });
    while(itms.length){
