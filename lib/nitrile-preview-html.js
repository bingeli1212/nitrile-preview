...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  hdgs_to_part(title,label,level,style){
    ...
    return(`<h1 id='${label}' data-row='${style.row}' class='${cls}' style='${css}' > Part ${style.partnum} &#160; ${this.uncode(style,title)} </h1>`);
  }
  /////////////////////////////////////////////////////////////////////
  //
  // translator function
  ...
  hdgs_to_section(title,label,level,style){
    ...
    const css = this.css(cls);
    if(style.chapnum){
      var level = ""+style.chapnum+"."+style.level;
    }else{
      var level = style.level;
    }
    return(`<h2 id='${label}' data-row='${style.row}' class='${cls}' style='${css}' > ${level} <span title='${style.note}:${1+style.row}'>${this.uncode(style,title)}</span> </h2>`);
  }
  /////////////////////////////////////////////////////////////////////
  //
  // translator function
  ...
  hdgs_to_subsection(title,label,level,style){
    ...
    return(`<h3 id='${label}' data-row='${style.row}' class='${cls}' style='${css}' > ${level} <span title='${style.note}:${1+style.row}'>${this.uncode(style,title)}</span> </h3>`);
  }
  /////////////////////////////////////////////////////////////////////
  //
  // translator function
  ...
  hdgs_to_subsubsection(title,label,level,style){
    ...
    return(`<h4 id='${label}' data-row='${style.row}' class='${cls}' style='${css}' > ${level} <span title='${style.note}:${1+style.row}'>${this.uncode(style,title)}</span> </h4>`);
  }
  /////////////////////////////////////////////////////////////////////
  /////////////////////////////////////////////////////////////////////
  ...
  fence_to_ink(style,ss,ssi){
    ...
    var subtitle = style.subtitle||'';
    p.style = style;
    var text = `\n<svg data-bundle="INK" data-label="${style.label}" width="${this.fix(p.width*this.MM_TO_PX)}" height="${this.fix(p.height*this.MM_TO_PX)}" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="currentColor" stroke="none" viewBox="0 0 ${this.fix(p.vw)} ${this.fix(p.vh)}" >${text}</svg>\n`;
  ...
  fence_to_img(style,ss,ssi){
    ...
    if(itms.header){
      header = itms.header.join(' ');
    }
    if(itms.footer){
      footer = itms.footer.join(' ');
    }
    if(itms.subtitle){
      subtitle = itms.subtitle.join(' ');
    }
    var p = {};
    var [viewport_width,viewport_height,viewport_unit] = this.g_to_viewport_array(style);
    ...
    if(image.length){
      var imgsrc = this.choose_image_file(image);
      var { imgsrc, imgid } = this.to_request_image(imgsrc);
    }else{
      let s_svg = `<svg width="${this.fix(p.vw)}" height="${this.fix(p.vh)}" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="currentColor" stroke="currentColor" viewBox="0 0 ${this.fix(p.vw)} ${this.fix(p.vh)}" ></svg>`;
      var { imgid } = this.to_request_svgid(s_svg);
      var imgsrc = `data:image/svg+xml;charset=UTF-8,${encodeURIComponent(s_svg)}`;
    }
    if(type=='canvas'){
      var imgwidget = `<img style='position:relative;visibility:hidden;width:${p.width}mm;height:${p.height}mm;' width='${p.vw}' height='${p.vh}' src='${imgsrc}' data-canvasid='${imgid}-canvas' id='${imgid}' alt='${imgsrc}' onload="img_onload(this)"/>`;
      var canvaswidget = `<canvas style='position:absolute;z-index:1;width:${p.width}mm;height:${p.height}mm;left:0;right:0;top:0;bottom:0;' width='${p.vw}' height='${p.vh}' onmouseenter="canvas_onmouseenter(this)" onmouseleave="canvas_onmouseleave(this)" onmousedown="canvas_onmousedown(this)" onmousemove="canvas_onmousemove(this)" onmouseup="canvas_onmouseup(this)" ontouchstart="canvas_ontouchstart(this)" ontouchmove="canvas_ontouchmove(this)" ontouchend="canvas_ontouchend(this)" ontouchcancel="canvas_ontouchcancel(this)" onfocusin="canvas_onfocusin(this)" onfocusout="canvas_onfocusout(this)" onkeydown="canvas_onkeydown(this)" data-imgid='${imgid}' data-paletteid='${imgid}-palette' data-colorinputid='${imgid}-colorinput' id='${imgid}-canvas' data-type='${type}' data-unit='${unit}' tabIndex='1' > </canvas>`;
      var datalist = `<datalist id='${imgid}-datalist'> ${itms.color?(itms.color.map(s=>"<option>"+s+"</option>").join("")):''} </datalist>`;
      ...
      var text = `<form class='CANVASFORM' action='/action.php' method='post' onreset='form_onreset(this)' onsubmit='form_onsubmit(this)' style='position:relative;' data-imgsrc='${imgsrc}' data-imgid='${imgid}' data-paletteid='${imgid}-palette' data-canvasid='${imgid}-canvas' > ${imgwidget} ${canvaswidget} ${palettewidget} </form>`
    }else{
      var imgwidget = `<img style='position:relative;visibility:visible;width:${p.width}mm;height:${p.height}mm;' width='${p.vw}' height='${p.vh}' src='${imgsrc}' data-canvasid='${imgid}-canvas' id='${imgid}' alt='${imgsrc}' />`;
      //var headerwidget = `<div style='position:absolute;z-index:1;width:${p.width}mm;height:${p.height}mm;display:flex;flex-direction:column;justify-content:space-between;align-items:center;left:0;right:0;top:0;bottom:0;' width='${p.vw}' height='${p.vh}' > <span>${this.uncode(style,header)}</span> <span>${this.uncode(style,footer)}</span> </div>`;
      var headerwidget = '';
      //var text = `<form class='CANVASFORM' action='' method='' onreset='' onsubmit='' style='position:relative;' data-imgsrc='${imgsrc}' data-imgid='${imgid}' data-paletteid='${imgid}-palette' data-canvasid='${imgid}-canvas' > ${imgwidget} ${headerwidget} </form>`
      var text = imgwidget;
    }
    p.style = style;
    var frame = (style.frame)?`frame='box'`:``;
    var s_style = this.css('SUBTITLE');
    p.img = `\n<table class='PARAGRAPH IMG' style='${this.css("PARAGRAPH IMG")}' ${frame} cellpadding='0' cellspacing='0' data-row='${style.row}' >\n                                                                                                     \n<colgroup/><thead/><tbody><tr><td>${text}</td></tr></tbody>\n</table>`;
    p.fig = `\n<table class='          IMG' style='${this.css("          IMG")}' ${frame} cellpadding='0' cellspacing='0' data-row='${style.row}' >\n<caption align='bottom' class='SUBTITLE' style='${s_style}' >${this.uncode(style,subtitle)}</caption>\n<colgroup/><thead/><tbody><tr><td>${text}</td></tr></tbody>\n</table>`;
    var o = [];
    o.push(p);
    return o;
  }
  ...
  fence_to_dia(style,ss,ssi){
    ...
    o = o.map((p) => {
      ...
      var { imgid } = this.to_request_svgid(s_svg);
      if(type=='canvas'){
        var imgwidget = `<img style="position:relative;visibility:hidden;width:${p.width}mm;height:${p.height}mm;" width="${p.vw}" height="${p.vh}" src="data:image/svg+xml;charset=UTF-8,${encodeURIComponent(s_svg)}" data-canvasid="${imgid}-canvas" id="${imgid}" onload="img_onload(this)"/>`;
        var canvaswidget = `<canvas style='position:absolute;z-index:1;width:${p.width}mm;height:${p.height}mm;left:0;right:0;top:0;bottom:0;' width='${p.vw}' height='${p.vh}' onmouseenter="canvas_onmouseenter(this)" onmouseleave="canvas_onmouseleave(this)" onmousedown="canvas_onmousedown(this)" onmousemove="canvas_onmousemove(this)" onmouseup="canvas_onmouseup(this)" ontouchstart="canvas_ontouchstart(this)" ontouchmove="canvas_ontouchmove(this)" ontouchend="canvas_ontouchend(this)" ontouchcancel="canvas_ontouchcancel(this)" onfocusin="canvas_onfocusin(this)" onfocusout="canvas_onfocusout(this)" onkeydown="canvas_onkeydown(this)" data-imgid='${imgid}' data-paletteid='${imgid}-palette' data-colorinputid='${imgid}-colorinput' id='${imgid}-canvas' data-type='${type}' data-unit='${unit}' tabIndex='1' > </canvas>`;
        ...
        var headerwidget = `<div style='position:absolute;z-index:1;width:${p.width}mm;height:${p.height}mm;display:flex;flex-direction:column;justify-content:space-between;align-items:center;left:0;right:0;top:0;bottom:0;' width='${p.vw}' height='${p.vh}' > <span>${this.uncode(style,header)}</span> <span>${this.uncode(style,footer)}</span> </div>`;
        var text = `<form class='CANVASFORM' action='' method='' onreset='' onsubmit='' style='position:relative;' data-imgsrc='' data-imgid='${imgid}' data-paletteid='${imgid}-palette' data-canvasid='${imgid}-canvas' > ${imgwidget} ${headerwidget} </form>`
      }
      var frame = (style.frame)?`frame="box"`:``;
      var s_style = this.css("SUBTITLE");
  ...
  fence_to_tab(style,ss,ssi) {
    ...
    p.img = `<table class='PARAGRAPH TAB' style='${this.css("PARAGRAPH TAB")};${fontsize_css}' ${frame} ${rules} cellpadding='0' cellspacing='0' data-row='${style.row}' >                                                                                                         \n${text}\n</table>`;
    p.fig = `<table class='          TAB' style='${this.css("          TAB")};${fontsize_css}' ${frame} ${rules} cellpadding='0' cellspacing='0' data-row='${style.row}' >\n<caption align='bottom' class='SUBTITLE' style='${s_style}' > ${this.uncode(style,subtitle)} </caption>\n${text}\n</table>`;
    var o = [];
    o.push(p);
    return o;
  }
  ...
  phrase_to_img(style,cnt){
    ...
    if(style['$'].buffers.hasOwnProperty(cnt)){
      ...
      if(itms.image){
        ...
        let css = '';
        if(g.frame){
          css += ';outline:thin solid currentColor';
        }
        s_img = `<img style='${css};width:${p.width}mm;height:${p.height}mm;' width='${p.width*this.MM_TO_PT}' height='${p.height*this.MM_TO_PT}' src="${imgsrc}"/>`;
      }
    }
    return s_img;
  }
  ...
  phrase_to_dia(style,cnt){
    ...
    if(style['$'].buffers.hasOwnProperty(cnt)){
      ...
      if(o.length){
        let p = o[0];
        let width = this.g_to_width_float(g);
        let height = this.g_to_height_float(g);
        let css = '';
        if(g.frame){
          css += ';outline:thin solid currentColor';
        }
        if(width && height){
          s_svg = `<svg style='${css}' width='${width*this.MM_TO_PT}' height='${height*this.MM_TO_PT}' xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="currentColor" stroke="currentColor" viewBox="0 0 ${this.fix(p.vw)} ${this.fix(p.vh)}" >${p.svgtext}</svg>`;
        }else if(width){
          s_svg = `<svg style='${css}' width='${width*this.MM_TO_PT}' xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="currentColor" stroke="currentColor" viewBox="0 0 ${this.fix(p.vw)} ${this.fix(p.vh)}" >${p.svgtext}</svg>`;
        }else if(g.height){
          s_svg = `<svg style='${css}' height='${height*this.MM_TO_PT}' xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="currentColor" stroke="currentColor" viewBox="0 0 ${this.fix(p.vw)} ${this.fix(p.vh)}" >${p.svgtext}</svg>`;
        }else{
          s_svg = `<svg style='${css}' width='${p.vw}' xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="currentColor" stroke="currentColor" viewBox="0 0 ${this.fix(p.vw)} ${this.fix(p.vh)}" >${p.svgtext}</svg>`;
        }
      }
    }
    return s_svg;
  ...
  float_to_equation(title,label,style,bundles){
    ...
    bundles.forEach((bundle,i,arr) => {
      ...
      if(o.length){
        ...
        all.push(`<span class='EQUATIONROW' style='${this.css("EQUATIONROW")}'> <span style='margin-left:auto;margin-right:auto'>${p.text}</span> <span style='font-size:smaller;'> (${eqnnum}${sub}) </span> </span>`);
      }
    })
    all.push(`</figure>`);
    return all.join('\n');
  ...
  float_to_listing(title,label,style,bundles){
    ...
    if(bundles.length){
      let o = [];
      o = this.do_bundle(style,bundles[0],'vtm');
      ss = o[0].ss;
