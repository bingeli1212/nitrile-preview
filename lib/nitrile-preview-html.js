...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  fence_to_ink(style,ss){
    ...
    var max_vh = vgap + vgap + npara*fs*this.PT_TO_MM;//fs font
    ...
    all.push( `<text style="font-family:monospace;white-space:pre;font-size:${fontsize}pt;" text-anchor="start" x="0" y="0" >` );
    for (var i=0; i < npara; ++i) {
      let s = ss[i];
      s = this.polish(style,s);
      s = this.replace_blanks_with(s,'&#160;');
      var x = 0;
      all.push( `<tspan y="${vgap + (i+extra_dy)*fontsize*this.PT_TO_MM}mm" x="0">${s}</tspan>` );
    }
    all.push( `</text>`);
    var img = all.join('\n');    
  ...
  float_to_figure(title,label,style,ss){
    ...
    if(style.wrap){
      ...
      itms.forEach((p,j,arr) => {
        let img = this.to_subfigure_img(p.img,p.subc,p.w,j,style.subfigure);
        all.push(img);
      });
      var text = all.join('\n');
  ...
  to_subfigure_img(img,sub,w,seq,subfigure){
    ...
    let o = [];
    if(w){
      o.push(`<table cellpadding='0' cellspacing='0' width='${w}' style='display:inline-table;'>`);
    }else{
      o.push(`<table cellpadding='0' cellspacing='0' style='display:inline-table;'>`);
    }
    if(subfigure){
      sub = `(${this.int_to_letter_a(seq+1)}) ${sub}`;
    }
    if(sub){
