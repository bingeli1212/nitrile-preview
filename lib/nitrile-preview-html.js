...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  hdgs_to_chapter(title,label,style){
    return(`<h1 id='${label}' class='CHAPTER' style='${this.css("CHAPTER")}' data-partnum='${style.__partnum}' data-chapnum='${style.__chapnum}' > ${this.polish(style,title)} </h1>`);
  }
  hdgs_to_section(title,label,level,style){
    return(`<h2 id='${label}' class='SECTION' style='${this.css("SECTION")}' data-partnum='${style.__partnum}' data-chapnum='${style.__chapnum}' data-level='${level}' > ${this.polish(style,title)} </h2>`);
  }
  hdgs_to_subsection(title,label,level,style){
    return(`<h3 id='${label}' class='SUBSECTION' style='${this.css("SUBSECTION")}' data-partnum='${style.__partnum}' data-chapnum='${style.__chapnum}' data-level='${level}' > ${this.polish(style,title)} </h3>`);
  }
  hdgs_to_subsubsection(title,label,level,style){
    return(`<h4 id='${label}' class='SUBSUBSECTION' style='${this.css("SUBSUBSECTION")} data-partnum='${style.__partnum}' data-chapnum='${style.__chapnum}' data-level='${level}' > ${leader} ${this.polish(style,title)} </h4>`);
  }
  //////////////////////////////////////////////////////////////////////////
  //
  //////////////////////////////////////////////////////////////////////////
    ...
    itms.forEach((p,j,arr) => {
      if(p.type=='bundle'){
        if(p.fig){
          ...
          let tab = `<figure class='SUBFIGURE' style='${this.css("SUBFIGURE")};width:${w};'>${img}<figcaption class='SUBCAPTION' style='${this.css("SUBCAPTION")}'>${subtitle}</figcaption></figure>`;
          let sub = `<span class='SUBCAPTION' style='${this.css("SUBCAPTION")}'>${subtitle}</span>`;
          onerow.push([w,p.img,sub,tab]);
        }
      }
      if(p.type=='\\\\'||j+1==arr.length){
        ...
        if(n){
          ...
          all.push(`<div style='display:grid;grid-template-columns:${onerow.map(p=>p[0]).join(" ")};' > ${onerow.map(s=>s[1]).join('')} ${onerow.map(s=>s[2]).join('')} </div>`);
          onerow = [];
        }
      }
    });
  ...
  float_to_example(title,label,style,splitid,body,bodyrow){
    let ss = body.map((s) => this.uncode(style,s)).map((s) => `<dt>${this.icon_dag}&#160;${s}</dt>`);
    var text = ss.join('\n');
  ...
  float_to_enumeration(title,label,style,splitid,body,bodyrow){
    let ss = body.map((s)=>`<li value='${this.get_next_enum(style)}'>${this.uncode(style,s)}</li>`);
    var text = ss.join('\n');
    return `<ol class='ENUMERATION' style='${this.css("ENUMERATION")};' data-row='${style.row}' >${text}</ol>`;
  }
