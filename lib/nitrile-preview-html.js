...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  rows_to_tabular(rows,style){
    ...
    var table_top = hrows.indexOf('t');
    var table_bot = hrows.indexOf('b');
    let k = rows.length;
    ...
    for (var j = 0; j < k; ++j) {
      ...
      pp.forEach((x, i) => {
        ...
        mypp.push(`<td style='width:${frs[i]};padding:${pad};border-top:${border_top};border-bottom:${border_bot};border-left:${border_left};border-right:${border_right};background-color:${color};height:${height}'>${text}</td>`);        
      });
      var p = mypp.join('');
      var p = `<tr>${p}</tr>`;
      border_top = '';
