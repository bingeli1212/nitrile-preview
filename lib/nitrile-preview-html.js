...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  do_HDGS(block) {
    let {hdgn,title,style,label} = block;
    var o = [];
  ...
  do_FLOA(block) {
    ...
    switch(block.style.floatname){
      ...
      case 'Equation': {
        ...
        if(1){
          let o = [];
          o.push(`<figure class='Equation' style='position:relative;'>`);
          o.push(`<div style='position:absolute;top:50%;transform:translate(0,-50%);'>${idnum}</div>`);
