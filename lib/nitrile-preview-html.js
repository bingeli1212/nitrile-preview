...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
    ...
    rows.forEach((pp,j) => {
      pp.forEach((p,i) => {
        if(style.head && j==0){
          ...
          p.raw = this.to_formatted_text(p.raw,fm[i]);
        }
      })
    })
    rows.forEach((pp,j) => {
  ...
  float_to_center(title,label,style,splitid,body,bodyrow){
    ...
    var text = all.join('&#160;\n');
    return `<figure class='CENTER' style='${this.css("CENTER")};display:flex;flex-direction:row;justify-content:center;align-items:flex-end;' data-row='${style.row}' >${text}</figure>`;
  }
