...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  fence_to_tabular(style,ss) {
    ...
    var text = this.rows_to_tabular(rows,style,n,ww);
    //
    //put it together
    //
    var css = '';
    ...
    var cols = ww.map((x,i,arr) => `<col width='${x.pw}'/>`).join('\n');
    var cols = `<colgroup> ${cols} </colgroup>`;
    var text = `<table class='TABULAR' ${attrs.join(' ')} style='${css}'  > ${cols} ${text} </table>`;
    var o = [];
    o.push({img:text,sub:''});
    return o;
  }
  ...
  ///
  //////////////////////////////////////////////////////////////////////////////////////
  body_to_list(style,body){
    let o = this.fence_to_list(style,body);
    let all = [];
  ...
  rows_to_tabular(rows,style,n,ww){
    // var n = rows.length ? rows[0].length : 1;
    ...
    */
    ...
    rows.forEach((pp,j) => {
      let border_top = '';
      if (pp.upper==2) {
        d.push(`<tr><td style='border-top:1px solid;border-bottom:1px solid;' colspan='${n}' height='0'></td></tr>`);
      } else if (pp.upper==1){
      ...
      pp.forEach((x,i) => {
        let {raw,text} = x;
        // if(i>0 && vbars[i]==='||'){
        //   pp1.push(`<td style='border-left:1px solid;border-right:1px solid;border-top:${border_top};border-bottom:${border_bot};padding:0.75pt;'></td>`);
        // }
