...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  fence_to_tab(style,ss,ssi) {
    ...
    var stretch = this.assert_float(style.stretch,0,0,1);
    var width = 0;
    var {rows} = this.ss_to_tab_rows(style,ss,ssi);
    rows.forEach((pp,j) => {
      pp.width = 0;
      pp.forEach((p,i) => {
  ...
  float_to_flushleft(title,label,style,splitid,body,bodyrow){
    ...
    var text = all.join('\n');
    return `<div class='FLUSHLEFT' style='${this.css("FLUSHLEFT")};display:flex;flex-direction:column;align-items:flex-start;' data-row='${style.row}' >${text}</div>`;
  }
  ...
  float_to_tabulate(title,label,style,splitid,body,bodyrow){
    var bundles = this.body_to_all_bundles(style,body,bodyrow);
    var all = [];
    var maxn = 1;
    var ww = [];
    if(bundles.length) {
      ...
      ss =  ss.map((s,j,arr) => {
        let xx = s.split(/\s{3,}/);
        maxn = xx.length;
        let isdhline = this.re_is_dhline.test(xx[0]);
        if(j==0 && isdhline){
        ...
        xx = xx.map((x,i) => `<td style='width:${ww[i]?''+ww[i]+'mm':''};padding:0 2mm;'>${x}</td>`);
        s = xx.join('');
        s = `<tr>${s}</tr>`;
        return s;
      });
      all.push(`<div class='TABULATE' style='${this.css("TABULATE")};display:flex;flex-direction:column;align-items:flex-start;' data-row='${style.row}' >`);
      all.push(`<table>`);
      ss.forEach(s => {
        if(s){
          all.push(s);
