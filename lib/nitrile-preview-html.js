...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  fence_to_canvas(style,ss,ssi){
    ...
    var frame = (style.frame)?`frame='box'`:``;
    var s_style = this.css('SUBTITLE');
    var text = `<form class='CANVASFORM' action='/post' method='post' onsubmit='canvas_onsubmit(this)' style='position:relative;' data-imgsrc='${imgsrc}' data-imgid='${imgid}' data-canvasid='${imgid}-canvas' > ${imgwidget} ${canvaswidget} ${palettewidget} </form>`
