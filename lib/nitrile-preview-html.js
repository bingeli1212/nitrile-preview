    ...
    if(style.wrap){
      itms.forEach((p,j,arr) => {
        if(p.type=='bundle'){
          let img = p.img;
          if(style.wrap=='left'){
            img = `<figure class='FIGURE WRAPLEFT'>${this.to_caption_text(title,label,style,"figure")} ${p.subfigure}</figure>`;
          }else{
            img = `<figure class='FIGURE WRAPRIGHT'>${this.to_caption_text(title,label,style,"figure")} ${p.subfigure}</figure>`;
          }
          all.push(img);
        }
      });
