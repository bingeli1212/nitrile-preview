...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  do_PART(block) {
    ...
    var { title, style, switches } = block;
    let idnum=this.get_refmap_value(style,'partnum');
    var o = [];
    var title = this.uncode(title,style,switches);
  ...
  do_HDGS(block) {
    let {hdgn,title,name,style,switches,row1,row2} = block;
    var o = [];
  ...
  do_PLST(block) {
    ...
    for (var item of items) {
      ...
      if(more && more.length){
        more.forEach((plitem) => {
          let {lines} = plitem;
          extra_text += `<p class='PARA'> ${this.untext(lines,style,switches)} </p>`;
        });
      }
      switch (bull) {
        case 'OL': {
  ...
  do_HRLE(block) {
    var {id,row1,row2,sig,style,switches,text} = block;
    var o = []; 
    text = this.uncode(text,style,switches);
  ...
  phrase_to_parbox(s,style,switches) {
    ...
    return `<span style='display:inline-block;width:${css_width};'> ${this.uncode(s,style,switches)} </span>`;
  }
  ///
  /// fences
  ///
  fence_to_math(ss,style,switches) {
    var str = ss.join('\n');
    var {s} = this.tokenizer.to_svgmath(str,style,'math');
    return s;
  }
  fence_to_framed(ss,style,switches){
  ...
  fence_to_diagram(ss,style,switches){
  ...
  fence_to_img(ss,style,switches) {
    var fname = ss[0]||'';
    let src = fname;
  ...
  fence_to_blockquote(ss,style,switches){
    var text = this.join_para(ss);
    var text = this.uncode(text,style,switches);
    var css = '';
  ...
  fence_to_verbatim(ss,style,switches){
  ...
  fence_to_list(ss,style,switches) {
    ...
    var css = this.to_css_fontsize(style,switches);
  ...
  to_caption_text(caption,style,switches,name){
    ...
    let caption_text = this.uncode(caption,style,switches);
    if(idnum){
      var lead = `${tag} ${idnum}:`
    }else{
      var lead = `${tag}:`
  ...
  rows_to_tabular(rows,style,switches){
    var o = [];
    ...
    // add the title
    if(style.title){
      let caption_text = this.uncode(style.title,style,switches);
  ...
  itms_to_subfigures(itms,style,switches) {
    ...
    itms.forEach( (p,i,arr) => {
      let {type,ss,g,sub} = p;
      if(type=='framed'){
        p.sub = this.uncode(p.sub,style,switches);
        subid++;
...
${j+n >= itms.length && hlines.indexOf('b') >= 0 ? 'border-bottom:1px solid;' : ''} '>${x}</td>`);
  ...
  do_FLOA_listing(block,caption,ss,style,switches){
    ...
    block.html = `<figure class='LISTING' > ${this.to_caption_text(caption,style,switches,"listing")} ${text} </figure>`;
  }
  do_FLOA_figure(block,caption,ss,style,switches){
    ...
    block.html = `<figure class='FIGURE' > ${this.to_caption_text(caption,style,switches,"figure")} ${text} </figure>`;
  }
  do_FLOA_tablex(block,caption,ss,style,switches){
    var text = this.fence_to_tabularx(ss,style);
    block.html = `<figure class='TABLE' > ${this.to_caption_text(caption,style,switches,"table")} ${text} </figure>`;
  }
  ...
  do_FLOA_longtable(block,caption,ss,style,switches){
    ...
    o.push(this.to_caption_text(caption,style,switches,"table"));
    o.push(text);
    o.push('</figure>');
    text = o.join('\n');  
    block.html = text;
  ...
  to_css_fontsize(style,switches){
