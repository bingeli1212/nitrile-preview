...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  fence_to_dia(style,ss){
    ...
    o = o.map((p) => {
      ...
      var frame = (style.frame)?`frame='box'`:``;
      var s_svg = `<svg class='DIASVG' width='100%' height='auto' xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="currentColor" stroke="currentColor" viewBox="0 0 ${this.fix(p.vw)} ${this.fix(p.vh)}" >${p.svgtext}</svg>`;
