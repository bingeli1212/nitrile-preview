...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  float_to_equation(title,label,style,ss){
    ...
    idnum = `(${idnum})`;
    var text = `<table width='100%'><tr><td><nobr>${idnum}</nobr></td><td width='100%'>${text}</td></tr></table>`;
  ...
  float_to_columns(title,label,style,ss){
    ...
    for(let j=0; j < total; j+=n){
      ...
      o.push(`<div style='${para_css}' style='columns:${n}'>`);
      for(let i=0; i < n; ++i){
  ...
  to_css_map(){
    ...
    this.add_css_map_entry(
      map,
      'ITEMIZE, PARAGRAPH, INDENT, EQUATION, TABBING, FIGURE, TABLE, LONGTABLE, LISTING, COLUMNS',[
