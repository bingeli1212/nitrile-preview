  ...
  float_to_tabbing(title,label,style,splitid,body,bodyrow){
    ...
    rows.forEach((pp,j) => {
      if(j==0){
        ww = pp;
        pp.forEach((p) => {
          p.text = this.uncode(style,p.hd);
        });
        let s = '<tr>' + pp.map((p,i) => `<td style='width:${ww[i].pw}ex;padding:0;'>${p.text}</td>`).join('') + '</tr>';
        if(pp.map(p=>p.hd.length).reduce((acc,cur)=>acc+cur,0)>0){
          all.push(s);
        }
      }else{
        pp.forEach((p) => {
          p.text = this.polish(style,p.raw);
        });
        let s = '<tr>' + pp.map((p,i) => `<td style='width:${ww[i].pw}ex;padding:0;'>${p.text}</td>`).join('') + '</tr>';
        all.push(s);
      }
    ...
    all.unshift(`<table>`);
    all.unshift(`<div class='TABBING' style='${this.css("TABBING")};display:flex;flex-direction:column;align-items:flex-start;' data-row='${style.row}' >`);
    all.push(`</table>`)
    all.push(`</div>`);
    return all.join('\n');
  }  
