...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  fence_to_ink(style,ss){
    ...
    var max_vh = vgap + vgap + npara*12*this.PT_TO_MM;//each font is 12pt height, even though the font-size is 10pt
    ...
    for (var i=0; i < npara; ++i) {
      ...
      var x = 0;
      all.push( `<tspan y="${vgap + (i+extra_dy)*12*this.PT_TO_MM}mm" x="0">${s}</tspan>` );
    }
    all.push( `</text>`);
    var img = all.join('\n');    
