...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  rows_to_tabular(rows,style){
    ...
    for (var j = 0; j < k; ++j) {
      ...
      pp.forEach((x, i) => {
        ...
        mypp.push(`<td style='width:${frs[i]};padding:${pad};border-top:${border_top};border-bottom:${border_bot};border-left:${border_left};border-right:${border_right};background-color:${color};'>${text}</td>`);        
      });
      var p = mypp.join('');
      var p = `<tr>${p}</tr>`;
      border_top = '';
  ...
  itms_to_bull(itms,style){
    ...
    if(itms.length){
      let pp = itms.map((p,i) => {
        if(p.bull=='--'){
          return `<div class='ITEM'>${bullet}${nbsp}${p.text}</div>`;
        }
        if(p.bull=='**'){
          return `<div class='ITEM'>${i+1}.${nbsp}${p.text}</div>`  
        }
        if(p.type == '0'){
          return `<div class='ITEM'>${p.value}${p.ending}${nbsp}${text}</div>`;
        }
        if(p.type == 'a'){
          return `<div class='ITEM'>${this.to_a_letter(p.value)}${p.ending}${nbsp}${text}</div>`;
        }
        if(p.type == 'A'){
          return `<div class='ITEM'>${this.to_A_letter(p.value)}${p.ending}${nbsp}${text}</div>`;
        }
        if(p.type == 'DL'){
          return `<div class='ITEM'><b>${p.value}${p.ending}</b>${nbsp}${text}</div>`;
        }
        return `<div class='ITEM'>${p.text}</div>`
      });
      return `<div>${pp.join('\n')}</div>`;
    }
    return `<div></div>`;  
