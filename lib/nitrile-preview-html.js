...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  fence_to_canvas(style,ss,ssi){
    ...
    var canvaswidget = `<canvas style='position:relative;z-index:20;' width='${p.width}' height='${p.height}' onclick='canvas_onclick(this)' data-imgid='${imgid}' id='${imgid}-canvas' > </canvas>`;
    ...
    var frame = (style.frame)?`frame='box'`:``;
    var s_style = this.css('SUBTITLE');
    var text = `<form action='/post' method='post' onsubmit='canvas_onsubmit(this)' style='position:relative;' data-imgsrc='${imgsrc}' data-imgid='${imgid}' data-canvasid='${imgid}-canvas' > ${imgwidget} ${canvaswidget} ${submitwidget} </form>`
