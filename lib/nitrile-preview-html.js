...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  rows_to_tabular(rows,style){
    ...
    var frs = this.string_to_td_width(style.fr||'');
    ///determine if first row is a hline
    var table_top = hrows.indexOf('t')>=0 || hrows.indexOf('*')>=0;
    var table_bot = hrows.indexOf('b')>=0 || hrows.indexOf('*')>=0;
    let k = rows.length;
    ...
    for (var j = 0; j < k; ++j) {
      ...
      }else if(j==0 && hrows.indexOf('t') >= 0){
        border_top = '1px solid';
      }else if(j==k-1 && hrows.indexOf('b') >= 0){
        border_bot = '1px solid';
      }else if (hrows.indexOf(j) >= 0){
        border_top = '1px solid';
      }
      ///remove the '---' or '==='
      ...
      pp.forEach((x, i) => {
        ...
        if(style.head && j==0){
          ht = '';
          mypp.push(`<th style='width:${frs[i]};padding:${pad};border-top:${border_top};border-bottom:${border_bot};border-left:${border_left};border-right:${border_right};background-color:${color};height:${ht}'>${text}</th>`);        
        }else{
          mypp.push(`<td style='width:${frs[i]};padding:${pad};border-top:${border_top};border-bottom:${border_bot};border-left:${border_left};border-right:${border_right};background-color:${color};height:${ht}'>${text}</td>`);        
        }
      });
      var p = mypp.join('');
      var p = `<tr>${p}</tr>`;
  ...
  itms_to_bull(itms,style){
    ...
    if(itms.length){
      let pp = itms.map((p,i) => {
        ...
        if(p.type == 'DL'){
          return `<div class='ITEM'><b>${value}${p.ending}</b>${nbsp}${text}</div>`;
        }
