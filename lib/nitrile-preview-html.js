...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  fence_to_tab(style,ss,ssi) {
    ...
    itm.img  = ``;
    itm.tab  = `<table style='margin:0;page-break-inside:avoid;${width_css}' ${frame} ${rules} cellpadding='0' cellspacing='0' data-row='${style.row}' >\n<caption align='bottom' class='SUBTITLE' style='${t_style}'>${this.to_tab_subcaptions(style)}</caption>\n${text}\n</table>`;
    'page-break-inside: avoid',
  ...
  fence_to_par(style,ss,ssi){
    ...
    var itm = {};
    itm.img = '';
    itm.tab  = `<p style='margin:0;page-break-inside:avoid;text-align:${talign};' data-row='${style.row}' >${text}</p>`;
    itm.subtitle = subtitle;
    itm.style = style;
    return itm;
  }
  fence_to_vtm(style,ss,ssi){
    ...
    itm.ss = ss;
    itm.img = '';
    itm.tab = `<p style='margin:0;page-break-inside:avoid;' data-row='${style.row}' >${text}</p>`;
    itm.style = style;
    itm.subtitle = subtitle;
    return itm;
  }
  ...
  float_to_columns(title,label,style,splitid,body,bodyrow){
    ...
    var n = itms.length;
    var all = [];
    all.push(`<figure class='COLUMNS' style='${this.css("COLUMNS")};display:grid;grid-template-columns:repeat(${n},${100/n}%);' data-row='${style.row}' >`);
    itms.forEach((p,j,arr) => {
      if(p.img){
        all.push(p.img);
      }else if(p.tab){
