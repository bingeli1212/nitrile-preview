...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  hdgs_to_part(title,label,style){
    return(`<h1 id='${label}' class='PART' style='${this.css("PART")}' data-partnum='${style.__partnum}' > ${this.uncode(style,title)} </h1>`);
  }
  hdgs_to_chapter(title,label,style){
    return(`<h1 id='${label}' class='CHAPTER' style='${this.css("CHAPTER")}' data-partnum='${style.__partnum}' data-chapnum='${style.__chapnum}' > ${this.uncode(style,title)} </h1>`);
  }
  hdgs_to_section(title,label,level,style){
    return(`<h2 id='${label}' class='SECTION' style='${this.css("SECTION")}' data-partnum='${style.__partnum}' data-chapnum='${style.__chapnum}' data-level='${level}' > ${this.uncode(style,title)} </h2>`);
  }
  hdgs_to_subsection(title,label,level,style){
    return(`<h3 id='${label}' class='SUBSECTION' style='${this.css("SUBSECTION")}' data-partnum='${style.__partnum}' data-chapnum='${style.__chapnum}' data-level='${level}' > ${this.uncode(style,title)} </h3>`);
  }
  hdgs_to_subsubsection(title,label,level,style){
    return(`<h4 id='${label}' class='SUBSUBSECTION' style='${this.css("SUBSUBSECTION")} data-partnum='${style.__partnum}' data-chapnum='${style.__chapnum}' data-level='${level}' > ${leader} ${this.uncode(style,title)} </h4>`);
  }
  //////////////////////////////////////////////////////////////////////////
  //
  //////////////////////////////////////////////////////////////////////////
  ...
  item_ol_to_text(style,i,item,nblank){
    ...
    var css = this.css('LI');
    if(item.bullet=='*'){
      let a = style.__memo['*']||0;
      a++;
      style.__memo['*'] = a; 
      o.push(`<li class='LI' style='${css}' type='1'  data-row='${item.row}' value='${a}' > ${text}`);
    }else if(item.type == 'A'){
      let value = i+1;
  ...
  fence_to_img(style,ss,ssi){
    ...
    if(type=='canvas'){
      ...
      var fig = `<img style="border:${border};aspect-ratio:${width}/${height};" src="${imgsrc}" id="${imgid}" onload="img_onload(this)"> </img>`;
      var imgwidget = img;
      var imgid = imgid;
      ...
      var fig = `<object style="border:${border};" type="image/svg+xml" data="data:image/svg+xml;charset=UTF-8,${encodeURIComponent(svg)}"></object>`;
      ...
      var fig = `<object style="border:${border};" type="image/svg+xml" data="data:image/svg+xml;charset=UTF-8,${encodeURIComponent(demo_svg)}"></object>`;
      itm.img  = objectwidget;
    }else if(type=='ink'){
      ...
      if(1){
        ...
        var fig = `<svg style="border:${border}" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="currentColor" stroke="currentColor" viewBox="0 0 ${this.fix(vw)} ${this.fix(vh)}">${imagetext}${svgtext}</svg>`;
      }
  ...
  fence_to_tab(style,ss,ssi) {
    ...
    var textsmaller = '';
    if(style.textsmaller){
      textsmaller = 'smaller';
    }
    itm.tab  = `<table class='TAB' style='margin:0;page-break-inside:avoid;table-layout:fixed;width:${width_css};font-size:${textsmaller};' ${frame} ${rules} cellpadding='0' cellspacing='0' data-row='${style.row}' >${text}</table>`;
    return itm;
  }
  fence_to_par(style,ss,ssi){
    ...
    var itm = {};
    itm.par  = `<p style='margin:0;page-break-inside:avoid;text-align:${textalign};font-size:${textsmaller};font-style:${textoblique}' data-row='${style.row}' >${text}</p>`;
    return itm;
  }
  /////////////////////////////////////////////////////////////////////
  /// 
  ...
  to_caption_text(style,title,capid,splitid,captext){
    ...
    return `<figcaption class='FIGCAPTION' style='${this.css("FIGCAPTION")};${s_style}' >${lead} </figcaption>`;
  }
  to_subfig_num(j) {
  ...
  phrase_to_img(style,cnt){
    ...
    var id = g1.id||'';
    var g0 = style.__switches['img']||{};
    if(style.__buffers.hasOwnProperty(id)){
      let {ss,g} = style.__buffers[id];
      g = {...g0,...g,...g1};
      ...
      if(itm.images && itm.images.length){
        var imgsrc = this.choose_image_file(itm.images);
        var { imgsrc, imgid } = this.to_request_image(imgsrc);
        var imagetext = `<image x="0" y="0" width="${vw}" height="${vh}" xlink:href="${imgsrc}" id="${imgid}" preserveAspectRatio="none"/>`;
      }else{
        var imagetext = '';
      }
      var style_css = `border:${border};width:${width}mm`;
  ...
  phrase_to_wrapimgleft(style,cnt){
    ...
    if(style.__buffers.hasOwnProperty(id)){
      ...
      var style_css = `border:${border};width:${width}mm`;
      var img = `<svg style="float:left;${style_css}" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="currentColor" stroke="currentColor" viewBox="0 0 ${this.fix(vw)} ${this.fix(vh)}">${imagetext}${svgtext}</svg>`;
    }
    return img;
  }
  //////////////////////////////////////////////////////////////////////////////////////
  ...
  phrase_to_wrapimgright(style,cnt){
    ...
    if(style.__buffers.hasOwnProperty(id)){
      ...
      var style_css = `border:${border};width:${width}mm`;
      var img = `<svg style="float:right;${style_css}" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="currentColor" stroke="currentColor" viewBox="0 0 ${this.fix(vw)} ${this.fix(vh)}">${imagetext}${svgtext}</svg>`;
    }
    return img;
  }
  //////////////////////////////////////////////////////////////////////////////////////
  ...
  float_to_preformatted(title,label,style,splitid,body,bodyrow){
    ...
    step.forEach((p,i,arr) => {
      if(i==0){
        if(p.value=='+'){
          ...
          all.push(`<dd>${this.uncode(style,this.join_para(p.lines))}`);
        }else if(p.value=='-'){
          all.push(`<ul class='DETAILS' style='${this.css("DETAILS")}' data-row='${style.row}'>`);
          all.push(`<li value='${p.value}'>${this.uncode(style,this.join_para(p.lines))}`);
        }else if(p.value=='*'){
          let a = style.__memo['*']||0;
          a++;
          style.__memo['*'] = a;
          all.push(`<ol class='DETAILS' style='${this.css("DETAILS")}' data-row='${style.row}'>`);
          all.push(`<li value='${a}'>${this.uncode(style,this.join_para(p.lines))}`);
        }else{
          all.push(`<ol class='DETAILS' style='${this.css("DETAILS")}' data-row='${style.row}'>`);
          all.push(`<li value='${p.value}'>${this.uncode(style,this.join_para(p.lines))}`);
        }
        ...
        if(this.body_is_bundle(p.lines)){
          ...
          let lines = this.body_to_body(p.lines,this.re_body_is_gthan).map(x=>`${this.icon_dag}&#160;${this.uncode(style,x)}`);
  ...
  float_to_figure(title,label,style,splitid,body,bodyrow){
    ...
    itms.forEach((p,j,arr) => {
      if(p.type=='bundle'){
        if(p.fig){
          let img = this.add_css_style('width:100%',p.fig);
          let subtitle = this.to_fig_subtitle(p.style,p.subtitle);
          let w = (p.stretch)?`${p.stretch*100}%`:`${p.width}mm`;
          let tab = `<figure class='SUBFIGURE' style='${this.css("SUBFIGURE")};width:${w};'>${img}<figcaption class='FIGSUBCAPTION' style='${this.css("FIGSUBCAPTION")}'>${subtitle}</figcaption></figure>`;
          onerow.push([w,tab]);
        }
      }
  ...
  float_to_multicols(title,label,style,splitid,body,bodyrow){
    var bundles = this.body_to_all_bundles(style,body,bodyrow);
  ...
  float_to_example(title,label,style,splitid,body,bodyrow){
    let ss = body.map((s) => this.uncode(style,s)).map((s) => `<span>${this.icon_dag}&#160;${s}</span>`);
    var text = ss.join('<br/>\n');
    return `<p class='EXAMPLE' style='${this.css("EXAMPLE")};' data-row='${style.row}' >${text}</p>`;
  }
  float_to_description(title,label,style,splitid,body,bodyrow){
    let ss = body.map((s)=>s.split('\n')).map(([dt,dd]) => `<dt><strong>${this.polish(style,dt)}</strong></dt><dd>${this.uncode(style,dd)}</dd>`);
    var text = ss.join('\n');
    return `<dl class='DESCRIPTION' style='${this.css("DESCRIPTION")};' data-row='${style.row}' >${text}</dl>`;
  }
  float_to_enumeration(title,label,style,splitid,body,bodyrow){
    let ss = body.map((s)=>`<li>${this.uncode(style,s)}</li>`);
    var text = ss.join('\n');
    return `<dl class='ENUMERATION' style='${this.css("ENUMERATION")};' data-row='${style.row}' >${text}</dl>`;
  }
  float_to_tabbing(title,label,style,splitid,body,bodyrow){
    var ss = body.map((s)=>s.split('\n').map(x=>x.trim()));
    var ww = ss.map((ss)=>ss[0]).map(x=>parseInt(x)||1);
    ss = ss.map((ss)=>ss.slice(1));
    var m = ss.map((ss)=>ss.length).reduce((acc,cur)=>Math.max(acc,cur),0);
    var n = ss.length;
    var rows = 'x'.repeat(m).split('').map(s=>'y'.repeat(n).split('').map(y=>''));
    ss.forEach((ss,i)=>{
      ss.forEach((s,j)=>{
        rows[j][i]=s;
      })
    })
    var total = ww.reduce((acc,cur)=>acc+cur,0);
    var all = [];
    rows.forEach((ss,j) => {
      ss = ss.map((s) => {
        return this.uncode(style,s);
      });
      let template = ww.map(w=>`${w}fr`).join(' ');
      let content = ss.map(s=>`<span>${s}</span>`).join(' ');
      let s = `<span style='width:100%;display:inline-grid;grid-template-columns:${template};'>${content}</span>`;
      all.push(s);
    });
    var text = all.join('<br/>\n');
  ...
  ss_to_ink(style,ss,ssi){
    ...
    var fig = `<svg style="border:${border}" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="currentColor" stroke="none" viewBox="0 0 ${this.fix(vw)} ${this.fix(vh)}" >${svgtext}</svg>`;
    var { imgsrc, imgid } = this.to_request_svgid(svg);
    if(stretch>0){
