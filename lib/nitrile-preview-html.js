...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  fence_to_canvas(style,ss,ssi){
    var width = parseInt(style.width)||(60);//this is mm = 12*5
    var height = parseInt(style.height)||(35);//this is mm = 7*5
    var img = style.img||'';
    var width = width*this.MM_TO_PX;
    var height = height*this.MM_TO_PX;
    var width = Math.round(width);//have to turn it into integers otherwise the canvas might not draw the image exact
    var height = Math.round(height);//same
    if(img){
      var imgsrc = `${img}.png`;
      var { imgsrc, imgid } = this.to_request_image(imgsrc);
      var imgwidget = `<img style='position:absolute;' width='${width}' height='${height}' src='${imgsrc}' id='${imgid}' />`;
    }else{
      var imgwidget = '';
    }
    var canvaswidget = `<canvas style='position:relative;z-index:20;' width='${width}' height='${height}' onclick='canvas_onclick(this)' data-imgid='${imgid}' > </canvas>`;
    var p = {};
    p.subc = style.subtitle||'';
    p.subc = this.uncode(style,p.subc);
    p.style = style;
    var frame = (style.frame)?`frame='box'`:``;
    var s_style = this.css('SUBTITLE');
    var text = `<span style='position:relative;display:inline-block'> ${imgwidget} ${canvaswidget} </span>`
