...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  to_caption_text(style,title,idnum,id){
  ...
  float_to_listing(title,label,style,bundles){
    ...
    return `<figure class='LISTING' style='${css}' data-row='${style.row}' > ${this.to_caption_text(style,title,idnum,'Listing')} ${text} </figure>`;
  }
  float_to_figure(title,label,style,bundles){
    ...
    if(style.wrapfig=='right'){
      ...
      return `<figure id='${label}' class='FIGURE' style='${css};${flex_css}' data-row='${style.row}' > ${this.to_caption_text(style,title,idnum,'Figure')} ${text} </figure>`;
    }
  }
  float_to_table(title,label,style,bundles){
    var idnum = style.idnum;
    var o = this.do_bundle(style,bundles[0],'tab');
    var fig = o[0].fig;
    var onerow = [];
    onerow.push(fig);
    let flex_css = 'display:flex;flex-direction:column;row-gap:0.1em;';
    ...
    return `<figure id='${label}' class='TABLE' style='${css};${flex_css}' data-row='${style.row}' > ${this.to_caption_text(style,title,idnum,'Table')} ${text} </figure>`;
  }
