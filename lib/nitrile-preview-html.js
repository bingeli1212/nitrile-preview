...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  fence_to_img(style,ss) {
    ...
    return `<img style='${css}' src='${imgsrc}' id='${imgid}' alt='${imgsrc}'></img>`;
  }
  fence_to_verbatim(style,ss){
    ...
    css += `line-height:1`;
    css += `display:inline-block;`;
    return `<div class='VERBATIM' style='${css}'  > ${text} </div>`;
  }
  ...
  para_to_lines(style,ss) {
    ...
    ss.forEach((s) => {
      ...
      all.push(`<label> ${s} </label>`);
    });
    var text = all.join('<br/>\n')
    return `<div class='PARA LINES' >\n ${text} \n</div>`;
  }
  para_to_sample(style,body){
    ...
    if(style.type=='paragraph'){
      ...
      var text = `<div class='PARA SAMPLE' >\n ${text} \n</div>`;
      return text;
  ...
  para_to_plaintext(style,body){
    ...
    return `<div class='PARA PLAINTEXT' >\n ${text} \n</div>`;
  }
  ...
  float_to_sample(title,label,style,body){
    return this.para_to_sample(style,body);
  }
