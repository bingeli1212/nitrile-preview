...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  hdgs_to_part(title,label,partnum,chapternum,level,style){
    var cls = 'PART';
    return(`<h1 data-row='${style.row}' class='${cls}'> <small> Part ${partnum} </small> <br/> ${this.uncode(style,title)} </h1>`);
  }
  hdgs_to_chapter(title,label,partnum,chapternum,level,style){
    var cls = 'CHAPTER';
    this.addto_refmap(label,`${chapternum}`);
    return(`<h1 data-row='${style.row}' class='${cls}'> <small> Chapter ${chapternum} </small> <br/> ${this.uncode(style,title)} </h1>`);
  }
  hdgs_to_section(title,label,partnum,chapternum,level,style){
    var cls = 'SECTION';
    if(chapternum){
      this.addto_refmap(label,`${chapternum}.${level}`);
      return(`<h2 data-row='${style.row}' class='${cls}'> ${chapternum}.${level} &#160; ${this.uncode(style,title)} </h2>`);  
    }
    this.addto_refmap(label,`${level}`);
    return(`<h2 data-row='${style.row}' class='${cls}'> ${level} &#160; ${this.uncode(style,title)} </h2>`);
  }
  hdgs_to_subsection(title,label,partnum,chapternum,level,style){
    var cls = ('SUBSECTION');
    if(chapternum){
      this.addto_refmap(label,`${chapternum}.{level}`);
      return(`<h3 data-row='${style.row}' class='${cls}'> ${chapternum}.${level} &#160; ${this.uncode(style,title)} </h3>`);  
    }
    this.addto_refmap(label,`${level}`);
    return(`<h3 data-row='${style.row}' class='${cls}'> ${level} &#160; ${this.uncode(style,title)} </h3>`);
  }
  hdgs_to_subsubsection(title,label,partnum,chapternum,level,style){
    var cls = ('SUBSUBSECTION');
    if(chapternum){
      this.addto_refmap(label,`${chapternum}.{level}`);
      return(`<h4 data-row='${style.row}' class='${cls}'> ${chapternum}.${level} &#160; ${this.uncode(style,title)} </h4>`);  
    }
    this.addto_refmap(label,`${level}`);
    return(`<h4 data-row='${style.row}' class='${cls}'> ${level} &#160; ${this.uncode(style,title)} </h4>`);
  }
  /////////////////////////////////////////////////////////////////////
  //
  /////////////////////////////////////////////////////////////////////
  item_dl_to_text(style,i,item,nblank){
    ...
    if(nblank){
      if(text){
        o.push(`<dt class='PARA DT' data-row='${item.row}' > <b>${value}</b> </dt> <dd class='PACK DD' data-row='${item.row+1}' > ${text} </dd> <dd class='PARA DD'>`);
      }else{
        o.push(`<dt class='PARA DT' data-row='${item.row}' > <b>${value}</b> </dt>                                                              <dd class='PARA DD'>`);
      }
    }else{
      if(text){
        o.push(`<dt class='PACK DT' data-row='${item.row}' > <b>${value}</b> </dt> <dd class='PACK DD' data-row='${item.row+1}' > ${text} </dd> <dd class='PACK DD'>`);
      }else{
        o.push(`<dt class='PACK DT' data-row='${item.row}' > <b>${value}</b> </dt>                                                              <dd class='PACK DD'>`);
      }
    }
    ...
    //     if(p.lines){
    //       o.push(`<span class='PARA PR' data-row='${p.row}' > ${this.untext(style,p.lines)} </span>`);
    //     }else if(p.itemize) {
  ...
  item_ol_to_text(style,i,item,nblank){
    ...
    if(item.type == 'A'){
      ...
      o.push(`<li class='${li_cls}' type=''  data-row='${item.row}' value='${item.value}' > ${text}`)
    }else if(item.type == 'a'){
      let value = i+1;
      value = this.int_to_letter_a(value);
      o.push(`<li class='${li_cls}' type=''  data-row='${item.row}' value='${item.value}' > ${text}`)
      ...
      o.push(`<li class='${li_cls}'          data-row='${item.row}' value='${item.value}' > ${text}`);
    }else if(item.bullet=='*'){
      let value = i+1;
      o.push(`<li class='${li_cls}'          data-row='${item.row}' value='${value}'      > ${text}`);
    }else{
      let value = i+1;
      value = this.int_to_letter_i(value);
      o.push(`<li class='${li_cls}' type=''  data-row='${item.row}' value='${item.value}' > ${text}`);
  ...
  item_ul_to_text(style,i,item,nblank){
    ...
    o.push(`<li class='${li_cls}' data-row='${item.row}' > ${text}`);
    // if(item.more && item.more.length){
    //   var style = {...style};
    //   item.more.forEach((p) => {
  ...
  itemize_to_text(style,itemize){
    ...
    switch (bull) {
      case 'DL': {
        ...
        o.push(`<dl class='PARAGRAPH ITEMIZE ${dl_cls}' data-row='${style.row}' >`);
      ...
      case 'OL': {
        ...
        o.push(`<ol class='PARAGRAPH ITEMIZE ${ol_cls}' data-row='${style.row}' style='padding-left:2.0em;'>`);
        items.forEach((item,j) => {
          style.row = item.row;
      ...
      case 'UL': {
        ...
        o.push(`<ul class='PARAGRAPH ITEMIZE ${ul_cls}' data-row='${style.row}' style='padding-left:2.0em;'>`);
        items.forEach((item,j) => {
          style.row = item.row;           
  ...
  fence_to_parbox(style,ss,ssi){
    ...
    if(w && h){           
      var stretch = 0;
      var img = `\n<table class='PARAGRAPH PARBOX' ${frame} cellpadding='0' cellspacing='0' style='display:inline-table' width='${w}' height='${h}' data-row='${style.row}' >\n<caption align='bottom' class='SUBCAPTION'> </caption>\n<colgroup/><thead/><tbody><tr><td style='vertical-align:bottom'>${text}</td></tr></tbody>\n</table>`;
      var fig = `\n<table class='          PARBOX' ${frame} cellpadding='0' cellspacing='0' style='display:inline-table' width='${w}' height='${h}' data-row='${style.row}' >\n<caption align='bottom' class='SUBCAPTION'> </caption>\n<colgroup/><thead/><tbody><tr><td style='vertical-align:bottom'>${text}</td></tr></tbody>\n</table>`;
    }else if(w){           
      var stretch = 0;
      var img = `\n<table class='PARAGRAPH PARBOX' ${frame} cellpadding='0' cellspacing='0' style='display:inline-table' width='${w}'               data-row='${style.row}' >\n<caption align='bottom' class='SUBCAPTION'> </caption>\n<colgroup/><thead/><tbody><tr><td style='vertical-align:bottom'>${text}</td></tr></tbody>\n</table>`;
      var fig = `\n<table class='          PARBOX' ${frame} cellpadding='0' cellspacing='0' style='display:inline-table' width='${w}'               data-row='${style.row}' >\n<caption align='bottom' class='SUBCAPTION'> </caption>\n<colgroup/><thead/><tbody><tr><td style='vertical-align:bottom'>${text}</td></tr></tbody>\n</table>`;
    }else{
      var stretch = 1;
      var img = `\n<table class='PARAGRAPH PARBOX' ${frame} cellpadding='0' cellspacing='0' style='display:inline-table' width='100%'               data-row='${style.row}' >\n<caption align='bottom' class='SUBCAPTION'> </caption>\n<colgroup/><thead/><tbody><tr><td style='vertical-align:bottom'>${text}</td></tr></tbody>\n</table>`;
      var fig = `\n<table class='          PARBOX' ${frame} cellpadding='0' cellspacing='0' style='display:inline-table' width='100%'               data-row='${style.row}' >\n<caption align='bottom' class='SUBCAPTION'> </caption>\n<colgroup/><thead/><tbody><tr><td style='vertical-align:bottom'>${text}</td></tr></tbody>\n</table>`;
    }
    var o = [];
    o.push({img,subc,stretch,w,h,fig,style});
    return o;
  }
  fence_to_displaymath(style,ss,ssi) {
    ...
    if(w){           
      var stretch = 0;
      var img = `\n<table class='PARAGRAPH DISPLAYMATH' ${frame} ${rules} cellpadding='0' cellspacing='0' style='display:inline-table' width='${w}' data-row='${style.row}' >\n<caption align='bottom' class='SUBCAPTION' >         </caption>\n<colgroup/><thead/><tbody><tr><td>${text}</td></tr></tbody>\n</table>`;
      var fig = `\n<table class='          DISPLAYMATH' ${frame} ${rules} cellpadding='0' cellspacing='0' style='display:inline-table' width='${w}' data-row='${style.row}' >\n<caption align='bottom' class='SUBCAPTION' > ${subc} </caption>\n<colgroup/><thead/><tbody><tr><td>${text}</td></tr></tbody>\n</table>`;
    }else{
      var stretch = 1;
      var img = `\n<table class='PARAGRAPH DISPLAYMATH' ${frame} ${rules} cellpadding='0' cellspacing='0' style='display:inline-table' width='100%' data-row='${style.row}' >\n<caption align='bottom' class='SUBCAPTION' >         </caption>\n<colgroup/><thead/><tbody><tr><td>${text}</td></tr></tbody>\n</table>`;
      var fig = `\n<table class='          DISPLAYMATH' ${frame} ${rules} cellpadding='0' cellspacing='0' style='display:inline-table' width='100%' data-row='${style.row}' >\n<caption align='bottom' class='SUBCAPTION' > ${subc} </caption>\n<colgroup/><thead/><tbody><tr><td>${text}</td></tr></tbody>\n</table>`;
    }
    var o = [];
    o.push({img,subc,stretch,w,h,fig,style});
    return o;
  ...
  fence_to_math(style,ss,ssi) {
    ...
    if(w){           
      var stretch = 0;
      var img = `\n<table class='PARAGRAPH DISPLAYMATH' ${frame} ${rules} cellpadding='0' cellspacing='0' style='display:inline-table' width='${w}' >\n<caption align='bottom' class='SUBCAPTION' data-row='${style.row}' >         </caption>\n<colgroup/><thead/><tbody><tr><td>${text}</td></tr></tbody>\n</table>`;
      var fig = `\n<table class='          DISPLAYMATH' ${frame} ${rules} cellpadding='0' cellspacing='0' style='display:inline-table' width='${w}' >\n<caption align='bottom' class='SUBCAPTION' data-row='${style.row}' > ${subc} </caption>\n<colgroup/><thead/><tbody><tr><td>${text}</td></tr></tbody>\n</table>`;
    }else{
      var stretch = 1;
      var img = `\n<table class='PARAGRAPH DISPLAYMATH' ${frame} ${rules} cellpadding='0' cellspacing='0' style='display:inline-table' width='100%' >\n<caption align='bottom' class='SUBCAPTION' data-row='${style.row}' >         </caption>\n<colgroup/><thead/><tbody><tr><td>${text}</td></tr></tbody>\n</table>`;
  ...
  fence_to_ink(style,ss,ssi){
    ...
    if(ENCODE){
      ...
      var s_svg = `<svg width='100%' xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' fill='currentColor' stroke='none' viewBox='0 0 ${this.fix(vw)} ${this.fix(vh)}' >${text}</svg>`;
    }
    var o = [];
    ...
    if(style.stretch){
      var img = `\n<table class='PARAGRAPH INK' ${frame} cellpadding='0' cellspacing='0' style='display:inline-table' width='100%' data-row='${style.row}' >\n<caption align='bottom' class='SUBCAPTION' style='${s_style}' >       </caption>\n<tr><td>${s_svg}</td></tr>\n</table>`;
      var fig = `\n<table class='          INK' ${frame} cellpadding='0' cellspacing='0' style='display:inline-table' width='100%' data-row='${style.row}' >\n<caption align='bottom' class='SUBCAPTION' style='${s_style}' >${subc}</caption>\n<tr><td>${s_svg}</td></tr>\n</table>`;
    }else{
      var img = `\n<table class='PARAGRAPH INK' ${frame} cellpadding='0' cellspacing='0' style='display:inline-table' width='${w}' data-row='${style.row}' >\n<caption align='bottom' class='SUBCAPTION' style='${s_style}' >       </caption>\n<tr><td>${s_svg}</td></tr>\n</table>`;
      var fig = `\n<table class='          INK' ${frame} cellpadding='0' cellspacing='0' style='display:inline-table' width='${w}' data-row='${style.row}' >\n<caption align='bottom' class='SUBCAPTION' style='${s_style}' >${subc}</caption>\n<tr><td>${s_svg}</td></tr>\n</table>`;
    }
    o.push({img,subc,w,h,stretch,fig,style});
    return o;
  }
  fence_to_dia(style,ss,ssi){
    ...
    o = o.map((p) => {
      ...
      if(style.stretch){
        p.img = `\n<table class='PARAGRAPH DIA' ${flt} ${frame} cellpadding='0' cellspacing='0' style='display:inline-table' width='100%' data-row='${style.row}' >\n<caption align='bottom' class='SUBCAPTION' style='${s_style}' >         </caption>\n<colgroup/><thead/><tbody><tr><td>${s_svg}</td></tr></tbody>\n</table>`;
        p.fig = `\n<table class='          DIA' ${flt} ${frame} cellpadding='0' cellspacing='0' style='display:inline-table' width='100%' data-row='${style.row}' >\n<caption align='bottom' class='SUBCAPTION' style='${s_style}' >${p.subc}</caption>\n<colgroup/><thead/><tbody><tr><td>${s_svg}</td></tr></tbody>\n</table>`;
      }else{
        p.img = `\n<table class='PARAGRAPH DIA' ${flt} ${frame} cellpadding='0' cellspacing='0' style='display:inline-table' width='${w}' data-row='${style.row}' >\n<caption align='bottom' class='SUBCAPTION' style='${s_style}' >         </caption>\n<colgroup/><thead/><tbody><tr><td>${s_svg}</td></tr></tbody>\n</table>`;
        p.fig = `\n<table class='          DIA' ${flt} ${frame} cellpadding='0' cellspacing='0' style='display:inline-table' width='${w}' data-row='${style.row}' >\n<caption align='bottom' class='SUBCAPTION' style='${s_style}' >${p.subc}</caption>\n<colgroup/><thead/><tbody><tr><td>${s_svg}</td></tr></tbody>\n</table>`;
      }
      return p;
    });
    return o;
  ...
  fence_to_verbatim(style,ss,ssi){
    ...
    var subc = '';
    var stretch = 1;
    var img = `\n<table class='PARAGRAPH VERBATIM' ${frame} ${rules} cellpadding='0' cellspacing='0' style='display:inline-table' width='100%' data-row='${style.row}' >\n<caption align='bottom' class='SUBCAPTION'> ${subc} </caption>\n<colgroup/><thead/><tbody><tr><td>${text}</td></tr></tbody>\n</table>`;
    var fig = `\n<table class='          VERBATIM' ${frame} ${rules} cellpadding='0' cellspacing='0' style='display:inline-table' width='100%' data-row='${style.row}' >\n<caption align='bottom' class='SUBCAPTION'> ${subc} </caption>\n<colgroup/><thead/><tbody><tr><td>${text}</td></tr></tbody>\n</table>`;
    var o = [];
    o.push({img,subc,stretch,fig,style});
    return o;
  }
  fence_to_center(style,ss,ssi){
    var text = this.join_para(ss);
    var text = this.uncode(style,text);
    text = `<span style='font-style:${style.fontstyle};font-weight:${style.fontweight};'> ${text} </span>`;
    var frame = '';
    var rules = '';
    var subc = '';
    var stretch = 1;
    var img = `\n<table class='PARAGRAPH CENTER' ${frame} ${rules} cellpadding='0' cellspacing='0' style='display:inline-table' width='100%' data-row='${style.row}' >\n<caption align='bottom' class='SUBCAPTION'>         </caption>\n<colgroup/><thead/><tbody><tr><td>${text}</td></tr></tbody>\n</table>`;
    var fig = `\n<table class='          CENTER' ${frame} ${rules} cellpadding='0' cellspacing='0' style='display:inline-table' width='100%' data-row='${style.row}' >\n<caption align='bottom' class='SUBCAPTION'> ${subc} </caption>\n<colgroup/><thead/><tbody><tr><td>${text}</td></tr></tbody>\n</table>`;
    var o = [];
    o.push({img,subc,stretch,fig,style});
    return o;
  }
  fence_to_flushright(style,ss,ssi){
    ...
    var subc = '';
    var stretch = 1;
    var img = `\n<table class='PARAGRAPH FLUSHRIGHT' ${frame} ${rules} cellpadding='0' cellspacing='0' style='display:inline-table' width='100%' data-row='${style.row}' >\n<caption align='bottom' class='SUBCAPTION'>         </caption>\n<colgroup/><thead/><tbody><tr><td>${text}</td></tr></tbody>\n</table>`;
    var fig = `\n<table class='          FLUSHRIGHT' ${frame} ${rules} cellpadding='0' cellspacing='0' style='display:inline-table' width='100%' data-row='${style.row}' >\n<caption align='bottom' class='SUBCAPTION'> ${subc} </caption>\n<colgroup/><thead/><tbody><tr><td>${text}</td></tr></tbody>\n</table>`;
    var o = [];
    o.push({img,subc,stretch,fig,style});
    return o;
  }
  fence_to_flushleft(style,ss,ssi){
    ...
    var subc = '';
    var stretch = 1;
    var img = `\n<table class='PARAGRAPH FLUSHLEFT' ${frame} ${rules} cellpadding='0' cellspacing='0' style='display:inline-table' width='100%' data-row='${style.row}' >\n<caption align='bottom' class='SUBCAPTION'>         </caption>\n<colgroup/><thead/><tbody><tr><td>${text}</td></tr></tbody>\n</table>`;
    var fig = `\n<table class='          FLUSHLEFT' ${frame} ${rules} cellpadding='0' cellspacing='0' style='display:inline-table' width='100%' data-row='${style.row}' >\n<caption align='bottom' class='SUBCAPTION'> ${subc} </caption>\n<colgroup/><thead/><tbody><tr><td>${text}</td></tr></tbody>\n</table>`;
    var o = [];
    o.push({img,subc,stretch,fig,style});
    return o;
  }
  fence_to_tabular(style,ss,ssi) {
    ...
    if(style.head && rows.length){
      ...
      pp.forEach((p,i) => {
        let {raw,text} = p;
        let css = '';
        css += `padding:1.0mm 1.5mm; text-align:${ww[i].ta}`;
        if(pp.top){
    ...
    var img = `<table class='PARAGRAPH TABULAR' ${flt} ${frame} ${rules} cellpadding='0' cellspacing='0' style='display:inline-table' data-row='${style.row}' >\n<caption align='bottom' class='SUBCAPTION' style='${s_style}' >       </caption>\n${colgroup}\n${text}\n</table>`;
    var fig = `<table class='          TABULAR' ${flt} ${frame} ${rules} cellpadding='0' cellspacing='0' style='display:inline-table' data-row='${style.row}' >\n<caption align='bottom' class='SUBCAPTION' style='${s_style}' >${subc}</caption>\n${colgroup}\n${text}\n</table>`;
    var o = [];
    o.push({img,subc,w,h,stretch:0,fig,style});
    return o;
  }
  fence_to_verse(style,ss,ssi) {
    ...
    var ss = itms.map((p,i,arr) => {
      ...
      if(p.type=='UL'){
        return(`<span data-row='${ssi+i}' >${bullet} ${p.text}</span>`);
      }
      else if(p.type=='OL'){
        return(`<span data-row='${ssi+i}' >${i+1}. ${p.text}</span>`);  
      }
      else if(p.type == '1'){
        if(!p.ending) p.ending = '.';
        return(`<span data-row='${ssi+i}' >${p.value}${p.ending} ${p.text}</span>`);
      }
      else if(p.type == 'a'){
        if(!p.ending) p.ending = '.';
        return(`<span data-row='${ssi+i}' >${this.int_to_letter_a(p.value)}${p.ending} ${p.text}</span>`);
      }
      else if(p.type == 'A'){
        if(!p.ending) p.ending = '.';
        return(`<span data-row='${ssi+i}' >${this.int_to_letter_A(p.value)}${p.ending} ${p.text}</span>`);
        ...
        return(`<span data-row='${ssi+i}' ><b>${p.value}</b> ${p.text}</span>`)
      }
      else if(p.type == 'INDENTED'){
        //empty CB
        let s = '&#160;'.repeat(p.value);
        return(`<span data-row='${ssi+i}' > ${s}${p.text}</span> `);
      }
      else {
        ///ordinary line without a leading bullet
        return(`<span data-row='${ssi+i}' >${p.text}</span>`);
      }
    });
    var text = ss.join('<br/>\n')
    let width = this.g_to_width_float(style);
    ...
    if(w){           
      var stretch = 0;
      var img = `\n<table class='PARAGRAPH VERSE' ${frame} ${rules} cellpadding='0' cellspacing='0' style='display:inline-table' width='${w}' data-row='${style.row}' >\n<caption align='bottom' class='SUBCAPTION'>         </caption>\n<colgroup/><thead/><tbody><tr><td>${text}</td></tr></tbody>\n</table>`;
      var fig = `\n<table class='          VERSE' ${frame} ${rules} cellpadding='0' cellspacing='0' style='display:inline-table' width='${w}' data-row='${style.row}' >\n<caption align='bottom' class='SUBCAPTION'> ${subc} </caption>\n<colgroup/><thead/><tbody><tr><td>${text}</td></tr></tbody>\n</table>`;
    }else{
      var stretch = 1;
      var img = `\n<table class='PARAGRAPH VERSE' ${frame} ${rules} cellpadding='0' cellspacing='0' style='display:inline-table' width='100%' data-row='${style.row}' >\n<caption align='bottom' class='SUBCAPTION'>         </caption>\n<colgroup/><thead/><tbody><tr><td>${text}</td></tr></tbody>\n</table>`;
      var fig = `\n<table class='          VERSE' ${frame} ${rules} cellpadding='0' cellspacing='0' style='display:inline-table' width='100%' data-row='${style.row}' >\n<caption align='bottom' class='SUBCAPTION'> ${subc} </caption>\n<colgroup/><thead/><tbody><tr><td>${text}</td></tr></tbody>\n</table>`;
    }
    var o = [];
    o.push({img,subc,stretch,w,h,fig,style});
    return o;
  ...
  fence_to_quote(style,ss,ssi){
    ...
    if(w){           
      var stretch = 0;
      var img = `\n<table class='PARAGRAPH QUOTE' ${frame} ${rules} cellpadding='0' cellspacing='0' style='display:inline-table' width='${w}' data-row='${style.row}' >\n<caption align='bottom' class='SUBCAPTION'>         </caption>\n<colgroup/><thead/><tbody><tr><td>${text}</td></tr></tbody>\n</table>`;
      var fig = `\n<table class='          QUOTE' ${frame} ${rules} cellpadding='0' cellspacing='0' style='display:inline-table' width='${w}' data-row='${style.row}' >\n<caption align='bottom' class='SUBCAPTION'> ${subc} </caption>\n<colgroup/><thead/><tbody><tr><td>${text}</td></tr></tbody>\n</table>`;
    }else{
      var stretch = 1;
      var img = `\n<table class='PARAGRAPH QUOTE' ${frame} ${rules} cellpadding='0' cellspacing='0' style='display:inline-table' width='100%' data-row='${style.row}' >\n<caption align='bottom' class='SUBCAPTION'>         </caption>\n<colgroup/><thead/><tbody><tr><td>${text}</td></tr></tbody>\n</table>`;
      var fig = `\n<table class='          QUOTE' ${frame} ${rules} cellpadding='0' cellspacing='0' style='display:inline-table' width='100%' data-row='${style.row}' >\n<caption align='bottom' class='SUBCAPTION'> ${subc} </caption>\n<colgroup/><thead/><tbody><tr><td>${text}</td></tr></tbody>\n</table>`;
    }
    var o = [];
    o.push({img,subc,stretch,w,h,fig,style});
    return o;
  ...
  ///
  //////////////////////////////////////////////////////////////////////////////////////
  text_to_text(style,text,row){
    text = this.uncode(style,text);
    return `<p class='PARAGRAPH NORMAL' data-row='${row}'>${text}</p>`;
  }
  /////////////////////////////////////////////////////////////////////
  ///
  ///
  ...
  float_to_equation(title,label,style,ss,ssi){
    ...
    var text = `<p class='PARAGRAPH EQUATION' style='position:relative;' > ${span} ${text} </p>`;
    return text;
  }
  float_to_listing(title,label,style,ss,ssi){
    ...
    var ss = ss.map((s,i,arr) => {
      ...
      return(`<span class='LINE' data-row='${ssi+i}' style='--lineno:"${i}";' ><code style='white-space:pre;'>${s}</code></span>`);
    });
    ss = ss.slice(1,-1);
    var text = ss.join('\n');
    var text = `<span class='BODY' data-row='${ssi}' > ${text} </span>`;
    return `<figure class='PARAGRAPH LISTING' data-row='${style.row}' > ${this.to_caption_text(title,label,style,"listing")} ${text} </figure>`;
  }
    ...
    if(style.wrap){
      ...
      itms.forEach((p,j,arr) => {
        if(p.type=='bundle'){
          ...
          let seq = p.seq;
          if(style.senum){
            fig = this.fig_to_senum(style.senum, seq,fig);
          }
          all.push(fig);
        }else if(p.type=='\\\\'){
          all.push(`<br/>`);
      ...
      var text = all.join('\n');
      if(style.type=='table'){
        return `<figure class='PARAGRAPH FIGURE' style='${s_style}' data-row='${style.row}' > ${this.to_caption_text(title,label,style,"table")} ${text} </figure>`;
      }else{
        return `<figure class='PARAGRAPH FIGURE' style='${s_style}' data-row='${style.row}' > ${this.to_caption_text(title,label,style,"figure")} ${text} </figure>`;
      }
    }else{
      ...
      itms.forEach((p,j,arr) => {
        if(p.type=='bundle'){
          ...
          all.push(`<span class='SUBROW' style='${s_style}' > ${onerow.join('&#160;\n')} </span>`);
          onerow = [];
        }
      });
      if(onerow.length){
      ...
      var text = all.join('\n');
      if(style.type=='table'){
        return `<figure class='PARAGRAPH FIGURE' data-row='${style.row}' > ${this.to_caption_text(title,label,style,"table")} ${text} </figure>`;
      }else{
        return `<figure class='PARAGRAPH FIGURE' data-row='${style.row}' > ${this.to_caption_text(title,label,style,"figure")} ${text} </figure>`;
      }
    }
  }
  float_to_longtabu(title,label,style,ss,ssi){
    ...
    text = `<table class='BODY' ${frame} ${rules} cellpadding='0' cellspacing='0' style='display:table'  data-row='${ssi}' >\n${colgroup}\n${text}\n</table>`;
    return `<figure class='PARAGRAPH LONGTABU'  data-row='${style.row}' > ${this.to_caption_text(title,label,style,"table")} ${text} </figure>`;0
  }
  float_to_columns(title,label,style,ss,ssi){
    ...
    var n = row.length;
    all.push(`<p class='PARAGRAPH COLUMNS' style='column-count:${n}' data-row='${style.row}' >`);
    for(let i=0; i < n; ++i){
      let p = row[i];
      all.push(`<span style='break-before:column;'> ${p.img} </span>`);
    }
    all.push(`</p>`);
    return all.join('\n');
  }
  ...
  to_caption_text(title,label,style,key){
    ...
    if(none && !caption_text){
      ...
      return `<figcaption class='FIGCAPTION' style='${s_style}' >${lead} </figcaption>`;
    }else{
  ...
  //////////////////////////////////////////////////////////////////////////////////////
  untext(style,ss){
    var text = super.untext(style,ss);
    var text = text.trim();
    return text;
  }
  ...
  fig_to_senum(senum, seq,fig){
    if(typeof fig === 'string'){
      fig = fig.trim();
      const re = /^<table\b(.*?)>(.*)<\/table>$/s;
      var v;
      if((v=re.exec(fig))!==null){
        let num = ""+seq;
        if(senum=='a'){
          num = this.int_to_letter_a(seq);
        }else if(senum=='A'){
          num = this.int_to_letter_A(seq);
        }else if(senum=='i'){
          num = this.int_to_letter_i(seq);
        }else if(senum=='I'){
          num = this.int_to_letter_I(seq);
        }
        num = `(${num})`;
        ...
        if(attributes.hasOwnProperty('style')){
          ...
          let fig = this.string_from_tag_attributes(attributes);
          return `<table ${fig}>${v[2]}</table>`;
        }
  ...
  plst_to_text(style,itemize){
    ...
    //return `<p class='PARAGRAPH ITEMIZE' data-row='${style.row}' > ${text} </p>`;
    return text;
  }
  ////////////////////////////////////////////////////////////////////////////////
  //
  ...
  cove_to_text(style,cove){
    var o = [];
    o.push(`<span class='COVE' data-row='${style.row}' style='--bull:"&#x25B8;";--pad:-1.0em;' >`);
    var n = cove.length;
    var p0 = cove[0];
    var pn = cove[cove.length-1];
    if(n > 2 && p0.text=='```' && pn.text=='```'){
      //everything inbetween fixed font size
      cove.forEach((p,i,arr) => {
        if(i==0) return; //ignore first line
        if(i==arr.length-1) return; //ignore last line
        o.push(`<span data-row='${p.n}' ><code style='white-space:pre;'>${this.polish_verb(style,p.text)}</code></span>`);
        o.push('<br/>');
      })
    }else{
      //all normal font size
      cove.forEach((p,i,arr) => {
        o.push(`<span data-row='${p.n}' > ${this.uncode(style,p.text)} </span>`);
        if(i==arr.length-1) {
        }else{
          o.push('<br/>');
        }
    ...
    o.push('</span>')
    var text = o.join('\n')
    return `<p class='PARAGRAPH INDENTING' data-row='${style.row}' > ${text} </p>`;
  }
  ////////////////////////////////////////////////////////////////////////////////
  //
  //
  ...
  math_to_text(style,math){
    let ss = math.body;
    var text = this.tokenizer.to_fence_math(style,ss);
    return `<p class='PARAGRAPH MATH' data-row='${style.row}' style='text-align:center;' > ${text} </p>`;
  }
  ////////////////////////////////////////////////////////////////////////////////
  //
  //
  ...
  samp_to_text(style,samp){
    ...
    var text = ss.join('<br/>\n');
    return(`<p class='PARAGRAPH SAMPLE' data-row='${style.row}' > ${text} </p>`);
  }
  ////////////////////////////////////////////////////////////////////////////////
  //
  //
  ...
  prim_to_text(style,title,body){
    ...
    return `<p class='PARAGRAPH PRIMARY' data-row='${style.row}' > ${title} ${text} </p>`;
  }
  ////////////////////////////////////////////////////////////////////////////////
  //
  //
  ...
  norm_to_text(style,body){
    var text = this.untext(style,body).trim();
    //return `<p class='PARAGRAPH NORMAL' data-row='${style.row}' > ${text} </p>`;
    return text;
  }
