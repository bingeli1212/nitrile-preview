...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  hdgs_to_part(title,label,idnum,idcap,partnum,chapternum,level,style){
    ...
    return(`<div class='${cls}' style='position:absolute;top:0;right:0;bottom:0;left:0;display:flex;flex-direction:column;justify-content:center;align-items:center;font-size:200%;' > <span><small> Part ${idnum} </small></span> <span>${this.uncode(style,title)}</span> </div>`);
  }
  /////////////////////////////////////////////////////////////////////
  //
  // translator function
  ...
  hdgs_to_chapter(title,label,idnum,idcap,partnum,chapternum,level,style){
    ...
    return(`<h1 id='${label}' data-row='${style.row}' class='${cls}' style='${css}' > <span style='font-weight:normal;font-size:80%' > Chapter ${idnum} </span> <br/> ${this.uncode(style,title)} </h1>`);
  }
  /////////////////////////////////////////////////////////////////////
  //
  // translator function
  ...
  hdgs_to_section(title,label,idnum,idcap,partnum,chapternum,level,style){
    ...
    return(`<h2 id='${label}' data-row='${style.row}' class='${cls}' style='${css}' > ${idnum} &#160; ${this.uncode(style,title)} </h2>`);
  }
  /////////////////////////////////////////////////////////////////////
  //
  // translator function
  ...
  hdgs_to_subsection(title,label,idnum,idcap,partnum,chapternum,level,style){
    ...
    return(`<h3 id='${label}' data-row='${style.row}' class='${cls}' style='${css}' > ${idnum} &#160; ${this.uncode(style,title)} </h3>`);
  }
  /////////////////////////////////////////////////////////////////////
  //
  // translator function
  ...
  hdgs_to_subsubsection(title,label,idnum,idcap,partnum,chapternum,level,style){
    ...
    return(`<h4 id='${label}' data-row='${style.row}' class='${cls}' style='${css}' > ${idnum} &#160; ${this.uncode(style,title)} </h4>`);
  }
  /////////////////////////////////////////////////////////////////////
  /////////////////////////////////////////////////////////////////////
  ...
  plst_to_text(style,itemize){
    ...
    var css = this.css('PLST');
    return `<span class='PLST' style='${css};width:100%;display:inline-flex;justify-content:flex-start;' data-row='${style.row}' > ${text} </span>`;
  }
  ////////////////////////////////////////////////////////////////////////////////
  //
  // translator function
  ...
  float_to_paragraph(title,label,style,body,bodyrow){
    ...
    let css = this.css('PARAGRAPH NORMAL');
    return `<div class='PARAGRAPH NORMAL' style='${css}' > ${text} </div>`;
  }  
  /////////////////////////////////////////////////////////////////////
  //
  // translator function
  ...
  float_to_figure(title,label,idnum,idcap,style,ss,ssi){
    ...
    itms.forEach((p,j,arr) => {
      if(p.type=='bundle'){
        ...
        let n = onerow.length;
        all.push(`<span class='SUBROW' style='${subrow_css}' > ${onerow.join('&#160;')} </span>`);
        onerow = [];
      }
    });
    if(onerow.length){
  ...
  float_to_columns(title,label,idnum,idcap,style,ss,ssi){
    ...
    all.push(`<div class='PARAGRAPH COLUMNS' style='${css}' data-row='${style.row}' >`);
