...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  fence_to_fml(style,ss,ssi) {
    ...
    itm.par  = `\n<table class='BUNDLE FML' style='${this.css("BUNDLE FML")}' cellpadding='0' cellspacing='0' data-row='${style.row}' >\n<caption align='bottom' class='SUBTITLE' style='${s_style}' > ${subtitle} </caption>\n<colgroup/><thead/><tbody><tr><td>${img}</td></tr></tbody>\n</table>`;
    itm.style = style;
  ...
  fence_to_ink(style,ss,ssi){
    ...
    itm.par  = `\n<table class='BUNDLE INK' style='${this.css("BUNDLE INK")}' cellpadding='0' cellspacing='0' data-row='${style.row}' >\n<caption align='bottom' class='SUBTITLE' style='${s_style}' > ${subtitle} </caption>\n<tr><td>${img}</td></tr>\n</table>`;
    itm.style = style;
    itm.subtitle = subtitle;
  ...
  fence_to_dia(style,ss,ssi){
    ...
    if(type=='canvas'){
      ...
      itm.par  = `\n<table class="BUNDLE DIA" style="${this.css("BUNDLE DIA")}" cellpadding="0" cellspacing="0" data-row="${style.row}" >\n<caption align="bottom" class="SUBTITLE" style="${s_style}" >${subtitle}</caption>\n<colgroup/><thead/><tbody><tr><td>${img}</td></tr></tbody>\n</table>`;
    }else if(type=='ball'){
      var svg = itm.svg;
  ...
  fence_to_tab(style,ss,ssi) {
    ...
    itm.par  = `<table class='BUNDLE TAB' style='${this.css("BUNDLE TAB")};${fontsize_css}' ${frame} ${rules} cellpadding='0' cellspacing='0' data-row='${style.row}' >                                                                                                         \n${text}\n</table>`;
    itm.style = style;
    itm.subtitle = subtitle;
    return itm;
  }
  fence_to_par(style,ss,ssi){
    ...
    itm.par  = `\n<table class='BUNDLE PAR' style='${this.css("BUNDLE PAR")};${fontsize_css}' ${frame} ${rules} cellpadding='0' cellspacing='0' data-row='${style.row}' >\n                                                                                 \n<colgroup/><thead/><tbody><tr><td style='vertical-align:bottom;'>${text}</td></tr></tbody>\n</table>`;
    itm.subtitle = subtitle;
    itm.style = style;
    return itm;
  }
  fence_to_vtm(style,ss,ssi){
    ...
    itm.par = `\n<table class='BUNDLE VTM' style='${this.css("BUNDLE VTM")};${fontsize_css}' ${frame} ${rules} cellpadding='0' cellspacing='0' data-row='${style.row}' >\n                                                                                 \n<colgroup/><thead/><tbody><tr><td>${text}</td></tr></tbody>\n</table>`;
    itm.style = style;
    itm.subtitle = subtitle;
    return itm;
  }
  ...
  float_to_verbatim(title,label,style,body,bodyrow){
    ...
    if(hew>1){
      ...
      return `<p class='VERBATIM' style='${css};column-count:${hew};${fontsize_css};' >${text}</p>`;
    }else{
      var text = ss.join('<br/>\n');
      var css = this.css('VERBATIM');
      return`<p class='VERBATIM' style='${css};${fontsize_css};' data-row='${style.row}' >${text}</p>`;
    }
  }
  ...
  float_to_record(title,label,style,body,bodyrow){
    ...
    step.forEach((p,i) => {
      if(i==0){
        ...
        let text = ss.join('<br/>')
        all.push(`<span class='RECORDROW' style='${this.css("RECORDROW")}'>${text}</span>`);
    ...
    var text = all.join('\n');
    return`<ol class='RECORD OL' style='${this.css("RECORD OL")}' data-row='${style.row}' > ${text} </ol>`;
  } 
