    ...
    itms.forEach((p,j,arr) => {
      if(p.type=='bundle'){
        if(p.img){
          ...
          subtitle = `<span class='FIGSUBCAPTION' style='${this.css("FIGSUBCAPTION")}'>${subtitle}</span>`;
      ...
      if(p.type=='\\\\'||j+1==arr.length){
        let n = onerow.length;
        if(n){
          all.push(`<div class='FIGUREROW' style='${this.css("FIGUREROW")};grid-template-columns:${onerow.map(s=>s[0]).join(' ')}' > ${onerow.map(s=>s[1]).join('')} ${onerow.map(s=>s[2]).join('')} </div>`);
          onerow = [];
        }
      }
    });
