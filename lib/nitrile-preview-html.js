...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  fence_to_tabular(style,ss,ssi) {
    ...
    if(style.head && rows.length){
      ...
      pp.forEach((p,i) => {
        let {raw,text} = p;
        let padding = '0.2835em 0.4252em';//these settings are chosen to match those of beamer and creamer
        let css = `padding:${padding}; text-align:${ww[i].ta}`;
        if(pp.upper){
  ...
  float_to_listing(title,label,style,ss,ssi){
    ...
    return `<figure class='PARAGRAPH LISTING' style='${css}' data-row='${style.row}' > ${this.to_caption_text(title,label,style,"listing")} ${text} </figure>`;
  }
  float_to_figure(title,label,style,ss,ssi){
    ...
    if(style.type=='right'){
      ...
      return `<figure class='PARAGRAPH FIGURE' style='${css}' data-row='${style.row}' > ${this.to_caption_text(title,label,style,"table")} ${text} </figure>`;
    }else{
      return `<figure class='PARAGRAPH FIGURE' style='${css}' data-row='${style.row}' > ${this.to_caption_text(title,label,style,"figure")} ${text} </figure>`;
    }
  }
  float_to_longtabu(title,label,style,ss,ssi){
    ...
    var captin = this.to_longtabu_caption_text(title,label,style,"table");
  ...
  to_longtabu_caption_text(title,label,style,key){
    let salign = style.salign;
    let caption_text = this.uncode(style,title).trim();
    let none = this.assert_int(style.nocaption);
    let tag = this.to_tag(key);
