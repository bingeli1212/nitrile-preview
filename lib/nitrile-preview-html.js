...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  cove_to_text(style,cove){
    ...
    return `<p class='PARAGRAPH COVE' style='${css};position:relative;padding-left:30pt;' > <span style='position:absolute;left:20pt;'> &#x2023; </span> ${text} </p>`;
  }
  ...
  lave_to_text(style,lave){
    ...
    if(1){
      let all = [];
      //all normal font size
      lave.forEach((p,i,arr) => {
        all.push(`<span style='text-align:left' data-row='${p.n}' > ${this.uncode(style,p.text)} </span>`);
        if(i==arr.length-1) {
        }else{
          all.push('');
        }
