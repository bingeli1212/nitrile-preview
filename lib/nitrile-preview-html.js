...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  hdgs_to_section(title,label,level,style){
    ...
    return(`<h2 id='${label}' data-row='${style.row}' class='${cls}' style='${css}' > ${idnum} <span title='${style.note}:${1+style.row}'>${this.uncode(style,title)}</span> </h2>`);
  }
  /////////////////////////////////////////////////////////////////////
  //
  // translator function
  ...
  hdgs_to_subsection(title,label,level,style){
    ...
    return(`<h3 id='${label}' data-row='${style.row}' class='${cls}' style='${css}' > ${idnum} <span title='${style.note}:${1+style.row}'>${this.uncode(style,title)}</span> </h3>`);
  }
  /////////////////////////////////////////////////////////////////////
  //
  // translator function
  ...
  hdgs_to_subsubsection(title,label,level,style){
    ...
    return(`<h4 id='${label}' data-row='${style.row}' class='${cls}' style='${css}' > ${idnum} <span title='${style.note}:${1+style.row}'>${this.uncode(style,title)}</span> </h4>`);
  }
  /////////////////////////////////////////////////////////////////////
  /////////////////////////////////////////////////////////////////////
  ...
  fence_to_ink(style,ss,ssi){
    ...
    var s_svg = `\n<svg data-bundle='INK' data-label='${style.label}' width='${this.fix(p.width)}' xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' fill='currentColor' stroke='none' viewBox='0 0 ${this.fix(p.vw)} ${this.fix(p.vh)}' >${text}</svg>\n`;
    var frame = (style.frame)?`frame='box'`:``;
    var s_style = this.css('SUBTITLE');
  ...
  fence_to_canvas(style,ss,ssi){
    ...
    var imgsrc = itms.imgsrc||'';
    var { imgsrc, imgid } = this.to_request_image(imgsrc);
    var imgwidget = `<img style='position:absolute;visibility:hidden;' width='${p.width}' height='${p.height}' onload='canvas_onload(this)' src='${imgsrc}' data-canvasid='${imgid}-canvas' id='${imgid}' alt='${imgsrc}' />`;
    var canvaswidget = `<canvas style='position:relative;z-index:1;' width='${p.width}' height='${p.height}' onmouseenter="canvas_onmouseenter(this)" onmouseleave="canvas_onmouseleave(this)" onmousedown="canvas_onmousedown(this)" onmousemove="canvas_onmousemove(this)" onmouseup="canvas_onmouseup(this)" onfocusin="canvas_onfocusin(this)" onfocusout="canvas_onfocusout(this)" data-imgid='${imgid}' data-paletteid='${imgid}-palette' data-colorinputid='${imgid}-colorinput' id='${imgid}-canvas' tabIndex='1' > </canvas>`;
    var palettewidget = `<div class='CANVASPALETTE' style='z-index:2;position:absolute;top:100%;left:0;visibility:hidden;' id='${imgid}-palette' ><input type='submit'/><input type='reset'/><input type='color' style='width:20px' value='#476fc7' onchange='canvas_oncolorchange(this)' data-canvasid='${imgid}-canvas' id='${imgid}-colorinput' /></div>`;
    p.subc = style.subtitle||'';
    p.subc = this.uncode(style,p.subc);
    p.style = style;
    var frame = (style.frame)?`frame='box'`:``;
    var s_style = this.css('SUBTITLE');
    var text = `<form class='CANVASFORM' action='/post' method='post' onreset='canvas_onreset(this)' onsubmit='canvas_onsubmit(this)' style='position:relative;' data-imgsrc='${imgsrc}' data-imgid='${imgid}' data-paletteid='${imgid}-palette' data-canvasid='${imgid}-canvas' > ${imgwidget} ${canvaswidget} ${palettewidget} </form>`
  ...
  fence_to_parbox(style,ss,ssi){
    ...
    if(width){
      width = `${width*this.MM_TO_PX}px`;
    }else{
      width = '';
    }
    text = `<span style='margin-left:auto;margin-right:auto;display:inline-block;width:${width};text-align:${talign};font-style:${fontstyle};font-family:${fontfamily};'> ${text} </span>`;
    var s_style = this.css('SUBTITLE');
  ...
  phrase_to_ref(style,cnt){
    ...
    if(label){
      if(this.idnum_map.hasOwnProperty(label)){
        ...
        return `<s style='${ref_css}' >${this.smooth(label)}</s>`;
      }
    }
    return "??";
  }
  ...
  phrase_to_url(style,cnt){
    return `<span class='URL' style='${this.css("URL")}'>${this.smooth(cnt)}</span>`
  }
  //////////////////////////////////////////////////////////////////////////////////////
  /// 
  /// translator function
  ...
  cove_to_text(style,cove){
    ...
    text = `<span style='display:inline-block;position:relative;padding-left:3em;'> <span style='position:absolute;left:2em;'> &#x25B8; </span> ${text} </span>`;
    let css = this.css('PARAGRAPH COVE');
    return `<p class='PARAGRAPH COVE' style='${css}' > ${text} </p>`;
  }
  ...
  float_to_paragraph(title,label,style,body,bodyrow){
    ...
    if(isbundle){
      ...
      var text = this.sand_to_text(style,sand);
    }else if(iscove){
  ...
  float_to_equation(title,label,style,ss,ssi){
    ...
    itms.forEach((p,j,arr) => {
      let ss = p.ss;
      ss = ss.slice(1,-1);
      let text = this.tokenizer.to_fence_math(style,ss);
      p.text = text;
    });
    ...
    itms.forEach((p,j,arr) => {
      ...
      var text = p.text;
      o.push(`<span style='margin-left:auto;margin-right:auto;display:block;position:relative;text-align:center;' >`)
      o.push(`${text}`);
      if(!idnum){
        idnum = idnum0;
      }
      o.push(`<span style='display:inline-block;position:absolute;right:0;' > (${idnum}) </span>`);
      o.push(`</span>`);
    })
    o.push(`</figure>`);
    return o.join('\n');
  ...
  float_to_figure(title,label,style,ss,ssi){
    ...
    }else if(style.wrapfig=='table'){ 
      return `<figure id='${label}' class='FIGURE' style='${css}' data-row='${style.row}' > ${this.to_caption_text(style,title,idnum,idcap,'Figure')} ${text} </figure>`;
    }else{
      return `<figure id='${label}' class='FIGURE' style='${css}' data-row='${style.row}' > ${this.to_caption_text(style,title,idnum,idcap,'Figure')} ${text} </figure>`;
    }
  }
  ...
  float_to_column(title,label,style,ss,ssi){
    ...
    var n = row.length;
    var css = this.css('COLUMN');
    var css_columnrow = this.css('COLUMNROW');
    all.push(`<figure class='COLUMN' style='${css}' data-row='${style.row}' >`);
  ...
  float_to_vspace(title,label,style,ss,ssi){
    var all = [];
    all.push('');
    all.push(`<div style='height:${title}em'> </div>`);
    return all.join('\n');
  }
}
