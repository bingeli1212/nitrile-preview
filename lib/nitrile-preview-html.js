...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  do_PRIM(block) {
    ...
    var text = `<div class='PRIM' style='${css}' > ${text} </div>`;
    return text;
  }
  do_PARA(block){
  ...
  fence_to_diagram(ss,style){
    ...
    if(style.animation){
      var env_array = this.range_total_to_env_array(style.range,style.animation);
      var o = [];
  ...
  fence_to_animation(ss,style){
    ...
    var env_array = this.range_total_to_env_array(style.range,style.total);
    var o = [];
    for(let env of env_array) {
      var { s, vw, vh, text } = this.diagram.to_diagram(ss,style,env);
      if(text){
  ...
  fence_to_colorbutton(ss,g){
    ...
    if(s){
      ...
      var { s, vw, vh, text } = this.diagram.to_diagram(ss,g,env);
      var css = '';
      css += `box-sizing:border-box;`;
      css += `text-indent:0;`;
      css += `vertical-align:middle;`;
  ...
  float_to_figure(title,ss,g){
    ...
    itms.forEach((p,j,arr) => {
      ...
      if(id=='diagram' || id=='framed' || id=='img'){
        seq++;
        p.sub = `(${this.to_a_letter(seq)}) ${this.uncode(p.sub,g)}`;
