...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  phrase_to_displaymath (str,style) {  
    var {s} = this.tokenizer.to_svgmath(str,style,1);
    return s;
  }
  phrase_to_uri(cnt) {
  ...
  itms_to_bull(itms,style){
    ...
    let pp = itms.forEach((p,i,arr) => {
      ...
      if(p.type=='UL'){
        d.push(`<label class='ITEM'>${bullet}${nbsp}${text}</label>`);
      }
      else if(p.type=='OL'){
        d.push(`<label class='ITEM'>${i+1}.${nbsp}${text}</label>`);  
      }
      else if(p.type == '0'){
        if(!p.ending) p.ending = '.';
        d.push(`<label class='ITEM'>${value}${p.ending}${nbsp}${text}</label>`);
      }
      else if(p.type == 'a'){
        if(!p.ending) p.ending = '.';
        d.push(`<label class='ITEM'>${this.to_a_letter(value)}${p.ending}${nbsp}${text}</label>`);
      }
      else if(p.type == 'A'){
        if(!p.ending) p.ending = '.';
        d.push(`<label class='ITEM'>${this.to_A_letter(value)}${p.ending}${nbsp}${text}</label>`);
        ...
        d.push(`<label class='ITEM'><b>${value}</b><br/>${text}</label>`)
      }
      else if(p.type == 'LL'){
        d.push(`<label class='ITEM'>${text}</label>`);
      }
      else if(p.type == 'CB'){
        if(!p.bull.trim().length){
          d.push(`<input type='checkbox' class='ITEM'>${text}</input>`);
        }else{
          d.push(`<input type='checkbox' class='ITEM' checked>${text}</input>`);
        }
      }
      else {
        d.push(`<label class='ITEM'>${text}</label>`);
    ...
    return `<form style='font-size:${this.string_to_fontsize_percent(style.fontsize)}' >${d.join('<br/>')}</form>`;
  }
