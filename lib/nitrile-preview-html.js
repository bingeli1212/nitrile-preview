    ...
    function board_onmousemove(evt){
      ...
      if(ctx.mydown&&evt.buttons){
        ...
        //erase
          ...
          ctx.myposx = evt.offsetX*sx;
          ctx.myposy = evt.offsetY*sy;
          ctx.clearRect(ctx.myposx,ctx.myposy,6,6);
        }else{
          ctx.beginPath();
          ctx.lineWidth = 3;
          ctx.lineCap = "round";
          ctx.strokeStyle = "black";
          ctx.moveTo(ctx.myposx,ctx.myposy);
          let sx = canvas.getAttribute("width")/canvas.getBoundingClientRect().width;
          let sy = canvas.getAttribute("height")/canvas.getBoundingClientRect().height;
          ctx.myposx = evt.offsetX*sx;
          ctx.myposy = evt.offsetY*sy;
          ctx.lineTo(ctx.myposx,ctx.myposy);
          ctx.stroke();
        }
      }else{
        ctx.mydown = 0;
    ...
    function board_onmousedown(evt){
      ...
      if(1){
        ...
        //erase
          ...
          ctx.lineCap = "round";
          ctx.fillStyle = "black";
          ctx.fillRect(ctx.myposx-1.5,ctx.myposy-1.5,3,3);
        }
      }
    }
