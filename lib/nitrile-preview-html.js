...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  fence_to_tabular(style,ss,ssi) {
    ...
    if(style.head && rows.length){
      ...
      pp.forEach((p,i) => {
        let {raw,text} = p;
        let css = '';
        css += `padding:0.2em 0.3em; text-align:${ww[i].ta}`;
        if(pp.top){
  ...
  float_to_equation(title,label,style,ss,ssi){
    ...
    var text = `<div class='PARAGRAPH EQUATION' style='position:relative;' > ${span} ${text} </div>`;
    return text;
  }
  float_to_listing(title,label,style,ss,ssi){
    ...
    var ss = ss.map((s,i,arr) => {
      ...
      all.push(`<div class='LINE' data-row='${ssi+i}' style='--lineno:"${i}";' ><code style='white-space:pre;'>${s}</code></div>`);
    });
    var text = all.join('\n');
    var text = `<div class='BODY' data-row='${ssi}' > ${text} </div>`;
  ...
  float_to_columns(title,label,style,ss,ssi){
    ...
    var n = row.length;
    all.push(`<div class='PARAGRAPH COLUMNS' style='column-count:${n}' data-row='${style.row}' >`);
    for(let i=0; i < n; ++i){
      let p = row[i];
  ...
  plst_to_text(style,itemize){
    ...
    return `<div class='PARAGRAPH ITEMIZE' data-row='${style.row}' > ${text} </div>`;
  }
  ////////////////////////////////////////////////////////////////////////////////
  //
  //
  ...
  cove_to_text(style,cove){
    var o = [];
    o.push(`<ul class='UL' style='padding-left:1.0em;list-style-type:none;'>`)
    o.push(`<li class='LI' data-row='${style.row}' style='--bull:"&#x25B8;";--pad:-1.0em;' >`);
    var n = cove.length;
    var p0 = cove[0];
    ...
    o.push('</ul>')
    var text = o.join('\n')
    return `<div class='PARAGRAPH INDENTING' data-row='${style.row}' > ${text} </div>`;
  }
  ////////////////////////////////////////////////////////////////////////////////
  //
  //
  ...
  samp_to_text(style,samp){
    ...
    var text = o.join('\n');
    return(`<div class='PARAGRAPH SAMPLE' data-row='${style.row}' > ${text} </div>`);
  }
  ////////////////////////////////////////////////////////////////////////////////
  //
  //
  ...
  prim_to_text(style,title,body){
    ...
    return `<div class='PARAGRAPH PRIMARY' data-row='${style.row}' > ${title} ${text} </div>`;
  }
  ////////////////////////////////////////////////////////////////////////////////
  //
  //
  ...
  norm_to_text(style,body){
    var text = this.untext(style,body).trim();
    return `<div class='PARAGRAPH NORMAL' data-row='${style.row}' > ${text} </div>`;
  }
