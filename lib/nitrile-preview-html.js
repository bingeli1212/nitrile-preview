...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
    ...
    if(style.subfigure){
      ...
      itms.forEach( (p,i,arr) => {
        ...
        if(type=='framed'){
          ...
          p.img = this.to_subfigure_html(this.fence_to_framed(ss,g),p.sub,g.width,style);
        }else if(type=='diagram'){
          p.sub = this.uncode(p.sub,style);
          seqno++;
          p.sub = `(${this.to_a_letter(seqno)}) ${p.sub}`;
          p.img = this.to_subfigure_html(this.fence_to_diagram(ss,g),p.sub,g.width,style);
        }else if(type=='img'){
          p.sub = this.uncode(p.sub,style);
          seqno++;
          p.sub = `(${this.to_a_letter(seqno)}) ${p.sub}`;
          p.img = this.to_subfigure_html(this.fence_to_img(ss,g),p.sub,g.width,style);
        }else{
          p.img = '<br/>'
        }
      });
  ...
  to_subfigure_html(cnt,cnt2,width,style){
    var css1 = '';
    css1 += `text-align:center;`
    css1 += `width:${width};`
    css1 += this.to_css_fontsize(style);
    var css2 = '';
    css2 += `text-align:center;`
    css2 += `width:${width};`
    css2 += this.to_css_fontsize(style);
    css2 += `padding-top:${this.padding_sub_top};`
    css2 += `padding-bottom:${this.padding_sub_bottom};`
    var d = [];
    d.push(`<span style="display: table;">`);
      d.push(`<span style="display: table-row;">`);
        d.push(`<span style="display: table-cell;${css1}">`);
          d.push(`${cnt}`);
        d.push(`</span>`);
      d.push(`</span>`);
      d.push(`<span style="display: table-row;">`);
        d.push(`<span style="display: table-cell;${css2}">`);
          d.push(`${cnt2}`);
        d.push(`</span>`);
      d.push(`</span>`);
    d.push(`</span>`);
