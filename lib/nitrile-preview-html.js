...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  float_to_figure(title,label,style,splitid,body,bodyrow){
    ...
    itms.forEach((p,j,arr) => {
      if(p.type=='bundle'){
        if(p.img){
          let fig = this.img_to_fig(p.style,p.img,p.subtitle);
          onerow.push(fig);
        }
      }else if(p.type=='\\\\'){
        let n = onerow.length;
  ...
  img_to_fig(style,img,subtitle){
    return `\n<table class="COLUMN" style="${this.css("COLUMN")}" cellpadding="0" cellspacing="0" data-row="${style.row}" >\n<caption align="bottom" class="SUBTITLE" style="${this.css('SUBTITLE')}" >${subtitle}</caption>\n<colgroup/><thead/><tbody><tr><td>${img}</td></tr></tbody>\n</table>`;
  }
  ss_to_ink(style,ss,ssi){
    ...
    itm.img = `<img style="border:${border};width:${width}mm;height:${height}mm" src="${imgsrc}" id="${imgid}" onload="img_onload(this)"/>`;
    itm.tab = '';
    itm.style = style;
    itm.subtitle = subtitle;
    itm.width = width;
