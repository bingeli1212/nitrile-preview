...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  fence_to_img(style,ss,ssi){
    ...
    if(type){
      ...
      var text = `<img style='position:relative;visibility:visible;width:${p.width}mm;height:${p.height}mm;' width='${p.vw}' height='${p.vh}' src='${imgsrc}' data-canvasid='${imgid}-canvas' id='${imgid}' alt='${imgsrc}' />`;
    }
    p.subc = style.subtitle||'';
    p.subc = this.uncode(style,p.subc);
    p.style = style;
