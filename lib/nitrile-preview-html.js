...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  body_to_argumentlist(style,ss){
    ...
    ss = ss.filter(v => (v!==null));
    ss = ss.map(v => `${this.icon_bullet} ${this.uncode(style,v[1])} ${this.uncode(style,v[2])}`);
    return ss.join('<br/>\n');
  }  
  /////////////////////////////////////////////////////////////////////
  //
  ...
  item_dl_to_text(style,i,item,nblank){
    ...
    if(item.type=='verb'){
      value = this.polish(style,item.value);
      value = `<code class='CODE'>${value}</code>`;
    }
    ...
    step.forEach((p,i,arr) => {
      if(i==0){
        if(p.value=='+'){
          ...
          all.push(`<dt class='DETAILS DT' style='${this.css("DETAILS DT")}'>${this.uncode(style,p.term)}</dt>`);
