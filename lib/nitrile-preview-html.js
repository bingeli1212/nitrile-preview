  ...
  itms_to_bull(itms,style){
    ...
    if(itms.length){
      let pp = itms.map((p,i,arr) => {
        ...
        if(p.type=='UL'){
          return `<div class='ITEM ${CB}'>${bullet}${cb}${nbsp}${text}</div>`;
        }
        if(p.type=='OL'){
          return `<div class='ITEM ${CB}'>${i+1}.${cb}${nbsp}${text}</div>`  
        }
        if(p.type == '0'){
          return `<div class='ITEM ${CB}'>${value}${p.ending}${cb}${nbsp}${text}</div>`;
        }
        if(p.type == 'a'){
          return `<div class='ITEM ${CB}'>${this.to_a_letter(value)}${p.ending}${cb}${nbsp}${text}</div>`;
        }
        if(p.type == 'A'){
          return `<div class='ITEM ${CB}'>${this.to_A_letter(value)}${p.ending}${cb}${nbsp}${text}</div>`;
        }
        if(p.type == 'DL'){
          return `<div class='ITEM ${CB}'><b>${value}${cb}</b><br/>${text}</div>`;
        }
        return `<div class='ITEM ${CB}'>${cb}${text}</div>`
      });
      return `<div>${pp.join('\n')}</div>`;
    }
    return `<div></div>`;  
