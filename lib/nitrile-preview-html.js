...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  hdgs_to_part(title,label,style){
    ...
    var leader = `Part ${style.__partnum}`;
    if(this.parser.data['suppress-part-enumeration']){
      leader = '';
    }
    return(`<h1 id='${label}' data-row='${style.row}' class='${cls}' style='${css}' > ${leader} ${this.uncode(style,title)} </h1>`);
  }
  /////////////////////////////////////////////////////////////////////
  //
  // translator function
  ...
  hdgs_to_chapter(title,label,style){
    const cls = ('CHAPTER');
    const css = this.css(cls);
    var leader = `Chapter ${style.__chapnum}`;
  ...
  hdgs_to_section(title,label,hdgn,level,style){
    ...
    if(this.parser.data['center-section']){
      css += ';text-align:center';
    }
    if(hdgn==1){
      return(`<h2 id='${label}' data-row='${style.row}' class='SECTION' style='${this.css("SECTION")}${css}' > ${leader} <span title='${this.polish(style,style.__note)}:${1+style.row}'>${this.uncode(style,title)}</span> </h2>`);
    }else if(hdgn==2){
      return(`<h3 id='${label}' data-row='${style.row}' class='SECTION' style='${this.css("SECTION")}${css}' > ${leader} <span title='${this.polish(style,style.__note)}:${1+style.row}'>${this.uncode(style,title)}</span> </h3>`);
    }else{
      return(`<h4 id='${label}' data-row='${style.row}' class='SECTION' style='${this.css("SECTION")}${css}' > ${leader} <span title='${this.polish(style,style.__note)}:${1+style.row}'>${this.uncode(style,title)}</span> </h4>`);
    }
  }
  //////////////////////////////////////////////////////////////////////////
  //
  ...
  fence_to_tab(style,ss,ssi) {
    ...
    itm.tab  = `<table class='TAB' style='margin:0;page-break-inside:avoid;width:${width_css};table-layout:fixed;' ${frame} ${rules} cellpadding='0' cellspacing='0' data-row='${style.row}' >${text}</table>`;
    return itm;
  }
  ...
  fence_to_vtm(style,ss,ssi){
    ...
    var itm = {};
    itm.ss = ss;
    itm.par = `<p style='margin:0;page-break-inside:avoid;font-size:${textsmaller};' data-row='${style.row}' >${text}</p>`;
    return itm;
  }
  /////////////////////////////////////////////////////////////////////
  /// 
  ...
  float_to_preformatted(title,label,style,splitid,body,bodyrow){
    ...
    var text = ss.join('\n');
    return`<pre class='PREFORMATTED' style='${this.css("PREFORMATTED")};font-size:${textsmaller};' data-row='${style.row}' >${text}</pre>`;
  }
    ...
    step.forEach((p,i,arr) => {
      if(i==0){
        ...
        if(this.body_is_bundle(p.lines)){
          ...
          let lines = this.body_to_body(p.lines,this.re_body_is_examp).map(x=>`${this.icon_dag}&#160;${this.uncode(style,x)}`);
          ...
          var text = lines.join('<br/>\n')
        }else if(this.re_body_is_lline.test(p.lines[0])){
          let lines = this.body_to_body(p.lines,this.re_body_is_lline).map(x=>`${this.uncode(style,x)}`);
          var text = lines.join('<br/>\n')
        }else{
