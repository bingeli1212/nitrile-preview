...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  fence_to_img(style,ss,ssi){
    ...
    if(type=='canvas'){
      ...
      var text = `<form class='CANVASFORM' action='/action.php' method='post' onreset='form_onreset(this)' onsubmit='form_onsubmit(this)' style='position:relative;' data-imgid='${imgid}' > ${imgwidget} ${canvaswidget} ${palettewidget} </form>`
    }else{
      var imgwidget = p.img;
