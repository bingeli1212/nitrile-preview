...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  fence_to_tab(g,ss) {
    ...
    var ww = this.string_to_template_array(g.template);
    var rows = this.ss_to_tab_rows(ss,ssi,ww.length,g);
    rows.forEach((pp,j) => {
      pp.forEach((p,i) => {
    ...
    var textsmaller = '';
    this.string_to_array(g.text).forEach(s => {
      if(s=='smaller'){
        textsmaller = 'smaller';
      }
    });
  ...
  fence_to_par(g,ss){
    ...
    var itm = {};
    itm.par  = `<p style='margin:0;page-break-inside:avoid;text-align:${textalign};font-size:${textsmaller};font-style:${textoblique}' data-row='${g.row}' >${text}</p>`;
    return itm;
  }
  /////////////////////////////////////////////////////////////////////
  /// 
