...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  hdgs_to_section(title,label,partnum,chapternum,level,style){
    ...
    return(`<h2 row='${style.row}' class='${css}'> ${level} &#160; ${this.uncode(style,title)} </h2>`);
  }
  hdgs_to_subsection(title,label,partnum,chapternum,level,style){
    ...
    this.addto_refmap(label,`${level}`);
    return(`<h3 row='${style.row}' class='${css}'> ${level} &#160; ${this.uncode(style,title)} </h3>`);
  }
  hdgs_to_subsubsection(title,label,partnum,chapternum,level,style){
    ...
    this.addto_refmap(label,`${level}`);
    return(`<h4 row='${style.row}' class='${css}'> ${level} &#160; ${this.uncode(style,title)} </h4>`);
  }
  /////////////////////////////////////////////////////////////////////
  //
  /////////////////////////////////////////////////////////////////////
  ...
  item_ol_to_text(style,i,item,nblank){
    ...
    if(item.type == 'A'){
      o.push(`<li class='${li_cls}' type='A' value='${item.value}'> ${text}`)
    }else if(item.type == 'a'){
      o.push(`<li class='${li_cls}' type='A' value='${item.value}'> ${text}`)
    }else if(item.type == 'I'){
      o.push(`<li class='${li_cls}' type='I' value='${item.value}'> ${text}`)
    }else if(item.type == 'i'){
      o.push(`<li class='${li_cls}' type='i' value='${item.value}'> ${text}`)
    }else if(item.value) {
      o.push(`<li class='${li_cls}' value='${item.value}'> ${text}`);
    }else if(item.bullet=='*'){
      let value = i+1;
      o.push(`<li class='${li_cls}' value='${value}'> ${text}`);
    }else{
      o.push(`<li class='${li_cls}' type='I' value='${item.value}'> ${text}`);
    }
    if(item.more && item.more.length){
  ...
  item_ul_to_text(style,i,item,nblank){
    ...
    if(key){
      if(keytype=='var'){
        o.push(`<li class='${li_cls}'> ${this.literal_to_var(style,key)}${sep}${text}`);
      }else if(keytype=='verb'){
        o.push(`<li class='${li_cls}'> ${this.literal_to_verb(style,key)}${sep}${text}`);
      }else{
        o.push(`<li class='${li_cls}'> ${this.polish(style,key)}${sep}${text}`);
      }
    }else{
      o.push(`<li class='${li_cls}'> ${text}`);
    }
    if(item.more && item.more.length){
      item.more.forEach((p) => {
        if(p.lines){
  ...
  fence_to_displaymath(style,ss) {
    ...
    var displaymath_cls = ('DISPLAYMATH');
    if(style.float){
      /// return text
    }else{
      var text = `<div class='${displaymath_cls}' > ${text} </div>`;
    }
    var o = [];
    o.push({img:text,subc:'',stretch:1,subfigure:text});
    return o;
  }
  fence_to_ink(style,ss){
    ...
    var subc = '';
    if(style.stretch){
      var subfigure = `\n<table cellpadding='0' cellspacing='0' class='SUBFIGURE' width='100%' >\n<caption align='bottom' class='SUBCAPTION'>${subc}</caption>\n<tr><td>${img}</td></tr>\n</table>`;
    }else{
      var subfigure = `\n<table cellpadding='0' cellspacing='0' class='SUBFIGURE' width='${w}' >\n<caption align='bottom' class='SUBCAPTION'>${subc}</caption>\n<tr><td>${img}</td></tr>\n</table>`;
    }
    o.push({img,subc,w,h,stretch,subfigure});
    return o;
  }
  fence_to_dia(style,ss){
    ...
    o = o.map((p) => {
      ...
      if (style.stretch) {         
        ...
        css += `; width:${width}mm`;
        css += `; height:${width*(vh/vw)}mm`;
        var w = width*this.MM_TO_PX;
        var h = width*(vh/vw)*this.MM_TO_PX;
      } else if (style.height) {
        css += `; width:${height*(vw/vh)}mm`;
        css += `; height:${height}mm`;
        var w = vw/vh*height*this.MM_TO_PX;
        var h = height*this.MM_TO_PX;
      } else {
        css += `; width:${p.viewport_width*p.viewport_unit}mm`;
        css += `; height:${p.viewport_height*p.viewport_unit}mm`;
        var w = p.viewport_width*p.viewport_unit*this.MM_TO_PX;
        var h = p.viewport_height*p.viewport_unit*this.MM_TO_PX
      }
      let s_svg = `<svg class='DIA' style="${css}" width='${w}' height='${h}' xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="currentColor" stroke="currentColor" viewBox="0 0 ${this.fix(p.vw)} ${this.fix(p.vh)}" >${p.svgtext}</svg>`;
  ...
  fence_to_verbatim(style,ss){
    ...
    var img = `<table ${frame} class='VERBATIM' style='${css}'> <tr><td>${img}</td></tr> </table>`;
    var o = [];
    var subc = '';
    o.push({img,subc,stretch:1,subfigure:img});
    return o;
  }
  fence_to_center(style,ss){
    ...
    var css = '';
    var text = `<div class='CENTER' style='${css}' > ${text} </div>`;
    var o = [];
    o.push({img:text,subc:'',stretch:1,subfigure:text});
    return o;
  }
  fence_to_flushright(style,ss){
    ...
    var text = this.uncode(style,text);
    var css = '';
    var text = `<div class='FLUSHRIGHT' style='${css}' > ${text} </div>`;
    var o = [];
    o.push({img:text,subc:'',stretch:1,subfigure:text});
    return o;
  }
  fence_to_flushleft(style,ss){
    ...
    var text = this.uncode(style,text);
    var css = '';
    var text = `<div class='FLUSHLEFT' style='${css}' > ${text} </div>`;
    var o = [];
    o.push({img:text,subc:'',stretch:1,subfigure:text});
    return o;
  }
  fence_to_tabular(style,ss) {
    ...
    var img = `<table ${attrs.join(' ')} class='TABULAR'>\n${colgroup}\n${text}\n</table>`;
    var subfigure = `<table ${attrs.join(' ')} class='TABULAR'>\n<caption align='bottom' class='SUBCAPTION'>${subc}</caption>\n${colgroup}\n${text}\n</table>`;
    var o = [];
    var stretch = style.stretch;
    o.push({img,subc,stretch,subfigure});
    return o;
  }
  fence_to_verse(style,ss) {
    ...
    var text = d.join('\n')
    var text = `<div class='VERSE'> ${text} </div>`;
    var o = [];
    o.push({img:text,subc:'',stretch:1,subfigure:text});
    return o;
  }
  ...
  float_to_figure(title,label,style,ss){
    ...
    if(style.wrap){
      ...
      var text = all.join('\n');
      var text = `<div class='PARAGRAPH' > ${text} </div>`;
      return text;
    }
    else{
      itms.forEach((p,j,arr) => {
        if(p.type=='bundle'){
          //let img = this.to_subfigure_img(p.img,p.subc,p.w,p.stretch,p.seq,style.subfigure);
          let img = p.subfigure;
          all.push(img);
        }else if(p.type=='\\\\'){
          all.push(`<br/>`);
  ...
  float_to_columns(title,label,style,ss){
    ...
    rows.forEach((row) => {
      all.push(`<div class='PARA' style='columns:${n}'>`);
      all.push(row.join(sep))
      all.push(`</div>`);
    });
    var text = all.join('\n');
    return `<figure class='COLUMNS' > ${text} </figure>`;  
  }
  /////////////////////////////////////////////////////////////////////
  /// 
  /// 
  ...
  to_wraprightfigure_img(img){
    var img = `<table class='WRAPRIGHT'><tr><td>${img}</td></tr></table>`
    return img;
  }
  to_wrapleftfigure_img(img){
    var img = `<table class='WRAPLEFT'><tr><td>${img}</td></tr></table>`
    return img;
  }
  __to_subfigure_img__(img,sub,w,stretch,seq,subfigure){
    let o = [];
    if(stretch){
      o.push(`<table cellpadding='0' cellspacing='0' class='SUBFIGURE' width='100%' >`);
    }else if(w){
      o.push(`<table cellpadding='0' cellspacing='0' class='SUBFIGURE' width='${w}' >`);
    }else{
      o.push(`<table cellpadding='0' cellspacing='0' class='SUBFIGURE' >`);
    }
    if(subfigure){
      sub = `(${this.int_to_letter_a(seq)}) ${sub}`;
    }
    if(sub){
      o.push(`<caption align='bottom' class='SUBCAPTION'>${sub}</caption>`);
    }
  ...
  phrase_to_colorbutton(style,cnt){
    return `<button type='button' style='background-color:${cnt};border:none;outline:1px solid black;display:inline-block;padding:0.5em 1.0em;'></button>`;
  }
