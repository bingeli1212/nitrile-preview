...
class NitrilePreviewReport extends NitrilePreviewLatex {
  ...
  to_document(){
    ...
    var landscape = this.parser.data['landscape']?'landscape':'';
    var twocolumn = this.parser.data['twocolumn']?'twocolumn':'';
    var article   = this.parser.data['article']?'article':'';
    var titlelines = [];
    ...
    var center_section_lines = [];
    if(this.parser.data['center-section']){
      center_section_lines.push(`\\renewcommand\\secheadstyle{\\centering\\Large\\normalfont\\noindent}`);
    }
    return `\
% !TEX program = upLatex 
\\let\\printglossary\\relax
\\documentclass[dvipdfmx,10pt,${landscape},${twocolumn},${article}]{memoir}
