...
class NitrilePreviewContex extends NitrilePreviewTranslator {
  ...
  constructor(parser) {
    ...
    this.contex_interlinespace_verbatim = 'line=8pt';
    this.contex_interlinespace_small = 'line=9pt';
  ...
  ///
  /////////////////////////////////////////////////////////////////////////////
  hdgs_to_part(title,label,idnum,idcap,partnum,chapternum,pagenum,level,style){
    var o = [];
  ...
  hdgs_to_chapter(title,label,idnum,idcap,partnum,chapternum,pagenum,level,style){
    var o = [];
    var raw = this.revise(title);    
    var title = this.uncode(style,title);
    var label = label||`chapternum.${chapternum}`;
    o.push('');
    o.push(`\\startchapter[title={${idnum} ~ ${title}},reference={${label}},bookmark={${raw}}]`);
    return o.join('\n')
  }
  hdgs_to_section(title,label,idnum,idcap,partnum,chapternum,pagenum,level,style){
    var o = [];
    var raw = this.revise(title);    
    var title = this.uncode(style,title);
    var label = label||`chapternum.${chapternum}.${level}`;
    o.push('');
    o.push(`\\startsection[title={${idnum} ~ ${title}},reference={${label}},bookmark={${raw}}]`);
    return o.join('\n')
  } 
  hdgs_to_subsection(title,label,idnum,idcap,partnum,chapternum,pagenum,level,style){
    var o = [];
    var raw = this.revise(title);    
    var title = this.uncode(style,title);
    var label = label||`chapternum.${chapternum}.${level}`;
  ...
  hdgs_to_subsubsection(title,label,idnum,idcap,partnum,chapternum,pagenum,level,style){
    var o = [];
    var raw = this.revise(title);    
    var title = this.uncode(style,title);
    var label = label||`chapternum.${chapternum}.${level}`;
  ...
  fence_to_verbatim(style,ss,ssi){
    ...
    var img = `\\framed[frame=off,location=bottom,strut=no]{${text}}`
    var frm = `\\framed[frame=off,location=top,   strut=no]{${text}}`
    img = `\\small\\setupinterlinespace[${this.contex_interlinespace_verbatim}]${img}`
    frm = `\\small\\setupinterlinespace[${this.contex_interlinespace_verbatim}]${frm}`
    var subc = style.subtitle||'';
    var subc = this.uncode(style,subc);
    var o = [];
  ...
  fence_to_tabular(style,ss,ssi){
    ...
    if(1){
      img = `{\\small\\setupinterlinespace[${this.contex_interlinespace_small}]${img}}`;
      frm = `{\\small\\setupinterlinespace[${this.contex_interlinespace_small}]${frm}}`;
    }
    var o = [];
    var subc = style.subtitle||'';
    var subc = this.uncode(style,subc);
  ...
  fence_to_parbox(style,ss,ssi) {
    ...
    if(1){
      text = `{\\small\\setupinterlinespace[${this.contex_interlinespace_small}] ${text}}`;
    }
    var img = text;
  ...
  ///
  ///////////////////////////////////////////////////////////////////////////////////////////////
  float_to_figure(title,label,idnum,idcap,pagenum,style,ss,ssi){
    ...
    if(style.type=='right'){
      ...
      o.push(`[here]`);
      o.push(`[${label}]`);
      o.push(`{${idcap} ${idnum} : ${caption}}`);
      o.push(`{\\startalignment[${s_align}] \\dontleavehmode ${text} \\stopalignment}`);
      return o.join('\n');
    }
  }
  float_to_equation(title,label,idnum,idcap,pagenum,style,ss,ssi){
  ...
  float_to_listing(title,label,idnum,idcap,pagenum,style,ss,ssi){
    ...
    if(this.contex_interlinespace_small){
    ...
    o.push('\\stoptabulate');
    var text = o.join('\n');
    text = `\\setupinterlinespace[${this.contex_interlinespace_verbatim}]${text}`
  ...
  float_to_longtabu(title,label,idnum,idcap,pagenum,style,ss,ssi) {
    var ss = this.ss_to_first_bundle(style,ss,ssi);
    var ss = ss.slice(1,-1);
  ...
  float_to_column(title,label,idnum,idcap,pagenum,style,ss,ssi){
    var itms = this.ss_to_figure_itms(style,ss,ssi);
    var row = [];
    itms.forEach((p,j,arr) => {
      if(p.type=='bundle'){
  ...
  float_to_page(title,label,idnum,idcap,pagenum,style,ss,ssi){
    return '';
  }
  ///
  ///
  ...
  to_preamble_body(){
    ...
    all.push(`\\setupinterlinespace[${this.contex_interlinespace}]`);
  ...
  to_preamble_essentials(){
    ...
    all.push(`\\setuphead[chapter][style=${this.context_style_chapter},number=no]`);
    all.push(`\\setuphead[section][style=${this.context_style_section},number=no]`);
    all.push(`\\setuphead[subsection][style=${this.context_style_subsection},number=no]`);
    all.push(`\\setuphead[subsubsection][style=${this.context_style_subsubsection},number=no]`);
    all.push(`\\setuphead[subsubsubsection][style=${this.context_style_subsubsubsection},number=no]`);
    all.push(`\\definefloat[listing][listings]`);
    all.push(`\\definedescription[latexdesc][`);
    all.push(`  headstyle=normal, style=normal, align=flushleft,`);
    all.push(`  alternative=hanging, width=fit, before=, after=]`);
  ...
  cove_to_text(style,cove){
    ...
    all.push('\\startlatexdesc{\\small{$\\blacktriangleright$}}');
    var n = cove.length;
    var p0 = cove[0];
    var pn = cove[n-1];
    if(n >= 2 && p0.text.startsWith('```') && pn.text.startsWith('```')){
      ...
      // \dontleavehmode
      // \startlatexdesc{\small{$\triangleright$}}
      // \small\setupinterlinespace[line=9pt]\framed[frame=off,location=top,   strut=no]{\starttabulate[|lT|][]
      // \NC {\texthash}include<stdio> \NC\NR
      // \NC int~main(){\textbraceleft} \NC\NR
      // \NC ~~return~0; \NC\NR
      // \NC {\textbraceright} \NC\NR
      // \stoptabulate}
      // \stoplatexdesc
      // \stopnarrower
      ...
      // \dontleavehmode
      // \startlatexdesc{\small{$\triangleright$}}
      // {\texthash}include<stdio> int main(){\textbraceleft} return 0; \crlf {\textbraceright} 
      // \stoplatexdesc
      // \stopnarrower
      let dd = cove.map((p,i,arr) => {
  ...
  cave_to_text(style,cave){
    ...
    if(n >= 2 && p0.text.startsWith('```') && pn.text.startsWith('```')){
      ...
      let text = dd.join(' ');
      text = `\\framed[frame=off,location=top,strut=no]{\\starttabulate[|cp|][]\\NC ${text} \\NC\\NR\\stoptabulate}`;
      all.push(text);
    }
  ...
  samp_to_text(style,samp){
    ...
    opts.push(`style=small`);
    s_interlinespace = `\\setupinterlinespace[${this.contex_interlinespace_verbatim}]`;
    let all = [];
    all.push('');
