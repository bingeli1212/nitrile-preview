...
class NitrilePreviewContex extends NitrilePreviewTranslator {
  ...
  fence_to_flushright(style,ss){
    ...
    var all = [];
    all.push('');
    all.push(`\\startalignment[flushright]`);
    all.push(text);
    all.push(`\\stopalignment`);
    return all.join('\n');
  }
  fence_to_lines(style,ss) {
    ...
    all.push('\\startlines');
    itms.forEach((p,i,arr) => {
      let crlf = (i+1==arr.length)?'':'\\crlf';
      if(p.item && p.hfill && p.latex){
        all.push(`${p.item} ~ ${p.latex} ${crlf}`)
      }else if(p.item && p.latex){
        all.push(`${p.item} ${p.latex} ${crlf}`)
      }else if(p.item){
        all.push(`${p.item} ${crlf}`)
      }else if(p.latex){
        all.push(`${p.latex} ${crlf}`)
      }else{
  ...
  ss_to_contex_externalfigure(ss,style){
    var fname = this.choose_contex_image_file(ss);
