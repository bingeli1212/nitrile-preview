...
class NitrilePreviewContex extends NitrilePreviewTranslator {
  ...
  float_to_alignright(title,label,style,subtitles,body,bodyrow){
    ...
    all.push('\\startalignment[flushright]\\dontleavehmode');
    all.push(text);
    all.push('\\stopalignment')
    return all.join('\n');
  }
  float_alignleft(title,label,style,subtitles,body,bodyrow){
    var bundles = this.body_to_all_bundles(style,body);
    var itms = bundles.map((bundle) => this.do_bundle(style,bundle));
    var d = [];
    var one = [];
  ...
  float_to_aligncenter(title,label,style,subtitles,body,bodyrow){
    var bundles = this.body_to_all_bundles(style,body);
    var itms = bundles.map((bundle) => this.do_bundle(style,bundle));
    var d = [];
    var one = [];
