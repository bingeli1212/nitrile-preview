...
class NitrilePreviewContex extends NitrilePreviewTranslator {
  ...
  fence_to_ink(style,ss){
    ...
    if(width && height){
      ...
      all.push(`currentpicture := currentpicture xsized (${inkwidth/inkheight*height}mm) ysized (${height}mm);`)
      var w = `${inkwidth/inkheight/height}mm`
    }else{
      var w = `${inkwidth}mm`
    }
    all.push(`\\stopMPcode`);
  ...
  fence_to_tabular(style,ss){
    ...
    if(1){
      ...
      var ww = this.halign_to_contex_width_and_align(n,style.halign);
      var all = [];
      ...
      if(1){
        ...
        ww.map((x, i) => {
          let {width,align} = x;
          if(width && align){
            all.push(`\\setupTABLE[c][${i + 1}][width=${width},align=${align}]`);
          }else if(align){
      ...
      if(1){
        ...
        if(style.small){
          d.push(`\\starttabulate[|${pcol}|][bodyfont=small]`);  
        }else{
        ...
        if(style.head && rows.length){
          let pp = rows.shift();
          if(style.frame==1 || style.frame=='box' || style.frame=='above' || style.frame=='hsides'){
            d.push('\\HL');
          }else if(pp.upper==2){
            d.push('\\HL');
            d.push('\\HL')
          ...
          var s = this.ss_to_contex_tabulate_row_s(myss,style);
          d.push(s);          
          ...
          var s = this.ss_to_contex_tabulate_row_s(myss,style);
          d.push(s);          
          if(style.rules=='rows'||style.rules=='all'){
            d.push('\\HL');
          }else if(d.lower==2){
            d.push('\\HL');
            d.push('\\HL');
        ...
        rows.forEach((pp,j,arr) => {
          ...
          d.push(s);
          if(j+1==arr.length){
            if(style.frame==1||style.frame=='box'||style.frame=='below'||style.frame=='hsides'){
              d.push('\\HL');
            }else if(pp.lower==2){
              d.push('\\HL');
              d.push('\\HL');
  ...
  fence_to_dia(style,ss){
    ...
    o.forEach((p) => {
      ...
      if(style.float=='right'){
        p.img = `\\placefigure[right,none][]{}{${p.img}}`;
      }else if(style.float=='left'){
        p.img = `\\placefigure[left,none][]{}{${p.img}}`;
      }
    });
    return o;
  }
  fence_to_img(style,ss) {
    ...
    ///getting ready to turn it into 'mp'
    let [viewport_width,viewport_height,viewport_unit,viewport_grid] = this.g_to_viewport_array(style);
    let xm = viewport_width;
    let ym = viewport_height;
    let unit = viewport_unit;
    let grid = viewport_grid;
    var [img,w] = this.to_mp_image(style,fname,xm,ym,unit,grid);
  ...
  __fence_to_verbatim(style,ss){
    ...
    var opts = [];
    opts.push(`unit=0.2em`);
    if(style.small){
      opts.push(`bodyfont=small`);
    }
    let all = [];
    opts.push(`indenting=no`);
    opts.push(`before={}`);
    opts.push(`after={}`);
    /// the indenting=yes allows the verbatim block to be indented if document indenting has been set
    all.push(`\\starttabulate[|T${p}|][${opts.join(',')}]`);
    ss.forEach((s) => {
      s = `\\NC ${s} \\NC\\NR`;
      all.push(s);
    });
  ...
  fence_to_flushleft(style,ss){
    ...
    d.push('');
    d.push('\\blank')
    d.push(`\\startalignment[flushleft]`);
    d.push(`${text}`);
    d.push(`\\stopalignment`);
    d.push('\\blank')
    var text = d.join('\n');
  ...
  fence_to_quote(style,ss){
    ...
    var all = [];
    all.push('\\startnarrower[1*left,1*right]');
    all.push(text);
  ...
  text_to_text(style,text){
    var text = text.trim();
    if(text.length){
      if(style.float=='itemize'){
        return `\\noindent{}${this.uncode(style,text)}`;
      }else{
  ...
  float_to_itemize(title,label,style,itemize){
    style = {...style,title,label,float:'itemize'};
    var all = [];
  ...
  float_to_figure(title,label,style,ss){
    ...
    let nocaption = this.assert_int(style.nocaption);
    let figalign = style.figalign;
  ...
  float_to_listing(title,label,style,ss){
    ...
    var o = [];
    o.push('\\starttabulate[|l|Tl|][bodyfont=small]');
    ss.forEach( (s,j,arr) => {
      s = this.polish_verb(style,s);
      if (!s) {
        s = "~";
  ...
  phrase_to_vspace(style,cnt){
    return `\\blank[${cnt}]`;
  }
  phrase_to_hspace(style,cnt){
  ...
  to_preamble_fonts(){
    ...
    all.push(`\\definefontfamily [dejavu] [serif] [dejavuserif]    [rscale=0.85]`);
    all.push(`\\definefontfamily [dejavu] [sans]  [dejavusans]     [rscale=0.85]`);
    all.push(`\\definefontfamily [dejavu] [mono]  [dejavusansmono] [rscale=0.85]`);
    all.push(`\\definefontfamily [dejavu] [math]  [dejavumath]     [rscale=0.85]`);
    all.push(`%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%`);
    all.push(`\\definefontfamily [office] [serif] [Times New Roman]`);
    all.push(`\\definefontfamily [office] [sans]  [Arial] [rscale=0.9]`);
  ...
  to_mp_image(style,fname,xm,ym,unit,grid){
    ...
    if (background=='grid') {
      let gridcolor = this.diagram.g_to_gridcolor_string(style);
      gridcolor = this.diagram.string_to_mp_color(gridcolor,0);
      all.push(`for i=0 step ${grid} until ${ym}: draw (0,i*u) --- (${xm}*u,i*u) withcolor ${gridcolor}; endfor;`);
      all.push(`for i=0 step ${grid} until ${xm}: draw (i*u,0) --- (i*u,${ym}*u) withcolor ${gridcolor}; endfor;`);
    } else {
