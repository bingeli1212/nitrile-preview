...
class NitrilePreviewContex extends NitrilePreviewTranslator {
  ...
  fence_to_tab(style,ss,ssi){
    ...
    if(style.frame=='hsides'){
      var tab = `\\framed[frame=off,topframe=on,bottomframe=on,location=bottom,strut=no]{${text}}`;
    }else if(style.frame){
      var tab = `\\framed[frame=on,location=bottom,strut=no]{${text}}`;
    }else{
      var tab = `\\framed[frame=off,location=bottom,strut=no]{${text}}`;
    }
    var subtitle = this.to_fig_subtitle(style);
    return({img,tab,subtitle,style});
  }
