...
class NitrilePreviewContex extends NitrilePreviewTranslator {
  ...
  fence_to_fml(style,ss,ssi) {
    ...
    var w = '';
    return({img,text,sss,width:w,subtitle,style});
  }
  fence_to_ink(style,ss,ssi){
    ...
    var subtitle = this.to_fig_subtitle(style);
    return({img,subtitle,width,height,style,text});
  }
  fence_to_dia(style,ss,ssi){
    ...
    let img = itm.img;
    if(style.frame){
      itm.img = `\\framed[frame=on,location=bottom,strut=no]{${img}}`
    }else{
      itm.img = `\\framed[frame=off,location=bottom,strut=no]{${img}}`
    }
  ...
  fence_to_tab(style,ss,ssi){
    ...
    var subtitle = this.to_fig_subtitle(style);
    var text = img;
    return({img,subtitle,style,text});
  }
  fence_to_par(style,ss,ssi) {
    ...
    return({img,subtitle,style,text:img});
  }
  fence_to_vtm(style,ss,ssi){
    ...
    var subtitle = this.to_fig_subtitle(style);
    return({img,ss,ss0,subtitle,style,text});
  }
  ///////////////////////////////////////////////////////////////////////////////////////////////
  ///
  ///
    ...
    if(!style.partition){
      ...
      itms.forEach((p,j,arr) => {
        if(p.type=='bundle'){
          ...
          o.push(`\\hbox{${s_interls}\\startcombination[1*1] {${p.img}} {${subtitle}} \\stopcombination}`);
        }else if(p.type=='\\\\'){
          o.push('\\\\');
        }
      });
  ...
  float_to_listing(title,label,style,bundles){
    ...
    if(bundles.length){
      ...
      ss = itm.ss0;//raw ss without encoding, used for \\starttyping
