    ...
    step.forEach((p,i) => {
      if(i==0){
        if(p.value=='+'){
          ...
          if(p.lines.length){
            all.push(`\\startitemize[itemalign=flushleft,leftmargin=0pt]`);   
            all.push(`\\sym{\\bf ${this.smooth(style,p.term)}}\\crlf\n${this.uncode(style,this.join_para(p.lines))}`);
          }else{
            all.push(`\\startitemize[itemalign=flushleft,leftmargin=0pt]`);   
            all.push(`\\sym{\\bf ${this.smooth(style,p.term)}}`);
          }
        }else{
          all.push(`\\startitemize[itemalign=flushright,leftmargin=0pt,width=15pt,distance=5pt]`);   
          all.push(`\\sym{${p.value}.} ${this.uncode(style,p.term)}`);
        }
      }else{
