...
class NitrilePreviewContex extends NitrilePreviewTranslator {
  ...
  hdgs_to_part(title,style){
    var o = [];
    var raw = title;
    var label = style.label||`partnum.${style.partnum}`;
  ...
  hdgs_to_chapter(title,style){
    var o = [];
    var raw = title;
    var label = style.label||`chapter.${style.chapnum}`;
    o.push('');
  ...
  hdgs_to_section(title,style){
    ...
    var label = style.label||`chapter.${style.chapnum}.${style.level}`;
    if(style.chapnum){
  ...
  fence_to_fml(style,ss,ssi) {
    ...
    var fig = img;
    var par = text;
    return({img,fig,tab,par,subtitle,style});
  }
  fence_to_ink(style,ss,ssi){
    ...
    var fig = img;
    var par = `\\starttabulate[|p|]\\NC ${img} \\NC\\NR\\stoptabulate`;
    var tab = `\\starttabulate[|p|]\\NC ${img} \\NC\\NR\\stoptabulate`;
    var subtitle = this.to_fig_subtitle(style);
    return({img,fig,par,tab,subtitle,style});
  }
  fence_to_dia(style,ss,ssi){
    var itm = this.diagram.to_diagram(style,ss);  
    itm.fig = itm.img;
    itm.par = `\\starttabulate[|p|]\\NC ${itm.img} \\NC\\NR\\stoptabulate`;
    itm.tab = `\\starttabulate[|p|]\\NC ${itm.img} \\NC\\NR\\stoptabulate`;
  ...
  fence_to_tab(style,ss,ssi){
    ...
    if(style.frame=='hsides'){
      var img = `\\framed[frame=off,topframe=on,bottomframe=on,location=bottom,strut=no]{${text}}`;
    }else if(style.frame){
  ...
  fence_to_par(style,ss,ssi) {
    ...
    let img = `\\framed[${opts.join(',')}]{${ss.join('\\crlf\n')}}`
    var subtitle = this.to_fig_subtitle(style);
    if(style.textalign=='c'){
      var text = `\\startalignment[center]\\startlines\n${ss.join('\\crlf\n')}\n\\stoplines\\stopalignment`;
    }else if(style.textalign=='r'){
      var text = `\\startalignment[flushright]\\startlines\n${ss.join('\\crlf\n')}\n\\stoplines\\stopalignment`;
    }else{
      var text = `\\startalignment[flushleft]\\startlines\n${ss.join('\\crlf\n')}\n\\stoplines\\stopalignment`;
    }
    var fig = img;
    var par = text;
    var tab = text;
  ...
  fence_to_vtm(style,ss,ssi){
    ...
    var text1 = all.join('\\\\\n');
    var text2 = all.map(s => `\\NC ${s} \\NC\\NR`).join('\n');
    var img = `\\framed[width=fit,frame=off,location=bottom,strut=no,align=right]{\\tt ${text1}}`
    var fig = img;
    var par = `\\starttabulate[|Tp|]\n${text2}\n\\stoptabulate`;
    var tab = `\\starttabulate[|Tp|]\n${text2}\n\\stoptabulate`;
    var subtitle = this.to_fig_subtitle(style);
    return({img,fig,par,tab,subtitle,style});
  }
  ///////////////////////////////////////////////////////////////////////////////////////////////
  ...
  ///
  ///////////////////////////////////////////////////////////////////////////////////////////////
  float_to_example(title,label,style,body,bodyrow){
    var ss = body.map((s) => this.uncode(style,s));
    var text = ss.join('\\crlf\n');
    var all = [];
    const bull = `\\small\\triangleright`;                    
    all.push('');
    all.push('\\blank');
  ...
  float_to_verbatim(title,label,style,body,bodyrow){
    let ss = body.map((s,j) => {
      let x = s;
      x = x.trimEnd();
      if (!x) {
  ...
  float_to_flushleft(title,label,style,body,bodyrow){
  ...
  float_to_center(title,label,style,body,bodyrow){
    var bundle = this.try_extract_bundle(style,body,bodyrow,0);
    var itm = this.do_bundle(style,bundle);
    var text = '';
    if(itm.img){
      text = `\\hfill${itm.img}\\hfill~`;
    }else{
      text = itm.tab;
    }
    var all = [];
  ...
  float_to_body(title,label,style,body,bodyrow){
  ...
  float_to_record(title,label,style,body,bodyrow){
  ...
  float_to_figure(title,label,style,body,bodyrow){
    ...
    if(!style.partition){
      ...
      itms.forEach((p,j,arr) => {
        if(p.type=='bundle'){
          ...
          o.push(`\\hbox{${s_interls}\\startcombination[1*1] {${p.fig}} {${subtitle}} \\stopcombination}`);
        }else if(p.type=='\\\\'){
          o.push('\\\\');
        }
      });
  ...
  float_to_equation(title,label,style,body,bodyrow){
    var bundles = this.body_to_all_bundles(style,body,bodyrow);
    ...
    bundles.forEach((bundle,j,arr) => {
      ...
      let text = `\\hfill${itm.img}\\hfill({${eqnnum}${sub}})`;
      all.push(text);
      all.push('\\godown[0pt]');
    })
  ...
  float_to_listing(title,label,style,body,bodyrow){
    var bundles = this.body_to_all_bundles(style,body,bodyrow);
    if(style.split){
      let all_splitids = this.string_to_int_array(style.splitids);
      let all_splitstarts = this.string_to_int_array(style.splitstarts);
      bundles.forEach((bundle,j) => bundle.splitid = all_splitids[j]);
      bundles.forEach((bundle,j) => bundle.splitstart = all_splitstarts[j]);
      bundles = bundles.filter((bundle,j) => bundle.splitid==style.splitid);
    }
    ...
    bundles.forEach((bundle,j,arr) => {
      ...
      o.push(`\\starttyping[start=${splitstart+1},numbering=line,space=fixed,margin=10pt]`);
      ss.forEach( (s) => {
        o.push(s);
      });
      o.push('\\stoptyping');
      ...
      if(style.pagenum){
        if(j>0){
          all.push('');
          all.push('\\page');
          all.push(`\\setcounter[userpage][${style.pagenum+j}]`)
        }
      }
      all.push('')
  ...
  float_to_table(title,label,style,body,bodyrow) {
    var bundles = this.body_to_all_bundles(style,body,bodyrow);
    if(style.split){
      let all_splitids = this.string_to_int_array(style.splitids);
      let all_splitstarts = this.string_to_int_array(style.splitstarts);
  ...
  float_to_columns(title,label,style,body,bodyrow){
    var bundles = this.body_to_all_bundles(style,body,bodyrow);
    ...
    itms.forEach((p,j,arr) => {
      ...
      let img = this.to_column_img(p.img,p.subtitle);
      all.push(img);
    });
    all.push(`\\stopcolumns`);
    return all.join('\n');
  }
  float_to_lines(title,label,style,body,bodyrow){
    var bundles = this.body_to_all_bundles(style,body,bodyrow);
    var align = 'flushleft';
    if(style.textalign=='c'){
      align='center';
    ...
    if(bundles.length) {
      ...
      if(hew>1){
        all.push(`\\defineparagraphs[para][n=${hew}]`);
        all.push(`\\startpara`);
        let k = Math.ceil(ss.length/hew);
        for(let i=0; i < hew; ++i){
          if(i>0){
            all.push(`\\para`);
          }
          all.push(`\\startlines`);
          let ss1 = ss.slice(i*k,(i+1)*k);
  ...
  float_to_itemize(title,label,style,body,bodyrow){
    var bundles = this.body_to_all_bundles(style,body,bodyrow);
    if(style.split){
      let all_splitids = this.string_to_int_array(style.splitids);
      let all_splitstarts = this.string_to_int_array(style.splitstarts);
  ...
  float_to_vspace(title,label,style,body,bodyrow){
    var all = [];
  ...
  to_subcaptioned_tabular(img,style,subcaptions){
    ...
    return `\\hbox{\\startcombination[1*1] ${img} {\\raggedright ${onerow.join(' \\crlf ')}} \\stopcombination}`;
  }
