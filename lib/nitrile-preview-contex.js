...
class NitrilePreviewContex extends NitrilePreviewTranslator {
  ...
  fence_to_img(style,ss) {
    ...
    if(stretch){
      opts.push(`width=${stretch}\\textwidth`);
      opts.push(`height=${stretch*(viewport_height/viewport_width)}\\textwidth`);
      ...
      opts.push(`width=${width}mm`);
      opts.push(`height=${width*(viewport_height/viewport_width)}mm`);
      // opts.push('keepaspectratio');
    }else if(height){
      opts.push(`height=${height}mm`);
      opts.push(`width=${height*(viewport_width/viewport_height)}mm`);
      // opts.push('keepaspectratio');
    }else{
      const stretch = 1;
      opts.push(`width=${stretch}\\textwidth`);
  ...
  fence_to_verbatim(style,ss){
    ...
    if(style.small){
      ...
      d.push(`\\starttabulate[|T${p}|][unit=0.2em,bodyfont=small,indenting=${indenting}]`);
    }else{
      d.push(`\\starttabulate[|T${p}|][unit=0.2em,indenting=${indenting}]`);
    }
    ss.forEach( (s,j,arr) => {
