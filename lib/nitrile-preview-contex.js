...
class NitrilePreviewContex extends NitrilePreviewTranslator {
  ...
  ww_to_context_starttable_pcol(ww,n){
    var ss = ww.map((p) => {
      if(p.width){
        return `p(${p.width-1}mm)`;//remove 1mm from the actual width for \starttable
    ...
    if(n > 2 && p0.text=='```' && pn.text=='```'){
      ...
      cove.forEach((p,i,arr) => {
        ...
        if (x.length==0) {
          ...
          x = String.fromCodePoint(0xAD)+x;//this is to allow for whitespaces to be preserved in case they appear at the beginning of a line
          x = `{\\tt ${x}}`;
          all.push(`${x}\\\\`);
        }
      })
