...
class NitrilePreviewContex extends NitrilePreviewTranslator {
  ...
  fence_to_img(style,ss) {
    ...
    if(stretch){
      var mywidth=(`${stretch}\\textwidth`);
      var myheight=(`${stretch*(viewport_height/viewport_width)}\\textwidth`);
  ...
  float_to_figure(title,label,style,ss){
    ...
    if(style.wrap){
      ...
      itms.forEach((p,j,arr) => {
        if(p.type=='bundle'){
          all.push(`\\hbox{\\startcombination[1*1] {${p.img}} {${p.sub}} \\stopcombination}`);
        }
      });
      var o = [];
      o.push('')
  ...
  float_to_table(title,label,style,ss){
    ...
    if(style.subfigure){
      ...
      itms.forEach((p,j,arr) => {
        if(p.type=='bundle'){
          all.push(`{\\startcombination[1*1] {${p.img}} {${p.sub}} \\stopcombination}`);
        }
      });
      var o = [];
      o.push('')
