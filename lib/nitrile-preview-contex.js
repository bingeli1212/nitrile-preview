...
class NitrilePreviewContex extends NitrilePreviewTranslator {
  ...
  to_symbol_doc(){
    if(1){
      ...
      for(let symbol in this.tokenizer.pjson.symbol){
        ...
        line = this.uncode(this.parser.style,line);
        body.push(line);
        body.push('');
      }
      var text = body.join('\n')
  ...
  item_dl_to_text(i,item,style,item_labels){
    var o = [];
    var text = this.uncode(style,item.text);
  ...
  item_hl_to_text(i,item,style){
    var o = [];
    o.push(`\\startHL{}${this.uncode(style,item.text)} \\\\`);
    if(item.more && item.more.length){
      item.more.forEach((p) => {
        let { lines, itemize } = p;
        if(lines){
  ...
  ///float_to_XXX
  ///
  float_to_list(title,label,style,body){
    var all = [];
    var text = this.fence_to_list(style,body);
    all.push('');
    all.push(text);
    return all.join('\n');
  }
