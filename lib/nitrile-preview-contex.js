...
class NitrilePreviewContex extends NitrilePreviewTranslator {
  ...
  fence_to_fml(style,ss,ssi) {
    ...
    var w = '';
    return({img,frm,text,sss,width:w,subtitle,style});
  }
  fence_to_ink(style,ss,ssi){
    ...
    var subtitle = style.subtitle||'';
    return({img,frm,subtitle,width,height,style,text});
  }
  fence_to_dia(style,ss,ssi){
    ...
    if(style.frame){
      ...
      itm.frm = `\\framed[frame=on,location=top   ,strut=no]{${img}}`
    }else{
      itm.img = `\\framed[frame=off,location=bottom,strut=no]{${img}}`
      itm.frm = `\\framed[frame=off,location=top   ,strut=no]{${img}}`
    }
    return itm;
  }
  ...
  fence_to_par(style,ss,ssi) {
    ...
    var frm = img.replace('location=bottom','location=top');
    var subtitle = style.subtitle||'';
    var text = ss.join('\n');
    if(style.textalign=='c'){
  ...
  fence_to_vtm(style,ss,ssi){
    ...
    var frm = `\\framed[width=fit,frame=off,location=top,   strut=no,align=right]{\\tt ${text1}}`
    if(style.fontsize=='small'){
    ...
    var subtitle = style.subtitle||'';
    return({img,frm,ss,ss0,subtitle,style,text});
  }
  ///////////////////////////////////////////////////////////////////////////////////////////////
  ///
  ///
  ...
  float_to_equation(title,label,style,bundles){
    ...
    bundles.forEach((bundle,j,arr) => {
      let itm = this.do_bundle(style,bundle,'fml')[0];
      let sss = itm.sss;
