...
class NitrilePreviewContex extends NitrilePreviewTranslator {
  ...
  fence_to_img(style,ss) {
    ...
    if(width && height){
      var mywidth=(`${width}`);
      var myheight=(`${height}`);
      unit = unit * (width/(viewport_width*viewport_unit));
    }else if(width){
      var mywidth=(`${width}`);
      ...
      var myheight=(`${height}`);
      unit = unit * (height/(viewport_height*viewport_unit));
    }else{
    ...
    var img = this.to_mp_image(style,fname,mywidth,myheight,fit,frame,xm,ym,unit,grid);
  ...
  float_to_paragraph(title,label,style,body){
    ...
    if(button_text){
      // var text = `\\fbox{${button_text}} ${text}`;
  ...
  float_to_ink(title,label,style,body){
    ...
    var all = [];
    all.push('');
    all.push(this.untext(style,body));
    return all.join('\n');
  }
  ...
  float_to_figure(title,label,style,ss){
    ...
    if(style.wrap){
      ...
      itms.forEach((p,j,arr) => {
        if(p.type=='bundle'){
          p.subc = `(${this.int_to_letter_a(j+1)}) ${p.subc}`
  ...
  //
  ////////////////////////////////////////////////////////////////////////////////
  to_mp_image(style,imagefile,width,height,fit,frame,xm,ym,unit,grid){
    var background = this.g_to_background_string(style);
    var all = [];
    all.push(`\\startMPcode`);
    all.push(`numeric vw; vw := ${width}mm;`);
    all.push(`numeric vh; vh := ${height}mm;`);
    all.push(`draw unitsquare sized(vw,vh) withcolor white ;`);
    all.push(`picture p ; p := externalfigure "${imagefile}" ;`);
