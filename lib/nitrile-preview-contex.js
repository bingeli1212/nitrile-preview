...
class NitrilePreviewContex extends NitrilePreviewTranslator {
  ...
  itemize_to_text(style,itemize){
    ...
    switch (bull) {
      ...
      case 'OL': {
        if(!nblank){
          o.push(`\\startitemize[n,packed][inbetween={}]`);
        }else{
          o.push(`\\startitemize[n][inbetween={}]`);
        }
        items.forEach((item,j) => {
      ...
      case 'UL': {
        if(!nblank){
          o.push(`\\startitemize[packed][inbetween={}]`);
        }else{
          o.push(`\\startitemize[][inbetween={}]`);
        }
        items.forEach((item,j) => {
  ...
  fence_to_quote(style,ss){
    ...
    var all = [];
    all.push('\\startquotation');
    all.push(text);
    all.push('\\stopquotation');
    var text = all.join('\n');
    var o = [];
    o.push({img:text,subc:''});
    return o;
