...
class NitrilePreviewContex extends NitrilePreviewTranslator {
  ...
  hdgs_to_section(title,label,level,style){
    ...
    var raw = title;
    var title = this.uncode(style,title);
    var label = label||`chapter.${chapnum}.${level}`;
    o.push('');
    o.push(`\\startsection[title={${idnum}   ${title}},reference={${label}},bookmark={${raw}}]`);
    return o.join('\n')
  } 
  hdgs_to_subsection(title,label,level,style){
    ...
    var label = label||`chapter.${chapnum}.${level}`;
    o.push('');
    o.push(`\\startsubsection[title={${idnum}   ${title}},reference={${label}},bookmark={${raw}}]`);
    return o.join('\n')
  } 
  hdgs_to_subsubsection(title,label,level,style){
    ...
    var raw = title;
    var title = this.uncode(style,title);
    var label = label||`chapnum.${chapnum}.${level}`;
    o.push('');
    o.push(`\\startsubsubsection[title={${idnum}   ${title}},reference={${label}},bookmark={${raw}}]`);
    return o.join('\n')
  }
  //////////////////////////////////////////////////////////////////////////
  ///
  ...
  fence_to_fml(style,ss,ssi) {
    ...
    var w = '';
    var o = [];
    o.push({img,frm,text,sss,width:w,subc,style});
    return o;
  }
  fence_to_ink(style,ss,ssi){
    ...
    var subc = this.uncode(style,subc);
    var o = [];
    o.push({img,frm,subc,width,height,style});
    return o;
  }
  ...
  fence_to_vtm(style,ss,ssi){
    ...
    var img = `\\framed[frame=off,location=bottom,strut=no]{${text}}`
    var frm = `\\framed[frame=off,location=top,   strut=no]{${text}}`
    img = `\\switchtobodyfont[${this.contex_bodyfontsize*0.9}pt]\\setupinterlinespace[line=${this.contex_bodyfontsize*0.9}pt]${img}`
    frm = `\\switchtobodyfont[${this.contex_bodyfontsize*0.9}pt]\\setupinterlinespace[line=${this.contex_bodyfontsize*0.9}pt]${frm}`
    var subc = style.subtitle||'';
    var subc = this.uncode(style,subc);
    var o = [];
  ...
  fence_to_tab(style,ss,ssi){
    ...
    if(1){
      img = `{\\switchtobodyfont[${this.contex_bodyfontsize*0.9}pt]\\setupinterlinespace[line=${this.contex_bodyfontsize*0.9*this.contex_bodylineheight}pt]${img}}`;
      frm = `{\\switchtobodyfont[${this.contex_bodyfontsize*0.9}pt]\\setupinterlinespace[line=${this.contex_bodyfontsize*0.9*this.contex_bodylineheight}pt]${frm}}`;
    }
    var o = [];
    var subc = style.subtitle||'';
    var subc = this.uncode(style,subc);
  ...
  fence_to_par(style,ss,ssi) {
    ...
    var text = `{\\switchtobodyfont[${this.contex_bodyfontsize*0.9}pt]\\setupinterlinespace[line=${this.contex_bodyfontsize*0.9*this.contex_bodylineheight}pt]${text}}`;
    var img = text;
  ...
  float_to_figure(title,label,style,ss,ssi){
    ...
    if(!style.partition){
      ...
      itms.forEach((p,j,arr) => {
        if(p.type=='bundle'){
          let subc = p.subc;
          o.push(`\\hbox{${s_interls}\\startcombination[1*1] {${p.img}} {${subc}} \\stopcombination}`);
        }else if(p.type=='\\\\'){
          o.push('\\\\');
        }
      });
  ...
  float_to_equation(title,label,style,ss,ssi){
    ...
    var idnum0 = style.idnum0;
    itms.forEach((p,j,arr) => {
      let ss = p.ss;
      ss = ss.slice(1,-1);
      let str = ss.join('\n');
      let used = new Set();
      let sss = this.tokenizer.to_cmath(str,style,used,1);
      let o = [];
      var idnum = `idnum${j}`;
      var label = `label${j}`;
      idnum = style[idnum];
      ...
      // idnum = idnum.split('.').pop();
      o.push(`\\def\\mytagforformulaA#1{${idnum}}`);
      o.push(`\\defineconversion[dummyA][\\mytagforformulaA]`);
      o.push(`\\setupformulae[numberconversion=dummyA]`);
      o.push(`\\placeformula`)
      o.push(`\\startformula`)
      o.push(`\\startmathalignment[n=${ncols}]`)
      sss.forEach((ss) => {
          let s = ss.join(' \\NC ');
          s = `\\NC ${s} \\NR`
          o.push(s);
  ...
  float_to_listing(title,label,style,ss,ssi){
    ...
    o.push('\\stoptabulate');
    var text = o.join('\n');
    text = `\\switchtobodyfont[${this.contex_bodyfontsize*0.9}pt]\\setupinterlinespace[line=${this.contex_bodyfontsize*0.9}pt]${text}`
  ...
  float_to_longtabu(title,label,style,ss,ssi) {
    var ss = this.ss_to_first_bundle(style,ss,ssi);
    var ss = ss.slice(1,-1);
    var o = this.fence_to_tab(style,ss,ssi);
    var text = o[0].img;
    var caption = this.uncode(style,title);
    var o = [];
    o.push('')
  ...
  float_to_vspace(title,label,style,ss,ssi){
    ...
    var vspace = (vspace*this.contex_bodyfontsize+2*this.contex_whitespacesize)*this.PT_TO_MM;
    all.push('');
  ...
  phrase_to_ref(style,cnt){
    ...
    if(label){
      if(this.idnum_map.hasOwnProperty(label)){
        ...
        ///let text = `\\goto{${idnum}}[${label}]`;
        let text = `\\inframed[frame=off]{\\underbar{${idnum}}}`;
        return text;
      }else{
  ...
  to_preamble_fonts(){
    ...
    for(let font of this.fonts){
      let {fid,fontname,fontfamily} = font;
      if(fontfamily=='serif'||fontfamily=='sans'||fontfamily=='mono'||fontfamily=='math'){
        all.push(`\\definefontfamily[${fid}][${fontfamily}][${fontname}]`);
      }else if(fontfamily=='contex'){
        all.push(`\\definefontfamily[${fid}][serif][${fontname}]`);
        all.push(`\\definefontfamily[${fid}][sans][${fontname}]`);
        all.push(`\\definefontfamily[${fid}][mono][${fontname}]`);
        all.push(`\\definefontfamily[${fid}][math][${fontname}]`);
      }
    }
    return all.join('\n')
  }
  ...
  to_preamble_essentials(){
    ///parse contex.bodyfont
    if(1){
      let ss = this.parser.conf_to_string('bodyfont','').split(',').map(s => s.trim());
      ss.forEach((s) => {
        if(s=='dejavu'||s=='office'||'linux'){
          this.contex_bodyfontfamily = s;
        }else if(s=='ss'){
          this.contex_bodyfontvariant = s;
        }else if(s.match(/^\d+$/)){
          this.contex_bodyfontsize = parseInt(s);
        }
      });
    }
    var all = [];
    ...
    all.push(`  headstyle=normal, style=normal, align=flushleft, margin=1em,`);
    all.push(`  alternative=hanging, width=broad]`);
    all.push(`\\definealternativestyle [myfontsmaller]  [\\switchtobodyfont[${this.contex_bodyfontsize*0.9}pt]] []`);
    all.push(`\\usesymbols[mvs]`);
    all.push(`\\setupbodyfont[${this.contex_bodyfontfamily},${this.contex_bodyfontvariant},${this.contex_bodyfontsize}pt]`);
    all.push(`\\setupinterlinespace[line=${this.contex_bodylineheight*this.contex_bodyfontsize}pt]`);
  ...
  plst_to_text(style,itemize){
    ...
    if(style.fontsize||style.fontstyle){
      ...
      if(style.fontsize=='small'){
        all.push(`\\switchtobodyfont[${this.contex_bodyfontsize*0.9}pt]`);
      }
  ...
  samp_to_text(style,samp){
    ...
    s_interlinespace = `\\switchtobodyfont[${this.contex_bodyfontsize*0.9}pt]\\setupinterlinespace[line=${this.contex_bodyfontsize*0.9*this.contex_bodylineheight}pt]`;
  ...
  sand_to_text(style,sand){
    ...
    if(hew>1){
      ...
      nn.forEach((nl,j) => {
        ...
        let [n1,n2] = nl;
        let ss1 = ss.slice(n1,n2);
        all.push(`\\startalignment[flushleft]${s_interlinespace}`);
        all.push(`\\startlines[${opts.join(',')}]`);
        ss1.forEach((s) => {
          all.push(`{${s}}`);
        });
