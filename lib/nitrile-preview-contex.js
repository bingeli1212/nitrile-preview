...
class NitrilePreviewContex extends NitrilePreviewTranslator {
  ...
  item_dl_to_text(i,item,nblank){
    ...
    if(!nblank){
      o.push(`\\startDLpacked{${value}} ${text} \\crlf`);           
    }else{
      o.push(`\\startDL{${value}} ${text} \\crlf`);           
    }
  ...
  fence_to_displaymath(style,ss) {
    var style = this.update_style_from_switches(style,'displaymath');
  ...
  fence_to_list(style,ss) {
    ...
    var circleboxo = String.fromCodePoint(0x274D); 
    var squareboxo = String.fromCodePoint(0x2752); 
    var circlebox = String.fromCodePoint(0x25CF);
    var squarebox = String.fromCodePoint(0x25A0);
    var circlebox = `{\\switchtobodyfont[zapfdingbats]${circlebox}}`;
    var circleboxo = `{\\switchtobodyfont[zapfdingbats]${circleboxo}}`
    var squarebox = `{\\switchtobodyfont[zapfdingbats]${squarebox}}`;
    var squareboxo = `{\\switchtobodyfont[zapfdingbats]${squareboxo}}`
  ...
  fence_to_plaintext(style,ss){
    var style = this.update_style_from_switches(style,'plaintext')
      ...
      if((v=re_char.exec(line))!==null){
        ...
        switch(s) {
          ...
          default: {
            let cc = s.codePointAt(0);
            if(this.cc_symbol_map.has(cc)){
              let s1 = this.cc_symbol_map.get(cc).cex;
              if(s1){
                s = `{${s1}}`
              }
            }
