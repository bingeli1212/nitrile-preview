...
class NitrilePreviewContex extends NitrilePreviewTranslator {
  ...
  constructor(parser) {
    ...
    this.icon_nabla = '{\\nabla}';
    this.icon_trianglenone = '{\\small\\small~~}';
    this.icon_triangledown = '{\\small\\small\\triangledown}';
    this.icon_dag = '{\\dag}';
    this.icon_hollowbox = '{\\square}'; 
    this.icon_checkedbox = '{\\blacksquare}';             
    this.icon_cdigits = [`\\utfchar{"278a}`,`\\utfchar{"278b}`,`\\utfchar{"278c}`,`\\utfchar{"278d}`,`\\utfchar{"278e}`,`\\utfchar{"278f}`,`\\utfchar{"2790}`,`\\utfchar{"2791}`,`\\utfchar{"2792}`,`\\utfchar{"2793}`]
    ///configuration
    ...
    this.contex_blank = 0;
    this.contex_whitespace = 'medium';
    this.contex_noindent = 0;
    this.contex_details_ul_symbol = '{\\small\\small\\blacktriangleright}';
  }
  /////////////////////////////////////////////////////////////////////////////
  ///
  /////////////////////////////////////////////////////////////////////////////
  ...
  hdgs_to_section(title,label,level,style){
    var o = [];
    o.push('');
    o.push(`\\startsection[title={${this.uncode(style,title)}},reference={${label}},bookmark={${title}}]`);
    return o.join('\n')
  } 
  hdgs_to_subsection(title,label,level,style){
    var o = [];
    o.push('');
    o.push(`\\startsubsection[title={${this.uncode(style,title)}},reference={${label}},bookmark={${title}}]`);
    return o.join('\n')
  } 
  hdgs_to_subsubsection(title,label,level,style){
    var o = [];
    o.push('');
    o.push(`\\startsubsubsection[title={${this.uncode(style,title)}},reference={${label}},bookmark={${title}}]`);
    return o.join('\n')
  } 
  //////////////////////////////////////////////////////////////////////////
  ///
  ...
  fence_to_par(g,ss) {
    ...
    var par = `\\framed[width=${w},height=${h},frame=${f},align=${a}]{${text}}`;
    var itm = {};
    itm.par = par;
    itm.out = par;
    return itm;
  ...
  float_to_preformatted(title,label,style,subtitles,body,bodyrow){
    ...
    let all = [];
    all.push('');
    all.push(`\\blank[${this.contex_whitespace}]`);//need a blank, otherwise there are no vertical spaces in front of it---dont know why
    ...
    step.forEach((p,i,arr) => {
      if(i==0){
        if(p.value=='+'){
          ...
          if(this.contex_details_ul_symbol){
            all.push(`\\sym{${this.contex_details_ul_symbol}} ${this.uncode(style,this.join_para(p.lines).trim())}`);
          }else{
        ...
        if(this.re_body_is_fence.test(p.lines[0])){
          ...
          let lines = this.body_to_body(p.lines,this.re_body_is_gthan).map((x,i)=>{
            if(i==0){
              ...
              return `\\color[white]{${this.icon_triangledown}}~~${this.uncode(style,x)}`;
            }
          });
          var text = lines.join('\\\\\n')
          all.push('');
          ...
          var totaln = ww.reduce((acc,cur)=>acc+cur,0);
          var fmt = ww.map(x=>`k0p(${x/totaln}\\textwidth)`).join('|');
          let d = rows.map((pp)=>{
            let ss1 = pp.map(p=>p.text);
  ...
  float_to_body(title,label,style,subtitles,body,bodyrow){
    ...
    step.forEach((p,i,arr) => {
      if(i==0){
        ...
        if(this.re_body_is_fence.test(p.lines[0])){
          let bundles = this.body_to_all_bundles(style,p.lines);
          let itms = bundles.map((bundle) => this.do_bundle(bundle));
          itms.forEach((p,i,arr)=>{
            if(p.type=='bundle'){
              if(p.img){
                d.push(p.img);
    ...
    if(type=='img'){
      let img = d.join('\n');
      all.push(`\\placefigure[right,none][]{}{${img}}${text}`);
    }else if(type=='tab'){
      let tab = d.join('\n');
      all.push(`\\placetable[right,none][]{}{${tab}}${text}`);
    }else{
      all.push(text);
    }
    return all.join('\n');
  ...
  rows_to_multi(rows) {
    ...
    return d.join('\n');
    /*
    o.push(`\\blank[${this.contex_whitespace}]`);
  ...
  prim_to_text(style,title,body){
    ...
    all.push('');
    if(style.rank==2) {
      all.push(`\\blank[${this.contex_whitespace}]\\indent\\mbox{\\bi ${title}} ~ ${text}`);
    }else{
      all.push(`\\blank[${this.contex_whitespace}]\\noindent\\mbox{\\bf ${title}} ~ ${text}`);
    } 
    return all.join('\n');
