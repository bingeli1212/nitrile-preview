...
class NitrilePreviewContex extends NitrilePreviewTranslator {
    ...
    step.forEach((p,i) => {
      if(i==0){
        ...
        if(this.re_body_is_fence.test(p.lines[0])){
          ...
          all.push('');
          if(mytype=='img'){
            all.push(`\\blank[none]\\noindent`);//HACK: for images if \\blank is specified there is too much visible vertical spaces
          }else{
            all.push(`\\blank[${this.contex_whitespace}]\\noindent`);
          }
          all.push('\\startalignment[flushleft]\\dontleavehmode');
  ...
  float_to_enumeration(title,label,style,subtitles,body,bodyrow){
    let ss = body.map((s,i)=>`\\sym {${1+i}.} ${this.uncode(style,s)}`);
    var text=ss.join('\n');
    var all = [];
    all.push('');
    all.push(`\\blank[${this.contex_whitespace}]`)
