...
class NitrilePreviewContex extends NitrilePreviewTranslator {
  ...
  fence_to_fml(style,ss,ssi) {
    ...
    sss = sss.map((ss) => {
      ss = ss.map((s,i) => (i==0)?`\\math{${s}}`:`\\math{\\;${s}}`)
      return ss;
    })
    sss = sss.map((ss) => {
    ...
    var text = all.join('\n');
    var fml = `\\framed[frame=off,location=bottom,strut=no]{${text}}`
  ...
  float_to_center(title,label,style,splitid,body,bodyrow){
    ...
    bundles.forEach((bundle,j,arr) => {
      ...
      if(itm.fml){
        ...
        text = `\\dontleavehmode\\hbox{${itm.tab}}`;
      }else if(itm.par){
        text = itm.par;
      }
      all.push(text);
      ...
      ss =  ss.map((s,j,arr) => {
        ...
        if(j==0){
          if(isdhline){
            ...
            xx = xx.map(x => `p(${x}mm)`);//there are extra spaces between columns
            fmt = xx.join('|');
            return '';
          }
        }
  ...
  to_context_starttable_pcol(ww){
    var ss = ww.map((w) => {
      if(w.ta=='c'){
        return `cp(${w.pw-3}mm)`;//remove 1mm from the actual width for \starttable
      }else if(w.ta=='r'){
        return `rp(${w.pw-3}mm)`;
      }else{
        return `p(${w.pw-3}mm)`;
