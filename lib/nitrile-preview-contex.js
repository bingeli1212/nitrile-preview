...
class NitrilePreviewContex extends NitrilePreviewTranslator {
  ...
  fence_to_ink(style,ss){
    ...
    var max_inkheight = vgap + vgap + npara*12*this.PT_TO_MM;//fs font
    ...
    ss.forEach((s,j,arr) => {
      ...
      let x = 0;
      let y = inkheight - ((j+1)*12)*this.PT_TO_MM + vgap;
      all.push(`pair a ; a := (${this.fix(x)}*u,${this.fix(y)}*u) ;`);
      all.push(`label.rt("\\tt\\switchtobodyfont[10pt]${s}",a) ;`);
      //all.push(`picture one; one := textext("\\tt\\switchtobodyfont[12pt]${s}");`);
      //all.push(`numeric halfw; halfw := bbwidth(one)/2;`);
      //let j = arr.length - i - 1;
      //all.push(`draw one shifted (halfw,${6+j*12*this.PT_TO_MM}*u) shifted (${vgap+vgap}*u,${vgap}*u);`);
    });
