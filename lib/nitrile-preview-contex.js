...
class NitrilePreviewContex extends NitrilePreviewTranslator {
  ...
  fence_to_par(style,ss,ssi) {
    ...
    let text1 = `\\framed[${opts.join(',')}]{${text}}`
    if(style.fontsize=='small'){
      var img = `{\\switchtobodyfont[${this.bodyfontsize*0.9}pt]\\setupinterlinespace[line=${this.bodyfontsize*0.9*this.contex_bodylineheight}pt]${text1}}`;
    }else{
      var img = text1;
    }
