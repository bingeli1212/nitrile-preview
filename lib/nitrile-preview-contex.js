...
class NitrilePreviewContex extends NitrilePreviewTranslator {
  ...
  fence_to_tab(style,ss,ssi){
    ...
    var ff = this.fontstyle_to_array(style.fontstyle);
  ...
  fence_to_par(style,ss,ssi) {
    ...
    if(1){
      text = this.to_fontstyled_text(text,style.fontstyle);
    }
    if(1){
      text = `{\\switchtobodyfont[${this.contex_bodyfontsize*0.9}pt]\\setupinterlinespace[line=${this.contex_bodyfontsize*0.9*this.contex_bodylineheight}pt]${text}}`;
    }
    var img = text;
