...
class NitrilePreviewContex extends NitrilePreviewTranslator {
  ...
  fence_to_formula(style,ss,ssi) {
    ...
    var frm = `\\framed[frame=off,location=bottom,strut=no]{${text}}`
  ...
  fence_to_ink(style,ss,ssi){
    ...
    if(style.frame){
      ...
      var frm = `\\framed[frame=on,location=bottom,strut=no]{${text}}`;
    }else{
      var img = `\\framed[frame=off,location=bottom,strut=no]{${text}}`;
      var frm = `\\framed[frame=off,location=bottom,strut=no]{${text}}`;
    }
  ...
  fence_to_dia(style,ss,ssi){
    ...
    o.forEach((p) => {
      ...
      if(style.frame){
        ...
        p.frm = `\\framed[frame=on,location=bottom,strut=no]{${img}}`
      }else{
        p.img = `\\framed[frame=off,location=bottom,strut=no]{${img}}`
        p.frm = `\\framed[frame=off,location=bottom,strut=no]{${img}}`
      }
    });
    return o;
  }
  ...
  fence_to_tabular(style,ss,ssi){
    ...
    /// no frame
    if(hew>1){
      let s1 = pcol.repeat(hew);
      all.push(`\\starttable[${s1}]`);
    }else{
    ...
    if(1){
      ...
      while(i < ss.length){
        ...
        ss1 = ss1.map((s,j,arr) => {
          ...
          }else if(style.rules=='groups'||style.rules=='cols'||style.rules=='all'){
            s2 = '\\VL '+s2.slice(4);
          }
          s += s2;
          return s;
