...
class NitrilePreviewContex extends NitrilePreviewTranslator {
  ...
  ///
  //////////////////////////////////////////////////////////////////////////
  item_dl_to_text(i,item,item_labels){
    var o = [];
  ...
  item_hl_to_text(i,item){
    var o = [];
    o.push(`\\startHL{}${this.uncode(item.style,item.text)} \\\\`);
    if(item.more && item.more.length){
      item.more.forEach((p) => {
        if(p.lines){
          o.push(`${this.untext(p.style,p.lines)}`);
    ...
    switch (bull) {
      case 'OL': {
        ...
        items.forEach((item,j) => {
          let text = this.item_ol_to_text(j,item);
          o.push(text);
        });
        o.push('\\stopitemize')
        break;
      ...
      case 'UL': {
        o.push(`\\startitemize[]`);
        items.forEach((item,j) => {
          let text = this.item_ul_to_text(j,item);
          o.push(text);
        });
        o.push('\\stopitemize')
        break;
      ...
      case 'DL': {
        var item_labels = [];
        items.forEach((item,j) => {
          let text = this.item_dl_to_text(j,item,item_labels);
      ...
      case 'HL': {
        items.forEach((item,j) => {
          let text = this.item_hl_to_text(j,item);
          o.push(text);
        });
        break;
      }
