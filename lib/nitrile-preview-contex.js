...
class NitrilePreviewContex extends NitrilePreviewTranslator {
  ...
  fence_to_parbox(style,ss,ssi) {
    ...
    if(style.frame){
      text = `\\starttable[s0|${w}|]\\HL\\VL{${text}}\\VL\\NR\\HL\\stoptable`;
    }else{
      text = `\\starttable[s0|${w}|]\\NC{${text}}\\NC\\NR\\stoptable`;
    }
  ...
  fence_to_ink(style,ss,ssi){
    ...
    var text = all.join('\n');
    if(style.frame){
      text = `\\starttable[s0|l|]\\HL\\VL{${text}}\\VL\\NR\\HL\\stoptable`;
    }else{
      text = `\\starttable[s0|l|]\\NC{${text}}\\NC\\NR\\stoptable`;
    }
  ...
  fence_to_verbatim(style,ss,ssi){
    ...
    let all = [];
    all.push('\\starttabulate[|l|][bodyfont=tt]');
    if(style.small){
      all.push('\\small');
    }
    ss.forEach((s) => {
  ...
  to_preamble_fonts(){
    ...
    all.push(`\\definefontfamily [dejavu] [serif] [dejavuserif]    [rscale=1.00]`);
    all.push(`\\definefontfamily [dejavu] [sans]  [dejavusans]     [rscale=1.00]`);
    all.push(`\\definefontfamily [dejavu] [mono]  [dejavusansmono] [rscale=1.00]`);
    all.push(`\\definefontfamily [dejavu] [math]  [dejavumath]     [rscale=1.00]`);
    all.push(`%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%`);
    all.push(`\\definefontfamily [office] [serif] [Times New Roman]`);
    all.push(`\\definefontfamily [office] [sans]  [Arial] [rscale=0.9]`);
  ...
  cove_to_text(style,cove){
    ...
    if(n > 2 && p0.text.startsWith('```') && pn.text.startsWith('```')){
      ...
      if(o.length){
        let img = o[0].img;
        img = `\\framed[frame=off,location=top]{${img}}`
        all.push(img);
      }
    }else{
