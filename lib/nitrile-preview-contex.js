...
class NitrilePreviewContex extends NitrilePreviewTranslator {
  ...
  constructor(parser) {
    ...
    this.name_symbol_map = this.tokenizer.name_symbol_map;
  ...
  to_symbol_doc(){
    if(1){
      ...
      for(let symbol in this.tokenizer.pjson.symbol){
        let line = `(&${symbol};) (&math{a + &${symbol}; + b + \\${symbol} + c}) &tt{${symbol}}`;
    ...
    var all = [];
    all.push(`% !TEX program = ConTeXt (LuaTeX)`);
    all.push(`\\enabletrackers[fonts.missing]`);
    all.push(`\\setupindenting[no,medium]`);
    all.push(`\\setupwhitespace[medium]`);
    all.push(`\\usesymbols[mvs]`);
    all.push(`\\enableregime[utf] % enable unicode fonts`);
    all.push(`\\definefontfamily[linuxlibertine][serif][linuxlibertineo]`);
    all.push(`\\definefontfamily[linuxlibertine][sans][linuxbiolinumo]`);
    all.push(`\\definefontfamily[linuxlibertine][tt][linuxlibertinemonoo]`);
    all.push(`\\definefontfamily[dejavu][serif][dejavuserif]`);
    all.push(`\\definefontfamily[dejavu][sans][dejavusans]`);
    all.push(`\\definefontfamily[dejavu][tt][dejavusansmono]`);
    all.push(`\\definefontfamily[noto][serif][notoserifnormal]`);
    all.push(`\\definefontfamily[noto][sans][notosansnormal]`);
    all.push(`\\definefontfamily[noto][tt][notosansmononormal]`);
    all.push(`\\definefontfamily[zapfdingbats][serif][zapfdingbats]`);
    all.push(`\\definefontfamily[zapfdingbats][sans][zapfdingbats]`);
  ...
  hdgs_to_chapter(title,label,style){
    var style = this.update_style_from_switches(style,'chapter');
    var o = [];
    var raw = title.replace(/#/g,'');
    var title = this.uncode(title);
    o.push('');
  ...
  hdgs_to_section(title,label,style){
    var style = this.update_style_from_switches(style,'section');
    var o = [];
    var raw = title.replace(/#/g,'');
    var title = this.uncode(title);
    o.push('');
  ...
  hdgs_to_subsection(title,label,style){
    var style = this.update_style_from_switches(style,'subsection');
    var o = [];
    var raw = title.replace(/#/g,'');
    var title = this.uncode(title);
    o.push('');
  ...
  hdgs_to_subsubsection(title,label,style){
    var style = this.update_style_from_switches(style,'subsubsection');
    var o = [];
    var raw = title.replace(/#/g,'');
    var title = this.uncode(title);
    o.push('');
  ...
  ///
  //////////////////////////////////////////////////////////////////////////
  item_dl_to_text(i,item,style,item_labels){
    var o = [];
    var text = this.uncode(item.text,style);
    ...
    if(item.more && item.more.length){
      item.more.forEach((p) => {
        let { lines, itemize } = p;
        if(lines){
          o.push(`${this.untext(lines,style)}`);
        }else if(itemize) {
          o.push(`${this.itemize_to_text(itemize,style)}`);
        }
      });
    }
    o.push(`\\stopDL`);
  ...
  item_hl_to_text(i,item,style){
    var o = [];
    o.push(`\\startHL{}${this.uncode(item.text,style)} \\\\`);
    if(item.more && item.more.length){
      item.more.forEach((p) => {
        let { lines, itemize } = p;
        if(lines){
    ...
    switch (bull) {
      case 'OL': {
        ...
        items.forEach((item,j) => {
          let text = this.item_ol_to_text(j,item,style);
          o.push(text);
        });
        o.push('\\stopitemize')
        break;
      ...
      case 'UL': {
        o.push(`\\startitemize[]`);
        items.forEach((item,j) => {
          let text = this.item_ul_to_text(j,item,style);
          o.push(text);
        });
        o.push('\\stopitemize')
        break;
      ...
      case 'DL': {
        var item_labels = [];
        items.forEach((item,j) => {
          let text = this.item_dl_to_text(j,item,style,item_labels);
      ...
      case 'HL': {
        items.forEach((item,j) => {
          let text = this.item_hl_to_text(j,item,style);
          o.push(text);
        });
        break;
      }
  ...
  do_PARA(block){
    ...
    var text = this.untext(body,style).trim();
    if(text.startsWith('\\hbox') || text.startsWith(`\\ruledhbox`)){
  ...
  fence_to_parbox(style,ss) {
    var style = this.update_style_from_switches(style,'parbox');
  ...
  fence_to_list(style,ss) {
    ...
    var leftmargin = '';
    var parsep = 0;
    var text = this.to_margined_text(itms,parsep,leftmargin,style);
    return text;
  }
  ...
  fence_to_img(style,ss) {
    ...
    if(style.wrap){
      ...
      text = `\\placefigure[${wrap},none][]{}{${text}}`;
      return text;
    }else{
      return `\\hbox{${text}}`;
    }
  }
  fence_to_blockquote(style,ss){
    var style = this.update_style_from_switches(style,'blockquote')
    var o = [];
    o.push('\\startquotation[1*left,1*right]')
    var s = this.join_para(ss);
    var s = this.uncode(s,style);
    o.push(s);
    o.push('\\stopquotation');
    var text = o.join('\n')
    return text;
  }
  ...
  float_to_listing(title,label,style,ss){
    ...
    o.push('\\stoptabulate');
    var text = o.join('\n')
    title = this.uncode(title,style).trim();
    o = [];
    o.push('')
    o.push(`\\placelisting`);
  ...
  float_to_lines(title,label,style,ss){
    var style = this.update_style_from_switches(style,'lines');
    var style = this.update_style_from_switches(style,'blockquote');
    var clusters = this.ss_to_clusters(ss);
    clusters = clusters.map((ss) => {
      var s = this.join_para(ss);
      var s = this.polish(s,style);
      return `\n\\startnarrower[1*left,1*right]\n${s}\n\\stopnarrower`
    })
  ...
  polish_verb(unsafe,style){
    unsafe = this.polish(unsafe,style);
  ...
  polish(line,style){
        ...
        switch(s) {
          ...
          default: {
            ...
            if(this.cc_symbol_map.has(cc)){
              s = this.cc_symbol_map.get(cc).cex;
              s = `{${s}}`
            }
            break;
          }
    ...
    safe = this.rubify(safe,style.vmap);
    return safe;
  }
  ///
  ...
  smooth (line) {
    ...
    while(line.length){
      if((v=re_entity.exec(line))!==null){
        ...
        line=v[2];
        try {
          let s = this.tokenizer.get_cex_symbol(name);
          safe+=s;
        } catch(e) {
          safe+=name;
        }
  ...
  literal_to_double(style,cnt){
    let s = this.polish_verb(cnt,style);
    s = `{\\tt{}${s}}`;
    s = `\\quotation{${s}}`;
    return s;
  }
  ...
  phrase_to_inlinemath(style,cnt,cnt2,cnt3){
    let s = this.tokenizer.to_cmath(cnt,style,0);
    return `\\math{${s}}`;
  }
  phrase_to_displaymath(style,cnt,cnt2,cnt3){
    let s = this.tokenizer.to_cmath(cnt,style,1);
    let o = [];
    o.push('\\startformula');
    o.push(s);
    o.push('\\stopformula')
  ...
  phrase_to_img(style,cnt,cnt2,cnt3){
    ...
    return this.fence_to_img(style,[cnt]);
  }
  phrase_to_diagram(style,cnt,cnt2,cnt3){
    var cnt2 = `{${cnt2}}`;
    style = this.string_to_style(cnt2,style)[0];
    return this.fence_to_diagram(style,[cnt]);
  }
  phrase_to_framed(style,cnt,cnt2,cnt3){
    var cnt2 = `{${cnt2}}`;
    style = this.string_to_style(cnt2,style)[0];
    return this.fence_to_framed(style,[cnt]);
  }
