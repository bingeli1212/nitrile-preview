...
class NitrilePreviewContex extends NitrilePreviewTranslator {
  ...
  fence_to_verbatim(style,ss,ssi){
    ...
    var frm = `\\framed[frame=off,location=top,   strut=no]{${text}}`
    if(1){
      img = `\\small\\setupinterlinespace[${this.contex_interlinespace_small_verbatim}]${img}`
      frm = `\\small\\setupinterlinespace[${this.contex_interlinespace_small_verbatim}]${frm}`
    }else{
      img = `\\setupinterlinespace[${this.contex_interlinespace_verbatim}]${img}`
      frm = `\\setupinterlinespace[${this.contex_interlinespace_verbatim}]${frm}`
    }
    var subc = style.subtitle||'';
    var subc = this.uncode(style,subc);
    var o = [];
  ...
  float_to_listing(title,label,idnum,idcap,style,ss,ssi){
    ...
    var text = o.join('\n');
    if(this.contex_line_small){
      text = `\\setupinterlinespace[${this.contex_interlinespace_small_verbatim}]${text}`
    }else{
  ...
  samp_to_text(style,samp){
    ...
    if(this.contex_samp_small){
      opts.push(`style=small`);
      s_interlinespace = `\\setupinterlinespace[${this.contex_interlinespace_small_verbatim}]`;
    }else{
