...
class NitrilePreviewContex extends NitrilePreviewTranslator {
  ...
  fence_to_par(style,ss,ssi) {
    ...
    if(style.textalign=='c'){
      var tab = `\\starttabulate[|k0cp|]\n${text2}\n\\stoptabulate`;
    }else if(style.textalign=='r'){
      var tab = `\\starttabulate[|k0rp|]\n${text2}\n\\stoptabulate`;
    }else{
      var tab = `\\starttabulate[|k0p|]\n${text2}\n\\stoptabulate`;
    }
    return({img,tab,subtitle,style});
  }
  ...
  float_to_center(title,label,style,splitid,body,bodyrow){
    ...
    bundles.forEach((bundle,j,arr) => {
      ...
      if(itm.img){
        ...
        let text = `${itm.tab}`;
        text = `\\hbox{${text}}`;///it has been observed that \starttable by itself are not being centered, however with \hbox it will
        all.push(text);
      }
    });
    all.push('\\stopalignment')
  ...
  float_to_columns(title,label,style,splitid,body,bodyrow){
    ...
    itms.forEach((p,j,arr) => {
      ...
      if(p.img){
        ...
        // let tab = this.to_column_img(p.tab,p.subtitle);
        all.push(p.tab);
      }
    });
