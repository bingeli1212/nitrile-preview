...
class NitrilePreviewContex extends NitrilePreviewTranslator {
  ...
  fence_to_verbatim(style,ss){
    ...
    var indenting = (style.float)?'no':'yes';
    if(style.small){
      /// the indenting=yes allows the verbatim block to be indented if document indenting has been set
      d.push(`\\starttabulate[|T${p}|][unit=0.2em,bodyfont=small,indenting=${indenting},before={},after={}]`);
    }else{
      d.push(`\\starttabulate[|T${p}|][unit=0.2em,indenting=${indenting},before={},after={}]`);
    }
    ss.forEach( (s,j,arr) => {
  ...
  float_to_paragraph(title,label,style,body){
    ...
    if(button_text){
      ...
      var text = `\\dontleavehmode \\framed{${button_text}} ${text}`;
    }else{
