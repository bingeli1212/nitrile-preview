...
class NitrilePreviewContex extends NitrilePreviewTranslator {
  ...
  fence_to_tabular(style,ss){
    ...
    if(style.float){
      ...
      var ww = this.halign_to_contex_pcols_length(n,style.halign);
      var all = [];
      /// setup for hlines
      if(0){
        if(style.hrule=='---'){ 
      ...
      if(1){
        for (var j = 1; j <= rows.length; j++) {
          ...
          break;//only do it for the first row
        }
      }
      if(1){
      ...
      ///layout 'bTABLEhead'
      /*
      var pp0 = rows.shift();///always the same width as n
      all.push(`\\bTABLEhead`);
      if(pp0){
        if(pp0.lower==2){
          all.push(`\\bTR[bottomframe=on] \\bTH ${pp0.map(x=>x.text).join(' \\eTH \\bTH ')} \\eTH \\eTR`);
          all.push(`\\bTR[bottomframe=on,height=1.50pt] \\eTR`);
        }else if(pp0.lower==1){
          all.push(`\\bTR[bottomframe=on] \\bTH ${pp0.map(x=>x.text).join(' \\eTH \\bTH ')} \\eTH \\eTR`);
        }else{
          all.push(`\\bTR[] \\bTH ${pp0.map(x=>x.text).join(' \\eTH \\bTH ')} \\eTH \\eTR`);
        }
      }
      all.push(`\\eTABLEhead`);
      */
      ...
      for (let j = 0; j < rows.length; j++) {
        if(j==0){
          ...
          if(pp.lower==2){
            all.push(`\\bTR[bottomframe=on] \\bTH ${mypp.join(' \\eTH \\bTH ')} \\eTH \\eTR`);
            all.push(`\\bTR[bottomframe=on,height=1.50pt] \\eTR`);
          }else if(pp.lower==1){
            all.push(`\\bTR[bottomframe=on] \\bTH ${mypp.join(' \\eTH \\bTH ')} \\eTH \\eTR`);
          }else{
            all.push(`\\bTR[] \\bTH ${mypp.join(' \\eTH \\bTH ')} \\eTH \\eTR`);
          }
          all.push(`\\eTABLEhead`);
        }else{
          all.push(`\\bTABLEbody`);
          ...
          var mypp = pp.map(p => `{${p.text}}`);
          if(pp.lower==2){
            all.push(`\\bTR[bottomframe=on] \\bTD ${mypp.join(' \\eTD \\bTD ')} \\eTD \\eTR`);
            all.push(`\\bTR[bottomframe=on,height=1.50pt] \\eTR`);
          }else if(pp.lower==1){
            all.push(`\\bTR[bottomframe=on] \\bTD ${mypp.join(' \\eTD \\bTD ')} \\eTD \\eTR`);
          }else{
            all.push(`\\bTR[] \\bTD ${mypp.join(' \\eTD \\bTD ')} \\eTD \\eTR`);
          }
          all.push(`\\eTABLEbody`);
        }
      }
      ...
      ///non-float, use 'starttabulate' because it "flows" with the text, such that if it is inside a itemized env it will be indented like normal text would, in addition, two '\HL' would have generated a double horizontal rule
      var rows = this.ss_to_tabular_rows(ss,style);
      var rows = this.update_rows_by_hrule(rows,style.hrule);
      rows.forEach((pp) => {
        pp.forEach((p) => {
      ...
      if(1){
        let d = [];
        var ww = this.halign_to_contex_pcols(n,style.halign);
    ...
    if(style.float=='figure' || style.float=='table'){
      ...
      if(1){
        ...
        if(style.isfigure){
          ...
          d.push(`\\starttable[|${pcol}|][frame=off]`);
        }
        ////////
        //let title_text = this.uncode(style,title);
        //d.push(`\\NC \\use{${n}}\\ReFormat[c]{${title_text}} \\MR`)
      ...
      all.push(`\\bTABLEhead`);
      all.push(`\\bTR \\bTH ${pp0.join(' \\eTH \\bTH ')} \\eTH \\eTR`);
      all.push(`\\eTABLEhead`);
      ///layout 'bTABLEbody'
      all.push(`\\bTABLEbody`);
      for (let j = 0; j < rows.length; j++) {
  ...
  halign_to_contex_pcols_length(n,halign) {
    /// \begin{tabular}{lcr}{
    /// | >{\hsize=0.5\hsize\raggedright\arraybackslash}X
    ///  | >{\hsize=0.5\hsize\centering\arraybackslash}X
    ///  | >{\hsize=2.0\hsize\raggedleft\arraybackslash}X | }
