...
class NitrilePreviewContex extends NitrilePreviewTranslator {
  ...
  fence_to_tabular(style,ss,ssi){
    ...
    if(style.fontsize=='small'){
      img = `{\\small\\setupinterlinespace[line=10.0pt]${img}}`;
      frm = `{\\small\\setupinterlinespace[line=10.0pt]${frm}}`;
    }
    var o = [];
    var subc = style.subtitle||'';
    var subc = this.uncode(style,subc);
  ...
  fence_to_verbatim(style,ss,ssi){
    ...
    if(style.fontsize=='small'){
      all.push('\\small\\setupinterlinespace[line=10.0pt]');
    }
    ss.forEach((s) => {
      s = `\\NC ${s} \\NC\\NR`;
      all.push(s);
  ...
  cove_to_text(style,cove){
    ...
    all.push(`\\setupparagraphs[mycove][1][width=.1\\textwidth,align=left]`);
    all.push(`\\setupparagraphs[mycove][2][width=.9\\textwidth]`);
