...
class NitrilePreviewContex extends NitrilePreviewTranslator {
  ...
  fence_to_parbox(ss,g) {
    ...
    var contex_width = this.string_to_contex_width(g.width,1,'\\textwidth');
  ...
  fence_to_framed(ss,style){
    ...
    var { text } = this.framed.to_framed(style,ss);
    return text;
  }
  ...
  fence_to_diagram(ss,style){
    ...
    if(style.wrap){
      ...
      text = `\\placefigure[${wrap},none][]{}{\\hbox{${text}}}`;
      return text;
  ...
  fence_to_img(ss,g) {
    ...
    if(g.wrap){
      let wrap = g.wrap=='left'?'left':'right';
