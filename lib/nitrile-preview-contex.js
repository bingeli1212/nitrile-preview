...
\\setupbodyfont[linuxlibertine,11pt]
\\starttext
${titlelines.join('\n')}
${toclines.join('\n')}
${texlines.join('\n')}
\\stoptext
  ...
  to_symbol_doc(){
    ...
    for(let symbol in this.tokenizer.pjson.symbol){
      let cex = this.tokenizer.get_cex_symbol(symbol);
      let row = [];
      row.push({raw:'',text:symbol});
      row.push({raw:'',text:cex});
      rows.push(row);
    }
    var all = [];
    rows.forEach((pp) => {
      all.push(`${pp[1].text} (${pp[0].text})\n`)
    })
    var text = all.join('\n')
    console.log('james','text=',text)
    var style_chapter = this.conf('context.chapter','\\bfd');
    var style_section = this.conf('context.section','\\bfa');
    var style_subsection = this.conf('context.subsection','\\bf');
    var style_subsubsection = this.conf('context.subsubsection','\\bf');
  ...
  smooth (line) {
    ...
    while(line.length){
      if((v=re_entity.exec(line))!==null){
        ...
        line=v[2];
        try{
          let s = this.tokenizer.get_cex_symbol(symbol);
          safe+=s;
        }catch(e){
          safe+=symbol;
        }
  ...
  __smooth__ (unsafe) {
    ...
    if(1){
      ...
      for (let c of unsafe) {
        ...
        if (this.cc_symbol_map.has(cc)) {
          c = this.cc_symbol_map.get(cc).cex;
        }
        new_unsafe += c;
      }
      unsafe = new_unsafe;
