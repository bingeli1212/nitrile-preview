...
class NitrilePreviewContex extends NitrilePreviewTranslator {
  ...
  fence_to_par(style,ss,ssi) {
    ...
    var par = `\\starttabulate[|k0${a}p|][bodyfont=${textsmaller}]\n${text2}\n\\stoptabulate`;
    return({par,style});
  }
  ///////////////////////////////////////////////////////////////////////////////////////////////
  ///
  ...
  float_to_preformatted(title,label,style,splitid,body,bodyrow){
    ...
    all.push(`\\startalignment[right]${textsmaller}`);
  ...
  float_to_example(title,label,style,splitid,body,bodyrow){
    var ss = body.map((s) => this.uncode(style,s)).map(s=>`${this.icon_dag}\\ ${s}`);
  ...
  float_to_description(title,label,style,splitid,body,bodyrow){
    ...
    let ss = body.map((s)=>s.split('\n')).map(([dt,dd]) => {
      let v;
      if((v=re_quoted.exec(dt))!==null){
        dt = `"${this.polish(style,v[1])}"`;
      }else{
        dt = this.polish(style,dt);
      }
      if(dd.length){
        return `\\sym {\\bold ${dt}}\\crlf\n${this.uncode(style,dd)}`
      }else{
        return `\\sym {\\bold ${dt}}`
      }
    });
  ...
  float_to_tabbing(title,label,style,splitid,body,bodyrow){
    ...
    var fmt = ww.map(w => `k0p(${w/total}\\textwidth)`).join('|');//there are extra spaces between columns
