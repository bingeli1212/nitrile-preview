...
class NitrilePreviewContex extends NitrilePreviewTranslator {
  ...
  fence_to_tab(style,ss,ssi){
    ...
    if(rows.title){
      all.push(`\\bTR[] \\bTD[nc=${ww.length},leftframe=off,rightframe=off] ${this.polish(style,rows.title)} \\eTD`)
    }
    if(1){
      heads.forEach((pp,j,arr) => {
        if(j==0){
  ...
  fence_to_par(style,ss,ssi) {
    ...
    var par = `\\starttabulate[|k0${textalign}${textoblique}p|][bodyfont=${textsmaller}]\n${text2}\n\\stoptabulate`;
    return({par,style});
  }
  ///////////////////////////////////////////////////////////////////////////////////////////////
  ///
