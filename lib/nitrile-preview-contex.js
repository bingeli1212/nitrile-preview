...
class NitrilePreviewContex extends NitrilePreviewTranslator {
  ...
  fence_to_tab(style,ss,ssi){
    ...
    var stretch = this.assert_float(style.stretch,1,0,1);
    ...
    if(1){
      all.push(`\\setupTABLE[frame=off,loffset=2pt,roffset=2pt,toffset=0pt,boffset=0pt]`);
    }
    /// setup for hlines
    if(style.rules){
    ...
    if(1){
      ...
      ww.map((x, i) => {
        ...
        all.push(`\\setupTABLE[c][${i + 1}][width=${pw/totalw*stretch}\\textwidth,align=${ta}]`);
      });  
    }
    if(0){
    ...
    if(style.head){
      let heads = rows.slice(0,1);
      rows = rows.slice(1);
      heads.forEach((pp,j) => {
        all.push(`\\bTABLEhead`);
        let myrowtext = pp.map(p=>p.text).map(x=>`\\bTD ${x} \\eTD`).join(' ');
  ...
  fence_to_par(style,ss,ssi) {
    ss = this.ss_to_backslashed_ss(style,ss);//uncode as well
  ...
  float_to_tabbing(title,label,style,splitid,body,bodyrow){
    ...
    if(bundles.length) {
      ...
      ss =  ss.map((s,j,arr) => {
        ...
        if(j==0){
          if(isdhline){
            ...
            xx = xx.map(x => `p(${x}ex)`);//there are extra spaces between columns
            fmt = xx.join('|');
            return '';
          }else{
            fmt = xx.map(x=>`p(10ex)`).join('|');
          }
        }
        s = xx.join(' \\NC ');
        s = `\\NC ${s} \\NC\\NR`;
      ...
      if(fmt){
        ...
        all.push(`\\starttabulate[|${'l'.repeat(maxn).split('').join('|')}|]`);
      }
      ss.forEach(s => {
        if(s){
          all.push(s);
  ...
  ss_to_ink(style,ss,ssi){
    ...
    if (style.stretch) {
      all.push(`currentpicture := currentpicture xsized (TextWidth) ;`) //'TextWidth' is a keyword in MF
