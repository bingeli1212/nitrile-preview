...
class NitrilePreviewContex extends NitrilePreviewTranslator {
  ...
  fence_to_verse(style,ss) {
    ...
    var all = [];
    all.push('\\starttabulate[|l|][before={},after={},indenting=yes]');
    itms.forEach((p,i,arr) => {
      if(p.indented){
  ...
  float_to_wrapfig(title,label,style,ss){
    ...
    let wrap = style.location=='left'?'left':'right';
    var o = [];
    itms.forEach((p,j,arr) => {
      if(p.type=='bundle'){
        if(p.subc){
          var text = `\\placefigure[${wrap}][]{${p.subc}}{${p.img}}`;  
        }else{
  ...
  float_to_figure(title,label,style,ss){
    ...
    if(style.wrap){
      ...
      itms.forEach((p,j,arr) => {
        if(p.type=='bundle'){
          o.push(`\\hbox{\\startcombination[1*1] {${p.img}} {${p.subc}} \\stopcombination}`);
        }else if(p.type=='\\\\'){
          o.push('\\\\');
        }
      });
