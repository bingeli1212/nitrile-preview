...
class NitrilePreviewContex extends NitrilePreviewTranslator {
  ...
  fence_to_tabular(style,ss){
    if(style.float=='figure' || style.float=='table'){
      ...
      var pcols = this.halign_to_contex_pcols_length(n,style.halign);
      var all = [];
      var d = [];
      pcols.map((x, i) => {
        if(x){
          d.push(`\\setupTABLE[c][${i + 1}][width=${x}]`);
        }
      });
      /// setup for hlines
      ...
      var text = d.join('\n');
      if(title.length){
        all.push(`\\placetable[here,split,${none?'none':''}][${label}]{${title}}{${text}}`);
      }else{
        all.push(`\\blank\n${text}`)
      }
      var text = all.join('\n');
  ...
  fence_to_img(style,ss) {
    ...
    var opts = [];
    if(style.flex){
      opts.push(`width=${style.flex}\\textwidth`);
  ...
  float_to_longtable(title,label,style,ss) {
    ...
    for(let itm of itms){
      if(itm.type=='bundle' && itm.key=='tabular'){
        var text = itm.img;
        text = `\\placetable[split][${label}]{${this.uncode(style,label)}}{${text}}`;
        all.push(text);
      }
    }
    return all.join('\n');
