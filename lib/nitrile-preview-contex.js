...
class NitrilePreviewContex extends NitrilePreviewTranslator {
  ...
  constructor(parser) {
    ...
    this.contex_linespace = '11.5pt';
    this.contex_linespace_small = '10pt';
    this.contex_linespace_small_tight = '9pt';
  ...
  fence_to_tabular(style,ss,ssi){
    ...
    if(style.fontsize=='small'){
      img = `{\\small\\setupinterlinespace[line=${this.contex_linespace_small}]${img}}`;
      frm = `{\\small\\setupinterlinespace[line=${this.contex_linespace_small}]${frm}}`;
    }
    var o = [];
    var subc = style.subtitle||'';
    var subc = this.uncode(style,subc);
  ...
  fence_to_verbatim(style,ss,ssi){
    ...
    var frm = `\\framed[frame=off,location=top,strut=no]{${text}}`
    if(style.fontsize=='small'){
      img = `\\small\\setupinterlinespace[line=${this.contex_linespace_small_tight}]${img}`
      frm = `\\small\\setupinterlinespace[line=${this.contex_linespace_small_tight}]${frm}`
    }
    var subc = style.subtitle||'';
    var subc = this.uncode(style,subc);
    var o = [];
  ...
  fence_to_parbox(style,ss,ssi) {
    ...
    if(style.fontsize=='small'){
      text = `{\\small\\setupinterlinespace[line=${this.contex_linespace_small}] ${text}}`;
    }
    var img = text;
    var frm = text;
    var subc = style.subtitle||'';
  ...
  float_to_figure(title,label,idnum,idcap,style,ss,ssi){
    ...
    if(this.contex_caption_small){
      s_interls = `\\setupinterlinespace[line=${this.contex_linespace_small_tight}]`
    }
    if(style.swrap){
      let o = [];
  ...
  float_to_listing(title,label,idnum,idcap,style,ss,ssi){
    ...
    var text = o.join('\n');
    if(this.contex_line_small){
      text = `\\setupinterlinespace[line=${this.contex_linespace_small_tight}]${text}`
    }
  ...
  samp_to_text(style,samp){
    ...
    if(this.contex_samp_small){
      opts.push(`style=small`);
      s_interlinespace = `\\setupinterlinespace[line=${this.contex_linespace_small_tight}]`;
    }
    let all = [];
