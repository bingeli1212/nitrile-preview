      ...
      if((v=re_char.exec(line))!==null){
        ...
        switch(s) {
          ...
          default: {
            ...
            if(this.tokenizer.cc_symbol_map.has(cc)){
              let {cex,cmath,fonts} = this.tokenizer.cc_symbol_map.get(cc);
              s = this.tokenizer.get_cex_symbol(cex,cmath,cc,fonts);
            }    
            break;
          }
        } 
  ...
  smooth (line) {
    ...
    while(line.length){
      if((v=re_entity.exec(line))!==null){
        s = v[1];
        line=v[2];        
        if(this.tokenizer.name_symbol_map.has(s)){
          let {cex,cmath,cc,fonts} = this.tokenizer.name_symbol_map.get(s);
          s = this.tokenizer.get_cex_symbol(cex,cmath,cc,fonts);
        }
        safe+=s;
        continue;
