...
class NitrilePreviewContex extends NitrilePreviewTranslator {
  ...
  fence_to_img(style,ss) {
    ...
    ///NOTE that it is impossible to implement the 'contain' and 'cover' in CONTEX as setting either the width or height will
    /// honor that and keep the aspect ratio, but it does not automatically adjust it so that it either uses width or height
    /// depending on the situation, thus currently it is always a 'fill'
  ...
  float_to_figure(title,label,style,ss){
    ...
    if(style.wrap){
      ...
      itms.forEach((p,j,arr) => {
        ...
        all.push(`\\hbox{\\startcombination[1*1] {${p.img}} {${p.subc}} \\stopcombination}`);   
      });
      var o = [];
      o.push('')
      o.push(`\\placefigure`);
  ...
  float_to_table(title,label,style,ss){
    ...
    if(style.subfigure){
      ...
      itms.forEach((p,j,arr) => {
        if(p.type=='bundle'){
          all.push(`{\\startcombination[1*1] {${p.img}} {${p.subc}} \\stopcombination}`);
        }
      });
      var o = [];
      o.push('')
