...
class NitrilePreviewContex extends NitrilePreviewTranslator {
  ...
  //  \blank\noindent
  //  \dontleavehmode
  //  \startitemize[leftmargin=30pt]
  //  \sym {\small{$\blacktriangleright$}} {\texthash}include<stdio> \crlf int main(){\textbraceleft} \crlf   return 0; \crlf {\textbraceright}
  //  \stopitemize
  //  \blank\noindent
    const pointer = `\\symbol[martinvogel 1][MVRightArrow]`;
    // const pointer = `\\symbol[martinvogel 2][PointingHand]`;
    var all = [];
    all.push('');
    all.push('\\blank');
  ...
  samp_to_text(style,samp){
    ...
    if(hew>1){
      let k = Math.ceil(ss.length/2);
      let ss1 = ss.slice(0,k);
      let ss2 = ss.slice(k+1);
      let all = [];
      all.push('');
      all.push('\\blank');//need a blank, otherwise there are no vertical spaces in front of it---dont know why
      all.push(`\\defineparagraphs[sidebyside][n=2]`);
