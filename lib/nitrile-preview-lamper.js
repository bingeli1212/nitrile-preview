...
class NitrilePreviewLamper extends NitrilePreviewLatex {
  ...
  constructor(parser) {
    ...
    this.latex = this.parser.conf_to_list('latex');
  }
  to_document() {
    ...
    this.parser.blocks.forEach((block) => {
      ...
      if(this.only.length){
        if(this.only.indexOf(block.style.src)>=0){
          block.latex = text;
        }
      }else{
        block.latex = text;
...
\\end{document}
  ...
  to_titlelines(){
    ...
    if (mytitle) {
      titlelines.push(`\\title{${this.smooth(mytitle)}}`);
      titlelines.push(`\\author{${this.smooth(myauthor)}}`);
      titlelines.push(`\\date{${this.smooth(mydate)}}`);
    }
    return titlelines;
  }
  to_toclines(){
