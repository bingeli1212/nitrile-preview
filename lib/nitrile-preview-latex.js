...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  float_to_figure(title,ss,style){
    ...
    var style = this.update_g_hints(style,this.hint_figure);
    ...
    itms.forEach((p,j,arr) => {
      ...
      if(key){
        if(p.type=='phrase'){
          ...
          all.push(`\\subfigure[{\\footnotesize ${p.sub}}]{${p.img}}`)
    ...
    if(title){
      o.push(`\\caption{${this.uncode(title,style)}}`);
      if(style.label){
    ...
    while(line.length){
      if((v=re_char.exec(line))!==null){
        ...
        switch(s) {
          ...
          case  "\\":    s = "{\\char92}" ; break;
          case  "⁻¹":    s = "\\textsuperscript{-1}"; break;
          case  "⁻²":    s = "\\textsuperscript{-2}"; break;
          case  "⁻³":    s = "\\textsuperscript{-3}"; break;
          case  "¹":     s = "\\textsuperscript{1}"; break;
          case  "²":     s = "\\textsuperscript{2}"; break;
          case  "³":     s = "\\textsuperscript{3}"; break;
          default: {
            ...
            if(this.unicode_symbol_map.has(cc)){
              s = this.unicode_symbol_map.get(cc).tex;
            }
            break;
          }
        } 
