...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  fence_to_tabular(style,ss,ssi) {
    ...
    rows.forEach((pp,j,arr)=>{
      ...
      if(j==0 && style.head && style.rules=='groups'){
        ...
        //do not add hline after the last row
      }else if(pp.lower==2){
        all.push('\\hline');
      }else if(pp.lower==1){
        all.push('\\hline');
  ...
  fence_to_parbox(style,ss,ssi){
    ...
    if(style.stretch){
      ...
      var w = (stretch)?`${stretch}\\linewidth`:`\\linewidth`;
      text = `\\parbox[][][b]{${w}}{${ragged} ${text}}`;
    }else if(style.width){
    ...
    if(style.frame){
      var img = `\\begin{tabular}{@{}|l|@{}}\\tabularnewline\n${text}\\tabularnewline\\hline\\end{tabular}`;
    }else{
      var img = `\\begin{tabular}{@{}l@{}}${text}\\tabularnewline\\end{tabular}`
    }
    img = text;
  ...
  float_to_figure(title,label,style,ss,ssi){
    ...
    var salign = this.latex_caption_align;
    ...
    if(style.swrap){
      ...
      itms.forEach((p) => {
        if(p.type=='bundle'){
          ...
          let subalign = '\\centering';
          if(this.latex_caption_align=='l'){
            subalign = '\\raggedright'
          }else if(this.latex_caption_align=='r'){
            subalign = '\\raggedleft'
          }
          let subfontsize = '';
          if(this.latex_caption_small){
            subfontsize = '\\footnotesize'
          }      
          o.push(`\\begin{threeparttable} ${p.img} \\begin{tablenotes}[flushleft] \\item ${subalign} ${subfontsize} ${subc} \\end{tablenotes} \\end{threeparttable}`);
          // }else if(p.w){
