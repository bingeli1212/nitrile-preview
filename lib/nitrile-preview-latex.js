...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  fence_to_ink(style,ss) {
    ...
    all.push(`\\setlength{\\unitlength}{1pt}`);
    all.push(`\\begin{picture}(${vw},${vh})`);
    ss.forEach((s,j) => {
      s = this.polish_verb(style,s);
      all.push(`\\put(0,${vh-(j+1)*12+vgap}){\\ttfamily\\fontsize{12pt}{12pt}\\selectfont{}${s}}`);
    });
    ...
    if(width){
      img = `\\resizebox{${width}mm}{${vh/vw*width}mm}{${img}}`;
    }
    if(style.float){
    }else{
      img = `\\hfill\\break\n${img}\\hfill\\break`;
