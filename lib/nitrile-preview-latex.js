...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  to_latex_features() {
    let latex_features = this.conf('latex.features').split('\n')
    return latex_features;
  }
  to_core_packages (latex_program,latex_features) {
    let input = '';
    if(latex_program=='lualatex'){
      input = `\
\\usepackage{fontspec}
...
`
  ...
  to_geometry_layout(latex_program,latex_features){
  ...
  to_fonts_layout(latex_program,latex_features){
    ...
    if(latex_program=='lualatex'){
      all.push(`\\usepackage{ruby}`)
      all.push(`\\newfontfamily{\\jp}[Scale=0.85]{ipaexmincho}`);
      all.push(`\\newfontfamily{\\cn}[Scale=0.85]{arplsungtilgb}`);
      all.push(`\\newfontfamily{\\tw}[Scale=0.85]{arplmingti2lbig5}`);
      all.push(`\\newfontfamily{\\kr}[Scale=0.85]{baekmukbatang}`);
    }
    if(latex_program=='xelatex' && latex_features.indexOf('xecjk') >= 0){
  ...
  to_extra_packages(latex_program,latex_features){
  ...
  to_post_setups(latex_program,latex_features){
