...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  para_to_tabbing(ss,style){
    var rows = this.ss_to_table_rows(ss,style);
    ///data will have already been unmasked
    var n = 1;
    if(rows.length){
      n = rows[0].length;
    }
    ...
    rows.forEach((row,i,arr) => {
      ...
      if(i+1<arr.length){
        ...
        if(style.skipamount=='small'){
          skipamount = `\\smallskipamount`;
        }else if(style.skipamount=='med'){
          skipamount = `\\medskipamount`;
        }else if(style.skipamount=='big'){
          skipamount = `\\bigskipamount`;
        }
      }
      if(skipamount) skipamount = `[${skipamount}]`
      if(style.head && i==0){
        row = row.map((x) => `\\textbf{${x.text}}`);
      }else {
        row = row.map((x) => `${x.text}`);
      }
      row = row.map((x,i,arr) => `\\parbox{${frs[i]}\\linewidth}{${x}}`)
      let s = row.join('\\>');
      s = `${s}\\\\${skipamount}`
      d.push(s);
    });
  ...
  para_to_choices(ss,style) {
    let itms = this.ss_to_bull_itms(ss);
    let text = this.itms_to_bull(itms,style,1);
    return text;
  }
  ...
  string_to_latex_height(width_s,zoom) {
    ///this function is similar to string_to_latex_width except that
    ///if the input is empty, is a percentage value, it returns an
    ///empty string, rather than part of the \\linewidth
    zoom = parseFloat(zoom)||1;
    if (!width_s) {
      return '';
    }
  ...
  itms_to_bull(itms,style){
    ...
    itms.forEach((p,i,arr) => {
      ...
      var cb = '';
      if(style.checkbox){
        let ischecked = this.string_has_item(style.selection,p.bull);
        var cb = `${ischecked?cbox:box}~~`;  
      }
      if(p.type == 'OL'){
        p.text = `${i+1}.${nbsp}${cb}${text}`
      }
      else if(p.type == 'UL'){
        p.text = `\\textbullet${nbsp}${cb}${text}`
      }
      else if(p.type == '0'){
        p.text = `${p.value}${p.ending}${nbsp}${cb}${text}`;
      }
      else if(p.type == 'a'){
        p.text = `${this.to_a_letter(p.value)}${p.ending}${nbsp}${cb}${text}`;
      }
      else if(p.type == 'A'){
        p.text = `${this.to_A_letter(p.value)}${p.ending}${nbsp}${cb}${text}`;
      }
        ...
        if(text){
          p.text = `\\textbf{${cb}${this.unmask(p.value)}}\\hfill\\break\n${text}`;
        }else{
          p.text = `\\textbf{${cb}${this.unmask(p.value)}}`;
        }
      }
      else{
    ...
    if(0){
      ...
      var d = [];
      d.push('');
      var leftmargin = this.get_leftmargin('bull');
      leftmargin = leftmargin||'0pt';
      d.push(`\\begin{list}{}{\\setlength\\itemsep{0pt}\\setlength\\parsep{0pt}\\setlength\\leftmargin{3em}\\setlength\\itemindent{-3em}}`);
      itms.forEach((p,i,arr) => {
        if(this.string_has_item(style.selected,p.item)){
          d.push(`\\item ${cbox}~~${p.text}`);  
        }else{
          d.push(`\\item ${box}~~${p.text}`);
        }
      });
      d.push('\\end{list}')
      return d.join('\n')
      /*
      d.push(`\\begin{tabular}{@{}p{\\linewidth}@{}}`);
      d.push(ss.join('\\tabularnewline\n'));
      d.push(`\\end{tabular}`);
      return d.join('\n')
      */
    }else{
      ...
      var leftmargin = this.get_leftmargin('bull');
      leftmargin = leftmargin||'0pt';
      //d.push(`\\begin{list}{}{\\setlength\\itemsep{0pt}\\setlength\\parsep{0pt}\\setlength\\leftmargin{${leftmargin}}}`);
      if(style.checkbox){
        var leftmargin='3em'
      }else{
        var leftmargin='1.5em'
      }
      d.push(`\\begin{list}{}{\\setlength\\itemsep{0pt}\\setlength\\parsep{0pt}\\setlength\\leftmargin{${leftmargin}}\\setlength\\itemindent{-${leftmargin}}}`);
      itms.forEach((p,i,arr) => {
        d.push(`\\item ${p.text}`);
      });
      d.push('\\end{list}')
...
`;
  ...
  img_to_imagefile(src,g,is_figure){
    ...
    var height = this.string_to_latex_height(g.height,zoom);
    this.imgs.push(src);
    var opts = [];
    ...
    if(g.halign=='c'){
      var text = `\\centering\\includegraphics[${opts.join(',')}]{${src}}`;
    }else{
      var text = `\\includegraphics[${opts.join(',')}]{${src}}`;
    }
    if(is_figure){
      var text = `\\parbox[][][c]{\\linewidth}{${text}}`
    }else{
      var text = `\\parbox[][][c]{\\linewidth}{${text}}`
    }
    return text;    
