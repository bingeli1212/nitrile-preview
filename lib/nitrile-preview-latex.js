...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
    ...
    if(style.wrap){
      ...
      itms.forEach((p) => {
        if(p.type=='bundle'){
          ...
          if(p.tbl){
            o.push(`\\begin{threeparttable} ${p.img} \\begin{tablenotes}[flushleft] \\item ${subc} \\end{tablenotes} ${subc} \\end{threeparttable}`);
          }else if(p.w){
            o.push(`\\begin{minipage}[t]{${p.w}} ${p.img} \\raggedright ${subc} \\end{minipage}`);
          }else{
            o.push(`\\begin{minipage}[t]{\\linewidth} ${p.img} \\raggedright ${subc} \\end{minipage}`);
          }
        }else if(p.type=='\\\\'){
          o.push('\\\\');
        }
      ...
      itms.forEach((p) => {
        if(p.type=='bundle'){
          ...
          if(p.tbl){
            onerow.push(`\\begin{threeparttable} ${p.img} \\begin{tablenotes}[flushleft] \\item ${subc} \\end{tablenotes} ${subc} \\end{threeparttable}`);
          }else if(p.w){
            onerow.push(`\\begin{minipage}[t]{${p.w}} ${p.img} \\raggedright ${subc} \\end{minipage}`);
          }else{
            onerow.push(`\\begin{minipage}[t]{\\linewidth} ${p.img} \\raggedright ${subc} \\end{minipage}`);
          }
        }else if(p.type=='\\\\'){
          let n = onerow.length;
          o.push(onerow.join('~%\n'));
  ...
  float_to_longtabu(title,label,style,ss,ssi){
    ...
    var n = (rows.length) ? rows[0].length : 1;
    var latex = this.g_to_latex_float(style);
    var pcols = this.halign_to_latex_pcols(n,style.halign,latex);
    var pcol = this.pcols_to_latex_pcol(pcols,style,latex);
    var all = [];
    all.push('');
    if(style.head && rows.length){
      let pp = rows.shift();
