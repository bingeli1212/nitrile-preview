...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  fence_to_tab(style,ss,ssi) {
    ...
    var pcols = this.ww_to_latex_pcols(ww,stretch);
    ...
    if(stretch){
      ...
      let totalw = ww.map(p=>p.pw).reduce((acc,cur)=>acc+cur,0);
      if(style.frame=='hsides'){
  ...
  float_to_columns(title,label,style,splitid,body,bodyrow){
    ...
    bundles.forEach(bundle => {
      let p = this.do_bundle(style,bundle);
      o.push(`\\begin{minipage}[t]{${1/n}\\linewidth}`);
      let text = '';
      if(p.fml){
        text = `\\(${p.fml}\\)`;
      }else if(p.img){
  ...
  ww_to_latex_pcols(ww,stretch) {
    ...
    if(stretch){
      ...
      var pp = ww.map((w,j,arr) => {
        let pw = w.pw;
        pw = pw/totalw*arr.length; 
        if(w.ta=='c'){
          return(`>{\\hsize=${pw}\\hsize\\centering\\arraybackslash}X`);
        }else if(w.ta=='r'){
          return(`>{\\hsize=${pw}\\hsize\\raggedleft\\arraybackslash}X`);
        }else{
          return(`>{\\hsize=${pw}\\hsize\\raggedright\\arraybackslash}X`);
        }
      });
    }else{
      var pp = ww.map(w => {
        let pw = Math.max(0,w.pw-4);
        if(w.ta=='c'){
          return(`>{\\centering\\arraybackslash}p\{${pw}mm\}`);
        }else if(w.ta=='r'){
          return(`>{\\raggedleft\\arraybackslash}p\{${pw}mm\}`);
        }else{
          return(`>{\\raggedright\\arraybackslash}p\{${pw}mm\}`);
        }
      });
    }
    return pp;
