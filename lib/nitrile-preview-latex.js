...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  hdgs_to_section(title,label,level,style){
    ...
    if(style.name=='chapter'){
      o.push(`\\section*{${style.chapnum}.${style.sectnum} ${title}}${this.to_latexlabelcmd(label)}`);
    }else{
      o.push(`\\section*{${style.sectnum} ${title}}${this.to_latexlabelcmd(label)}`);
    }
    return o.join('\n')
  }
  hdgs_to_subsection(title,label,level,style){
    ...
    o.push('');
    if(style.name=='chapter'){
      o.push(`\\subsection*{${style.chapnum}.${style.sectnum}.${style.subsnum} ${title}}${this.to_latexlabelcmd(label)}`);
    }else{
      o.push(`\\subsection*{${style.sectnum}.${style.subsnum} ${title}}${this.to_latexlabelcmd(label)}`);
    }
    return o.join('\n')
  }
  hdgs_to_subsubsection(title,label,level,style){
    ...
    o.push('');
    if(style.name=='chapter'){
      o.push(`\\subsubsection*{${style.chapnum}.${style.sectnum}.${style.subsnum}.${style.ssubnum} ${title}}${this.to_latexlabelcmd(label)}`);
    }else{
      o.push(`\\subsubsection*{${style.sectnum}.${style.subsnum}.${style.ssubnum} ${title}}${this.to_latexlabelcmd(label)}`);
    }
    return o.join('\n')
  }
  ...
  fence_to_ink(style,ss,ssi) {
    ...
    var subc = this.uncode(style,subc);
    var o = [];
    o.push({img,subc,width,height,style});
    return o;
  }
  ...
  float_to_figure(title,label,style,bundles){
    ...
    if(!style.partition){
      ...
      itms.forEach((p) => {
        if(p.type=='bundle'){
          let subc = p.subc;
          if(subc){
            o.push(`\\begin{threeparttable} ${p.img} \\begin{tablenotes}[flushleft] \\item ${caption_align} ${caption_fontsize} ${subc} \\end{tablenotes} \\end{threeparttable}`);
          }else{
      ...
      itms.forEach((p) => {
        if(p.type=='bundle'){
          let subc = p.subc;
          if(subc){
            onerow.push(`\\begin{threeparttable} ${p.img} \\begin{tablenotes}[flushleft] \\item ${caption_align} ${caption_fontsize} ${subc} \\end{tablenotes} \\end{threeparttable}`);
          }else{
    ...
    if(style.wrapfig=='right'){
      ...
      o.push(`{${caption_fontsize} {Figure ${idnum}} : ${title}}\\vspace{0.5ex}\\break`);
      if(label){
        o.push(`\\label{${label}}`);
      }
      o.push(text);
  ...
  float_to_table(title,label,style,bundles){
    ...
    if(this.latex_figure_nofloat){
      ...
      o.push(`\\begin{${caption_environ}}`);
      o.push(`${caption_align}`);    
      o.push(`{${caption_fontsize} {Table ${idnum}${splitid}} : ${title}}\\vspace{0.5ex}\\break`);
      if(label){
        o.push(`\\label{${label}}`);
      }
      o.push(text);
      ...
      o.push(`${caption_align}`);    
      o.push(`{${caption_fontsize} {Table ${idnum}} : ${title}}\\vspace{0.5ex}\\break`);
      if(label){
        o.push(`\\label{${label}}`);
      }
      o.push(text);
  ...
  float_to_equation(title,label,style,bundles){
    bundles.forEach((p,j,arr)  => {
      ...
      let str = ss.join('\n')
      let used = new Set();
      let text = this.tokenizer.to_align_math(str,style,used,1);
      p.text = text;
    });
  ...
  float_to_listing(title,label,style,bundles){
    ...
    if(this.latex_caption_small){
      opts.push(`title={\\small Listing ${idnum}${splitid} : ${title}}`);
    }else{
      opts.push(`title={Listing ${idnum}${splitid} : ${title}}`);
    }
    if(label){
  ...
  sand_to_text(style,sand){
    ...
    if(hew>1){
      ...
      nn.forEach((nl,j) => {
        ...
        let ss1 = ss.slice(n1,n2);
        let text1 = ss1.join('\\\\');
        all.push(`\\begin{flushleft}${fontsize_css}`);               
        all.push(text1);
  ...
  to_preamble_essentials_xelatex(){
    ...
    if(this.bodyfontsuit=='linux'){
      if(this.bodyfontvariant=='ss'){
        all.push(`\\renewcommand{\\baselinestretch}{0.95}`);
        all.push(`\\setmainfont[Ligatures=TeX]{Libertinus Sans}`);
        all.push(`\\linespread{0.8}\\selectfont`);
      }else{
        all.push(`\\renewcommand{\\baselinestretch}{0.95}`);
