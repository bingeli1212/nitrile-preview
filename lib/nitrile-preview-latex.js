...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  pcols_to_pcol(pcols,style){
    ...
    if(style.vline){
      ...
      console.log('james','vline',vline,'vcols',vcols)
      var n = pcols.length;
      var mycols = [];
      for(var i=0; i < n; ++i){
        mycols.push(vcols[i]||'');
  ...
  rows_to_tabular(rows,style){
    ...
    if(style.cellcolor){
      let my = this.string_to_array(style.cellcolor);
    ...
    for (let j = 0; j < nrows; ++j) {
      ...
      var pp = pp.map(x => {
        if(cellcolors.has(x)){
          let my_color = cellcolors.get(x);
          return `\\cellcolor{${my_color}}{}${x}`
        }else{
          return x;
        }
      })
