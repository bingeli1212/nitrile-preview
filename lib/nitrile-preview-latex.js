...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  do_HDGS(block){
    var {hdgn,name,title,style,switches} = block;
    var o = [];
    o.push('');
    var raw = title;
    var title = this.uncode(title,style,switches);//note that it might have something like \jp
                                 //which is fine because the "bookmark" package 
                                 //will remove it
    var star='';
    var label = style.label||'';
    if(name == 'part'){
      o.push(`\\part{${this.uncode(title,style,switches)}}`);
  ...
  do_PLST(block){
    var o = [];
    var {plitems,style,switches} = block;
    ...
    for (var item of items) {
      ...
      text = text || '';
      text = this.uncode(text,style,switches);
      if (bull==='OL'||bull==='UL') bull0 = bull;
      let extra_text = '';
      if(more && more.length){
        more.forEach((p) => {
          let {lines} = p;
          extra_text+=`\n\n${this.untext(lines,style,switches).trim()}`;///make sure to always insert a blank line between each non-item paragraph
        });
      }
      switch (bull) {
        case 'OL': {
  ...
  do_SAMP(block){
    ...
    if(style.samp=='framed'){
      var text = this.to_SAMP_framed(ss,style,switches);
    }else {
      var text = this.to_SAMP_verbatim(ss,style,switches);
    }
    block.latex = text;
  }
  do_HRLE(block){
    var o = [];
    var {id,row1,row2,sig,title,switches} = block;
    o.push('');
    title = this.uncode(title,style,switches);
    o.push(`\\begin{center}`);
  ...
  do_PRIM(block){
    var o = [];
    var {hdgn,title,body,style,switches} = block;
    o.push('');
    var v;
    const indent = '~'.repeat(5);
    title = this.uncode(title,style,switches);
    let s0 = body[0]||'';
    let text = this.untext(body,style,switches);
    if (hdgn===0) {
  ...
  do_PARA(block){
    var {body,style,switches} = block;
    var text = this.untext(body,style,switches);
    var o = [];
    o.push('');
    o.push(text);
    text = o.join('\n');
  ...
  // phrase_to_XXX
  //
  phrase_to_inlinemath(s,style,switches) {
    var s = this.tokenizer.to_lmath(s,style);
    return `\\(${s}\\)`;
  }
  phrase_to_displaymath(s,style,switches) {
    var s = this.tokenizer.to_lmath(s,style,1);
    return `\\[${s}\\]`;
  }
  phrase_to_ref (s,style,switches) {
  ...
  phrase_to_link(s,style,switches) {
    if(s){
      return `\\url{${s}}`
    }
    return '??';
  }
  phrase_to_parbox(s,style,switches) {
    var latex_width = this.string_to_latex_width(style.width,1,'1cm');
    return `\\parbox{${latex_width}}{${this.uncode(s,style,switches)}}`
  }
  //
  // fence_to_XXX
  // 
  fence_to_math(ss,style) {
    var str = ss.join('\n')
    let s = this.tokenizer.to_lmath(str,style);
    return `\\(${s}\\)`;
  }
  ...
  fence_to_img(ss,style) {
    var text = this.ss_to_includegraphics(ss,style);
    if (style.float) {
      let f = (style.float == 'left') ? 'l' : 'r';
  ...
  fence_to_blockquote(ss,style){
    var leftmargin = this.conf('latex.blockquote.leftmargin',0,'int');
    var fontsize = this.conf('latex.blockquote.fontsize');
    ...
    o.push('');
    o.push(`\\begin{list}{}{\\setlength\\itemsep{0pt}\\setlength\\parsep{0pt}\\setlength\\leftmargin{${leftmargin}pt}}`);
    ss.forEach(s => {
      o.push(`\\item ${s}`)
    })
    o.push(`\\end{list}`)
  ...
  fence_to_verbatim(ss,style){
    var leftmargin = this.conf('latex.verbatim.leftmargin',0,'int');
    var fontsize = this.conf('latex.verbatim.fontsize');
    var fontsize = this.tokenizer.to_latex_fontsize(fontsize);
    var ss = ss.map(s => {
      s = this.polish(s);
      s = s.replace(/\s/g, "~");
  ...
  fence_to_tabularx(ss,style,switches) {
    var title = style.title||'';
    var width = style.width||'';
    var rows = this.ss_to_tabular_rows(ss,style);
    rows.forEach((pp) => {
      pp.forEach((p) => {
        p.text = this.uncode(p.raw,style,switches);
      })
    })
    let n = 1;
    if(rows.length){
    ...
    if(1){
      ...
      o.push(`\\begin{tabularx}{${latex_width}}{${pcol}}`);
      if(title){
        let title_text = this.uncode(title,style,switches);
      ...
      if(style.head){
        for(var j=0; j < style.head && rows.length; j++){
          var pp = rows[0];
          rows = rows.slice(1);
      ...
      var text = this.rows_to_tabular(rows, style, switches, hsizes);
      o.push(text);
    ...
    var text = this.to_fontsize_text(text,style);
    return text;
  }
  fence_to_tabular(ss,style,switches) {
    var title = style.title||'';
    var rows = this.ss_to_tabular_rows(ss,style,switches);
    rows.forEach((pp) => {
      pp.forEach((p) => {
        p.text = this.uncode(p.raw,style,switches);
      })
    ...
    if(1){
      ...
      var text = this.rows_to_tabular(rows, style, switches);
      o.push(text);
      if(hrows.indexOf('b')>=0 || hrows.indexOf('*')>=0){
        o.push('\\hline')
      }
  ...
  fence_to_tabbing(ss,style,switches){
    var rows = this.ss_to_tabular_rows(ss,style);
    rows.forEach((pp) => {
      pp.forEach((p) => {
        p.text = this.uncode(p.raw,style,switches);
  ...
  fence_to_list(ss,style,switches) {
    var itms = this.ss_to_bull_itms(ss,style,switches);
    var text = this.itms_to_bull(itms,style,switches);
    var text = this.to_fontsize_text(text,style,switches);
    return text;
  }
  /// 
  /// paras
  /// 
  to_SAMP_framed(ss,style,switches){
    ss = this.trim_samp_body(ss);
    var text = this.ss_to_framed(ss,style,switches);
    var o = [];
    o.push('');
  ...
  to_SAMP_verbatim(ss,style,switches){
    ss = this.trim_samp_body(ss);
    var o = [];
    o.push('');
    ss = ss.map(s => {
  ...
  rows_to_tabular(rows,style,switches,hsizes){
  ...
  itms_to_bull(itms,style,switches){
    ...
    if(style.bullet){
      try{
        let s = this.tokenizer.get_tex_symbol(style.bullet);
        bullet = s;
      }catch(e){
      }
    }
    itms.forEach((p,i,arr) => {
      p.text = this.uncode(p.text,style,switches);
      p.bull = this.uncode(p.bull,style,switches);
      p.value = this.uncode(p.value,style,switches);
      if(p.type == 'UL'){
        p.latex = `${bullet}${nbsp}${p.text}`
      }
      else if(p.type == 'OL'){
        p.latex = `${i+1}.${nbsp}${p.text}`
      }
      else if(p.type == '1'){
        if(!p.ending) p.ending = '.';
        p.latex = `${p.value}${p.ending}${nbsp}${p.text}`;
      }
      else if(p.type == 'a'){
        if(!p.ending) p.ending = '.';
        p.latex = `${this.to_a_letter(p.value)}${p.ending}${nbsp}${p.text}`;
      }
      else if(p.type == 'A'){
        if(!p.ending) p.ending = '.';
        p.latex = `${this.to_A_letter(p.value)}${p.ending}${nbsp}${p.text}`;
        ...
        if(p.text){
          p.latex = `\\textbf{${p.value}}\\hfill\\break\n${p.text}`;
        }else{
          p.latex = `\\textbf{${p.value}}`;
        }
      }
      else if(p.type == 'CB'){
        if(!p.bull.trim().length){
          p.latex = `${squareboxo} ${p.text}`;
        }
        else{
          p.latex = `${squarebox} ${p.text}`;
        }
      }
      else if(p.type == 'RB'){
        if(!p.bull.trim().length){
          p.latex = `${circleboxo} ${p.text}`;
        }
        else{
          p.latex = `${circlebox} ${p.text}`;
        }
      }
      else{
    ...
    if(itms.length){
      var o = [];
      o.push('');
      if(this.string_has_item(style.user,'isframesubtitle')){
        o.push(`\\begin{list}{}{\\setlength\\itemsep{0pt}\\setlength\\parsep{0pt}}`);
      }else{
  ...
  itms_to_subfigures(itms,style,switches) {
    var d = [];
    itms.forEach((p,i,arr) => {
      var {type,ss,g,sub} = p;
      p.sub = this.uncode(sub,style,switches);
      if(type=='diagram'){
        p.img = this.fence_to_diagram(ss,g,switches);
      }else if(type=='framed'){
        p.img = this.fence_to_framed(ss,g,switches);
      }else if(type=='img'){
        p.img = this.fence_to_img(ss,g,switches); 
      }else{
        p.img = '\\\\';
      }
    });
...
`;
  ...
  to_report_part(top) {
    ...
    all.push('');
    if(my){
      all.push(`\\part{${this.uncode(my.title,my.style,my.switches)}}`);
    }
    top.forEach((o,i,arr) => {
      if (Array.isArray(o)) {
  ...
  to_report_chapter(top) {
    ...
    let all = [];
    all.push('');
    all.push(`\\chapter{${this.uncode(my.title,my.style,my.switches)}}${this.to_latexlabelcmd(my.style.label)}`);
    top.forEach((o,i,arr) => {
      if (Array.isArray(o)) {
  ...
  to_report_section(top) {
    ...
    let all = [];
    all.push('');
    all.push(`\\section{${this.uncode(my.title,my.style,my.switches)}}${this.to_latexlabelcmd(my.style.label)}`);
    top.forEach((o, i) => {
      if (Array.isArray(o)) {
  ...
  to_report_subsection(top) {
    ...
    let all = [];
    all.push('');
    all.push(`\\subsection{${this.uncode(my.title,my.style,my.switches)}}${this.to_latexlabelcmd(my.style.label)}`);
    top.forEach((o,i) => {
      if(Array.isArray(o)){
  ...
  to_report_subsubsection(top) {
    ...
    let all = [];
    all.push('');
    all.push(`\\subsubsection{${this.uncode(my.title,my.style,my.switches)}}${this.to_latexlabelcmd(my.style.label)}`);
    top.forEach((o,i) => {
      if(Array.isArray(o)){
      }else{
        all.push(o.latex);
  ...
  to_titlelines(){
    ...
    if(block && block.sig=='FRNT'){
      ...
      for(let t of data){
        let [key,val] = t;
        if(key=='title'){
          titlelines.push(`\\title{${this.uncode(val,style,switches)}}`);
        }
        else if(key=='author'){
          titlelines.push(`\\author{${this.uncode(val,style,switches)}}`);
        }
      }
    }
    return titlelines;
  ...
  ss_to_framed(ss,style,switches){
  ...
  ss_to_includegraphics(ss,style,switches){
    if(ss.length){
      var fname = ss[0];
    }else{
      var fname = '';
  ...
  ///NOTE 
  do_FLOA_equation(block,caption,ss,style,switches){
  ...
  do_FLOA_listing(block,caption,ss,style,switches){
    let opts = [];
    if(caption){
      var caption_text = this.uncode(caption,style,switches);
  ...
  do_FLOA_figure(block,caption,ss,style,switches){
    if(style.subfigure){
      let itms = this.ss_to_figure_itms(ss,style);
      var text = this.itms_to_subfigures(itms,style,switches);
    }else{
      var text = this.untext(ss,style,switches);
    }
    let d = [];
    if(style.wide==1){
  ...
  do_FLOA_tablex(block,caption,ss,style,switches){
    ///extract caption
  ...
  do_FLOA_table(block,caption,ss,style,switches){
    ///extract caption
  ...
  do_FLOA_longtable(block,caption,ss,style,switches){
    var rows = this.ss_to_tabular_rows(ss,style);
    rows.forEach((pp) => {
      pp.forEach((p) => {
        p.text = this.uncode(p.raw,style,switches);
  ...
  ///
  to_fontsize_text(text,style,switches){
    if(style.fontsize){
