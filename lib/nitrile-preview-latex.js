...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  phrase_to_framed(fname,id,style) {
    var text = '';
    if(id){
      var {body} = this.fetch_from_notes(id);
      if(body){
        text = this.fence_to_framed(body,style);
      }
    }
    return text;
  }
  phrase_to_diagram(fname,id,style) {
    var text = '';
    if(id){
      var {body} = this.fetch_from_notes(id);
      if(body){
        text = this.fence_to_diagram(body,style);
      }
    }
    return text;
  }
  phrase_to_img(fname,id,style) {
    if(fname){
      ///external image file
  ...
  fence_to_diagram(ss,style){
    let text = this.body_to_diagram(ss,style);
    if(style.float){
      let f = (style.float == 'left') ? 'l' : 'r';
  ...
  fence_to_framed(ss,style){
    var text = this.body_to_framed(ss,style);
    if(style.float){
      let f = (style.float == 'left') ? 'l' : 'r';
      text = `\\begin{wraptable}{${f}}{0pt}${text}\\end{wraptable}~`;
    }else if(style.figure){
  ...
  para_to_longtable(ss,style){
    ...
    if(style.head){
      ...
      if(pp && this.row_is_dhline(pp)){
        ...
        d.push('\\hline');
        rows = rows.slice(1);
      }else if(this.string_has_item(hline,'t')){
        d.push('\\hline');
      }
      d.push('\\endhead');
    }else{
  ...
  itms_to_figure(itms,style) {
    ...
    itms.forEach((p,i,arr) => {
      ...
      if(type=='diagram' && id){
        ...
        g.figure = 1;
        let text = this.phrase_to_diagram('',id,g);
        p.img = text;
        p.sub = this.unmask(sub);
      }else if(type=='framed' && id){
        ///this is a embedded diagram
        g.figure = 1;
        let text = this.phrase_to_framed('',id,g);
        p.img = text;
        p.sub = this.unmask(sub);
        ...
        g.figure = 1;
        let text = this.phrase_to_img(fname,'',g); 
