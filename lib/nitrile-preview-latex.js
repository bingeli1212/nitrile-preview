...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  do_HDGS(block){
    ...
    o.push('');
    var raw = title;
    var title = this.uncode(title);//note that it might have something like \jp
                                 //which is fine because the "bookmark" package 
                                 //will remove it
    var star='';
    var label = style.label||'';
    if(name == 'part'){
      o.push(`\\part{${this.uncode(title)}}`);
  ...
  fence_to_tabularx(ss,style) {
    ...
    if(1){
      ...
      if(title){
        let title_text = this.uncode(title);
  ...
  toFramedFigure (text) {
    ...
    for (var pp of text) {
      pp = pp.map( x => {
          ...
          var { height } = opts;
          if (height) {
            return `\\begin{subfigure}[t]{${width}\\linewidth}\\includegraphics[keepaspectratio=true,height=${height},width=\\linewidth]{${src}}\\caption{${this.uncode(sub)}}\\end{subfigure}`;
          } else {
            return `\\begin{subfigure}[t]{${width}\\linewidth}\\includegraphics[keepaspectratio=true,width=\\linewidth]{${src}}\\caption{${this.uncode(sub)}}\\end{subfigure}`;
          }
      });
  ...
  rows_to_xltabular(rows,islabeled,label,caption){
    ...
    if(islabeled){
      if(label){
        d.push(`\\caption{${this.uncode(caption)}\\label{${label}}}\\\\`);
      }else{
        d.push(`\\caption{${this.uncode(caption)}}\\\\`);
      }
    }
    for (var j = 0; j < nrows; ++j) {
      var pp = rows[j];
  ...
  itms_to_bull(itms,style){
    ...
    let squareboxo = this.uncode('&SquareBoxO;');
    let squarebox = this.uncode('&SquareBox;');
    let circleboxo = this.uncode('&CircleBoxO;');
    let circlebox = this.uncode('&CircleBox;')
    const nbsp = '~~';
    if(style.bullet){
      try{
    ...
    itms.forEach((p,i,arr) => {
      p.text = this.uncode(p.text);
      p.bull = this.uncode(p.bull);
      p.value = this.uncode(p.value);
      if(p.type == 'UL'){
...
`;
  ...
  do_FLOA_listing(block,caption,ss,style,switches){
    let opts = [];
    if(caption){
      var caption_text = this.uncode(caption);
