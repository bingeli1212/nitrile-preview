...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  itms_to_bull(itms,style){
    ...
    itms.forEach((p,i,arr) => {
      ...
      if(p.type == 'checkbox'){
        if(this.string_has_item(style.selection,p.value)){
          p.text = `${cbox}~~${p.value}${p.ending}~${text}`;  
        }else{
          p.text = `${box}~~${p.value}${p.ending}~${text}`;  
        }
      }
      else if(p.type == 'OL'){
        p.text = `${i+1}.${nbsp}${text}`
      }
      else if(p.type == 'UL'){
        p.text = `\\textbullet${nbsp}${text}`
      }
      else if(p.type == '0'){
        p.text = `${p.value}${p.ending}${nbsp}${text}`;
      }
      else if(p.type == 'a'){
        p.text = `${this.to_a_letter(p.value)}${p.ending}${nbsp}${text}`;
      }
      else if(p.type == 'A'){
        p.text = `${this.to_A_letter(p.value)}${p.ending}${nbsp}${text}`;
      }
        ...
        if(text){
          p.text = `\\textbf{${this.unmask(p.value)}}\\hfill\\break\n${text}`;
        }else{
          p.text = `\\textbf{${this.unmask(p.value)}}`;
        }
      }
      else{
