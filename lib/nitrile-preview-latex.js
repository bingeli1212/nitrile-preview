...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  item_ul_to_text(style,i,item,nblank){
    ...
    var text = this.untext(style,item.body);
    if(key){
      o.push(`\\item \\textbf{${key}} ${sep} ${text}`)
    }else{
      o.push(`\\item ${text}`);
    }
    if(item.more && item.more.length){
  ...
  fence_to_img(style,ss) {
    ...
    if(fname.length){
      ...
      var img = `\\setlength{\\unitlength}{1cm}\\begin{picture}(12,7)\\graphpaper[1](0,0)(${viewport_width},${viewport_height})\\put(1,3){empty filename}\\end{picture}`;
