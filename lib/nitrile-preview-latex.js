...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  phrase_to_ref (label) {
    ...
    if(g){
      ...
      if(name=='equation'){
        return `Equation~\\ref{${label}}`;
      }
      if(name=='figure'){
        return `Figure~\\ref{${label}}`;
      }
      if(name=='listing'){
        return `Listing~\\ref{${label}}`;
      }
      if(name=='table'){
        return `Table~\\ref{${label}}`;
      }
    }
  ...
  phrase_to_img(cnt) {
    ...
    if(g.src){
      ...
      if(g.width && g.aspectratio){
        let width = this.string_to_latex_width(g.width);
        let height = this.string_to_latex_height_with_aspect_ratio(g.width,g.aspectratio);
  ...
  rows_to_tabulate(rows,style){
    ...
    if(style.width){
      let w = style.width;
      w = this.string_to_latex_width(w);
