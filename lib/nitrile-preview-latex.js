...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  phrase_to_img(cnt) {
    ...
    if(0 && g.src){
      ///NOTE:: note used because this method does not allow for
      ///images to keep their aspect ratio while being shown maximized
      let src = g.src;
      this.imgs.push(src);
      let my_opt = [];
      ...
      if(g.width){
        ...
        let height = this.string_to_latex_width(g.width,rate);
        my_opt.push(`width=${width}`);
        my_opt.push(`height=${height}`)
      }else{
        my_opt.push(`width=\\linewidth`);
        my_opt.push(`height=${rate}\\linewidth`);
      }
      if(g.frame==1){
        my_opt.push(`frame`)
      }
    ...
    if(g.src){
      ...
      let rate = asp[1]/asp[0];
      if(g.width){
        var width = this.string_to_latex_width(g.width,1);
        var height = this.string_to_latex_width(g.width,rate);
        my_opt.push(`width=${width}`);
        my_opt.push(`height=${height}`)
      }else{
        var width = `\\linewidth`;
        var height = `${rate}\\linewidth`
        my_opt.push(`width=${width}`);
        my_opt.push(`height=${height}`);
      }
      my_opt.push(`keepaspectratio`)
      var text = `\\includegraphics[width=\\linewidth,keepaspectratio]{${src}}`;
      var text = `\\parbox[][${height}][c]{${width}}{${text}}`
      if(g.frame==1){
        var text = `\\fbox{${text}}`; 
      }
      if(g.float) {
  ...
  fence_to_table(ss,style) {
    ...
    let text = this.rows_to_tabular(rows, style);
    if(1){
      ...
      if(caption){
        ...
        o.push(`\\multicolumn{${n}}{c}{${caption_text}}\\tabularnewline`)
      }
  ...
  para_to_table(ss,style){
    ...
    if(style.long){
      ...
      if(caption){
        ...
        if(style.label){
          d.push(`\\label{${style.label}}\\tabularnewline`)
        }else{
      ...
      if(rows.length){
        ...
        d.push(`${header.join(' & ')}\\tabularnewline`);
        if(rows.length && this.row_is_hline(rows[0])){
          rows = rows.slice(1);
          d.push('\\hline \\endhead')
        }else if(rows.length && this.row_is_dhline(rows[0])){
          rows = rows.slice(1);
          d.push('\\hline\\hline \\endhead')
        }else if(hrows.indexOf('t')>=0 || hrows.indexOf('*')>=0){
          d.push('\\hline \\endhead');
        }
        ///footer
        if (hrows.indexOf('b')>=0){
          d.push(`\\hline \\endfoot`);
        } 
      }
  ...
  smooth (unsafe) {
    ...
    unsafe = unsafe.replace(this.re_all_subs, (match,p1,p2) => {
          // I_1
          return `${T6}${T5}text${T3}${p1}${T4}${T2}${p2}${T6}`;  // octal code \02 is for underscore
      })
  ...
  rows_to_tabular(rows,style){
    ...
    for (let j = 0; j < nrows; ++j) {
      ...
      if(!has_hline && this.hrows_has_items('*+',hrows)){
        has_hline = 1;
      }
      if(j>0 && has_hline){
        if(has_hline==2){
  ...
  itms_to_bull(itms,style){
    let pp = itms.map((p,i) => {
      ...
      if(p.bull == '**'){
        return `\\hangindent=0.7cm ${i+1}.${nbsp}${text}`
      }
      if(p.bull == '--'){
        return `\\hangindent=0.7cm \\textbullet${nbsp}${text}`
      }
      if(p.type == '0'){
        return `\\hangindent=0.7cm ${p.value}${p.ending}${nbsp}${text}`;
      }
      if(p.type == 'a'){
        return `\\hangindent=0.7cm ${this.to_a_letter(p.value)}${p.ending}${nbsp}${text}`;
      }
      if(p.type == 'A'){
        return `\\hangindent=0.7cm ${this.to_A_letter(p.value)}${p.ending}${nbsp}${text}`;
      }
      if(p.type == 'DL'){
        return `\\hangindent=0.7cm \\textbf{${p.value}}${p.ending}${nbsp}${text}`;
      }
      return `\\hangindent=0.7cm ${text}`
    });
    var o = [];
    o.push(`\\begin{tabularx}{\\linewidth}{@{}>{\\hsize=1.00\\hsize\\raggedright\\arraybackslash}X@{}}`);
  ...
  itms_to_lines(itms,style){
    ...
    d.push(`\\begin{tabularx}{\\linewidth}{@{}${pcol}@{}}`);
    for(let i = 0; i < k; ++i){
