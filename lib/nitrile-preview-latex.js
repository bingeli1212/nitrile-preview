...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  fence_to_ink(style,ss) {
    ...
    ss.forEach((s,j) => {
      ...
      all.push(`\\put(0,${inkheight-(j+1)*10*this.PT_TO_MM+vgap}){\\ttfamily\\fontsize{10pt}{10pt}\\selectfont{}${s}}`);
    });
  ...
  fence_to_img(style,ss) {
    ...
    if(fname.length){
      var img = this.to_tikz_image(style,fname,mywidth,myheight,fit,xm,ym,unit,grid);
      if(style.frame){
        img = `\\frame{${img}}`;
      }
    }else{
  ...
  ///
  //////////////////////////////////////////////////////////////////////////////////////
  to_tikz_image(style,imagefile,width,height,fit,xm,ym,unit,grid){
