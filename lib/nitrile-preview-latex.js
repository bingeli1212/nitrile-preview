...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  phrase_to_framed(fname,id,g) {
    if(id){
      ///embedded diagram
      var text = this.note_to_framed(id,g);
      if(text){
        ///if valid
        if (g.float) {
  ...
  phrase_to_diagram(fname,id,g) {
    if(id){
      ///embedded diagram
      var text = this.note_to_diagram(id,g);
      if(text){
        ///if valid
        if (g.float) {
          let f = (style.float == 'left') ? 'l' : 'r';
  ...
  phrase_to_img(fname,id,g) {
    if(fname){
      ///external image file
      var text = this.fname_to_img(fname,g);
      if(text && g.float) {
  ...
  para_to_verb(ss,style){
    var text = this.fence_to_verb(ss,style);
    if(ss.length && style._){
      let o = [];
      o.push('');
      o.push('\\begin{flushleft}');
      o.push(text);
  ...
  itms_to_figure(itms,style) {
    ...
    itms.forEach((p,i,arr) => {
      ...
      if(type=='diagram' && id){
        ...
        let text = this.note_to_diagram(id,g);
        /// this is so that if the image file is smaller than the width of the table data cell then it is centered horizontally
        text = `\\parbox[][][c]{\\linewidth}{\\centering{}${text}}`
        p.img = text;
        p.sub = this.unmask(sub);
      }else if(type=='framed' && id){
        ///this is a embedded diagram
        let text = this.note_to_framed(id,g);
        /// this is so that if the image file is smaller than the width of the table data cell then it is centered horizontally
        text = `\\parbox[][][c]{\\linewidth}{\\centering{}${text}}`
        p.img = text;
        p.sub = this.unmask(sub);
      }else if(type=='img' && fname){
        ///this is an external img file
        fname = this.svg_to_png_src(fname);
        let text = this.fname_to_img(fname,g); 
        /// this is so that if the image file is smaller than the width of the table data cell then it is centered horizontally
        text = `\\parbox[][][c]{\\linewidth}{\\centering{}${text}}`
        p.img = text;
        p.sub = this.unmask(sub);
...
`;
  ...
  ss_to_framed(ss,style){
    ...
    if(1){
      text = `\\setlength\\fboxsep{1pt}\\fbox{${text}}`
    }
    return text;
  }
  fname_to_img(fname,g){
  ...
  note_to_diagram(id,g){
    let {body,style} = this.fetch_from_notes(id);
    if(body){
      var text = this.ss_to_diagram(body,g);
      return text;
    }
    return '';
  }
  note_to_framed(id,g){
    let {body,style} = this.fetch_from_notes(id);
    if(body){
      var text = this.ss_to_framed(body,g);
      return text;
    }
    return '';
  }
