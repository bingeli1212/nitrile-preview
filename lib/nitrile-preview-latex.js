...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  img_to_imagefile_for_figure(src,g){
    var zoom = parseFloat(g.zoom)||1;
    this.imgs.push(src);
    var text = `\\centering\\includegraphics[width=${zoom}\\linewidth,keepaspectratio]{${src}}`;
  ...
  itms_to_figure(itms,style) {
    ...
    itms.forEach(p => {
      ...
      if(p.img._ && p.img._[0] == '#'){
        ...
        let text = this.img_to_imagefile_for_figure(src,g); 
        p.img = text;
        p.sub = this.unmask(p.sub);
      }
    });
