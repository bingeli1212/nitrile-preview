...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  fence_to_img(style,ss) {
    ...
    if(fname.length){
      var img = this.to_tikz_image(style,fname,fit,xm,ym,unit,grid);
      if(style.frame){
        img = `\\frame{${img}}`;
      }
      var img = `\\resizebox{${mywidth}mm}{${myheight}mm}{${img}}`;
    }else{
      let vw = mywidth;
      let vh = myheight;
  ...
  fence_to_verbatim(style,ss){
    ...
    var all = [];
    if(style.float=='figure'||style.float=='table'){
      if(style.frame){
      ...
      var img = all.join('\n')
      if(width){
        img = `\\resizebox{${width}mm}{!}{${img}}`;
      }
      var img = `\\raisebox{\\height}{${img}}`;
      if(style.frame){
        img = `\\frame{${img}}`;
      }
    }else{
  ...
  fence_to_parbox(style,ss){
    ...
    if(width && height){
      var w = `${width}mm`;
      text = `\\parbox[][${height}mm][t]{${width}mm}{${text}}`;
    }else if(width){
      var w = `${width}mm`;
      text = `\\parbox{${width}mm}{${text}}`;
    }else{
      var w = '';
  ...
  float_to_figure(title,label,style,ss){
    ...
    if(style.wrap){
      ...
      itms.forEach((p) => {
        if(p.type=='bundle'){
          if(p.w){
            onerow.push(`\\begin{minipage}[t]{${p.w}} ${p.img} ${p.subc} \\end{minipage}`);
          }else{
            onerow.push(`\\begin{minipage}[t]{\\linewidth} ${p.img} ${p.subc} \\end{minipage}`);
          }
        }else if(p.type=='\\\\'){
          o.push('\\\\');
        }
      ...
      itms.forEach((p) => {
        if(p.type=='bundle'){
          ...
          if(p.w){
            onerow.push(`\\begin{minipage}[t]{${p.w}} ${p.img} ${subc} \\end{minipage}`);
          }else{
            onerow.push(`\\begin{minipage}[t]{\\linewidth} ${p.img} ${subc} \\end{minipage}`);
          }
        }else if(p.type=='\\\\'){
          let n = onerow.length;
          o.push(`\\begin{tabular}{${'l'.repeat(n)}} \n ${onerow.join(' &\n')} \\\\\n \\end{tabular}`);
          o.push('\\\\');
          onerow = [];
        }
      })
  ...
  float_to_table(title,label,style,ss){
    ...
    if(style.wrap){
      ...
      itms.forEach((p,j,arr) => {
        if(p.type=='bundle'){
          if(this.name=='memoir'){
            o.push(`\\subbottom[][${p.subc}]{${p.img}}`)
          }else{
            o.push(`\\subfigure[][${p.subc}]{${p.img}}`)
          }
        }else if(p.type=='\\\\'){
          o.push(`\\\\`);
        }
      ...
      itms.forEach((p) => {
        if(p.type=='bundle'){
          if(p.subc && p.w){
            ...
            o.push(`\\begin{threeparttable}`);
            o.push(p.img);
            o.push(`\\begin{tablenotes}[flushleft]`);
            o.push(`\\item ${p.subc}`);
            o.push(`\\end{tablenotes}`);
            o.push(`\\end{threeparttable}`);
          }else{
            o.push(p.img);
          }
        }else if(p.type=='\\\\'){
  ...
  float_to_columns(title,label,style,ss){
    ...
    rows.forEach((row,j) => {
      ...
      for(let k=0; k < n; ++k){
        ...
        var text = text||'~';
        var text = text.trim();
        var text = `\\parbox[][][t]{\\linewidth}{\\raggedright{}${text}}`;
        if(k>0){
  ...
  untext(style,body){
    ...
    if(text.endsWith(prefix)){
      text = text.substr(0,text.length-prefix.length);
    }
    text = text.trim();
    all.push('');
    all.push(text);
  ...
  to_tikz_image(style,fname,fit,xm,ym,unit,grid){
    ...
    var gridcolor = this.diagram.string_to_tikz_color(gridcolor,0);
    var all = [];
