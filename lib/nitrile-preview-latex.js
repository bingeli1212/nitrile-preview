...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  fence_to_img(style,ss) {
    ...
    if(fit=='contain'){
      ...
      //NOTE that the 'cover' is impossible in LATEX as includegraphics does not have an option for it
      ///thus it falls back to 'contail'
      opts.push('keepaspectratio');
    }else{
      //default is fill
    }
  ...
  fence_to_verbatim(style,ss){
    ...
    if(style.float=='figure'||style.float=='table'){
      ...
      var img = `\\hfill\\break${img}\\hfill\\break`;
    }
    var subc = '';
    var o = [];
    o.push({img,subc});
  ...
  float_to_figure(title,label,style,ss){
    ...
    if(style.subfigure){
      ...
      itms.forEach((p,j,arr) => {
        if(p.type=='bundle'){
          if(this.name=='memoir'){
            d.push(`\\subbottom[][${p.subc}]{${p.img}}`)
          }else{
            d.push(`\\subfigure[][${p.subc}]{${p.img}}`)
          }
          let s = d.join('\n');
          all.pop();
          all.push(s);
  ...
  float_to_table(title,label,style,ss){
    ...
    if(style.wrap){
      ...
      itms.forEach((p,j,arr) => {
        if(this.name=='memoir'){
          ///note that "memoir" class does not work well with subfigure package, thus we use the subbottom which is the native command of this class
          d.push(`\\subbottom[][${p.subc}]{${p.img}}`)  
        }else{
          d.push(`\\subfigure[][${p.subc}]{${p.img}}`)
        }
