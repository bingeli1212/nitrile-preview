...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  fence_to_tab(style,ss,ssi) {
    ...
    rows.forEach((pp,j) => {
      pp.forEach((p,i) => {
        p.text = this.to_formatted_text(p.raw,fm[i]);
    ...
    var pcol = this.ww_to_latex_pcol(ww,hew,style.rules,style.side,vrules);
  ...
  ss_to_ink(style,ss,ssi) {
    ...
    var stretch = this.assert_float(style.stretch,0,0,1);
    if(stretch > 0){
      img = `\\resizebox{${stretch}\\textwidth}{!}{${img}}`;
    }else{
