...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  constructor(parser) {
    ...
    this.icon_nabla = '{$\\nabla$}'; 
    this.icon_trianglenone = '{~~~}'; 
    this.icon_triangledown = '\\ensuremath{\\triangledown}'; 
    this.icon_dag = '{\\dag}'; 
  ...
  float_alignright(title,label,style,subtitles,body,bodyrow){
        ...
        if(this.re_body_is_fence.test(p.lines[0])){
          ...
          let lines = this.body_to_body(p.lines,this.re_body_is_gthan).map((x,i)=>{
            if(i==0){
              ...
              return `\\textcolor{white}{${this.icon_triangledown}}~~${this.uncode(style,x)}`;
            }
          });
          var text = lines.join('\\\\\n')
          all.push('');
          ...
          let lines = this.body_to_body(p.lines,this.re_body_is_astrk).map((x,i)=>{
            return `${this.icon_cdigits[i]}~~${this.uncode(style,x)}`;
          });
          var text = lines.join('\\\\\n')
          all.push('');
          all.push(text);
        }else if(this.re_body_is_hyphe.test(p.lines[0])){
          let lines = this.body_to_body(p.lines,this.re_body_is_hyphe).map((x,i)=>{
            return `${this.icon_bullet}~~${this.uncode(style,x)}`;
          });
          var text = lines.join('\\\\\n')
          all.push('');
          all.push(text);
          ...
          var fmt = ww.map((x,i,arr)=>`>{\\hsize=${x/totaln*arr.length}\\hsize\\raggedright\\arraybackslash}X`).join('');
          let d = rows.map((pp)=>{
            let ss1 = pp.map(p=>p.text);
            ss1 = ss1.map(x=>this.uncode(style,x));
            let s1 = `\\begin{tabularx}{\\linewidth}{${fmt}}${ss1.join(" & ")}\\end{tabularx}`;
            return(s1);
          });
          var text = d.join('\n')
          all.push('');
  ...
  float_to_body(title,label,style,subtitles,body,bodyrow){
    ...
    if(img){
      d.forEach((img)=>{
        all.push(`\\setlength{\\intextsep}{0pt}\\begin{wrapfigure}{r}{0pt}`);
        all.push(img);
        all.push(`\\end{wrapfigure}`);
      });
      all.push(text);
    }else if(tab){
      d.forEach((tab)=>{
        all.push(`\\setlength{\\intextsep}{0pt}\\begin{wraptable}{r}{0pt}`);
        all.push(tab);
        all.push(`\\end{wraptable}`);
      });
      all.push(text);
    }else{
      all.push(text);
  ...
  float_to_tabbing(title,label,style,subtitles,body,bodyrow){
    ...
    var totaln = ww.reduce((acc,cur)=>acc+cur,0);
    var fmt = ww.map(x=>`\\=\\hspace{${x/totaln}\\linewidth}`).join('') + ' \\kill';
    var all = [];
