...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  itms_to_bull(itms,style,ischoices=0){
    let ss = itms.map((p,i) => {
      ...
      if(p.type == 'DL'){
        if(text){
          return `\\textbf{${this.unmask(p.value)}}\\hfill\\break\n${text}`;
        }else{
          return `\\textbf{${this.unmask(p.value)}}`;
        }
      }
    ...
    if(ischoices){
      ...
      if(ischoices){
        ss = ss.map((s,i,arr) => {
          return `\\hangindent=3em ${box}~~${s}`
        })
      }
      var o = [];
      o.push('');
      o.push(`\\begin{tabular}{@{}p{\\linewidth}@{}}`);
      o.push(ss.join('\\tabularnewline\n'));
      o.push(`\\end{tabular}`);
      return o.join('\n')
    }else{
      var o = [];
      ...
      o.push(`\\begin{list}{}{\\setlength\\itemsep{0pt}\\setlength\\parsep{0pt}\\setlength\\leftmargin{${leftmargin}}}`);
      o.push(text);
      o.push('\\end{list}')
      return o.join('\n')
    }
