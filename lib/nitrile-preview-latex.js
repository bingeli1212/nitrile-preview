...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  fence_to_tab(style,ss,ssi) {
    ...
    if(rows.title){
      all.push(`\\multicolumn{${ww.length}}{c}{${this.polish(style,rows.title)}} \\\\`)
    }
  ...
  float_to_details(title,label,style,subtitles,body,bodyrow){
    ...
    step.forEach((p,i,arr) => {
      if(i==0){
        ...
        if(p.value=='+'){
          if(p.lines.length){
            all.push(`\\item[${this.uncode(style,p.term)}] \\hfill\\break ${this.uncode(style,this.join_para(p.lines).trim())}`)
          }else{
            all.push(`\\item[${this.uncode(style,p.term)}]~`)
          }
        }else if(p.value==':'){
          if(p.lines.length){
