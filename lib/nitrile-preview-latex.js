...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  constructor(parser) {
    ...
    //this.squareboxo = this.uncode('&SquareBoxO;',style);
    //this.squarebox = this.uncode('&SquareBox;',style);
    //this.circleboxo = this.uncode('&CircleBoxO;',style);
    //this.circlebox = this.uncode('&CircleBox;',style)
    this.circlebox  = '\\ding{108}'                       
    this.circleboxo = '\\ding{109}'                       
    this.squarebox  = '\\ding{110}'                       
    this.squareboxo = '\\ding{111}'                       
  }
  do_HDGS(block){
  ...
  fence_to_tabularx(ss,style) {
    ...
    if(1){
      ...
      if(title){
        let title_text = this.uncode(title,style);
  ...
  toFramedFigure (text) {
    ...
    for (var pp of text) {
      pp = pp.map( x => {
          ...
          var { height } = opts;
          if (height) {
            return `\\begin{subfigure}[t]{${width}\\linewidth}\\includegraphics[keepaspectratio=true,height=${height},width=\\linewidth]{${src}}\\caption{${this.uncode(sub,style)}}\\end{subfigure}`;
          } else {
            return `\\begin{subfigure}[t]{${width}\\linewidth}\\includegraphics[keepaspectratio=true,width=\\linewidth]{${src}}\\caption{${this.uncode(sub,style)}}\\end{subfigure}`;
          }
      });
  ...
  cols_to_tabulary(cols){
    ...
    for (var j = 0; j < nrows; ++j) {
      ...
      if (j == 0) {
        ...
        var kk = kk.map(k => k.map(x => this.uncode(x,style)));
        ...
        var pp = pp.map(x => this.uncode(x,style));
        s.push(`\\noalign{\\vspace{${vpadding}pt}}`);
        s.push(`${pp.join(' & ')}\\\\`);
      }
    }
  ...
  rows_to_xltabular(rows,islabeled,label,caption){
    ...
    if(islabeled){
      if(label){
        d.push(`\\caption{${this.uncode(caption,style)}\\label{${label}}}\\\\`);
      }else{
        d.push(`\\caption{${this.uncode(caption,style)}}\\\\`);
      }
    }
    for (var j = 0; j < nrows; ++j) {
      var pp = rows[j];
  ...
  itms_to_bull(itms,style){
    ...
    let squarebox  = this.squarebox;
    let squareboxo = this.squareboxo;
    let circlebox  = this.circlebox;
    let circleboxo = this.circleboxo;
    const nbsp = '~~';
    if(style.bullet){
      try{
  ...
  itms_to_figure_tabular(itms,style) {
    var d = [];
...
`;
  ...
  to_report_part(top) {
    ...
    all.push('');
    if(my){
      all.push(`\\part{${this.uncode(my.title,style)}}`);
    }
    top.forEach((o,i,arr) => {
      if (Array.isArray(o)) {
  ...
  to_report_chapter(top) {
    ...
    let all = [];
    all.push('');
    all.push(`\\chapter{${this.uncode(my.title,style)}}${this.to_latexlabelcmd(my.style.label)}`);
    top.forEach((o,i,arr) => {
      if (Array.isArray(o)) {
  ...
  to_report_section(top) {
    ...
    let all = [];
    all.push('');
    all.push(`\\section{${this.uncode(my.title,style)}}${this.to_latexlabelcmd(my.style.label)}`);
    top.forEach((o, i) => {
      if (Array.isArray(o)) {
  ...
  to_report_subsection(top) {
    ...
    let all = [];
    all.push('');
    all.push(`\\subsection{${this.uncode(my.title,style)}}${this.to_latexlabelcmd(my.style.label)}`);
    top.forEach((o,i) => {
      if(Array.isArray(o)){
  ...
  to_report_subsubsection(top) {
    ...
    let all = [];
    all.push('');
    all.push(`\\subsubsection{${this.uncode(my.title,style)}}${this.to_latexlabelcmd(my.style.label)}`);
    top.forEach((o,i) => {
      if(Array.isArray(o)){
      }else{
        all.push(o.latex);
