...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  do_SAMP(block){
    ...
    if(id=='equation'){
      ...
      var text = this.samp_to_framed(body,style);
    }else {
  ...
  fence_to_tabular(ss,g) {
    ...
    let pcols = this.halign_to_latex_pcols(n,g.halign);
    let pcol = this.pcols_to_latex_pcol(pcols,g.vrule);
    let o = [];
  ...
  fence_to_longtabu(ss,g){
    var g = this.update_style_from_switches(g,'longtabu');
    var rows = this.ss_to_tabular_rows(ss,g);
    var rows = this.update_rows_by_hrule(rows,g.hrule);
    rows.forEach((pp) => {
      pp.forEach((p) => {
  ...
  float_to_blockquote(title,ss,g){
    ...
    clusters = clusters.map((ss) => {
      ...
      //s = `{\\ttfamily{}${s}}`
      return `\\begin{quote}${s}\\end{quote}`
    })
  ...
  float_to_verbatim(title,ss,g){
    ...
    if(0){
      ...
      var leftmargin = this.assert_float(g.leftmargin);
      var parsep = 0;
      var text = this.to_margined_text(pp,parsep,leftmargin,g);
      return text;
    }
  }
  ...
  float_to_longtable(title,ss,g){
    ...
    var pcols = this.fr_to_latex_pcols(n,g.fr);
    var pcol = this.pcols_to_latex_pcol(pcols,g.vrule);
    var title = g.title||'';
    var label = g.label||''
    var o = [];
