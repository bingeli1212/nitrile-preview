...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  fence_to_tab(style,ss,ssi) {
    ...
    var [pcol,totalw] = this.ww_to_latex_pcol(ww,stretch,hew,style.rules,style.side,vrules);
    ...
    if(stretch){
      if(style.frame=='hsides'){
        all.push(`\\begin{tabularx}{\\textwidth}{${pcol}}`);
        all.push('\\hline')
      }else if(style.frame){
        all.push(`\\begin{tabularx}{\\textwidth}{|${pcol}|}`);
        all.push('\\hline')
      ...
      if(style.frame=='hsides'){
        all.push(`\\begin{tabularx}{${totalw}mm}{${pcol}}`);
        all.push('\\hline')
      }else if(style.frame){
        all.push(`\\begin{tabularx}{${totalw}mm}{|${pcol}|}`);
        all.push('\\hline')
  ...
  float_to_tabulate(title,label,style,splitid,body,bodyrow){
    ...
    if(bundles.length) {
      ...
      ss =  ss.map((s,j,arr) => {
        ...
        if(j==0){
          if(isdhline){
            xx = xx.map(x => x.length);
            xx = xx.map(x => `\\hspace{2mm}\\=\\hspace{${x}mm}\\hspace{2mm}`)
            fmt = xx.join('') + ' \\kill';
            return '';
          }else{
            let xx1 = xx.map(x => `\\hspace{2mm}\\=${x}\\hspace{2mm}`)
            fmt = xx1.join('') + ' \\kill';
          }
        }
  ...
  ___float_to_tabulate(title,label,style,splitid,body,bodyrow){
    var bundles = this.body_to_all_bundles(style,body,bodyrow);
    var all = [];
    var maxn = 1;
    var fmt = [];
    if(bundles.length) {
      ...
      all.push('');
      if(fmt){
        all.push(`\\begin{ctabular}[l]{${fmt}}`);
      }else{
        all.push(`\\begin{ctabular}[l]{${'l'.repeat(maxn)}}`);
      }
      ss.forEach(s => {
        if(s){
          all.push(s);
  ...
  ww_to_latex_pcol(ww,stretch,hew,rules,side,vrules) {
    /// \begin{tabular}{lcr}{
    /// | >{\hsize=0.5\hsize\raggedright\arraybackslash}X
    ///  | >{\hsize=0.5\hsize\centering\arraybackslash}X
    ///  | >{\hsize=2.0\hsize\raggedleft\arraybackslash}X | }
    ...
    ww = ww1;
    var totalw = ww.map(w => w.pw).reduce((acc,cur)=>acc+cur,0);
    ww.forEach((w,i,arr) => {
      let pw = w.pw;
    ...
    totalw += (hew-1)*1;//add 1mm for each additional hew, IMPORTANT: only do this after the previous step so that the extra 1mm are not counted towards each column
    var pcol = '';
    var ss = [];
