...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  fence_to_fml(style,ss,ssi) {
    ...
    return({img,subtitle,text,style});
  }
    ...
    if(!style.partition){
      ...
      itms.forEach((p) => {
        if(p.type=='bundle'){
          ...
          if(subtitle){
            o.push(`\\begin{threeparttable} ${p.img} \\begin{tablenotes}[flushleft] \\item ${caption_align} ${caption_fontsize} ${subtitle} \\end{tablenotes} \\end{threeparttable}`);
          }else{
      ...
      itms.forEach((p) => {
        if(p.type=='bundle'){
          let subtitle = p.subtitle;
          if(subtitle){
            onerow.push(`\\begin{threeparttable} ${p.img} \\begin{tablenotes}[flushleft] \\item ${caption_align} ${caption_fontsize} ${subtitle} \\end{tablenotes} \\end{threeparttable}`);
          }else{
  ...
  float_to_equation(title,label,style,bundles){
    ...
    bundles.forEach((bundle,i,arr) => {
      ...
      all.push(`\\hfill${itm.img}\\hfill{(${eqnnum}${sub})}`);
      all.push(`\\\\`);
    });
    if(all[all.length-1]=='\\\\'){
  ...
  phrase_to_dia(style,cnt){
    ...
    if(style['$'].buffers.hasOwnProperty(cnt)){
      ...
      img = p.img;
      if(g.frame){
        img = `\\frame{${img}}`;///this step is necessary to remove any baseline that is embedded within each tikzpicture because otherwise this picture will try to baseline-align with neighboring text such that it rises above it
      }else{
