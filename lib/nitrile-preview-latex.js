...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  phrase_to_img(s) {
    ...
    if(typeof src == 'string' && src.charAt(0)=='#'){
      ...
      ///external image file
      var text = this.img_to_imagefile(src,g);
      if(text && g.float) {
  ...
  para_to_quote(ss,style) {
    ...
    let ldquo = this.unmask('&ldquo;')
    let rdquo = this.unmask('&rdquo;');
    let o = [];
    o.push('');
    o.push('\\begin{quote}');
    o.push(`${ldquo}${text}${rdquo}`);
    o.push('\\end{quote}')
    text = o.join('\n')
    return text;
  }
  ...
  itms_to_figure(itms,style) {
    ...
    itms.forEach((p,i,arr) => {
      p.g.width = `100%`;
      if(p.src && p.src.charAt(0) == '#'){
        ///this is a embedded diagram
        let id = p.src.substr(1);
        let g = p.g;
        ...
        let text = this.img_to_imagefile(src,g,1); 
        p.img = text;
        p.sub = this.unmask(p.sub);
      }
    });
...
`;
  ...
  img_to_imagefile(src,g,isfigure=0){
    ...
    if(isfigure){
      var text = `\\parbox[][][c]{\\linewidth}{\\centering${text}}`
    }else{
      var text = text;
    }
    return text;    
