...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  para_to_table(ss,style){
    var {caption,ss} = this.ss_to_caption_ss(ss,style);
    var rows = super.ss_to_table_rows(ss,style);
    rows = rows.map((ss) => ss.map(s => {
      s = this.unmask(s);
      return s;
    }));
    ...
    if(style.long){
      var frs = this.string_to_frs_with_gap(style.fr||'',n,0.05);
      var pcols = frs.map(x => `L{${this.fix(x)}\\linewidth}`);
      ...
      if(rows.length){
        ...
        if(rows.length && this.pp_is_hline(rows[0])){
          rows = rows.slice(1);
          d.push('\\hline \\endhead')
        }else if(rows.length && this.pp_is_dhline(rows[0])){
          rows = rows.slice(1);
          d.push('\\hline\\hline \\endhead')
        }else if(hrows.indexOf('t')>=0){
          d.push('\\hline \\endhead');
        }
        ///footer
        if (hrows.indexOf('b')>=0){
      ...
      if(1){
        ...
        if(style.fr){
          let frs = this.string_to_frs(style.fr,n);
          let pcols = this.to_tabularx_pcols(frs);
          let pcol = this.pcols_to_pcol(pcols,style);
          let o = [];
          let hrows = this.string_to_array(style.hline||'');
  ...
  pcols_to_pcol(pcols,style){
    ...
    if(style.vline){
      ...
      let vlines = this.string_to_array(vline);
      let ncols = pcols.length;
      var vcols = '|'.repeat(ncols+1).split('');
      vcols = vcols.map((x,i) => {
        if(vlines.indexOf('*')>=0 || vlines.indexOf(`${i}`)>=0){
          return x;
        }else{
          return '';
        }
  ...
  rows_to_tabular(rows,style){
    ...
    for (var j = 0; j < nrows; ++j) {
      ...
      if(j > 0 && hrows.indexOf('*')>=0 && has_hline==0){
        d.push('\\hline');
      }
      has_hline = 0;
      d.push(`${pp.join(' & ')}\\tabularnewline`);
    }
    return d.join('\n');
  }
  cols_to_multi(cols){
  ...
  itms_to_lines(itms,style){
    ...
    var z = itms.length - n * m;
    var k = z ? (m + 1) : (m);
    var pcol = this.to_tabularx_pcols(this.string_to_frs(style.fr,n)).join('');
    var cols = [];
