...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  constructor(parser) {
    ...
    this.unicode_symbol_map = this.tokenizer.build_unicode_symbol_map_for_tex();
  }
  to_conf_step(){
  ...
  do_SAMP(block){
    ...
    o.push(`\\begin{flushleft}`)
    o.push(`\\begin{adjustwidth}{${this.to_conf_step()}}{}`);
    o.push(text);
  ...
  fence_to_math(ss, style) {
    let line = this.make_line(ss.join('\n'),style);
  ...
  phrase_to_img(g) {
    if(g.src){
      let src = g.src;
      this.imgs.push(src);
      let width = this.string_to_latex_length(g.width);
      let height = this.string_to_latex_length(g.height);
      let my_opt = [];
      if(width){
  ...
  fence_to_tabulate(ss,style) {
    ...
    if(style.floatname=='Tabulate'){
      let o = [];
      o.push(`\\begin{table}[ht]`);
      o.push(`\\centering`);
      var caption_text = (this.unmask(style.caption));
  ...
  pols_to_pcol(pcols,style){
    if(style.border==1){
      ///vertical border on every column
      return `|${pcols.join('|')}|`;
    }
    else if(style.border==2){
      return `|${pcols.join('')}|`
    }
    else return pcols.join('');
  }
  ...
  smooth (unsafe) {
    ...
    for(let c of unsafe){
      if(this.unicode_symbol_map.has(c)){
        c = this.unicode_symbol_map.get(c);
      }
      new_unsafe += c;
    }
    unsafe = new_unsafe;
...
`
  ...
  to_core_packages () {
    let latex_program = this.to_latex_program();
    let unicode_math='';
    let cjk_packages='';
    if(latex_program=='lualatex'||latex_program=='xelatex'){
      unicode_math='\\usepackage{unicode-math}'
    }
    if(latex_program=='pdflatex'&&this.flags.is_cjk){
      cjk_packages=`\
\\usepackage{pinyin}
\\usepackage[overlap,CJK]{ruby}
\\renewcommand\\rubysep{0.05ex}`;
    }
    return `\
\\usepackage{graphicx}
\\usepackage{caption}
...
`
  ...
  itms_to_verse(itms,style){
    ...
    return `\\begin{tabu} to \\linewidth{@{}X[1,l]@{}}\n${pp.join('\\\\\n')}\n\\end{tabu}`;
  }
  ...
  to_cjk(s,fn){
    var locale = this.to_latex_program();
    if(locale === 'pdflatex'){
      this.flags.is_cjk = 1;
      switch(fn) {
        case 'cn': {
        ...
        case 'jp': {
          return `\\begin{CJK*}{UTF8}{goth}${s}\\end{CJK*}`
          break;
        }
        case 'kr': {
