  ...
  float_to_details(title,label,style,splitid,body,bodyrow){
    ...
    step.forEach((p,i,arr) => {
      if(i==0){
        ...
        if(p.value=='+'){
          if(p.lines.length){
            all.push(`\\item[${this.uncode(style,p.term)}]\\hfill\\break\n${this.uncode(style,this.join_para(p.lines))}`)
          }else{
            all.push(`\\item[${this.uncode(style,p.term)}]~`)
          }
        }else if(p.value==':'){
          if(p.lines.length){
            all.push(`\\item[${this.smooth(style,p.term)}]\\hfill\\break\n${this.uncode(style,this.join_para(p.lines))}`)
          }else{
            all.push(`\\item[${this.smooth(style,p.term)}]~`)
          }
        }else{
  ...
  halign_to_latex_pcols(n,halign) {
    ...
    while(halign && halign.length){
      if((v=re_lcr.exec(halign))!==null){
        ...
        pp.push(`>{\\raggedright\\arraybackslash}p\{${w}mm\}`);
      }
      else {
        break;
      }
