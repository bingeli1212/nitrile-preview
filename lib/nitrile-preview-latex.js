...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  phrase_to_frac(s,style) {
    var ss = s.split('/');
    var s0 = ss[0]||'';
    var s1 = ss[1]||'';
    var s0 = this.uncode(s0,style);
    var s1 = this.uncode(s1,style);
    return `\\begin{tabular}{@{}c@{}}{${s0}}\\\\\\hline{${s1}}\\end{tabular}`;
  }
  //
  // fence_to_XXX
  // 
  ...
  fence_to_blockquote(ss,style){
    ...
    var ss = [];
    ss.push(s);
    var text = to_margined_text(ss,style);
  ...
  fence_to_verbatim(ss,style){
    ...
    var leftmargin = this.assert_float(style.leftmargin,0);
    ...
    var text = this.to_margined_text(ss,style);
    var text = this.to_fontsized_text(ss,style);
    return text;
  }
  ...
  rows_to_tabular(rows,style,hsizes){
    ...
    for (let j = 0; j < rows.length; ++j) {
      ...
      if (this.row_is_hline(pp)) {
        ...
        flag_hline = 1;
        continue;
      }else if(this.string_has_item(style.hrule,'-') && j > 0 && flag_hline==0){
        d.push('\\hline')
      }
      ///add colorcell
      var mypp = pp.map(x => {
...
`;
  ...
  ///NOTE
  ///
  to_margined_text(ss,style){
    var leftmargin = this.assert_float(style.leftmargin);
    let d = [];
    d.push(`\\begin{list}{}{\\setlength\\itemsep{0pt}\\setlength\\parsep{0pt}\\setlength\\leftmargin{${leftmargin}mm}}`);
    ss.forEach((s) => {
      d.push(`\\item ${s}`)
    })
    d.push(`\\end{list}`);
  ...
  ///NOTE
  to_fontsized_text(text,style){
    var fs = this.string_to_latex_fontsize(style.fontsize)
    if(fs){
      let d = [];
      d.push('\\bgroup');
      d.push(`${fs}`);
