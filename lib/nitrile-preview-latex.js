...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  phrase_to_inlinemath(str,style) {
    var s = this.tokenizer.to_lmath(str,style);
    return `\\(${s}\\)`;
  }
  phrase_to_displaymath(str,style) {
    var s = this.tokenizer.to_lmath(str,style,1);
    return `\\[${s}\\]`;
  }
  phrase_to_math(mode,style) {
    if(mode.body){
      let str = mode.body.join('\n')
  ...
  phrase_to_framed(mode,style) {
    var {name,fname,id,body} = mode;
    var style = this.string_to_style(name,style);
    if(id){
      var {body} = this.fetch_from_notes(id);
    }
    if(body){
      var text = this.body_to_framed(body,style);
      if(style.float){
  ...
  phrase_to_diagram(mode,style) {
    ...
    var text = '';
    if(body){
      text = this.body_to_diagram(body,style);
      if(style.float){
        let f = (style.float == 'left') ? 'l' : 'r';
        text = `\\begin{wraptable}{${f}}{0pt}${text}\\end{wraptable}~`;
      }else if(style.para_id=='figure'){
  ...
  phrase_to_tabular(mode,style) {
    var {ss,style} = this.fetch_ss_from_mode_with_style(mode,style);
