...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  fence_to_par(style,ss,ssi){
    var textsmaller = '';
    if(style.textsmaller){
      textsmaller = `\\footnotesize`
    }
    ss = this.ss_to_backslashed_ss(ss);
    ss = ss.map(s => this.uncode(style,s));
    var a = `p{\\linewidth}`;
    if(style.textalign=='c'){
      var a = `>{${textsmaller}\\centering}${a}`;
    }else if(style.textalign=='r'){
      var a = `>{${textsmaller}\\raggedleft}${a}`;
    }else{
      var a = `>{${textsmaller}\\raggedright}${a}`;
    }
  ...
  fence_to_vtm(style,ss,ssi){
    ...
    var text = ss.join('\n');
    var all = [];
    all.push(`\\begin{tabular}{@{}>{${textsmaller}\\raggedright}p{\\linewidth}@{}}`);
  ...
  float_to_preformatted(title,label,style,splitid,body,bodyrow){
    ...
    let textsmaller = '';
    if(style.textsmaller){
      textsmaller = 'fontsize=\\small'
    }
    var all = [];
    all.push('');
    all.push(`\\begin{Verbatim}[${textsmaller}]`);
    ...
    step.forEach((p,i,arr) => {
      if(i==0){
        ...
        if(p.value=='+'){
          ...
          let a = style.__memo['*']||0;
          a++;
          all.push(`\\item[${a}.] ${this.uncode(style,this.join_para(p.lines))}`);
          style.__memo['*'] = a;
        }else{
  ...
  float_to_example(title,label,style,splitid,body,bodyrow){
    let ss=body.map((s) => this.uncode(style,s)).map(s=>`${this.icon_dag}\\ ${s}`)
    var text=ss.join('\\\\\n');
    var all = [];
    all.push('');
  ...
  float_to_description(title,label,style,splitid,body,bodyrow){
    let ss = body.map((s)=>s.split('\n')).map(([dt,dd]) => `\\item[${this.polish(style,dt)}] ${this.uncode(style,dd)}`);
    var text=ss.join('\n');
    var all = [];
    all.push('');
  ...
  float_to_enumeration(title,label,style,splitid,body,bodyrow){
    let ss = body.map((s)=>`\\item[${this.get_next_enum(style)}.] ${this.uncode(style,s)}`);
    var text=ss.join('\n');
    var all = [];
    all.push('');
    all.push(`\\begin{packed_enumerate}`);
    all.push(text);
    all.push(`\\end{packed_enumerate}`)
    return all.join('\n');
  }
  float_to_tabbing(title,label,style,splitid,body,bodyrow){
    ...
    var fmt = ww.map(w => `\\=\\hspace{${w/total}\\linewidth}`).join('') + ' \\kill';
    var all = [];
    all.push(fmt);
    rows.forEach((ss,j) => {
      ss = ss.map((s) => {
