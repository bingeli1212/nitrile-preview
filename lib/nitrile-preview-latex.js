...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  do_SAMP(block){
    ...
    if(style.samp=='framed'){
      var text = this.para_to_framed(ss,style);
    }else {
      var text = this.para_to_verbatim(ss,style);
    }
    block.latex = text;
  }
  do_HRLE(block){
