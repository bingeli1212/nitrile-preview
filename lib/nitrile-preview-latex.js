...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  fence_to_img(style,ss) {
    ...
    if(stretch){
      opts.push(`width=${stretch}\\linewidth`);
      opts.push(`height=${stretch*(viewport_height/viewport_width)}\\linewidth`);
  ...
  fence_to_verbatim(style,ss){
    ...
    d.forEach(x => all.push(x));
    all.push('\\end{tabular}');
    var text = all.join('\n')
    if(style.frame){
      text = `\\fbox{${text}}`;
    }
    //text = `\\hfill\\break${text}\\hfill\\break{}`;
    var o = [];
    o.push({img:text,sub:''});
    return o;
  }
  ...
  fence_to_tabular(style,ss) {
    if(style.float=='longtable'){
      ...
      //
      var n = (rows.length) ?  rows[0].length : 1;
      var latex = this.g_to_shrink_float(style);
