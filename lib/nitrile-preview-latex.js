...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  float_to_equation(title,label,style,ss,ssi){
    ...
    itms.forEach((p,j,arr)  => {
      ...
      let str = ss.join('\n')
      let used = new Set();
      let text = this.tokenizer.to_lmath(str,style,used,1);
      p.text = text;
    });
  ...
  literal_to_verb(style,cnt){
    ...
    s = s.replace(/-/g, "-{}")
    return `{\\textquotedblleft\\texttt{${s}}\\textquotedblright}`;
  }
  //////////////////////////////////////////////////////////////////////////////////////
  //
  // translator function
  ...
  phrase_to_ref(style,cnt){
    ...
    if(label){
      if(this.idnum_map.hasOwnProperty(label)){
        ...
        return `\\sout{${this.smooth(label)}}`;///requires the \usepackage[normalem]{ulem}
      }
    }else{
      return "??";
    }
  ...
  phrase_to_hrule(style,cnt){
    ...
    if(wd){
      return `{\\rule{${wd}}{0.4pt}}`;
    }else{
      return `{\\rule{10mm}{0.4pt}}`;
    }
  }
  //////////////////////////////////////////////////////////////////////////////////////
  //
  ...
  cove_to_text(style,cove){
    ...
    all.push(`\\item[{$\\vartriangleright$}] ${text}`);
    all.push(`\\end{list}`)
    return all.join('\n');
  }
  ////////////////////////////////////////////////////////////////////////////////
  ...
  samp_to_text(style,samp){
    ...
    if(this.latex_samp_small){
      all.push(`\\begin{Verbatim}[xleftmargin=8mm,fontsize=\\small]`);
    }else{
  ...
  sand_to_text(style,sand){
    ...
    var all = [];
    all.push('');
    all.push(`\\begin{list}{}{\\setlength\\itemsep{0pt}\\setlength\\parsep{0pt}\\setlength\\leftmargin{8mm}}`);
    all.push(`\\item[] ${text}`);
    all.push(`\\end{list}`)
    return all.join('\n')
  }
