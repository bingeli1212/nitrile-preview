...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  float_to_table(title,label,style,bundles){
    ...
    var all_splitids = this.string_to_int_array(style.splitids);
    var all_splitstarts = this.string_to_int_array(style.splitstarts);
    var all = [];
    bundles.forEach((bundle,j,arr) => {
      let itm = this.do_bundle(style,bundle,'tab');
      let text = itm.tab;
      let splitid = all_splitids[j]||0;
      let splitstart = all_splitstarts[j]||0;
  ...
  to_multicols_img(img,tab,subtitle){
    if(subtitle){
