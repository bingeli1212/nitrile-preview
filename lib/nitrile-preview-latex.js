...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  fence_to_img(style,ss) {
    ...
    var text = this.ss_to_latex_includegraphics(ss,style);
    if (style.isfigure) {
      text = text;
    }else if (style.wrap) {
      if(style.wrap == 'left'){
        text = `\\begin{wapfigure}{l}{0pt}\n${text}\n\\end{wrapfigure}~`;
      }else{
        text = `\\begin{wapfigure}{r}{0pt}\n${text}\n\\end{wrapfigure}~`;
      }
    }else{
  ...
  fence_to_verbatim(style,ss){
    ...
    var fontsize = this.assert_float(style.fontsize,);
    ...
    var text = this.to_fontsized_text(text,style);
    return text;
  }
  fence_to_tabular(style,ss) {
    ...
    o.push(this.rows_to_tabular(rows, style));
    o.push('\\end{tabular}')
    var text = o.join('\n')
    text = this.to_fontsized_text(text,style);
    return text;
  }
  ...
  polish_verb(line,style){
    line = this.polish(line,style);
  ...
  phrase_to_colorbutton(style,cnt,cnt2,cnt3){
    ...
    var env = {};
    var { text } = this.diagram.to_diagram(style,ss,env);
    return text;
  }
  ...
  to_xelatex_document(documentclass,documentclassopt,postsetup,titlelines,body) {
    ...
    var newfontfamily = this.conf_to_list('xelatex.post');
  ...
  to_lualatex_document(documentclass,documentclassopt,postsetup,titlelines,body) {
    ...
    ///     Here are some multilingual Unicode fonts: this is Arabic text: {\A السلام عليكم}, 
    ///     this is Hebrew: {\H שלום}, and here's some Japanese: {\J 今日は}.
    var newfontfamily = this.conf_to_list('lualatex.post');
    newfontfamily.forEach((s) => {
      all.push(s);
    })
    postsetup.forEach((s) => {
