...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  float_to_details(title,label,style,splitid,body,bodyrow){
    ...
    step.forEach((p,i,arr) => {
      if(i==0){
        ...
        if(p.value=='+'){
          if(p.lines.length){
            all.push(`\\item[${this.polish(style,p.term)}] \\hfill\\break ${this.uncode(style,this.join_para(p.lines).trim())}`)
          }else{
  ...
  float_to_description(title,label,style,splitid,body,bodyrow){
    let ss = body.map((s)=>s.split('\n')).map(([dt,dd]) => {
      if(dd.length){
        return `\\item[${this.polish(style,dt)}] \\hfill\\break ${this.uncode(style,dd)}`;
      }else{
        return `\\item[${this.polish(style,dt)}]~`;
      }
    });
    var text=ss.join('\n');
    var all = [];
