...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  itms_to_bull(itms,style){
    let pp = itms.map((p,i) => {
      ...
      if(p.bull == '**'){
        return `\\hangindent=0.7cm ${i+1}.~${text}`
      }
      if(p.bull == '--'){
        return `\\hangindent=0.7cm \\textbullet~${text}`
      }
      if(p.type == '0'){
        return `\\hangindent=0.7cm ${p.value}${p.ending}~${text}`;
      }
      if(p.type == 'a'){
        return `\\hangindent=0.7cm ${this.to_a_letter(p.value)}${p.ending}~${text}`;
      }
      if(p.type == 'A'){
        return `\\hangindent=0.7cm ${this.to_A_letter(p.value)}${p.ending}~${text}`;
      }
      if(p.type == 'DL'){
        return `\\hangindent=0.7cm \\textbf{${p.value}}${p.ending}~${text}`;
      }
