...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
      ...
      ss =  ss.map((s,j,arr) => {
        ...
        if(j==0){
          if(isdhline){
            ...
            xx = xx.map(x => `\\hspace{1ex}\\=\\hspace{${x}ex}\\hspace{1ex}`)
            fmt = xx.join('') + ' \\kill';
            return '';
          }else{
            let xx1 = xx.map(x => `\\hspace{1ex}\\=\\hspace{10ex}\\hspace{1ex}`)
            fmt = xx1.join('') + ' \\kill';
          }
        }
  ...
  ___float_to_tabbing(title,label,style,splitid,body,bodyrow){
    var bundles = this.body_to_all_bundles(style,body,bodyrow);
    var all = [];
    var maxn = 1;
    var fmt = [];
