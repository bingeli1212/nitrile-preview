...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  itms_to_bull(itms,style){
    ...
    let squareboxo = this.uncode('&SquareBoxO;',style);
    let squarebox = this.uncode('&SquareBox;',style);
    let circleboxo = this.uncode('&CircleBoxO;',style);
    let circlebox = this.uncode('&CircleBox;',style)
    const nbsp = '~~';
    if(style.bullet){
      try{
