...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  itms_to_figure(itms,style) {
    ...
    var text = '';
    itms.forEach((p,i,arr) => {
      var {type,fname,id,body,g,sub} = p;
      g.width = `100%`;
      if(type=='diagram' && id){
        ...
        var {body} = this.fetch_from_notes(id);
        body = body||[];
        text = this.fence_to_diagram(body,g);
        p.img = text;
        p.sub = this.unmask(sub);
      }else if(type=='diagram' && body){
        ///this is a embedded diagram
        g.figure = 1;
        text = this.fence_to_diagram(body,g);
        p.img = text;
        ...
        ///this is a embedded diagram
        g.figure = 1;
        text = this.phrase_to_framed('',id,g);
        p.img = text;
        p.sub = this.unmask(sub);
        ...
        g.figure = 1;
        text = this.phrase_to_img(fname,'',g); 
