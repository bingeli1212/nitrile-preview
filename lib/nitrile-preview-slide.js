  ...
  to_html_frame(top,the_slides){
    ...
    if(d.length==0){
      ...
      w.forEach((o,i) => {
        ...
        w.forEach((p,j) => {
          if(i==j){
            all.push(`<li class='PLST'><b>${p.title}</b> ${p.text.length?'-':''} ${p.text}`);
            o.contents.forEach(x => all.push(`<div class='TEXT'>${x.html}</div>`));
            all.push('</li>')
          }else{
      ...
      w.forEach((o,i) => {
        ...
        all.push(d.join('\n'));///main-slide contents text
        all.push(`<div class='TEXT'>`);
        all.push(`${cbox} <b>${o.title}</b> ${o.text.length?'-':''} ${o.text}`);
        all.push(`</div>`);
