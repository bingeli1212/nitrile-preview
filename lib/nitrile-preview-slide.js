...
`;
  ...
  write_one_frame(id,order,icon,frame,issub,topframe){
    ...
    //NOTE: main contents
    //
    all.push(`<article class='SLIDE' id='${frameid}' row='${frame.style.row}' style='--boardpng:url("${boardpng}");background-image:var(--boardpng);background-repeat:no-repeat;background-position:bottom right;' >`);
    if(issub){
