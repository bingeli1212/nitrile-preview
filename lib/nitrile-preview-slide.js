...
class NitrilePreviewSlide extends NitrilePreviewHtml {
  ...
  to_slide_document() {
    ...
    var fontsize = this.conf('slide.font-size','10pt');
    var lineheight = this.conf('slide.line-height','1.5');
    var topmargin = 0;
    var paddingleftplst = 24;
    if(1){
      var the_slides = [];
...
<style>
  ...
  .slide div.TEXT.BLOCKQUOTE {
    font-style: italic;
  }
  .slide div.TEXT.BLOCKQUOTE::before {
    content: "\\0022";
  }
  .slide div.TEXT.BLOCKQUOTE::after {
    content: "\\0022";
  }
  ...
  .slide ul.PLST {
    margin: 0;
    padding-left: ${paddingleftplst}px;
    position: relative;
    list-style: none;
  }
  ...
  to_html_frame(top,the_slides){
    ...
    if(sigs[0]=='HDGS'){
      ...
      if(d.length > 0){
        ...
        all.push(`<ol class='PLST'>`);
        w.forEach((o, i) => {
          all.push(`<li class='PLST TOPIC' type='a' value='${i+1}'> <i>${o.title}</i> </li>`);
        });
        all.push(`</ol>`);
        all.push('</section>');
        all.push('');
      ...
      w.forEach((o,i) => {
        ...
        all.push(d.join('\n'))
        all.push(`<ol class='PLST'>`);
        all.push(`<li class='PLST TOPIC' type='a' value='${i+1}'> <i>${o.title}</i>`);
      ...
      w.forEach((o,i) => {
        all.push(`<li class='PLST'> <b>${o.title}</b> </li>`);
      });
      all.push(`</ul>`);
      all.push(`</section>`);
      all.push('');
      ...
      w.forEach((o,i) => {
        ...
        all.push(`<ul class='PLST'>`);
        if(o.text){
          all.push(`<li class='PLST SOLUTION'> <b>${o.title}</b>: <i>${o.text}</i> `);
        }else{
          all.push(`<li class='PLST SOLUTION'> <b>${o.title}</b>`);
        }
        o.contents.forEach(x => all.push(`${x.html}`));
        all.push('</li>')
        all.push(`</ul>`);
