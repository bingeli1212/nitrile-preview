  ...
  to_html_frame(top,the_slides){
    ...
    if(d.length==0){
      ...
      w.forEach((o,i) => {
        ...
        all.push(`<div class='TEXT'>`);
        w.forEach((p,j) => {
          all.push(`<div>${(i==j)?cbox:box} ${p.title}</div>`);
        });
        all.push(`</div>`);
        all.push(o.contents.map(x => x.html).join('\n'));
        all.push('</section>');
        all.push('');
