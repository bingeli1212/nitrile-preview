...
`;
  ...
  write_contents_frame(topframes){
    ...
    topframes.forEach((topframe,i,arr) => {
      ...
      all.push(`<li> ${icon} ${topframe.title} </li>`);
      if(i+1==arr.length){
        all.push(`</ul>`);
      }
    });
  ...
  write_frame(frame){
    ...
    frame.solutions.forEach((o,i,arr) => {
      ...
      ///TESTING
      var the_icon = this.icon_writing_hand + ' ' + this.to_circled_number_svg(i+1);
    ...
    frame.solutions.forEach((o,i,arr) => {
      ...
      if(1){
        all.push(`<hgroup>`);
        all.push(`<h1 class='frametitle'>${icon} ${title} </h1>`);
        all.push(`<h2 class='framesubtitle'> ${the_icon} <i>${o.title}</i> &#160; ${this.untext(o.body,o.style).trim()} </h2>`)
        all.push(`</hgroup>`);
      }
      all.push(`<div class='element'>`)
