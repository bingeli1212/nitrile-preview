...
class NitrilePreviewSlide extends NitrilePreviewHtml {
  ...
  to_slide_document() {
    ...
    var margintop = '10px';
    var button_html='';
    var title = this.unmask(this.conf('title',''));
    var data = `\
<!DOCTYPE html>
<html>
<head>
...
<style>
  ...
  .slide ul.PLST.OUTER > li {
    margin: ${topmargin} 0;
  }
  .slide ol.PLST.OUTER > li {
    margin: ${topmargin} 0;
  }
  .slide dl.PLST.OUTER > dt{
    margin: ${topmargin} 0;
  }
  .slide dl.PLST > dd {
  ...
  .slide ul.SOLUTION.EDIT > li {
    position: relative;
    margin: ${topmargin} auto;
  }
  .slide ul.SOLUTION.EDIT > li::before {
    content: "";
    position: absolute;
    display: inline-block;
    right: 100%;
...
`;
    ...
    if(main_frames.length){
      ...
      main_frames.forEach((frame,i,arr) => {
        if(i==0){
          ...
          all.push(`<ul class='MAIN'>`)
        }
        all.push(`<li> ${frame.title} </li>`);
        if(i+1==arr.length){
          all.push('</ul>')
      ...
      main_frames.forEach((frame) => {
        let out = this.write_frame(frame, asterisk);
        outs.push(out);
      });
    }
    if(ex_frames.length){
      ...
      ex_frames.forEach((frame,i,arr) => {
        if(i==0){
          all.push(`<section class='slide'>`);
          all.push(`<h2 class='frametitle'> Exercises </h2>`);
          all.push(`<div class='container'><div class='element'>`)
          all.push(`<ul class='EX'>`)
        }
        all.push(`<li> ${frame.title} </li>`);
        if(i+1==arr.length){
          all.push('</ul>')
      ...
      ex_frames.forEach((frame) => {
        let out = this.write_frame(frame, pencil);
        outs.push(out);
      });
    }
    return outs.join('\n');
  ...
  write_frame(frame,icon){
    ...
    if(frame.solutions.length){
      ...
      frame.solutions.forEach((o,i,arr) => {
        ...
        all.push(`<li> <i>${o.title}</i> : <span style='display:inline-block;width:4em;border-bottom:1px solid'></span> </li>`);
        if(i+1==arr.length){
          all.push(`</ul>`);
        }
      });
      ...
      frame.solutions.forEach((o,i,arr) => {
        all.push(`<section class='slide'>`);
        all.push(`<h2 class='frametitle'>${icon} ${title} </h2>`);
        all.push(`<h3 class='framesubtitle'> <i>${o.title}</i> : <u>${o.text.trim()}</u> </h3>`)
        all.push(`<div class='container'><div class='element'>`)
        all.push(`<ul class='SOLUTION EDIT' >`)
  ...
  to_frontmatter_frame(block){
    ...
    <p style='text-align:center'>${this.unmask(date)}</p>
    </div></div>
    </section>
    `;
    return data;
