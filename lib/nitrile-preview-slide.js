...
class NitrilePreviewSlide extends NitrilePreviewHtml {
  ...
  constructor(parser) {
    ...
    this.n_pack = '1.0pt';
    this.n_half = '2.25pt';
    this.n_marginleft = '6.5mm';
    this.n_marginright = '6.5mm';
  }
  to_document() {
    //this.translate();
    ///translate to html
...
`;
    ...
    top.sections.forEach((sect,i,arr) => {
      ...
      if(sect.sections.length){
        ...
        topframe.subframes.forEach((subframe,j) => {
          ...
          let out = this.write_frame_one(id,order,icon,subframe,1);
          d.push(out);
        });
      }else{
        let id = `${i+1}`;
        let order = i+1;
        let icon = '';
        let out = this.write_frame_one(id,order,icon,topframe,0);
        d.push(out);
      }
    });
    var main = d.join('\n');
  ...
  write_frame_folder(id,frame,subframes){
    ...
    subframes.forEach((subframe,j,arr) => {
      ...
      //let icon = `<span style='position:absolute;right:100%;top:50%;transform:translate(-1.5ex,-50%);'>${j+1}</span>`;
      let icon = `<span style='position:absolute;right:100%;top:50%;transform:translate(-1.5ex,-50%);'>${this.icon_folder}</span>`;
  ...
  write_frame_one(id,order,icon,frame,issub){
    let all = [];
    let v = null;
    const re_choice = /^(.*?):([\w\/]+)$/;
    //
