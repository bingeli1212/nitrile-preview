...
<style>
  ...
  .slide ul.BOX > li.BOX::before {
    content: "\\2610";
    position: absolute;
    display: inline-block;
    right: 100%;
    transform: translate(-7px,-2px) scale(1.3,1.3);
    font-weight: bold;
  }
  ...
  write_frame(frame){
    ...
    if(frame.solutions.length){
      ...
      frame.solutions.forEach((o,i,arr) => {
        if(i==0){
          all.push(`<ul class='BOX'>`);
        }
        all.push(`<li class='BOX' > <i>${o.title}</i> &#160; ____________ </li>`);
        if(i+1==arr.length){
          all.push(`</ul>`);
        }
      });
      ...
      frame.solutions.forEach((o,i,arr) => {
        all.push(`<section class='slide'>`);
        all.push(`<h2 class='frametitle'>${++this.frameid} &#160; ${title} </h2>`);
        all.push(`<ul class='CBOX' >`)
        if(o.text){
          all.push(`<li class='CBOX' > <i>${o.title}</i> &#160; <u>${o.text}</u> </li>`);
        }else{
          all.push(`<li class='CBOX' > <i>${o.title}</i> &#160; ____________ </li>`);
        }
      ...
      frame.topics.forEach((o,i,arr) => {
        if(i==0) { all.push(`<ul class='DIGIT'>`); }
        all.push(`<li class='DIGIT${i+1}' > <i>${o.title}</i> </li>`);
