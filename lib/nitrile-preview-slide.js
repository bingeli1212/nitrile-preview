...
.PARAGRAPH, .ITEMIZE, .EQUATION, .FIGURE, .TABLE, .LONGTABLE, .LISTING, .COLUMNS, .SAMPLE {
  text-indent: initial;
  margin-top: 6pt;   
  margin-bottom: 6pt;   
  margin-left:  8mm;
...
`;
  ...
  write_frame_toc(topframes){
    ...
    topframes.forEach((topframe,j,arr) => {
      ...
      if(n==0){
        ...
        all.push(`<ul class='PARA OL' style='padding-left:1.0em;list-style-type:none;position:relative;' >`);
      }
      //let icon = `<span style='position:absolute;right:100%;top:50%;transform:translate(-1.12ex,-50%);'>${j+1}</span>`;
      let order = j+1;
      all.push(`<li class='PACK LI' style='--bull:"${order}";--pad:-1.0em;' > <b> <a href='#frame${j+1}'> ${this.uncode(topframe.style,topframe.title)} </a> </b> </li>`);
      n++;
    });
    all.push(`</ul>`);
    all.push('</div>')
  ...
  write_frame_folder(id,frame,subframes){
    let slidetitle_css = ('TITLE');
    let all = [];
    //
    //NOTE: main contents
    //
    all.push(`<article class='SLIDE' id='frame${id}' data-row='${frame.style.row}'>`);
    ...
    if(1){
      ...
      subframes.forEach((subframe,j,arr) => {
        if(j==0){
          all.push(`<ul class='PARA UL' style='list-style:none;padding-left:0em;position:relative;'>`);
        }
        //let icon = `<span style='position:absolute;right:100%;top:50%;transform:translate(-1.12ex,-50%);'>${j+1}</span>`;
  ...
  write_one_frame(id,order,frame,issub,topframe){
    let slidetitle_css = ('TITLE');
    let slidesubtitle_css = ('SUBTITLE');
    let icon = this.icon_folder;
    let all = [];
    //
    //NOTE: main contents
    ...
    frame.solutions.forEach((o,i,arr) => {
      if(o.choice){
        ...
        all.push(`<div class='PARAGRAPH'> ${this.icon_solution} <u>${this.uncode(o.style,o.title)}</u> ${text} </div>`);
      }else{
        all.push(`<div class='PARAGRAPH'> ${this.icon_solution} <u>${this.uncode(o.style,o.title)}</u> </div>`);
      }
    });
    //
    //NOTE: board name
    ...
    frame.solutions.forEach((o,i,arr) => {
      ...
      if(o.choice){
        ...
        let row = o.style.row;
        all.push(`<h2 class='${slidesubtitle_css}' data-row='${row}' > ${icon} <u> ${title} </u> &#160; ${text} </h2>`)
      }else{
        let icon = this.icon_solution;
        let title = this.uncode(o.style,o.title).trim();
