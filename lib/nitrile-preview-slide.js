  ...
  write_frame_folder(id,frame,subframes){
    ...
    if(1){
      ...
      subframes.forEach((subframe,j,arr) => {
        ...
        icon = '';
        all.push(`<li style='position:relative;font-decoration:underline;'> <span style='position:absolute;left:-2em' > ${icon} </span> <a href='#frame${id}.${j+1}'> ${this.uncode(subframe.style,subframe.title)} </a> </li>`);
      });
      all.push(`</ul>`);
      all.push('</article>');
      all.push('');
