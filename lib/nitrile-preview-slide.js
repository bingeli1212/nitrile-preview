...
`;
  ...
  write_frame_folder(id,frame,subframes){
    ...
    all.push(`</div>`);
    all.push(`<canvas class='EDITBOARD' id='${boardid}' data-type='board' data-boardimgid='${boardimgid}' data-boardname='${boardname}' data-frameid='${frameid}' width='483' height='361' style='background-image:url("${boardpng}")' ></canvas>`);
    all.push('</article>');
    all.push('');
    //
    //NOTE: end
  ...
  write_one_frame(id,order,icon,frame,issub,topframe){
    ...
    frame.solutions.forEach((o,i,arr) => {
      ...
      all.push(`<canvas class='EDITBOARD' id='${boardid}x${i}' data-type='board' data-boardname='${boardname}' data-frameid='${frameid}' width='483' height='361' ></canvas>`);
      all.push('</article>');
      all.push('');
    })
    //
  ...
  to_setup_script(){
    ...
    function board_start(ctx,canvas){
      ...
      if(all_op=='pencil'){
        ...
        ctx.putImageData(ctx.d.myface, 0,0);
        if(ctx.d.altKey){
          let x0 = ctx.d.myposx0;
          let y0 = ctx.d.myposy0;
          let x2 = ctx.d.myposx0;
          let y2 = ctx.d.myposy0;
          ctx.clearRect(x0,y0, x2-x0,y2-y0);
