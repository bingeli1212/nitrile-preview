...
`;
  ...
  write_frame_folder(id,frame,subframes){
    ...
    if(boardpng){
      all.push(`<img src='${boardpng}' width='${114*this.MM_TO_PX}' height='${80*this.MM_TO_PX}' style='position:absolute;left:9mm;top:10mm;' />`)
    }
  ...
  write_one_frame(id,order,frame,issub,topframe){
    ...
    all.push(`<article class='SLIDE' id='frame${id}' data-row='${frame.style.row}' style='position:relative;' >`);
    if(boardpng){
      all.push(`<img border='0' src='${boardpng}' alt='' width='${114*this.MM_TO_PX}' height='${80*this.MM_TO_PX}' style='position:absolute;left:9mm;top:10mm;' />`)
    }
    if(issub){
