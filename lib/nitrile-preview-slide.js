...
<style>
  ...
  .slide div.TEXT ul,ol {
    padding-left: 20px;
  }

  .slide div.PLST ul,ol {
    padding-left: 20px;
  }
...
`;
  ...
  to_html_frame(top,the_slides){
    ...
    if(d.length==0){
      ...
      all.push(`<h2 class='frametitle'>${my_id} ${mtitle}</h2>`);
      ...
      all.push(`</section>`);
      all.push('');
      the_slides.push({id:my_id,idnum:my_id,title:mtitle});
      ///child-slides
      w.forEach(({title,contents},i) => {
        let sid = `${my_id}-${i}`;
        let stitle = `${mtitle} (${title})`;
        all.push(`<section id='slide${sid}' class='slide'>`);
        all.push(`<h2 class='frametitle'>${my_id} ${stitle}</h2>`);
        all.push(d.join('\n'));
        all.push(`<p class='TEXT'>`);
        all.push(`&#x2611; ${title}`);
        all.push(`</p>`);
        ...
        all.push('</section>');
        all.push('');
        the_slides.push({id:sid,idnum:my_id,title:stitle});
      })
    }else{
      ///multiple solutions
      ///1st slide
      ...
      w.forEach((o,i) => {
        let sid = `${my_id}-${i}`;
        let stitle = `${mtitle} - ${o.title}`;
        all.push(`<section id='slide${sid}' class='slide'>`);
        all.push(`<h2 class='frametitle'>${my_id} ${stitle}</h2>`);
        all.push(d.join('\n'));///main-slide contents text
        ///move the first block of 'contents' to be at the same level as 'title' if that
        ///block is a 'TEXT'
        if(o.contents.length && o.contents[0].sig=='TEXT'){
          all.push(`<div class='TEXT'>`);
          all.push(`&#x2612; ${o.title} - ${this.unmask(this.join_para(o.contents[0].body))}`);
          o.contents = o.contents.slice(1);
          all.push(`<ul style='margin:0'>`);
