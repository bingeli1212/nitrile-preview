...
`;
  ...
  write_one_frame(id,order,icon,frame,issub,topframe){
    ...
    all.push(`<canvas class='EDITBOARD' id='${boardid}' data-type='board' data-imgid='${imgid}' data-boardname='${boardname}' data-frameid='${frameid}' width='541' height='420' tabindex='1' onkeypress='canvas_keypress()' ></canvas>`);
    all.push('</article>');
    all.push('');
    // 
  ...
  to_setup_script(){
    ...
    function board_setup_canvas(canvas){
      ...
      ctx.d.myfig = {x:0,y:0,w:0,h:0,rotate:0,skewX:0,skewY:0,mystroke:1.0,mycolor:'DarkSlateBlue'};
      ...
      if(img){
        ctx.drawImage(img,0,0,541,420);
      }
      var s = ctx.getImageData(0,0, 541,420);
      ctx.d.myface = s;
      ...
      if(ctx.d.myop=='shape'){
        ...
        case 'KeyD':
          if(shiftKey){
            ...
            ctx.d.myundo.push({s,mybasis,myop,myarg,myfig});
            ctx.d.myredo = [];
            ctx.d.myface = ctx.getImageData(0,0, 541,420);
    ...
    function pencil_stroke_between(myposx0,myposy0,myposx,myposy,lw,ctx){
      lw = Math.max(1,lw);
      let dx = myposx-myposx0;
      let dy = myposy-myposy0;
      let Dx = Math.abs(dx);
      ...
      if(ctx.d.myop=='select'){
        ...
        if(ctx.d.mybasis.a){
          ...
          if(ctx.d.mybasis.a=='move'){
            ...
            ctx.d.mybasis.s = ctx.getImageData(0,0, 541,420);
            ///draw hilite
            ctx.d.mybasis.dx = dx;
            ctx.d.mybasis.dy = dy;
            board_draw_hilite(ctx,canvas);
    ...
    function board_end(ctx,canvas){
      if(ctx.d.myop=='object'){
        ...
        let s = ctx.getImageData(0,0, 541,420);
        ///normalize ctx.d.mybasis
        ctx.d.mybasis.x += ctx.d.mybasis.dx;
        ctx.d.mybasis.y += ctx.d.mybasis.dy;
        ctx.d.mybasis.dx = 0;
