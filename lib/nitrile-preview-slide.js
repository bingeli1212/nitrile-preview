...
`;
  ...
  write_frame_toc(topframes){
    ...
    topframes.forEach((topframe,i,arr) => {
      ...
      let icon = `<span style='position:absolute;right:100%;top:50%;transform:translate(-1.5ex,-50%);'>${i+1}.</span>`;
      all.push(`<li class='PACK' style='position:relative'> ${icon} <b> ${this.uncode(topframe.style,topframe.title)} </b> </li>`);
      if(i+1==arr.length){
        all.push(`</ul>`);
      }
    });
  ...
  write_frame_folder(id,frame,subframes){
    ...
    //
    all.push(`<article>`);
    all.push(`<h1 class='frametitle'> <b>${id}. ${this.uncode(frame.style,frame.title)}</b> </h1>`);
    ...
    subframes.forEach((subframe,j,arr) => {
      ...
      all.push(`<li class='PACK' style='position:relative'> ${icon} <b> ${this.uncode(subframe.style,subframe.title)} </b> </li>`);
      if(j+1==arr.length){
        all.push(`</ul>`);
      }
    });
