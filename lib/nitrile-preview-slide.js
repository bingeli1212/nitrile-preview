...
`;
  ...
  write_contents_frame(topframes){
    ...
    topframes.forEach((topframe,i,arr) => {
      ...
      all.push(`<li class='PACK' style='position:relative'> ${this.uncode(topframe.style,topframe.title)} ${icon} </li>`);
      if(i+1==arr.length){
        all.push(`</ul>`);
      }
    });
  ...
  write_topics_frame(frame,subframes){
    ...
    subframes.forEach((subframe,i,arr) => {
      ...
      icon = `<span style='position:absolute;right:100%;top:50%;transform:translate(-1ex,-50%);'>${icon}</span>`;
      all.push(`<li class='PACK' style='position:relative'> ${this.uncode(subframe.style,subframe.title)} ${icon} </li>`);
      if(i+1==arr.length){
        all.push(`</ul>`);
      }
    });
  ...
  write_frame(frame){
    ...
    frame.solutions.forEach((o,i,arr) => {
      all.push(`<article>`);
      if((v=re_choice.exec(o.title))!==null){
        all.push(`<h1 class='frametitle'>${icon} <b>${this.uncode(o.style,v[1])}</b> </h1>`);
        let text = this.to_choice(o.style,o.body,v[2])
        all.push(`<h2 class='framesubtitle'> <b>${text}</b> </h2>`)
      }else{
        all.push(`<h1 class='frametitle'>${icon} <b>${this.uncode(o.style,o.title)}</b> </h1>`);
