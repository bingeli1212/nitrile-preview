...
<style>
  ...
  .slide div.BULL .DLITEM {
    margin-left: 0.75cm;
    text-indent: -0.75cm;
  }
  .slide div.BULL .BLITEM {
    margin-left: 0.75cm;
    text-indent: -0.50cm;
  }
  ...
  to_html_frame(top,the_slides){
    ...
    if(sigs[0]=='HDGS'){
      ...
      if(d.length > 0){
        ...
        all.push(`<h2 class='frametitle'>${++this.my_frameid}. ${my_title} </h2>`);
      ...
      w.forEach((o,i) => {
        all.push(`<section class='slide'>`);
        if(my_type=='exercise'){
          all.push(`<h2 class='frametitle'>${++this.my_frameid}. <i>${my_title}</i> </h2>`);
        }else{
          all.push(`<h2 class='frametitle'>${++this.my_frameid}. <i>${my_title}</i> </h2>`);
        }
        //if(o.text){
