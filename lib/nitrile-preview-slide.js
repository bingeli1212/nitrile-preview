...
`;
  ...
  write_frame_toc(topframes){
    ...
    var css_subtitle = this.css('SUBTITLE');
    ...
    topframes.forEach((topframe,j,arr) => {
      ...
      if(n==0){
        ...
        all.push(`<h2 class='SUBTITLE' style='${css_subtitle}' > &#160; </h2>`);
  ...
  write_frame_folder(id,frame,subframes){
    ...
    if(boardpng){
      ...
      all.push(`<img src='${imgsrc}' id='${imgid}' alt='' width='${114*this.MM_TO_PX}' height='${80*this.MM_TO_PX}' style='position:absolute;left:10mm;top:10mm;' />`)
    }
  ...
  write_one_frame(id,order,frame,issub,topframe){
    ...
    if(issub){
      ...
      all.push(`<h2 class='SUBTITLE' style='${css_subtitle}' > ${icon} <i>${this.uncode(frame.style,frame.title)}</i> </h2>`);
    }else{
      all.push(`<h1 class='TITLE'    style='${css_title}' > ${order} ${this.uncode(topframe.style,topframe.title)} </h1>`);
      all.push(`<h2 class='SUBTITLE' style='${css_subtitle}' > &#160; </h2>`);
    }
    ...
    frame.solutions.forEach((o,i,arr) => {
      ...
      if(o.choice){
        ...
        let row = o.style.row;
        all.push(`<h2 class='SUBTITLE' style='${css_subtitle}' data-row='${row}' > ${icon} <i> ${title} </i> &#160; ${text} </h2>`)
      }else{
        let icon = this.icon_solution;
        let title = this.uncode(o.style,o.title).trim();
