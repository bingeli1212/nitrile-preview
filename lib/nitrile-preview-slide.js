...
`;
  ...
  write_frame_folder(id,frame,subframes){
    ...
    all.push(`</div>`);
    all.push(`<canvas class='EDITBOARD' id='${boardid}' data-type='board' data-boardimgid='${boardimgid}' data-boardname='${boardname}' data-frameid='${frameid}' width='484' height='361' tabindex='1' onkeypress='canvas_keypress()' ></canvas>`);
    all.push('</article>');
    all.push('');
    //
    //NOTE: end
  ...
  write_one_frame(id,order,icon,frame,issub,topframe){
    ...
    frame.solutions.forEach((o,i,arr) => {
      ...
      all.push(`<canvas class='EDITBOARD' id='${boardid}x${i}' data-type='board' data-boardname='${boardname}' data-frameid='${frameid}' width='484' height='361' tabindex='1' onkeypress='canvas_keypress()' ></canvas>`);
      all.push('</article>');
      all.push('');
    })
    //
  ...
  to_setup_script(){
    ...
    function canvas_keypress(){
      return;
      console.log('canvas_keypress()', event.code);
    ...
    function body_keypress(){
      console.log('body_keypress()', event.code);
      var body = event.target;
      console.log('tagName',body.tagName);    
      if(body.tagName=='CANVAS'){
        ...
        var shiftKey = event.shiftKey;
        var keycode = event.code;
        board_handle_keydown(shiftKey,keycode,null,null);
      }
    }
    `;
  }
