...
`;
  ...
  write_frame_folder(id,frame,subframes){
    ...
    subframes.forEach((subframe,j,arr) => {
      ...
      all.push(`<li class='${li_css}' style='position:relative;font-style:oblique'> ${id}.${j+1} <b> <a href='#frame${id}.${j+1}'> ${this.uncode(subframe.style,subframe.title)} </a> </b> </li>`);
      if(j+1==arr.length){
        all.push(`</ul>`);
      }
    });
  ...
  write_one_frame(id,order,icon,frame,issub,topframe){
    ...
    if(issub){
      all.push(`<h1 class='${slidetitle_css}'>${order} ${this.uncode(topframe.style,topframe.title)}<br/><span style='font-size:0.6em;font-style:oblique'> ${this.uncode(frame.style,frame.title)}</span> </h1>`);
    }else{
