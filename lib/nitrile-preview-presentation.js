...
class NitrilePreviewPresentation {
  ...
  to_beamer_solution(o) {
    ...
    let text = '';
    let raw = '';
    let ischoices = /^\w+:$/.test(my.title);
    raw = this.translator.join_para(my.body);
    text = this.translator.unmask(this.translator.join_para(my.body));
    return {title,contents,text,ischoices,raw};
  }
  to_beamer_topic(o) {
