...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  translate_block(block){
    if(block.style && block.sig && block.switches){
      block.style = this.update_style_from_switches(block.style,block.sig,block.switches);
    }
    switch (block.sig) {
  ...
  s_to_phrase(name,s,g,switches){
    var newtext = '';
    if(name==='link'){
      newtext += this.phrase_to_link(s,g,switches);
    }else if(name==='ref'){
      newtext += this.phrase_to_ref(s,g,switches);
    }else if(name==='parbox'){
      var {g,s} = this.string_to_g_and_s(s,g);
      newtext += this.phrase_to_parbox(s,g,switches);
    }else if(name==='diagram'){
      var {g,s} = this.string_to_g_and_s(s,g);
      var ss = this.fetch_ss_from_ss_map(s);
      newtext += this.ss_to_fence(name,ss,g,switches);
    }
    return newtext;
  }
  ss_to_fence(name,ss,g,switches){
    if(g.load){
      let name0 = g.load;
    ...
    var newtext = '';
    if(name==='img'){
      newtext += this.fence_to_img(ss,g,switches);
    }else if(name==='diagram'){
      var g = this.update_style_from_switches(g,name,switches);
      newtext += this.fence_to_diagram(ss,g,switches);
    }else if(name==='animation'){
      var g = this.update_style_from_switches(g,name,switches);
      newtext += this.fence_to_animation(ss,g,switches);
    }else if(name==='framed'){
      var g = this.update_style_from_switches(g,name,switches);
      newtext += this.fence_to_framed(ss,g,switches);
    }else if(name==='math'){
      var g = this.update_style_from_switches(g,name,switches);
      newtext += this.fence_to_math(ss,g,switches);
    }else if(name=='tabularx'){
      var g = this.update_style_from_switches(g,name,switches);
      newtext += this.fence_to_tabularx(ss,g,switches);
    }else if(name=='tabular'){
      var g = this.update_style_from_switches(g,name,switches);
      newtext += this.fence_to_tabular(ss,g,switches);
    }else if(name=='tabbing'){
      var g = this.update_style_from_switches(g,name,switches);
      newtext += this.fence_to_tabbing(ss,g,switches);
    }else if(name=='blockquote'){
      var g = this.update_style_from_switches(g,name,switches);
      newtext += this.fence_to_blockquote(ss,g,switches);
    }else if(name=='verbatim'){
      var g = this.update_style_from_switches(g,name,switches);
      newtext += this.fence_to_verbatim(ss,g,switches);
    }else{
      var g = this.update_style_from_switches(g,'list',switches);
      newtext += this.fence_to_list(ss,g,switches);
    }
    return newtext;
  }
  untext(ss,style,switches) {
    ...
    while((v=re.exec(line))!==null){
      ...
      var txt = this.uncode(txt,style,switches);
      newtext += txt;
      var ss = v[1].split('\n')
      var s0 = ss[0]||'';
      var ss = ss.slice(1);
      var ss = this.trim_samp_body(ss);
      var {id,style:g} = this.string_to_id_and_style(s0);
  ...
  uncode(line,style,switches) {
    ...
    while ((v = re_uncode.exec(line)) !== null) {
      var i = v.index;
      var txt = line.slice(start_i, i);
      var txt = this.untouch(txt,style,switches);
      newtext += txt;
      if (v[1] !== undefined) {
        var key = v[1];
        var cnt = v[2];
  ...
  untouch (line,style,switches) {
    ...
    while ((v = re.exec(line)) !== null) {
      var i = v.index;
      var txt = line.slice(start_i,i);
      var txt = this.unphrase(txt,style,switches);
      newtext += txt;
  ...
  unphrase(line,style,switches){
    ...
    while ((v = re.exec(line)) !== null) {
      ...
      if (v[1] !== undefined && v[2] !== undefined) {
        ...
        var s = v[2];
        var g = {};
        var txt = this.s_to_phrase(name,s,g,switches);
        if(!txt){
        ...
        var s = v[3]
        var txt = this.phrase_to_inlinemath(s,g,switches);
        newtext += txt;

      } else if (v[4] !== undefined) {
        var s = v[4]
        var txt = this.phrase_to_displaymath(s,g,switches);
        newtext += txt;
  ...
  ss_to_bull_itms(ss,style,switches){
    const re = re_ss_bull;
    let itms = [];
    const re_0  = /^(\d+[\.\)])\s+(.*)$/;
    const re_a  = /^([a-z][\.\)])\s+(.*)$/;
  ...
  update_style_from_switches(style,name,switches){
    var myswitch = switches[name];
    if(myswitch){
  ...
  string_to_g_and_s(s,g){
    const re_g_and_s = /^(\[.*?\]|)(.*)$/;
    let v;
    if((v=re_g_and_s.exec(s))!==null){
      var {style:g} = this.string_to_style(v[1],g);
      s = v[2];
    }
    return {g,s};
  }
