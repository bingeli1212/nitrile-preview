...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  do_CAPT(block){
    ...
    if(id=='center'){
      ...
      text = this.float_to_center(title,label,style,body,bodyrow);
    }else if(id=='equation'){
      style = this.update_style_from_switches(style,id);
  ...
  do_PARA(block){
    ...
    var text = '';
    var isbundle = this.is_bundle(body);
    var isstep = this.is_step(body);
    var isplst = this.is_plst(body);
    if(isbundle){
      style = this.update_style_from_switches(style,'flushleft');
      text = this.float_to_flushleft(title,label,style,body,bodyrow);
    }else if(isstep){
      ...
      body = this.trim_para_at(body,4);
      text = this.float_to_verbatim(title,label,style,body,bodyrow);
    }else if(body[0].startsWith('~ ')){
      body = this.trim_para_at(body,2);
      style = this.update_style_from_switches(style,'verbatim');
      text = this.float_to_verbatim(title,label,style,body,bodyrow);
    }else if(body[0].startsWith('> ')){
      body = this.trim_para_by(body,'> ');
  ...
  body_to_record(ss,ssi){
    ...
    let v;
    ss.forEach((s,j,arr) => {
      if(j==0 && ((v=this.re_is_step.exec(s))!==null)){
        let value = v[1]||'';
        s=v[3]||'';
        let n=ssi+j;
        let text = s;
        ...
        let verbs = [];
        if(s.startsWith('~ ')){
          verbs.push(s.slice(2));
        }else if(s.startsWith('| ')){
          lines.push(s.slice(2));
        }
        o.push({value,text,verbs,lines,n});
      }else if(s=='\\\\'){
        let value = '';
        let text = '';
        let n=ssi+j;
        ...
        if(o.length){
          ...
          if(s.startsWith('~ ')){
            p.verbs.push(s.slice(2));
          }else if(s.startsWith('| ')){
            p.lines.push(s.slice(2));
          }
          o.push(p);
        }
      }
  ...
  split_bundle(block){    
    let o = [];
    let ii = this.string_to_int_array(block.style.splitrows);
    ii = ii.filter(i => i>0);
    if(ii.length){
