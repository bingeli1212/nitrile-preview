...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  do_CAPT(block){
    ...
    if(id=='@'){
      ...
      var style = this.update_style_from_switches(style,id);
      var text = this.float_to_longtable(title,label,style,body,bodyrow);
    }else if(id=='columns'){
      var style = this.update_style_from_switches(style,id);
      var text = this.float_to_columns(title,label,style,body,bodyrow);
    }else if(id=='tabbing'){
      var style = this.update_style_from_switches(style,id);
      var text = this.float_to_tabbing(title,label,style,body,bodyrow);
    }else{
      var style = this.update_style_from_switches(style,'sample');
      var text = this.float_to_sample(title,label,style,body,bodyrow);
    }
    return text;
  }
  do_SAMP(block){
  ...
  do_PLST(block){
    ...
    var itemize = this.plitems_to_itemize(plitems);
    var title = '';
    var label = '';
  ...
  do_PRIM(block) {
    ...
    var label = '';
    style.rank = rank;
    if(body.length && body[0].trim().length==0){
      body = body.slice(1);
      if(body.length && re_plst.test(body[0])){
        let plitems = this.body_to_plitems(body);
  ...
  untext(style,ss) {
    ...
    while(i < ss.length) {
      var i0 = i;//save i
      var [key,g,d,ic] = this.try_extract_bundle(style,ss,i);//NOTE that it is import to use this 'style' as template as otherwise the style.buffers and other stuffs aren't copied to sub style
      if(ic==0){
        var s = ss[i];      
        i++;
  ...
  uncode(style,line) {
    ...
    while ((v = re.exec(line)) !== null) {
      ...
      if (v[1] !== undefined) {
        ...
        ///literal_to_math or bundle
        var s = v[3];
        s = s.trim();
        var txt = '';
        var v1 = null;
        if((v1=re_phrase.exec(s))!==null){
          let key = v1[1];
          let pref = v1[2];
          let g = {...this.parser.style};
          g = this.string_to_style(pref,g);
          let d = [];
          let o = this.do_bundle(key,g,d,0);
          if(o.length){
            txt = o[0].img;
          }else{
            txt = '';
  ...
  ss_to_tabular_rows(ss,style){
    ...
    if(style.visual==2){
      ...
      ///
      while(ss.length && ss[0].startsWith('"""')){
        let s = ss.shift();
        s = s.substr(3);
        subtitles.unshift(s);
      }
  ...
  ss_to_figure_itms(style,ss,ssi){
    ...
    while(i < ss.length) {
      var i0 = i;//save i
      var [key,g,d,ic] = this.try_extract_bundle(style,ss,i);
      console.log('ss=',ss,'ssi=',ssi,'i0=',i0)
      if(ic==0){
        var s = ss[i];     
        i++;
        if(s=='\\\\'){
  ...
  do_phrase(key,style,cnt){
    ...
    switch (key) {
      ...
      case 'framebox': {
        txt = this.phrase_to_framebox(style,cnt);
        break;
      }
      default: {
  ...
  body_to_plitems(body){
    ...
    for(let s of body) {
      ...
      if((v=re_plst.exec(s))!==null){
        ...
        item.nblank = 0;
        item.bullet = v[1];
        item.body.push(' '.repeat(v[1].length+v[2].length)+v[3]);
        plitems.push(item);
      }else{
        if(item){
          item.body.push(s);
