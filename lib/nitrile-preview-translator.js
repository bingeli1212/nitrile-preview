...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  do_bundle(key,style,body){
    ...
    if(key==='dia'){
      ...
      style = this.update_style_from_switches(style,key);
      o = this.fence_to_par(style,body);
    }else if(key=='center'){
      style = this.update_style_from_switches(style,key);
  ...
  uncode(style,line) {
    ...
    while ((v = re.exec(line)) !== null) {
      ...
      if (v[1] !== undefined) {
        ...
        ///literal_to_node
        var s = v[3]
        var txt = this.literal_to_node(style,s);
        newtext += txt;
  ...
  ss_to_tabular_rows(ss,style){
    ...
    if(style.visual==2){
      ...
      ///
      while(ss.length && ss[ss.length-1].startsWith('"""')){
        let s = ss.pop();
        s = s.substr(3);
        subtitles.unshift(s);
      }
  ...
  do_phrase(key,style,cnt){
    ...
    switch (key) {
      ...
      case 'ss': {
        txt = this.phrase_to_ss(style,cnt);
        break;
      }
      case 'tt': {
      ...
      case 'overstrike': {
        txt = this.phrase_to_overstrike(style,cnt);
        break;
      }
      case 'verb': {
        txt = this.phrase_to_verb(style,cnt);
        break;
      }
      case 'code': {
        txt = this.phrase_to_code(style,cnt);
        break;
      }
      case 'var': {
        txt = this.phrase_to_var(style,cnt);
        break;
      }
      case 'g': {
