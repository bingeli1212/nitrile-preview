...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  do_identify(block, A) {
    ...
    if (sig == 'PART') {
      ...
      let mystyle = this.get_ss_style(block.body);
      style = {...style,...mystyle};
      var { name, label, title } = style;
  ...
  plitems_to_items (plitems) {
    ...
    for (var plitem of plitems) {
      ...
      if (action === 'push') {
        ...
        items.push({bull:'LI',bullet,value,text,keys,row1,row2,more});
      } else {
        if (action === 'pop') {
  ...
  get_ss_style(ss){
    ...
    if(n>1&&ss[0].startsWith('~~~')&&ss[n-1].startsWith('~~~')){ 
      let str = ss[0].slice(3);
      let {style} = this.string_to_fenceid(str);
      return style;
    }
    if(n>1&&ss[0].startsWith('```')&&ss[n-1].startsWith('```')){ 
      let str = ss[0].slice(3);
  ...
  fetch_pic_from_notes(id){
    if(this.notes.has('pic')) {
      let pics = this.notes.get('pic');
      ///'pics' is an JS Array
      for(let pic of pics){
