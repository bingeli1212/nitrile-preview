...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  uncode(line,style) {
    ...
    while ((v = re.exec(line)) !== null) {
      ...
      if (v[3] !== undefined) {
        var s = v[3]
        var txt = this.literal_to_double(style,'literaldouble',s);
        newtext += txt;
        
      } else if (v[4] !== undefined) {
        var s = v[4]
        var txt = this.literal_to_single(style,'literalsingle',s);
        newtext += txt;
    ...
    while ((v = re.exec(line)) !== null) {
      ...
      if (v[1] !== undefined) {
        ...
        switch (key) {
          ...
          case 'columnbreak': {
            txt = this.phrase_to_columnbreak(style,cnt,cnt2,cnt3);
            break;
          }
          case 'qquad': {
