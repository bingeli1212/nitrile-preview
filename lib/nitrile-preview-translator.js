...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  uncode(line,style) {
    ...
    while ((v = re.exec(line)) !== null) {
      ...
      if (v[1] !== undefined) {
        var s = v[1]
        var txt = this.do_phrase(style,'inlinemath',s);
        newtext += txt;

      } else if (v[2] !== undefined) {
        var s = v[2]
        var txt = this.do_phrase(style,'displaymath',s);
        newtext += txt;
        
      } else if (v[3] !== undefined) {
        var s = v[3]
        var txt = this.do_phrase(style,'literaldouble',s);
        newtext += txt;
        
      } else if (v[4] !== undefined) {
        var s = v[4]
        var txt = this.do_phrase(style,'literalsingle',s);
        newtext += txt;
  ...
  unphrase (line,style) {
    ...
    while ((v = re.exec(line)) !== null) {
      ...
      if (v[1] !== undefined) {
        ...
        ///
        newtext += this.do_phrase(style,key,this.unphrase(cnt,style),this.unphrase(cnt2,style),this.unphrase(cnt3,style));
        ///
