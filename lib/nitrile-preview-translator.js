...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  do_bundle(key,style,body){
    ...
    if(key==='dia'){
      ...
      o = this.fence_to_dia(style,body);
    }else if(key==='ink'){
      style = this.update_style_from_switches(style,key);
      ...
      style = this.update_style_from_switches(style,key);
      o = this.fence_to_math(style,body);
    }else if(key=='tabular'){
      style = this.update_style_from_switches(style,key);
      ...
      style = this.update_style_from_switches(style,key);
      o = this.fence_to_parbox(style,body);
    }else if(key=='center'){
      style = this.update_style_from_switches(style,key);
  ...
  untext(style,body) {
    ...
    while(i < body.length) {
      var i0 = i;//save i
      var [key,g,d,ic] = this.try_extract_bundle(style,body,i);//NOTE that it is import to use this 'style' as template as otherwise the style.buffers and other stuffs aren't copied to sub style
      if(ic==0){
        var s = body[i];      
        i++;
