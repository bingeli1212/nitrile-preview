...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  s_to_phrase(name,s,g){
    var newtext = '';
    if(name==='link'){
      newtext += this.phrase_to_link(s,g);
    }else if(name==='ref'){
      newtext += this.phrase_to_ref(s,g);
    }else{
      newtext += this.phrase_to_link(s,g);
    }
    return newtext;
  ...
  unmask(line,style) {
    ...
    while((v=re.exec(line))!==null){
      ...
      var name = ss[0]||'';
      var ss = ss.slice(1);
      var [g] = this.string_to_style(name,style);
      var txt = this.ss_to_fence(fence_id,ss,g);
      newtext += txt;
      /// move on the next
  ...
  unphrase(line,style){
    ...
    while ((v = re.exec(line)) !== null) {
      ...
      if (v[1] !== undefined && v[2] !== undefined) {
        ...
        var txt = this.str_to_phrase(fence_id,str,style);
        newtext += txt;
      }
      start_i = re.lastIndex;
    }
  ...
  ss_to_figure_itms(ss,style){
    ...
    while((v=re.exec(line))!==null){
      ...
      if(v[1] !== undefined && v[2] !== undefined){
        var type = v[1];
        var ss = v[2].split('\n');
        var [g] = this.string_to_style(ss[0]);
        var ss = ss.slice(1);
        var sub = '';
  ...
  fence_to_animation(mode,style){
    ...
    for(let j=0; j<steps; ++j){
      ...
      let mode = {body:ss1};
      var s = this.fence_to_diagram(mode,style);
      d.push(s);
    }
    var text = d.join('\n')
    return text;
