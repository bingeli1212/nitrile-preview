...
const allfontfamilies = ['serif','sans','mono','math','xelatex'];
  ...
  constructor(parser) {
    ...
    for(let s of fonts){
      ...
      ss.forEach((s) => {
        ...
        let v2 = allfontfamilies.indexOf(s);
        if(v1>=0){
          fid = s;
        }else if(v2>=0){
          fontfamily = s; 
  ...
  do_bundle(style,ss,ssi){
    ...
    if(s0.startsWith("```") && sn.startsWith("```")){
      ...
      if(v){
        ...
        }else if(style.subcaptions && style.subcaptions.hasOwnProperty("_")){
          style.subtitle = style.subcaptions["_"];
        }
        style.row = ssi;
        style.float = key;
      }
