...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  ss_to_table_rows(ss,style){
    ...
    if(re_rows.test(ss[0])){
      ...
      /// if style.n is set, we will consider each source line 
      /// a table cell by itself, and we will try to figure out
      /// the maximum row based on n, and then then fill the first
      /// column first before filling the second one, and then the third one
