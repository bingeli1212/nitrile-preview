...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  do_PARA(block){
    ...
    if(this.body_is_bundle(body)){
    ...
    }else if(this.re_body_is_itemize.test(body[0])||this.re_body_is_enumerate.test(body[0])||this.re_body_is_description.test(body[0])){
      ...
      body = this.body_to_example(body);
  ...
  do_bundle(style,bundle,key=''){
    if(bundle){
      ...
      if(g.restore){
        let name0 = g.id||'_';
        if(this.re_is_symbol_name.test(g.restore)){
          name0 = g.restore;
        }
        let lineno = 1;
      ...
      if(g.save){
        let name0 = g.id||'_';
        if(this.re_is_symbol_name.test(g.save)){
          name0 = g.save;
        }
  ...
  ss_to_tabbing_rows(ss,ssi,env){
    ...
    hh.forEach((s,j) => {
      ...
      if(j==0){
        ...
        while((v=re.exec(s))!==null){
          ...
          var pw = v[1].length;
          var ta = '';
          pp.push({i,n:0,ta,pw,raw:'',text:'',hd:''});
        }
        pp.forEach((p,i,arr) => {
          if(i+1==arr.length){
          }else{
    ...
    var rows = this.ss_to_tab_rows(ss,ssi+1,ww.length,env);
    rows.unshift(ww);        
    return rows;
  }
  ///////////////////////////////////////////////////////////////////////////////
  ...
  ss_to_plitems(style,ss,ssi){
    ...
    ss.forEach((s,i,arr) => {
      ...
      if((v=this.re_body_is_itemize.exec(s))!==null){
        ...
        plitems.push(item);
      }else if((v=this.re_body_is_enumerate.exec(s))!==null){
        item = {};
        item.n = ssi+i;
        item.nblank = 0;
        item.body = [];
        ...
        item.body.push(' '+v[2]+v[3]);
        plitems.push(item);
      }else if((v=this.re_body_is_description.exec(s))!==null){
        item = {};
        item.n = ssi+i;
        item.nblank = 0;
        item.body = [];
  ...
  /////////////////////////////////////////////////////////////////////////////
  /////////////////////////////////////////////////////////////////////////////
  body_to_example(body){
    var all = [];
    body.forEach((s)=>{
      let v;
      if((v=this.re_body_is_example.exec(s))!==null){
        all.push(v[3]);
      }else{
        if(all.length){
          let s1 = all.pop();
  ...
  /////////////////////////////////////////////////////////////////////////////
  body_to_body(body){
    //convert some common unicode formatting characters to ASCII formatting characters
    body = body.map((s)=>{
      s = s.replace(/\u{ff3c}\u{ff3c}$/su,'\\\\');
      return s;
    })
    body = body.map((s)=>{
      s = s.replace(/^\u{ff1e}\s/su,'> ');
      return s;
    })
    return body;
  }
