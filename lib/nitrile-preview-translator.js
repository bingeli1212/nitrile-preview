...
const re_hrle = /^\*{3}$/u;
const re_sep = /^\s*(-{3,})$/u;
const re_ss_bull = /^(-|\+|\*|>|\d+[\*\)\.]|[a-zA-Z][\*\)\.]|\[[A-Za-z0-9]\]:)\s+(.*)$/u;
const re_uri = /^\w+:\/\//u;
...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  ss_to_bull_itms(ss){
    ...
    if((v=re_ss_bull.exec(ss[0]))!==null){
      bull0 = v[1];
      if(re_checkbox.test(bull0)) { type = 'checkbox'; ending='.'; }
      else if(re_n.test(bull0)) { type = '0'; ending=bull0.substr(-1); if(ending=='*') ending='.'; }
    ...
    ss.forEach(s => {
      if((v=re.exec(s))!==null){
        ...
        let text = v[2];
        let value = '';
        if(type=='checkbox' && re_checkbox.test(bull)){
          value = bull.substr(1,1);
          itms.push({bull,text,value,ending,type});
          return;
        }else if(type=='0' && re_n.test(bull) && bull.substr(-1)==bull0.substr(-1)){
          value = parseInt(bull);
          itms.push({bull,text,value,ending,type});
          return;
  ...
  ss_to_fenced_ss(ss){
    ...
    if(n>1 && (v=re_cluster_line.exec(s0))!==null && (w=re_cluster_line.exec(sz))!==null && v[1]===w[1] && v[2]===w[2]){ 
      let head = v[3].trim();
      var {fenceid,style} = this.string_to_fenceid(head);
      ss = ss.slice(1,n-1);
      ss = this.trim_samp_body(ss);
    }
