...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  constructor(parser) {
    ...
    this.re_dhline = /^={1,}$/;
    this.re_style = /^\{(.*)\}$/;
    this.re_dl_value = /^([\w\-]+)(\s+-\s+)(.*)$/;//it has to start with a word otherwise it could grab a hyphen minus from a math expression
    this.re_ul_key1 = /^([\w\-]+)(\s+-\s+|:\s+)(.*)$/;//same as above
    this.re_ul_key2 = /^(\S+)(\s+-)$/;//same as above
  ...
  do_bundle(key,style,body){
    ...
    if(key==='img'){
      ...
      style = this.update_style_from_switches(style,key);
      o = this.fence_to_canvas(style,body);
    }else if(key==='diagram'){
      style = this.update_style_from_switches(style,key);
    ...
    for (var plitem of plitems) {
      ...
      if (bullet) {
        ...
        if (bullet[0] == '-'){
          ...
          // let re0 = /^(.*?)(\s+-\s+)(.*)$/;
          let v0;
          if(plitem.body.length==1 && (v0=this.re_ul_key1.exec(plitem.body[0]))!==null){
            key = v0[1];
            sep = v0[2];
            body = [v0[3]].concat(plitem.body.slice(1));
            text = body.join('\n');
          }else if(plitem.body.length>1 && (v0=this.re_ul_key2.exec(plitem.body[0]))!==null){
            key = v0[1];
            sep = v0[2];
          ...
          bull = 'DL';
          // let re0 = /^(.*?)(\s+-\s+)(.*)$/;
          let v0 = this.re_dl_value.exec(plitem.body[0]);
          if(plitem.body.length==1 && v0){
            value = v0[1];
            body = [v0[3]].concat(plitem.body.slice(1));
            text = body.join('\n');
          }else{
