...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  untext(ss,style) {
    ...
    if(n>1 && (v=re_cluster_line.exec(s0))!==null && (w=re_cluster_line.exec(sz))!==null && v[1]===w[1] && v[2]===w[2]){ 
      let head = v[3].trim();
      let {fenceid,style:style1} = this.string_to_fenceid(head);
      ...
      if(fenceid==='diagram'){
        return this.para_to_diagram(ss,style);
      }
      if(fenceid==='figure'){
  ...
  ss_to_itemized_itms(ss){
    ...
    ss.forEach(s => {
      ...
      if(this.line_is_indented(s)){
        ...
        let value='';
        if(bull=='OL'){
          if(v[1].codePointAt(0) >= 65 && v[1].codePointAt(0) <= 90) {
            type='A';
            value = v[1].codePointAt(0) - 65 + 1;
          }else if (v[1].codePointAt(0) >= 97 && v[1].codePointAt(0) <= 122) {
            type='a';
            value = v[1].codePointAt(0) - 97 + 1;
          }else{
            type='1';
            value = parseInt(v[1]);
          }
  ...
  ss_to_bull_itms(ss){
    ...
    let bull0;
    let ending;
    if((v=re_ss_bull.exec(ss[0]))!==null){
      bull0 = v[1];
      if(re_n.test(bull0)) { type = '0'; ending=bull0.substr(-1); if(ending=='*') ending='.'; }
      else if(re_a.test(bull0)) { type = 'a'; ending=bull0.substr(-1); if(ending=='*') ending='.'; }
      else if(re_A.test(bull0)) { type = 'A'; ending=bull0.substr(-1); if(ending=='*') ending='.'; }
      else if(bull0=='::') type = 'DL';
      else if(bull0=='++') type = 'HD';
      else if(bull0=='--') type = 'UL';
      else if(bull0=='**') type = 'OL';
    }
    ss.forEach(s => {
      if((v=re.exec(s))!==null){
        let bull = v[1];
        let text = v[2];
        let value = '';
        if(type=='0' && re_n.test(bull) && bull.substr(-1)==bull0.substr(-1)){
          value = parseInt(bull);
          itms.push({bull,text,value,ending,type});
          return;
        }else if(type=='a' && re_a.test(bull) && bull.substr(-1)==bull0.substr(-1)){
          value = bull[0].codePointAt(0) - 'a'.codePointAt(0) + 1;
          itms.push({bull,text,value,ending,type});
          return;
        }else if(type=='A' && re_A.test(bull) && bull.substr(-1)==bull0.substr(-1)){
          value = bull[0].codePointAt(0) - 'A'.codePointAt(0) + 1;
          ...
          itms.push({bull,text,value,ending,type});
          return;
        }else if(type=='OL' && bull==bull0){
          value = itms.length+1;
          itms.push({bull,text,value,ending,type});
          return;
        }else if(type=='UL' && bull==bull0){
          itms.push({bull,text,value,ending,type});
          return;
        }else if(type=='HD' && bull==bull0){
          itms.push({bull,text,value,ending,type});
          return;
        }
      }
