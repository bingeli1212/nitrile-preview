...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  s_to_phrase(name,s,g){
    ...
    if(name==='link'){
      ...
      newtext += this.phrase_to_parbox(s,g);
      ...
      newtext += this.ss_to_fence(name,ss,g);
    }else if(name==='img'){
      var {g,s} = this.string_to_g_and_s(s,g);
      if(s.startsWith('#')){
        var ss = this.fetch_ss_from_ss_map(s.slice(1));
  ...
  ss_to_fence(name,ss,g){
    ...
    if(name==='img'){
      ...
      var g = this.update_style_from_switches(g,name);
      ...
      var g = this.update_style_from_switches(g,name);
      newtext += this.fence_to_tabularx(ss,g);
  ...
  unphrase(line,style){
    ...
    while ((v = re.exec(line)) !== null) {
      ...
      if (v[1] !== undefined && v[2] !== undefined) {
        ...
        var s = v[2];
        var g = {};
        var txt = this.s_to_phrase(name,s,g);
        if(!txt){
        ...
        var s = v[3]
        var txt = this.phrase_to_inlinemath(s,g);
        newtext += txt;

      } else if (v[4] !== undefined) {
        var s = v[4]
        var txt = this.phrase_to_displaymath(s,g);
        newtext += txt;
  ...
  do_FLOA(block){
    ...
    if(title=='equation'){
      ...
      this.do_FLOA_tablex(block,caption,ss,style);
    }else if(title=='table'){
