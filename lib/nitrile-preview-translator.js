...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  constructor(parser) {
    ...
    this.my_bundle_map = new Map();
    this.fonts = [];       
    if(0){
      this.add_fid('cn','arplsungtilgb','');
      this.add_fid('tw','arplmingti2lbig5','');
      this.add_fid('jp','ipaexmincho','');
      this.add_fid('kr','baekmukbatang','');
    }
    // 'cjkfontmap' 
    ...
    this.program = this.parser.conf_to_string('program');
  ...
  do_FRNT(block){
    ...
    if(style.name=='chapter'){
      var text = this.hdgs_to_chapter(title,style);
    }
    return text;
  }
  do_PART(block){
    var {title,label,level,style} = block;
    var text = this.hdgs_to_part(title,style);
    return text;
  }
  do_CHAP(block){
    var {title,label,level,style} = block;
  ...
  do_HDGS(block){
    ...
    var text = this.hdgs_to_section(title,style);
    return text;
  }
  do_CAPT(block){
    var {id,title,label,body,bodyrow,style} = block;
    var text = '';
    var bundles = [];
    var o = [];
    o.push('');
    if(id=='equation'){
      ...
      bundles = this.body_to_all_bundles(style,body,bodyrow);
      text = this.float_to_equation(title,label,style,bundles);
    }else if(id=='listing'){
      style = this.update_style_from_switches(style,id);
      bundles = this.body_to_all_bundles(style,body,bodyrow);
      text = this.float_to_listing(title,label,style,bundles)
    }else if(id=='figure'){
      style = this.update_style_from_switches(style,id);
      bundles = this.body_to_all_bundles(style,body,bodyrow);
      text = this.float_to_figure(title,label,style,bundles);
    }else if(id=='table'){
      style = this.update_style_from_switches(style,id);
      bundles = this.body_to_all_bundles(style,body,bodyrow);
      text = this.float_to_table(title,label,style,bundles);
    }else if(id=='multicols'){
      style = this.update_style_from_switches(style,id);
      bundles = this.body_to_all_bundles(style,body,bodyrow);
      text = this.float_to_multicols(title,label,style,bundles);
    }else if(id=='lines'){
      style = this.update_style_from_switches(style,id);
      bundles = this.body_to_all_bundles(style,body,bodyrow);
      text = this.float_to_lines(title,label,style,bundles);
    }else if(id=='itemize'){
      style = this.update_style_from_switches(style,id);
      bundles = this.body_to_all_bundles(style,body,bodyrow);
      text = this.float_to_itemize(title,label,style,bundles);
    }else if(id=='page'){
      style = this.update_style_from_switches(style,id);
      bundles = this.body_to_all_bundles(style,body,bodyrow);
      text = this.float_to_page(title,label,style,bundles);
    }else if(id=='vspace'){
      style = this.update_style_from_switches(style,id);
      bundles = this.body_to_all_bundles(style,body,bodyrow);
      text = this.float_to_vspace(title,label,style,bundles);
    }
    return text;
  }
  do_PARA(block){
    ...
    var text = '';
    if(style.isbundle){
      let bundle = this.try_extract_bundle(style,body,bodyrow,0);
      text = this.bundle_to_text(style,bundle);
    }else if(style.issamp){
      let samp = this.ss_to_samp(body,bodyrow);
      style = this.update_style_from_switches(style,'samp');
      text = this.samp_to_text(style,samp);
    }else if(style.issand){
      let sand = this.ss_to_sand(body,bodyrow);
      style = this.update_style_from_switches(style,'sand');
      text = this.sand_to_text(style,sand);
    }else if(style.iscove){
      let cove = this.ss_to_cove(body,bodyrow);
      style = this.update_style_from_switches(style,'cove');
      text = this.cove_to_text(style,cove);
    }else if(style.iscave){
      let cave = this.ss_to_cave(body,bodyrow);
      style = this.update_style_from_switches(style,'cave');
      text = this.cave_to_text(style,cave);
    }else if(style.islave){
      let lave = this.ss_to_lave(body,bodyrow);
      style = this.update_style_from_switches(style,'lave');
      text = this.lave_to_text(style,lave);
    }else if(style.isstep){
      let step = this.ss_to_step(body,bodyrow);
      style = this.update_style_from_switches(style,'step');
      text = this.step_to_text(style,step);  
    }else if(style.isplst){
      let plitems = this.ss_to_plitems(body,bodyrow);
      let itemize = this.plitems_to_itemize(plitems);
      style = this.update_style_from_switches(style,'plst');
      text = this.plst_to_text(style,itemize);  
    }else{
      style = this.update_style_from_switches(style,'body');
      text = this.body_to_text(style,body);
    }  
    return text;
  }
  do_PRIM(block) {
  ...
  do_SECO(block) {
    ...
    var label = '';
    style.rank = rank;
    return this.float_to_secondary(title,label,style,body,bodyrow);
  }
  ...
  do_bundle(style,bundle,key=''){
    ...
    if(id && subcaption){
      style.subtitle = `${subcaption}`;
    }else if(subcaption1){
      style.subtitle = subcaption1;
    }
    style.row = ssi;
    style.float = key;
    if(style.restore){
      ...
      if(this.my_bundle_map.has(name0)){
        let ss0 = this.my_bundle_map.get(name0);
    ...
    if(style.save){
      let name0 = style.id||'';
      name0 = '_'+name0;
      this.my_bundle_map.set(name0,ss);
    }
    var itm = {};
    //update from switch
    if(key=='dia'){
      style = this.update_style_from_switches(style,key);
      itm = this.fence_to_dia(style,ss,ssi);
    }else if(key=='ink'){
      style = this.update_style_from_switches(style,key);
      itm = this.fence_to_ink(style,ss,ssi);
    }else if(key=='fml'){
      style = this.update_style_from_switches(style,key);
      itm = this.fence_to_fml(style,ss,ssi);
    }else if(key=='tab'){
      style = this.update_style_from_switches(style,key);
      itm = this.fence_to_tab(style,ss,ssi);
    }else if(key=='par'){
      style = this.update_style_from_switches(style,key);
      itm = this.fence_to_par(style,ss,ssi);
    }else{
      key = 'vtm';
      style = this.update_style_from_switches(style,key);
      itm = this.fence_to_vtm(style,ss,ssi);
    }
  ...
  uncode(style,line) {
    ...
    const re = /(`{1,2}.*?`{1,2})|(?<!\w)\*\*([^\*\s]+)\*\*(?!\w)|(?<!\w)\*([^\*\s]+)\*(?!\w)|\\\((.*?)\\\)|\\\[(.*?)\\\]|(?<!\\)(&[A-Za-z]+\{.*?\})|(?<!\w)(".*?")(?!\w)|(?<!\\)(\{\{.*?\}\})/ugs; //cannot be global because it is used recursively
    while ((v = re.exec(line)) !== null) {
      ...
      newtext += txt;
      if (v[1] !== undefined) {
        ///literal_to_verb or literal_to_math depending o the number of backquotes
        var s = v[1];
        if(s.startsWith('``')){
          let s1 = s.replace(/`/g,'').trim();
          var txt = this.literal_to_math(style,s1);
        }else{
          let s1 = s.replace(/`/g,'').trim();
        ...
        ///literal_to_quotation
        var s = v[7];
        if(1){
          var s = s.slice(1,-1);
          var txt = this.literal_to_quotation(style,s);
          newtext += txt;
        }
        ...
        if(1){
          var s = s.slice(2,-2);
          var txt = this.literal_to_span('em',style,s);
          newtext += txt;
        }
  ...
  ss_to_tabular_rows(style,ss,ssi){
    ...
    if(style.direction=='column'){
      ...
      /// row is marked by the presence of a hline, or a double-backslash
      ss.forEach((s,i) => {
        var re = this.re_split_doublebackslash;
        var ss = s.split(re);
        pp = ss.map((raw) => {
          let row = ssi+i;
          return {raw,row};
    ...
    for (var plitem of plitems) {
      ...
      if (bullet) {
        ...
        if (bullet[0] == '-'){
          ...
          value = '';
          let v0;
          if(plitem.body.length==1 && (v0=this.re_is_key_term.exec(plitem.body[0].trim()))!==null){
            value = v0[1]+v0[2]+v0[1];
            body = [v0[3]];
            text = body.join('\n');
            type = '1';
            bull = 'UL';///changed to 'UL'
            let v1;
            if((v1=this.re_is_var.exec(value))!==null){
              keytype = 'var';
              key = v1[1];
            }else if((v1=this.re_is_verb.exec(value))!==null){
              keytype = 'verb';
              key = v1[1];
            }else if((v1=this.re_is_quotation.exec(value))!==null){
              keytype = 'quotation';
              key = v1[1];
            }else{
              keytype = '';
  ...
  build_default_pagenum_map(){
    ...
    for (let block of this.parser.blocks) {
      if(block.style.name=='part' || block.style.name=='chapter'){
        ...
        pp.push({block});
        o.push(pp);
      }else if(block.style.name=='page'){
        pp = [];
        pp.push({block});
        o.push(pp);
      }else if(block.style.split){
        this.split_bundle(block);
        let all_ids = this.string_to_int_array(block.style.splitids);
        all_ids.forEach((splitid,j,arr) => {
          if(j==0){
            let block1 = block;//use the original block and style
            block1.style.splitid = splitid;
            pp.push({block:block1});
          }else{
            pp = [];
            o.push(pp);
            let block1 = {...block};//duplicate and create a new block
            block1.style = {...block1.style};//duplicate and create a new style as well
            block1.style.splitid = splitid;
            pp.push({block:block1});
          }
        });
  ...
  ss_to_plitems(ss,ssi){
    ...
    ss.forEach((s,i,arr) => {
      let v;
      if((v=this.re_is_plst.exec(s))!==null){
        item = {};
        item.n = ssi+i;
        item.nblank = 0;
        item.body = [];
  ...
  add_fid(fid,fontname,program){
    let p = {fid,program,fontname};
    this.fonts.push(p);
  }
  ...
  to_subcaption(key,style){
    var val = '';
    for(let p of style.subcaptions){
      if(p[0]==key){
        val = p[1];
      }
    }
  ...
  /////////////////////////////////////////////////////////////////////////////
  /////////////////////////////////////////////////////////////////////////////
  to_fig_subtitle(style,def_title=''){
    var all = [];
    var subtitle = style.subtitle||def_title;
  ...
  split_bundle(block){    
    let o = [];
    let ii = this.string_to_int_array(block.style.split);
    let [s0,ss,sn] = this.unwrap_bundle(block.body);
    let sh = [];//the head line
    if(block.style.head){
      let head = parseInt(block.style.head)||0;
      if(head){
        sh = ss.slice(0,head);
        ss = ss.slice(head); 
      }
    ...
    while(total_n > 0){
      ...
      o.push({ss2,splitid,splitstart});
    }
    let body = [];
    o.forEach((p) => {
      let {ss2,splitid,splitstart} = p;
      body = body.concat(ss2);
    });
    let splitids = o.map(p => p.splitid).join(' ');
    let splitstarts = o.map(p => p.splitstart).join(' ');
    block.style.splitids = splitids;
    block.style.splitstarts = splitstarts;
    block.body = body;
    return o;
  }
}
