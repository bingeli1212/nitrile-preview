...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  do_CAPT(block){
    ...
    if(id=='@'){
      ...
      var style = this.update_style_from_switches(style,id);
      var text = this.float_to_ink(title,label,style,body);
    }else{
    ...
    for (var plitem of plitems) {
      ...
      if (bullet) {
        ...
        if (bullet[0] == '-'){
          ...
          body = plitem.body;
          text = body.join('\n');
          if((v0=this.re_ul_key_var.exec(body0))!==null){
            key = v0[1];
            sep = v0[2] + ' ';
            keytype = 'var';
            body = [v0[3]].concat(plitem.body.slice(1));
            text = body.join('\n');
          }else if((v0=this.re_ul_key_verb.exec(body0))!==null){
            key = v0[1];
            sep = v0[2] + ' ';
            keytype = 'verb';
            body = [v0[3]].concat(plitem.body.slice(1));
            text = body.join('\n');
          }else if((v0=this.re_ul_key.exec(body0))!==null){
            key = v0[1];
            sep = v0[2] + ' ';
            keytype = '';
            body = [v0[3]].concat(plitem.body.slice(1));
