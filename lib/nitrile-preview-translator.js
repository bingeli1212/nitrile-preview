...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  s_to_phrase(name,s,g){
    ...
    if(name==='link'){
      ...
      var [g,s] = this.string_to_style(s,g);
      if(s.startsWith('#')){
  ...
  untext(ss,style) {
    ...
    while((v=re.exec(line))!==null){
      ...
      var [id,g] = this.string_to_id_and_style(s0,g);
  ...
  unphrase(line,style){
    ...
    while ((v = re.exec(line)) !== null) {
      ...
      if (v[1] !== undefined && v[2] !== undefined) {
        ...
        var s = v[5]
        var txt = this.phrase_to_uncode(s,g);
        newtext += txt;
        
      } else if (v[6] !== undefined) {
        var s = v[6]
        var txt = this.phrase_to_unquote(s,g);
        newtext += txt;
  ...
  ss_to_figure_itms(ss,style){
    ...
    ss.forEach((s,j,arr) => {
      ...
      if(v){
        if(fence_flag==0){
          let [id,g] = this.string_to_id_and_style(v[1],style);;
          itm = {};
          itm.id = id;
          itm.g = g;
          itm.ss = [];
