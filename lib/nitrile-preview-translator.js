...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  s_to_phrase(name,s,g,switches){
    var newtext = '';
    if(name==='link'){
      newtext += this.phrase_to_link(s,style,switches);
    }else if(name==='ref'){
      newtext += this.phrase_to_ref(s,style,switches);
    }else if(name==='parbox'){
      var {g,s} = this.string_to_g_and_s(s);
  ...
  string_to_g_and_s(s){
    ...
    if((v=re_g_and_s.exec(s))!==null){
      var {style:g} = this.string_to_style(v[1]);
      s = v[2];
    }
    return {g,s};
  }
