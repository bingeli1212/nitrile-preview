...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  do_identify(block, A) {
    ...
    var { floatname, label, title } = style;
    ...
    if(n>1&&ss[0].startsWith('~~~')&&ss[n-1].startsWith('~~~')){ 
      ...
      if(ss.length){
        ...
        if(fenceid==='longtable'){
          return this.para_to_longtable(ss,style2);
        }
        if(fenceid==='imgrid'){
          return this.para_to_imgrid(ss,style2);
        }
        if(fenceid==='lines'){
        ...
        ///auto-detect
        if(this.para_is_lines(ss)){
          ///ss could be modified
          return this.para_to_lines(ss,style2);
        }
  ...
  para_is_lines(ss){
    let re = /^>\s+(.*)$/;
    let v;
    if(ss.length && re.test(ss[0])){
      ///parse > lines
      let new_ss = [];
      for(let s of ss){
        if(re_indentedline.test(s)){
          if(new_ss.length){
            let new_s = new_ss.pop();
            s = this.join_line(new_s,s);
            new_ss.push(s);
          }
          continue;
        }
    ...
    for (var plitem of plitems) {
      ...
      if (bullet) {
        ...
        if (bullet[0] == '-'){
          ...
          bull = 'UL';
          if(lead==''){
            keys = this.string_to_keys(plitem.body[0]);
            if(keys){
      ...
      if (action === 'push') {
        ...
        items.push({bull:'LI',bullet,value,text,dl,keys,row1,row2,more});
      } else {
        if (action === 'pop') {
