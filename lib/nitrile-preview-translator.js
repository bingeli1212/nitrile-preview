...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  do_PARA(block){
    ...
    if(this.body_is_bundle(body)){
      ...
      body = this.trim_para_at(body,2);
      style = this.update_style_from_switches(style,'tabular');
  ...
  do_bundle(style,bundle,key=''){
    ...
    if(bundle){
      ...
      g = bundle.g;//bundle.g has already been merged with parent style
    }
    if(g.restore){
      let name0 = g.id||'_';
      let lineno = parseInt(g.restore)||1;
      if(g['$'].buffers.hasOwnProperty(name0)){
        let {ss:ss0} = g['$'].buffers[name0];
    ...
    if(g.save){
      let name0 = g.id||'_';
      g['$'].buffers[name0] = {ss,key,g};
    }
    var id = g.id;
    g.subtitle = g.subtitle||'';
    var subcaption = this.to_subcaption(id,g);
    var subcaption1 = this.to_subcaption('',g);
    if(id && subcaption){
    ...
    //update from switch
    if(key=='img'){
      g = this.update_style_from_switches(g,key);
  ...
  ///////////////////////////////////////////////////////////////////////////////
  ///////////////////////////////////////////////////////////////////////////////
  ss_to_tabbing_rows(ss,ssi){
    const re_aux = /^\^(\$\d+):(\w+)\s*=\s*(.*)$/;
    var rows = [];
    var ww = null;
    ss.forEach((s,j) => {
      var pp = [];
      if(j==0){
        ...
        while((v=re.exec(s))!==null){
          var i = v.index;
          var pw = v[1].length;
          pp.push({i,n:0,pw,raw:'',hd:''});
        }
        pp.forEach((p,i,arr) => {
          if(i+1==arr.length){
          }else{
            p.n = arr[i+1].i - arr[i].i;
          }
        });
        rows.push(pp);
        ww = pp;
      }else{
        if((v=re_aux.exec(s))!==null){
          let id = parseInt(v[1].slice(1));
          let cmd = v[2];
          let cnt = v[3];
          if(id>=1 && id<=ww.length){
            let w = ww[id-1];
            if(cmd=='fn'||cmd=='fm'||cmd=='hd'){
              w[cmd]=cnt;
            }
          }
        }else{
          ww.forEach((w) => {
            let raw = s.slice(w.i,w.i+w.n);
            if(w.n==0){
              raw = s.slice(w.i);
            }
            pp.push({raw});
    ...
    //
    // perform calculation and add hd
    //
    var expr = new NitrilePreviewExpr(null);
    rows.forEach((pp,j) => {
      if(j==0){
      }
      if(j>0){
        let g = {};
        let xx = pp.map(p=>p.raw);
        xx.forEach((x,i) => {
          let key = '$'+`${1+i}`;
          g[key] = x;
        });
        ...
        pp.forEach((p,i) => {
          ...
          if(ww[i].fn){
            let fn = ww[i].fn
            let [num] = expr.extract_next_expr(fn,g);
            raw = num;
          }
          if(ww[i].fm){
            let fm = ww[i].fm;
            raw = this.to_formatted_text(raw,fm);
          }
          p.raw = raw;
        });
      }
    ...
    //
    // convert all Complex to string
    //
    rows.forEach(pp => {
      pp.forEach(p => {
        if(typeof p.raw != "string"){
          p.raw = p.raw.toString();
        }
      })
    });
    return rows;
  ...
  ss_to_tab_rows(ss,ssi,max_n){
    ...
    ss.forEach((s) => {
      if(s.startsWith('& ')){
        ...
        s = s.slice(2);
        //start a new row
        if(this.re_split_doublebackslash.test(s)){
          var ss = s.trim().split(this.re_split_doublebackslash).map(x=>x.trim());
        }else{
          var ss = s.trim().split(this.re_split_triplespace).map(x=>x.trim());
        }
        let pp = ss.map((raw) => {
          let row = ssi+i;
          return {raw,row};
        ...
        i++;
        if(i==1){///first column 
          startj = rows.length;//all future columns start from this row
        ...
        if(cmd=='& '){
          ...
          ++j;
          if(i==1){
            let pp = 'x'.repeat(max_n).split('').map(x => {return {raw:'',row:''}});
            rows.push(pp);
          }
          if(j<rows.length){
            if(i <= max_n){
    ...
    rows.forEach((pp) => {
      ...
      let xx = pp.map(p=>p.raw);
      xx.forEach((x,i) => {
        let key = this.int_to_letter_A(1+i);
        g[key] = x;
      });
      pp.forEach((p) => {
        let raw = p.raw.trim();
        if(raw.startsWith('${') && raw.endsWith('}')){
          raw = raw.slice(2,-1);
          let [num] = expr.extract_next_expr(raw,g);
          p.raw = num;//a Complex number
        }
      });
    });
    // return
  ...
  ss_to_plitems(style,ss,ssi){
    ...
    ss.forEach((s,i,arr) => {
      let v;
      if((v=this.re_body_is_itemize.exec(s))!==null){
        item = {};
        item.n = ssi+i;
        item.nblank = 0;
        item.body = [];
