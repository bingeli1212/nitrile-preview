...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  translate_block(block){
    block.style.vmap = block.vmap;
    block.style.switches = block.switches;
    switch (block.sig) {
  ...
  do_PLST(block){
    ...
    if(!nblank){
      var body = this.trim_samp_body(para);
      return this.fence_to_list(body,style);
    }else{
  ...
  do_fence(key,style,body){
    ...
    var newtext = '';
    if(key==='img'){
      newtext += this.fence_to_img(body,style);
    }else if(key==='diagram'){
      newtext += this.fence_to_diagram(body,style);
    }else if(key==='framed'){
      newtext += this.fence_to_framed(body,style);
    }else if(key==='math'){
      newtext += this.fence_to_math(body,style);
    }else if(key=='tabular'){
      newtext += this.fence_to_tabular(body,style);
    }else if(key=='tabbing'){
      newtext += this.fence_to_tabbing(body,style);
    }else if(key=='verbatim'){
      newtext += this.fence_to_verbatim(body,style);
    }else if(key=='parbox'){
      newtext += this.fence_to_parbox(body,style);
    }else if(key=='colorbutton'){
      newtext += this.fence_to_colorbutton(body,style);
    }else{
      newtext += this.fence_to_list(body,style);
    }
    return newtext;
  }
  untext(ss,style) {
    ...
    while((v=re.exec(line))!==null){
      ...
      newtext += txt;
      var body = v[1].split('\n')
      var [key,style1,body] = this.extract_fence(body,0,style);
      var txt = this.do_fence(key,style1,body);
      newtext += txt;
      /// move on the next
  ...
  unphrase (line,style) {
    ...
    while ((v = re.exec(line)) !== null) {
      ...
      if (v[1] !== undefined) {
        ...
        cnt = this.unphrase(cnt,style);
        cnt2 = this.unphrase(cnt2,style);
        cnt3 = this.unphrase(cnt3,style);
        /*
  ...
  do_phrase(key,style,cnt,cnt2,cnt3){
    ...
    switch (key) {
      ...
      case 'vspace': {
        txt = this.phrase_to_vspace(style,cnt,cnt2,cnt3);
        break;
      }
      case 'hspace': {
        txt = this.phrase_to_hspace(style,cnt,cnt2,cnt3);
        break;
      }
