...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  do_SAMP(block){
    ...
    if(id=='@'){
      ...
      if(style.type=='paragraph'){
        ...
        clusters.forEach((ss) => {
          var s = this.text_to_sample(style,ss);
          var s = `<div class='PARA'> ${s} </div>`;
          all.push(s);
        })
      }else{
        var s = this.text_to_sample(style,body);
        var s = `<div class='PARA'> ${s} </div>`;
        all.push(s);
      }
      var text = all.join('\n');
  ...
  do_fence(key,style,body){
    ...
    if(key==='img'){
      ...
      style = this.update_style_from_switches(style,key);
      return this.fence_to_lines(style,body);
  ...
  untext(style,body) {
    ...
    if(s0.trim()==='~~~' && sZ.trim()==='~~~'){
      ...
      return this.text_to_sample(g,ss);
    }
    ...
    ///no fences at all
    return this.text_to_plaintext(style,body);
  }
  uncode(style,line) {
