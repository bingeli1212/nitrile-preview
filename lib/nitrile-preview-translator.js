...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  do_CAPT(block){
    var {id,title,label,style,subtitles,body,bodyrow} = block;
    var text = '';
    var o = [];
    o.push('');
    if(id=='flushleft'){
      text = this.float_to_flushleft(title,label,style,subtitles,body,bodyrow);
    }else if(id=='center'){
      text = this.float_to_center(title,label,style,subtitles,body,bodyrow);
    }else if(id=='equation'){
      ...
      text = this.float_to_wrapfig(title,label,style,subtitles,body,bodyrow);
    }else if(id=='wraptab'){
      text = this.float_to_wraptab(title,label,style,subtitles,body,bodyrow);
    }else if(id=='multicol'){
  ...
  do_PARA(block){
    var {title,label,style,subtitles,body,bodyrow} = block;
    var text = '';
    if(this.body_is_bundle(body)){
    ...
    }else if(this.re_body_is_hyphe.test(body[0])||this.re_body_is_numpe.test(body[0])){
    ...
    }else if(this.re_body_is_2spce.test(body[0])){
      body = body.map((s,i)=>{
        if(i==0) return s.slice(2);
        else     return s;
      });
      text = this.float_to_blockquote(title,label,style,subtitles,body,bodyrow);
      ...
      text = this.float_to_preformatted(title,label,style,subtitles,body,bodyrow);
    }else if(this.re_body_is_lesss.test(body[0])){
      body = this.body_to_body(body,this.re_body_is_lesss,2);
      ...
      text = this.float_to_example(title,label,style,subtitles,body,bodyrow);
    ...
    }else if(this.re_body_is_astrk.test(body[0])){
      body = this.body_to_body(body,this.re_body_is_astrk);
      text = this.float_to_enumeration(title,label,style,subtitles,body,bodyrow);
    }else if(this.re_body_is_excla.test(body[0])){
      body = this.body_to_body(body,this.re_body_is_excla);
      text = this.float_to_linesright(title,label,style,subtitles,body,bodyrow);
    }else if(this.re_body_is_dolla.test(body[0])){
      body = this.body_to_body(body,this.re_body_is_dolla);
      text = this.float_to_linescenter(title,label,style,subtitles,body,bodyrow);
    }else if(this.re_body_is_vtbar.test(body[0])){
      body = this.body_to_body(body,this.re_body_is_vtbar);
      text = this.float_to_linesleft(title,label,style,subtitles,body,bodyrow);
    }else{
  ...
  do_PRIM(block) {
    var {rank,title,style,subtitles,body,bodyrow} = block;
    var label = '';
    style.rank = rank;
  ...
  do_bundle(style,bundle,key=''){
    if(bundle){
      ...
      if(g.restore){
        let name0 = g.id||'';
        let lineno = 1;
        if(style.__parser.buff.hasOwnProperty(name0)){
          let {ss:ss0} = style.__parser.buff[name0];
          if(ss0 && Array.isArray(ss0)){
            let ss1 = ss.slice(0,lineno-1);
            let ss2 = ss0;
            let ss3 = ss.slice(lineno-1);
            ss = ss1.concat(ss2).concat(ss3);
            //g = {...g0,...g};//IMPORTANT, cannot import 'g' because of interferrence
          }
        }
      }
      if(g.save){
        let name0 = g.id||'';
        style.__parser.buff[name0] = {ss,key,g};
      }
      g.row = ssi;
      var itm = {};
      //update from switch
  ...
  do_phrase(key,style,cnt){
    ...
    switch (key) {
      ...
      case 'wrapimgleft': {
        txt = this.phrase_to_wrapimgleft(style,cnt);
        break;
      }
      case 'wrapimgright': {
        txt = this.phrase_to_wrapimgright(style,cnt);
        break;
      }
      //
      case 'calc': {
        txt = this.phrase_to_calc(style,cnt);
        break;
      }
      default: {
  ...
  phrase_to_calc(style,cnt){
    const expr = new NitrilePreviewExpr(null);
    const ss = cnt.split(',').map(s=>s.trim());
    var raw = '';
    if(ss.length==1){
      let [num] = expr.extract_next_expr(ss[0],style);
      raw = num;
    }else if(ss.length>1){
      let [num] = expr.extract_next_expr(ss[1],style);
      raw = this.to_formatted_text(num,ss[0]);
    }
    return raw;
  }
  /////////////////////////////////////////////////////////////////////////////
  ...
  ss_to_plitems(style,ss){
    ...
    ss.forEach((s,i,arr) => {
      ...
      if((v=this.re_body_is_hyphe.exec(s))!==null){
        ...
        plitems.push(item);
      }else if((v=this.re_body_is_numpe.exec(s))!==null){
        item = {};
        item.n = ssi+i;
        item.nblank = 0;
        item.body = [];
