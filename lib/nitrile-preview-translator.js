...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  do_CAPT(block){
    ...
    if(id=='@'){
      ...
      var style = this.update_style_from_switches(style,id);
      var text = this.float_to_columns(title,label,idnum,idcap,style,body,bodyrow);
    }
    return text;
  }
  do_PARA(block){
  ...
  do_bundle(style,ss,ssi){
    ...
    if(s0.startsWith("```") && sn.startsWith("```")){
      ...
      if(v){
        ...
        if(style.seq && style.senum){
          let num = ""+style.seq;
          if(style.senum=='a'){
            num = this.int_to_letter_a(style.seq);
          }else if(style.senum=='A'){
            num = this.int_to_letter_A(style.seq);
          }else if(style.senum=='i'){
            num = this.int_to_letter_i(style.seq);
          }else if(style.senum=='I'){
            num = this.int_to_letter_I(style.seq);
          }
          num = `(${num}) `;
          style.subtitle = num + style.subtitle;
        }
        style.row = ssi;
      }
      ss = ss.slice(1,-1);
    ...
    if(key==='dia'){
      ...
      style = this.update_style_from_switches(style,key);
      o = this.fence_to_math(style,ss,ssi);
    }else if(key=='tabular'){
      style = this.update_style_from_switches(style,key);
  ...
  ss_to_figure_itms(style,ss,ssi){
    ...
    while(i < ss.length) {
      ...
      if(ic==0){
        ...
        ++seq;
        let o = this.do_bundle({...style,seq},d,ssi+i0);
