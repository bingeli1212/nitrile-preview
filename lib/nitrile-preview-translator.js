...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  do_fence(key,style,body){
    ...
    if(key==='img'){
      ...
      return this.fence_to_line(style,body);
    }else if(key=='list'){
  ...
  untext(style,body) {
    ...
    if(s0.trim()==='---' && sZ.trim()==='---'){
      ///surrounded by --- and ---
      ///create an empty style1 with 'switches' and 'vmap'
      var g = this.string_to_style('',style);
      var ss = body.slice(1,body.length-1);
      var ss = this.trim_samp_body(ss);
      return this.para_to_lines(g,ss);
    }
    if(s0.trim()==='~~~' && sZ.trim()==='~~~'){
      ///surrounded by ~~~ and ~~~
      ///create an empty style1 with 'switches' and 'vmap'
      ...
      var ss = body.slice(1,body.length-1);
      var ss = this.trim_samp_body(ss);
      return this.para_to_verbatim(g,ss);
    }
    var v0 = this.re_fence.exec(s0.trimLeft());
    var vZ = this.re_fence.exec(sZ.trimLeft());
    if(v0 && vZ){
      ///this is a fenced block
      var [key,g,ss] = this.extract_fenced_cluster(body,0,style);
      var ss = this.trim_samp_body(ss);
      return this.do_fence(key,g,ss);
    }
    ///no fences at all
    return this.para_to_plaintext(style,body);
  }
  uncode(style,line) {
  ...
  unescape(style,line) {
    ...
    var start_i = 0;
    var newtext = '';
    const re = /(\\[A-Za-z0-9_]+\[[^\[\]]*\])|(\\[A-Za-z0-9_]+\{[^\{\}]*\})|(\\[A-Za-z0-9_]+)|(\\[^A-Za-z0-9_])/gs;
    while ((v = re.exec(line)) !== null) {
      var i = v.index;
      var txt = line.slice(start_i,i);
      var txt = this.flatten(style,txt);
  ...
  phrase_to_displaymath(style,cnt,cnt2,cnt3){
    var cnt2 = `{${cnt2}}`;
    var g = this.string_to_style(cnt2,style);
    var ss = [cnt];
    return this.do_fence('displaymath',g,ss);
  }
  phrase_to_img(style,cnt,cnt2,cnt3){
    var cnt2 = `{${cnt2}}`;
    var g = this.string_to_style(cnt2,style);
    var ss = [cnt];
    return this.do_fence('img',g,ss);
  }
  phrase_to_diagram(style,cnt,cnt2,cnt3){
    var cnt2 = `{${cnt2}}`;
    var g = this.string_to_style(cnt2,style);
    var ss = [cnt];
    return this.do_fence('diagram',g,ss);
  }
  phrase_to_framed(style,cnt,cnt2,cnt3){
    var cnt2 = `{${cnt2}}`;
    var g = this.string_to_style(cnt2,style);
    var ss = [cnt];
    return this.do_fence('framed',g,ss);
  }
  /////////////////////////////////////////////////////////////////////////////
  ///
  ///
