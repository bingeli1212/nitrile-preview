  ...
  untouch (line,style) {
    ...
    while ((v = re.exec(line)) !== null) {
      ...
      if (v[1] !== undefined) {
        ...
        if(key=='qquad' || key=='low' || key=='high' || key=='br' 
        || key=='em' || key=='b' || key=='i' || key=='u' || key=='tt'){
           newtext += this.text_to_typeface(style,key,this.untouch(cnt,style));
        }
        else if(key=='math' || key=='sqrt' || key=='overline' 
                            || key=='overleftrightarrow' || key=='overrightarrow' 
                            || key=='underleftrightarrow' || key=='underrightarrow'){
          newtext += this.text_to_typeface(style,key,this.untouch(cnt,style));
        }
        else if(key=='FRAC' || key=='frac' || key=='BINOM' || key=='binom' || key=='root' 
        || key=='sub' || key=='sup'){
          ///2-arguments
          ...
          newtext += this.text_to_typeface(style,key,this.untouch(cnt,style),this.untouch(cnt2,style));
        }
        else if(key=='int' || key=='sum' || key=='subsup'){
          ///2nd-argument
          var next_i = this.extract2(line,start_i);
          if(next_i>start_i){
            var cnt2 = line.slice(start_i+1,next_i-1);
          ...
          newtext += this.text_to_typeface(style,key,this.untouch(cnt,style),this.untouch(cnt2,style),this.untouch(cnt3,style));
        }
        else {
