...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  do_CAPT(block){
    ...
    if(id=='equation'){
      ...
      text = this.float_to_equation(title,label,style,body,bodyrow);
    }else if(id=='listing'){
      style = this.update_style_from_switches(style,id);
      text = this.float_to_listing(title,label,style,body,bodyrow)
    }else if(id=='figure'){
      style = this.update_style_from_switches(style,id);
      text = this.float_to_figure(title,label,style,body,bodyrow);
    }else if(id=='table'){
      style = this.update_style_from_switches(style,id);
      text = this.float_to_table(title,label,style,body,bodyrow);
    }else if(id=='columns'){
      style = this.update_style_from_switches(style,id);
      text = this.float_to_columns(title,label,style,body,bodyrow);
    }else if(id=='lines'){
      style = this.update_style_from_switches(style,id);
      text = this.float_to_lines(title,label,style,body,bodyrow);
    }else if(id=='itemize'){
      style = this.update_style_from_switches(style,id);
      text = this.float_to_itemize(title,label,style,body,bodyrow);
    }else if(id=='page'){
      style = this.update_style_from_switches(style,id);
      text = this.float_to_page(title,label,style,body,bodyrow);
    }else if(id=='vspace'){
      style = this.update_style_from_switches(style,id);
      text = this.float_to_vspace(title,label,style,body,bodyrow);
    }
    return text;
  }
  do_PARA(block){
    var {title,label,body,bodyrow,style} = block;
    var text = '';
    if(style.isbundle){
