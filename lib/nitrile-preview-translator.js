...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  s_to_phrase(name,s,style,switches){
    ...
    if(name==='link'){
      ...
      let ss = this.fetch_ss_from_ss_map(s);
      newtext += this.ss_to_fence(name,ss,style,switches);
    }
    return newtext;
  }
  ss_to_fence(name,ss,style,switches){
    ...
    var newtext = '';
    if(name==='img'){
      newtext += this.fence_to_img(ss,style,switches);
    }else if(name==='diagram'){
      var style = this.update_style_from_switches(style,name,switches);
      newtext += this.fence_to_diagram(ss,style,switches);
    }else if(name==='animation'){
      var style = this.update_style_from_switches(style,name,switches);
      newtext += this.fence_to_animation(ss,style,switches);
    }else if(name==='framed'){
      var style = this.update_style_from_switches(style,name,switches);
      newtext += this.fence_to_framed(ss,style,switches);
    }else if(name==='math'){
      var style = this.update_style_from_switches(style,name,switches);
      newtext += this.fence_to_math(ss,style,switches);
    }else if(name=='tabularx'){
      var style = this.update_style_from_switches(style,name,switches);
      newtext += this.fence_to_tabularx(ss,style,switches);
    }else if(name=='tabular'){
      var style = this.update_style_from_switches(style,name,switches);
      newtext += this.fence_to_tabular(ss,style,switches);
    }else if(name=='tabbing'){
      var style = this.update_style_from_switches(style,name,switches);
      newtext += this.fence_to_tabbing(ss,style,switches);
    }else if(name=='blockquote'){
      var style = this.update_style_from_switches(style,name,switches);
      newtext += this.fence_to_blockquote(ss,style,switches);
    }else if(name=='verbatim'){
      var style = this.update_style_from_switches(style,name,switches);
      newtext += this.fence_to_verbatim(ss,style,switches);
    }else{
      var style = this.update_style_from_switches(style,'list',switches);
      newtext += this.fence_to_list(ss,style,switches);
    }
    return newtext;
  }
  ...
  unphrase(line,style,switches){
    ...
    while ((v = re.exec(line)) !== null) {
      ...
      if (v[1] !== undefined && v[2] !== undefined) {
        ...
        var s = v[3]
        var txt = this.phrase_to_inlinemath(s,style,switches);
        newtext += txt;

      } else if (v[4] !== undefined) {
        var s = v[4]
        var txt = this.phrase_to_displaymath(s,style,switches);
        newtext += txt;
