...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  do_PARA(block){
    ...
    if(this.paragraph_style){
      style = {...style,...this.paragraph_style};
      this.paragraph_style = null;
    }
    var title = '';
    var label = '';
    ...
    if(isbundle){
      let bundle = this.try_extract_bundle(style,body,0);
  ...
  is_step(ss){
    return ss.length && re_step.test(ss[0]);
  }
  is_plst(ss){
