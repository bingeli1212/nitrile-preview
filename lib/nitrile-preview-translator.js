...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  do_CAPT(block){
    var {id,title,label,bundles,style} = block;
    var text = '';
    var o = [];
    o.push('');
    if(id=='@'){
  ...
  do_PARA(block){
    var {body,bodyrow,bundle,style} = block;
    var text = '';
    if(style.isbundle){
  ...
  build_default_pagenum_map(){
    ...
    for (let block of this.parser.blocks) {
      if(block.style.name=='part' || block.style.name=='chapter'){
        ...
        let subbundles = this.split_bundle(block);
        block.bundles = subbundles;///change the original block bundles list  
        block.style.splitpage = 1;       
        subbundles.forEach((bundle,j,arr) => {
          if(j==0){
            let block1 = block;//use the original block and style
            block1.bundle = bundle;//set the block.bundle member, which only points to the active bundle
  ...
  split_bundle(block){    
    ...
    while(total_n > 0){
      ...
      let bundle = this.try_extract_bundle(block2.style,ss2,0);      
      bundle.splitid = splitid;
      bundle.splitstart = splitstart;
      block2.bundles = [];
      block2.bundles.push(bundle); 
      o.push(bundle);
    }
    return o;
  }
