...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  do_PLST(block){
    ...
    if(!nblank && plitems.length > 1){
      var body = this.trim_samp_body(para);
      var title = '';
      var label = '';
      return this.float_to_list(title,label,style,body);
    }else{
  ...
  do_PARA(block){
    ...
    var title = '';
    var label = '';
    var text = this.float_to_paragraph(title,label,style,body);
    return text;
  }
  rubify (src,vmap) {
    vmap = vmap||[]
