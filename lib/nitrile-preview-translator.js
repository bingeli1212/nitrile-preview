...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  constructor(parser) {
    ...
    this.re_dhline = /^={1,}$/;
    this.re_style = /^\{(.*)\}$/;
    this.re_dl_term  = /^([\w\-\(\)\.]+)(\s+-\s+|:\s+)(.*)$/;//it has to start with a word otherwise it could grab a hyphen minus from a math expression
    this.re_ul_key   = /^([\w\-\(\)\.]+)(\s+-\s+|:\s+)(.*)$/;//same as above
    ...
    for (var plitem of plitems) {
      ...
      if (bullet) {
        ...
        if (bullet[0] == '-'){
          ...
          value = '';
          let v0;
          if(plitem.body.length && (v0=this.re_ul_key.exec(plitem.body[0]))!==null){
            key = v0[1];
            sep = v0[2];
