...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  untext(ss,style) {
    ...
    let s0 = ss[0];
    let sz = ss[n-1];
    const re_cluster_fence = /^(~{3,})(.*)$/;
    if(n>1 && (v=re_cluster_fence.exec(s0))!==null && (w=re_cluster_fence.exec(sz))!==null){ 
      let {id,style:style1} = this.string_to_id_and_style(v[2].trim());
      style = {...style, ...style1};
      ss = ss.slice(1,n-1);
      ss = this.trim_samp_body(ss);
      let para_id = id;
  ...
  mode_to_phrase(fence_id,mode,fence_style){
    var newtext = '';
    if(fence_id==='img'){
      newtext += this.phrase_to_img(mode,fence_style);
    }else if(fence_id==='link'){
      newtext += this.phrase_to_link(mode,fence_style);
    }else if(fence_id==='ref'){
      newtext += this.phrase_to_ref(mode,fence_style);
    }else if(fence_id==='diagram'){
      newtext += this.phrase_to_diagram(mode,fence_style);
    }else if(fence_id==='animation'){
      newtext += this.phrase_to_animation(mode,fence_style);
    }else if(fence_id==='framed'){
      newtext += this.phrase_to_framed(mode,fence_style);
    }else if(fence_id==='math'){
      newtext += this.phrase_to_math(mode,fence_style);
    }else if(fence_id=='tabular'){
      newtext += this.phrase_to_tabular(mode,fence_style);
    }else if(fence_id=='tabbing'){
      newtext += this.phrase_to_tabbing(mode,fence_style);
    }else if(fence_id=='blockquote'){
      newtext += this.phrase_to_blockquote(mode,fence_style);
    }else{
      newtext += this.phrase_to_verbatim(mode,fence_style);
    }
    return newtext;
  }
  unmask(line,style) {
    ...
    while((v=re.exec(line))!==null){
      ...
      newtext += txt;
      var fence_id = v[1];
      var ss = v[2].trimRight().split('\n')
      var name = ss[0]||'';
      var body = ss.slice(1);
      var mode = {body};
      var [fence_style] = this.string_to_style(name,style);
      var txt = this.mode_to_phrase(fence_id,mode,fence_style);
      newtext += txt;
      /// move on the next
  ...
  untouch (line,style) {
    ...
    var newtext = '';
    var k = 0;
    const re = /(?<!\\)(&[A-Za-z]+\{)|(?<!\\)(\\\()|(?<!\\)(\\\[)/g; //cannot be global because it is used recursively
    while ((v = re.exec(line)) !== null) {
      var i = v.index;
      var txt = line.slice(start_i,i);
      ...
      else if (v[2] !== undefined && ((k=line.indexOf('\\)',i+2)) >= i)){
        // need to find a closing fence for \( ... \)
        i += 2;
        start_i = k;
        var scan_text = line.slice(i,start_i);
        var scan_text = this.phrase_to_inlinemath(scan_text,style);
        newtext += scan_text;
        start_i += 2;
        re.lastIndex = start_i;
        continue;
      ...
      else if (v[3] !== undefined && ((k=line.indexOf('\\]',i+2)) >= i)) {
      
        i += 2;
        start_i = k;
        var scan_text = line.slice(i,start_i);
        var scan_text = this.phrase_to_displaymath(scan_text,style);
        newtext += scan_text;
        start_i += 2;
        re.lastIndex = start_i;
        continue;
  ...
  unphrase(line,style){
    ...
    while ((v = re.exec(line)) !== null) {
      ...
      if (v[1] !== undefined && v[2] !== undefined) {
        var fence_id = v[1];
        var s = v[2];
        var mode = this.string_to_mode(s);
        var txt = this.mode_to_phrase(fence_id,mode,style);
        newtext += txt;
      }else if(v[3] !== undefined && v[4] !== undefined){
        // this part of the code will never happen because the triple-fence has already been processed prior to this function being called!
        var fence_id = v[3];
        var ss = v[4].split('\n');
        var body = ss.slice(1);
        var mode = {body};
  ...
  ss_to_table_rows(ss,style){
    ...
    if(re_rows.test(s0)){
      ...
      ss.forEach(s => {
        ...
        if(this.line_is_indented(s)){
          ...
          // insert a new with all elements filled this element
          s = s.trim();
          if(re_hline_or_hhline.test(s)){
            let pp = 'x'.repeat(1).split('').map(x => s);
            pp = pp.map((raw) => {
              return {raw};
            });
            data.push(pp);
    ...
    else if(this.split_line_by_verticalbar(s0).length>1){
    ...
    else if(this.split_line_by_doublespace(s0).length>1){
      ss = ss.filter((s) => s.trim().length);
      ///else we let the line split by itself
      ss.forEach((s) => {
        let pp = this.split_line_by_doublespace(s);
        pp = pp.map(p => p.trim());
        pp = pp.map((raw) => {
          return {raw};
        });
      ...
      // we will consider each line a table cell, 
      // everything will be the first row unless
      // a blank line or double-backslash 
      // is encountered, or when style.n is set
      var cols = [];
      cols.push([]);
      for(let s of ss){
        if(s.length==0){
  ...
  ss_to_figure_itms(ss,style){
    ...
    while((v=re.exec(line))!==null){
      ...
      if(v[1] !== undefined && v[2] !== undefined){
        let type = v[1];
        let s = v[2];
        let mode = this.string_to_mode(s);
        let sub = '';
        let g = style;
        ...
        let type = v[3];
        let ss = v[4].split('\n');
        let [g] = this.string_to_style(ss[0]);
        let body = ss.slice(1);
        let sub = '';
        let mode = {body};
        d.push({type,mode,g,sub});
  ...
  do_NOTE(block) {
    ...
    ///add to this.notes
    this.notes.set(title,{body,style});
    ///special processing for vocabulary
    if(title=='__vocabulary__'){
      this.vmap = [];
  ...
  do_FLOA(block){
    ...
    if(title=='equation'){
      this.do_FLOA_equation(block,caption,ss,style)
    }else if(title=='listing'){
      this.do_FLOA_listing(block,caption,ss,style)
    }else if(title=='figure'){
      this.do_FLOA_figure(block,caption,ss,style);
    }else if(title=='table'){
      this.do_FLOA_table(block,caption,ss,style);
    }else if(title=='longtable'){
      this.do_FLOA_longtable(block,caption,ss,style);
    }
  }
  fetch_from_notes(title){
  ...
  phrase_to_animation(mode,style){
    var {body} = mode;
    if(!body) return '';
    ...
    for(let j=0; j<steps; ++j){
      ...
      let mode = {body:ss1};
      var s = this.phrase_to_diagram(mode,style);
      d.push(s);
    }
    var text = d.join('\n')
    return text;
  ...
  fetch_ss_from_mode_with_style(mode,style){
  ...
  fetch_fname_from_mode_with_style(mode,style){
    var {fname,name} = mode;
    [style] = this.string_to_style(name,style);
    return {fname,style}
  }
