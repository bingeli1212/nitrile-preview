...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  do_FRNT(block){
    var {hdgn,name,title,label,idnum,idcap,level,partnum,chapternum,pagenum,style} = block;
    if(block.name=='chapter'){
      var text = this.hdgs_to_chapter(title,label,idnum,idcap,partnum,chapternum,pagenum,level,style);
    }else if(block.name=='part'){
      var text = this.hdgs_to_part(title,label,idnum,idcap,partnum,chapternum,pagenum,level,style);
    }else{
      var text = '';
    }
    return text;
  ...
  do_HDGS(block){
    var {hdgn,name,title,label,idnum,idcap,level,partnum,chapternum,pagenum,style} = block;
    if(hdgn==1){
      var text = this.hdgs_to_section(title,label,idnum,idcap,partnum,chapternum,pagenum,level,style);
    }
    else if(hdgn==2){
      var text = this.hdgs_to_subsection(title,label,idnum,idcap,partnum,chapternum,pagenum,level,style);
    }
    else{
      var text = this.hdgs_to_subsubsection(title,label,idnum,idcap,partnum,chapternum,pagenum,level,style);
    }
    return text;
  }
  do_CAPT(block){
    var {id,title,label,idnum,idcap,bodyrow,body,style} = block;
    ...
    if(id=='@'){
      ...
      var text = this.float_to_equation(title,label,idnum,idcap,style,body,bodyrow);
    }else if(id=='listing'){
      var style = this.update_style_from_switches(style,id);
      var text = this.float_to_listing(title,label,idnum,idcap,style,body,bodyrow)
    }else if(id=='figure'){
      var style = this.update_style_from_switches(style,id);
      var text = this.float_to_figure(title,label,idnum,idcap,style,body,bodyrow);
    }else if(id=='longtabu'){
      var style = this.update_style_from_switches(style,id);
      var text = this.float_to_longtabu(title,label,idnum,idcap,style,body,bodyrow);
    }else if(id=='column'){
      var style = this.update_style_from_switches(style,id);
      var text = this.float_to_column(title,label,idnum,idcap,style,body,bodyrow);
    }else if(id=='page'){
      var style = this.update_style_from_switches(style,id);
      var text = this.float_to_page(title,label,idnum,idcap,style,body,bodyrow);
    }
    return text;
  }
  do_PARA(block){
  ...
  do_bundle(style,ss,ssi){
    ...
    if(key==='dia'){
      ...
      style = this.update_style_from_switches(style,key);
      o = this.fence_to_formula(style,ss,ssi);
    }else if(key=='tabular'){
      style = this.update_style_from_switches(style,key);
  ...
  to_capt_idnum(name,block){
    ...
    if(block){
      ...
      pp = this.chapter_map[chapternum];
      if(pp){
      }else{
        pp = {};
        pp.equation_num = 0;
        ...
        pp.table_num = 0;
        pp.listing_num = 0;
        this.chapter_map[chapternum] = pp;
      }
      if(name){
  ...
  build_default_pagenum_map(){
    ...
    o.push(pp);
    for (let block of this.parser.blocks) {
      if(block.sig=='FRNT' && block.name==''){
        //ignore this block
      }else if(block.sig=='FRNT' && (block.name=='chapter' || block.name=='part')){
        pp = [];
        pp.push({block});
        o.push(pp);
      }else if(block.id=='page'){
        pp = [];
        pp.push({block});
        o.push(pp);
      }else{
  ...
  is_cave(ss){
    return ss.length && is_cave.test(ss[0]);
  }
  is_plst(ss){
