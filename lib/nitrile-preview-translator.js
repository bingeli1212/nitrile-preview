...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  s_to_phrase(name,s,g){
    ...
    if(name==='link'){
      ...
      var {style:g,s} = this.string_to_style(s,g);
      if(s.startsWith('#')){
  ...
  untouch (line,style) {
    ...
    while ((v = re.exec(line)) !== null) {
      ...
      if (v[1] !== undefined) {
        ...
        if(key=='sub'){
          newtext += this.text_to_typeface(this.untouch(cnt,style),'sub');
        }
        else if(key=='sup'){
          newtext += this.text_to_typeface(this.untouch(cnt,style),'sup');
        }
        else if(key=='frac'){
          newtext += this.text_to_typeface(this.untouch(cnt,style),'frac');
        }
        else if(key=='br'){
