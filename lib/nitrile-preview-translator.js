...
const re_ss_bull = /^(-|\+|\*|>|\d+[\)\.]|[a-zA-Z][\)\.]|\[[\w\s]\])\s+(.*)$/u;
...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  rubify (src) {
    ...
    while (j < src.length) {
      i0 = src.length;
      found = 0;
      if(this.block && this.block.vmap){
        for (var rubyitem of this.block.vmap) {
          var {rb,rt} = rubyitem;
  ...
  untext(ss,style) {
    ...
    if(style.nspace){
      return this.para_to_samp(ss,style);
    }
    return this.para_to_plaintext(ss,style);    
  }
  unmask(line,style) {
    ...
    var fence_style = {};
    var nonfence_ss = [];
    body.forEach((line,i,arr) => {
      if((v=re_fence.exec(line))!==null){
        if(fence_flag==1){
          // terminate previous fence
          fence_ss = this.trim_samp_body(fence_ss);
          let body = fence_ss;
          let mode = {body};
          if(fence_id==='diagram'){
            newtext += this.phrase_to_diagram(mode,fence_style);
          }else if(fence_id==='animate'){
            newtext += this.phrase_to_animate(mode,fence_style);
          ...
          }else if(fence_id=='vbox'){
            newtext += this.phrase_to_vbox(mode,fence_style);
          }else{
            newtext += this.phrase_to_verb(mode,fence_style);
          }
          fence_flag = 0;
          newtext += '\n';// always adds a new line    
          fence_ss = [];      
        }else{
          // start a new fence ss
          let {id, style:style1} = this.string_to_id_and_style(v[1]);
          fence_id = id;
          fence_style = {...style, ...style1};
          fence_flag = 1;
          fence_ss = [];
          // flush out the prevous non-fence lines
          var txt = nonfence_ss.join('\n');
          var txt = this.uncode(txt,style);
          newtext += txt;
          nonfence_ss = [];
        }
      }else{
        // stash away the nonfence lines
        if(fence_flag){
          fence_ss.push(line);
        }else{
          nonfence_ss.push(line);
        }
      }
    });
    var txt = nonfence_ss.join('\n')
  ...
  untouch (line,style) {
    ...
    while ((v = re_untouch.exec(line)) !== null) {
      ...
      if (v[1] !== undefined) {
        ...
        if(key=='uri'){
          let mode = this.string_to_mode(cnt);
          newtext += this.phrase_to_uri(mode,style);
        }
        else if(key=='ref'){
          let mode = this.string_to_mode(cnt);
          newtext += this.phrase_to_ref(mode,style);
        }
        else if(key=='framed'){
          let mode = this.string_to_mode(cnt);
          newtext += this.phrase_to_framed(mode,style);
        }
        else if(key=='diagram'){
          let mode = this.string_to_mode(cnt);
          newtext += this.phrase_to_diagram(mode,style);
        }
        else if(key=='img'){
          let mode = this.string_to_mode(cnt);
          newtext += this.phrase_to_img(mode,style);
        }
        else if(key=='em'){
  ...
  ss_to_figure_itms(ss,style){
    ...
    //
    var re = /&img(\{.*?\})|&diagram(\{.*?\})|&framed(\{.*?\})|```diagram\b(.*?)```|```framed\b(.*?)```/gs;
    //var re = /```diagram\b/g;
    var line = ss.join('\n')
    let d = [];
    var v;
    ...
    while((v=re.exec(line))!==null){
      ...
      if(v[1] !== undefined){
        ...
        let s = v[1];
        s = s.slice(1,-1);
        let {caption,fname,id} = this.string_to_mode(s);
        let g = this.string_to_style(caption,style);
        let sub = '';
        let mode = {fname,id};
  ...
  do_NOTE(block) {
    ...
    block.notes.set(title,{body,style});
    ///special processing for vocabulary
    if(this.block.title=='__vocabulary__'){
      this.addto_vmap(body,this.block.vmap);
    }
  }
  fetch_from_notes(title){
    if(this.block.notes.has(title)) {
      return this.block.notes.get(title);
    }
    return null;
  }
