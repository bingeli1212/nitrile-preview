...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  ss_to_bull_itms(ss){
    ...
    if((v=re_ss_bull.exec(ss[0]))!==null){
      if(re_n.test(v[1]))      { type = '0'; bull0=v[1].slice(0,-1); ending0=v[1].substr(-1); }
      else if(re_a.test(v[1])) { type = 'a'; bull0=v[1].slice(0,-1); ending0=v[1].substr(-1); }
      else if(re_A.test(v[1])) { type = 'A'; bull0=v[1].slice(0,-1); ending0=v[1].substr(-1); }
      else if(v[1]=='+') type = 'DL';
      else if(v[1]=='>') type = 'HD';
      else if(v[1]=='-') type = 'UL';
    ...
    ss.forEach(s => {
      if((v=re.exec(s))!==null){
        ...
        let text = v[2];
        let value = '';
        if(type=='0' && re_n.test(v[1]) && ending==ending0){
          value = parseInt(bull);
          itms.push({bull,text,value,ending,type});
          return;
        }else if(type=='a' && re_a.test(v[1]) && ending==ending0){
