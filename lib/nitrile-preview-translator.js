...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  untext(ss,style) {
    ...
    while((v=re.exec(line))!==null){
      ...
      var g = {};
      g.switches = style.switches;
      var {id,style:g} = this.string_to_id_and_style(s0,g);
  ...
  untouch (line,style) {
    ...
    while ((v = re.exec(line)) !== null) {
      ...
      if (v[1] !== undefined) {
        ...
        if(key=='br'){
          newtext += this.text_to_typeface(this.untouch(cnt,style),'br');
        }
        else if(key=='em'){
  ...
  vrule_to_vbars(n,vrule){
    ...
    var re = /^\|{0,2}$/;
    var vbars = vrule.split('+');
    var vbars =  vbars.filter(x => re.test(x));
    while (vbars.length < n+1){
      vbars.push('');
    }
    return vbars;
  }
