...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  untext(ss,style) {
    ...
    if(n>1&&ss[0].trim().startsWith('~~~')&&ss[n-1].trim().startsWith('~~~')){ 
      ...
      if(fenceid==='equation'){
        style2.fenceid = 'equation';
        return this.para_to_equation(ss,style2);
      }
      if(fenceid==='table'){
        style2.fenceid = 'table';
        return this.para_to_table(ss,style2);
      }
      if(fenceid==='diagram'){
        style2.fenceid = 'diagram';
        return this.para_to_diagram(ss,style2);
      }
      if(fenceid==='figure'){
        style2.fenceid = 'figure';
        return this.para_to_figure(ss,style2);
      }
      if(fenceid==='listing'){
        style2.fenceid = 'listing';
        return this.para_to_listing(ss,style2);
      }
      if(fenceid=='sides'){
        style2.fenceid = 'sides';
        return this.para_to_multi(ss,style2);
      }
      if(fenceid=='quote'){
        style2.fenceid = 'quote';
        return this.para_to_blockquote(ss,style2);
      }
      ///if a ~~~ block then default to 'lines'
      style2.fenceid = 'lines';
      return this.para_to_lines(ss,style2);
    } 
    /// trim samp body
    if(1){
      let style2 = {...style};
      ...
      if(this.ss_is_verse(ss)){
        ...
        ///> blah blah blah
        ///  blah blah
        style2.fenceid = 'verse';
        return this.para_to_verse(ss,style2);
      }
      if(this.ss_is_bull(ss)){
        ///-- This is great
        ///-- this is great
        style2.fenceid = 'bull';
        return this.para_to_bull(ss,style2);
      }
  ...
  unmask(line,style) {
    ...
    while ((i=line.indexOf(fence3,start_i)) >= 0) {
      ...
      let {fenceid,style:style1} = this.string_to_fenceid(s0);
      let style2 = {...style,...style1};
      ss = this.trim_samp_body(ss);
      if(fenceid==='diagram'){
  ...
  ss_to_lines_itms(ss,style){
    let itms = [];
    ss = this.join_indented_lines(ss);
    ss.forEach(s => {
      itms.push({text:s});
    })
    return itms;
  ...
  ss_to_table_rows(ss,style){
    ...
    if(re_rows.test(ss[0])){
      ...
      ss.forEach(s => {
        ...
        else if(s.match(this.re_dhline)){
          let pp = 'x'.repeat(n).split('').map(x => '===');
          pp = pp.map((raw) => {
            return {raw};
          });
    ...
    let max_n = data.reduce((acc,pp) => Math.max(acc,pp.length),0);
    data = data.map((pp) => {
  ...
  plitems_to_items (plitems) {
    ...
    for (var plitem of plitems) {
      ...
      if (bullet) {
        ...
        if (bullet[0] == '-'){
          ...
          if(lead==''){
            ...
            if(keys){
              br = (s.length==0);
              if(s.length || plitem.body.length > 1){
                ///this is to ensure 'text' is set to empty when there is no follow-on text, import because the translator engine reply on it
                text = s;
                text += '\n'
          ...
          if(Number.isFinite(num)){
            type = '1';
            value = `${num}`;
          }else if(bullet[0].charCodeAt(0) >= 65 && bullet[0].charCodeAt(0) <= 90) {
            type = 'A';
            value = bullet[0].charCodeAt(0) - 65 + 1
          }else if(bullet[0].charCodeAt(0) >= 97 && bullet[0].charCodeAt(0) <= 122) {
            type = 'a';
            value = bullet[0].charCodeAt(0) - 97 + 1
          }else{
            type = '1';
            value = '';
          }
      ...
      if (action === 'push') {
        ...
        items.push({bull:'LI',bullet,type,ending,value,br,text,keys,row1,row2,more});
      } else {
        if (action === 'pop') {
