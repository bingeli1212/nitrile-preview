    ...
    body.forEach((line,i,arr) => {
      if((v=re_fence.exec(line))!==null){
        if(fence_flag==1){
          ...
          if(fence_id==='diagram'){
            newtext += this.fence_to_diagram(fence_ss,fence_style);
          }else if(fence_id==='animate'){
            newtext += this.fence_to_animate(fence_ss,fence_style);
          }else if(fence_id==='framed'){
            newtext += this.fence_to_framed(fence_ss,fence_style);
          }else if(fence_id==='math'){
            newtext += this.fence_to_math(fence_ss,fence_style);
          }else if(fence_id=='tabulate'||fence_id=='table'){
            newtext += this.fence_to_table(fence_ss,fence_style);
          }else if(fence_id=='vbox'){
            newtext += this.fence_to_vbox(fence_ss,fence_style);
          }else{
            newtext += this.fence_to_verb(fence_ss,fence_style);
          }
          fence_flag = 0;
    ...
    while ((v = re_untouch.exec(line)) !== null) {
      ...
      if (v[1] !== undefined) {
        ...
        if(key=='ruby'){
          ...
          newtext += this.phrase_to_ref(mode);
        }
        else if(key=='framed'){
          let {caption,fname,id} = this.string_to_mode(cnt);
          let g = this.string_to_style(caption);
          newtext += this.phrase_to_framed(fname,id,g);
        }
        else if(key=='diagram'){
          let {caption,fname,id} = this.string_to_mode(cnt);
          let g = this.string_to_style(caption);
          newtext += this.phrase_to_diagram(fname,id,g);
        }
        else if(key=='img'){
          let {caption,fname,id} = this.string_to_mode(cnt);
          let g = this.string_to_style(caption);
          newtext += this.phrase_to_img(fname,id,g);
        }
        else if(key=='em'){
  ...
  ss_to_figure_itms(ss,style){
    ...
    while((v=re.exec(line))!==null){
      ...
      if(v[1] !== undefined){
        ...
        let body = null;
        let sub = '';
        d.push({type,fname,id,body,g,sub});
      }
