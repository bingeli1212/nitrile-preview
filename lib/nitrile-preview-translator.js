...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  do_PARA(block){
    ...
    if(this.body_is_bundle(body)){
      ...
      text = this.float_to_itemize(title,label,style,splitid,body,bodyrow);
    }else if(this.body_is_verbatim(body)){
      style = this.update_style_from_switches(style,'verbatim');
      body = this.trim_para_verbatim(body,4);
  ...
  do_bundle(style,bundle,key=''){
    ...
    if(g.restore){
      ...
      if(g['$'].buffers.hasOwnProperty(name0)){
        let {ss:ss0,g:g0} = g['$'].buffers[name0];
    ...
    ss.forEach((s,i) => {
      if(s.startsWith('& ')){
        ...
        if(this.re_split_doublebackslash.test(s)){
          var ss = s.slice(2).trim().split(this.re_split_doublebackslash);
        }else{
          var ss = s.slice(2).trim().split(this.re_split_triplespace);
        }
        let pp = ss.map((raw) => {
          let row = ssi+i;
          return {raw,row};
        ...
        if(this.re_split_doublebackslash.test(s)){
          ...
          var ss = s.slice(2).trim().split(this.re_split_triplespace).map(x=>x.trim());
        }
        if(ncols==1){
          startm = rows.length;
          ss.forEach((s,j) => {
        ...
        if(ncols <= max_n){
          ss.forEach((s,j) => {
            m = startm+j;//declared above
            if(m<rows.length){
              rows[m][ncols-1].raw = s;
              rows[m][ncols-1].row = ssi+i;
            }
          });
        }
      }else if(s.startsWith('  ')){
