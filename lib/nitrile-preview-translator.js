...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  constructor(parser) {
    ...
    for(let s of fonts){
      ...
      let program = '';
      ss.forEach((s) => {
        let v1 = cjkfontnames.indexOf(s);
        if(v1>=0){
          fid = s;
        }else if(s.startsWith('0x')){
          start = parseInt(s);
        }else if(s.startsWith('"')&&s.endsWith('"')){
          fontname = s.slice(1,-1);
        }else{
          program = s;
        }
      });
      let v = cjkfontnames.indexOf(fid);
      if(v>=0 && v<4){
        ///'jp','cn','tw','kr'
        this.add_fid(fid,fontname,program);
      }else if(v>=0){
  ...
  uncode(style,line) {
    ...
    const re = /(`{1,2}.*?`{1,2})|(?<!\w)\*\*([\w\-\.\,:\(\)\[\]\/]+)\*\*(?!\w)|(?<!\w)\*([\w\-\.\,:\(\)\[\]\/]+)\*(?!\w)|\\\((.*?)\\\)|\\\[(.*?)\\\]|(?<!\\)(&[A-Za-z]+\{[\w\-\.\,:\(\)\[\]\/]*\})|(?<!\\)(\{\{.*?\}\})/gs; //cannot be global because it is used recursively
