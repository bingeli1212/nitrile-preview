...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  ss_to_fence(name,ss,g){
    ...
    if(name==='img'){
      ...
      newtext += this.fence_to_combination(ss,g);
    }else if(name=='longtabu'){
      newtext += this.fence_to_longtabu(ss,g);
    }else{
  ...
  unphrase(line,style){
    ...
    while ((v = re.exec(line)) !== null) {
      ...
      if (v[1] !== undefined && v[2] !== undefined) {
        ...
        var s = v[5]
        var txt = this.phrase_to_uncode(s,style);
        newtext += txt;
        
      } else if (v[6] !== undefined) {
        var s = v[6]
        var txt = this.phrase_to_unquote(s,style);
        newtext += txt;
