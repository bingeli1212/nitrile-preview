...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  do_fence(key,style,body){
    ...
    if(key==='img'){
      ...
      style = this.update_style_from_switches(style,key);
      o = this.fence_to_list(style,body);
    ...
    }else if(key=='longtabu'){
      style = this.update_style_from_switches(style,key);
      o = this.fence_to_longtabu(style,body);
    }else{
      key = 'verbatim';
      style = this.update_style_from_switches(style,key);
      o = this.fence_to_verbatim(style,body);
  ...
  untext(style,body) {
    if(this.body_is_list(body)){
      let key = 'list';
      style = this.update_style_from_switches(style,key);
      var text = this.body_to_list(style,body);
      return text;
    }else{
      var all = [];
      var text = '';
      all.push(text);
      while(body.length) {
        let [key,g,d] = this.try_extract_bundle(body);
        if(key===null){
          let s = body.shift();
  ...
  ss_to_tabular_rows(ss,style){
    ...
    if(flag==0){
      let re_row = /^\\row\b\s*(.*)$/;
      let re_data = /^\\data\b\s*(.*)$/;
      let re_col = /^\\col\b\s*(.*)$/; 
      let re_hline = /^\\hline\b/;
      let re_hhline = /^\\hhline\b/;
      let pp = [];
      ss.forEach(s => {
        let v;
  ...
  try_extract_bundle(ss){
    ...
    var key = v[1];
    var g = this.string_to_style(v[2],this.parser.style);//extract_bundle()
    var d = [];
    ss.shift();
    while(ss.length){
      let s = ss.shift();
