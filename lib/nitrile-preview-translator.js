...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  get_refmap_value(style, key) {
    ...
    if (this.refmap && this.refmap.has(myid)) {
      let g = this.refmap.get(myid);
      if (typeof g == 'object') {
        let val = g[key];
        if (val) return val;
        else return '';
  ...
  do_identify(block, A) {
    ...
    var { sig, hdgn, floatname, title, style } = block;
    ...
    if (sig == 'PART') {
      A.parts++;
      let partnum = A.parts;
      this.refmap.set(myid, { sig, id, floatname, title, partnum });
    } else if (sig == 'HDGS') {
      let idnum;
      if (hdgn == 0) {
        A.chapters++;
      ...
      let partnum = A.parts;
      this.refmap.set(myid, { sig, id, floatname, title, idnum, chnum, partnum });
    } else if (floatname) {
