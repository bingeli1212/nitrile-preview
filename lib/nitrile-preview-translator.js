...
class NitrilePreviewTranslator extends NitrilePreviewBase {
    ...
    for(var s of body) {
      if(this.re_begin_end_fence.test(s.trimLeft())){
        if(!is_in_fence){
          ...
          let [key,g,ss1] = this.extract_bundle(ss,0);
  ...
  ///
  ////////////////////////////////////////////////////////////////////
  ss_to_img_srcs_and_subtitle(body,style){
    var srcs = [];
    var subtitle = '';
  ...
  ss_to_figure_itms(body,style){
    ...
    var sub = '';
    var seq = 0;
    var maxn = this.assert_int(style.n,0);
    var n = 0;
    const re_sub = /^&\s+(.*)$/;
    for(var s of body) {
      if(this.re_begin_end_fence.test(s.trimLeft())){
  ...
  extract_bundle(ss,i){
    ...
    var v = re.exec(s0);
    var key = '';
    var g = {...this.parser.style};
    var ss = [];
    if(v){
      key = v[1];
      g = this.string_to_style(v[2],g);//extract_bundle()
    }
    i++;
    while(i < ss.length){
      let s = ss[i];  
