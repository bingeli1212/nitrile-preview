...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  do_PARA(block){
    ...
    var text = '';
    if(style.isbundle){
      text = this.float_to_bundle(title,label,style,body,bodyrow);
    }else if(style.issamp){
      style = this.update_style_from_switches(style,'sample');
      body = this.to_sample_body(body);
      text = this.float_to_sample(title,label,style,body,bodyrow);
    }else if(style.iscove){
      style = this.update_style_from_switches(style,'example');
      body = this.to_example_body(body);
      text = this.float_to_example(title,label,style,body,bodyrow);
      ...
      style = this.update_style_from_switches(style,'record');
      text = this.float_to_record(title,label,style,body,bodyrow);  
    }else if(style.isplst){
      style = this.update_style_from_switches(style,'itemize');
      text = this.float_to_itemize(title,label,style,body,bodyrow);
    }else{
      style = this.update_style_from_switches(style,'body');
      text = this.float_to_body(title,label,style,body,bodyrow);
    }  
    return text;
  }
  do_PRIM(block) {
  ...
  to_example_body(ss){
    let o = [];
    ss.forEach((s,i,arr) => {
      if(this.re_is_fullline.test(s)){     
        o.push(s.slice(1).trim()); 
      }else{
        if(o.length){
          let text = o.pop();
  ...
  to_sample_body(ss){
    var o = [];
    ss.forEach((s,i,arr) => {
      s = s.slice(4);//remove the first 4 characters
      o.push(s);
    });
    return o;
  }  
