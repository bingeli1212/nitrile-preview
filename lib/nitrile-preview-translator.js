...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  untext(ss,style) {
    ...
    if(n>1 && (v=re_cluster_line.exec(s0))!==null && (w=re_cluster_line.exec(sz))!==null && v[1]===w[1] && v[2]===w[2]){ 
      ...
      if(fenceid=='animate'){
        return this.para_to_animate(ss,style);
      }
      ///auto detect
  ...
  ss_to_figure_itms(ss,style){
    ...
    ss.forEach(s => {
      ...
      if (this.line_is_indented(s)) {
        ...
        let cnt = v[1];
        let sub = v[2];
        let img = this.img_string_to_style(cnt);
        d.push({ img, sub });
      }
    });
    return d;
  ...
  img_string_to_style(line) {
    ...
    pp.forEach((p) => {
      if((v=re_1.exec(p))!==null){
        var key = v[1];
        var val = v[2];
        d[key] = val;
      }else if((v=re_2.exec(p))!==null){
        var key = v[1];
        var val = 1;
        d[key] = val;
      }
