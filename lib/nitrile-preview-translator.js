...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  do_fence(key,style,body){
    ...
    if(key==='img'){
      ...
      style = this.update_style_from_switches(style,key);
      return this.fence_to_center(style,body);
    }else if(key=='flushright'){
      style = this.update_style_from_switches(style,key);
      return this.fence_to_flushright(style,body);
    }
    return '';
  }
  untext(style,body) {
  ...
  fontify_cjk(text) {
    ...
    for (var j=0; j < text.length; ++j) {
      ...
      } else if(this.tokenizer.has_font_name_for_cc(cc)) {
        var fns = 0;
        // fns should be '16' for 'dingbats'
        var myfontname = this.tokenizer.get_font_name_for_cc(cc);
        var k;
        for (k=0; k < cjkfontnames.length; ++k) {
          if(myfontname.localeCompare(cjkfontnames[k])==0){
            fns = 1 << k;
            break;
          }
        }
