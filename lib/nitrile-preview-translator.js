...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  do_FRNT(block){
    var {hdgn,name,title,label,idnum,idcap,level,partnum,chapternum,style} = block;
    if(block.name=='chapter'){
      var text = this.hdgs_to_chapter(title,label,idnum,idcap,partnum,chapternum,level,style);
    }else if(block.name=='part'){
      var text = this.hdgs_to_part(title,label,idnum,idcap,partnum,chapternum,level,style);
    }else{
      var text = '';
    }
    return text;
  ...
  do_HDGS(block){
    var {hdgn,name,title,label,idnum,idcap,level,partnum,chapternum,style} = block;
    if(hdgn==1){
      var text = this.hdgs_to_section(title,label,idnum,idcap,partnum,chapternum,level,style);
    }
    else if(hdgn==2){
      var text = this.hdgs_to_subsection(title,label,idnum,idcap,partnum,chapternum,level,style);
    }
    else{
      var text = this.hdgs_to_subsubsection(title,label,idnum,idcap,partnum,chapternum,level,style);
    }
    return text;
  }
  do_CAPT(block){
  ...
  build_default_pagenum_map(){
    ...
    o.forEach((pp,i) => {
      ...
      pp.unshift({x:`<div class='HEADING' id='page${pagenum}' style='${this.css("HEADING")}' > ${pagenum} </div>`});
