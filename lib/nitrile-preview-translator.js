...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  do_SAMP(block){
    ...
    if(id=='@'){
      ...
      var text = this.float_to_math(title,label,style,body);
    }else if(id=='equation'){
      ...
      var text = this.float_to_framed(title,label,style,body);
    }else if(id=='lines'){
      var text = this.float_to_lines(title,label,style,body);
    }else if(id=='tabbling'){
  ...
  do_fence(key,style,body){
    ...
    //update from switch
    if(key==='img'){
      return this.fence_to_img(style,body);
    }else if(key==='diagram'){
      return this.fence_to_diagram(style,body);
    }else if(key==='framed'){
      return this.fence_to_framed(style,body);
    }else if(key==='displaymath'){
      return this.fence_to_displaymath(style,body);
    }else if(key=='tabular'){
      return this.fence_to_tabular(style,body);
    }else if(key=='list'){
      return this.fence_to_list(style,body);
    }else if(key=='verbatim'){
      return this.fence_to_verbatim(style,body);
    }
    return '';
  }
  untext(style,body) {
    body = this.trim_samp_body(body);
    var s0 = body[0]||'';
    var sZ = body[body.length-1]||'';
    if(s0.trim()==='~~~' && sZ.trim()==='~~~'){
      ///surrounded by ~~~ and ~~~
      var [style1] = this.string_to_style('{}',style);
      var body1 = body.slice(1,body.length-1);
      var body1 = this.trim_samp_body(body1);
      return this.fence_to_verbatim(style1,body1);
    }
    var v0 = this.re_fence.exec(s0.trimLeft());
    var vZ = this.re_fence.exec(sZ.trimLeft());
    if(v0 && vZ){
      ///this is a fenced block
      var [key,style1,body1] = this.extract_fence(body,0,style);
      var body1 = this.trim_samp_body(body1);
      return this.do_fence(key,style1,body1);
    }
    ///no fences at all
    return this.fence_to_plaintext(style,body);
  }
  uncode(style,line) {
  ...
  ss_to_figure_itms(ss,style){
    ...
    clusters.forEach((cluster,j,arr) => {
      let s0 = cluster[0];
      if((v=re_phrase.exec(s0))!==null){
        var [key,cnt,cnt2,cnt3] = this.extract_phrase(s0,0,v[1]);
        itm = {};
        itm.key = key;
        itm.type = 'phrase';
        itm.style = style;
        ...
        itm.cnt3 = cnt3;
        itm.seq = ++seq;
        itm.sub = this.join_para(cluster.slice(1));
        itms.push(itm);
      }else if((v=re_fence.exec(s0))!==null){
        var [key,g,body,cluster] = this.extract_fence(cluster,0,style);
        itm = {};
        itm.key = key;
        itm.type = 'fence';
        itm.style = g;
        itm.body = body;
        itm.seq = ++seq;
        itm.sub = this.join_para(cluster);
        itms.push(itm);
      }else if(s0.trim()=='\\\\'){
        itm = {};
        itm.type = '\\\\';
  ...
  extract_fence(cluster,i,style){
    ...
    if(v){
      key = v[1];
      g = this.string_to_style(v[2],g)[0];
    }
    i++;
  ...
  do_phrase(key,style,cnt,cnt2,cnt3){
    ...
    switch (key) {
      ...
      case 'inlinemath': {
        txt = this.phrase_to_inlinemath(style,cnt,cnt2,cnt3);
        break;
      }
      case 'displaymath': {
        txt = this.phrase_to_displaymath(style,cnt,cnt2,cnt3);
        break;
      }
      case 'ref': {
      ...
      ///following are fence related phrases
      case 'displaymath':{
        txt = this.phrase_to_displaymath(style,cnt,cnt2,cnt3);
        break;
      }
      case 'img':{
        txt = this.phrase_to_img(style,cnt,cnt2,cnt3);
        break;
      }
      case 'diagram':{
        txt = this.phrase_to_diagram(style,cnt,cnt2,cnt3);
        break;
      }
      case 'framed':{
        txt = this.phrase_to_framed(style,cnt,cnt2,cnt3);
        break;
      }
      ///following are math phrases
      case 'math': {
        txt = this.phrase_to_math(style,cnt,cnt2,cnt3);
        break;
      }
      case 'sqrt': {
        txt = this.phrase_to_sqrt(style,cnt,cnt2,cnt3);
        break;
      }
      case 'dfrac': {
        txt = this.phrase_to_dfrac(style,cnt,cnt2,cnt3);
        break;
      }
      case 'frac': {
        txt = this.phrase_to_frac(style,cnt,cnt2,cnt3);
        break;
      }
      case 'dbinom': {
        txt = this.phrase_to_dbinom(style,cnt,cnt2,cnt3);
        break;
      }
      case 'binom': {
        txt = this.phrase_to_binom(style,cnt,cnt2,cnt3);
        break;
      }
      case 'root': {
        txt = this.phrase_to_root(style,cnt,cnt2,cnt3);
        break;
      }
      case 'overline': {
        txt = this.phrase_to_overline(style,cnt,cnt2,cnt3);
        break;
      }
      case 'overleftrightarrow': {
        txt = this.phrase_to_overleftrightarrow(style,cnt,cnt2,cnt3);
        break;
      }
      case 'overrightarrow': {
        txt = this.phrase_to_overrightarrow(style,cnt,cnt2,cnt3);
        break;
      }
      case 'underleftrightarrow': {
        txt = this.phrase_to_underleftrightarrow(style,cnt,cnt2,cnt3);
        break;
      }
      case 'underrightarrow': {
        txt = this.phrase_to_underrightarrow(style,cnt,cnt2,cnt3);
        break;
      }
      case 'subsup': {
        txt = this.phrase_to_subsup(style,cnt,cnt2,cnt3);
        break;
      }
      case 'sub': {
        txt = this.phrase_to_sub(style,cnt,cnt2,cnt3);
        break;
      }
      case 'sup': {
        txt = this.phrase_to_sup(style,cnt,cnt2,cnt3);
        break;
      }
      case 'sum': {
        txt = this.phrase_to_sum(style,cnt,cnt2,cnt3);
        break;
      }
      case 'int': {
        txt = this.phrase_to_int(style,cnt,cnt2,cnt3);
        break;
      }
      default: {
  ...
  phrase_to_displaymath(style,cnt,cnt2,cnt3){
    var cnt2 = `{${cnt2}}`;
    var style = this.string_to_style(cnt2,style)[0];
    var body = [cnt];
    return this.do_fence('displaymath',style,body);
  }
  phrase_to_img(style,cnt,cnt2,cnt3){
    var cnt2 = `{${cnt2}}`;
    var style = this.string_to_style(cnt2,style)[0];
    var body = [cnt];
    return this.do_fence('img',style,body);
  }
  phrase_to_diagram(style,cnt,cnt2,cnt3){
    var cnt2 = `{${cnt2}}`;
    var style = this.string_to_style(cnt2,style)[0];
    var body = [cnt];
    return this.do_fence('diagram',style,body);
  }
  phrase_to_framed(style,cnt,cnt2,cnt3){
    var cnt2 = `{${cnt2}}`;
    style = this.string_to_style(cnt2,style)[0];
    var body = [cnt];
    return this.do_fence('framed',style,body);
  }
  /////////////////////////////////////////////////////////////////////////////
  ///
  ///
