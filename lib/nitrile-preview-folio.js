...
`;
  ...
  to_output_html() {
    ...
    o.forEach((pp,i) => {
      ...
      pp.unshift({x:`<div class='HEADING' id='pagenum.${pagenum}' style='${this.css("HEADING")}' > ${pagenum} </div>`});
  ...
  to_toc_html(){
    ...
    o.forEach((pp,j) => {
      ...
      pp.forEach((p) => {
        ...
        if(block.name=='part'){
          all.push(`<tr> <td> Part ${partnum} &#160; ${title} </td> <td style='text-align:right' > <a href='#pagenum.${pagenum}' style='${cssanchor}' >${pagenum}</a> </td> </tr>`);
        }else{
          all.push(`<tr> <td style='padding-left:1em;' > ${chapternum} &#160; ${title} </td> <td style='text-align:right' > <a href='#pagenum.${pagenum}' style='${cssanchor}' >${pagenum}</a> </td> </tr>`);
        }
      });
      all.push(`</table>`);    
      all.push(`</article>`);
