...
class NitrilePreviewFolio extends NitrilePreviewHtml {
  ...
  constructor(parser) {
    ...
    this.add_css_map_entry(this.css_map,
      'EQUATION, FIGURE, LISTING, LONGTABU, COLUMN', [
        'margin-left: 0pt',
        'margin-right: 0pt',
      ]
    );
...
`;
  ...
  to_output_html() {
    ...
    o.forEach((pp,i) => {
      ...
      all.push(`<div class='HEADER' id='page${pagenum}' style='${this.css("HEADER")}' > ${pagenum} </div>`);
      pp.forEach((p) => {
  ...
  to_toc_html(){
    ...
    o.forEach((pp,j) => {
      ...
      all.push(`<div class='HEADER' id='toc.${j+1}' style='${this.css("HEADER")}' > </div>`);
