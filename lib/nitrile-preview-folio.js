...
`;
  ...
  to_output() {
    ...
    o.forEach((pp,i) => {
      ...
      pp.push({x:`<div class='HEADING' id='pagenum.${pagenum}' style='position:absolute;top:5mm;right:10mm;' > ${pagenum} </div>`});
      pp.push({x:`</article>`});
    });
    ///
    ///flatten it
  ...
  to_toc(){
    ...
    o.forEach((pp) => {
      ...
      let css = this.css('TOC');
      all.push(`<h1 class='TOC' style='${css}'> Table Of Contents </h1>`);
