...
`;
  ...
  to_output() {
    ...
    for (let block of this.parser.blocks) {
      ...
      if(!block.style.parser){
        chapters.push( [block] );
      } else {
        if (chapters.length==0){
          chapters.push( [block] );
        } else if (chapters[chapters.length-1][0].style.parser === block.style.parser) {
    ...
    while (i < pp.length) {
      ...
      id = i+1;
      all.push(`<article class='FOLIO' id='frame${id}' >`);
      while (i < pp.length) {
        let p = pp[i];
        i++;
        let x = this.translate_block(p.block);
        all.push(x);       
        if(pp[i] && pp[i].block.sig == 'HDGS' && pp[i].block.hdgn == 1){
          break;
        }            
      }
      all.push(`</article>`);
