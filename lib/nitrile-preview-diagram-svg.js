...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  do_output(){
    ...
    if(this.background=='grid'){
      let gridalpha = this.g_to_gridalpha_float(this.env);
      let gw = viewport_grid*viewport_unit*this.MM_TO_PX;
      let bgid = this.get_css_id();
      o.push(`<pattern patternUnits="userSpaceOnUse" id="${bgid}" width="${gw}" height="${gw}"> <polyline points="0 0 ${gw} 0 ${gw} ${gw} 0 ${gw}" fill="none" stroke="currentColor" /> </pattern>`);
      o.push(`<rect x="0" y="0" width="${vw}" height="${vh}" fill="url(#${bgid})" stroke="currentColor" opacity="${gridalpha}" />`);
    }
    o.push(s);    
    var svgtext = o.join('\n');
    if(this.width && this.height){
      var img = `<svg width='${this.fix(this.width*this.MM_TO_PX)}' height='${this.fix(this.height*this.MM_TO_PX)}' xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="currentColor" stroke="currentColor" viewBox='0 0 ${this.fix(vw)} ${this.fix(vh)}' >${svgtext}</svg>`;
    }else if(this.width){
      var img = `<svg width='${this.fix(this.width*this.MM_TO_PX)}'                                                 xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="currentColor" stroke="currentColor" viewBox='0 0 ${this.fix(vw)} ${this.fix(vh)}' >${svgtext}</svg>`;
    }else if(this.height){
      var img = `<svg                                               height='${this.fix(this.height*this.MM_TO_PX)}' xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="currentColor" stroke="currentColor" viewBox='0 0 ${this.fix(vw)} ${this.fix(vh)}' >${svgtext}</svg>`;
    }else{
      var img = `<svg width='${this.fix(vw)}'                       height='${this.fix(vh)}'                        xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="currentColor" stroke="currentColor" viewBox='0 0 ${this.fix(vw)} ${this.fix(vh)}' >${svgtext}</svg>`;
    }
    var output = { img,viewport_width,viewport_height,viewport_unit,viewport_grid,vw,vh,svgtext };
  ...
  p_header(opt,x,y,vpw,vph,txts,g){
    var txt = txts[0]||'';
    ...
    if(is_math){
      ...
      height *= factor;
      if(opt=='l'){
        return `<g transform='translate(${5},${5})'><svg width="${width}" height="${height}" fill="currentColor" stroke="none" viewBox="0 0 ${vw} ${vh}"><defs>${defs.join('\n')}</defs>${s}</svg></g>`;  
      }else if(opt=='r'){
        return `<g transform='translate(${vpw-width-5},${5})'><svg width="${width}" height="${height}" fill="currentColor" stroke="none" viewBox="0 0 ${vw} ${vh}"><defs>${defs.join('\n')}</defs>${s}</svg></g>`;  
      }else{
        return `<g transform='translate(${(vpw-width)/2},${5})'><svg width="${width}" height="${height}" fill="currentColor" stroke="none" viewBox="0 0 ${vw} ${vh}"><defs>${defs.join('\n')}</defs>${s}</svg></g>`;
      }
    } else {
      var txt = this.translator.smooth(txt);
      if(opt=='l'){
        return `<text x="${5}" y="${5}" font-size="${fontsize}pt" text-anchor="start" ${this.g_to_svg_textdraw_str(g)} dy="0.85em">${txt}</text>`;
      }else if(opt=='r'){
        return `<text x="${vpw-5}" y="${5}" font-size="${fontsize}pt" text-anchor="end" ${this.g_to_svg_textdraw_str(g)} dy="0.85em">${txt}</text>`;
      }else{
        return `<text x="${vpw/2}" y="${5}" font-size="${fontsize}pt" text-anchor="middle" ${this.g_to_svg_textdraw_str(g)} dy="0.85em">${txt}</text>`;
      }
    }
  }
  p_subtitle(opt,x,y,vpw,vph,txts,g){
    var txt = txts[0]||'';
    var fontsize = this.g_to_fontsize_float(g);
    if(!fontsize){
      fontsize = this.fontsize;
    ...
    if(is_math){
      ...
      height *= factor;
      if(opt=='sw'){
        return `<g transform='translate(${5},${vph-height})'><svg width="${width}" height="${height}" fill="currentColor" stroke="none" viewBox="0 0 ${vw} ${vh}"><defs>${defs.join('\n')}</defs>${s}</svg></g>`;  
      }else if(opt=='se'){
        return `<g transform='translate(${vpw-width-5},${vph-height})'><svg width="${width}" height="${height}" fill="currentColor" stroke="none" viewBox="0 0 ${vw} ${vh}"><defs>${defs.join('\n')}</defs>${s}</svg></g>`;  
      }else{
      ...
      var txt = this.translator.smooth(txt);
      if(opt=='sw'){
        return `<text x="${2.6}" y="${vph}" font-size="${fontsize}pt" text-anchor="start" ${this.g_to_svg_textdraw_str(g)} dy="-2pt">${txt}</text>`;
      }else if(opt=='se'){
        return `<text x="${vpw-2.6}" y="${vph}" font-size="${fontsize}pt" text-anchor="end" ${this.g_to_svg_textdraw_str(g)} dy="-2pt">${txt}</text>`;
      }else{
        return `<text x="${vpw/2}" y="${vph}" font-size="${fontsize}pt" text-anchor="middle" ${this.g_to_svg_textdraw_str(g)} dy="-2pt">${txt}</text>`;
      }
    }
  }
  p_image(x,y,w,h,ta,txts,g){
    var imgsrc = this.translator.choose_image_file(txts);
    var { imgsrc, imgid } = this.translator.to_request_image(imgsrc);
    //shift the image based on the settings of 'ta'
    switch(ta){
      case 'ctr' : x+=w/2; y+=h/2; break;
      case 'ulft': x-=w;           break;
      case 'llft': x-=w;   y-=h;   break;
      case 'urt' :                 break;
      case 'lrt' :         y-=h;   break;
      case 'top' : x+=w/2;         break;
      case 'bot' : x+=w/2; y-=h;   break;
      case 'lft' : x-=w;   y-=h/2; break;
      case 'rt'  :         y-=h/2; break;
    }
    //draw image
    x = this.localx(x);
  ...
  p_cairo(px,py,txt,extent,g){
    ...
    pp.forEach((p,i,arr) => {
      if(p.key=='latin' || p.key=='punc' || p.key=='unicode'){
        ...
        o.push(`<text transform="translate(${this.fix(x)} ${this.fix(y)}) scale(${factor})" w="${this.fix(p.w)}" h="${this.fix(p.h)}" mid="${this.fix(p.mid)}" font-family="${fontfamily}" font-style="${fontstyle}" font-weight="${fontweight}" font-size="${this.tokenizer.fs}pt" text-anchor="${anchor}" ${this.g_to_svg_textdraw_str(g)} dy="${this.tokenizer.text_dy_pt}pt" textLength="${this.fix(textlength)}" lengthAdjust="spacingAndGlyphs">${txt}</text>`);
      }else if(p.key=='svg'){
        var x = p.x;
        var y = p.y + p.dy;
        var w = p.w;
  ...
  p_fillpath(coords,_g){
    ...
    for(var item of items){
      ...
      if(this.has_shades(g)){
        o.push(`<path d="${s}" ${this.g_to_svg_shadeonly_str(g)}/>`);  
      }else{
        o.push(`<path d="${s}" ${this.g_to_svg_fillonly_str(g)}/>`);  
      }
    }
    var img = o.join(' ');
    var img = `<g transform="translate(${this.origin_x*this.u},${-this.origin_y*this.u}) translate(0,${this.viewport_height*this.u}) scale(${this.origin_sx},${this.origin_sy}) translate(0,${-this.viewport_height*this.u})"> ${img} </g>`;
  ...
  p_drawpath(coords,_g){
    ...
    for(var item of items){
      ...
      if(this.has_strokes(g)){
        o.push(`<path d="${s}" ${this.g_to_svg_drawonly_str(g)}/>`);
      }
    }
    var img = o.join(' ');
    var img = `<g transform="translate(${this.origin_x*this.u},${-this.origin_y*this.u}) translate(0,${this.viewport_height*this.u}) scale(${this.origin_sx},${this.origin_sy}) translate(0,${-this.viewport_height*this.u})"> ${img} </g>`;
  ...
  p_arrow(coords,_g){
    ...
    for(var item of items){
      ...
      if(this.has_strokes(g)){
        o.push(`<path d="${s}" ${this.g_to_svg_drawonly_str(g)} marker-end="url(#${id1})" />`);
      }
    }
    var img = o.join(' ');
    var img = `<g transform="translate(${this.origin_x*this.u},${-this.origin_y*this.u}) translate(0,${this.viewport_height*this.u}) scale(${this.origin_sx},${this.origin_sy}) translate(0,${-this.viewport_height*this.u})"> ${img} </g>`;
  ...
  p_revarrow(coords,_g){
    ...
    for(var item of items){
      ...
      if(this.has_strokes(g)){
        o.push(`<path d="${s}" ${this.g_to_svg_drawonly_str(g)} marker-start="url(#${id2})" />`);
      }
    }
    var img = o.join(' ');
    var img = `<g transform="translate(${this.origin_x*this.u},${-this.origin_y*this.u}) translate(0,${this.viewport_height*this.u}) scale(${this.origin_sx},${this.origin_sy}) translate(0,${-this.viewport_height*this.u})"> ${img} </g>`;
  ...
  p_dblarrow(coords,_g){
    ...
    for(var item of items){
      ...
      let id2 = this.create_arrowhead('start',g);
      if(this.has_strokes(g)){
        o.push(`<path d="${s}" ${this.g_to_svg_drawonly_str(g)} marker-end="url(#${id1})" marker-start="url(#${id2})" />`);
      }
    }
    var img = o.join(' ');
    var img = `<g transform="translate(${this.origin_x*this.u},${-this.origin_y*this.u}) translate(0,${this.viewport_height*this.u}) scale(${this.origin_sx},${this.origin_sy}) translate(0,${-this.viewport_height*this.u})"> ${img} </g>`;
  ...
  p_ellipse(cx,cy,rx,ry,g){
    ...
    if(this.has_shades(g)){
      o.push(`<ellipse cx="${this.fix(cx)}" cy="${this.fix(cy)}" rx="${this.fix(rx)}" ry="${this.fix(ry)}" ${this.g_to_svg_shadeonly_str(g)}/>`);
    }else if(this.has_fills(g)){
      o.push(`<ellipse cx="${this.fix(cx)}" cy="${this.fix(cy)}" rx="${this.fix(rx)}" ry="${this.fix(ry)}" ${this.g_to_svg_fillonly_str(g)}/>`);
    }        
    if(this.has_strokes(g)){
      o.push(`<ellipse cx="${this.fix(cx)}" cy="${this.fix(cy)}" rx="${this.fix(rx)}" ry="${this.fix(ry)}" ${this.g_to_svg_drawonly_str(g)} />`);
    }
    var img = o.join(' ');
    var img = `<g transform="translate(${this.origin_x*this.u},${-this.origin_y*this.u}) translate(0,${this.viewport_height*this.u}) scale(${this.origin_sx},${this.origin_sy}) translate(0,${-this.viewport_height*this.u})"> ${img} </g>`;
    return img;  
  ...
  p_circle(cx,cy,r,g){
    ...
    r = this.localdist(r);
    if(this.has_shades(g)){
      o.push(`<circle cx="${this.fix(cx)}" cy="${this.fix(cy)}" r="${this.fix(r)}" ${this.g_to_svg_shadeonly_str(g)}/>`);
    }else if(this.has_fills(g)){
      o.push(`<circle cx="${this.fix(cx)}" cy="${this.fix(cy)}" r="${this.fix(r)}" ${this.g_to_svg_fillonly_str(g)}/>`);
    }        
    if(this.has_strokes(g)){
      o.push(`<circle cx="${this.fix(cx)}" cy="${this.fix(cy)}" r="${this.fix(r)}" ${this.g_to_svg_drawonly_str(g)} />`);
    }
    var img = o.join(' ');
    var img = `<g transform="translate(${this.origin_x*this.u},${-this.origin_y*this.u}) translate(0,${this.viewport_height*this.u}) scale(${this.origin_sx},${this.origin_sy}) translate(0,${-this.viewport_height*this.u})"> ${img} </g>`;
    return img;
  ...
  p_rect(x,y,w,h,rdx,rdy,g){
    ...
    if(this.has_shades(g)){
      o.push(`<rect x="${this.fix(x)}" y="${this.fix(y-h)}" width="${this.fix(w)}" height="${this.fix(h)}" rx="${this.fix(rdx)}" ry="${this.fix(rdy)}" ${this.g_to_svg_shadeonly_str(g)} />`);
    }else if(this.has_fills(g)){
      o.push(`<rect x="${this.fix(x)}" y="${this.fix(y-h)}" width="${this.fix(w)}" height="${this.fix(h)}" rx="${this.fix(rdx)}" ry="${this.fix(rdy)}" ${this.g_to_svg_fillonly_str(g)} />`);
    }
    if(this.has_strokes(g)){
      o.push(`<rect x="${this.fix(x)}" y="${this.fix(y-h)}" width="${this.fix(w)}" height="${this.fix(h)}" rx="${this.fix(rdx)}" ry="${this.fix(rdy)}" ${this.g_to_svg_drawonly_str(g)} />`);
    }
    var img = o.join(' ');
    var img = `<g transform="translate(${this.origin_x*this.u},${-this.origin_y*this.u}) translate(0,${this.viewport_height*this.u}) scale(${this.origin_sx},${this.origin_sy}) translate(0,${-this.viewport_height*this.u})"> ${img} </g>`;
    return img;
  ...
  p_line(x1,y1,x2,y2,g){
    ...
    if(this.has_strokes(g)){
      if(arrowhead==1){
        let id1 = this.create_arrowhead('end',g);
        o.push(`<line x1="${this.fix(x1)}" y1="${this.fix(y1)}" x2="${this.fix(x2)}" y2="${this.fix(y2)}" ${this.g_to_svg_drawonly_str(g)} marker-end="url(#${id1})" />`);
      }else if(arrowhead==2){
        let id2 = this.create_arrowhead('start',g);
        o.push(`<line x1="${this.fix(x1)}" y1="${this.fix(y1)}" x2="${this.fix(x2)}" y2="${this.fix(y2)}" ${this.g_to_svg_drawonly_str(g)} marker-start="url(#${id2})" />`);
      }else if(arrowhead==3){
        let id1 = this.create_arrowhead('end',g);
        let id2 = this.create_arrowhead('start',g);
        o.push(`<line x1="${this.fix(x1)}" y1="${this.fix(y1)}" x2="${this.fix(x2)}" y2="${this.fix(y2)}" ${this.g_to_svg_drawonly_str(g)} marker-end="url(#${id1})" marker-start="url(#${id2})" />`);
      }else{
        o.push(`<line x1="${this.fix(x1)}" y1="${this.fix(y1)}" x2="${this.fix(x2)}" y2="${this.fix(y2)}" ${this.g_to_svg_drawonly_str(g)} />`);
      }
    }
    var img = o.join(' ');
    var img = `<g transform="translate(${this.origin_x*this.u},${-this.origin_y*this.u}) translate(0,${this.viewport_height*this.u}) scale(${this.origin_sx},${this.origin_sy}) translate(0,${-this.viewport_height*this.u})"> ${img} </g>`;
  ...
  p_polyline(points,g){
    ...
    var o = [];
    if(this.has_strokes(g)){
      o.push(`<polyline points="${s}" ${this.g_to_svg_drawonly_str(g)} />`);
    }
    var img = o.join(' ');
    var img = `<g transform="translate(${this.origin_x*this.u},${-this.origin_y*this.u}) translate(0,${this.viewport_height*this.u}) scale(${this.origin_sx},${this.origin_sy}) translate(0,${-this.viewport_height*this.u})"> ${img} </g>`;
    return img;
  ...
  p_polygon(points,g){
    ...
    var o = [];
    if(this.has_shades(g)){
      o.push(`<polygon points="${s}" ${this.g_to_svg_shadeonly_str(g)} />`);
    }else if(this.has_fills(g)){
      o.push(`<polygon points="${s}" ${this.g_to_svg_fillonly_str(g)} />`);
    }
    if(this.has_strokes(g)){
      o.push(`<polygon points="${s}" ${this.g_to_svg_drawonly_str(g)} />`);
    }
    var img = o.join(' ');
    var img = `<g transform="translate(${this.origin_x*this.u},${-this.origin_y*this.u}) translate(0,${this.viewport_height*this.u}) scale(${this.origin_sx},${this.origin_sy}) translate(0,${-this.viewport_height*this.u})"> ${img} </g>`;
    return img;
  ...
  p_qbezier_line(x0,y0, x1,y1, x2,y2, g){
    ...
    if(this.has_strokes(g)){
      if(arrowhead==1){
        let id1 = this.create_arrowhead('end',g);
        o.push(`<path d="M ${this.fix(x0)},${this.fix(y0)} Q ${this.fix(x1)},${this.fix(y1)},${this.fix(x2)},${this.fix(y2)}" ${this.g_to_svg_drawonly_str(g)} marker-end="url(#${id1})" />`);
      }else if(arrowhead==2){
        let id2 = this.create_arrowhead('start',g);
        o.push(`<path d="M ${this.fix(x0)},${this.fix(y0)} Q ${this.fix(x1)},${this.fix(y1)},${this.fix(x2)},${this.fix(y2)}" ${this.g_to_svg_drawonly_str(g)} marker-start="url(#${id2})" />`);
      }else if(arrowhead==3){
        let id1 = this.create_arrowhead('end',g);
        let id2 = this.create_arrowhead('start',g);
        o.push(`<path d="M ${this.fix(x0)},${this.fix(y0)} Q ${this.fix(x1)},${this.fix(y1)},${this.fix(x2)},${this.fix(y2)}" ${this.g_to_svg_drawonly_str(g)} marker-end="url(#${id1})" marker-start="url(#${id2})" />`);
      }else{
        o.push(`<path d="M ${this.fix(x0)},${this.fix(y0)} Q ${this.fix(x1)},${this.fix(y1)},${this.fix(x2)},${this.fix(y2)}" ${this.g_to_svg_drawonly_str(g)} />`);
      }
    }
    var img = o.join(' ');
    var img = `<g transform="translate(${this.origin_x*this.u},${-this.origin_y*this.u}) translate(0,${this.viewport_height*this.u}) scale(${this.origin_sx},${this.origin_sy}) translate(0,${-this.viewport_height*this.u})"> ${img} </g>`;
  ...
  p_cbezier_line(x0,y0, x1,y1, x2,y2, x3,y3, g){
    ...
    if(this.has_strokes(g)){
      if(arrowhead==1){
        let id1 = this.create_arrowhead('end',g);
        o.push(`<path d="M ${this.fix(x0)},${this.fix(y0)} C ${this.fix(x1)},${this.fix(y1)},${this.fix(x2)},${this.fix(y2)},${this.fix(x3)},${this.fix(y3)}" ${this.g_to_svg_drawonly_str(g)} marker-end="url(#${id1})" />`);
      }else if(arrowhead==2){
        let id2 = this.create_arrowhead('start',g);
        o.push(`<path d="M ${this.fix(x0)},${this.fix(y0)} C ${this.fix(x1)},${this.fix(y1)},${this.fix(x2)},${this.fix(y2)},${this.fix(x3)},${this.fix(y3)}" ${this.g_to_svg_drawonly_str(g)} marker-start="url(#${id2})" />`);
      }else if(arrowhead==3){
        let id1 = this.create_arrowhead('end',g);
        let id2 = this.create_arrowhead('start',g);
        o.push(`<path d="M ${this.fix(x0)},${this.fix(y0)} C ${this.fix(x1)},${this.fix(y1)},${this.fix(x2)},${this.fix(y2)},${this.fix(x3)},${this.fix(y3)}" ${this.g_to_svg_drawonly_str(g)} marker-end="url(#${id1})" marker-start="url(#${id2})" />`);
      }else{
        o.push(`<path d="M ${this.fix(x0)},${this.fix(y0)} C ${this.fix(x1)},${this.fix(y1)},${this.fix(x2)},${this.fix(y2)},${this.fix(x3)},${this.fix(y3)}" ${this.g_to_svg_drawonly_str(g)} />`);
      }
    }
    var img = o.join(' ');
    var img = `<g transform="translate(${this.origin_x*this.u},${-this.origin_y*this.u}) translate(0,${this.viewport_height*this.u}) scale(${this.origin_sx},${this.origin_sy}) translate(0,${-this.viewport_height*this.u})"> ${img} </g>`;
  ...
  g_to_svg_fontfamily_str(g) {
    var f = this.g_to_fontstyle_string(g)||'';
    var ss = [];
    switch(f){
      case 't':
      case 'T':
