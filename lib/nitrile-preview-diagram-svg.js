...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  do_output(){
    ...
    if(this.background=='grid'){
      let gridopacity = this.g_to_gridopacity_float(this.config);
      ...
      o.push(`<rect x="0" y="0" width="${vw}" height="${vh}" fill="url(#${bgid})" stroke="currentColor" opacity="${gridopacity}" />`);
    }
    ///user commands
    o.push(s);    
    ///package into 'svgtext'
    var svgtext = o.join('\n');
    ///subtitle
    var img = `<svg class='DIASVG' width='100%' xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="currentColor" stroke="currentColor" viewBox="0 0 ${this.fix(vw)} ${this.fix(vh)}" >${svgtext}</svg>`;
  ...
  p_image(x,y,w,h,ta,label,g){
    var srcs = this.string_to_array(label);///multiple candidate img files are separated by space
    var imgsrc = this.translator.choose_image_file(srcs);
