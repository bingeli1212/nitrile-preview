...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  do_output(){
    ...
    ///width and height
    var p = {};
    var width = this.g_to_width_float(this.style);
    var height = this.g_to_height_float(this.style);
    if(width && height){
      p.width = width;
      p.height = height;
    }else if(width){
    ...
    if(this.my_images.length){
      ...
      var img = `<img style="width:${p.width}mm;height:${p.height}mm" src="${imgsrc}" id="${imgid}" onload="img_onload(this)"/>`;
    }else{
      var svgtext = o.join("\n");
      var svg = `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="currentColor" stroke="currentColor" viewBox="0 0 ${this.fix(vw)} ${this.fix(vh)}" >${svgtext}</svg>`;
      var { imgsrc, imgid } = this.translator.to_request_svgid(svg);
      var img = `<img style="width:${p.width}mm;height:${p.height}mm" src="${imgsrc}" id="${imgid}" onload="img_onload(this)"/>`;
    }
    var subtitle = this.my_subtitle;
    var output = { img,imgsrc,imgid,viewport_width,viewport_height,viewport_unit,viewport_grid,vw,vh,svgtext,subtitle };
    return(output);
  }
  localx(x) {
