...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  do_output(){
    ...
    if(this.my_images.length){
      ...
      if(this.stretch){
        var img = `<img style="border:${p.border};width:${this.stretch*100}%;" src="${imgsrc}" id="${imgid}" onload="img_onload(this)"> </img>`;
      }
    }else{
      var svgtext = o.join("\n");
    ...
    var output = { img,imgsrc,imgid,stretch,width,height,vw,vh,svgtext,subtitle,svg };
    return(output);
  }
  localx(x) {
