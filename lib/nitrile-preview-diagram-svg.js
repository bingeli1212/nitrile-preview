...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  p_subtitle(opt,vpw,vph,txt,g){
    ...
    if(g.math){
      ...
      height *= factor;
      if(opt=='sw'){
        return `<g transform='translate(${0},${vph-height})'><svg width="${width}" height="${height}" fill="currentColor" stroke="none" viewBox="0 0 ${vw} ${vh}"><defs>${defs.join('\n')}</defs>${s}</svg></g>`;  
      }else if(opt=='se'){
        return `<g transform='translate(${vpw-width},${vph-height})'><svg width="${width}" height="${height}" fill="currentColor" stroke="none" viewBox="0 0 ${vw} ${vh}"><defs>${defs.join('\n')}</defs>${s}</svg></g>`;  
      }else{
      ...
      if(opt=='sw'){
        return `<text x="${0}" y="${vph}" font-size="${fontsize}pt" text-anchor="start" ${this.g_to_svg_textdraw_str(g)} dy="-2pt">${txt}</text>`;
      }else if(opt=='se'){
        return `<text x="${vpw}" y="${vph}" font-size="${fontsize}pt" text-anchor="end" ${this.g_to_svg_textdraw_str(g)} dy="-2pt">${txt}</text>`;
      }else{
