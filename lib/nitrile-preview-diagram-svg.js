...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  p_subtitle(w,h,txt,g){
    ...
    if(g.math){
      ...
      width *= factor;
      height *= factor;
      return `<g transform='translate(${x-width/2},${y-height})'><svg width="${width}" height="${height}" fill="currentColor" stroke="none" viewBox="0 0 ${vw} ${vh}"><defs>${defs.join('\n')}</defs>${s}</svg></g>`;
    } else {
      var txt = this.translator.smooth(txt);
      return `<text x="${x}" y="${y}" font-size="${fontsize}pt" text-anchor="middle" ${this.g_to_svg_textdraw_str(g)} dy="-0.1em">${txt}</text>`;
    }
  }
