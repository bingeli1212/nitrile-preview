...
`);
  ...
  write_paper_part(top) {
    ...
    if(1){
      ...
      let all = [];
      all.push(`<h1>Part ${this.num_parts}. ${this.uncode(my.title,my.style,my.switches)}</h1>`);
      var html = all.join('\n')
      html = `<section class='part'>${html}</section>`
      var title = this.uncode(my.title,my.style,my.switches);
      every.push({html,title});
    }
    top.forEach((o, i) => {
      if (Array.isArray(o) && o.length && o[0].sig=='HDGS' && o[0].hdgn=='0') {
  ...
  write_paper_chapter(top) {
    ...
    all.push(`<h1>${this.uncode(my.title,my.style,my.switches)}</h1>`);
    top.forEach((o, i) => {
      if (Array.isArray(o)) {
        var {html} = this.write_paper_section(o);
        all.push(html);
