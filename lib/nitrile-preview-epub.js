...
`);
  ...
  write_paper_part(top) {
    ...
    this.num_parts++;
    let all = [];
    let {title,label,idnum,idcap,partnum,chapternum,level,style} = my;
    let text = this.hdgs_to_part(title,label,idnum,idcap,partnum,chapternum,level,style);
    all.push(text);
    var html = all.join('\n')
  ...
  write_paper_chapter(top) {
    ...
    let all = [];
    let {title,label,idnum,idcap,partnum,chapternum,level,style} = my;
    let text = this.hdgs_to_chapter(title,label,idnum,idcap,partnum,chapternum,level,style);
    all.push(text);
    top.forEach((o, i) => {
      if (Array.isArray(o)) {
        var {html} = this.write_paper_section(o);
  ...
  write_paper_section(top) {
    ...
    let all = [];
    let {title,label,idnum,idcap,partnum,chapternum,level,style} = my;
    let text = this.hdgs_to_section(title,label,idnum,idcap,partnum,chapternum,level,style);
    const re = /<h2\s(.*)<\/h2>$/;
    let v = re.exec(text);
    if(v){
      text = `<h1 ${v[1]}</h1>`;
