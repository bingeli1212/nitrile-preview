...
class NitrilePreviewEpub extends NitrilePreviewHtml {
  ...
  async to_epub_document_async () {
    ...
    for(let src of this.imgs){
      ...
      let mime = '';
      try {
        [imgbuff,mime] = await this.parser.read_image_file_async(src);
        console.log('retrieved image',fsrc,id,i+1);
      } catch (e) {
        console.error(e.toString());
      }
      this_images.push({refid:`image${i+1}`,href:src,mime,imgbuff});
    }
    ...
    blocks.forEach(block => {
      let {sig,hdgn,title,style,html} = block;
      if(!style) return;
      ...
      if(this.num_parts){
        ...
        this.addto_saveasmap(refid,saveas,html,saveasmap);
        if (sig=='PART') {
        ...
        let saveas = `${subseq}.xhtml`;
        this.addto_saveasmap(subseq,saveas,html,saveasmap);
        if(sig=='HDGS'&&hdgn==1){
    ...
    for (let [saveas,p] of saveasmap) {
      ...
      this_contents.push({refid:p.refid,href:saveas,mime,htmls:p.htmls});
    }
    console.log(mytoc);
...
`);
    ...
    for (let p of this_images) {
      let {href,imgdata} = p;
      this_map.set(href,imgdata);
    }

    /// generate style.css

    this_map.set('style.css',mycss);
...
`);
  ...
  phrase_to_ref(sig, label, floatname, idnum, refid, id) {
  ...
  addto_saveasmap(refid,saveas,html,saveasmap){
    if(!saveasmap.has(saveas)){
      let htmls = [];
      saveasmap.set(saveas,{refid,htmls});
    }
