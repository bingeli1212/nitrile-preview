...
`);
  ...
  write_paper(top) {
    ...
    if(this.paper.num_parts == 0 && this.paper.num_chapters == 0){
      ...
      top.forEach((o, i, arr) => {
        if (Array.isArray(o) && o.length && o[0].sig=='FRNT' && o[0].name=='chapter') {
  ...
  write_paper_part(top) {
    ...
    this.num_parts++;
    let all = [];
    let {title,label,idnum,idcap,partnum,chapternum,pagenum,level,style} = my;
    let text = this.hdgs_to_part(title,label,idnum,idcap,partnum,chapternum,pagenum,level,style);
    all.push(text);
    var html = all.join('\n')
    html = `<section class='part'>${html}</section>`
    title = this.uncode(my.style,my.title);
    every.push({html,title});
    top.forEach((o, i) => {
      if (Array.isArray(o) && o.length && o[0].sig=='FRNT' && o[0].name=='part') {
  ...
  write_paper_chapter(top) {
    ...
    let all = [];
    let {title,label,idnum,idcap,partnum,chapternum,pagenum,level,style} = my;
    let text = this.hdgs_to_chapter(title,label,idnum,idcap,partnum,chapternum,pagenum,level,style);
    all.push(text);
    top.forEach((o, i) => {
      if (Array.isArray(o)) {
        var {html} = this.write_paper_section(o);
  ...
  write_paper_section(top) {
    ...
    let all = [];
    let {title,label,idnum,idcap,partnum,chapternum,pagenum,level,style} = my;
    let text = this.hdgs_to_section(title,label,idnum,idcap,partnum,chapternum,pagenum,level,style);
    const re = /<h2\s(.*)<\/h2>$/;
    let v = re.exec(text);
    if(v){
      text = `<h1 ${v[1]}</h1>`;
  ...
  write_paper_subsection(top) {
    ...
    let css = 'clear:both';
    all.push('');
    all.push(`<h1 class='SUBSECTION' style='${css}'>${my.idnum} &#160; ${this.uncode(my.style,my.title)}</h1>`);
    top.forEach((o,i) => {
      if(Array.isArray(o)){
  ...
  write_paper_subsubsection(top) {
    ...
    let css = 'clear:both';
    all.push('');
    all.push(`<h1 class='SUBSUBSECTION' style='${css}'>${my.idnum} &#160; ${this.uncode(my.style,my.title)}</h1>`);
    top.forEach((o,i) => {
      if(Array.isArray(o)){
      }else{
        var html = this.translate_block(o);
