...
`);
  ...
  write_paper_chapter(top) {
    ...
    this.num_chapters++;
    let css = 'clear:both';
    all.push(`<h1 class='CHAPTER' style='${css}'>Chapter ${my.idnum} &#160; ${this.uncode(my.style,my.title)}</h1>`);
    top.forEach((o, i) => {
      if (Array.isArray(o)) {
        var {html} = this.write_paper_section(o);
        all.push(html);
  ...
  write_paper_section(top) {
    ...
    let css = 'clear:both';
    all.push('');
    all.push(`<h1 class='SECTION' style='${css}'>${my.idnum} &#160; ${this.uncode(my.style,my.title)}</h1>`);
    top.forEach((o, i) => {
      if (Array.isArray(o)) {
