...
class NitrilePreviewParser extends NitrilePreviewBase {
  ...
  async read_import_async(){
    let body = this.conf('import','','list');
    ...
    this.modes.forEach((item,i,arr) => {
      let { fname, title, style, subparser } = item;
      if (style.part) {
        /// insert a new PART block
        let sig = 'PART';
        let block = {sig,title,style};
        this.blocks.push(block);
      } else if (subparser) {
        /// 
        /// insert a HDGN block
        ...
        let sig = 'HDGS';
        let hdgn = 0;
        let title = subparser.conf('title','Untitled');
        let notes = subparser.notes;
        let vmap = subparser.vmap;
        let parser = subparser;
        let block = {sig,hdgn,title,fname,style,notes,vmap,parser};
        this.blocks.push(block);
        ///
