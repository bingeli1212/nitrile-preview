...
class NitrilePreviewParser extends NitrilePreviewBase {
  ...
  read_lines(lines) {
    ...
    if(lines.length){
      ...
      if(re_frnt.test(lines[0].trim())){
        ...
        data.push([key,[val]]); //note that 'val' is added as a list
        para = lines.slice(n0,n);
        style.subseq = n0;
        while(n < lines.length && lines[n].trim().length==0){
          n++;
      ...
      if(1){
        ...
        block = {sig, style, para, data, vmap, switches};
        var para = block.para;
    ...
    //
    //NOTE: read switches and vmap entries
    //
  ...
  read_para (lines,n,hdgn) {
    ...
    for (let i=0; !done && n < lines.length; ++i,++n) {
      ...
      if(i==0 && (v=re_capt.exec(line))!==null){
        ...
        if(v[1]=='@'){
          ...
          [id,style] = this.string_to_id_and_style(v[1]);
        }
        continue;
      }
      ...
      if(i==0){
        type = 'para';
        nspace = 0;
        if((v=re_floa_begin.exec(line))!==null){
          is_cluster = 1;
          title = v[1];
    ...
    // update this block with switches
    let vmap = this.vmap;
    let switches = this.switches;
    return {sig, hdgn, rank, id, title, label, fname, style, body, nblank, para, plitems, vmap, switches};
  }
  is_han (cc) {
    ///
  ...
  async read_import_async(){
    ...
    items.forEach((item,i,arr) => {
      ...
      if(subparser) {
        ...
        let para = [];
        let plitems = [];
        this.blocks.push({sig, hdgn, rank, id, title, label, fname, style, body, nblank, para, plitems, vmap, switches});
      }
    });
    return
  }
