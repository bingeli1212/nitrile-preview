...
class NitrilePreviewParser extends NitrilePreviewBase {
  ...
  read_lines(lines) {
    ...
    if(lines.length){
      ...
      block = {sig, style, title, label, name, para, data, parser:this};
      block.row1 = 0
      block.row2 = n;
      block.style.partnum = 0;
      block.style.chapternum = 0;
      block.style.pagenum = 0;
  ...
  read_para (lines,n) {
    ...
    style.partnum = partnum;
    style.chapternum = chapternum;
    style.pagenum = 0;
    style.idnum = '';
    style.label = label;
    let parser = this;
    return {sig, hdgn, rank, id, title, label, level, name, fname, style, bodyrow, body, nblank, para, plitems, parser};
  }
  is_han (cc) {
    ///
