...
class NitrilePreviewParser extends NitrilePreviewBase {
  ...
  read_para (lines,n) {
    ...
    if(type == 'spcl'){
      ...
      else if (floatid == 'input') {
        var floatname = 'Input';
        var title = 'Input';
      }
      else if (floatid == 'include') {
        var floatname = 'Include';
        var title = 'Include';
      }
  ...
  async read_image_file_async(src) {
    ///
    /// Returns a Promise that resolves to a NodeJs Buffer() and a mime-type pair
    /// ... [imgbuf,mime]
    ///
    /// The input argumetn is 'src', which is a src relative to the current
    /// 'dirname'; the full path string is to be constructed 
    ///
    /// NOTE: that the 'fs' method returns a new Buffer object
    /// and the 'Buffer#toString' method offers a way to 
    /// return a base64-encoded string.
    ///
    let fsrc = path.join(this.dirname,src);

    return new Promise((resolve, reject) => {
      fs.readFile(fsrc, null, function (err, imgbuf) {
        if (err) {
          reject(err.toString());
        } else {
