...
class NitrilePreviewParser extends NitrilePreviewBase {
      ...
      for (let t of data) {
        ...
        if (key.localeCompare(in_key)==0) {
          ...
          if(type=='float'){
            ...
            ///NOTE that the return value 'ret_val' has been switched to of type 'Array'
          }else{
            ret_val = val;
            if(suffix){
              ret_val = `${ret_val}${suffix}`;
  ...
  async read_import_async(){
    ...
    lines.forEach((line) => {
      ...
      items.push({fname,title,name,subparser,promise})
    });
    ///
    /// wait for all
    ///
    ...
    this.modes.forEach((item,i,arr) => {
      let { title, name, subparser } = item;
      if (subparser) {
        subparser.blocks.forEach((block) => {
          // assign the 'name' attribute to the block
          block.name = name;
        ...
        // create a new HDGS block, with its name attribute set to 'title', mostly for 'part'
        let sig = 'HDGS';
        let hdgn = 0;
        name = name;
        let style = {};
        this.blocks.push({sig,hdgn,title,name,style});
      }
    });
    return
  }
