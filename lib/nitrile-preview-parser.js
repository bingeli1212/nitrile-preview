  ...
  read_lines(lines) {
    ...
    if(lines.length){
      ...
      if(!name){
        ...
        name = this.join_para(name);
      }
      let partnum = 0;
      let chapternum = 0;
      block = {sig, style:this.style, title, label, name, partnum, chapternum, para, data, parser:this};
      block.row1 = 0
      block.row2 = n;
  ...
  async read_import_async(){
    ...
    this.imports.forEach((item,i,arr) => {
      ...
      if(name=='part'){
        ...
        let parser = null;
        let block = {sig, hdgn, rank, id, title, label, partnum, chapternum, level, name, fname, style, body, nblank, para, plitems, data, parser};  
        block.name = name;
        this.blocks.push(block);
      }else if(name=='chapter'){
        this.num_chapter++;
