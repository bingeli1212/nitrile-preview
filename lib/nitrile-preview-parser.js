  ...
  read_lines(lines) {
    ...
    if(lines.length){
      ...
      if(sig){
        ...
        block.style.switches = this.switches;
  ...
  read_para (lines,n,hdgn) {
    ...
    for (let i=0; !done && n < lines.length; ++i,++n) {
      ...
      if(i==0 && (v=re_hdgs.exec(line))!==null){
        type = 'hdgs';
        bull = v[1];
        var [style,title] = this.string_to_style(v[2])
        hdgn = bull.length;
        done = 1;
        continue;
      }
      ...
      if(i==0 && (v=re_capt.exec(line))!==null){
        type = 'samp';
        is_capt = 1;
        [id,style,title] = this.string_to_id_and_style(v[1]);
        continue;
      }
    ...
    return {sig, hdgn, rank, id, title, fname, style, body, nspace, para, plitems};
  }
  is_han (cc) {
    ///
