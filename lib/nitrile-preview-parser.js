...
class NitrilePreviewParser extends NitrilePreviewBase {
  ...
  read_lines(lines) {
    ...
    if(lines.length){
      ...
      this.titlepage = this.data['titlepage']||0;
      this.tocpage = this.data['tocpage']||0;
  ...
  read_para (lines,n) {
    ...
    style.label = label;
    let parser = this;
    return {sig, hdgn, rank, id, caste: id, title, label, level, fname, style, bodyrow, body, nblank, para, plitems, parser};
  }
  is_han (cc) {
    ///
