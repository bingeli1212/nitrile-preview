...
class NitrilePreviewParser extends NitrilePreviewBase {
    ...
    if(lines.length){
      ...
      if(re_frnt.test(lines[0].trim())){
        ...
        for (; n < lines.length; n++) {
          ...
          if((v=re_buf.exec(s))!==null){
            //buffer
            let name = v[2];
            if(name.length){
              ///create a new buffer for this name
              save_buf = [];
              this.buffers[name] = save_buf;
            }else{
              ///terminate any existing buffer, if any
              save_buf = null;
            }
            continue;
          }
  ...
  read_para (lines,n) {
    ...
    for (let i=0; !done && n < lines.length; ++i,++n) {
      ...
      if(i==0 && (v=re_sdot.exec(line))!==null){
        ...
        is_capt = 1;
        id = v[1];
        style = this.string_to_style(v[2],style);//read_para()
        continue;
      }
  ...
  async read_import_async(){
    ...
    this.imports.forEach((line) => {
      let {name,title,fname} = this.string_to_mode(line);
      if(fname){
        // [chapter](./myfile.md)
