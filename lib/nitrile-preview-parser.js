...
class NitrilePreviewParser extends NitrilePreviewBase {
  ...
  read_lines(lines) {
    ...
    if(lines.length){
      ...
      if(re_is_frnt_fence.test(lines[0].trim())){
        ...
        for (; n < lines.length; n++) {
          ...
          if(savebuf){
            ...
            if(g.id){
              this.buffers[g.id] = {g,key,ss:savebuf};
            }
      ...
      let vmap = this.vmap;
      block = {sig, para, style, buffers, switches, vmap, title:this.title, label:this.label, parser:this};
      block.row1 = 0
      block.row2 = n;
  ...
  read_para (lines,n) {
    ...
    return {sig, hdgn, rank, id, caste: id, title, label, level, name, fname, style, buffers, switches, vmap, bodyrow, body, nblank, para, parser};//plitems is current not used
  }
  is_han (cc) {
    ///
