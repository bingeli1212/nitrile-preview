...
class NitrilePreviewParser extends NitrilePreviewBase {
  ...
  read_para (lines,n) {
    ...
    if(type == 'spcl'){
      ...
      var [style,islabeled,label,caption] = this.read_caption_ref(caption);
      var lines = [];
      ...
      sig = 'TEXT';
      body = items.map(p => `$ ${p.math}`);
      body.unshift('~~~{align:center}')
      body.push('~~~');
      return { para, sig, wide, caption, label, islabeled, style, floatname, body, hdgn, title, nspace, items };
    }
    else if(type == 'samp'){
  ...
  async read_mode_async() {
    ...
    for (let d of this.mode) {
      ...
      if (name === 'config') {
        ...
        let hdgn = subn;
        let style = {};
        this.blocks.push({sig,title,hdgn,label,refid,subseq,name,subf,subn,style});
