    ...
    if(lines.length){
      ...
      if(chapternum){
        ...
        if(Number.isFinite(chapternum)){
          this.num_chap = chapternum;
          this.style.chapternum = this.num_chap;
        }
      }
  ...
  read_para (lines,n) {
    ...
    for (let i=0; !done && n < lines.length; ++i,++n) {
      ...
      if(i==0 && (v=re_part.exec(line))!==null){
        ...
        var {label,title} = this.caption_to_label_and_title(caption);
        this.num_part++;
        this.style.partnum = this.num_part;
        continue;
      }
  ...
  async read_import_async(){
    ...
    this.imports.forEach((item,i,arr) => {
      ...
      if(name=='part'){
        ...
        block.style.name = name;
        block.style.partnum = this.num_part;
        this.blocks.push(block);
        ...
        if(subparser) {
          for( let block of subparser.blocks ) {
            ...
            block.style.src = src;
            block.style.partnum = this.num_part;
            block.style.chapternum = this.num_chap;
            this.blocks.push(block);
          }
        }
      }
