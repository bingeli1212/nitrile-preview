...
const re_is_capt = /^\.([A-Za-z]*)\s*(.*)$/;
const re_is_hrle = /^\*{3,}$/u;
const re_is_prim = /^\[\s+(.*?)\s+\]\s*(.*)$/;
const re_is_seco = /^\[\[\s+(.*?)\s+\]\]\s*(.*)$/;

const re_is_subcaption = /^\((\w*)\)\s*(.*)$/;

const re_is_frnt_fence  = /^---$/;
const re_is_frnt_entry  = /^([a-zA-Z][a-zA-Z\.\-]*)\s*:\s*(.*)$/;
...
class NitrilePreviewParser extends NitrilePreviewBase {
  ...
  read_lines(lines) {
    ...
    if(lines.length){
      var sig = '';
      var para = [];
      if(re_is_frnt_fence.test(lines[0].trim())){
        sig = 'FRNT';
        for(n=1; n < lines.length; n++){
          if(re_is_frnt_fence.test(lines[n].trim())){
            n++;
            break;
          }
          let line = lines[n];
          ...
          if(this.line_is_fullline(line)){
            let v;
            if((v=re_is_frnt_entry.exec(line))!==null){
              let key = v[1];
              let txt = v[2].trim();
              dd.push([key,txt]);
            }
        ...
        for (; n < lines.length; n++) {
          ...
          if(savebuf){
            ...
            p.line = line;
            this.imports.push(p);
          }else if((v=re_is_vmap.exec(s))!==null){
            //%~日本・にほん
            //%~日本/にほん
            savebuf = null;
            let rb = v[1];
        ...
        dd.push([key,[txt]]); //note that 'val' is added as a list
        while(n < lines.length && lines[n].trim().length==0){
          n++;
        }
        para = lines.slice(0, n);
      ...
      this.style.chapnum = this.chapnum;
      let style = {...this.style};
      block = {sig, para, style, title:this.title, label:this.label, parser:this};
      block.row1 = 0
      block.row2 = n;
      block.style.row = 0;
      block.style.label = this.label;
      block.style.name = this.name;
      ...
      if(sig=='CAPT'){
        ...
        if(block.style.split){
          ...
          while(total_n > 0){
            ...
            let splitid = this.int_to_letter_a(1+k);
            let splitstart = n1;
            ...
            if(k==1){
              this.label_map.set(block2.style.label,block2);
            }
          }
        }else{
          let bundles = this.ss_to_all_bundles(block.style,block.body,block.bodyrow);
          block.bundles = bundles;
          this.blocks.push(block);
          this.label_map.set(block.style.label,block);
        }
      }else{
        this.blocks.push(block);
        this.label_map.set(block.style.label,block);
  ...
  read_para (lines,n) {
    ...
    for (let i=0; !done && n < lines.length; ++i,++n) {
      ...
      if(type=='capt'){
        ...
        if(line.length==0){
          ...
          let v = re_is_subcaption.exec(line);
          if(v){
            let id = v[1];
            let text = v[2];
            subcaptions[id] = text;
    ...
    if(sig == 'NOTE'){
      ...
      style.sig = sig;
      style.lineno = n0;
      style.isbundle = this.is_bundle(body);
      style.issamp = this.is_samp(body);
      style.issand = this.is_sand(body);
      style.iscove = this.is_cove(body);
      style.iscave = this.is_cave(body);
      style.islave = this.is_lave(body);
      style.isstep = this.is_step(body);
      style.isplst = this.is_plst(body);
      if(style.isbundle){
        bundle = this.try_extract_bundle(style,body,0);
      }  
    } 
    else if(sig == 'HRLE'){
      style.sig = sig;
    ...
    style.hdgn = hdgn;
    let parser = this;
    return {sig, hdgn, rank, id, caste: id, title, label, level, fname, style, bodyrow, body, nblank, para, plitems, bundle, parser};
  }
  is_han (cc) {
    ///
  ...
  conf_to_float(in_key,def_val=0){
    if(this.blocks.length){
      let val = this.blocks[0].style['$'].data[in_key];
      if(val){
  ...
  async read_import_async(){
    ...
    this.imports.forEach((item,i,arr) => {
      ...
      if(name=='part'){
        ...
        chapnum++;
        if(subparser) {
          if(subparser.blocks.length){
            subparser.blocks[0].style.name = 'chapter';
          }
          subparser.blocks.forEach((block,i) => {
            if(i==0){
              block.style.name = 'chapter';
            }
