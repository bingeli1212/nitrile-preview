...
class NitrilePreviewParser extends NitrilePreviewBase {
  ...
  async read_lines_async(lines,r) {
    r = parseInt(r)||0;
    this.initialize();
    var nlines = 0;
    var row1 = 0;
    ...
    while (nlines < lines.length) {
      ...
      let blocks = await this.do_import_blocks(block,r-1);
      o = o.concat(blocks);
    }
  ...
  read_para (lines,n,hdgn) {
    ...
    if(lines.length && n0===0){
      if(re_frnt.test(lines[0].trim())){
        ...
        style.subseq = n0;
        return {para, sig, style, body, hdgn, title, nspace, items, data};
      }
    ...
    if(type == 'spcl'){
      ...
      style.subseq = n0;
      style.sig = sig;
      return { para, sig, style, body, hdgn, title, nspace, items};
    }
    else if(type == 'hdgs'){
      style.sig = sig;
      style.subseq = n0;
  ...
  async __read_mode_async() {
    ...
    for (var d of this.mode) {
      ...
      if (subf) {
        ...
        let fname = path.resolve(path.dirname(this.fname,subf));
    ...
    console.log('__read_mode_async completed','total blocks',this.blocks.length);
  }
  ...
  async read_file_async(fname,r) {
    ...
    await this.read_lines_async(lines,r);
  }
