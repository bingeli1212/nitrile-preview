...
class NitrilePreviewParser extends NitrilePreviewBase {
  ...
  constructor() {
    ...
    this.switches = {};
    this.vmap = [];
    this.style.parser = this;
    this.style.switches = this.switches;
    this.style.vmap = this.vmap;
    this.style.buffers = this.buffers;
  }
  /////////////////////////////////////////////////////////////////////
  /// read_line
  /////////////////////////////////////////////////////////////////////
  read_lines(lines) {
    ...
    if(lines.length){
      ...
      if(re_frnt.test(lines[0].trim())){
        ...
        for (; n < lines.length; n++) {
          ...
          if((v=re_import.exec(s))!==null){
            //%^import [chapter](./ch1.md)
            let text = v[1];
            this.imports.push(text);
            continue;
          }
          if((v=re_page.exec(s))!==null){
            //%^import [chapter](./ch1.md)
            let text = v[1];
            let num = parseInt(text);
            if(Number.isFinite(num)){
              this.num_page += num;
              this.pages.push(this.num_page);
            }
            continue;
          }
      ...
      sig = 'FRNT';  
      data = dd;
      block = {sig, style:this.style, para, data, parser:this};
      block.row1 = 0
      block.row2 = n;
    ...
    while (n < lines.length) {
      ...
      var sig  = block.sig;
      if(sig=='HDGS'){
        seq = 0;//only HDGS could be 0
      }else{
        ++seq;//all other blocks will start with 1
      }
  ...
  read_para (lines,n) {
    ...
    for (let i=0; !done && n < lines.length; ++i,++n) {
      ...
      if(i==0 && (v=re_hdgs.exec(line))!==null){
        ...
        bull = v[1];
        caption = v[2];
        var {label,title} = this.caption_to_label_and_title(caption);
        hdgn = bull.length;
        level = this.hdgn_to_level(hdgn);
        continue;
      }
      if(i==0 && (v=re_part.exec(line))!==null){
        ...
        if(name=='part'){
          this.num_part++;
          partnum = this.num_part;
        }else if(name=='chapter'){
          this.num_chapter++;
          chapternum = this.num_chapter;
          this.num_sect = 0;
          this.num_subs = 0;
          this.num_ssub = 0;
        }
      ...
      if(type=='capt'){
        if(line.length==0){
          ...
          if(isbody){
            ...
            let v = re_subcaption.exec(line);
            if(v){
              let seq = v[1];
              let text = v[2];
              subcaptions[seq] = text;
            }
          }else{
    ...
    let parser = this;
    return {sig, hdgn, rank, id, title, label, level, name, partnum, chapternum, fname, style, bodyrow, body, nblank, para, plitems, parser};
  }
  is_han (cc) {
    ///
  ...
  async read_file_async(fname) {
    ...
    var lines = out.split('\n');
    console.log('parser: '+fname+' number of lines: '+lines.length);
  ...
  hdgn_to_level(hdgn){
    ...
    if(hdgn==1){
      ...
      this.num_subs += 1;
      this.num_ssub = 0;
      level = `${this.num_sect}.${this.num_subs}`;
    }else{
      this.num_ssub + 1;
      level = `${this.num_sect}.${this.num_subs}.${this.num_ssub}`;
    }
    return level;
  }
  ///
  ...
  async read_import_async(){
    ...
    this.imports.forEach((line) => {
      if((v=re_file.exec(line))!==null){
        ...
        if(1){
          subparser.buffer = {...this.buffer};
          subparser.switches = {...this.switches};
          subparser.vmap = [].concat(this.vmap);
          subparser.style = {};
          subparser.style.switches = subparser.switches;
          subparser.style.vmap = subparser.vmap;
          subparser.style.buffers = subparser.buffers;
        }
        let promise = subparser.read_file_async(fullfname);  
        items.push({name,title,fname,subparser,fullfname,promise})
        return;
      }
    ...
    items.forEach((item,i,arr) => {
      ...
      if(name=='part'){
        this.num_part++;
        let partnum = this.num_part;
        let chapternum = this.num_chapter;
        let style = {...this.style};
        let parser = null;
        this.blocks.push({sig, hdgn, rank, id, title, label, partnum, chapternum, level, name, fname, style, body, nblank, para, plitems, parser});  
      }else if(name=='chapter'){
        this.num_chapter++;
        if(subparser) {
          let block = subparser.blocks[0];
          if(block && block.sig=='FRNT'){
            title = subparser.conf_to_string('title','NO TITLE');
            label = subparser.conf_to_string('label','');
            block.title = title;
            block.label = label;
            block.sig = 'HDGS';
            block.hdgn = 0;
        ...
        // this.blocks.push({sig, hdgn, rank, id, title, label, partnum, chapternum, level, name, fname, style:this.style, body, nblank, para, plitems});
        for( let block of subparser.blocks ) {
          let partnum = this.num_part;
          let chapternum = this.num_chapter;
          block.name = name;
          block.partnum = partnum;
          block.chapternum = chapternum;
          this.blocks.push(block);
        }
      }
    });
  ...
  caption_to_label_and_title(caption){
    var label = '';
    var title = caption.replace(/&label\{(\S+)\}/g,(match,p1) => {
      label = p1;
      return '';
    });
    return {label,title};
