...
class NitrilePreviewParser extends NitrilePreviewBase {
  ...
  read_lines(lines) {
    ...
    if(lines.length){
      ...
      sig = 'FRNT';  
      data = dd;
      block = {sig, style:this.style, para, data};
      block.row1 = 0
      block.row2 = n;
  ...
  read_para (lines,n) {
    ...
    for (let i=0; !done && n < lines.length; ++i,++n) {
      ...
      if(i==0 && (v=re_cove.exec(line))!==null){
        type = 'cove';
        body.push(line);///make sure to pass in the complete line, without any prefix empty spaces, because later one the extra_bundle() replies on this information to remove empty lines of a bundle body
        continue;  
      }
      ...
      if(type=='plst'){
        ...
        ///this is the 2nd line or later for a cluster, 
      ...
      if(type=='capt'){
        ...
        ///line does not conform to the capt, we quit
        if(line.length==0){
          is_capt = 0;
        }
        if(is_capt){
          ...
          continue;
        }else if(this.line_is_fullline(line)){
          break;
        }else{
          body.push(line);
          continue;
    ...
    if(sig == 'NOTE'){
      ...
      if(id && caption){
        const re_style = /^(\{.*?\})\s*(.*)$/;
        const re_label = /^&label\{(\S+)\}\s*(.*)$/;
        let v;
        if((v=re_style.exec(caption))!==null){
          style = this.string_to_style(v[1],style);
          caption = v[2];
        }
        if((v=re_label.exec(caption))!==null){
          label = v[1];
          caption = v[2];
        }
        title = caption;
    ...
    return {sig, hdgn, rank, id, title, label, level, name, partnum, chapternum, fname, style, body, nblank, para, plitems};
  }
  is_han (cc) {
    ///
