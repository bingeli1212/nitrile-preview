...
class NitrilePreviewNode extends NitrilePreviewBase {
  ...
  async to_translator_document_async (filename,all_program) {

    if(path.extname(filename)!=='.md'){
      throw "File does not end with .md"
    }
    console.log('all_program=',all_program);
    const parser = new NitrilePreviewParser();
    await parser.read_file_async(filename)
    await parser.read_master_async();
    await parser.read_import_async();
    var translator = parser.conf_to_string('translator');
    var programs = this.string_to_style(`{${all_program}}`,{});
    console.log('programs=',programs);
    var program = programs[translator]||''; 
    await this.run_translator(filename,parser,translator,program,false);
  }
  ...
  async to_beamer_document_async (filename,program) {
    ...
    await parser.read_master_async();
    await parser.read_import_async();
    await this.run_translator(filename,parser,'beamer',program,false);
  }
  ...
  async run_translator (filename,parser,trans,program,viewflag) {
