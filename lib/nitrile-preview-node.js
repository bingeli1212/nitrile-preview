...
class NitrilePreviewNode extends NitrilePreviewBase {
  ...
  async toHtmlDoc(fname) {
    ...
    var peek = parser.conf_to_string('peek');
    console.log(peek);
    switch(peek){
      case 'flowerpot': {
...
setInterval(function () {
  //console.log('trying to fetch','window.location.pathname',window.location.pathname.toString());
  //console.log('trying to fetch','window.document',window.document.toString());
...
`;
  ...
  async to_xhtml_doc_async (fname) {
    ...
    await parser.read_file_async(fname)    
    await parser.read_import_async();
    var xhtml = parser.conf_to_string('xhtml');
  ...
  async to_contex_doc_async (fname) {
    ...
    await parser.read_file_async(fname)
    await parser.read_import_async();
    var contex = parser.conf_to_string('contex');
  ...
  async to_latex_doc_async (fname) {
    ...
    await parser.read_file_async(fname)
    await parser.read_import_async();
    var latex = parser.conf_to_string('latex');
    var texfile = this.md_to_tex_file(fname);
    switch(latex){
      case 'beamer': {
