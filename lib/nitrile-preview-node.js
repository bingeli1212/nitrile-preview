...
class NitrilePreviewNode extends NitrilePreviewBase {
  ...
  async to_translator_document_async (fname) {
    ...
    var tname = parser.conf_to_string('translator');
    var pname = parser.conf_to_string('program');
    console.log('tname=',tname);
    console.log('pname=',pname);
    await this.run_translator(fname,parser,tname,pname,false);
  }
  ...
  async to_beamer_document_async (filename) {
    ...
    await parser.read_master_async();
    await parser.read_import_async();
    var pname = parser.conf_to_string('program','pdflatex');
    await this.run_translator(filename,parser,'beamer',pname,false);
  }
  ...
  async to_article_document_async (filename) {
    ...
    await parser.read_import_async();
    var pname = parser.conf_to_string('program','pdflatex');
    await this.run_translator(filename,parser,'article','pdflatex',false);
  }
  ...
  async to_camper_document_async (filename) {
    ...
    await parser.read_master_async();
    await parser.read_import_async();
    var pname = parser.conf_to_string('program','camper');
    await this.run_translator(filename,parser,'camper',pname,false);
  }
  ...
  async to_creamer_document_async (filename) {
    ...
    await parser.read_master_async();
    await parser.read_import_async();
    var pname = parser.conf_to_string('program','creamer');
    await this.run_translator(filename,parser,'creamer',pname,false);
  }
  ...
  async run_translator (mdfile,parser,tname,pname,viewflag) {
    ...
    switch(tname){
      ...
      case 'beamer': {
        ...
        console.log(dname); 
        console.log(fname); 
        const exe = require('child_process').spawn(pname||'pdflatex',
