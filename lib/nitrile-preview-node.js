...
const { NitrilePreviewCleamer } = require('./nitrile-preview-cleamer');
...
class NitrilePreviewNode extends NitrilePreviewBase {
  ...
  async to_pango_document_async (filename) {
    ...
    await parser.read_master_async();
    await parser.read_import_async();
    var myfile = await this.run_translator(filename,parser,'pango','',false);
  }

  async to_beamer_document_async (filename,program) {
    ...
    await parser.read_master_async();
    await parser.read_import_async();
    var myfile = await this.run_translator(filename,parser,'beamer',program,false);
  }

  async to_camper_document_async (filename) {
    ...
    await parser.read_master_async();
    await parser.read_import_async();
    var myfile = await this.run_translator(filename,parser,'camper','',false);
  }

  async to_cleamer_document_async (filename) {
    ...
    await parser.read_master_async();
    await parser.read_import_async();
    var myfile = await this.run_translator(filename,parser,'cleamer','',false);
  }
  ...
  async to_article_document_async (filename) {
    ...
    await parser.read_master_async();
    await parser.read_import_async();
    var myfile = await this.run_translator(filename,parser,'article','pdflatex',false);
  }
  ...
  async to_epub_document_async (filename) {
    ...
    await parser.read_master_async();
    await parser.read_import_async();
    var myfile = await this.run_translator(filename,parser,'epub','',false);
  }
  ...
  async run_translator (filename,parser,one,program,viewflag) {
    ...
    switch(one){
      ...
      case 'camper': {
        ...
        let dname = path.dirname(texfile);
        let fname = path.basename(texfile);
        let program = path.resolve(dname,'./camper.sh');
        console.log(program); 
        console.log(dname); 
        console.log(fname); 
      ...
      case 'cleamer': {
        ...
        let dname = path.dirname(texfile);
        let fname = path.basename(texfile);
        let program = path.resolve(dname,'./cleamer.sh');
        console.log(program); 
        console.log(dname); 
        console.log(fname); 
        const exe = require('child_process').spawn(program,
