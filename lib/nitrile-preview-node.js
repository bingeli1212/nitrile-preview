...
class NitrilePreviewNode extends NitrilePreviewBase {
  ...
  async to_beamer_document_async (filename) {
    ...
    await parser.read_master_async();
    await parser.read_import_async();
    var myfile = await this.run_translator(filename,parser,'beamer','pdflatex',false);
  }
  ...
  async to_doc_symbols_async(type){
    const parser = new NitrilePreviewParser();
    const translator = new NitrilePreviewPage(parser);
    const name_symbol_map = translator.tokenizer.name_symbol_map;
    ...
    for(let iter of symbol_group_map){
      ...
      all.push(`  ${group}`);
      all.push('');
      all.push('  | symbol | name | html ');
      all.push('  ---------------------------');
      for(let name of names){
        let {html} = name_symbol_map.get(name);
        all.push(`  | &${name}; | ${name} | ${html}`);
      }
      all.push('');
    }
    var data = all.join('\n')
