...
class NitrilePreviewNode {
  ...
  async to_program_document_async (filename) {
    ...
    const one = parser.conf_to_string('translator'); 
    const program = parser.conf_to_string('program'); 
    ...
    switch(one){
      case 'beamer': {
        ...
        exe.on('close',(code) => { 
          ...
          const exe = require('child_process').spawn('open',[pdffile])
        }); 
        break;
      }
      case 'slide': {
        ...
        let dname = path.dirname(htmlfile);
        let fname = path.basename(htmlfile);
        console.log(dname); 
        console.log(fname); 
        const exe = require('child_process').spawn('open',[htmlfile]);
        break;        
      }
    }
  }
  ...
  async to_article_document_async (filename, program) {
    if(path.extname(filename)!=='.md'){
      throw "File does not end with .md"
    }
    /// parser blocks
  ...
  async to_report_document_async (filename, program) {
    if(path.extname(filename)!=='.md'){
      throw "File does not end with .md"
    }
    /// parser blocks
  ...
  ///
  ///Following are CONTEX related documents
  ///
  ...
  async to_camerile_document_async (filename) {
    if(path.extname(filename)!=='.md'){
      throw "File does not end with .md"
    }
    const parser = new NitrilePreviewParser();
  ...
  ///
  ///Following are HTML related documents
  ///
