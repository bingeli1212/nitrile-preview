...
class NitrilePreviewNode extends NitrilePreviewBase {
  ...
  async to_xhtml_doc_async (filename) {
    if(path.extname(filename)==='.xhtml'){
      const data = utils.read_text_file_async(filename);
      return data;
    }
    ...
    if(parser.conf_to_string('root')){
      filename = parser.conf_to_string('root');
      parser = new NitrilePreviewParser();
      await parser.read_file_async(filename);
      await parser.read_import_async();
    }
    var doc = parser.conf_to_string('doc');
    var xhtmlfile = this.md_to_xhtml_file(filename);
    switch(doc){
      case 'slide': {
        const translator = new NitrilePreviewSlide(parser);
        const data = translator.to_document();
  ...
  async to_contex_doc_async (filename) {
    ...
    var doc = parser.conf_to_string('doc');
    var texfile = this.md_to_tex_file(filename);
    switch(doc){
      case 'slide': {
  ...
  async to_latex_doc_async (filename) {
    if(path.extname(filename)==='.tex'){
      const data = utils.read_text_file_async(filename);
      return data;
    }
  ...
  async to_epub_doc_async (filename) {
    ...
    let epubfname = this.md_to_epub_file(filename);
    return [zipdata,epubfname];
  }

  async to_slide_doc_async (filename) {
    if(path.extname(filename)=='.xhtml'){
      const data = utils.read_text_file_async(filename);
      return [data,filename];
    }
    ...
    var doc = parser.conf_to_string('doc');
    var outfname = this.md_to_xhtml_file(filename);
    const translator = new NitrilePreviewSlide(parser);
    const data = translator.to_document();
  ...
  async to_memoir_doc_async (fname) {
    ...
    await parser.read_import_async();
    var pname = 'pdflatex';
    await this.run_translator(fname,parser,'memoir',pname,false);
  }
  ...
  async to_article_doc_async (filename) {
