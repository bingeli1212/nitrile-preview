...
class NitrilePreviewDiagramCanvas extends NitrilePreviewDiagram {
  ...
  to_canvas(style,ss) {
    ...
    this.init_internals();
    this.config = {...style};
    var [viewport_width,viewport_height,viewport_unit,viewport_grid] = this.translator.g_to_viewport_array(style);
    ...
    if(1){
      ...
      let s_svg = `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 ${vw} ${vh}">
                   ...
                   <polyline points="0 0 ${gw} 0 ${gw} ${gw} 0 ${gw}" fill="none" stroke="lightgray"/></pattern>
                   <rect x="0" y="0" width="${vw}" height="${vh}" fill="url(#id)" stroke="lightgray"/></svg>`;
