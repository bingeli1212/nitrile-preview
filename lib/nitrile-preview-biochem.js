...
\\setuphead[chapter][command=\\BookchapterCommand,page=chapter,numbercolor=red,alternative=inmargin,number=no]
...
\\define[2]\\SectionCommand%
  {\\framed
     ...
     {\\hbox to.011\\textwidth{}\\hbox to.05\\textwidth{\\strut#1}\\vtop{\\hsize\\dimexpr\\hsize-.061\\textwidth-1em\\relax\\begstrut#2
\\endstrut}}}
\\setuphead[section][command=\\SectionCommand,numbercolor=red,number=no]
...
\\setupcaption[table][style={\\ssx\\setupinterlinespace[line=2.5ex]},align=flushleft,number=no]
...
\\setupcaption[figure][style={\\ssx\\setupinterlinespace[line=2.5ex]},align=flushleft,number=no]
    ...
    o.forEach((pp) => {
      ...
      pp.forEach((p) => {
        ...
        if(block.sig=='PART'){
          ...
          all.push(`~~~~${chapnum} ~ ${title}\\hfill\\at[${label}]`)
        }
      });
      all.push(`\\stoplines`);
      all.push(`\\vfill`);
