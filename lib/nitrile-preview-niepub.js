  ...
  async to_document_async () {
    ...
    for (let block of this.parser.blocks) {
      if(block.style.name=='part'){
        pp = [];
        pp.name='part';
        pp.title=this.smooth(this.style,block.title);
        every.push(pp);
        pp.push({block}); 
      }else if(block.style.name=='chapter'){
        pp = [];
        pp.name='chapter';
        pp.title=this.smooth(this.style,block.title);
        every.push(pp);
    ...
    every.forEach((pp,i) => {
      ...
      pp.forEach(p => {
        p.block.style.saveas = pp.href;
      });
    });
    ///
    /// only call building of idnum_map after saveas has been assigned to each block
    ///
