  ...
  toInnerSvg (v, compact=false) {
    ...
    if (v === undefined || v === null) {
      ...
      switch (v[0]) {
        ...
        case '\\leftright':
          ...
          if (v[0] === '\\pipe') {
            ...
            var ss = results.map( x => x.s );
            var s = ss.join(' ');
            if(0){///context way of typesetting 'displaymath'
      ...
      if(id){
        ...
        ///replaced by \minus{}
        if(this.translator.unicode_symbol_map.has(v)){
          var s = this.translator.unicode_symbol_map.get(v).cmath;
          return {s};
        }
