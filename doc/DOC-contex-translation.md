...
# The "\starttable" inside a figure

When "\starttable" is to appear inside a "\placefigure", for each row
the text simply appears more closer to the top "\HL" than they normally
would when they are not placed inside a "\placefigure". Simply put, the
row height of the table becomes smaller when this table is placed
inside a figure.

# The "\FR" for next row

For a "\starttable", the "\FR" command is used to generate a new row.
THe documentation says that it this command should be used for a row 
that is at the top of the table, such that it will adjust the top space
appropriately. However, the experiment has shown that the vertical
paddings of each table row seems to look just fine. 

In addition, this 
the vertical/horizontal padding settings 
for a HTML translation has been specifically adjusted so that
the output of the same table should have matched
roughly to the same height of the table
that is generated by CONTEX when the table
row is typeset by "\FR".

It should be pointed out that the "rscale=0.96" setting of the font, the current choice
of the font family (dajavu) and the font size (10pt), and the "ss" choice,
the "s2" placed in front of the alignment parameters
for the "\starttable", and the "\FR" all have to play together 
...
- Sometimes parentheses are seem going on top of the characters 
  before it
- The "tabulate" environement seems to have extra vertical spaces 
  between rows   
- Cannot change background colors for table cells of start/stop/table 
- For "details" block type, it is not possible to generate 
  leading solidified blank spaces for lines
- For nislide.js, the text will sometimes show on top of figures
  that are "wrapfig".
- Not be able to find a way to place subtitles underneath a 
  start/stop/table generated by a "tab" bundle. 
- Not able to center-align an IMG within a "tab"
- if two "\bTABLE" are placed next to each other side-by-side,    
  there isn't any visible white space insert between them and the two
  tables are closely touch each other
